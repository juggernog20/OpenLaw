...
const stamp =
  state.stamp ?? (state.stamp = new Date().toISOString().slice(0, 16).replace(/[-:T]/g, ""));
keepState();
...
async function step(
  ...
  console.log(
    `${entry.id} [${role}/${method}] ${entry.result.toUpperCase()} ${action}${entry.result === "fail" ? `\n   ${entry.actual}` : ""}`,
  );
  save();
...
async function openCard(page, name) {
  ...
  await until(
    async () => (await b.getAttribute("aria-expanded")) === "true",
    `${name} did not open`,
  );
}
...
async function loadModels(page, entry, label) {
  ...
  const t0 = Date.now();
  const resP = page.waitForResponse((r) => r.url().endsWith("/api/v1/ai-connector/models"), {
    timeout: 90000,
  });
  await btn.click();
  ...
  await until(
    async () => !(await page.getByRole("button", { name: "Loading models…" }).count()),
    "listing still loading",
    30000,
  );
  return { res, body, ms };
}
...
const envNames = (svc) =>
  JSON.parse(dockerOut("inspect", "--format", "{{json .Config.Env}}", container(svc))).map(
    (e) => e.split("=")[0],
  );
const envHash = (svc, name) => {
  const hit = JSON.parse(
    dockerOut("inspect", "--format", "{{json .Config.Env}}", container(svc)),
  ).find((e) => e.startsWith(`${name}=`));
  return hit ? sha(hit.slice(name.length + 1)) : null;
};
const logsOf = (svc, since) =>
  dc("logs", "--no-color", "--no-log-prefix", ...(since ? ["--since", since] : []), svc);
const psql = (sql) =>
  execFileSync(
    "docker",
    [
...
log.identities = [
  ...
  {
    role: "legal_team_member (negative check only)",
    person: "Nadia Haddad",
    entry: "password sign-in",
  },
  {
    role: "operator",
    person: "the agent with docker compose -p " + PROJECT,
    entry: "container tools on the owned lab; signed-out formal reader",
  },
];
...
if (run("A")) {
  await step(
    ...
    async () => {
      ...
      const labels = (await page.locator("label[for^='answer-style-']").allInnerTexts()).map((t) =>
        t.trim(),
      );
      const checked = await page
        .locator("input[name='answer-style']:checked")
        .getAttribute("value");
      const connect = await page
        .getByText("Connect an AI provider to choose an answer style.", { exact: true })
        .count();
      must(
        Object.values(exp).every((v) => v === "false"),
        `cards ${q(exp)}`,
      );
      must(c === "Not connected" && conv === 0, `chip ${c} conv ${conv}`);
      must(
        disabled.length === 3 && disabled.every(Boolean) && connect === 1 && checked === "sentence",
        `radios ${q(disabled)} connect ${connect} checked ${checked}`,
      );
  ...
  await step(
    ...
    "administrator",
    "browser-walkthrough",
    "/settings/ai-analysis Provider card",
    ...
    async () => {
      ...
      for (const [value, label] of [
        ...
        seen[label] = {
          baseUrl: baseLabel,
          protocol: (await page.locator("#ai-protocol").count())
            ? await options(page, "#ai-protocol")
            : null,
      ...
      must(
        seen["Azure OpenAI"].baseUrl === "Deployment endpoint" &&
          seen["Azure OpenAI"].load === 0 &&
          seen["Azure OpenAI"].modelRole === null,
        `azure ${q(seen["Azure OpenAI"])}`,
      );
      must(
        q(seen["Custom endpoint"].protocol) ===
          q(["Anthropic Messages", "OpenAI-compatible chat completions", "Gemini"]) &&
          seen["Custom endpoint"].baseUrl === "Base URL",
        `custom ${q(seen["Custom endpoint"])}`,
      );
      must(
        seen.Ollama.baseUrl === null && seen.Ollama.keyRequired === false,
        `ollama ${q(seen.Ollama)}`,
      );
      return `Provider lists ${q(presets)}. Per preset: ${Object.entries(seen)
        .map(
          ([k, v]) =>
            `${k}: endpoint label ${q(v.baseUrl)}, Protocol ${q(v.protocol)}, Load models ${v.load}, Enter model ID manually ${v.manualToggle}, Model ${v.modelRole === "combobox" ? "combobox" : "plain text box"} prefilled ${q(v.model)}, key required ${v.keyRequired}`,
        )
        .join("; ")}.`;
    },
  );
  ...
  await step(
    ...
    async () => {
      ...
      must(
        label === "Output token limit per API call" &&
          attrs.min === "1024" &&
          attrs.max === "262144" &&
          attrs.value === "32768" &&
          attrs.exact === "32768",
...
if (run("B")) {
  await step(
    ...
    async (e) => {
      ...
      must(
        res.status() >= 400 && /^The provider refused model discovery with HTTP \d+\.$/.test(err),
        `status ${res.status()} shown ${q(err)} body ${q(body)}`,
      );
  ...
  await step(
    ...
    "operator",
    "browser-walkthrough",
    "/settings/ai-analysis Provider card (Ollama)",
    ...
    async (e) => {
      ...
      must(
        baseUrl === 0 && res.status() >= 400 && err,
        `baseUrl ${baseUrl} status ${res.status()} err ${err}`,
      );
  ...
  await step(
    ...
    async (e) => {
      ...
      await sleep(300);
      const narrowed = (
        await page.getByRole("listbox", { name: "Models" }).getByRole("option").allInnerTexts()
      ).map((t) => t.trim());
      ...
      e.listing = {
        ...
        selectedListed: body.models.some((m) => m.id === MODEL),
      };
      must(
        prefilled === "~openai/gpt-latest" && !loadBefore && loadWithKey,
        `prefilled ${prefilled} load ${loadBefore}/${loadWithKey}`,
      );
      must(
        narrowed.length >= 1 && narrowed.every((t) => /gpt-oss-120b/.test(t)),
        `narrowed ${q(narrowed)}`,
      );
      must(
        afterBlur === prefilled && chosen === MODEL && afterRefresh === MODEL,
        `blur ${afterBlur} chosen ${chosen} refresh ${afterRefresh}`,
      );
      must(
        manualRole === null && back === 1 && listRole === "combobox",
        `manual ${manualRole} ${back} ${listRole}`,
      );
      must(!after.configured && after.savedKeys.length === 0, `saved ${q(after)}`);
...
if (run("C")) {
  await step(
    ...
    async (e) => {
      ...
      await openCard(page, "Answer style");
      const radiosEnabled = await page
        .getByRole("radio")
        .evaluateAll((els) => els.every((x) => !x.matches(":disabled")));
      e.connector = st;
      must(
        keyBox === "" &&
          c === "Connected" &&
          pill === "Key in use" &&
          forget === 0 &&
          presetLabel === "OpenRouter (key saved)",
        `key ${keyBox.length} chip ${c} pill ${pill} forget ${forget} ${presetLabel}`,
      );
      must(
        conv === 1 && Object.values(sw).every((v) => v === "false") && useSwitch === "true",
        `conv ${conv} ${q(sw)} use ${useSwitch}`,
      );
      must(test.status === 200 && test.shown === "Connection successful.", `test ${q(test)}`);
      must(
        st.enabled && st.model === MODEL && st.preset === "openrouter" && radiosEnabled,
        `state ${q(st)} radios ${radiosEnabled}`,
      );
  ...
  await step(
    ...
    async (e) => {
      ...
      const st = safe(await connectorOf(admin));
      must(
        saved.status === 200 && st.model === BAD_MODEL && st.savedKeys.length === 1,
        `save ${saved.status} ${q(st)}`,
      );
      must(
        test.status >= 400 && /^The connection test failed\. /.test(test.shown),
        `test ${q(test)}`,
      );
  ...
  await step(
    ...
    async (e) => {
      ...
      const st = safe(await connectorOf(admin));
      must(
        saved.status === 200 && st.model === MODEL && st.savedKeys.length === 1,
        `save ${saved.status} ${q(st)}`,
      );
      must(test.status === 200 && test.shown === "Connection successful.", `test ${q(test)}`);
  ...
  await step(
    ...
    async (e) => {
      ...
      must(
        pillBefore === null && saved.status === 200 && pill === "Key in use",
        `pill ${pillBefore}/${pill} save ${saved.status}`,
      );
      must(
        test.status >= 400 && /^The connection test failed\. /.test(test.shown),
        `test ${q(test)}`,
    ...
    async (e) => {
      ...
      must(
        pillBack === "Key saved" &&
          forgetBack === 1 &&
          saved.status === 200 &&
          pillInUse === "Key in use",
        `back ${pillBack} ${forgetBack} save ${saved.status} ${pillInUse}`,
      );
      must(
        pillCustom === "Key saved" && forgetCustom === 1 && pillGone === null,
        `custom ${pillCustom} ${forgetCustom} gone ${pillGone}`,
      );
      must(
        refusal.status === 409 &&
          st.savedKeys.length === 1 &&
          st.savedKeys[0].preset === "openrouter" &&
          st.preset === "openrouter" &&
          presetAfter === "openrouter",
        `refusal ${refusal.status} ${q(st)}`,
      );
...
if (run("P")) {
  await step(
    ...
    async () => {
      ...
      await openCard(page, "Answer style");
      const otherOpen = await cardButton(page, "Contract analysis prompts").getAttribute(
        "aria-expanded",
      );
      const put = page.waitForResponse(
        (r) => r.url().endsWith("/api/v1/ai-connector") && r.request().method() === "PATCH",
      );
      ...
      const descs = (await region.locator("p[id^='answer-style-']").allInnerTexts()).map((t) =>
        t.trim(),
      );
      ...
      const convRows = await conv
        .locator("textarea")
        .evaluateAll((els) => els.map((e) => e.getAttribute("aria-label")));
      await openCard(page, "Contract analysis prompts");
      const ana = page.getByRole("region", { name: "Contract analysis prompts" });
      const anaRows = await ana
        .locator("textarea")
        .evaluateAll((els) => els.map((e) => e.getAttribute("aria-label")));
      const link = ana.getByRole("link", { name: "Contracts → Fields" });
      const href = await link.getAttribute("href");
      must(
        otherOpen === "false" && res.status() === 200 && st1 === "few_words" && st2 === "sentence",
        `open ${otherOpen} patch ${res.status()} ${st1}/${st2}`,
      );
      must(
        convRows.length === 5 &&
          /\{module\}/.test(convHint) &&
          anaRows.length === 7 &&
          href === "/settings/contracts/fields",
        `conv ${q(convRows)} ana ${q(anaRows)} ${href}`,
      );
  ...
  await step(
    ...
    "administrator",
    "browser-walkthrough",
    "/settings/ai-analysis Contract analysis prompts",
    ...
    async () => {
      ...
      const pre = (await admin.api("GET", "/ai-field-prompts")).json.prompts.find(
        (p) => p.slug === NOTICE,
      );
      if (pre.overridden) {
      ...
      const put = page.waitForResponse(
        (r) => r.url().endsWith("/api/v1/ai-field-prompts") && r.request().method() === "PUT",
      );
      await box.click();
      ...
      const l = (r) =>
        r.method() === "PUT" &&
        r.url().includes("/api/v1/ai-field-prompts") &&
        puts.push(r.postData());
      page.on("request", l);
      await box.click();
      await box.press("End");
      ...
      page.off("request", l);
      const stored = (await admin.api("GET", "/ai-field-prompts")).json.prompts.find(
        (p) => p.slug === NOTICE,
      );
      const maxlength = await box.getAttribute("maxlength");
      must(
        res.status() === 200 && stored.overridden && stored.prompt === NOTICE_TEXT,
        `save ${res.status()} ${q(stored)}`,
      );
      must(
        withBreak && afterEscape === NOTICE_TEXT && afterClear === NOTICE_TEXT && puts.length === 0,
        `break ${withBreak} esc ${q(afterEscape)} clear ${q(afterClear)} puts ${puts.length}`,
      );
...
if (run("D")) {
  await step(
    ...
    async () => {
      ...
      const add = async (name, type, prompt, style) => {
        ...
        if (await sel.count()) {
          await until(
            async () => /\(/.test(await sel.locator("option").first().innerText()),
            "default style not shown",
            8000,
          ).catch(() => {});
          seen.styleOptions = await sel
            .locator("option")
            .evaluateAll((els) =>
              els.map((o) => `${o.textContent.trim()}${o.disabled ? " [disabled]" : ""}`),
            );
        }
      ...
      const loc = await add(
        F.location,
        "Long text",
        "State where the supplier must store the customer's data and when it must delete that data.",
        null,
      );
      ...
      must(
        loc.aiPrompt === 1 &&
          loc.styleOptions?.[0] === "Organisation default (1-2 sentence summary)" &&
          !loc.styleOptions.some((o) => o.includes("[disabled]")),
        `long ${q(loc)}`,
      );
      must(
        sup.aiPrompt === 1 && sup.styleOptions?.includes("Full clause text [disabled]"),
        `text ${q(sup)}`,
      );
  ...
  await step(
    ...
    "administrator",
    "browser-walkthrough",
    "/settings/contracts/types/<Vendor>/form",
...
if (run("T")) {
  await step(
    ...
    async () => {
      ...
      await cardButton(page, "Contract analysis prompts").waitFor({ timeout: 20000 });
      await openCard(page, "Contract analysis prompts");
      const before = (await admin.api("GET", "/ai-field-prompts")).json.prompts
...
const runFacts = (r) => ({
  ...
  providerMs:
    r.startedAt && r.finishedAt ? Date.parse(r.finishedAt) - Date.parse(r.startedAt) : null,
...
if (run("E")) {
  await step(
    ...
    async (e) => {
      if (!state.contract) {
        const title = `${G} ${stamp}`;
        const r = await admin.api("POST", "/contracts", {
          title,
          contractTypeId: typeId("Vendor"),
          customFields: {},
      ...
      if (!state.uploadedAt) {
        ...
        const up = await page.request.post(`${BASE}/api/v1/contracts/${n}/documents`, {
          ...
          multipart: {
            kind: "draft_ours",
            file: {
              name: "doc030-live-analysis.docx",
      ...
      const r = await until(
        ...
        "no finished Analysis run within 6 minutes",
        360000,
        3000,
      );
      e.run = runFacts(r);
      e.run.uploadToFinishMs = Date.now() - state.uploadedAt;
      log.providerCalls.push({
        at: new Date().toISOString(),
        kind: "analysis run",
        label: "automatic",
        ...e.run,
      ...
      const sparkles = await page
        .getByRole("button", { name: /^View AI evidence for / })
        .evaluateAll((els) => els.map((x) => x.getAttribute("aria-label")));
      ...
      state.slugs = {
        location: slugOf(F.location),
        supplier: slugOf(F.supplier),
        unattached: slugOf(F.unattached),
      };
      keepState();
      const lists = r.outcome ?? {};
      const where = (slug) =>
        ["written", "kept", "unsupported", "invalid"].filter((k) =>
          (lists[k] ?? []).some((x) => x === slug || x.endsWith(slug)),
        );
      e.customFieldOutcome = Object.fromEntries(
        Object.entries(state.slugs).map(([k, v]) => [k, { slug: v, in: where(v) }]),
      );
      ...
      e.savedValues = {
        ...
        custom: Object.fromEntries(
          Object.entries(state.slugs).map(([k, v]) => [k, det.contract.customFields?.[v] ?? null]),
        ),
      ...
      must(
        unverified > 0 && sparkles.length > 0 && unSlug.length === 0,
        `unverified ${unverified} sparkles ${sparkles.length} unattached ${q(unSlug)}`,
      );
  ...
  await step(
    ...
    "administrator",
    "browser-walkthrough",
    "/contracts/<n> Overview, Fields, History",
    ...
    async (e) => {
      ...
      await page.waitForLoadState("networkidle").catch(() => {});
      const sparkle = page
        .getByRole("button", { name: /^View AI evidence for Notice period/ })
        .first();
      const aiCalls = [];
      const l = (r) =>
        /\/api\/v1\/(ai-connector|contracts\/\d+\/analysis$)/.test(r.url()) &&
        r.method() !== "GET" &&
        aiCalls.push(r.url());
      page.on("request", l);
      let panel = "";
      ...
      const modelShown = Object.entries({ ...texts, history: historyText })
        .filter(([, t]) => t.includes(MODEL))
        .map(([k]) => k);
      const analysisEntry = (historyText.match(
        /[^.]*completed an AI analysis of this contract[^.]*/,
      ) ?? [null])[0];
      e.modelShownOn = modelShown;
      must(
        marks.length > 0 && aiCalls.length === 0,
        `evidence marks ${marks.length} aiCalls ${aiCalls.length}`,
      );
  ...
  await step(
    ...
    "administrator",
    "live-provider-check",
    "/settings/ai-analysis and /contracts/<n>/fields",
    ...
    async (e) => {
      ...
      const pick = (c) =>
        JSON.stringify(
          Object.fromEntries(
            Object.entries(state.slugs ?? {}).map(([k, v]) => [k, c.customFields?.[v] ?? null]),
          ),
        );
      ...
      const r = await until(
        ...
        "manual run did not finish in 6 minutes",
        360000,
        3000,
      );
      e.run = runFacts(r);
      log.providerCalls.push({
        at: new Date().toISOString(),
        kind: "analysis run",
        label: "manual after Answer style change",
        ...e.run,
      });
      ...
      const p2 = page.waitForResponse(
        (x) => x.url().endsWith("/api/v1/ai-connector") && x.request().method() === "PATCH",
      );
      await page.getByRole("radio", { name: "1-2 sentence summary" }).click();
      await p2;
      must(
        def === "Organisation default (Few word summary)" && beforeCustom === run1Custom,
        `editor default ${def}; before ${beforeCustom} run1 ${run1Custom}`,
      );
      must(
        r.state === "ready" && r.trigger === "manual" && r.model === MODEL && running,
        `run ${q(e.run)} running ${running}`,
      );
...
if (run("F")) {
  await step(
    ...
    async () => {
      ...
      for (const id of [
        ...
        sws.push(await page.locator(`#${id}`).isDisabled());
      const saved = await saveConnector(page);
      const stillOff = safe(await connectorOf(admin));
      await openCard(page, "Answer style");
      const styleUsable = await page
        .getByRole("radio")
        .evaluateAll((els) => els.every((x) => !x.matches(":disabled")));
      const pill = await keyPill(page);
      await fieldsPage(n);
      ...
      must(
        res.status() === 200 && testDisabled && sws.every(Boolean),
        `disable ${res.status()} test ${testDisabled} ${sws}`,
      );
      must(
        saved.status === 200 && !stillOff.enabled && stillOff.hasApiKey && pill === "Key in use",
        `save ${saved.status} ${q(stillOff)} pill ${pill}`,
      );
      must(
        runBtn === 0 &&
          unverified > 0 &&
          refusal.status === 409 &&
          refusal.json?.detail === "No enabled AI connector is configured.",
        `run ${runBtn} unverified ${unverified} refusal ${refusal.status} ${q(refusal.json?.detail)}`,
      );
  ...
  await step(
    ...
    "administrator",
    "browser-walkthrough",
    "/settings/ai-analysis (Legal Team Member)",
...
if (run("O")) {
  await step(
    ...
    async () => {
      ...
      const same =
        envHash("app", "OPENLAW_SECRET_KEY") !== null &&
        envHash("app", "OPENLAW_SECRET_KEY") === envHash("worker", "OPENLAW_SECRET_KEY");
      const providerVars = [...appEnv, ...workerEnv].filter((n) =>
        /OPENROUTER|^AI_|ANTHROPIC|OPENAI|GROQ|GEMINI/i.test(n),
      );
      ...
      const runs = psql(
        "select trigger||':'||state||':'||model from contract_analysis_runs order by id",
      );
      must(
        same &&
          ...
          finished.length >= 2 &&
          keyHits.every((h) => h === 0) &&
          prefixHits.every((h) => h === 0),
        `same ${same} vars ${providerVars} finished ${finished.length} key ${keyHits} prefix ${prefixHits}`,
      );
  ...
  await step(
    ...
    "operator",
    "live-provider-check",
    `docker compose -p ${PROJECT} restart app worker; /settings/ai-analysis`,
    ...
    async (e) => {
      ...
      const exact = await page
        .getByRole("spinbutton", { name: "Exact output token limit" })
        .inputValue();
      ...
      await on;
      await until(
        async () => page.getByRole("button", { name: "Test connection" }).isEnabled(),
        "test stays disabled",
        10000,
      );
      ...
      const st = safe(await connectorOf(admin));
      must(
        /^Below 32,768 tokens/.test(warn) && exact === "30720" && c1 === "Turned off",
        `warn ${warn} exact ${exact} chip ${c1}`,
      );
      must(
        test.status === 200 &&
          test.shown === "Connection successful." &&
          saved2.status === 200 &&
          st.maxOutputTokens === 32768 &&
          st.enabled,
        `test ${q(test)} ${q(st)}`,
      );
  ...
  await step(
    ...
    "operator",
    "browser-walkthrough",
    "/documentation/configure-analysis (signed out)",
  ...
  await step(
    ...
    async () => {
      ...
      const del = page.waitForResponse(
        (r) => r.url().endsWith("/api/v1/ai-connector") && r.request().method() === "DELETE",
      );
      ...
      const unverified = await page.getByText("Unverified", { exact: true }).count();
      must(
        before.switches[0] === true && before.answerStyle === "full_clause",
        `before ${q(before)}`,
      );
      must(
        res.status() === 200 && conv === 0 && checked === "sentence" && disabled && connect === 1,
        `res ${res.status()} conv ${conv} checked ${checked} disabled ${disabled}`,
      );
      must(
        opts.includes("OpenRouter (key saved)") &&
          after.savedKeys.length === 1 &&
          !after.configured &&
          unverified > 0,
        `opts ${q(opts)} ${q(after)} unverified ${unverified}`,
      );
  ...
  await step(
    ...
    async (e) => {
      ...
      for (const id of [
        ...
        sws.push(await page.locator(`#${id}`).getAttribute("aria-checked"));
      ...
      must(
        pill === "Key saved" &&
          ...
          sws.every((v) => v === "false") &&
          style === "sentence",
        `pill ${pill}/${pill2} save ${saved.status} ${q(sws)} ${style}`,
      );
      must(
        !st.configured && st.savedKeys.length === 0 && opts.includes("OpenRouter"),
        `final ${q(st)} ${q(opts)}`,
      );
...
if (run("V")) {
    ...
    async (e) => {
      ...
      must(
        c === "Connected" &&
          ...
          test.shown === "Connection successful." &&
          st.model === MODEL,
        `chip ${c} pill ${pill} test ${q(test)} ${q(st)}`,
      );
  ...
  await step(
    ...
    async (e) => {
      ...
      log.providerCalls.push({
        at: new Date().toISOString(),
        kind: "analysis run",
        label: "re-walk automatic (run 3 of 5)",
        ...e.run,
      });
      ...
      keepState();
      must(
        r.state === "ready" && r.trigger === "automatic" && r.preset === "openrouter",
        `run ${q(e.run)}`,
      );
  ...
  await step(
    ...
    "administrator",
    "browser-walkthrough",
    "/contracts/<n> Overview, Fields, Documents, History",
    ...
    async () => {
      ...
      const noticeRow = tidy(
        await page
          .getByText(/Notice period/)
          .first()
      ...
      const modelNamed = Object.entries(all)
        .filter(([, t]) => t.includes(MODEL) || t.includes("gpt-oss"))
        .map(([k]) => k);
      const runModel = (await runOf(n)).model;
      const connectorModel = (await connectorOf(admin)).model;
      const det = (await admin.api("GET", `/contracts/${n}`)).json.contract;
      must(
        versionLabel && marks.length > 0 && aiCalls.length === 0,
        `version ${versionLabel} marks ${marks.length} aiCalls ${aiCalls.length}`,
      );
      must(
        overview > 0 && Object.keys(det.aiUnverified ?? {}).length > 0,
        `unverified ${overview}`,
      );
      must(modelNamed.length === 0, `model named on ${q(modelNamed)}`);
      must(
        runModel === connectorModel && runModel === MODEL,
        `run ${runModel} connector ${connectorModel}`,
      );
  ...
  await step(
    ...
    async () => {
      ...
      log.finalConnector = st;
      must(
        pill === "Key saved" && !st.configured && st.savedKeys.length === 0,
        `pill ${pill} ${q(st)}`,
      );
