// DOC-030 compatibility replay (set compat): copy of DOC-029/compat-r2/contracts-b-r2/c20-r1.mjs. Changes: record names DOC-029r2 to DOC-030 compat, screenshot name, and the Matter fixture type choice (see the DOC-030 note below).
// V-C20: "Relate, end, and archive Contracts", followed as written for one role.
import { openContractsDefaultView, selectContractsView } from "./lib.mjs";
...
export async function runC20(ctx, step) {
  ...
  const pick = async (dialog, num) => {
    ...
    await box.fill(`DOC-030 compat contracts-b C20 ${num} ${tag}`);
    const option = dialog
      .getByRole("option")
    ...
    async () => {
      const make = async (key, confidential = false) => {
        const r = await actor.api("POST", "/contracts", {
          title: `DOC-030 compat contracts-b C20 ${key} ${tag}`,
      ...
      // (Employment requires Country), so the first type no longer creates with a title alone.
      // Pick the first type whose creation Form asks for nothing beyond Title and Matter type.
      const rowsOf = (nodes) =>
  ...
  await step(
    ...
    async () => {
      ...
      await searchBox.fill(`DOC-030 compat contracts-b C20 B ${tag}`);
      await dialog
        .getByRole("listbox")
        .getByRole("option")
      ...
      await searchBox.fill(`DOC-030 compat contracts-b C20 A ${tag}`);
      await sleep(2000);
  ...
  await step(
    ...
    async () => {
      ...
      await card.screenshot({
        path: `${ctx.here}/contract-relations-and-ending-${role}-restricted-related.png`,
      });
    ...
    async () => {
      const matterLink = async (key, matterKey) => {
        ...
        await dialog
          ...
          .fill(`DOC-030 compat contracts-b C20 ${matterKey} ${tag}`);
  ...
  await step(
    ...
    async () => {
      ...
      await desc.fill("DOC-030 compat note added after the Contract ended.");
  ...
  await step(
    ...
    async () => {
      ...
      await p.getByRole("link", { name: `DOC-030 compat contracts-b C20 J ${tag}` }).click();
