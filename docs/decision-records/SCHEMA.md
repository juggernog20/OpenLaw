...
| `kind`                     | text (enum) | `draft_ours` \| `draft_theirs` \| `redline_theirs` \| `redline_ours` \| `executed` \| `amendment` \| `generated_redline` (**CTR-014** kinds + generated)                                                              |
...
| `entity_type`          | text (enum) | `matter` \| `contract` \| `document` \| `request`. Polymorphic with `entity_id`, so no FK — the documented exception, as `comments` has it. `contract` is written from M18 and `request` from M20/8 (NOT-002 group 5); the other two arrive with their modules                                                                                                                  |
