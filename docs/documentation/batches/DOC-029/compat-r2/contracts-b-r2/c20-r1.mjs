...
export async function runC20(ctx, step) {
  ...
  const pick = async (dialog, num) => {
    ...
    await box.fill(`DOC-029r2 contracts-b C20 ${num} ${tag}`);
    const option = dialog.getByRole("option").filter({ hasText: `C-${n[num]}` }).first();
    ...
    async () => {
      const make = async (key, confidential = false) => {
        const r = await actor.api("POST", "/contracts", {
          title: `DOC-029r2 contracts-b C20 ${key} ${tag}`,
        ...
        ctx.records.push({ article: "contract-relations-and-ending", role, purpose: key, reference: `C-${n[key]}` });
      };
      ...
      for (const key of ["M1", "M2"]) {
        const r = await actor.api("POST", "/matters", { title: `DOC-029r2 contracts-b C20 ${key} ${tag}`, matterTypeId, managerId: userId(PEOPLE[role].name) });
        expectThat(r.status === 201, `matter ${key} ${r.status}`);
        m[key] = r.json.matter.number;
        ctx.records.push({ article: "contract-relations-and-ending", role, purpose: key, reference: `M-${m[key]}` });
      }
  ...
  await step(
    ...
    async () => {
      ...
      await withResponse(p, (r) => r.request().method() === "POST" && r.url().endsWith(`/contracts/${n.A}/parent`), () => dialog.getByRole("button", { name: "Set parent" }).click());
      ...
      expectThat((afterCancel.parentChain ?? []).length === 0, `cancel left ${JSON.stringify(afterCancel)}`);
      expectThat(/Parent/i.test(aText) && aText.includes(`C-${n.B}`) && /Children/i.test(bText) && bText.includes(`C-${n.A}`), `A "${aText}" B "${bText}"`);
      expectThat(/already sits under this one/.test(refusal) && (bRel.parentChain ?? []).length === 0, `refusal "${refusal}" B ${JSON.stringify(bRel)}`);
  ...
  await step(
    ...
    async () => {
      const link = async (type) => {
        ...
        const s = await withResponse(p, (r) => r.request().method() === "POST" && r.url().endsWith(`/contracts/${n.A}/relations`), () => dialog.getByRole("button", { name: "Link contract" }).click());
        await sleep(600);
        let alert = "";
      ...
      await searchBox.fill(`DOC-029r2 contracts-b C20 B ${tag}`);
      await dialog.getByRole("listbox").getByRole("option").filter({ hasText: `C-${n.B}` }).first().waitFor({ timeout: 10000 });
      await searchBox.fill(`DOC-029r2 contracts-b C20 A ${tag}`);
      await sleep(2000);
      const listbox = dialog.getByRole("listbox");
      const selfOptions = await listbox.getByRole("option").filter({ hasText: `C-${n.A}` }).count();
      const optionsText = (await listbox.getByRole("option").allTextContents()).join(" | ");
      await dialog.getByRole("button", { name: "Cancel" }).click();
      expectThat([r1.s, r2.s, r3.s].every((s) => s < 300), `link statuses ${r1.s} ${r2.s} ${r3.s}`);
      expectThat(/Related/.test(aText) && /Renews/.test(aText) && /Amends/.test(aText), `A "${aText}"`);
      expectThat(/Renewed by/i.test(cText) && /Amended by/i.test(cText) && /Related/i.test(cText), `C "${cText}"`);
      expectThat(dup.s === 409 && /already linked that way/.test(dup.alert), `duplicate ${dup.s} "${dup.alert}"`);
      const selfApi = await actor.api("POST", `/contracts/${n.A}/relations`, { relatedContractNumber: n.A, relationType: "related" });
      expectThat(selfOptions === 0 && [400, 409].includes(selfApi.status), `self offered: ${optionsText}; direct self-link ${selfApi.status}`);
  ...
  await step(
    ...
    async () => {
      ...
      const section = card.locator("section, div").filter({ has: p.getByText(/^Related$/i) }).last();
      const removeButtons = card.getByRole("button", { name: "Remove link" });
      const before = (await relations(n.A)).links;
      let target = null;
      const count = await removeButtons.count();
      for (let i = 0; i < count; i++) {
        const holder = removeButtons.nth(i).locator("xpath=ancestor::*[self::ul or self::section or self::div][2]");
        const holderText = await text(holder);
        if (/^Related/i.test(holderText) || (/Related/i.test(holderText) && !/Renews|Amends/i.test(holderText))) {
          target = removeButtons.nth(i);
          break;
        }
      }
      expectThat(target, `no Related remove button among ${count}; section ${await text(section).catch(() => "")}`);
      await withResponse(p, (r) => r.request().method() === "DELETE", () => target.click());
      await sleep(800);
      const after = (await relations(n.A)).links;
      card = await relatedCard(n.A);
      await withResponse(p, (r) => r.request().method() === "DELETE", () => card.getByRole("button", { name: "Remove parent" }).click());
      await sleep(800);
      const finalRel = await relations(n.A);
      const aText = await text(await relatedCard(n.A));
      const b = await read(n.B);
      const c = await read(n.C);
      const types = (links) => (links ?? []).map((l) => l.relationType).sort().join(",");
      expectThat(types(before).includes("related") && !types(after).includes("related") && /renews/.test(types(after)) && /amends/.test(types(after)), `before ${JSON.stringify(before)} after ${JSON.stringify(after)}`);
      expectThat((finalRel.parentChain ?? []).length === 0 && b && c, `parent ${JSON.stringify(finalRel.parentChain)}`);
  ...
  const nudge = async (from, to, accept) => {
    ...
    await withResponse(p, (r) => r.request().method() === "POST" && r.url().endsWith(`/contracts/${n[from]}/relations`), () => dialog.getByRole("button", { name: "Link contract" }).click());
    ...
    const s = await withResponse(p, (r) => r.url().includes("/api/v1/"), () => prompt.getByRole("button", { name: accept ? "Flag as confidential" : "No, leave it open", exact: true }).click()).catch(() => "no request");
  ...
  await step(
    ...
    async () => {
      ...
      expectThat(promptText.includes(`C-${n.E}`) && e.isConfidential && d.isConfidential, `prompt "${promptText}" E ${e.isConfidential}`);
  ...
  await step(
    ...
    async () => {
      ...
      expectThat(promptText.includes(`C-${n.F}`) && f.isConfidential, `prompt "${promptText}" F ${f.isConfidential}`);
  ...
  await step(
    ...
    async () => {
      ...
      const links = await card.getByRole("link").evaluateAll((els) => els.map((e) => e.getAttribute("href")));
      const direct = await reader.api("GET", `/contracts/${n.I}`);
      const readerRel = JSON.stringify((await reader.api("GET", `/contracts/${n.H}/relations`)).json);
      expectThat(!h.isConfidential && i.isConfidential && (rel.links ?? []).length === 1, `H ${h.isConfidential} I ${i.isConfidential} links ${JSON.stringify(rel.links)}`);
      expectThat(/Restricted contract/.test(readerText) && !readerText.includes(`C-${n.I}`) && !readerText.includes(`C20 I ${tag}`) && !links.some((l) => l?.includes(`/contracts/${n.I}`)), `reader card "${readerText}" links ${links}`);
      expectThat(direct.status === 404 && !readerRel.includes(`C20 I ${tag}`), `direct ${direct.status}`);
  ...
  await step(
    ...
    async () => {
      const matterLink = async (key, matterKey) => {
        ...
        await dialog.getByRole("textbox", { name: "Search by Matter number or title" }).fill(`DOC-029r2 contracts-b C20 ${matterKey} ${tag}`);
        ...
        const warning = (await dialog.getByText(/different Confidential flags/).count()) ? await text(dialog.getByText(/different Confidential flags/)) : "";
        const s = await withResponse(p, (r) => r.request().method() === "POST" && r.url().endsWith(`/contracts/${n[key]}/matter`), () => dialog.getByRole("button", { name: "Link", exact: true }).click());
        await sleep(1000);
      ...
      await withResponse(p, (r) => r.request().method() === "DELETE" && r.url().endsWith(`/contracts/${n.J}/matter`), () => p.getByRole("button", { name: "Unlink" }).click());
      await sleep(800);
      ...
      expectThat(first.s < 300 && unlinkShown === 1 && linkShown === 0, `first ${first.s} unlink ${unlinkShown} link ${linkShown}`);
      expectThat(second.status === 409 && moved.s < 300 && JSON.stringify(linked).includes(`"number":${m.M2}`), `second ${second.status} moved ${moved.s} ${JSON.stringify(linked).slice(0, 200)}`);
      expectThat(/different Confidential flags/.test(mismatch.warning) && mismatch.s < 300 && d.isConfidential && matterConfidential === false, `mismatch "${mismatch.warning}" D ${d.isConfidential} M ${matterConfidential}`);
  ...
  await step(
    ...
    async () => {
      ...
      await withResponse(p, (r) => r.request().method() === "PATCH", () => p.getByRole("menuitemradio", { name: "Expired Ended" }).click());
      await sleep(1000);
      const c1 = await read(n.J);
      const desc = p.getByRole("textbox", { name: "Description" });
      await desc.fill("DOC-029r2 note added after the Contract ended.");
      const ds = await withResponse(p, (r) => r.request().method() === "PATCH", () => desc.press("Tab"));
      await p.getByRole("toolbar", { name: "Applets" }).getByRole("button", { name: "History" }).click();
      await sleep(2000);
      const history = await text(p.getByText(/Expired/).last().locator("xpath=ancestor::*[self::li or self::article or self::div][2]"));
      const view = await openList();
      const hiddenDefault = !(await findInList());
      await p.getByRole("button", { name: /^Filter/ }).click();
      await p.getByRole("dialog", { name: "Filter" }).getByRole("button", { name: "Show ended" }).click();
      await sleep(400);
      ...
      await p.goto(`${BASE}/contracts/${n.J}`);
      await p.getByRole("button", { name: / — move contract$/ }).click();
      await withResponse(p, (r) => r.request().method() === "PATCH", () => p.getByRole("menuitemradio", { name: "Active Active" }).click());
      await sleep(800);
      ...
      expectThat(c1.statusName === "Expired" && c1.stage === "ended" && ds === 200, `status ${c1.statusName} ${c1.stage} desc ${ds}`);
      expectThat(hiddenDefault && shownEnded, `view "${view}" hidden ${hiddenDefault} shown ${shownEnded} filter "${filterName}"`);
      expectThat(c2.statusName === "Active" && historyKeeps, `reopen ${c2.statusName} history ${historyKeeps}`);
  ...
  await step(
    ...
    async () => {
      const link = await actor.api("POST", `/contracts/${n.J}/relations`, { relatedContractNumber: n.C, relationType: "related" });
      await p.goto(`${BASE}/contracts/${n.J}`);
      await p.getByRole("button", { name: / — move contract$/ }).click();
      await withResponse(p, (r) => r.request().method() === "PATCH", () => p.getByRole("menuitemradio", { name: "Expired Ended" }).click());
      await sleep(800);
      ...
      await sleep(2000);
      const sectionBefore = p.locator("section").filter({ has: p.getByText("Linked Contracts", { exact: true }) });
      const sectionBeforeText = (await sectionBefore.count()) ? await text(sectionBefore.first()) : "";
      expectThat(sectionBeforeText.includes(`C-${n.J}`), `M-${m.M2} Linked Contracts before archive: "${sectionBeforeText}"`);
      await p.goto(`${BASE}/contracts/${n.J}`);
      await p.getByRole("button", { name: "Contract actions" }).click();
      await withResponse(p, (r) => r.request().method() === "POST" && r.url().endsWith(`/contracts/${n.J}/archive`), async () => {
      ...
      const titleEditable = await p.getByRole("textbox", { name: "Title" }).isEditable().catch(() => false);
      const c = await read(n.J);
      const linkedC = await read(n.C);
      const matter = (await actor.api("GET", `/matters/${m.M2}`)).json;
      await p.goto(`${BASE}/matters/${m.M2}`);
      await sleep(2000);
      const section = p.locator("section").filter({ has: p.getByText("Linked Contracts", { exact: true }) });
      const sectionText = (await section.count()) ? await text(section.first()) : "(no Linked Contracts section found)";
      ...
      const setFlag = async (label) => {
        await p.getByRole("button", { name: /^Filter/ }).click();
        await p.getByRole("dialog", { name: "Filter" }).getByRole("button", { name: label }).click();
        await sleep(400);
        await p.keyboard.press("Escape");
        await sleep(1500);
      };
      ...
      await p.getByRole("link", { name: `DOC-029r2 contracts-b C20 J ${tag}` }).click();
      await p.waitForURL(new RegExp(`/contracts/${n.J}`));
      await p.getByRole("button", { name: "Contract actions" }).click();
      await withResponse(p, (r) => r.request().method() === "POST" && r.url().endsWith(`/contracts/${n.J}/restore`), () => p.getByRole("menuitem", { name: "Restore" }).click());
      await sleep(1000);
      await p.reload();
      await sleep(1500);
      ...
      if (/\d/.test(leftFilter)) {
        await p.getByRole("button", { name: /^Filter/ }).click();
        const clear = p.getByRole("dialog", { name: "Filter" }).getByRole("button", { name: "Clear all" });
        if (await clear.count()) await clear.click();
        await p.keyboard.press("Escape");
        await sleep(800);
      }
      const restoredView = ctx.restoreView ? await selectContractsView(p, ctx.restoreView) : "Default view";
      expectThat(marker >= 1 && /Restore it to edit/.test(notice) && !titleEditable && c.archivedAt && c.statusName === "Expired", `marker ${marker} notice "${notice}" editable ${titleEditable} status ${c.statusName}`);
      expectThat(!linkedC.archivedAt && !(matter.matter?.archivedAt ?? matter.archivedAt) && !sectionText.includes(`C-${n.J}`), `linked C ${linkedC.archivedAt} matter section "${sectionText}"`);
      expectThat(!archivedOnly && both, `archived only ${archivedOnly} (${archivedOnlyName}); both ${both} (${bothName})`);
      expectThat(markerAfter === 0 && editableAfter && !c2.archivedAt && c2.statusName === "Expired" && restoreByAddress === 1 && !c3.archivedAt, `after restore marker ${markerAfter} editable ${editableAfter} status ${c2.statusName} address ${restoreByAddress}`);
