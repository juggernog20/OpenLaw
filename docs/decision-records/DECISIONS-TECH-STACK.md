...
### Addendum (2026-09-06) — better-auth 1.7.3 takes the account key back; the version-pin rule

The "version pin" line above stopped being true at [#340](https://github.com/juggernog20/OpenLaw/issues/340), which moved the three `@better-auth/*` packages from 1.6 to 1.7.0 and adopted 1.7's account identity: a required `accounts.issuer`, a unique (`issuer`, `account_id`), and a backfill migration (`0060_account_issuer`). **1.7.3 reverts that upstream** — accounts are keyed on (`provider_id`, `account_id`) "as in 1.6", `issuer` leaves the schema, `account.identityStrategy` is removed, and the reason given is the one #340's spec itself named as the cost: a backfill nobody could run for existing users. It shipped as a patch release.

**The reversal is taken, not resisted.** The stricter key was adopted for one stated case — two provider registrations naming the same IdP — and read against what the library does in that case with our configuration (the sso plugin only asks for exact account binding when a `resolveUser` hook is set; we set none), it bought nothing a person would notice: 1.7.2 reused the first registration's row, 1.7.3 links a second row to the same user by email, and either way the person signs in as one user. Staying on an orphaned point release, or carrying our own issuer enforcement on the credential path the "swappable implementation detail" stance keeps inside the library, were both worse than following. `0091_account_issuer_retired` drops the column and its index rather than relaxing them — a column better-auth neither reads nor writes would be NULL on every row after the upgrade and stale on every row before it — and 0060 stays in the chain untouched: shipped migrations are history, and an install passes through both. The `SCHEMA.md` `accounts` section is rewritten to match.
...
**Neither library touches the doc-engine seam, and that is the decision holding.** `DocEngine` names operations over documents the sidecar reads; an email is not one of them, and adding it would put a text format that parses in a millisecond behind an HTTP call to an 800 MB container. The `EmailUnreadableError` these libraries raise is classified terminal beside the engine's own terminal failures, because it means the same thing to the pipeline: a retry reads the same bytes.

### Addendum (2026-09-03) — settled in M32/1: compare is the fourth operation

This decision named compare from the start and left how it is driven open. This is how.

**`DocEngine.compare` takes an older and a newer Word file, each with its own format, and answers a DOCX whose difference is tracked changes** (the older text deleted, the newer text inserted, DOC-003). DOC, DOCX, ODT, and RTF are accepted on either side, and the two need not match. The failure split is the one the other three operations have: a format that is not Word and bytes that cannot be opened are terminal, a timeout and an unreachable engine are transient.
...
The private API is the cost of the fix. A React Router upgrade that renames or drops `UNSAFE_DataRouterContext` breaks this one file, and `routes/list-navigation.test.tsx` holds the 39 controlled cases that a replacement has to pass.
...
A request line carries the request id, the method, the path cut before `?`, and the client address. It never reads a header. A response line carries the status code. An error line carries the error's type, message, stack, and driver code. A failed query becomes its Postgres SQLSTATE, constraint, table, and column, and never the SQL text, the bind parameters, or the driver's `detail` line, which quotes the offending value. In any other error's message and stack header, a URL's userinfo, the value of a query parameter named `token`, `code`, `state`, `key`, `secret`, `password`, `signature`, or `api_key`, and a bearer value are each replaced with `***`; the list is fixed, and it is not a secret detector. The logger's `redact` list masks `req.headers.cookie`, `req.headers.authorization`, `err.params`, and `err.query` anyway, for any object logged under those keys by hand.
