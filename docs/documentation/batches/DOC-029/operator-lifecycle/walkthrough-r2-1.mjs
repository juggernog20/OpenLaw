// DOC-029r2 operator-lifecycle, round 1: independent container-operation walkthrough
// for the upgrade article (V-C46) from starting build d1d098ba to candidate 57e77e38.
//
// Written by the DOC-029r2 independent walkthrough agent (operator-lifecycle, round 1).
// Adapted from walkthrough-r1.mjs (DOC-029 round 1). Only the upgrade phases and the
// backup-based recovery drill that the upgrade article names are kept.
// It follows the article commands in private installation directories under
// /tmp/claude-1000/openlaw-docs-lc-r2-1 and appends observations to walkthrough-r2-1.json.
//
// Run one phase at a time from the repository root:
//   mise exec -- node docs/documentation/batches/DOC-029/operator-lifecycle/walkthrough-r2-1.mjs <phase>
// Phases: support-up, baseline-install, populate, upgrade, upgrade-start, upgrade-verify,
// portal-reach, recovery, recovery-verify, destroy.
//
// Keys, fixture passwords and mail tokens stay in /tmp/claude-1000/openlaw-docs-lc-r2-1
// (mode 600) or in memory. Browser sessions are never saved. The log receives only
// redacted, selected output.
...
const LC = "/tmp/claude-1000/openlaw-docs-lc-r2-1";
...
const PREFIX = "openlaw-docs-d029r21-lc";
...
// 23330, 23331, 23335 and 23336 were held by another agent's project when this run started.
const MAIL_UI = "http://127.0.0.1:23333";

const PROJECTS = {
  upgrade: {
    project: `${PREFIX}-upgrade`,
    port: 23338,
    subnets: ["10.239.70.0/24", "10.239.71.0/24"],
  },
  recovery: {
    project: `${PREFIX}-recovery`,
    port: 23339,
    subnets: ["10.239.72.0/24", "10.239.73.0/24"],
  },
};
...
const log = readJson(LOG, {
  ...
  batchRound: 2,
  round: 1,
  reviewer: "DOC-029r2 independent walkthrough agent (operator-lifecycle, round 1)",
  reviewerKind: "agent",
  method: "container-operation",
  candidate: CANDIDATE,
  baseline: BASELINE,
  script: "docs/documentation/batches/DOC-029/operator-lifecycle/walkthrough-r2-1.mjs",
  labProjects: Object.values(PROJECTS).map((p) => p.project),
  supportContainers: [MAIL_NAME],
  articles: {},
  images: {},
  phases: [],
  steps: [],
});
for (const id of ["upgrade", "backup-and-restore", "install"]) {
...
// Compose creates its own networks. Only when the host has no free address pool are the
// two networks pre-created with explicit subnets and the Compose labels, and that is logged.
function composeUp(p, command, options = {}) {
  let r = sh(command, p.dir, { timeout: 600_000, ...options });
  if (r.code !== 0 && /address pools have been fully subnetted/.test(r.stderr)) {
    ...
    r = sh(command, p.dir, { timeout: 600_000, ...options });
  }
...
function attachSupport(p) {
  ...
  for (const [container, alias] of [[MAIL_NAME, "lc-mail"]]) {
  ...
  await step(
    ...
    () => {
      if (sh(`docker inspect ${MAIL_NAME}`, root).code !== 0)
        must(
          `docker run -d --name ${MAIL_NAME} --label openlaw-docs-owner=${PREFIX} -p 127.0.0.1:23333:8025 axllent/mailpit:v1.30 --smtp-auth-accept-any --smtp-auth-allow-insecure`,
          root,
        );
      return `Mailpit ${MAIL_NAME} on the default bridge; joined to each project's backend network as lc-mail after Compose creates it; UI 127.0.0.1:23333`;
    },
  );
};
...
phases["baseline-install"] = async () => {
  ...
  await step(
    ...
    async () => {
      ...
      composeUp(p, "docker compose up -d --no-build --pull never");
      await waitReady(p, 2_400_000);
      attachSupport(p);
...
phases["populate"] = async () => {
  ...
  const people = {
    admin: {
      email: "morgan.baseline@lifecycle.example",
      displayName: "DOC-029r2 operator-lifecycle Morgan Baseline",
    },
    reader: {
      email: "rowan.reader@lifecycle.example",
      displayName: "DOC-029r2 operator-lifecycle Rowan Reader",
      role: "legal_team_member",
    },
    contributor: {
      email: "casey.contributor@lifecycle.example",
      displayName: "DOC-029r2 operator-lifecycle Casey Contributor",
      role: "contributor",
    },
  };
  const api = new Api(p.base);
  ...
  await step(
    ...
    async () => {
      ...
      await api.put("/api/v1/email-settings", {
        ...
        smtpFrom: "DOC-029r2 operator-lifecycle Lifecycle <openlaw@lifecycle.example>",
      });
      const since = Date.now();
    ...
    async () => {
      ...
      const entity = (
        await api.post("/api/v1/entities", {
          legalName: "DOC-029r2 operator-lifecycle Holdings Ltd",
      ...
      const field = (
        await api.post("/api/v1/fields", {
          displayName: "DOC-029r2 operator-lifecycle reference",
      ...
      const matter = (
        await api.post("/api/v1/matters", {
          title: "DOC-029r2 operator-lifecycle regulatory advice",
    ...
    async () => {
      ...
      const d1 = (
        await api.upload(
          ...
          "doc029r2-operator-lifecycle-draft.txt",
          Buffer.from("DOC-029r2 operator-lifecycle draft, original version.\n"),
          "text/plain",
        )
      ).document;
      await api.upload(
        `/api/v1/documents/${d1.id}/versions`,
        "doc029r2-operator-lifecycle-draft-v2.txt",
        Buffer.from("DOC-029r2 operator-lifecycle draft, later version.\n"),
        "text/plain",
      );
      const d2 = (
        await api.upload(
          `/api/v1/contracts/${confidential.number}/documents`,
          "doc029r2-operator-lifecycle-settlement.pdf",
          pdf([
            "DOC-029r2 operator-lifecycle settlement, original.",
      ...
      await api.upload(
        ...
        "doc029r2-operator-lifecycle-settlement-v2.pdf",
        pdf([
          "DOC-029r2 operator-lifecycle settlement, later.",
...
phases["upgrade"] = async () => {
  ...
  await step(
    ...
    () => {
      ...
      return `starting build: pg-boss job states: ${jobs || "(none)"}; contract_envelopes: ${env.code === 0 ? env.stdout.trim() || "(none)" : "query failed"}`;
    },
  );
...
async function verifyInstance(
  ...
  await step(
    ...
    async () => {
      ...
      if (s.page) {
        ...
        const shot = path.join(here, `r2-1-${p.name}-admin-confidential-contract.png`);
  ...
  await step(
    ...
    async () => {
      ...
      const name = `doc029r2-operator-lifecycle-${p.name}-${Date.now()}.pdf`;
      const doc = (
        await api.upload(
          `/api/v1/contracts/${c.number}/documents`,
          name,
          pdf([`DOC-029r2 operator-lifecycle new upload on ${label}.`, "Worker processing check."]),
          "application/pdf",
        )
      ).document;
  ...
  await step(
    ...
    () => {
      const check1 = must('(cd "$BACKUP_DIR" && sha256sum --check SHA256SUMS)', p.dir, { env })
        .stdout.trim()
        .replace(/\n/g, "; ");
      composeUp(p, "docker compose up -d postgres");
      return check1;
    },
  );
  await step(
...
phases["destroy"] = async () => {
  await step(
    ...
    () => {
      ...
      for (const c of [MAIL_NAME]) out.push(`${c} rm exit ${sh(`docker rm -f ${c}`, root).code}`);
      for (const p of Object.values(PROJECTS))
...
if (!phases[phase]) {
  console.error(`Usage: walkthrough-r2-1.mjs ${Object.keys(phases).join("|")}`);
  process.exit(2);
}
currentPhase = phase;
