 ...
 *
 * It is chrome, not a notification. There is no dismiss, no close, and
 * no state that could hide it. The component takes no such prop, so a
 * caller cannot add one. DD-014's failure mode is the person who has
 ...
 * may change the audience get a way to it (DD-014, CTR-022:
 * Administrators, the record's creator, and its Owner). Everyone else
 * reads the statement and nothing more. The caller decides, because the
 * page knows where this viewer stands on this record. The API is the
 ...
 * inline marker, where a 16px lock is taller than the text it marks. A
 * 36px strip has the room. */
const LOCK_SIZE = 16;
...
export function ConfidentialBanner({
   ...
   * all. A control nobody may use is worse than none. */
