...
- Have the information and any files you want Legal to review. Your Administrator configures the available request types and the questions on each form.

## Submit the form

1. On the Portal home, read any [**Before you submit** guidance](portal-knowledge.md) and select the request type that matches your work. The form can show its own **Before you submit** links beside the questions.
2. Under **About your request**, enter a **Title** that names what you need.
3. Check **Department**. It starts with your own Department when that Department is in the list; otherwise choose one. When the list has Departments, **Department** is required.
4. Choose **Urgency**. It starts at **Medium**; choose **Low**, **Medium**, **High**, or **Critical** to describe your need. This choice does not promise a response time.
5. Answer the questions that follow. Required questions are marked with an asterisk. If **Description** is shown, explain the background, the Counterparty if relevant, and the help you want from Legal.
6. Under **Attachments** at the end of the form, select **Choose files** or drop the files onto the form. You can select up to 20 files. Check the selected filenames and remove any you did not intend to send. If you exceed the limit, the form keeps only the files that fit and shows **A request carries at most 20 files.**
7. Select **Submit request** once. If a required answer is missing, the form names it. Fill in the marked question and submit again.
8. The confirmation reads **Thanks! Your request has been submitted to legal.** Keep it open while **Attaching your files…** shows. If a file failed, the confirmation names the file and the Request's **R-** reference.
9. Select **Open request** to check the saved answers and attachments. The line under the title shows the Request's R- reference.
...
Check **Your requests** on the Portal home, then open the Request to confirm what you submitted. A new Request shows **Open** and **Legal Owner: Not assigned yet**. Legal receives a Request to triage; a Contract or Matter is created only if Legal converts it.
...
The confirmation appears as soon as the Request exists, before every attachment has finished. An attachment failure does not undo the Request. Read the named failure and quote the R- reference it gives when you contact Legal. Submitting the whole form again would create another Request. Open that Request and attach the missing file to a reply. Check any file-size error against the limit shown by your instance. You can attach at most 256 MB of Request files in one hour. If you reach that limit, wait and send the rest on a reply.
...
You can submit at most 20 Requests in one hour. After that, the form tells you to wait before you submit another.
