...
| **ENT-001**  | Entities Settings → Types / Types → [type]                                           | Admin      | List view; add / rename / reorder / archive; `other` row protected; per-type attachment editor for `entity`/`global` Fields with order and required flags                                                                |
...
| **ENT-001**  | Entities Settings → Fields                                                           | Admin      | Catalog view scoped to `entity` + `global` fields per **CTR-016**'s shared `fields` catalog: add / rename / describe / archive; same machinery as the Matters/Contracts views                                            |
