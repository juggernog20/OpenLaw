// DOC-029 round 2 independent operator walkthrough, group "operator-install", round 2 (attempt 2).
...
// operator-install/walkthrough-r2-1.mjs. Attempt 2 walks the corrected email paragraph (contentSha256
// e50a783f...) in the phase mail-no-from and repeats every other phase. Round 2 changes: the
...
//   mise exec -- node docs/documentation/batches/DOC-029/operator-install/walkthrough-r2-2.mjs <phase>
//
// Private state (the clone, its .env, fixture passwords, browser traces) lives outside the
// repository in WORK. The sanitized log is walkthrough-r2-2.json beside this script. Every attempt
...
const WORK = "/tmp/claude-1000/doc029r2-opinstall-2";
const PRIVATE = path.join(WORK, "private");
const CLONE = path.join(WORK, "openlaw");
const FIX = path.join(WORK, "fixtures");
const PROJECT = "doc029r2-opinst2";
const FX_PROJECT = "doc029r2-opinst2-fx";
...
const LOG = path.join(here, "walkthrough-r2-2.json");
const REVIEWER = "DOC-029r2 independent walkthrough agent (operator-install, round 2)";
...
const log = existsSync(LOG)
  ...
  : {
      ...
      round: "2-2",
      basedOn: "docs/documentation/batches/DOC-029/operator-install/walkthrough-r2-1.mjs",
      reviewer: REVIEWER,
      reviewerKind: "agent",
      appCommit: COMMIT,
...
`,
  ...
  await step(
    ...
    async () => {
      ...
      await page.screenshot({ path: path.join(here, "r2-2-setup-through-proxy.png") });
      await context.close();
      return `GET ${ORIGIN} answered ${status} through Caddy, and the page shows the "Set up OpenLaw" heading. Screenshot r2-2-setup-through-proxy.png.`;
    },
  );
};
...
phases["firstrun-reset"] = async () => {
  await step(
    ...
    async () => {
      ...
      return `To repeat the article step from an empty instance, the reviewer stopped app and worker, dropped and recreated the openlaw database, and ran up -d --no-build --pull never again (exit 0, /readyz 200, needsSetup ${setup.needsSetup}). This is a reviewer reset, not an article step.`;
    },
  );
};
...
phases["checks-invite2"] = async () => {
  await step(
    ...
    async () => {
      ...
      return `Sign-in with ${COLLEAGUE.name}'s password answers ${rowan.signInStatus}. Retry: Settings -> Users -> Invite user sent a new test invitation (${r.inviteStatus}); "${msg.Subject}" reached the relay with a link on ${link.origin}. Opening it showed "Password set", and the new recipient signed in (${c.signInStatus}).`;
    },
  );
};
...
async function liveUpdate(ORIGIN) {
  ...
  expect(
    ...
    `comment post ${postStatus} (${rowan?.role}); ${COLLEAGUE.name} sign-in answered ${c.signInStatus}`,
  );
...
/** Read which email-dependent links the sign-in page offers, with the password form showing. */
async function signInPageOffers(p = "/auth/login") {
  const { page, context } = await newSession();
  try {
    ...
    const signInButton = await page.getByRole("button", { name: "Sign in", exact: true }).count();
    const offered = [];
...
phases["mail-no-from"] = async () => {
  const noFrom = async () => {
    ...
    return { up: r.code, ready };
  };
  await step(
    "V-C45",
    "SMTP_URL set to the working relay, SMTP_FROM unset, containers recreated: the API refuses a test email and the relay receives nothing",
    "Source stays env with no From address; saving a relay in the app is refused; POST /api/v1/email-settings/test is refused; nothing reaches the relay.",
    "container-operation",
    async () => {
      const u = await noFrom();
      const appFrom = compose("exec -T app printenv SMTP_FROM").stdout.trim();
      const wFrom = compose("exec -T worker printenv SMTP_FROM").stdout.trim();
      const before = await mailCount();
      const t = await testSend();
      await sleep(3000);
      const after = await mailCount();
      expect(t.source === "env" && t.from === null, `source ${t.source}, from ${t.from}`);
      expect(appFrom === "" && wFrom === "", `SMTP_FROM app "${appFrom}" worker "${wFrom}"`);
      expect(t.putStatus === 409 && t.status >= 400 && before === after, JSON.stringify(t));
      return `Set SMTP_URL to the working relay (smtp://relay:1025), removed SMTP_FROM, and ran up -d --no-build --pull never (exit ${u.up}, /readyz ${u.ready}). SMTP_FROM is empty in app and worker. email-settings source "${t.source}", fromAddress ${t.from}. Saving an app relay answered ${t.putStatus}. POST /api/v1/email-settings/test answered ${t.status} "${t.detail}". The relay received ${after - before} messages.`;
    },
  );
  await step(
    "V-C45",
    "SMTP_URL without SMTP_FROM: the sign-in page does not show Set up or reset your password or Email me a sign-in link; the API refuses sign-in link and password-setup requests; no email reaches the relay",
    "Both links are absent on the sign-in page; POST /api/v1/auth/magic-link and POST /api/v1/auth/password-setup answer with an error status; the relay count does not change.",
    "browser-walkthrough",
    async () => {
      const before = await mailCount();
      const legal = await signInPageOffers("/auth/login");
      const portal = await signInPageOffers("/portal/login");
      ...
      const addr = `doc029r2-nofrom-${Date.now()}@doc029-install.example`;
      const ml = await probe.post("/api/v1/auth/magic-link", { data: { email: addr } });
      const mlBody = await json(ml);
      const ps = await probe.post("/api/v1/auth/password-setup", { data: { email: addr } });
      const psBody = await json(ps);
      await probe.dispose();
      await sleep(4000);
      const after = await mailCount();
      const toAddr = (await mailList()).filter((m) =>
        (m.To ?? []).some((t) => t.Address === addr),
      ).length;
      expect(legal.signInButton > 0, `sign-in form not shown ${JSON.stringify(legal)}`);
      expect(legal.offered.length === 0, `legal sign-in page offers ${legal.offered}`);
      expect(
        ml.status() >= 400 && ps.status() >= 400,
        `magic ${ml.status()}, setup ${ps.status()}`,
      );
      expect(before === after && toAddr === 0, `mail ${before}->${after}, to address ${toAddr}`);
      return `/api/v1/auth/methods reports emailConfigured=${methods.emailConfigured}. On ${ORIGIN}/auth/login the password form with Sign in showed; the page offered ${legal.offered.join(", ") || "neither Set up or reset your password nor Email me a sign-in link"}. On /portal/login it offered ${portal.offered.join(", ") || "neither"}. For a fictional address, POST /api/v1/auth/magic-link answered ${ml.status()} "${mlBody.detail ?? mlBody.title ?? ""}" and POST /api/v1/auth/password-setup answered ${ps.status()} "${psBody.detail ?? psBody.title ?? ""}". The relay received ${after - before} messages (${toAddr} to that address).`;
    },
  );
  await step(
    "V-C45",
    ...
    async () => {
      ...
      const r = await inviteThroughSettings(
        ...
        "r2-2-invite-without-smtp-from.png",
      );
      await sleep(5000);
      const after = await mailCount();
      ...
      const logs = appLogLines(since, /not configured/i)
        .slice(0, 1)
        .join(" / ");
      expect(
      ...
      return `Settings -> Users -> Invite user -> Send invite: POST /api/v1/auth/invites answered ${r.inviteStatus}; the dialog closed; alerts on the page: "${r.alerts || "none"}". The row read "${r.rowBefore}" and after reload "${r.rowAfter}"; the API lists status ${r.apiStatus}. The relay received ${after - before} messages (${toInvitee} to the invitee). App log: ${logs || "no matching line"}. Screenshot r2-2-invite-without-smtp-from.png. This matches the article and is tracked as bug #889.`;
    },
  );
  await step(
    "V-C45",
    "SMTP_URL without SMTP_FROM: the welcome wizard email step warns about SMTP_FROM, has no Send test email button, and an Administrator cannot finish the wizard",
    "On a fresh database under the same environment, the email step shows a warning that the environment sets SMTP_URL but not SMTP_FROM; no Send test email button; Continue is disabled; a direct completion request is refused.",
    "browser-walkthrough",
    async () => {
      ...
      try {
        ...
        await page.goto(ORIGIN);
        await page.getByRole("heading", { name: "Set up OpenLaw" }).waitFor({ timeout: 30_000 });
        await page.getByLabel("Name", { exact: true }).fill("DOC-029r2 Wren Wizard");
        ...
        const emailAlert = (await page.getByRole("alert").allInnerTexts())
          ...
          .filter(Boolean)
          .join(" | ");
        const testButtons = await page.getByRole("button", { name: "Send test email" }).count();
        const contDisabled = await page.getByRole("button", { name: "Continue" }).isDisabled();
        const later = await page.getByRole("button", { name: "Set up later" }).count();
        await page
          .screenshot({ path: path.join(here, "r2-2-wizard-email-without-smtp-from.png") })
          .catch(() => {});
        const status = await json(await page.request.get(`${ORIGIN}/api/v1/onboarding`));
        ...
        const after = await json(await page.request.get(`${ORIGIN}/api/v1/onboarding`));
        result = {
          emailAlert,
          testButtons,
          contDisabled,
      ...
      expect(
        /SMTP_URL/.test(result.emailAlert) && /SMTP_FROM/.test(result.emailAlert),
        `alert "${result.emailAlert}"`,
      );
      expect(result.testButtons === 0, `Send test email buttons: ${result.testButtons}`);
      expect(result.contDisabled && result.later === 0, JSON.stringify(result));
      expect(result.completed === false && result.direct === 409, JSON.stringify(result));
      return `Created an empty database doc029wiz on the external Postgres, pointed DATABASE_URL at it with SMTP_URL set and SMTP_FROM unset, and recreated. Created the fictional Administrator DOC-029r2 Wren Wizard and walked the wizard to the email step (Step 5 of 9). It showed "${result.emailAlert}". It had ${result.testButtons} Send test email buttons and ${result.later} Set up later buttons, and Continue was ${result.contDisabled ? "disabled" : "enabled"}. A direct completion request answered ${result.direct} "${result.directDetail}"; the onboarding state read completed=${result.completed}. Screenshot r2-2-wizard-email-without-smtp-from.png. DATABASE_URL was then removed and the containers recreated on the bundled database, still with SMTP_URL set and SMTP_FROM unset.`;
    },
  );
  await step(
    "V-C45",
    "Contrast: restore the saved wizard relay (remove SMTP_URL) and recreate; the sign-in page shows both links again",
    "With the saved relay in use, /auth/login offers Set up or reset your password and Email me a sign-in link.",
    "browser-walkthrough",
    async () => {
      ...
      const legal = await signInPageOffers("/auth/login");
      expect(r.code === 0 && ready === 200, `up ${r.code}, ready ${ready}`);
      expect(legal.offered.length === 2, `offered ${legal.offered}`);
      return `Removed SMTP_URL and recreated (exit ${r.code}, /readyz ${ready}). ${ORIGIN}/auth/login now offers ${legal.offered.join(" and ")}.`;
    },
  );
};
...
phases["private-serve"] = async () => {
  await step(
    ...
    async () => {
      ...
      return `The private proxy started. curl with only the fictional CA as trust anchor and ${PRIVATE_HOST} resolved to ${PRIVATE_BIND} got HTTP ${status} from /readyz with ${headers.join("; ")}. curl without that CA exited ${untrusted.code} ("${firstLines(untrusted.stderr, 1)}"). The only listener on port ${PRIVATE_PORT} is ${proxyListen.join(", ")}; port ${PRIVATE_HTTP_PORT} listeners: ${http.join(", ") || "none"}. The direct app port on the host's LAN address 192.168.0.59:${APP_PORT} failed (curl exit ${lanDirect.code}), and the proxy did not answer on 127.0.0.1:${PRIVATE_PORT} (curl exit ${loopProxy.code}).`;
    },
  );
};
