// DOC-029 round 2 compatibility replay of the round 1 V-C56 walkthrough (batches/DOC-029/auto-docs/walkthrough-r1.mjs)
// against the admin2 lab built from 57e77e386be31b2a319f7143dd54d00123e65efe. Adapted by the
// DOC-029r2 compatibility reviewer (auto-docs): lab project, record names, reviewer, round, output paths and
// the fixture folder (reused from round 1 unchanged). No check, selector or expectation changed.
// Run from the repository root:
//   LAB_PASSWORD=... mise exec -- node docs/documentation/batches/DOC-029/compat-r2/auto-docs-r2/walkthrough-r2.mjs
...
const SHOTS = here;
const FIX = path.join(here, "../../auto-docs/fixtures");
...
const results = {
  ...
  group: "auto-docs",
  round: 2,
  replayOf: "batches/DOC-029/auto-docs/walkthrough-r1.mjs",
  issues: [745, 747],
  walkthroughReviewer: "DOC-029r2 compatibility reviewer (auto-docs)",
  reviewerKind: "agent",
  ...
  labProject: process.env.LAB_PROJECT ?? "openlaw-docs-41255c61-admin2",
...
async function walkthrough(role, page, displayName, options = {}) {
  ...
  const autoDocName = `DOC-029r2 auto-docs ${roleName} Services Agreement ${stamp}`;
  let recordId = null;
  let base = null;
    ...
    async () => {
      ...
      if (role === "administrator")
        await page.screenshot({
          path: path.join(SHOTS, "r2-admin-template-pane.png"),
          fullPage: true,
        });
  ...
  await step(
    ...
    async () => {
      ...
      const text = (await alert.textContent()).trim();
      if (role === "administrator")
        await page.screenshot({ path: path.join(SHOTS, "r2-admin-publish-refusal.png") });
      await dialog.getByRole("button", { name: "Cancel" }).click();
      await dialog.waitFor({ state: "hidden" });
      const state = (
        await page
  ...
  await step(
    ...
    async () => {
      ...
      if (role === "legal_team_member")
        await page.screenshot({
          path: path.join(SHOTS, "r2-member-orphaned-field.png"),
          fullPage: true,
        });
  ...
  if (options.twoDirectives) {
    await step(
      ...
      async () => {
        const name = `DOC-029r2 auto-docs ${roleName} Two directives ${stamp}`;
        await page.goto(`${BASE}/auto-docs`);
        await page.getByRole("button", { name: "Create Auto-Doc" }).first().click();
        const dialog = page.getByRole("dialog");
