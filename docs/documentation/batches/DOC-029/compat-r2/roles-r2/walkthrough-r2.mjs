// DOC-029 independent walkthrough (roles, round 2).
...
// Run from the worktree root after setup-r1.mjs and setup-r1-part2.mjs (round 2 reuses those fixtures):
//   LAB_PASSWORD=... mise exec -- node docs/documentation/batches/DOC-029/roles/walkthrough-r2.mjs
...
function labStatus() {
  try {
    ...
    const project = text.match(/(openlaw-docs-[0-9a-f]+-work2)-app-1/)?.[1] ?? null;
    const source = text.match(/source ([0-9a-f]{40})/)?.[1] ?? null;
    const image = (name) =>
      execFileSync("docker", ["inspect", "--format", "{{.Image}}", `${project}-${name}-1`], { encoding: "utf8" }).trim();
...
const results = {
  ...
  group: "roles",
  round: "compat-r2",
  reviewer: "DOC-029r2 compatibility reviewer (roles)",
  reviewerKind: "agent",
  appUrl: BASE,
  lab: labStatus(),
  articles: {
  ...
  fixtures: {
    ...
    confidentialEntity: "DOC-029r2 roles confidential entity Ltd",
...
async function step(article, role, id, expected, fn) {
  ...
  const entry = { article, id, role, method: "browser-walkthrough", expected, actual: null, result: "not-run", startedAt: new Date().toISOString(), at: null };
  ...
  console.log(`[${role}] ${entry.result.toUpperCase()} ${id}: ${String(entry.actual).slice(0, 300)}`);
  save();
  return entry;
}
...
async function go(page, url) {
  ...
  if (WENT_WRONG.test(await page.locator("body").innerText().catch(() => ""))) {
    results.transientErrors.push({ at: new Date().toISOString(), url, consoleErrors: (page.__errors ?? []).slice(-3) });
    await page.reload();
    await settle(page);
  }
}
...
async function applet(page, name) {
  ...
  if (!(await panel.isVisible().catch(() => false))) {
    await page.getByRole("toolbar", { name: "Applets" }).getByRole("button", { name: new RegExp(`^${name}\\b`) }).first().click();
  }
...
await step(RA, "administrator", "A1.accounts", "Users lists Administrator, Legal Team Member and Business User account types; no Contributor type exists; an Administrator can pick Legal Team Member for a Business User.", async () => {
  await go(D, "/settings/users");
  const table = await D.getByRole("table").innerText();
  const roleButton = D.getByRole("button", { name: "Business user — change the role of ravi.menon@helix.example" });
...
await step(RA, "legal_team_member", "A2.settings", "Organization settings remain Administrator-only.", async () => {
  await go(N, "/settings/users");
  const body = await N.locator("body").innerText();
  const users = await N.getByRole("button", { name: "Invite user" }).isVisible().catch(() => false);
...
await step(RA, "legal_team_member", "B1.open-records", "A Legal Team Member opens non-Confidential Contracts and Matters without a team row.", async () => {
...
await step(RA, "administrator", "B2.admin-refused", "Administrator status does not bypass the Confidential rule: without a team row or Legal Owner / Matter Manager, Daniel is refused; a copied link and a reload do not restore access.", async () => {
  const c = await openRecord(D, `/contracts/${CC}`);
  await D.reload();
  await settle(D);
  const cReload = await refused(D);
  const m = await openRecord(D, `/matters/${MC}`);
  check(c.refused && cReload && m.refused, `C refused ${c.refused}, reload ${cReload}, M refused ${m.refused}`);
...
await step(RA, "administrator", "B3.related-links", "A link to a related record does not grant reach: the parent Matter shows the Confidential child and linked Contract without a link.", async () => {
  await go(D, `/matters/${MO}`);
  const main = await D.getByRole("main").innerText();
  const childLink = await D.getByRole("link", { name: /DOC-029r2 roles confidential/ }).count();
  check(/Restricted Matter/.test(main) && /Restricted contract/.test(main) && childLink === 0, "restricted placeholders missing or links present");
...
await step(RA, "administrator", "B4.list-search", "Search and lists omit Confidential records the person cannot reach.", async () => {
  const titles = async (module) => {
    const all = [];
    let cursor = null;
    do {
      const page = await api(D, "GET", `/${module}${cursor ? `?cursor=${encodeURIComponent(cursor)}` : ""}`);
  ...
  await search.fill("DOC-029r2 roles");
  await sleep(2500);
  const table = (await D.getByRole("listbox").first().innerText().catch(async () => D.locator("body").innerText())).replace(/\s+/g, " ");
  await D.keyboard.press("Escape");
  check(/DOC-029r2 roles open/.test(table), "open records missing from search");
  check(!/DOC-029r2 roles confidential/.test(table), "listed");
  check(ct.includes("DOC-029r2 roles open contract") && !ct.includes("DOC-029r2 roles confidential contract") && !mt.includes("DOC-029r2 roles confidential matter"), "API lists it");
  return `Daniel\'s header Search for "DOC-029r2 roles" offered "${table.slice(0, 200)}" and no Confidential title; the full Contracts list (${ct.length} rows) and Matters list (${mt.length} rows) omitted C-${CC} and M-${MC}.`;
});

await step(RA, "legal_team_member", "B5.team-add-admin", "Nadia (Creator, Legal Owner) opens Contract team, selects Add team member, chooses Daniel in Person and selects Add. Each person appears once with no role picker. Daniel then reaches the record.", async () => {
...
await step(RA, "administrator", "B6.admin-audience", "An Administrator who can reach the Confidential record may change its audience.", async () => {
...
await step(RA, "legal_team_member", "B7.reader-no-audience-right", "Being able to read a Confidential record does not give the right to change its audience (Priya, Legal Team Member on the team, not Creator or Legal Owner).", async () => {
  await go(N, `/contracts/${CC}`);
...
await step(RA, "legal_team_member", "B8.remove-ends-access", "Removing a team row with the person's remove control ends Confidential access on the next read.", async () => {
  await go(N, `/contracts/${CC}`);
...
await step(RA, "legal_team_member", "B9.legal-owner", "On a Confidential Contract, being Legal Owner admits an Administrator without a team row; changing the owner back refuses them again.", async () => {
  await go(N, `/contracts/${CC}`);
...
await step(RA, "legal_team_member", "B10.matter-manager-and-team", "On a Confidential Matter, a team row or Matter Manager admits the person; removal refuses them.", async () => {
...
await step(RA, "legal_team_member", "B11.ltm-removal-open", "Removing a Legal Team Member from an open record does not end their ordinary access; a Confidential Document narrows to the named audience.", async () => {
  ...
  check(!before && onTeam && !afterDoc && danielDoc, JSON.stringify({ before, onTeam, afterDoc, danielDoc }));
...
await step(RA, "administrator", "C1.entity-admin-refused", "Every person needs a Grant for a Confidential Entity, Administrators too.", async () => {
...
await step(RA, "legal_team_member", "C2.entity-grant", "The creator has a Grant. A Legal Team Member with a Grant uses Manage access to give and remove Grants. A Grant applies to that Entity only.", async () => {
  ...
  check(await dialog.getByRole("button", { name: "Remove Nadia Haddad" }).isVisible(), "creator Grant missing");
  const options = await dialog.getByRole("combobox", { name: "Person" }).locator("option").allInnerTexts();
...
await step(RA, "legal_team_member", "C3.last-grant", "A Confidential Entity must keep at least one active person with a Grant.", async () => {
  await go(N, `/entities/${ENTITY}`);
  await N.getByRole("button", { name: "Manage access" }).click();
  const dialog = N.getByRole("dialog", { name: "Confidential access" });
  await dialog.waitFor();
  ...
  if (enabled) {
    ...
    if (await confirm.isVisible({ timeout: 1500 }).catch(() => false)) await confirm.getByRole("button").last().click();
    await sleep(2000);
    message = (await dialog.getByRole("alert").or(N.getByRole("status")).first().innerText().catch(() => "no alert text")).replace(/\s+/g, " ");
  }
...
await step(RA, "administrator", "C4.open-entity-manage", "An Administrator can manage access on an Entity that is not Confidential; a Legal Team Member without a Grant cannot.", async () => {
  const list = await api(D, "GET", "/entities");
  const open = list.json.entities.find((e) => !e.isConfidential && !e.archivedAt && /^Helix/.test(e.legalName));
  await go(D, `/entities/${open.id}`);
  const danielManage = await D.getByRole("button", { name: "Manage access" }).isVisible().catch(() => false);
  await go(N, `/entities/${open.id}`);
  const nadiaManage = await N.getByRole("button", { name: "Manage access" }).isVisible().catch(() => false);
...
// ===== Business User reach =====
await step(RA, "business_user", "D1.full-app-closed", "Business Users use the Portal; they cannot open the Entities module or full-app records and search.", async () => {
  const seen = [];
  for (const url of [`/contracts/${CO}`, "/search", "/entities", `/entities/${ENTITY}`, "/settings/users"]) {
    await go(J, url);
...
await step(RA, "business_user", "D2.not-on-team", "A Business User reaches only records whose teams include them.", async () => {
  const out = [];
  for (const url of [`/portal/contracts/${CO}`, `/portal/contracts/${CC}`, `/portal/matters/${MO}`, `/portal/matters/${MC}`]) {
    await go(J, url);
    out.push(`${url}: ${(await h1(J))}`);
  }
  check(out.every((o) => /not found/.test(o)), out.join("; "));
  return out.join("; ");
});

await step(RA, "business_user", "D3.conversion", "At conversion the Requester becomes Business Owner and a team member; the old Request address leads to the record.", async () => {
  ...
  check(title.includes("Jonas converted contract") && rows.some((r) => /Business Owner.*Jonas Weber/.test(r)) && list.includes(`C-${CJ}`), `${landed} ${title} ${rows}`);
  return `/portal/requests/${RJ} showed "${title}" (address ${landed}). Contract team rows: ${rows.join(" | ")}. Your Contracts lists C-${CJ}.`;
});

await step(RA, "legal_team_member", "D4.add-business-user", "Adding a Business User to the team gives them Portal access to the record.", async () => {
  await go(N, `/contracts/${CO}`);
  await addTeamMember(N, "Contract team", "Jonas Weber");
  const c = await openRecord(J, `/portal/contracts/${CO}`);
  check(!c.refused && c.title === "DOC-029r2 roles open contract", c.title);
  await go(J, "/portal/contracts");
  await J.getByRole("searchbox", { name: "Search Contracts" }).fill("DOC-029r2 roles open contract");
  await J.getByRole("searchbox", { name: "Search Contracts" }).press("Enter");
  await settle(J);
...
await step(RA, "business_user", "D5.comments-tiers", "Business Users read and post Full thread comments; they do not see Legal only or Working team; a later-added member reads the Full Thread history.", async () => {
  const reply = `DOC-029r2 roles reply from Jonas ${Date.now().toString().slice(-6)}`;
  await go(J, `/portal/contracts/${CO}`);
  const panel = await applet(J, "Comments");
  await sleep(1000);
  const text = (await panel.innerText()).replace(/\s+/g, " ");
  const audience = await panel.getByRole("radiogroup").or(panel.getByRole("group", { name: "Audience" })).count();
  ...
  const legal = await Promise.resolve().then(async () => {
    ...
    const radios = await p.getByRole("radio").evaluateAll((els) => els.map((e) => e.getAttribute("aria-label") ?? e.id));
    const radioLabels = await p.getByRole("group", { name: "Audience" }).innerText().catch(() => "");
    return { item: (await item.innerText()).replace(/\s+/g, " "), radios: radioLabels.replace(/\s+/g, " ") };
  });
  const fullRow = (await panel.getByRole("listitem").filter({ hasText: "DOC-029r2 roles Full thread note" }).first().innerText()).replace(/\s+/g, " ");
  const chipText = text.replace(/DOC-029r2 roles [A-Za-z ]*note/g, "");
  check(text.includes("DOC-029r2 roles Full thread note") && !text.includes("DOC-029r2 roles Legal only note") && !text.includes("DOC-029r2 roles Working team note") && audience === 0, text.slice(0, 300));
  check(/Contract Team/.test(fullRow.replace("DOC-029r2 roles Full thread note", "")) && !/Legal Only|Internal team/.test(chipText), `row "${fullRow}"`);
  return `Jonas joined the C-${CO} team after the three fixture notes were posted. His Comments showed the earlier Full Thread note with the Contract Team label ("${fullRow.slice(0, 110)}") and neither the Legal Only note nor the older Working Team note; no Legal Only or Internal team label and no audience picker. His reply posted. Nadia saw it as "${legal.item.slice(0, 120)}". Nadia's audience choices read "${legal.radios}".`;
});

await step(RA, "legal_team_member", "D5b.tier-labels", "In the Comments applet a Full Thread comment shows Contract Team on a Contract and Matter Team on a Matter; Legal Only and older Internal team comments show those labels; Legal chooses Contract Team or Matter Team above New comment, not Legal Only.", async () => {
  await go(N, `/contracts/${CO}`);
  const lp = await applet(N, "Comments");
  await lp.getByText("DOC-029r2 roles Full thread note").waitFor({ timeout: 20000 });
  const group = lp.getByRole("group", { name: "Audience" });
  const audience = (await group.innerText()).replace(/\s+/g, " ");
  const gBox = await group.boundingBox();
  const tBox = await lp.getByRole("textbox", { name: "New comment" }).boundingBox();
  const row = async (body) => (await lp.getByRole("listitem").filter({ hasText: body }).first().innerText()).replace(/\s+/g, " ").replace(body, "");
  const fullRow = await row("DOC-029r2 roles Full thread note");
  const legalRow = await row("DOC-029r2 roles Legal only note");
  const workRow = await row("DOC-029r2 roles Working team note");
  await N.screenshot({ path: path.join(here, "r2-legal-audience-labels.png") });
  await go(N, `/matters/${MO}`);
  const mp = await applet(N, "Comments");
  const maudience = (await mp.getByRole("group", { name: "Audience" }).innerText()).replace(/\s+/g, " ");
  check(/Legal Only/.test(audience) && /Contract Team/.test(audience) && /Legal Only/.test(maudience) && /Matter Team/.test(maudience), `audience ${audience} / ${maudience}`);
  check(gBox.y < tBox.y, `Audience y ${gBox.y}, New comment y ${tBox.y}`);
  check(/Contract Team/.test(fullRow) && /Legal Only/.test(legalRow) && /Internal team/.test(workRow), JSON.stringify({ fullRow, legalRow, workRow }));
  return `Contract Audience, above New comment, offered "${audience}"; Matter Audience offered "${maudience}". Row labels without the bodies: Full Thread note "${fullRow.slice(0, 80)}"; Legal Only note "${legalRow.slice(0, 80)}"; older Working Team note "${workRow.slice(0, 80)}". See r2-legal-audience-labels.png.`;
});
...
async function legalComment(page, url, audienceLabel, body) {
  ...
  return (await panel.getByRole("listitem").filter({ hasText: body }).first().innerText()).replace(/\s+/g, " ").replace(body, "");
}
async function portalHistory(page, url) {
  await go(page, url);
  const panel = await applet(page, "History");
  await sleep(800);
  const text = (await panel.innerText()).replace(/\s+/g, " ");
  // Actor and verb of the newest row only; the relative time ("this minute") can change between reads.
  return { text, top: text.replace(/^History\s*/, "").split(" ").slice(0, 3).join(" ") };
}
const activityCount = async (page, entityType, entityId) => {
  const r = await api(page, "GET", `/portal/activity?entityType=${entityType}&entityId=${entityId}`);
  const entries = r.json?.entries ?? [];
  return { status: r.status, first: entries[0]?.id ?? null, firstActor: entries[0]?.actor?.displayName ?? null, firstAction: entries[0]?.action ?? null, actions: entries.map((e) => e.action) };
};

await step(RA, "business_user", "D6.history-portal-limits", "The Portal History applet shows Contract Team comment activity and excludes Legal Only and Internal team activity. The Portal excludes legal Fields, Tasks, Key dates, Approvals and signing controls.", async () => {
  const stamp = Date.now().toString().slice(-6);
  const h0 = await portalHistory(J, `/portal/contracts/${CO}`);
  const a0 = await activityCount(J, "contract", fx.openContract.id);
  const legalChip = await legalComment(D, `/contracts/${CO}`, "Legal Only", `DOC-029r2 roles r2 Legal Only probe ${stamp}`);
  const older = await api(D, "POST", "/comments", { entityType: "contract", entityId: fx.openContract.id, body: `DOC-029r2 roles r2 Internal team probe ${stamp}`, visibility: "working_team" });
  await sleep(1000);
  const h1x = await portalHistory(J, `/portal/contracts/${CO}`);
  const a1 = await activityCount(J, "contract", fx.openContract.id);
  const teamChip = await legalComment(D, `/contracts/${CO}`, "Contract Team", `DOC-029r2 roles r2 Contract Team probe ${stamp}`);
  await sleep(1000);
  const h2 = await portalHistory(J, `/portal/contracts/${CO}`);
  const a2 = await activityCount(J, "contract", fx.openContract.id);
  const main = await J.getByRole("main").innerText();
  const forbidden = ["Tasks", "Key dates", "Approvals", "Signature", "Send for signature"].filter((w) => new RegExp(`\\b${w}\\b`).test(main));
  const stageButtons = await J.getByRole("button", { name: /move contract/ }).count();
  check(older.status < 300, `working_team probe ${older.status}`);
  check(/Legal Only/.test(legalChip) && /Contract Team/.test(teamChip), `${legalChip} / ${teamChip}`);
  check(a1.first === a0.first && h1x.top === h0.top && !/^Daniel Okafor/.test(h1x.top), `after Legal Only and Internal team: newest entry ${a0.first}->${a1.first}, top row "${h0.top}" -> "${h1x.top}"`);
  check(a2.first !== a1.first && a2.firstActor === "Daniel Okafor" && /^comment\./.test(a2.firstAction) && /^Daniel Okafor commented/.test(h2.top), `after Contract Team: newest ${a2.firstActor} ${a2.firstAction}, top row "${h2.top}"`);
  check(forbidden.length === 0 && stageButtons === 0, `${forbidden}`);
  return `Jonas's Portal History top row read "${h0.top}". Daniel posted a comment with Audience Legal Only (row label "${legalChip.slice(0, 60)}") and an older-tier Working Team comment (API stand-in, ${older.status}); the top row stayed "${h1x.top}" and the newest Portal activity entry did not change. Daniel then posted with Audience Contract Team (label "${teamChip.slice(0, 60)}"); the top row became "${h2.top}" (${a2.firstAction}). No Tasks, Key dates, Approvals or signing controls, and no Stage move buttons, on the Portal record.`;
});

await step(RA, "business_user", "D6b.history-no-record-changes", "Record changes do not appear in Portal History; the record details and Fields show the current values.", async () => {
  const h0 = await portalHistory(J, `/portal/contracts/${CO}`);
  const a0 = await activityCount(J, "contract", fx.openContract.id);
  await go(N, `/contracts/${CO}`);
  await h1(N);
  ...
  await settle(N);
  await sleep(1500);
  const descText = `DOC-029r2 roles r2 shared description ${Date.now().toString().slice(-6)}`;
  ...
  await go(J, `/portal/contracts/${CO}`);
  const overview = (await J.getByRole("region", { name: "Overview" }).innerText()).replace(/\s+/g, " ");
  const fields = (await J.getByRole("region", { name: "Fields" }).innerText()).replace(/\s+/g, " ");
  const h1b = await portalHistory(J, `/portal/contracts/${CO}`);
  const a1 = await activityCount(J, "contract", fx.openContract.id);
  await J.screenshot({ path: path.join(here, "r2-portal-history.png") });
  await go(N, `/contracts/${CO}`);
  await h1(N);
  await N.getByRole("combobox", { name: "Region" }).first().selectOption({ label: "EMEA" });
  await settle(N);
  check(/changed Region/.test(legalHist), `Legal History did not record the change: ${legalHist.slice(0, 160)}`);
  check(/APAC/.test(overview) && fields.includes(descText), `Overview ${overview.slice(0, 160)} / Fields ${fields.slice(0, 160)}`);
  check(a1.first === a0.first && h1b.top === h0.top && !/changed/.test(h1b.text) && !a1.actions.includes("contract.updated"), `Portal History top "${h0.top}" -> "${h1b.top}": ${h1b.text.slice(0, 160)}; actions ${a1.actions}`);
  return `Nadia changed Region to APAC and the Description in the full app; her History read "${legalHist.slice(0, 120)}". Jonas's Portal Overview showed APAC and Fields showed the new Description. His Portal History top row stayed "${h1b.top}", no row said "changed", and the Portal activity read had no contract.updated entry. See r2-portal-history.png. Nadia returned Region to EMEA.`;
});

await step(RA, "business_user", "D7.portal-team-add", "Business Users may add existing people from the Portal on non-Confidential records.", async () => {
  await go(J, `/portal/contracts/${CO}`);
...
await step(RA, "legal_team_member", "D8.business-owner-statements", "Owner statements appear on the person's row in both apps. Changing Business Owner does not add or remove membership. Removing membership leaves statements visible, and statements alone do not grant Portal access; removal ends access on the next read, including old links.", async () => {
  await go(N, `/contracts/${CO}`);
  ...
  const comm = await api(J, "GET", `/comments?entityType=contract&entityId=${fx.openContract.id}`);
  await go(N, `/matters/${MO}`);
  await pickPerson(N, "Business Owner", "Jonas Weber");
  ...
  await go(N, `/matters/${MO}`);
  await pickPerson(N, "Business Owner", "Unassigned");
  check(fullRows.some((r) => /Business Owner.*Jonas Weber/.test(r)) && portalRows.some((r) => /Business Owner.*Jonas Weber/.test(r)), "statement rows");
  check(jRefused && docs.status >= 400 && comm.status >= 400, `refused ${jRefused} docs ${docs.status} comments ${comm.status}`);
  check(!mRows.some((r) => r.includes("Jonas Weber") && !/Business Owner/.test(r)) && /not found/.test(jm.title), `${mRows} ${jm.title}`);
  return `Full-app row: ${fullRows.filter((r) => r.includes("Jonas")).join()}; Portal row: ${portalRows.filter((r) => r.includes("Jonas")).join()}. After Nadia removed Jonas's membership the full-app roster showed: ${afterRows.join(" | ")}. Jonas's reload of the old link showed "Contract not found" and the Documents read answered ${docs.status} and the Comments read answered ${comm.status}. Setting Jonas as Business Owner on M-${MO} left him outside the team (${mRows.join(" | ")}) and the Portal refused "${jm.title}". Both Business Owners were returned to Unassigned.`;
});

await step(RA, "business_user", "D9.related-no-reach", "Legal chooses Matter Team for a message to the business; a Full Thread comment shows Matter Team on a Matter and appears in Portal History; Legal Only stays hidden. A link or parent relation does not grant a Business User access to the related record.", async () => {
  await go(N, `/matters/${MO}`);
  await addTeamMember(N, "Matter team", "Jonas Weber");
  const stamp = Date.now().toString().slice(-6);
  const teamChip = await legalComment(N, `/matters/${MO}`, "Matter Team", `DOC-029r2 roles r2 Matter Team note ${stamp}`);
  const legalChip = await legalComment(N, `/matters/${MO}`, "Legal Only", `DOC-029r2 roles r2 Matter Legal Only note ${stamp}`);
  const m = await openRecord(J, `/portal/matters/${MO}`);
  const main = await J.getByRole("main").innerText();
  const jc = await applet(J, "Comments");
  await jc.getByText(`DOC-029r2 roles r2 Matter Team note ${stamp}`).waitFor({ timeout: 15000 });
  const jText = (await jc.innerText()).replace(/\s+/g, " ");
  const jRow = (await jc.getByRole("listitem").filter({ hasText: `DOC-029r2 roles r2 Matter Team note ${stamp}` }).first().innerText()).replace(/\s+/g, " ");
  const mh = await portalHistory(J, `/portal/matters/${MO}`);
  results.d9labels = { teamChip, legalChip, jRow, historyTop: mh.top };
  check(/Matter Team/.test(teamChip) && /Legal Only/.test(legalChip) && /Matter Team/.test(jRow) && !jText.includes(`Matter Legal Only note ${stamp}`) && !/Legal Only/.test(jText) && /^Nadia Haddad commented/.test(mh.top), JSON.stringify(results.d9labels));
  ...
  check(!m.refused && !/DOC-029r2 roles confidential (contract|matter)/.test(main) && /not found/.test(c.title) && /not found/.test(child.title), JSON.stringify({ m, c, child }));
  return `Nadia posted with Audience Matter Team (label "${teamChip.slice(0, 50)}") and with Legal Only on M-${MO}. Jonas saw the Matter Team comment ("${jRow.slice(0, 90)}"), not the Legal Only one, and his Portal History top row read "${mh.top}". With Jonas on the M-${MO} team he opened it (${/DOC-029r2 roles confidential (contract|matter)/.test(main) ? "a Confidential record title appeared" : "no Confidential record title appeared"}), but the linked C-${CC} showed "${c.title}" and the child M-${MC} showed "${child.title}". Nadia removed him afterwards.`;
});

await step(RA, "business_user", "D10.confidential-portal", "A Business User always needs a team row on a Confidential record; Confidential team changes stay with Legal.", async () => {
  await go(N, `/contracts/${CC}`);
  await addTeamMember(N, "Contract team", "Jonas Weber");
  const c = await openRecord(J, `/portal/contracts/${CC}`);
  const panel = await applet(J, "Contract team");
...
await step(RA, "business_user", "D11.archived-request", "An archived record leaves the Portal; the old Request link shows the original ask and an archived notice, without Documents or a conversation.", async () => {
  const archived = await api(N, "POST", `/contracts/${CJ}/archive`, {});
  check(archived.status < 300, `archive ${archived.status}`);
  await go(J, "/portal/contracts");
  await J.getByRole("searchbox", { name: "Search Contracts" }).fill("DOC-029r2 roles Jonas");
  await J.getByRole("searchbox", { name: "Search Contracts" }).press("Enter");
  await settle(J);
  const list = await J.getByRole("main").innerText();
  ...
  const comments = await J.getByRole("textbox", { name: "New comment" }).count() + (await J.getByRole("button", { name: "Comments" }).count());
  const restored = await api(N, "POST", `/contracts/${CJ}/restore`, {});
  check(!list.includes(`C-${CJ}`) && /not found/.test(direct.title) && /archived/.test(main) && main.includes("DOC-029r2 roles fictional ask from Jonas") && docs === 0 && comments === 0, main.slice(0, 300));
...
// Round 1 archived the 25 paging Matters in Z1. Restore them so Your Matters pages again (G4); Z1 archives them again.
await step(CG, "legal_team_member", "Z0.restore-paging", "Fixture: restore the 25 paging Matters archived at the end of round 1.", async () => {
  const codes = [];
  for (const m of fx.pagingMatters) codes.push((await api(N, "POST", `/matters/${m.number}/restore`, {})).status);
  check(codes.every((c) => c < 300 || c === 409), codes.join(","));
  return `Restore answered ${[...new Set(codes)].join(",")} for ${codes.length} paging Matters.`;
});

await step(CG, "business_user", "G0.fixture", "Fixture: Ravi submits a fresh Contract review Request; Legal converts it and uploads the primary Document (API stand-ins for the Portal form, the Inbox conversion and the Legal upload).", async () => {
  const stamp = Date.now().toString().slice(-6);
  ...
  const rq = await api(R, "POST", "/requests", { requestTypeId: rt.requestType.id, departmentId: sales, title: `DOC-029r2 roles Ravi request ${stamp}`, description: "DOC-029r2 roles fictional ask from Ravi.", urgency: "medium" });
  check(rq.status === 201, `request ${rq.status}`);
  const nda = (await api(D, "GET", "/contract-types")).json.contractTypes.find((t) => t.displayName === "NDA").id;
  const cv = await api(N, "POST", `/requests/${rq.json.request.number}/convert`, { title: `DOC-029r2 roles Ravi converted contract ${stamp}`, contractTypeId: nda });
  check(cv.status === 200, `convert ${cv.status}`);
  CR = cv.json.request.convertedRecord.number;
  const res = await N.request.post(`${BASE}/api/v1/contracts/${CR}/documents`, { headers: { origin: BASE }, multipart: { file: { name: "doc029-roles-supporting-v1.txt", mimeType: "text/plain", buffer: readFileSync(path.join(here, "fixtures", "doc029-roles-supporting-v1.txt")) } } });
...
await step(CG, "business_user", "G1.before-start", "Before Legal adds Ravi, the Matter is absent and refused. After Legal adds him, he selects Matters in the Portal navigation, chooses the record, and checks title and M- reference. A link or related record does not grant access.", async () => {
  ...
  await settle(R);
  await revealLink(R, "DOC-029r2 roles Ravi matter");
  await R.getByRole("link", { name: "DOC-029r2 roles Ravi matter" }).click();
  await settle(R);
  const title = await h1(R);
  const main = await R.getByRole("main").innerText();
  const comparable = await openRecord(R, `/portal/matters/${MO}`);
  check(/not found/.test(before.title) && title === "DOC-029r2 roles Ravi matter" && main.includes(`M-${MR}`) && /not found/.test(comparable.title), `${before.title} ${title}`);
...
await step(CG, "business_user", "G2.find-search", "Enter a title or reference in Search and press Enter or select Search. Contract search also matches the primary Counterparty.", async () => {
  const cp = await api(N, "POST", `/contracts/${CR}/counterparties`, { name: "DOC-029r2 Roles Fictional Supplier Ltd" });
  await go(R, "/portal/matters");
  const box = R.getByRole("searchbox", { name: "Search Matters" });
  await box.fill("DOC-029r2 roles Ravi");
  ...
  check(byTitle.includes(`M-${MR}`) && byRef.includes(`M-${MR}`) && byCp.includes(`C-${CR}`) && !byRef.includes("paging"), `${byTitle} / ${byRef} / ${byCp}`);
  return `Search "DOC-029r2 roles Ravi" + Enter listed M-${MR} (address ${new URL(url1).search}); "M-${MR}" + Search listed only M-${MR}; Contracts search "Roles Fictional Supplier" listed C-${CR} by its primary Counterparty (added by Legal, ${cp.status}).`;
});

await step(CG, "business_user", "G3.filter", "Select Filter, choose a property and value, select Apply. Contracts offer Stage, Type, Legal Owner and Expiry date; Matters offer Status, Type, Matter Manager and open or closed lifecycle. Filters combine with search; remove a chip or Clear all.", async () => {
  await go(R, "/portal/contracts");
  ...
  await settle(R);
  const box = R.getByRole("searchbox", { name: "Search Matters" });
  await box.fill("DOC-029r2 roles");
  await box.press("Enter");
  await settle(R);
  ...
  check(["Type", "Legal Owner", "Stage", "Expiry date"].every((p) => cprops.includes(p)) && ["Type", "Matter Manager", "Status", "Lifecycle"].every((p) => mprops.includes(p)) && chips === 1 && cleared === 0, `${cprops} / ${mprops} / chips ${chips}`);
  return `Contracts Filter offered ${cprops.join(", ")}; Matters Filter offered ${mprops.join(", ")}. Lifecycle choices: "${lifeChoices.slice(0, 80)}". Lifecycle Open plus search "DOC-029r2 roles" showed "${status}" with one filter chip (address ${url}). Clear all removed the chip.`;
});

await step(CG, "business_user", "G4.sort-columns-more-back", "Heading sort cycles ascending, descending, default. Columns chooses and reorders columns; a heading edge resizes. Show more adds the next page. Search, filters and sort stay in the address for Back and bookmarks.", async () => {
  await go(R, "/portal/matters");
  ...
  check(rowsAfter > rowsBefore && /sort=/.test(s1) && /sort=/.test(s2) && !/sort=/.test(s3) && back === bookmarked && Math.abs(box2.width - box.width) > 40, JSON.stringify({ rowsBefore, rowsAfter, s1, s2, s3, back, bookmarked, w1: box.width, w2: box2.width }));
...
await step(CG, "business_user", "G5.read-record", "Record details and Fields show current full-app values; Contracts and Matters both show Department and Region; a Contract shows Overview with Value and dates. Business Fields and Description are read-only; Legal's change appears on reload. Original request is read-only and does not change when Legal edits the live Description.", async () => {
  ...
  const editable = await R.getByRole("main").locator("input:not([type=file]):not([type=search]), textarea, select, [contenteditable=true]").evaluateAll((els) => els.filter((e) => !e.closest("[role=search]") && e.getAttribute("aria-label") !== "Search Documents" && !e.closest("form[role=search]")).map((e) => e.getAttribute("aria-label") ?? e.tagName));
  await go(N, `/contracts/${CR}`);
  await h1(N);
  const desc = N.getByRole("textbox", { name: "Description" });
  await desc.fill("DOC-029r2 roles description updated by Legal.");
  await desc.blur();
  await settle(N);
  await sleep(1500);
  await R.reload();
  ...
  check(/Department/.test(mMain) && /Region/.test(mMain) && /Department/.test(overview) && /Region/.test(overview) && /Value/.test(overview) && /Effective date/.test(overview) && fieldsAfter.includes("DOC-029r2 roles description updated by Legal.") && original.includes("DOC-029r2 roles fictional ask from Ravi.") && editable.length === 0, JSON.stringify({ editable, fieldsAfter, history }));
...
await step(CG, "business_user", "G6.documents-read", "Documents lists the Primary Document first with current Version, kind, uploader and date; the name opens a preview (processing may finish after upload); the download control downloads.", async () => {
  const pdf = await N.request.post(`${BASE}/api/v1/contracts/${CR}/documents`, { headers: { origin: BASE }, multipart: { file: { name: "doc029-roles-preview.pdf", mimeType: "application/pdf", buffer: readFileSync(path.join(here, "fixtures", "doc029-roles-preview.pdf")) } } });
  ...
  for (let i = 0; i < 12; i++) {
    ...
    const rendered = (await viewer.locator("canvas, img, iframe, embed, object").count()) + (/DOC-029 roles fictional preview paper/.test(previewText) ? 1 : 0);
  ...
  await viewer.getByRole("button", { name: "Close the document" }).click();
  const href = await region.getByRole("link", { name: /Download doc029-roles-supporting-v1.txt, version 1/ }).getAttribute("href");
  const dl = await R.request.get(new URL(href, BASE).toString());
  check(/^Primary Document doc029-roles-supporting-v1.txt Version 1 .*Nadia Haddad/.test(rows[0]) && dl.status() === 200 && shape > 0, JSON.stringify({ rows, shape, previewText }));
...
await step(CG, "business_user", "G7.upload-new-documents", "Select Upload documents, choose New documents under Add as, choose Kind and optional Note, select Upload. Kind and Note apply to every file. Successful files stay if another fails; Retry failed uploads retries only the failures.", async () => {
  await go(R, `/portal/contracts/${CR}`);
  ...
  await R.route(`**/api/v1/contracts/${CR}/documents`, async (route) => {
    ...
    if (!failedOnce && body.includes("doc029-roles-confidential-note.txt")) {
      failedOnce = true;
      return route.fulfill({ status: 503, contentType: "application/problem+json", body: JSON.stringify({ title: "Injected failure", status: 503, detail: "DOC-029 injected upload failure." }) });
    }
  ...
  await dialog.locator("input[type=file]").setInputFiles([path.join(here, "fixtures", "doc029-roles-business-upload.txt"), path.join(here, "fixtures", "doc029-roles-confidential-note.txt")]);
  const kinds = await dialog.getByRole("combobox", { name: "Kind" }).locator("option").allInnerTexts();
  await dialog.getByRole("combobox", { name: "Kind" }).selectOption({ index: 0 });
  await dialog.getByRole("textbox", { name: "Note (optional)" }).fill("DOC-029r2 roles business note");
  ...
  const text = (await R.getByRole("region", { name: "Documents" }).innerText()).replace(/\s+/g, " ");
  check(posts === 3 && names.filter((n) => n === "doc029-roles-business-upload.txt").length === 1 && uploads.length === 2 && /^Primary Document/.test((await R.getByRole("region", { name: "Documents" }).getByRole("listitem").first().innerText()).trim()) && uploads.every((d) => d.versions[0].note === "DOC-029r2 roles business note"), JSON.stringify({ posts, names, notes: uploads.map((d) => d.versions[0].note) }));
...
await step(CG, "business_user", "G8.add-version-signed-copy", "Add version opens the dialog with that Document selected and takes one file; it works on the primary Contract Document. Earlier versions expand; Signed copy marks the Version Legal pinned. Uploading a Version changes neither designation.", async () => {
  const pin = await api(N, "PUT", `/documents/${raviPrimary.id}/executed-version`, { versionId: raviPrimary.v1 });
  const pin2 = pin.status >= 400 ? await api(N, "POST", `/documents/${raviPrimary.id}/executed-version`, { versionId: raviPrimary.v1 }) : pin;
  await go(R, `/portal/contracts/${CR}`);
  const region = R.getByRole("region", { name: "Documents" });
  const row = region.getByRole("listitem").filter({ hasText: "doc029-roles-supporting-v1.txt" }).first();
  ...
  await dialog.locator("input[type=file]").setInputFiles(path.join(here, "fixtures", "doc029-roles-supporting-v2.txt"));
  await dialog.getByRole("button", { name: "Upload", exact: true }).click();
  await dialog.waitFor({ state: "hidden", timeout: 20000 });
  await settle(R);
  const row2 = region.getByRole("listitem").filter({ hasText: "doc029-roles-supporting" }).first();
  ...
  check(hadHistory === 0 && /New version of/.test(selected) && multiple === null && /Version 2/.test(text) && /Signed copy/.test(text) && /^Primary Document/.test(first) && /Primary Document/.test(text), JSON.stringify({ hadHistory, selected, multiple, text, first }));
...
await step(CG, "business_user", "G9.drop-and-no-folders", "Files can be dropped onto Documents; Business Users cannot manage folders or archive paper.", async () => {
  await go(R, `/portal/contracts/${CR}`);
  const region = R.getByRole("region", { name: "Documents" });
  const target = region.getByRole("button", { name: "Drop files here or click to upload" });
  const dt = await R.evaluateHandle(() => {
    const d = new DataTransfer();
    d.items.add(new File(["DOC-029r2 roles dropped paper"], "doc029-roles-dropped.txt", { type: "text/plain" }));
    return d;
  });
...
await step(CG, "business_user", "G10.search-documents", "Search Documents finds a Document by name or any Version's filename; Show more loads another page.", async () => {
  await go(R, `/portal/contracts/${CR}`);
  const region = R.getByRole("region", { name: "Documents" });
  ...
  await settle(R);
  const byName = await dregion.getByRole("button", { name: /^doc029-roles-page-/ }).allInnerTexts();
  check(/doc029-roles-supporting-v1.txt/.test(byOld) && /Version 2/.test(byOld) && before === 50 && after === 51 && byName.length === 1, JSON.stringify({ before, after, byName }));
...
await step(CG, "business_user", "G11.comments", "Open Comments, write a reply, attach a file, select Comment; @ mentions people who can hear it; Comment actions edits or deletes own comment; unread badge clears after load; draft and attachments survive switching applets; no audience picker.", async () => {
  const tag = `DOC-029r2 roles ask ${Date.now().toString().slice(-6)}`;
  await api(N, "POST", "/comments", { entityType: "matter", entityId: fx.raviMatter.id, body: "DOC-029r2 roles Legal reply for Ravi", visibility: "full_thread" });
  await go(R, `/portal/matters/${MR}`);
  const toolbarBtn = R.getByRole("toolbar", { name: "Applets" }).getByRole("button", { name: /^Comments/ });
  const badgeBefore = (await toolbarBtn.innerText()).trim() + " " + ((await toolbarBtn.getAttribute("aria-label")) ?? "");
  ...
  await box.fill("DOC-029r2 roles draft kept ");
  await panel.locator("input[type=file]").setInputFiles(path.join(here, "fixtures", "doc029-roles-business-upload.txt"));
  ...
  const mentionOptions = (await listbox.getByRole("option").allInnerTexts()).map((s) => s.replace(/\s+/g, " "));
  await listbox.getByRole("option").first().click();
  await p2.locator("input[type=file]").setInputFiles(path.join(here, "fixtures", "doc029-roles-business-upload.txt"));
  ...
  check(pickers === 0 && switched.startsWith("DOC-029r2 roles draft kept") && kept.startsWith("DOC-029r2 roles draft kept") && keptFile && posted.includes("doc029-roles-business-upload.txt") && mentionOptions.some((o) => /Nadia/.test(o)) && (!final.includes(`${tag} edited`) || /deleted/i.test(final)), JSON.stringify({ pickers, kept, keptFile, posted, mentionOptions, final: final.slice(0, 200) }));
...
await step(CG, "business_user", "G11b.reply-tier-label", "Your reply shows Contract Team on a Contract or Matter Team on a Matter (the Full Thread audience); the text below the composer names it; Legal Only and older Internal team messages stay outside this view.", async () => {
  const stamp = Date.now().toString().slice(-6);
  const hiddenL = await api(N, "POST", "/comments", { entityType: "matter", entityId: fx.raviMatter.id, body: `DOC-029r2 roles r2 hidden Legal Only ${stamp}`, visibility: "legal_only" });
  const hiddenW = await api(N, "POST", "/comments", { entityType: "matter", entityId: fx.raviMatter.id, body: `DOC-029r2 roles r2 hidden Internal team ${stamp}`, visibility: "working_team" });
  const out = {};
  for (const [label, url, module] of [["Matter Team", `/portal/matters/${MR}`, "Matter"], ["Contract Team", `/portal/contracts/${CR}`, "Contract"]]) {
    await go(R, url);
    const panel = await applet(R, "Comments");
    await sleep(1000);
    const body = `DOC-029r2 roles r2 Ravi reply on ${module} ${stamp}`;
    const composer = (await panel.innerText()).replace(/\s+/g, " ");
    const hint = (composer.match(/Visible to [^.]*\./) ?? [""])[0];
    await panel.getByRole("textbox", { name: "New comment" }).fill(body);
    await panel.getByRole("button", { name: "Comment", exact: true }).click();
    const item = panel.getByRole("listitem").filter({ hasText: body }).first();
    await item.waitFor({ timeout: 15000 });
    const row = (await item.innerText()).replace(/\s+/g, " ").replace(body, "");
    const text = (await panel.innerText()).replace(/\s+/g, " ");
    if (module === "Matter") await R.screenshot({ path: path.join(here, "r2-portal-reply-tier-label.png") });
    out[module] = { row, hint, hidden: text.includes(`hidden Legal Only ${stamp}`) || text.includes(`hidden Internal team ${stamp}`), otherLabels: /Legal Only|Internal team/.test(text) };
    check(new RegExp(label).test(row) && hint === `Visible to Legal and all ${module} team members.`, `${module}: row "${row}", hint "${hint}"`);
  }
  check(hiddenL.status < 300 && hiddenW.status < 300 && !out.Matter.hidden && !out.Matter.otherLabels && !out.Contract.otherLabels, JSON.stringify(out));
  return `On M-${MR} Ravi's reply row read "${out.Matter.row.slice(0, 80)}" and the composer text read "${out.Matter.hint}"; on C-${CR} the row read "${out.Contract.row.slice(0, 80)}" and the composer text read "${out.Contract.hint}". Legal Only and Working Team comments posted by Nadia on M-${MR} (API stand-ins, ${hiddenL.status}/${hiddenW.status}) did not appear, and no Legal Only or Internal team label appeared. See r2-portal-reply-tier-label.png.`;
});

await step(CG, "business_user", "G12.team-add", "Open Matter team, Add team member, choose an existing person, Add; existing members are excluded; for a Confidential record ask Legal; no Portal remove control.", async () => {
  await go(R, `/portal/matters/${MR}`);
  ...
  check(!options.includes("Ravi Menon") && !options.includes("Nadia Haddad") && legal.some((r) => r.includes("Amara Nwosu")) && removes === 0 && !cadd && /Ask Legal/.test(cnote), JSON.stringify({ options, legal, removes, cadd, cnote }));
...
await step(CG, "business_user", "G13.legal-limits", "Stage/Status moves, owners, parties, confidentiality, relationships, Tasks, Key dates, Approvals and signatures stay with Legal.", async () => {
  const out = [];
  for (const url of [`/portal/contracts/${CR}`, `/portal/matters/${MR}`]) {
    ...
    const bad = buttons.filter((b) => /move|Stage|Status|Owner|Confidential|Counterpart|Link|Task|Key date|Approv|Sign|Archive|Remove/i.test(b));
    const switches = await R.getByRole("switch").count();
    out.push({ url, bad, switches, sections: ["Tasks", "Key dates", "Approvals"].filter((w) => main.includes(w)) });
  }
  const write = await api(R, "PATCH", `/contracts/${CR}`, { title: "DOC-029r2 roles hijack" });
  check(out.every((o) => o.bad.length === 0 && o.switches === 0 && o.sections.length === 0) && write.status >= 400, JSON.stringify(out));
...
await step(CG, "business_user", "G14.close-archive-remove", "Closing a Matter or Ending a Contract does not remove Portal work; archiving removes the record from the Portal; removing Ravi's membership stops access.", async () => {
  const statuses = await api(D, "GET", "/matter-statuses");
  const closed = (statuses.json.matterStatuses ?? statuses.json.statuses ?? []).find((s) => s.category === "closed" && !s.archivedAt);
  const moved = await api(N, "PATCH", `/matters/${MR}`, { statusId: closed.id, closingNote: "DOC-029r2 roles fictional closing note." });
  await go(R, "/portal/matters");
  const box = R.getByRole("searchbox", { name: "Search Matters" });
  await box.fill("DOC-029r2 roles Ravi");
  await box.press("Enter");
  ...
  const reopenedStatus = await api(N, "PATCH", `/matters/${MR}`, { statusId: openStatus.id, confirmReopen: true });
  await go(N, `/matters/${MR}`);
  await removeTeamMember(N, "Matter team", "Ravi Menon", "matter");
  await R.reload();
  await settle(R);
  ...
  // Ending a Contract (API stand-in for the Legal stage move).
  const cBefore = await api(N, "GET", `/contracts/${CR}`);
  const cRow = cBefore.json?.contract ?? cBefore.json;
  const cst = await api(D, "GET", "/contract-statuses");
  const cstList = cst.json?.contractStatuses ?? cst.json?.statuses ?? [];
  const endedStatus = cstList.find((st) => st.stage === "ended" && !st.archivedAt);
  const ended = await api(N, "PATCH", `/contracts/${CR}`, { statusId: endedStatus.id, overrideSoftGate: true });
  ...
  const back = await api(N, "PATCH", `/contracts/${CR}`, { statusId: cRow.statusId ?? cRow.status?.id, overrideSoftGate: true });
  results.g14ended = { ended: ended.status, endedStatus: endedStatus?.displayName, back: back.status, openEnded, uploadEnded, commentEnded };
  check(ended.status < 300 && !openEnded.refused && uploadEnded && commentEnded, JSON.stringify(results.g14ended));
  check(moved.status < 300 && listClosed.includes(`M-${MR}`) && !openClosed.refused && uploadClosed && !listArchived.includes("DOC-029r2 roles Ravi matter") && /not found/.test(openArchived.title) && !reopened.refused && removed && docs.status >= 400, JSON.stringify({ moved: moved.status, openClosed, uploadClosed, openArchived, reopened, removed, docs: docs.status }));
  return `Legal moved M-${MR} to "${closed.displayName}" (API stand-in, ${moved.status}); Ravi still found and opened it with Upload documents enabled. After Legal archived it (${archived.status}) it left Your Matters and the link showed "${openArchived.title}". Legal restored it (${restored.status}) and Ravi opened it again; Legal then reopened it (${reopenedStatus.status}). Legal moved C-${CR} to the ended status "${endedStatus.displayName}" (API stand-in, ${ended.status}); Ravi still opened it with Upload documents and the Comments composer available; Legal moved it back (${back.status}). After Nadia removed Ravi through Matter team, his reload showed Matter not found and the Documents read answered ${docs.status}.`;
});

// Cleanup: archive the paging Matters so they leave the Portal and default lists.
await step(CG, "legal_team_member", "Z1.cleanup", "Fixture cleanup: archive the 25 paging Matters.", async () => {
  const codes = [];
  for (const m of fx.pagingMatters) codes.push((await api(N, "POST", `/matters/${m.number}/archive`, {})).status);
