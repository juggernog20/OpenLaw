...

  {
    ...
    await step(
      ...
      async () => {
        ...
        L.check(c.indexGuides === 57 && c.linked === 57 && c.opened === 57, JSON.stringify(c));
        return c;
      },
    );
    await step(
  ...
  for (const role of ["anonymous", "operator"]) {
    ...
    await step(
      ...
      async () => {
        ...
        const c = await crawl(page, complete);
        L.check(noscript && c.indexGuides === 57 && c.opened === 57, JSON.stringify(c));
  ...
  for (const js of [true, false]) {
    ...
    await step(
      ...
      async () => {
        ...
        L.check(c.indexGuides === 57 && c.opened === 57, JSON.stringify(c));
...

  {
    ...
    await step(
      ...
      async () => {
        ...
        L.check(
          c.indexGuides === 57 && c.opened === 56 && c.missing.join() === "backup-and-restore",
          JSON.stringify(c),
        );
...
async function restored(root, out) {
  ...
  await step(
    ...
    async () => {
      ...
      const c = await crawl(page, incomplete);
      L.check(c.opened === 57 && c.missing.length === 0, JSON.stringify(c));
