...
export async function run(log, fx) {
  ...
  const visible = must(
    await api(admin.page, "POST", "/contracts", {
      title: `DOC-029r2 access search document holder ${stamp}`,
      contractTypeId: nda,
      managerId: danielId,
    }),
    "c1",
  ...
  const second = must(
    await api(admin.page, "POST", "/contracts", {
      title: `DOC-029r2 access search second ${term}`,
      contractTypeId: nda,
      managerId: danielId,
    }),
    "c2",
  ...
  const hidden = must(
    await api(admin.page, "POST", "/contracts", {
      title: `DOC-029r2 access search confidential ${term}`,
      contractTypeId: nda,
      managerId: danielId,
      isConfidential: true,
    }),
  ...
  const up = await admin.page.request.post(`${BASE}/api/v1/contracts/${visible.number}/documents`, {
    ...
    multipart: {
      file: {
        name: `doc029r2-access-search-${stamp}.pdf`,
        mimeType: "application/pdf",
        buffer: pdf(
          `DOC-029r2 access fictional clause mentions ${term} only inside this Document.`,
        ),
      },
    },
  ...
  for (let i = 0; i < 90 && !indexed; i++) {
    ...
    indexed =
      JSON.stringify(r.body).includes(`doc029r2-access-search-${stamp}.pdf`) ||
  ...
  for (const role of ["administrator", "legal_team_member"]) {
    ...
    if (!process.env.VIEWS_ONLY) {
      ...
      const readerOpen =
        (await p
          ...
          .filter({ hasText: `doc029r2-access-search-${stamp}.pdf` })
          .count()) > 0 ||
        (await text(p.locator("body"))).includes(`doc029r2-access-search-${stamp}.pdf`);
      R(
    ...
    if (role === "legal_team_member")
      await p.screenshot({
        path: `${process.env.SHOT_DIR ?? "/tmp/claude-1000"}/r2-access-${role}-${Date.now()}.png`,
      });
    R(
