...
- **Status** — Accepted
- **Date** — 2026-08-14
- **Context** — `GET /contracts` answered every contract the viewer could reach, in one response, forever. So did one record's comment thread and one record's paper. None of the three has a ceiling: the contract list grows with use, a thread grows for as long as the record lives, and a contract holds as many documents as it needs (CTR-014). The house already answers this question twice — the activity feed and the audit log are both paged in the same shape — and `GET /contracts` already ordered by `contracts.number` descending with a comment saying no second sort key can tie. The keyset was sitting there unused.
- **Decision** —
