...
export async function runC18(ctx, step) {
  ...
  await step(
    ...
    async () => {
      ...
      const cp = await actor.api("POST", `/contracts/${num}/counterparties`, { name: `DOC-029r2 contracts-b Counterparty ${tag}` });
  ...
  await step(
    ...
    async () => {
      const t = await actor.api("POST", `/contracts/${num}/tasks`, { title: `DOC-029r2 contracts-b C18 early task ${tag}`, dueDate: "2026-12-01" });
  ...
  await step(
    ...
    () => routed("Create child contract", "Create child contract", "Open the child contract", `DOC-029r2 contracts-b C18 child ${tag}`, (rel, ref) => /Parent/i.test(rel) && rel.includes(ref)),
  );
  ...
  await step(
    ...
    () => routed("Create successor contract", "New successor contract", "Open the successor", `DOC-029r2 contracts-b C18 successor ${tag}`, (rel, ref) => /Renews/i.test(rel) && rel.includes(ref)),
  );
  ...
  await step(
    ...
    async () => {
      const created = await actor.api("POST", "/contracts", { title: `DOC-029r2 contracts-b C18 lapsed ${tag}`, contractTypeId: typeId("NDA"), managerId: userId(PEOPLE[role].name) });
