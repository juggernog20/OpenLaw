...
M35/7 builds the pair, person, answers, state, Word reference, failure, and timestamps. A ready Generation requires a Word reference; a failed Generation requires a JSON object with nonempty string `code` and `detail`. The PDF, email, and Contract columns follow with their tickets. Both version references are indexed for ownership checks and deletion checks.
