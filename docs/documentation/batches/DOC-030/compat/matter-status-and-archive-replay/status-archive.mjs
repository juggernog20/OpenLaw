// DOC-030 compatibility replay (set compat): copy of DOC-029/compat-r2/matters-r2/wt-status-archive-r1.mjs. Changes: record names DOC-030 compat to DOC-030 compat.
// V-C23 steps for "Change a Matter Status or archive it" (matter-status-and-archive.md).
import { expectThat, q, matterByNumber, patchAfter, listTitles } from "./lib.mjs";
...
export default async function statusArchive(ctx) {
  ...
  const title = (label) => `DOC-030 compat matters ${short} ${label} ${stamp}`;
  ...
  await step(
    ...
    async () => {
      ...
      await pick("Closed", "Closed");
      await dialog.waitFor();
      await note.fill("DOC-030 compat fictional closing note for the walkthrough.");
  ...
  await step(
    ...
    async () => {
      ...
      const taskAdd = await api(page, "POST", `/matters/${n}/tasks`, {
        title: "DOC-030 compat closed task",
      });
      await page.goto(`${BASE}/matters`);
      await page.waitForLoadState("networkidle");
  ...
  await step(
    ...
    async () => {
      ...
      const close = await api(page, "PATCH", `/matters/${n}`, {
        ...
        closingNote: "DOC-030 compat closed again before archiving.",
      });
      await page.goto(`${BASE}/matters/${n}`);
