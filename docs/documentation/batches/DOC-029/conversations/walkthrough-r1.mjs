...
async function mailSearch(query) {
  const r = await fetch(`${L.MAIL}/api/v1/search?query=${encodeURIComponent(query)}&limit=50`).then((x) => x.json());
  return r.messages ?? [];
}
...
async function memberComments() {
  ...
  await step(CA, role, "Choose an audience on a record: open the Contract, select Comments, read the audience options and descriptions", "Legal Only and Contract Team offered, Contract Team selected first, description below the composer changes with the choice", async () => {
    await p.goto(C);
    const a = await openComments(p);
    const radios = await a.getByRole("radio").evaluateAll((els) => els.map((e) => `${e.getAttribute("aria-label") ?? e.closest("label")?.textContent?.trim()}:${e.checked}`));
    ...
    expect(/Administrators and Legal Team Members/.test(legalText), `Legal Only description ${legalText}`);
    expect(!(await a.getByRole("radio", { name: /Internal team|Working team/ }).count()), "an Internal team option is offered");
  ...
  await step(CA, role, "Post one Legal Only and one Contract Team comment on the Contract and check name and audience badge", "Each post appears under Nadia Haddad with its audience badge", async () => {
    const a = applet(p);
  ...
  await step(CA, role, "Choose an audience on a Matter: Matter Team first, Legal Only available, post both", "Matter Team is the starting audience and posts carry Matter Team or Legal Only badges", async () => {
    await p.goto(M);
    const a = await openComments(p);
    const labels = (await a.getByRole("group", { name: "Audience" }).innerText()).replace(/\s+/g, " ").trim();
    expect(await a.getByRole("radio", { name: "Matter Team" }).isChecked(), "Matter Team not checked first");
    const teamText = await audienceText(a);
  ...
  await step(CA, role, "Mention a person: type @ and part of a name, pick on the People tab, check Mentioned, Remove, pick again, Comment", "People tab lists reachable people, Files tab present for Legal, Mentioned list with Remove, post succeeds", async () => {
    await p.goto(C);
    const a = await openComments(p);
  ...
  await step(CA, role, "Typing a name as ordinary text alone does not select a recipient", "No Mentioned list when the name is typed without choosing from the list", async () => {
    const a = applet(p);
    const box = a.getByRole("textbox", { name: "New comment" });
  ...
  await step(CA, role, "Widen the audience? on a Legal Only mention of a Business User: Cancel keeps editing without posting, Widen and post posts at the proposed audience", "Dialog names the proposed audience; Cancel posts nothing; Widen and post posts at Contract Team", async () => {
    const a = applet(p);
    await chooseTier(a, "Legal Only");
    const box = a.getByRole("textbox", { name: "New comment" });
    ...
    await a.getByRole("listbox", { name: "People and files you can mention" }).getByRole("option", { name: /Jonas Weber/ }).click();
    await a.getByRole("button", { name: "Comment", exact: true }).click();
  ...
  await step(CA, role, "Attach paper: Attach files, choose files, check filenames, Remove one, post, open the preview and use Download", "Selected filenames listed with Remove; post carries the remaining attachment; preview opens; Download retrieves the file", async () => {
    const a = applet(p);
    await chooseTier(a, "Contract Team");
...
async function memberComments2() {
  ...
  await step(CA, role, "A failed post: keep the draft, check the error, check for an existing post, retry once", "Error shown; text and paper stay in the composer; no row posted and no filed marker; retry posts exactly once", async () => {
    await p.goto(C);
    const a = await openComments(p);
    ...
    const paper = (await a.getByRole("list", { name: "Files attached to this comment" }).innerText()).trim();
  ...
  await step(CA, role, "File to Contract on an unfiled attachment: the preview also offers it; file as a new Document; the attachment stays and shows Filed to", "File to Contract offered on the attachment and in the preview; after filing the thread row shows Filed to with Document and version; the Document is on the record", async () => {
    const a = applet(p);
    ...
    expect(filedText.includes(docName) && stillThere > 0 && onRecord, `filed ${filedText} onRecord ${onRecord}`);
  ...
  await step(CA, role, "Edit or remove a post: Edit then Cancel, Edit then Save shows edited, Delete asks for confirmation and leaves the author tombstone; no Redact for a Legal Team Member", "Cancel leaves text; Save marks edited; edit box has no audience control; Delete confirmation then Comment deleted by its author.; menu has Edit and Delete only", async () => {
    const a = applet(p);
    ...
    expect(!afterCancel.includes("draft change") && !/edited/.test(afterCancel), `after cancel ${afterCancel}`);
    ...
    if (await other.count()) otherActions = String(await other.getByRole("button", { name: "Comment actions" }).count());
  ...
  await step(CA, role, "Show older and the unread Comments badge on a Matter with more comments than one page", "Badge counts unread comments she can see; Show older loads earlier comments; reading the loaded conversation updates the count", async () => {
    // Fixture: Daniel posts 55 Matter Team comments and 1 Legal Only comment on the Matter through the API.
    if (!fx.bulkPosted) {
      for (let i = 1; i <= 55; i++) {
        const r = await L.api(ctx.daniel.page, "POST", "/comments", { entityType: "matter", entityId: fx.matterId, body: `${tag} Daniel bulk update ${String(i).padStart(2, "0")}`, visibility: "full_thread", mentions: [] });
        expect(r.status < 300, `bulk ${r.status} ${JSON.stringify(r.body).slice(0, 200)}`);
      }
      const r = await L.api(ctx.daniel.page, "POST", "/comments", { entityType: "matter", entityId: fx.matterId, body: `${tag} Daniel matter legal only bulk note`, visibility: "legal_only", mentions: [] });
      expect(r.status < 300, `bulk legal ${r.status}`);
      fx.bulkPosted = true;
      writeFileSync(path.join(here, "fixtures-r1.json"), JSON.stringify(fx, null, 2) + "\n");
      prep(`Daniel Okafor posted 55 Matter Team comments and one Legal Only comment on M-${fx.matter.number} through the comments API, to give the Matter more than one page of comments, History entries, and notifications.`);
    }
    await p.goto(M);
    const toolbar = p.getByRole("toolbar", { name: "Applets" });
    await toolbar.waitFor();
    await p.waitForTimeout(2000);
    const before = (await toolbar.getByRole("button", { name: /^Comments/ }).innerText()).replace(/\s+/g, " ").trim();
    const beforeApi = (await L.api(p, "GET", `/comments/unread?entityType=matter&entityId=${fx.matterId}`)).body;
    const a = await openComments(p);
    ...
    const after = (await toolbar.getByRole("button", { name: /^Comments/ }).innerText()).replace(/\s+/g, " ").trim();
    const afterApi = (await L.api(p, "GET", `/comments/unread?entityType=matter&entityId=${fx.matterId}`)).body;
  ...
  await step(CA, role, "Read the Activity feed: History on the Matter, entries in order, Show older", "History lists actions on the record, including comment actions, with Show older for earlier changes", async () => {
    await p.goto(M);
    ...
    const first = (await panel.getByRole("listitem").first().innerText()).replace(/\s+/g, " ").trim();
    await panel.getByRole("button", { name: "Show older" }).click();
    await p.waitForTimeout(2000);
    const n2 = await panel.getByRole("listitem").count();
    const last = (await panel.getByRole("listitem").last().innerText()).replace(/\s+/g, " ").trim();
  ...
  await step(CA, role, "Choose an audience in the Inbox: a Request offers Legal only and Shared with requester, starting on Legal only; the Inbox Request has no History", "Two audiences, Legal only first, descriptions shown; posts carry their audience; no History applet", async () => {
    ...
    const a = await openComments(p);
    const labels = (await a.getByRole("group", { name: "Audience" }).innerText()).replace(/\s+/g, " ").trim();
    expect(await a.getByRole("radio", { name: "Legal only" }).isChecked(), "Legal only is not first");
    const legalText = await audienceText(a);
  ...
  await step(CA, role, "A converted Request in the Inbox has no conversation; open its linked record instead", "No Comments applet on the converted Request; Status links the Contract", async () => {
    ...
    const link = p.getByRole("region", { name: "Status" }).getByRole("link", { name: `C-${fx.contract.number}` });
...
async function adminComments() {
  ...
  await step(CA, role, "Choose an audience on a record and post at both audiences, one with Legal Only paper", "Legal Only and Contract Team offered; posts show under Daniel Okafor with their badges and attachment", async () => {
    await p.goto(C);
    const a = await openComments(p);
    const labels = (await a.getByRole("group", { name: "Audience" }).innerText()).replace(/\s+/g, " ").trim();
  ...
  await step(CA, role, "Another author's comment: Comment actions offers Redact and no Edit", "Menu on Nadia's live comment offers Redact only", async () => {
    const a = applet(p);
  ...
  await step(CA, role, "File to Matter on a Matter conversation attachment", "A Matter Team post with paper offers File to Matter", async () => {
    await p.goto(M);
    const a = await openComments(p);
  ...
  await step(CA, role, "Legal Request conversation: Comment actions on his own live comment edits and deletes; no File control on Request paper", "Edit then Save shows edited; Delete confirmation leaves the author tombstone; Request attachment has no File to control", async () => {
    await p.goto(`${L.BASE}/inbox/${fx.openRequest}`);
    const a = await openComments(p);
...
async function portalComments() {
  ...
  await step(CA, role, "Reply from the Portal on a never-converted Request: Comments applet, no audience picker, Legal only comments absent, mention and attach, Comment", "Shared with requester reply visible, Legal only note absent, reply posts under Jonas Weber with paper", async () => {
    ...
    await row.waitFor({ timeout: 15000 });
    const s = await rowSummary(row);
    expect(radios === 0 && shared === 1 && legal === 0 && /Jonas Weber/.test(s) && /doc029-conv-portal-note\.pdf/.test(s), `radios ${radios} shared ${shared} legal ${legal} row ${s}`);
  ...
  await step(CA, role, "Portal: Comment actions edits and deletes his own words", "Edit and Delete offered on his own comment, no Redact; edited marker; author tombstone", async () => {
    const a = applet(p);
  ...
  await step(CA, role, "After conversion the Request address opens the Contract; its Comments hold team comments and their paper but no Legal Only content, no filing, no audience picker", "Redirect to the Portal Contract; Contract Team rows and widened mention visible; Legal Only rows and memo absent; attachment preview and Download work; no File to Contract", async () => {
    ...
    expect(seen.team && seen.widened && seen.notice && seen.danielTeam && !seen.legalOnly && !seen.danielMemo && !seen.memoFile && radios === 0 && fileTo === 0 && same, JSON.stringify({ seen, radios, fileTo, same }));
  ...
  await step(CA, role, "Portal Matter: Show older and the unread badge; Legal Only bulk note absent", "Show older loads the earlier Matter Team comments; the Legal Only note is not in the view", async () => {
    await p.goto(`${L.BASE}/portal/matters/${fx.matter.number}`);
    const toolbar = p.getByRole("toolbar", { name: "Applets" });
    await toolbar.waitFor();
    await p.waitForTimeout(2000);
    const badge = (await toolbar.getByRole("button", { name: /^Comments/ }).innerText()).replace(/\s+/g, " ").trim();
    const a = await openComments(p);
  ...
  await step(CA, role, "Portal History on the Contract: shared comment activity only", "History shows comment entries for team comments only; no Legal Only activity", async () => {
    await p.goto(`${L.BASE}/portal/contracts/${fx.contract.number}`);
    await openApplet(p, "History");
    const panel = p.getByRole("complementary", { name: "History" });
    await panel.waitFor();
    await p.waitForTimeout(2000);
    const entries = (await panel.getByRole("listitem").allInnerTexts()).map((t) => t.replace(/\s+/g, " ").trim());
    const staff = (await L.api(ctx.nadia.page, "GET", `/activity?entityType=contract&entityId=${fx.contractId}&limit=100`)).body;
...
async function adminRedact() {
  ...
  await step(CA, role, "Redact a reached comment with paper, an author-deleted comment, and the comment whose attachment was filed", "Confirmation read first; thread says Comment removed by an Administrator.; attachment URL no longer serves; the filed Document stays on the record", async () => {
    await p.goto(C);
    const a = await openComments(p);
    const target = rowWith(a, `${tag} Nadia retry after a failed post`).first();
    const attachmentHref = await target.getByRole("link", { name: "Download doc029-conv-schedule.pdf" }).getAttribute("href");
    ...
    expect(before === 200 && after === 404 && remaining === 0 && tombs === 3 && docStays, JSON.stringify({ before, after, remaining, tombs, docStays }));
  ...
  await step(CA, role, "Archived record: the attachment offers no File to Contract", "No File to Contract on an archived Contract", async () => {
    const r = await L.api(p, "POST", "/comments", { entityType: "contract", entityId: fx.archiveId, body: `${tag} Daniel paper before archive`, visibility: "full_thread", mentions: [] });
    ...
    await postComment(p, `${tag} Daniel archive paper`, { tier: "Contract Team", files: ["doc029-conv-schedule.pdf"] });
    const beforeArchive = await rowWith(applet(p), `${tag} Daniel archive paper`).first().getByRole("button", { name: "File to Contract" }).count();
    const ar = await L.api(p, "POST", `/contracts/${fx.archive.number}/archive`);
    expect(ar.status < 300, `archive ${ar.status}`);
    prep(`Daniel Okafor archived C-${fx.archive.number} through the archive API after posting a Contract Team comment with paper in the browser.`);
    await p.goto(`${L.BASE}/contracts/${fx.archive.number}`);
    const a = await openComments(p);
...
async function portalAfter() {
  ...
  await step(CA, role, "Archived destination: the Request address shows the original request with no conversation", "Portal Request page shows the archived message and no Comments", async () => {
    ...
    expect(msg === 1 && comments === 0 && recordRead >= 400, `msg ${msg} comments ${comments} record ${recordRead}`);
  ...
  await step(CA, role, "A later-added Business User reads earlier team comments; a removed person loses them, including through the old Request link", "Amara sees earlier Contract Team comments after being added; after removal Jonas cannot open the Contract or its conversation through /portal/requests", async () => {
    const d = ctx.daniel.page;
    ...
    prep(`Daniel Okafor added Amara Nwosu (Business User) to the C-${fx.contract.number} team through the team API, then removed her again; later removed and restored Jonas Weber the same way.`);
    ...
    const amaraAfter = (await ap.locator("main").innerText()).replace(/\s+/g, " ").trim().slice(0, 160);
    ...
    const jonasAfter = (await p.locator("main").innerText()).replace(/\s+/g, " ").trim().slice(0, 200);
    ...
    const restore = await L.api(d, "POST", `/contracts/${fx.contract.number}/team`, { userId: fx.ids.jonas });
    expect(earlier === 1 && legal === 0 && rm.status < 300 && rj.status < 300 && amaraApi >= 400 && jonasApi >= 400 && jonasComments === 0 && restore.status < 300, JSON.stringify({ earlier, legal, amaraApi, jonasApi, jonasComments, restore: restore.status }));
...
async function memberNotifications() {
  ...
  await step(NT, role, "New requests: a new Request reaches the Legal user's bell and opens the Inbox Request", "New request item links /inbox/N and opens it", async () => {
    ...
    await p.goto(`${L.BASE}/`);
    const dialog = await openBell(p);
    const link = await findItem(p, dialog, new RegExp(`new request: DOC-029 conversations new request bell ${stamp}`));
...
async function portalNotifications() {
  ...
  await step(NT, role, "Portal bell: a Request receipt opens the Request; a converted Request's record item opens the Contract", "Receipt item links /portal/requests/N; a C-N comment item opens /portal/contracts/N", async () => {
    ...
    const dialog = await openBell(p);
    const receipt = await findItem(p, dialog, new RegExp(`received your request DOC-029 conversations new request bell ${stamp}`));
  ...
  await step(NT, role, "Portal Activity on your records: shared comments on his Contracts give a bell item and, with Email initially off, no mail", "A shared team comment on C-N gives an Activity on your records bell item and no email while that group's Email is off", async () => {
    const title = fx.contractTitle;
    // Amara Nwosu is added to the same Contract as a team member who did not raise its Request, for comparison.
    const add = await L.api(d, "POST", `/contracts/${fx.contract.number}/team`, { userId: fx.ids.amara });
    expect(add.status < 300, `add amara ${add.status}`);
    ctx.amara ??= await L.portalContext(L.PEOPLE.amara);
    const jonasPrefs = await (async () => { await p.goto(`${L.BASE}/portal/settings`); return switchStates(p, ["Activity on your records In-app", "Activity on your records Email", "Request updates Email"]); })();
    const since = new Date(Date.now() - 2000).toISOString();
    ...
    const pid = await comment(n, "contract", fx.contractId, `${tag} Nadia plain team comment`, "full_thread");
    await settleQueue();
    const jItems = await aboutComment(p, pid, since, true);
    const aItems = await aboutComment(ctx.amara.page, pid, since, true);
    const jMail = (await mailSearch(`to:"${email}"`)).filter((m) => m.Created >= since).map((m) => m.Subject).filter((x) => x.includes(title) || x.includes(`R-${fx.contractRequest}`));
    const aMail = (await mailSearch(`to:"${L.PEOPLE.amara.email}"`)).filter((m) => m.Created >= since).map((m) => m.Subject).filter((x) => x.includes(title));
    const rm = await L.api(d, "DELETE", `/contracts/${fx.contract.number}/team/${fx.ids.amara}`);
    prep(`Daniel Okafor added Amara Nwosu to the C-${fx.contract.number} team for the Portal Activity on your records comparison and removed her afterwards (HTTP ${rm.status}).`);
    ...
    expect(jItems.length === 1 && jItems[0].eventType === "comment.posted" && jMail.length === 0, `The guide says shared comments on his Contracts belong to Activity on your records (Email initially off). ${observed}`);
    return observed;
  });
...
async function morning() {
  ...
  expect(localHour(LA) >= 8 && localHour(HNL) < 8, `timezone window closed: LA ${localHour(LA)} HNL ${localHour(HNL)}`);
  ...
  await step(NT, "legal_team_member", "Set the daily briefing and group switches on the fresh accounts before the morning", "Saved indications for Dates approaching In-app off (Member No Date Bell) and Briefing Dates Email off (Admin No Date Email)", async () => {
    ...
    prep(`The Member Dates, Member No Date Bell, and Admin No Date Email accounts had their timezone set to ${LA} (local hour ${localHour(LA)}) through the preferences API; Member Honolulu stayed on ${HNL} (local hour ${localHour(HNL)}).`);
...
async function supplement() {
  ...
  if (only.includes("wt")) await step(CA, "legal_team_member", "Internal team: an older Working Team comment stays readable to Legal with its label, cannot be chosen, and stays out of the Business User view", "Contract shows Internal team badge; Inbox Request shows Working team badge; no such composer option; Jonas sees neither", async () => {
    const wc = await L.api(n, "POST", "/comments", { entityType: "contract", entityId: fx.contractId, body: `${tag} older working team note`, visibility: "working_team", mentions: [] });
    const wr = await L.api(n, "POST", "/comments", { entityType: "request", entityId: fx.openRequestId, body: `${tag} older working team request note`, visibility: "working_team", mentions: [] });
    expect(wc.status < 300 && wr.status < 300, `working team fixture ${wc.status} ${wr.status} ${JSON.stringify(wc.body).slice(0, 160)}`);
    prep(`Nadia Haddad posted one working_team comment on C-${fx.contract.number} and one on R-${fx.openRequest} through the comments API to stand in for older Working Team comments; the composer does not offer that tier.`);
    ...
    const cOptions = (await a.getByRole("group", { name: "Audience" }).innerText()).replace(/\s+/g, " ").trim();
    ...
    const rOptions = (await a.getByRole("group", { name: "Audience" }).innerText()).replace(/\s+/g, " ").trim();
    ...
    expect(/Internal team/.test(cs) && /Working team/.test(rs) && !/Internal|Working/.test(cOptions + rOptions) && jc === 0 && jr === 0, JSON.stringify({ cs, rs, cOptions, rOptions, jc, jr }));
  ...
  if (only.includes("errors")) await step(CA, "legal_team_member", "If the conversation or History cannot load: follow the message to reopen; retry Show older when only the earlier page failed", "The conversation could not be read. Reopen the panel to try again.; reopening recovers; The earlier comments could not be read. Try again. keeps loaded rows and the retry loads more; The history could not be read. Reopen the panel to try again.", async () => {
    await n.goto(`${L.BASE}/matters/${fx.matter.number}`);
    await n.getByRole("toolbar", { name: "Applets" }).waitFor();
    let block = true;
    const thread = (route) => (block && route.request().method() === "GET" && !/unread|mention/.test(route.request().url()) ? route.abort("failed") : route.continue());
    await n.route(/\/api\/v1\/comments\?/, thread);
    await openApplet(n, "Comments");
    const a = applet(n);
    const loadErr = a.getByText("The conversation could not be read. Reopen the panel to try again.");
    ...
    const histErr = panel.getByText("The history could not be read. Reopen the panel to try again.");
  ...
  if (only.includes("six")) await step(CA, "legal_team_member", "Attach files accepts up to five files", "Choosing six files keeps at most five in the list", async () => {
    await n.goto(`${L.BASE}/contracts/${fx.contract.number}`);
  ...
  if (only.includes("usual")) await step(NT, "legal_team_member", "When the selected recipient has left the team, editing the Key date offers Use the usual audience", "Edit key date explains the departed recipient and offers Use the usual audience; choosing it clears the selection", async () => {
    const d = ctx.daniel.page;
    const label = MORNING_STAMP ? `DOC-029 conversations ${MORNING_STAMP} narrowed review` : `${tag} narrowed review`;
...
async function shots() {
  ...
  const n = ctx.nadia.page;
  results.screenshots = [];
  await step(NT, "business_user", "Screenshot: Portal bell item for a team comment on the Contract converted from his Request", "Item reads replied on your request", async () => {
    await comment(n, "contract", fx.contractId, `DOC-029 conversations ${fx.stamp} screenshot team comment`, "full_thread");
    ...
    const item = dialog.getByRole("link", { name: new RegExp(`replied on your request ${fx.contractTitle}`) }).first();
    await item.waitFor();
    const text = (await item.innerText()).replace(/\s+/g, " ").trim();
    await j.screenshot({ path: path.join(here, "r1-portal-bell-converted-contract-comment.png") });
    await closeBell(j);
    results.screenshots.push("r1-portal-notification-settings.png", "r1-portal-bell-converted-contract-comment.png");
  ...
  await step(CA, "legal_team_member", "Screenshot: Contract conversation with audience badges, Filed to, and administrator tombstones", "Screenshot saved", async () => {
    await n.goto(`${L.BASE}/contracts/${fx.contract.number}`);
...
// ---------- main ----------
let fx = existsSync(path.join(here, "fixtures-r1.json")) ? JSON.parse(readFileSync(path.join(here, "fixtures-r1.json"), "utf8")) : null;
