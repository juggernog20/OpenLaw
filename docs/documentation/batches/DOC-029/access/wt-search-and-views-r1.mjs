...
// The "DOC-029 access" fresh accounts search and save views, so seeded accounts keep their saved views.
...
export async function run(log, fx) {
  ...
  const must = (r, what) => {
    if (r.status >= 300)
      throw new Error(`${what}: ${r.status} ${JSON.stringify(r.body).slice(0, 200)}`);
    return r.body;
  };
  ...
  const visible = must(
    await api(admin.page, "POST", "/contracts", {
      title: `DOC-029 access search document holder ${stamp}`,
      contractTypeId: nda,
      managerId: danielId,
    }),
    "c1",
  ...
  const second = must(
    await api(admin.page, "POST", "/contracts", {
      title: `DOC-029 access search second ${term}`,
      contractTypeId: nda,
      managerId: danielId,
    }),
    "c2",
  ...
  const hidden = must(
    await api(admin.page, "POST", "/contracts", {
      title: `DOC-029 access search confidential ${term}`,
      contractTypeId: nda,
      managerId: danielId,
      isConfidential: true,
    }),
  ...
  const up = await admin.page.request.post(`${BASE}/api/v1/contracts/${visible.number}/documents`, {
    ...
    multipart: {
      file: {
        name: `doc029-access-search-${stamp}.pdf`,
        mimeType: "application/pdf",
        buffer: pdf(`DOC-029 access fictional clause mentions ${term} only inside this Document.`),
      },
    },
  ...
  fx.created.push(
    `Contracts C-${visible.number} (PDF with the unique word), C-${second.number}, Confidential C-${hidden.number} for search`,
  );
  ...
  for (let i = 0; i < 90 && !indexed; i++) {
    ...
    indexed =
      JSON.stringify(r.body).includes(`doc029-access-search-${stamp}.pdf`) ||
      JSON.stringify(r.body).includes('"kind":"document"');
    if (!indexed) await sleep(2000);
  }
  log.record(
    A,
    "administrator",
    "fixture: Document text processed and searchable",
    "The PDF's word becomes searchable",
    `Indexed ${indexed} for a unique word in the PDF on C-${visible.number}`,
    indexed,
  );
  ...
  for (const role of ["administrator", "legal_team_member"]) {
    ...
    if (!process.env.VIEWS_ONLY) {
      ...
      await p
        ...
        .or(p.getByRole("link", { name: /See all results/ }))
        .or(p.getByRole("button", { name: /See all results/ }))
        .first()
        .click();
      ...
      const docRow = p
        .locator("main li")
        .filter({ hasText: `C-${visible.number}` })
        .first();
      ...
      await p
        ...
        .first()
        .click();
      await p.waitForURL(new RegExp(`/contracts/${visible.number}/documents\\?.*find=`), {
        timeout: 15000,
      });
      await sleep(4000);
      const findValue = await p
        .locator("input[type=search], input")
        .evaluateAll((els, t) => els.some((e) => e.value === t), term);
      const readerOpen =
        (await p
          .getByRole("complementary")
          .filter({ hasText: `doc029-access-search-${stamp}.pdf` })
          .count()) > 0 ||
        (await text(p.locator("body"))).includes(`doc029-access-search-${stamp}.pdf`);
      R(
      ...
      const directHidden = await p
        .getByText("Contract not found")
        .first()
      ...
      R(
        "the Confidential record outside reach stays hidden",
        "Direct link and search do not reveal it",
        `Direct link shows Contract not found ${directHidden}; API contract results include it ${JSON.stringify(hiddenApi.body).includes("search confidential")}`,
        directHidden && !JSON.stringify(hiddenApi.body).includes("search confidential"),
      );
      ...
      await p.route("**/api/v1/search**", (route) =>
        route.request().url().includes("cursor=") ? route.abort() : route.continue(),
      );
      await p.getByRole("button", { name: "Show more" }).click();
      const nextErr = await p
        .getByText("The next results could not be read. Try again.")
      ...
      const headerErr = await p
        .getByText("Search could not load")
        .first()
        .waitFor({ timeout: 10000 })
        .then(
          () => true,
      ...
      const pageErr = await p
        .getByText("Search could not load. Try again.")
        .first()
        .waitFor({ timeout: 15000 })
        .then(
          () => true,
      ...
      const addFilter = async (prop, value) => {
        ...
        await p
          ...
          .getByRole("button", { name: prop, exact: true })
          .click();
        const any = await p
          .getByRole("dialog", { name: "Filter" })
      ...
      const countText = async () =>
        (
          await text(p.getByRole("region", { name: "Contracts" }).getByRole("paragraph").first())
        ).trim();
      ...
      const nums = await p
        .locator("main tbody tr, main [role=rowgroup]:nth-of-type(2) [role=row]")
        .evaluateAll((rows) =>
          rows.map((r) => (r.innerText.match(/C-(\d+)/) ?? [])[1]).filter(Boolean),
        );
      const owners = nums
        .slice(0, 25)
        .map((n) => sql(`select manager_id from contracts where number=${n}`));
      ...
      const emptyState = await p
        .getByText("No contracts match these filters")
        .first()
        .waitFor({ timeout: 10000 })
        .then(
          () => true,
      ...
      R(
        ...
        "An empty filter answer shows the empty message; removing filters restores rows",
        `Owner Me shows the empty message ${emptyState}; rows after Clear all ${await p.locator("main tbody tr").count()}`,
        emptyState,
      );
    }
    const addFilter = async (prop, value) => {
    ...
    // Save a view.
    const viewName = `DOC-029 access ${role === "administrator" ? "Administrator" : "Member"} portfolio ${stamp % 100000}`;
    ...
    const sortIs = async (want) => {
      for (let i = 0; i < 40; i++) {
        if (
          (await p.getByRole("columnheader", { name: "Title" }).getAttribute("aria-sort")) === want
        )
          return true;
        await sleep(250);
      }
    ...
    const discarded =
      ...
      !(await text(viewsBtn)).includes("Modified");
    ...
    const builtIn =
      ...
      (await text(viewsBtn)).startsWith("Default view");
    ...
    const restoredSort = await p
      .getByRole("columnheader", { name: "Title" })
      .getAttribute("aria-sort");
    const restoredHeaders = await headers();
    if (role === "legal_team_member")
      await p.screenshot({
        path: new URL("./r1-legal_team_member-saved-view.png", import.meta.url).pathname,
      });
    R(
    ...
    await sleep(2500);
    const opensWithView =
      (await text(viewsBtn)).startsWith(viewName) &&
    ...
    await sleep(3000);
    const otherBtn = other.page
      .getByRole("region", { name: "Contracts" })
      .getByRole("button")
      .first();
    const otherHasView =
      (await text(otherBtn)).startsWith(newName) &&
      (await other.page.getByRole("button", { name: "Owner: Daniel Okafor" }).isVisible());
    ...
    R(
      ...
      delText.includes("The records in it are not touched.") &&
        !stillListed &&
