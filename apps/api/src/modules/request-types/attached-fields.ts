 ...
 *
 * **The required rule is this mount's own.** A `user` or `entity`
 * field may be attached to a request type and may never be marked
 * required on its form: the portal offers a requester no rows for
 * either, so a required one could never be answered. The refusal names
 * the field, and it reaches the Administrator who set the flag rather
 * than the requester who would meet it (the INT-002 M20/11 addendum).
 *
