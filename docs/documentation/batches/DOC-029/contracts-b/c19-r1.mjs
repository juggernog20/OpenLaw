...
export async function runC19(ctx, step) {
  ...
  await step(
    ...
    async () => {
      ...
      await dialog
        ...
        .fill(`DOC-029 contracts-b C19 cancelled ${tag}`);
  ...
  await step(
    ...
    async () => {
      ...
      await dialog
        ...
        .fill("DOC-029 initial note for the reviewer.");
      await dialog
      ...
      expectThat(
        first &&
          ...
          detailsText.includes("DOC-029 initial note") &&
  ...
  await step(
    ...
    async () => {
      ...
      await dialog
        .getByRole("textbox", { name: "Add a note" })
        .fill("DOC-029 note that fails once.");
      let failed = 0;
  ...
  await step(
    ...
    async () => {
      ...
      const find = (j) =>
        JSON.stringify(j).match(/\{[^{}]*"label":"DOC-029 price review[^{}]*\}/)?.[0] ?? "";
  ...
  await step(
    ...
    async () => {
      ...
      const w1 = await actor.api("POST", `/contracts/${num}/tasks`, {
        title: "DOC-029 archived write",
      });
      const w2 = await actor.api("POST", `/contracts/${num}/key-dates`, {
        date: isoPlusDays(5),
        label: "DOC-029 archived write",
      });
