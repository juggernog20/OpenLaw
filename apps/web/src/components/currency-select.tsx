...
import { notifyCurrenciesChanged, useCurrencies, type CurrencySettings } from "../lib/currencies";
...
export function AddCurrencyDialog({
  ...
  onAdded: (code: string, settings: CurrencySettings) => void;
  onClose: () => void;
}>) {
  const intl = useIntl();
  ...
  const options = currencyOptions({ locale: intl.locale }).filter(
    (option) =>
  ...
  async function add(code: string) {
    ...
    const result = await api
      .POST("/api/v1/org/currencies", { body: { code } })
      .catch(() => undefined);
      ...
      >
        ...
        <Input
          ...
          placeholder={intl.formatMessage({
            id: "currencies.searchPlaceholder",
            defaultMessage: "Search by currency code or name",
          })}
        />
        {error && (
          <p role="alert" className="mt-3 text-status-danger-fg">
            {error}
          </p>
        )}
        <ul
          ...
          {options.map((option) => (
            <li key={option.code}>
              <Button
                ...
                aria-label={`${option.code} ${option.displayName}`}
                disabled={busy}
                onClick={() => void add(option.code)}
              >
                <span className="w-10 shrink-0 font-medium">{option.code}</span>
                <span className="text-left">{option.displayName}</span>
              </Button>
            </li>
          ))}
        </ul>
        {!options.length && (
          <p className="py-4 text-muted">
            <FormattedMessage
              id="currencies.noMatches"
              defaultMessage="No currencies match your search."
            />
          </p>
        )}
...
export function CurrencySelect({
  ...
  const codes = new Set([...settings.currencies, ...(value ? [value] : [])]);
  const options = currencyOptions({ locale: intl.locale }).filter((option) =>
    codes.has(option.code),
  );
  function close() {
    setAdding(false);
  ...
  return (
    <>
      <select
        ...
        aria-busy={settings.loading || undefined}
        onBlur={(event) => {
          if (adding) event.stopPropagation();
          else onBlur?.(event);
        }}
        onKeyDown={onKeyDown}
        onChange={(event) => {
      ...
      >
        <option value="" disabled={!allowEmpty}>
          {placeholder ??
            intl.formatMessage({ id: "currencies.choose", defaultMessage: "Choose currency" })}
        </option>
      ...
      {settings.error && (
        <span role="alert" className="text-xs text-status-danger-fg">
          {settings.error}{" "}
          <Button variant="ghost" size="sm" onClick={settings.reload}>
            <FormattedMessage id="currencies.retry" defaultMessage="Retry" />
          </Button>
        </span>
      )}
      {adding && (
