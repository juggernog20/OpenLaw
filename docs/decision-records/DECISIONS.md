...
**11. Upgrade.** Each `request_type_fields` row becomes an On intake form Row on its destination type, attached if missing; module-only destinations map to that module's Default type; a Request type with no module follows its questions (a Contract destination when it holds a Contract Field or an intake built-in, because 0140 kept untargeted intake questions in Contracts; otherwise Matter), and one that holds Fields of both modules refuses the upgrade by name. _Amended 2026-09-21 during M39/2: the first text sent every no-module Request type to Matters, which refused every install still holding one._ Required is kept when either the Request row or the type row had it. Visible on Portal copies the Field's tag onto each Row. The ten intake rows become On intake form on the built-in Row of each type that attached them, and stored Request answers under those slugs are re-keyed to the built-in key. Answered Requests are otherwise untouched: `requests.custom_fields` stays keyed by slug and conversion copies by slug as today.

### Built addendum, 2026-09-22, M39, #1007 and #1020

The built Form follows this decision with these corrections and implementation choices. These points supersede the conflicting wording above.

- Title and Type are pinned only on Contract and Matter Forms. The API supplies them in memory with fixed switches. They have no stored built-in Row. Entity Forms contain catalog Fields and Branches; their native columns stay on the existing create form and record. Entity Rows show only Required for creation. Their stored `visible_on_portal` value is preserved, with no switch or Entity Portal record added.
- Enabling On intake form also enables Visible on Portal in the builder. Visible on Portal then locks with a reason. The API still refuses an intake Field whose Visible on Portal is off. This follows #1012 and replaces DES-090's proposed refusal before enabling intake.
- Title, Department and Urgency are the fixed Request basics, with Attachments last. Description is a configurable built-in Intake Row, seeded on in 0157 and on new types. The Form builder calls the built-in Entity Row **Entity** and the owning department Row **Department**. Record controls retain **Our entity** and **Owning department** where those labels already identify the fact.
- A Branch money operand is an integer in minor units and compares only the amount. It carries no currency. The builder labels those units explicitly; Value answers use their selected currency to convert amounts. This replaces the mock's ambiguous major-unit threshold. Value remains one Row although Request storage has `value_amount`, `value_currency`, and `value_cadence` keys.
- Migration 0157 merges Request attachments onto destination Forms. Shared destinations keep Required when either attachment required it. Untargeted Request types with Contract Fields or intake built-ins go to Contracts; otherwise they go to Matters. Unmappable or mixed-module attachments stop the upgrade with the Request type names. Existing `default` type slugs are reused and restored, keeping their seed marker. The separate `is_default` flag protects them and Start blank keeps them.
- Whole-Form replacement retains archived Field attachments for restoration. If their Branch disappears, it moves those retained attachments to the root. Removing a Branch in the builder keeps its children at that position. Detaching a Field keeps its definition and stored values.
- The retired demo Fields are no longer seeded. Migration 0159 deletes only the ten protected intake catalog rows, after removing their attachments. Existing ordinary `counterparty_name` and `needed_by` Fields and answers survive. Stored Request answers are re-keyed with canonical keys taking precedence; money, cadence, Term type and Counterparty names are normalized. Auto-Doc maps, AI provenance and prompt overrides migrate to the shared built-in identities too. The per-Field type mutation routes are retired; writes use the complete Form.
- Conversion keeps its fixed Title, Type and Priority controls and the Matter template choice, then draws the other Intake and Creation Rows in Form order. It re-reads the target Form under its type lock and uses ordinary record creation. Answers without a destination Row remain on the Request. A false Branch removes collection and required enforcement, but never deletes an existing record value.
- Matter Record Row preparation uses the `matter.record-preparation` queue and reuses Conversion draft storage with nullable `conversion_drafts.matter_id`. Execution leases and recovery use the existing preparation machinery. Contract Record Rows use the post-conversion Analysis run. Conditional Rows remain preparation targets even before their conditions resolve; human values remain protected.

The browser acceptance journey is `e2e/tests/61-m39-type-form.spec.ts`. It builds an NDA Branch, submits a Request through it, converts it, and checks the staff and Portal records. The guides describe the shipped switches and Touchpoint derivation. Independent documentation publication review remains separate from this implementation record.
...
| T5  | `guide`     | `openlaw_form_get`            | read  | always     | always        | The type Form (DD-028) for a Request type, a creation form for a Contract or Matter type, or an Auto-Doc's Live form by Auto-Doc id (#1060).                                |
...
- **Audit reach and rate limits.** The audit log already enforces record-level reach through `auditReachScope`; T35 remains in the M42 Administration Toolset scope. M40 adds a per-credential Tool call rate limit alongside the sign-in limits.
