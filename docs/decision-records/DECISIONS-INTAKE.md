...
**Conversion behavior superseded on 2026-09-11 by DD-023:** converted Requests leave **Your Requests** and their addresses redirect to the reachable Contract or Matter. Only an archived destination retains the original-ask stub; removal from a live record's team grants no fallback Request access. The historical behavior below records the earlier Portal shell.
...
**The basics are fixed and are not columns.** Every portal form collects ~~Summary~~ Title (INT-009), Description, Attachments, and Urgency. ~~Summary~~ Title, Description, and Urgency are required; Attachments are optional. The editor draws the four as locked rows so an Administrator can read the contract without being invited to change it, and nothing in the schema records them — a fixed set is a fact about the form, not a configuration of it. Urgency carries the DES-018 severity ramp (`low`, `medium`, `high`, `critical`), as this decision already recorded.
...
### Addendum (2026-09-09, [focus group, 2026-09-07](../reviews/focus-group-2026-09-07.md)) — the requester learns who has the Request and when to expect it back

Six of the nine Business User testers said the same thing: the request page says "Legal is working on this" and never says who, or until when. Urgency and Needed by went into a void. This was the strongest adoption signal in the group, so it is decided here rather than parked.

**Decision.** A Request shows its requester two facts it did not show before: the **owner**, the Legal Team Member assigned at triage (the INT-007 assignment addendum), and an **expected return date**. The owner appears on the request page and in the "Your requests" list as soon as the assignment exists, and clears when the assignment clears. The expected date is set by the owner on the intake page, with the request type's published turnaround as the suggested default, and the requester sees it as a date, not a duration. A status-change notification names the new status (the M20/10 vocabulary applies), and the daily briefing does not change.

**What is not decided.** Whether the expected date is a promise or an estimate in copy; whether it moves automatically when Needed by is earlier than the turnaround; whether a missed date shows a late state on the requester's list. Those are for the spec.

**Consequences.** One nullable date column on the Request, written by Member+ only. The requester-facing projection (M20/3) gains owner name and expected date. DES copy for the two labels. INT-003 keeps its shape: still notifications and the thread, no poke button; the requester now has less reason to ask.
...
| INT-008 | AI-assisted conversion: durable drafts, source tiers and per-value provenance | Accepted                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| INT-009 | Requests use Title throughout the app and store                               | Accepted                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
### INT-002 addendum (2026-09-09, [focus group, 2026-09-07](../reviews/focus-group-2026-09-07.md)) — the conversion lands counterparty, needed-by, and the Contract Owner

The Deputy GC persona rated this high and called it data loss on the main intake path:
Convert to contract said "Does not carry into the contract: Counterparty name and Needed
by", and the Contract it made had no counterparty, no deadline, and Owner Unassigned. The
carry-through rule only lands a collected value whose slug the target Type also attaches.
The other side of a Contract is a counterparty row, not a Field, and a deadline is a Key
date, so no slug intersection could ever land either. The seed had been patching around
this since M22 (`scripts/seed/requests.mjs`, `workTheRecord`): it added the counterparty
and the Owner by hand after every conversion. The product did not.

**Decision.** The conversion form draws two controls of its own beside the Fields. A
**Counterparty** text box on the contract arm only, and a **Needed by** date box on both
arms. Each prefills from the seeded request field of the same name (`counterparty_name`,
`needed_by`; `INTAKE_CARRY_SLUGS` in `@openlaw/shared`) where the form collected it, and
each is editable. A value in one of these boxes is never listed under "Does not carry":
the form lands it, so claiming it is lost would be untrue. Both are sent as their own body
members, `counterpartyName` and `neededBy`, and the server reads the body and never the
slugs. A form whose Fields are named differently still converts; the triager fills the two
boxes by hand.

On the contract arm, the name is found or created under CTR-011's name lock, matched
without regard to case, and linked as the primary counterparty, with the add route's own
`contract.counterparty_added` entry, `created` included. On the matter arm a counterparty
name is refused with 400: a Matter has no counterparty model, so the box is not drawn
there. The date becomes one Key date labelled "Needed by" on either record, note empty,
with the ordinary `key_date.added` entry. A date in the past still lands. It is a fact the
requester stated, and the deadline surfaces say it is behind us.

The person who completes a contract conversion becomes its Owner as well as its Creator,
the same rule the 2026-09-06 addendum gave the Matter Manager. The triage assignee does
not decide the Owner when someone else converts. Self-assignment narrates nothing beyond
`contract.created`, as the matter arm narrates nothing beyond `matter.created`.

**What does not change.** The Request keeps its `custom_fields` whole; both values are
copied, never moved. The Counterparty and Needed by request Fields stay Fields on the form,
and a Type that attaches a slug-matching Field still carries it as a Field. Nothing about
...
The shared Inbox gives Legal one place to pick up work requiring an owner. A separate queue would require another destination to monitor. The cost is a second tab and two record kinds within Inbox; generated Contracts stay Contracts and do not acquire synthetic Requests to fit the queue.

## INT-008 — Conversion drafts prepare editable Matter creation from cited Request sources

- **Status** — Accepted
- **Date** — 2026-09-11
- **Context** — [#824](https://github.com/juggernog20/OpenLaw/issues/824) and [#826](https://github.com/juggernog20/OpenLaw/issues/826): triage already has structured answers and a conversation, but opening a Matter requires re-entering them. An Analysis run records completed extraction; it is not a proposal waiting for creation.
- **Decision** — A **Conversion draft** is a durable, actor-scoped proposal before ordinary conversion. An Administrator opts into Matter preparation through the AI connector's independently persisted switch, off by default and subordinate to connector enablement. Settings are checked at scheduling, execution, publication and acceptance. Opening Convert to matter shows “Getting matter ready…” before the editable Convert dialog. Cancelling or continuing manually remains available; only submitting the dialog dispositions the Request.
  - The worker uses the configured protocol-neutral provider. Its input carries separately identified, revisioned sources and its output names the source of every quote. Request basics, labelled typed Fields and current, non-deleted conversation messages are collected in chronological order with author and message identity. Old revisions and activity-log text are never inputs. Input content is untrusted data. Quotes require a match within the named source, with Unicode, whitespace and case normalization. Eligible live Types and ordinary Field coercion constrain values.
  - Matter title, Type, description, priority, Needed by and applicable Fields are proposal targets. The model may use explicit later corrections. Unresolved contradictions are retained for review and do not supply a proposed value. No people, confidentiality, lifecycle, risk assessment, template choice or invented references are proposed. Templates remain explicit and ordinary validation, row-lock conversion, paper promotion and thread moves stay atomic.
  - Broadly visible proposals use only Full Thread messages and public scalar Request answers. Legal Only and Working Team messages, Legal Fields and reference Fields are collected with their source identities but excluded from this provider call. This conservative rule prevents restricted text contaminating a synthesized public value even when the provider gives it a public citation. ~~Authorized triagers see an omission notice~~; no restricted observations are persisted in proposals. _Amended 2026-09-21: the dialog draws no banner for restricted sources. Each restricted source already reads "restricted source" in the source list, and the omission is the rule rather than something the triager can act on. The draft still carries the `restricted_sources` warning, and the attachment, Field-budget and source-budget notices are unchanged._ Attachments are disclosed as unread until #827 (superseded by the #827 attachment-reading addendum below). Inputs are bounded to 200 public sources and 180,000 characters, with exclusions disclosed.
  - The draft row owns pending/ready/failed state, target, actor, source fingerprint, configured model, validated proposals, citations and warnings. pg-boss is a wake-up; a recovery sweep requeues pending work. A bounded execution lease prevents concurrent deliveries applying twice. Polling is read-only, repeated preparation is idempotent for a source snapshot, and retry reuses a failed row. Provider calls hold no conversion transaction open. A late result cannot remount or replace a manual dialog.
  - Draft edits and explicit confirmations are human. The server accepts provenance only for an unchanged stored proposal under the same actor, Request and source snapshot, bound to the selected eligible Type. The draft targets the union of eligible Types’ Fields; Type switching displays only the selected Type’s Fields and preserves edits and unrelated per-value markers. Null clears remain null. A changed source makes the proposal unavailable until prepared again. The Request's submitted text is never changed.
  - Matters store an `ai_unverified` map of value slug to Conversion draft id, accepted Type and write time. This mirrors Contract per-value clearing, but does not forge an Analysis run id: Contract provenance retains its existing Analysis run shape. Conversion citations use a shared `ConversionCitation`/`ConversionSuggestion` shape ready for the Contract conversion slice. Matter reads expose only markers for visible Fields; quote text is obtained through a separate, freshly authorized evidence read. Evidence resolves original Request answers or the same message identity after its thread moves. Changed, deleted, archived or unreachable sources return unavailable rather than stale text. Editing or confirming removes only that value's marker; retyping clears applicable Field markers. Needed by uses the Matter map and its Key date edits clear the entry; Next deadline and Home dates carry its unverified status.
- **Rationale** — A reversible proposal reduces re-entry while keeping disposition human. Separating broad-source extraction from restricted material protects derived values as well as evidence. Dedicated draft provenance preserves the glossary's distinction from an Analysis run.
- **Alternatives considered** — Concatenating sources loses quote identity. Feeding restricted text into public synthesis and filtering only citations cannot prevent disclosure. Reusing a Contract Analysis run for an unaccepted Matter proposal contradicts its recorded meaning. Automatically choosing a template would hide a deliberate creation choice.
- **Consequences** — The parked intake AI assistance graduates for this bounded workflow. Attachments, Contract preparation and the Contract post-conversion Analysis run follow in their own slices. Research, inferred status changes and new intake channels remain future work.

### INT-002 addendum (2026-09-11, #826) — a reviewed description may replace verbatim carry-through

INT-002's ordinary conversion still carries the original Request description verbatim. An AI-assisted Matter Convert dialog may instead submit its reviewed synthesized description, an edited description, or an explicit null clear. This changes only the newborn Matter description. The original Request description remains intact in its existing staff and requester views. INT-008 fixes the citation and per-value provenance rules for the new description.

### INT-008 addendum — narrowing a cited Field after conversion

An Administrator may retag ordinary catalog Fields as before. A Business-to-Legal retag is refused only while an unverified Matter value in a different slot still cites that Field's Request answer: hiding the source alone would leave an unreviewed derivative in a broader slot. Confirming or editing the dependent values removes this dependency. The refusal names no Matter or source content. Values in that same catalog Field follow its ordinary tag projection and do not block the retag. Conversion holds a shared lock on its source Fields through acceptance, so a racing retag and creation cannot both pass an obsolete visibility check. Existing drafts become stale when a source tag changes, and evidence reads recheck the current tag.

INT-008 manual continuation restores untouched prepared values to ordinary conversion defaults, preserving human edits and explicit confirmations. Leaving the Matter arm applies the same rule; a discarded draft never reapplies on return.

### INT-008 addendum (2026-09-11, #827) — attachment sources and bounds

Matter preparation includes Request attachments, then attachments on current non-deleted, non-redacted messages. Only Full Thread sources support broad Matter fields; Working Team and Legal Only paper is recorded as omitted, as are restricted answers and messages. Every attachment retains its own identity and revision. A failed or unsupported file does not discard the other sources, and evidence quotes must match that exact source's bounded text.

Attachment reads use the ordinary storage adapter, media sniffing/format routing, document engine PDF extraction and OCR, Office-to-PDF conversion, and email parser. They create no Matter, Contract, Document or Document Version. The Conversion draft stores the bounded read status/text and any stored display rendition reference. Polling and acceptance recompute metadata revisions only. Retrying a failed model call reuses the draft's attachment reads; source changes require a new draft. Filenames and file contents are untrusted provider data and never storage keys or instructions.

Reading order is deterministic: Request answers and chronological messages first (200 sources, 180,000 characters), then Request paper by creation time/id, then message paper by creation time/id. At most 20 eligible attachments are attempted, with 10 MiB per original/rendition, 50 MiB original bytes across the attempts, 30,000 characters per attachment and 180,000 characters across all supplied sources. There are 15 seconds per attachment and 45 seconds for the attachment stage; the model keeps its existing 125-second limit. Sources beyond a count, byte, character or runtime bound have an explicit omission reason. Truncated reads disclose truncation and accept evidence only from the retained text. The Convert dialog lists each attachment's outcome and states the limits; it never labels a partial read as analysis of all paper.

Ordinary conversion retains original Request downloads and records the immutable promoted Version on each Request attachment in its existing transaction. Message attachments move with their comments and retain existing filing semantics: preparation and conversion do not file them. Once filed, their existing exact-Version mapping supplies document evidence. Each evidence/preview read rechecks the live source, comment tier/deletion, parent reach, and the mapped Document's current audience and archived state. An unavailable promoted Request Version never falls back to the retained Request blob. Marking a Document Confidential is refused while an unverified Matter value cites it, until a person confirms or edits the derivative, extending the existing source-Field narrowing rule.

### INT-008 addendum (2026-09-11, #828) — Contract preparation

Conversion drafts also prepare Contracts. The independent Contract preparation switch is off by default and takes effect only while the AI connector is enabled. The same actor, source, quote, retry and publication checks apply to both modules. Contract proposals cover title, live Type, description, priority, counterparty name, Needed by and the chosen Type's attached scalar Fields. Initial preparation may suggest any supported live Type. The Request Type's configured destination stays the fallback until a person selects or confirms a Type in the dialog. That human choice limits subsequent proposals. The model does not choose a Matter template or create reference records. Submitting the counterparty control retains ordinary name matching and creation.

Changing Type or Re-targeting prepares the new target with its own loading status. Untouched proposals return to Request defaults while that preparation runs. Human edits, confirmations and explicit clears survive. Aborting a preparation or returning to manual conversion prevents its late reply from applying. A source snapshot and target module/Type bind every accepted value. A draft for an explicit Type cannot supply provenance to another Type.

Contracts keep Conversion draft provenance in their existing per-field `ai_unverified` map, with `draftId`, accepted Type and write time. Analysis-written entries retain their `runId`; a Conversion draft never receives a fabricated Analysis run identity. Conversion custom Field entries keep the `field:` prefix so a catalog slug cannot collide with an Overview value. Analysis entries retain their existing plain slugs. The Convert dialog and Contract Overview share source evidence and confirmation controls. Needed by points to the ordinary Key date, and its marker follows deadline reads until edited or confirmed. Field retagging and Document audience narrowing apply the same derivative-value protection to Contracts as to Matters.

This also extends the INT-002 reviewed-description addendum to Contract conversion. The newborn Contract receives the reviewed description or explicit null. The original Request remains intact. Preparation remains outside the conversion transaction; creating the record, storing provenance, promoting paper and moving the thread remain one ordinary disposition. The post-conversion Contract Analysis run belongs to #829.

The earlier INT-008 rule that leaving Matter permanently discards preparation is superseded. Each enabled module prepares again on return, while preserving human work.

### INT-008 addendum (2026-09-11) — complete source reading and useful explanations

Conversion preparation and subsequent Request-context Contract Analysis read the complete text of every eligible attachment. This supersedes the 20-attachment, 30,000-character per-file, 180,000-character total and shared byte/time budgets in the #827 addendum. Each original still follows the installation's upload-size bound; an individual stalled read fails after two minutes without suppressing later attachments. Long source text is processed in overlapping sections and supported findings are reconciled with their original source identities. No document tail is silently discarded to fit a model call. Failed reconciliation fails preparation rather than silently dropping a supported finding.

Each new proposal requests a short user-facing justification explaining the cited facts, stored beside its value and citations. The sparkle exposes that explanation only after all supporting sources pass the ordinary current-access and revision checks. It shows relevant quotes rather than repeating the full Request description. This is a conclusion explanation, not internal model deliberation. Historical suggestions without an explanation retain their citations.

Unchanged intake values remain human-provided: they do not acquire AI styling or new AI provenance merely because the model repeats them. The Convert dialog omits generic Field descriptions while retaining actionable validation errors. The client permits long-running preparation while the worker renews its lease; a stalled request or stopped worker still reaches the retry/manual path.
...
**The gap.** Two rules read the word "target" and mean different things. The scope rule (#355)
asks whether a catalog field belongs to the target's module, so a contract-scoped field may
go on any Contract-targeting form. The carry-through rule (INT-007, M21/9) asks whether the
default destination _type_ attaches that exact field, because a collected value lands on the
record only through a Field the type attaches. A field passes the first rule and fails the
second without anybody being told. On R-1 the "NDA review" form collected **Sensitive Data
Sharing**, the NDA contract type did not attach it, and the Convert dialog listed it under
"Does not carry into the contract". The user guide had recorded the gap as a fact to check by
hand; nothing in the product closed it.

**Decision.** The form editor closes it at the moment it opens. When an Administrator attaches
a field to a request type whose default destination is a concrete type, and that type does not
attach the field, the editor asks **"Attach {field} to {type} too?"** and says why in one
sentence: an answer collected on the form carries into the record only through a Field its
type attaches. **Attach to both** puts the field on the form and on the destination type in one
act. **Form only** attaches to the form alone, as before. Esc attaches nothing. The question is
asked, never assumed: the destination type grows only when the Administrator says so, which is
the house rule for every type edit here (refuse and explain, act only when asked).

**When it is not asked.** No default destination type, or a module alone ("Contract" with no
type): there is nothing to attach to. A **Default** field (Our entity, Counterparties, and the
other protected intake questions): it is already part of every record and carries on its own.
A destination type that already attaches the field: the outcome is already the one wanted. A
lookup of the destination type's fields that fails: the form attach is the act asked for, and a
read that could not run must not refuse it.

**The mechanics.** The request-type attach body gains one optional member, `alsoAttachToTarget`,
and its envelope gains `alsoAttachedTo` (the module, the type, its display name, and whether
this call put the field there). The companion attach runs in the attach's own transaction,
after the form row is written and under the same locks, through the destination mount's own
append-order and duplicate reads, and it records the destination mount's own DD-017 entry
(`contract_type_field.attached` or `matter_type_field.attached`, admin-only), so the log reads
as if that type's editor had done it. A destination type that already attaches the field
answers `attached: false` rather than refusing. Three refusals, by name, and each rolls the form
attach back with it: no default destination type, a default field, an archived destination
type. The member exists on the request-type mount alone; the two record-side type editors have
no target and state no rule, and their bodies drop the member on the way in.

**What this does not do.** It does not revisit attachments made before today: a form field the
destination type lacks stays a value with nowhere to land until an Administrator attaches it
to that type by hand, and the Convert dialog goes on naming it. Drawing that state on the
