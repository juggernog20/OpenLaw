...
const results = {
  ...
  runs: [
    ...(previous?.runs ?? []),
    { startedAt: new Date().toISOString(), sections: [...(ONLY ?? ["all"])] },
  ],
...
async function waitOrReload(page, locator, what) {
  ...
  observe(
    `${what} had not appeared after 15 s at ${new URL(page.url()).pathname} (page showed ${JSON.stringify(shown)}); the walkthrough reloaded once.`,
  );
  await page.reload();
  await settle(page, 2000);
...
if (!FX || process.env.NEW_FIXTURES) {
  ...
  const v1 = await upload(
    N,
    `/contracts/${shared.number}/documents`,
    "doc029-support-v1.txt",
    "text/plain",
  );
  const png = await upload(
    N,
    `/contracts/${shared.number}/documents`,
    "doc029-support-image.png",
    "image/png",
  );
  must(
    await N.api("POST", `/contracts/${shared.number}/folders`, { name: title("folder") }),
    "folder",
  );
  const folderId = must(
    await N.api("GET", `/contracts/${shared.number}/folders`),
    "folders",
  ).folders.find((f) => f.name === title("folder")).id;
  ...
  const secVal =
    typeof securityValue === "string"
      ? securityValue
      : (securityValue.value ?? securityValue.label);
  ...
  for (const key of ["buApproval", "buWithdraw", "buArchive"]) {
    ...
    must(
      await N.api("POST", `/contracts/${c.number}/approvals`, { approverIds: [ids.bu] }),
      `${key} approval`,
    );
  ...
  for (const mt of mtypes) {
    const r = await N.api("POST", "/matters", {
  ...
  const reg = must(
    await N.api("POST", "/entities", {
      legalName: title("register entity Ltd"),
      entityTypeId: etypes[0].id,
    }),
    "register entity",
  ).entity;
  const cls = must(
    await N.api("POST", `/entities/${reg.id}/share-classes`, {
      name: "Ordinary",
      votesPerShare: 1,
    }),
    "share class",
  );
  const classId = (cls.register?.classes ?? cls.classes ?? []).find?.(
    (c) => c.name === "Ordinary",
  )?.id;
  ...
  const holderEnt = must(
    await N.api("POST", "/entities", {
      legalName: title("holder entity Ltd"),
      entityTypeId: etypes[0].id,
    }),
    "holder entity",
  ).entity;
  ...
  FX.registerEntity = {
    id: reg.id,
    name: reg.legalName,
    holder: "Marta Kowalczyk",
    holderEntity: { id: holderEnt.id, name: holderEnt.legalName },
  };
  ...
  const kDoc = (
    await upload(N, `/knowledge/${kItem.id}/documents`, "doc029-support-v1.txt", "text/plain")
  ).document;
  ...
  const reviewType = must(await B.api("GET", "/portal/request-types/contract_review"), "rt")
    .requestType.id;
  const rq = must(
...
if (run("T")) {
  ...
  await step(
    ...
    async () => {
      ...
      const newest = await fetch(
        `${MAIL}/api/v1/search?query=${encodeURIComponent(`to:"${BU2_EMAIL}"`)}&limit=1`,
      ).then((r) => r.json());
      check(
        newest.messages[0].ID === second.messageId,
        "the second link is not in the newest message",
      );
      await c2.page.goto(second.href);
      await c2.page.waitForURL(
        (u) => u.pathname.startsWith("/portal") && !u.pathname.startsWith("/portal/login"),
        { timeout: 30000 },
      );
  ...
  await step(
    ...
    C49,
    "legal_team_member",
    "A record or search result is missing: list search, missing Matter link, and a Confidential Contract without access",
    ...
    async () => {
      ...
      must(
        await D.api("DELETE", `/contracts/${FX.adminConfidential.number}/team/${ids.nadia}`),
        "remove nadia",
      );
      check(
        opened === FX.adminConfidential.title && before === 0 && after > 0,
        `opened ${opened} search ${before}/${after}`,
      );
  ...
  await step(
    ...
    async () => {
      ...
      check(
        beforeArchive.h === FX.archived.title || archiveNow.status === 409,
        `before archive ${beforeArchive.h}`,
      );
      check(
  ...
  await step(
    ...
    async () => {
      ...
      const owner = await N.api("PATCH", `/contracts/${FX.request.contract}`, {
        businessOwnerId: null,
      });
      must(
        await N.api("DELETE", `/contracts/${FX.request.contract}/team/${ids.bu}`),
        `remove bu (owner clear ${owner.status})`,
      );
      await go(p, `/portal/requests/${FX.request.number}`);
      const without = {
        path: here_(p),
        h: await h1(p).catch(() => ""),
        text: (await mainText(p)).slice(0, 200),
      };
      check(without.path !== landed, `still opens ${JSON.stringify(without)}`);
      must(
        await N.api("POST", `/contracts/${FX.request.contract}/team`, { userId: ids.bu }),
        "readd bu",
      );
      await go(p, `/portal/requests/${FX.request.number}`);
  ...
  await step(
    ...
    C49,
    "business_user",
    "An edit or action is unavailable: Business User on a shared Contract",
    ...
    async () => {
      ...
      const inputs = await p
        .getByRole("main")
        .locator("region, section")
        .filter({ hasText: /Overview|Fields/ })
      ...
      const forbidden = buttons.filter((b) =>
        /Approve|Reject|Send for signature|Add task|Complete|move contract|Archive|Add approver|Edit/i.test(
          b,
        ),
      );
      check(
        /Security review/.test(fields) && !/Processes personal data/.test(fields),
        `fields ${fields}`,
      );
      check(
        inputs === 0 && forbidden.length === 0 && buttons.includes("Add version"),
        `inputs ${inputs} forbidden ${forbidden} buttons ${buttons}`,
      );
      const patch = await B.api("PATCH", `/contracts/${FX.shared.number}`, {
        customFields: { security_review: FX.shared.securityReview },
      });
  ...
  await step(
    ...
    async () => {
      ...
      const apiTry = await B.api("POST", `/portal/contracts/${FX.confidentialPortal.number}/team`, {
        userId: ids.amara,
      });
  ...
  await step(
    ...
    async () => {
      ...
      const alert = p
        .getByRole("alert")
        .or(p.getByText(/invalid|must be|at most|fewer/i))
        .first();
  ...
  await step(
    ...
    C49,
    "administrator",
    "An edit or action is unavailable: the Administrator reaches types, Statuses and Fields; the Legal Team Member does not",
    ...
    async () => {
      ...
      const adminHeadings = (
        await D.page.getByRole("main").getByRole("heading").allInnerTexts()
      ).map(clean);
      ...
      const formTab = D.page
        .getByRole("main")
        .getByRole("tab", { name: "Form" })
        .or(D.page.getByRole("main").getByRole("link", { name: "Form" }))
        .first();
      await formTab.click();
      await settle(D.page);
      ...
      check(
        adminHeadings.some((h) => /Field/i.test(h)),
        `admin ${adminHeadings}`,
      );
      check(
        formSwitches.some((s) => /Visible on Portal/.test(s.name)),
        `form ${JSON.stringify(formSwitches).slice(0, 300)}`,
      );
  ...
  const docCount = async (S, number) =>
    must(await S.api("GET", `/contracts/${number}/documents`), "docs").documents.length;
  await step(
    T,
    C49,
    "legal_team_member",
    ...
    async () => {
      ...
      check(
        /100 MB upload limit/.test(bigText) &&
          ...
          afterOne === before + 1 &&
          afterTwo === before + 2,
        `big ${bigText} counts ${before}/${afterRefusals}/${afterOne}/${afterTwo}`,
      );
  ...
  await step(
    ...
    C49,
    "legal_team_member",
    "An upload appears twice: Add version is the path for another round of the same paper",
    ...
    async () => {
      ...
      const items = await menuItems(
        p,
        p.getByRole("button", { name: "Actions for doc029-support-note.txt" }).first(),
      );
      ...
      await dialog
        .getByRole("button", { name: /^(Upload|Add version)$/ })
        .last()
        .click();
      await dialog.waitFor({ state: "hidden", timeout: 30000 });
      await settle(p);
      ...
      here_(p);
      check(
        docs.length === before && versions.some((v) => v === 2),
        `docs ${docs.length}/${before} versions ${versions}`,
      );
  ...
  await step(
    ...
    C49,
    "business_user",
    "An upload in the Portal: Add version on the primary Document and on another Document; no folder; reply attachments stay in the conversation",
    ...
    async () => {
      ...
      const folderTry = await uploadFile(p, `/contracts/${FX.shared.number}/documents`, {
        name: "doc030-support-folder-try.txt",
        mimeType: "text/plain",
        buffer: fixture("doc029-support-note.txt"),
        fields: { folderId: FX.shared.folderId },
      });
      const before = must(await N.api("GET", `/contracts/${FX.shared.number}/documents`), "docs")
        .documents.length;
      const comments = await applet(p, "Comments");
      ...
      await comments
        .getByLabel("Choose files for this comment")
        .setInputFiles(fixturePath("doc029-support-note.txt"));
      await comments.getByRole("textbox", { name: "New comment" }).press("Control+Enter");
      ...
      const after = must(await N.api("GET", `/contracts/${FX.shared.number}/documents`), "docs")
        .documents.length;
      await go(p, `/portal/contracts/${FX.shared.number}`);
      ...
      check(
        /Version 2/.test(primary.after) &&
          /Version 2/.test(other.after) &&
          !hasFolder &&
          folderTry.status >= 400,
        `primary ${primary.after} other ${other.after} folder ${hasFolder} ${folderTry.status}`,
      );
      check(
        after === before && listed === 0 && attachmentShown > 0,
        `docs ${before}->${after} listed ${listed} shown ${attachmentShown}`,
      );
  ...
  await step(
    ...
    async () => {
      ...
      const dialog = await uploadDialog(
        p,
        FX.uploads.number,
        fixturePath("doc029-support-damaged.docx"),
      );
      await dialog.waitFor({ state: "hidden", timeout: 30000 });
      await settle(p, 300);
      damaged = must(
        await N.api("GET", `/contracts/${FX.uploads.number}/documents`),
        "docs",
      ).documents.find((d) => d.title === "doc029-support-damaged.docx");
      await docsRegion(p)
        .getByRole("button", { name: "doc029-support-damaged.docx", exact: true })
        .or(docsRegion(p).getByRole("link", { name: "doc029-support-damaged.docx", exact: true }))
        .first()
        .click();
  ...
  for (const role of ["administrator", "legal_team_member"]) {
    ...
    await step(
      ...
      C49,
      role,
      "A notification is missing: staff groups with In-app, Email and Push, Briefing, Devices, Reminder lead times, and profile timezone",
      ...
      async () => {
        ...
        const leadSwitch = switches.find(
          (s) => s.name === "Use the organization's default lead times",
        );
        const leadList = (
          await page
        ...
        check(
          names.some((s) => /In-app$/.test(s)) &&
            names.some((s) => /Email$/.test(s)) &&
            names.some((s) => /Push$/.test(s)),
          `switches ${names}`,
        );
        check(
          briefing === 1 && turnOn === 1 && leadSwitch && tz === 1,
          `briefing ${briefing} turnOn ${turnOn} lead ${JSON.stringify(leadSwitch)} tz ${tz}`,
        );
  ...
  await step(
    ...
    C49,
    "business_user",
    "A notification is missing: Portal Notification settings, and In-app off stops the group's email",
    ...
    async () => {
      ...
      for (const g of [
        ...
        check(
          names.some((n) => n.startsWith(g)),
          `missing ${g} in ${names}`,
        );
      const lead = await p.getByText(/lead time/i).count();
      const caption = clean(
        await p
          .getByText(/In-app off/)
          .first()
          .textContent()
          .catch(() => ""),
      );
      const request = must(
        await N.api("GET", `/requests/${FX.openRequest.number}`),
        "request",
      ).request;
      const mails = async () => {
        const r = await fetch(
          `${MAIL}/api/v1/search?query=${encodeURIComponent(`to:"${BU_EMAIL}" subject:"R-${FX.openRequest.number}"`)}&limit=50`,
        ).then((x) => x.json());
        return (r.messages ?? []).length;
      };
      const reply = async (label) =>
        must(
          await N.api("POST", "/comments", {
            entityType: "request",
            entityId: request.id,
            body: `DOC-030 support Legal reply ${label} ${FX.stamp}`,
      ...
      check(
        before.find((s) => s.name === "Request updates In-app")?.checked,
        "Request updates In-app was not on at the start",
      );
      await inApp.click();
      await settle(p, 1000);
      ...
      const fmt = (list) =>
        list
          .filter((s) => s.name.startsWith("Request updates"))
          .map((s) => `${s.name}${s.checked ? " on" : " off"}${s.disabled ? " (disabled)" : ""}`);
  ...
  await step(
    ...
    C49,
    "legal_team_member",
    "I cannot approve: the named approver finds it under Your approvals in the bell and decides once",
    ...
    async () => {
      ...
      const section = p
        ...
        .or(
          p.locator("section").filter({ has: p.getByRole("heading", { name: "Your approvals" }) }),
        )
        .first();
      ...
      check(
        /Approved/.test(decided) && !after.includes("Reject") && again.status >= 400,
        `row ${decided} after ${after} again ${again.status}`,
      );
  ...
  await step(
    ...
    C49,
    "business_user",
    "I cannot approve (Portal): the named Business User opens Approvals and decides from the review page",
    ...
    async () => {
      ...
      check(
        listText.includes(FX.buApproval.title) && listText.includes(FX.buWithdraw.title),
        `list ${listText.slice(0, 300)}`,
      );
      ...
      const again = await B.api("POST", `/portal/approvals/${FX.buApproval.approvalId}/decision`, {
        decision: "rejected",
      });
      check(
        review.includes(FX.buApproval.title) && review.includes("doc029-support-v1.txt") && noRest,
        `review ${review.slice(0, 300)} headings ${regions}`,
      );
      check(
        controlsAfter === 0 && again.status >= 400,
        `after ${controlsAfter} again ${again.status}`,
      );
  ...
  await step(
    ...
    C49,
    "business_user",
    "I cannot approve (Portal): withdrawing the request or archiving the Contract removes the review page",
    ...
    async () => {
      ...
      check(
        beforeW.includes(FX.buWithdraw.title) && beforeA.includes(FX.buArchive.title),
        "review pages did not open before",
      );
      check(
        !afterW.text.includes(FX.buWithdraw.title) && !afterA.text.includes(FX.buArchive.title),
        `after ${JSON.stringify([afterW, afterA])}`,
      );
      check(
        !list.includes(FX.buWithdraw.title) && !list.includes(FX.buArchive.title),
  ...
  for (const role of ["administrator", "legal_team_member"]) {
    ...
    await step(
      ...
      async () => {
        ...
        const region =
          clean(await S.page.getByRole("main").innerText()).match(
            /Approvals & signing[^]{0,200}/,
          )?.[0] ?? "";
        await go(S.page, `/contracts/${FX.shared.number}/documents`);
        const items = await menuItems(
          S.page,
          S.page.getByRole("button", { name: "Actions for doc029-support-v1.txt" }),
        );
  ...
  await step(
    ...
    C49,
    "administrator",
    "Analysis or AI conversion is unavailable: AI analysis settings, Field prompts and the Request conversion switches",
    ...
    async () => {
      ...
      const prompts = await D.page
        .getByRole("region", { name: "Contract analysis prompts" })
        .count();
      const conn = must(await D.api("GET", "/ai-connector"), "ai").connector;
      aiState = {
        enabled: conn.enabled,
        configured: conn.configured,
        matterPreparation: conn.matterPreparation,
        contractPreparation: conn.contractPreparation,
        contractConversionAnalysis: conn.contractConversionAnalysis,
      };
      const names = switches.map((s) => s.name);
      const card = await D.page
        .getByRole("main")
        .getByRole("heading", { name: "Request conversion" })
        .count();
      check(prompts === 1, `prompts ${prompts}`);
      if (conn.configured)
        check(
          card > 0 &&
            names.includes("Prepare Matter conversions with AI") &&
            names.includes("Prepare Contract conversions with AI"),
          `switches ${names}`,
        );
      else
        check(card === 0 && /Not connected/.test(text), `card ${card} text ${text.slice(0, 120)}`);
  ...
  await step(
    ...
    C49,
    "legal_team_member",
    "Analysis is missing: the Fields header on a live and on an ended Contract; AI conversion falls back to manual",
    ...
    async () => {
      ...
      const ended = must(await N.api("GET", "/contracts?stage=ended&limit=5"), "ended")
        .contracts?.[0];
      let endedRun = null;
      if (ended) {
      ...
      check(
        aiState?.contractPreparation ? preparing : !preparing,
        `preparation ${aiState?.contractPreparation} dialog ${text.slice(0, 200)}`,
      );
...
if (run("K")) {
  ...
  const mcpBefore =
    must(await D.api("GET", "/mcp-settings"), "mcp").settings ??
    must(await D.api("GET", "/mcp-settings"), "mcp");
  const original = {
  ...
  async function requestKey(page, client, { toolsets, scope, note }) {
    ...
    const radios = await dialog
      .getByRole("radio")
      .evaluateAll((els) => els.map((e) => e.parentElement.textContent.trim()));
  ...
  async function openBell(page) {
    ...
    const section = page
      .locator("section")
      .filter({ has: page.getByRole("heading", { name: "Your approvals" }) })
      .first();
    await section.waitFor({ timeout: 15000 });
    return section;
  }
  ...
  try {
    await step(
      ...
      SC,
      "legal_team_member",
      "API key: open API keys from Personal in staff Settings while MCP is off",
      ...
      async () => {
        ...
        check(
          !original.enabled || !original.legalApiKeysEnabled ? request === 0 : request === 1,
          `request ${request}`,
        );
    ...
    await step(
      ...
      SC,
      "business_user",
      "API key: open API keys from Notification settings in the Portal while MCP is off",
      ...
      async () => {
        ...
        check(
          lastPage === "/portal/settings/api-keys" && request === 0,
          `path ${lastPage} request ${request}`,
        );
    ...
    await step(
      ...
      SC,
      "administrator",
      "API key: an Administrator turns on Enable MCP and the groups' API keys switches in Organization > MCP",
      ...
      async () => {
        ...
        check(
          after.enabled && after.legalApiKeysEnabled && after.businessApiKeysEnabled,
    ...
    await step(
      ...
      SC,
      "legal_team_member",
      "API key: Request a key names a Client, Toolsets and a scope, then Cancel request withdraws it",
      ...
      async () => {
        ...
        const opened = await requestKey(p, kname("cancelled client"), {
          toolsets: ["Comments"],
          scope: "read",
        });
        ...
        here_(p);
        check(
          /Client name/.test(opened.text) &&
            /Nothing is selected for you\./.test(opened.text) &&
            opened.checkedAtOpen === 0 &&
            /Note \(Optional\)/.test(opened.text),
          `dialog ${opened.text}`,
        );
        check(
          opened.radios.some((r) => /^Write\./.test(r)) &&
            /Pending approval/.test(pending) &&
            /Cancelled/.test(cancelled),
          `radios ${opened.radios} pending ${pending} cancelled ${cancelled}`,
        );
    ...
    await step(
      ...
      SC,
      "administrator",
      "API key: the Administrator denies from Your approvals in the bell and from API key requests with a note",
      ...
      async () => {
        ...
        await requestKey(N.page, kname("bell denied client"), {
          toolsets: ["Contracts"],
          scope: "read",
        });
        await requestKey(N.page, kname("noted denied client"), {
          toolsets: ["Contracts"],
          scope: "write",
          note: "DOC-030 support fictional reason for a write key",
        });
        const p = D.page;
        await go(p, "/");
        ...
        const item = section
          .getByRole("listitem")
          .filter({ hasText: kname("bell denied client") })
          .first();
        ...
        const denyDialog = await decideInDialog(
          p,
          kname("noted denied client"),
          "Deny",
          "DOC-030 support fictional denial note",
        );
        here_(p);
        await go(N.page, "/settings/api-keys");
        ...
        check(
          buttons.includes("Approve") &&
            ...
            /Denied/.test(bellRow) &&
            /Denied/.test(notedRow) &&
            /fictional denial note/.test(notedRow),
          `buttons ${buttons} rows ${bellRow} | ${notedRow}`,
        );
    ...
    await step(
      ...
      SC,
      "administrator",
      "API key: the Administrator approves from the bell; the requester sees Your key is ready once",
      ...
      async () => {
        await go(N.page, "/settings/api-keys");
        await requestKey(N.page, kname("read client"), {
          toolsets: ["Comments", "Contracts"],
          scope: "read",
        });
        ...
        const item = section
          .getByRole("listitem")
          .filter({ hasText: kname("read client") })
          .first();
        await item.waitFor({ timeout: 20000 });
        ...
        check(
          key.length > 20 &&
            /Send this key in the x-api-key header\./.test(readyText) &&
            /will not show this key again/.test(readyText) &&
            /Active/.test(row) &&
            again === 0 &&
            !pageHasKey,
          `ready ${readyShown} row ${row} again ${again}`,
        );
    ...
    await step(
      ...
      SC,
      "legal_team_member",
      "API key: the key works at the MCP address with x-api-key; a Read key cannot write; Revoke ends it at once",
      ...
      async () => {
        ...
        const noKey = await mcpCall(null, "initialize", {
          protocolVersion: "2025-06-18",
          capabilities: {},
          clientInfo: { name: "doc030-support", version: "1" },
        });
        const init = await mcpCall(key, "initialize", {
          protocolVersion: "2025-06-18",
          capabilities: {},
          clientInfo: { name: "doc030-support", version: "1" },
        });
        ...
        const write = await mcpCall(
          ...
          "tools/call",
          {
            ...
            arguments: {
              entityType: "contract",
              entityId: shared.id,
              body: "DOC-030 support write attempt with a Read key",
            },
          },
          3,
        );
        ...
        check(
          noKey.status === 401 &&
            init.status === 200 &&
            /Nadia/.test(whoText) &&
            /read-only|mcp_read_only/.test(writeText),
          `noKey ${noKey.status} init ${init.status} who ${whoText} write ${writeText}`,
        );
    ...
    await step(
      ...
      SC,
      "administrator",
      "API key: an Administrator's own request approves itself",
      ...
      async () => {
        ...
        await p
          ...
          .click();
        await settle(p);
        await requestKey(p, kname("administrator client"), {
          toolsets: ["Contracts"],
          scope: "read",
        });
        ...
        here_(p);
        check(
          /Active/.test(row) && /Revoked/.test(revoked) && own.length > 20,
          `row ${row} revoked ${revoked}`,
        );
    ...
    await step(
      ...
      SC,
      "legal_team_member",
      "API key: Write is absent from the request dialog while the organization is Read-only",
      ...
      async () => {
        ...
        results.settingsChanged.push({
          ...
          value: "on for one check, then off",
          at: new Date().toISOString(),
        });
        const p = N.page;
        let radios;
        try {
          ...
          must(
            await D.api("PATCH", "/mcp-settings", { readOnly: original.readOnly }),
            "read-only restore",
          );
        }
        await go(p, "/settings/api-keys");
        ...
        const dialog = p.getByRole("dialog", { name: "Request an API key" });
        await dialog.waitFor();
        const radiosAfter = await dialog
          .getByRole("radio")
          .evaluateAll((els) => els.map((e) => e.parentElement.textContent.trim()));
        await closeDialog(p);
        here_(p);
        check(
          radios.length === 1 && /^Read\./.test(radios[0]) && radiosAfter.length === 2,
          `radios ${radios} after ${radiosAfter}`,
        );
    ...
    await step(
      ...
      SC,
      "administrator",
      "API key lifetime: the organization sets 1 to 365 days",
      ...
      async () => {
        ...
        const msg = clean(
          await p
            .getByText(/API key lifetime must be a whole number from 1 to 365 days\./)
            .first()
            .textContent()
            .catch(() => ""),
        );
        ...
        here_(p);
        check(
          msg && saved === original.apiKeyLifetimeDays && final === original.apiKeyLifetimeDays,
    ...
    await step(
      ...
      SC,
      "business_user",
      "API key (Portal): request from Notification settings > API keys; the Administrator approves in API key requests; the key shows once",
...
if (run("R")) {
  ...
  await step(
    ...
    C50,
    "administrator",
    "Roles: the role control offers the fixed account roles only",
  ...
  await step(
    ...
    async () => {
      ...
      const stages = [
        ...new Set(((await mainText(p)).match(/Stage: (\w+)/g) ?? []).map((s) => s.slice(7))),
      ];
  ...
  await step(
    ...
    C50,
    "administrator",
    "Document type lists: Settings > Organization > Documents keeps one list per module",
    ...
    async () => {
      ...
      results.settingsChanged.push({
        section: SECTION,
        setting: "Contract Document type added",
        value: addedType,
        at: new Date().toISOString(),
      });
      here_(p);
      ...
      check(
        tabs.join("|") === "Matters|Contracts|Entities" &&
          six.every((t, i) => contracts.fixed[i] === t) &&
          contracts.fixed.length === 6 &&
  ...
  await step(
    ...
    C50,
    "administrator",
    "What your organization can configure: types and their Forms, Statuses, Fields, Document types, Templates, Approver groups, request types, default lead times, MCP",
    ...
    async () => {
      ...
      for (const [label, url] of [
        ...
        await go(p, url);
        const heads = (await p.getByRole("main").getByRole("heading", { level: 2 }).allInnerTexts())
          .map(clean)
      ...
      await p
        .getByRole("main")
        .getByRole("tab", { name: "Form" })
        .or(p.getByRole("main").getByRole("link", { name: "Form" }))
        .first()
        .click();
      await settle(p);
      ...
      check(
        Object.values(pages).every((v) => !/not found/i.test(v)) &&
          sw.some((s) => /On intake form/.test(s)) &&
          sw.some((s) => /Visible on Portal/.test(s)),
  ...
  await step(
    ...
    async () => {
      ...
      if (revoked === "Something went wrong.") {
        observe(
          `After the Grant was removed, the first reload of Confidential Entity ${FX.entity.id} showed "Something went wrong." instead of "Entity not found"; Reload then showed the not-found page. Not reproduced in a separate probe.`,
        );
  ...
  await step(
    ...
    C50,
    "legal_team_member",
    "Holding: owners come from the share register, show From register and cannot be edited; owners can be an Entity or an Individual",
    ...
    async () => {
      ...
      const members = clean(
        await p.getByRole("region", { name: "Register of members" }).innerText(),
      );
      const declared =
        clean(await p.getByRole("main").innerText()).match(
          /Declared owners not in the register[^]{0,120}/,
        )?.[0] ?? "(no declared owners card)";
      await go(p, `/entities/${FX.registerEntity.holderEntity.id}/ownership`);
      const owned = p
        .getByRole("region", { name: "Holdings in other Entities" })
        .or(p.getByRole("region", { name: "Owned Entities" }))
        .first();
      ...
      const row = owned
        ...
        .filter({ hasText: FX.registerEntity.name })
        .first();
      ...
      const locked = await row
        ...
        .count();
      const hint = await owned
        .getByText("From register")
        .first()
        .getAttribute("title")
        .catch(() => null);
      ...
      await dialog.waitFor();
      const dialogText = clean(await dialog.innerText());
      const ownerType = await dialog
        .getByRole("combobox", { name: "Owner type" })
        .or(dialog.getByRole("radiogroup", { name: "Owner type" }))
        .first()
        .innerText()
        .catch(() => "");
      await closeDialog(p);
      ...
      check(
        /Marta Kowalczyk/.test(members) &&
          ...
          members.includes(FX.registerEntity.holderEntity.name),
        `members ${members}`,
      );
    ...
    async () => {
      ...
      check(
        !/assign/i.test(kdText) &&
          ...
          entity === 1 &&
          counterparties === 1,
        `kd ${kdText} task ${taskText} entity ${entity} cp ${counterparties}`,
      );
  ...
  await step(
    ...
    C50,
    "legal_team_member",
    "Document type: chosen with Type at upload, No type by default, corrected later from the Type column",
    ...
    async () => {
      ...
      await dialog.waitFor({ state: "hidden", timeout: 30000 });
      await settle(p);
      const noteType = clean(
        await p
          .getByRole("combobox", { name: `Type of version 1 of ${typedFile}` })
          .locator("option:checked")
          .textContent(),
      );
      const png = p
        .getByRole("combobox", { name: /^Type of version \d+ of doc029-support-image\.png/ })
        .first();
      await png.selectOption({ label: "Draft · ours" });
      await settle(p, 1500);
      await p.reload();
      await settle(p);
      const pngAfter = clean(
        await p
          .getByRole("combobox", { name: /^Type of version \d+ of doc029-support-image\.png/ })
          .first()
          .locator("option:checked")
      ...
      const primary = clean(
        await p
          .getByRole("combobox", { name: /^Type of version \d+ of doc029-support-v1\.txt/ })
          .first()
          .locator("option:checked")
          .textContent(),
      );
      ...
      check(
        initial === "No type" &&
          options[0] === "No type" &&
          options.includes("Executed") &&
          options.includes(addedType) &&
          noteType === addedType &&
          pngAfter === "Draft · ours",
        `initial ${initial} options ${options} note ${noteType} png ${pngAfter}`,
      );
  ...
  await step(
    ...
    C50,
    "legal_team_member",
    "Executed pin, Comparison and Version delete: the Document actions",
    ...
    async () => {
      ...
      let doc = must(await N.api("GET", `/contracts/${files}/documents`), "docs").documents.find(
        (d) => d.title === "doc029-support-v1.txt",
      );
      if (!doc) {
        doc = (
          await uploadFile(N.page, `/contracts/${files}/documents`, {
        ...
        must(
          await uploadFile(N.page, `/documents/${doc.id}/versions`, {
      ...
      await waitOrReload(
        p,
        p.getByRole("button", { name: "Actions for doc029-support-v1.txt" }),
        "The Actions button for doc029-support-v1.txt",
      );
      ...
      const compareText =
        (await bodyText(p)).match(/Compares v\d+ and v\d+/)?.[0] ?? (await h1(p).catch(() => ""));
      const versions = must(
        await N.api("GET", `/contracts/${files}/documents`),
        "docs",
      ).documents.find((d) => d.id === doc.id).versions.length;
      ...
      check(
        memberItems.includes("Mark as executed copy") &&
          memberItems.includes("Compare with previous") &&
          versions === 2 &&
          versionDelete.length === 0 &&
          adminItems.some((i) => /delete/i.test(i)) &&
          !memberItems.some((i) => /delete/i.test(i)),
        `member ${memberItems} admin ${adminItems} versions ${versions}`,
      );
  ...
  await step(
    ...
    C50,
    "legal_team_member",
    "Knowledge files show the item's Knowledge type; organization settings stay with the Administrator; Personal holds lead times and API keys",
    ...
    async () => {
      ...
      const personal = (
        await p.getByRole("group", { name: "Personal" }).getByRole("link").allInnerTexts()
      ).map(clean);
      check(
        colType === kType &&
          invite === 0 &&
        ...
        `k ${kType}/${colType} invite ${invite} org ${org} personal ${personal}`,
      );
  ...
  await step(
    ...
    C50,
    "business_user",
    "Permissions and Portal files: staff workspace closed; Kind shows fixed kinds and General; earlier Versions read and download",
    ...
    async () => {
      ...
      const links = await docsRegion(p)
        .getByRole("link", { name: /^Download doc029-support-v[12]\.txt, version \d+$/ })
        .evaluateAll((els) => els.map((e) => e.getAttribute("aria-label") || e.textContent));
      const v1 = docsRegion(p)
        .getByRole("link", { name: /^Download .*v1\.txt, version 1$/ })
        .first();
      ...
      check(
        staffPath.startsWith("/portal") &&
          /General/.test(note) &&
          /Draft · ours/.test(png) &&
          /General/.test(primary),
        `staff ${staffPath} note ${note} png ${png} primary ${primary}`,
      );
  ...
  await step(
    ...
    C50,
    "business_user",
    "Approval Request row: the review page does not add the Business User to the team; a related Matter stays closed",
    ...
    async () => {
      ...
      const matter =
        (await bodyText(p)).match(/This Matter does not exist, or you cannot open it\./)?.[0] ?? "";
      here_(p);
      check(
        /This Contract does not exist, or you cannot open it\./.test(text) && !onTeam && matter,
        `text ${text} onTeam ${onTeam} matter ${matter}`,
      );
  ...
  await step(
    ...
    C50,
    "administrator",
    "Put the added Contract Document type back: archive it",
...
if (run("O")) {
  SECTION = "O";
  const articleLinks = (id) => [
    ...new Set(
      [
        ...articleBytes(id)
          .toString("utf8")
          .matchAll(/\]\(([a-z0-9-]+)\.md(#[a-z0-9-]+)?\)/g),
      ].map((m) => `${m[1]}${m[2] ?? ""}`),
    ),
  ];
  ...
  await step(
    ...
    C50,
    "operator",
    "Deployment operator: check the reference's operator facts against the lab containers and compose.yml",
    ...
    async () => {
      const compose = readFileSync(
        path.join(root, `.documentation-labs/${LAB}/source/compose.yml`),
        "utf8",
      );
      const composeUpload = compose
        .split("\n")
        .filter((l) => /MAX_UPLOAD_MB|STORAGE_PATH:|openlaw-files:|pgdata|postgres-data/.test(l))
        .map((l) => l.trim())
        .slice(0, 8);
      const exec = (svc, cmd) =>
      ...
      check(
        baseUrl === BASE && [401, 404].includes(mcp.status),
        `base ${baseUrl} mcp ${mcp.status}`,
      );
  ...
  await step(
    ...
    [C49, C50],
    "operator",
    "Links in the reviewed troubleshooting and reference bytes resolve in the formal documentation",
    ...
    async () => {
      ...
      const bad = [
        ...t.bad.map((b) => `troubleshooting -> ${b}`),
        ...r.bad.map((b) => `reference -> ${b}`),
      ];
      if (bad.length)
        observe(
          `Links not resolved in the bundled 067c1646 edition: ${bad.join("; ")}. The bundled edition predates the reviewed bytes.`,
        );
      check(
        bad.every((b) => /no anchor/.test(b)),
        `unresolved ${bad}`,
      );
