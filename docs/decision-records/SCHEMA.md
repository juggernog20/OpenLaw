...
Source: **ADO-001–010**, grilled 2026-09-13. M35/5 (#848, migration 0120) builds the record identity, template ownership, detection metadata, and immutable form snapshots below. M35/6, migration 0121, adds publication, maps, rules, audience, and the target Type. M35/8, migration 0123, adds delivery settings and outcomes. Audience allowlists remain the intended shape for the following M35 tasks.
...
M35/7 builds the pair, person, answers, state, Word reference, failure, and timestamps. A ready Generation requires a Word reference; a failed Generation requires a JSON object with nonempty string `code` and `detail`. M35/8 adds the PDF and email columns. The Contract columns follow with their ticket. Both version references are indexed for ownership checks and deletion checks.
