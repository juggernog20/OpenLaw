 ...
 * resolution answers the envelope it wrote and the next read answers it
...
function requestApi(
  ...
  const handler = (call: StubCall): Response | undefined => {
    if (call.url.pathname === "/api/v1/requests/45/resolve" && call.method === "POST") {
      resolutions.push(call.body);
...
async function openResolve(user: ReturnType<typeof userEvent.setup>) {
  await user.click(within(await subbar()).getByRole("button", { name: "Resolve" }));
