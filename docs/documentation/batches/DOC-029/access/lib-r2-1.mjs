...
export async function brandingAbove(page, headingText) {
  const api = await fetch(`${BASE}/api/v1/org/branding`)
    .then((r) => r.json())
    .catch(() => null);
...
export async function brandingMockRender(path, headingText) {
  ...
  const run = async (answer) => {
    ...
    return {
      logo,
      logoAbove: !!(imgBox && headBox && imgBox.y + imgBox.height <= headBox.y),
      body,
    };
  };
  const saved = await run({
    name: "DOC-029r2 Fictional Org",
  ...
  return {
    savedLogoShownAbove:
      saved.logo && saved.logoAbove && saved.body.startsWith("DOC-029r2 Fictional Org"),
    brokenLogoHidden: !broken.logo && broken.body.startsWith("DOC-029r2 Fictional Org"),
    noNameShowsOpenLaw:
      noName.body.startsWith("OpenLaw") && !noName.body.includes("Powered by OpenLaw"),
  };
}
