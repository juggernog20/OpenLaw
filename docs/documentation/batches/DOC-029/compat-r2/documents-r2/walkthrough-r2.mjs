...
import { PEOPLE, staffContext, portalContext, api, close, BASE, MAIL, here, root, sleep } from "../../documents/lib-r1.mjs";
...
function labStatus() {
  try {
    const text = execFileSync("mise", ["exec", "--", "node", "scripts/documentation/lab.mjs", "status", "work2"], {
      cwd: root,
      encoding: "utf8",
    });
...
async function shot(page, name) {
  await page.screenshot({ path: path.join(process.env.SHOT_DIR ?? "/tmp/claude-1000", name) }).catch(() => {});
  results.screenshots ??= [];
...
async function versions(role, person) {
  ...
  currentPage = page;
  const contract = await createContract(page, role, `DOC-029r2 documents ${label} versions Contract ${stamp}`);
  await addTeam(page, contract.number, PEOPLE.jonas.email);
  const matter = await createMatter(page, role, `DOC-029r2 documents ${label} versions Matter ${stamp}`);
  ...
  await step(A, S, role, "A Matter upload has no Kind control, and a Matter has no Primary mark or Executed pin", "No Kind column or composer Kind, no Make primary or Mark as executed copy", async () => {
    ...
    const ent = await api(page, "POST", "/entities", { legalName: `DOC-029r2 documents ${label} versions Entity ${stamp} Ltd`, entityTypeId: types[0].id });
  ...
  await step(A, S, role, "Before you start: an archived owning record must be restored before uploading", "The archived record offers no Upload; restoring brings it back; the seam refuses an upload meanwhile", async () => {
    const frozen = await createContract(page, role, `DOC-029r2 documents ${label} archived owner ${stamp}`);
  ...
  const legalOnly = await api(page, "POST", "/comments", undefined, { entityType: "contract", entityId: contractRow.id, body: `DOC-029r2 ${label} Legal only attachment`, visibility: "legal_only", file: part("doc029-services-text.pdf", `doc029-${label.toLowerCase()}-legal-only.pdf`) });
  const teamNote = await api(page, "POST", "/comments", undefined, { entityType: "contract", entityId: contractRow.id, body: `DOC-029r2 ${label} team attachment`, visibility: "full_thread", file: part("doc029-draft-v2.docx", `doc029-${label.toLowerCase()}-round.docx`) });
  ...
  await step(A, S, role, "File a Matter conversation attachment: File to Matter, no Kind, same Confidential switch label", "The Matter offers File to Matter without Kind; filing creates a Matter Document", async () => {
    ...
    const c = await api(page, "POST", "/comments", undefined, { entityType: "matter", entityId: m.id, body: `DOC-029r2 ${label} matter attachment`, visibility: "working_team", file: part("doc029-services-text.pdf", `doc029-${label.toLowerCase()}-matter.pdf`) });
    ...
    expect(kinds === 0 && docs.some((d) => d.title === `DOC-029r2 ${label} matter filed ${stamp}`), `kinds ${kinds}`);
...
async function folders(role, person) {
  ...
  const { context, page } = await staffContext(person);
  currentPage = page;
  const contract = await createContract(page, role, `DOC-029r2 documents ${label} folders Contract ${stamp}`);
  const recUrl = `/contracts/${contract.number}`;
  ...
  await step(A, S, role, "Import step 3: a Matter import has no Version kind control", "The Matter batch dialog shows Destination but no Version kind", async () => {
    const matter = await createMatter(page, role, `DOC-029r2 documents ${label} folders Matter ${stamp}`);
    await openDocumentsTab(page, `/matters/${matter.number}/documents`);
    await docsSection(page).getByRole("button", { name: "Upload", exact: true }).click();
...
async function previews(role, person) {
  ...
  const contract = await createContract(page, role, `DOC-029r2 documents ${label} previews Contract ${stamp}`);
  const recUrl = `/contracts/${contract.number}`;
...
async function portal() {
  ...
  const contract = await createContract(staff, "legal_team_member", `DOC-029r2 documents Portal Contract ${stamp}`);
  const recUrl = `/contracts/${contract.number}`;
  ...
  for (const [name, as] of [["doc029-services-text.pdf", "portal-thread.pdf"], ["doc029-draft-v1.docx", "portal-thread.docx"], ["doc029-bundle.zip", "portal-thread.zip"]]) {
    const thread = await api(staff, "POST", "/comments", undefined, { entityType: "contract", entityId: contractId, body: `DOC-029r2 Portal thread file ${as}`, visibility: "full_thread", file: part(name, as) });
    expect(thread.status === 201, `fixture thread ${thread.status} ${JSON.stringify(thread.body).slice(0, 160)}`);
  }
  const hidden = await createContract(staff, "legal_team_member", `DOC-029r2 documents Portal not on team ${stamp}`);
  ...
  const item = await api(staff, "POST", "/knowledge", { title: `DOC-029r2 documents Portal Knowledge ${stamp}`, knowledgeTypeId: kType });
  expect(item.status === 201, `fixture knowledge ${item.status}`);
  const kid = item.body.knowledgeItem.id;
  record("legal_team_member", "knowledge_item", `DOC-029r2 documents Portal Knowledge ${stamp}`, kid);
  ...
  const req = await api(page, "POST", "/requests", { requestTypeId: legalQuestion.id, departmentId: me.departmentId, title: `DOC-029r2 documents Portal request ${stamp}`, description: "DOC-029 fictional request.", urgency: "low" });
  ...
  await step("document-versions", "V-C26", role, "Before you start and Recover: Business Users cannot change designations, kinds or folders, archive or delete, and get no filing action", "The Portal shows no such controls and the staff seams refuse them", async () => {
    ...
    const attempts = {
      ...
      folder: (await api(page, "POST", `${recUrl}/folders`, { name: "DOC-029r2 portal folder" })).status,
  ...
  await step("document-folders", "V-C27", role, "Before you start: Business Users upload several files at the record root but cannot create folders, move Documents or import a folder structure", "No folder controls or folder picker in the Portal; folder and move seams refuse", async () => {
    ...
    const staffFolder = await api(staff, "POST", `${recUrl}/folders`, { name: `DOC-029r2 portal target ${stamp}` });
    const targetId = (await folderList(staff, recUrl)).find((f) => f.name === `DOC-029r2 portal target ${stamp}`)?.id;
    ...
    const create = await api(page, "POST", `${recUrl}/folders`, { name: "DOC-029r2 portal attempt" });
...
async function repository(role, person) {
  ...
  currentPage = page;
  const me = (await api(page, "GET", "/me")).body.user;
  const contract = await createContract(page, role, `DOC-029r2 documents ${label} repository Contract ${stamp}`);
  const recUrl = `/contracts/${contract.number}`;
  ...
  const folder = await api(page, "POST", `${recUrl}/folders`, { name: `DOC-029r2 ${label} Repo folder` });
  expect(folder.status === 201, `fixture folder ${folder.status}`);
  const folderId = (await folderList(page, recUrl)).find((f) => f.name === `DOC-029r2 ${label} Repo folder`).id;
  ...
  const matter = await createMatter(page, role, `DOC-029r2 documents ${label} repository Matter ${stamp}`);
  const matterDoc = await uploadApi(page, `/matters/${matter.number}/documents`, "doc029-services-text.pdf", { as: `repo-matter-${tag}.pdf` });
  const types = (await api(page, "GET", "/entities/types")).body.entityTypes;
  const ent = await api(page, "POST", "/entities", { legalName: `DOC-029r2 documents ${label} repository Entity ${stamp} Ltd`, entityTypeId: types[0].id });
  expect(ent.status === 201, `fixture entity ${ent.status}`);
  record(role, "entity", ent.body.entity.legalName, ent.body.entity.id);
  ...
  const k = await api(page, "POST", "/knowledge", { title: `DOC-029r2 documents ${label} repository Knowledge ${stamp}`, knowledgeTypeId: kType });
  ...
  const ad = await api(page, "POST", "/auto-docs", { name: `DOC-029r2 documents ${label} repository Auto-Doc ${stamp}` });
...
async function archive(role, person) {
  ...
  const { context, page } = await staffContext(person);
  currentPage = page;
  const contract = await createContract(page, role, `DOC-029r2 documents ${label} archive Contract ${stamp}`);
  const recUrl = `/contracts/${contract.number}`;
  const tag = `${label.toLowerCase()}-${stamp}`;
...
async function knowledgeSection(role, page, A, S, label) {
  await step(A, S, role, "Archive a Document step 1: a Knowledge Item's Documents section archives and restores; it has Set as primary and no Document folders", "Archive and Restore work in the Knowledge Documents section; Set as primary is offered; no New folder", async () => {
    const kType = (await api(page, "GET", "/knowledge?limit=1")).body.knowledgeItems?.[0]?.knowledgeTypeId;
    const k = await api(page, "POST", "/knowledge", { title: `DOC-029r2 documents ${label} archive Knowledge ${stamp}`, knowledgeTypeId: kType });
    expect(k.status === 201, `fixture knowledge ${k.status}`);
