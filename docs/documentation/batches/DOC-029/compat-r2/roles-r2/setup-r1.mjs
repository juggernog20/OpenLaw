...
const co = must(
  await api(d.page, "POST", "/contracts", {
    title: "DOC-029r2 roles open contract",
    contractTypeId: nda,
...
const mo = must(
  await api(d.page, "POST", "/matters", {
    title: "DOC-029r2 roles open matter",
...
const cc = must(
  await api(n.page, "POST", "/contracts", {
    title: "DOC-029r2 roles confidential contract",
    contractTypeId: nda,
    managerId: ids.nadia,
    isConfidential: true,
...
const mc = must(
  await api(n.page, "POST", "/matters", {
    title: "DOC-029r2 roles confidential matter",
    matterTypeId: commercial,
    managerId: ids.nadia,
    isConfidential: true,
...
const ent = must(
  await api(n.page, "POST", "/entities", {
    legalName: "DOC-029r2 roles confidential entity Ltd",
...
for (const [visibility, body] of [
  ["legal_only", "DOC-029r2 roles Legal only note"],
  ["working_team", "DOC-029r2 roles Working team note"],
  ["full_thread", "DOC-029r2 roles Full thread note"],
]) {
  must(
...
for (const [who, email, key] of [
  ...
  const rq = must(
    await api(b.page, "POST", "/requests", {
      ...
      departmentId: sales,
      title: `DOC-029r2 roles ${who} request`,
      description: `DOC-029r2 roles fictional ask from ${who}.`,
      urgency: "medium",
  ...
  const converted = must(
    await api(n.page, "POST", `/requests/${rq.number}/convert`, {
      title: `DOC-029r2 roles ${who} converted contract`,
      contractTypeId: nda,
    }),
    `convert ${who}`,
  );
