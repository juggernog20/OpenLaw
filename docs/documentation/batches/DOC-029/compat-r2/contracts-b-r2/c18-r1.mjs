...
export async function runC18(ctx, step) {
  ...
  await step(
    ...
    async () => {
      parentTitle = `DOC-029r2 contracts-b C18 ${tag}`;
      ...
      const cp = await actor.api("POST", `/contracts/${num}/counterparties`, {
        name: `DOC-029r2 contracts-b Counterparty ${tag}`,
      });
      expectThat(cp.status < 300, `counterparty answered ${cp.status}`);
      ctx.records.push({
        article: "terms-and-renewals",
        role,
  ...
  await step(
    ...
    async () => {
      ...
      expectThat(
        [s2, s3, s4].every((s) => s === 200),
        `PATCH statuses ${s2} ${s3} ${s4}`,
      );
      ...
      expectThat(
        ...
        `saved ${JSON.stringify([c.termType, c.effectiveDate, c.expiryDate, c.noticePeriodDays])}`,
      );
  ...
  await step(
    ...
    async () => {
      ...
      expectThat(
        !/No term dates|No effective date|No expiry date/.test(timelineText),
  ...
  await step(
    ...
    async () => {
      ...
      expectThat(
        ...
        `saved ${JSON.stringify([c.termType, c.expiryDate, c.renewalPeriodMonths])}`,
      );
  ...
  await step(
    ...
    async () => {
      ...
      expectThat(
        ...
        `after Fixed ${JSON.stringify([fixed.termType, fixed.renewalPeriodMonths, fixed.expiryDate])}`,
      );
      await overview();
      await withResponse(p, isPatchOf(num), () =>
        p.getByRole("combobox", { name: "Term type" }).selectOption({ label: "Auto-renewing" }),
      ...
      expectThat(
        ...
        `final ${JSON.stringify([c.termType, c.renewalPeriodMonths, c.expiryDate, c.noticeDeadline])}`,
      );
  ...
  await step(
    ...
    async () => {
      const t = await actor.api("POST", `/contracts/${num}/tasks`, {
        title: `DOC-029r2 contracts-b C18 early task ${tag}`,
        dueDate: "2026-12-01",
      });
  ...
  await step(
    ...
    async () => {
      ...
      const status = await withResponse(
        p,
        (r) => r.request().method() === "POST" && r.url().endsWith(`/contracts/${num}/renewal`),
      ...
      await overview();
      const lastRenewal = await text(
        p.getByText("Last renewal", { exact: true }).locator("xpath=.."),
      );
      ...
      expectThat(
        rows === 1 && !/—$/.test(lastRenewal),
        `renewal rows ${rows}; last renewal "${lastRenewal}"`,
      );
      expectThat(
  ...
  await step(
    ...
    async () => {
      ...
      const moved = await otherSession.api("PATCH", `/contracts/${num}`, {
      ...
      await p.getByRole("button", { name: "Renew", exact: true }).click();
      const fresh = await p
        .getByRole("dialog", { name: "Confirm renewal" })
        .getByRole("textbox", { name: "New expiry date" })
        .inputValue();
      await p
        .getByRole("dialog", { name: "Confirm renewal" })
      ...
      const rows = await p
        ...
        .count();
      expectThat(
        status === 409 && /already moved/.test(refusal),
  ...
  await step(
    ...
    async () => {
      ...
      await withResponse(
        p,
        (r) => r.request().method() === "POST" && r.url().includes(`/contracts/${num}/documents`),
        () => up.getByRole("button", { name: "Upload", exact: true }).click(),
        30000,
      );
      ...
      await add
        .locator("input[type=file]")
        .first()
        .setInputFiles(fix("doc029-renewal-amendment.pdf"));
      await withResponse(
        p,
        (r) => r.request().method() === "POST" && r.url().includes("/versions"),
        () =>
          add
      ...
      const versionsOf = async () =>
        (await actor.api("GET", `/contracts/${num}/documents`)).json.documents[0].versions;
      const vs = await versionsOf();
      const summary1 = vs
        .map((v) => `v${v.versionNumber} ${v.kind} executed=${v.isExecuted}`)
        .join("; ");
      ...
      expectThat(
        v2 && v2.kind === "amendment" && !v2.isExecuted && v1.isExecuted,
        `versions ${summary1}`,
      );
      await p.goto(`${BASE}/contracts/${num}/documents`);
      await p.reload();
      ...
      await row
        .getByRole("button", {
          name: /Actions for version 2 of|Actions for doc029-services-agreement.pdf/,
        })
        .first()
        .click();
      await p.getByRole("menuitem", { name: "Mark as executed copy" }).click();
      ...
      expectThat(
        vs2.find((v) => v.versionNumber === 2).isExecuted &&
          !vs2.find((v) => v.versionNumber === 1).isExecuted,
        `after mark ${summary2}`,
      );
  ...
  const routed = async (label, radio, button, newTitle, check) => {
    ...
    const typeLabel = await create
      .getByRole("combobox", { name: "Contract type" })
      .evaluate((el) => el.options[el.selectedIndex].text);
    const ownerLabel = await create
    ...
    const teamNames = JSON.stringify(
      (await actor.api("GET", `/contracts/${childNum}`)).json.team ?? [],
    );
    ...
    expectThat(
      facts.counterparty &&
        facts.entity &&
        facts.value?.amount === 990000 &&
        facts.termType === "auto_renew" &&
        facts.expiry === parent.expiryDate,
      `copied ${JSON.stringify(facts)}`,
    );
    expectThat(
      ownerLabel === "Unassigned" &&
        facts.manager === null &&
        facts.status === "Draft" &&
        facts.priority === "medium" &&
        facts.risk === null &&
        facts.confidential === false,
      `not copied ${ownerLabel} ${JSON.stringify(facts)}`,
    );
    expectThat(
      !teamNames.includes(PEOPLE.businessUser.name) && !teamNames.includes(PEOPLE[other].name),
      `team ${teamNames.slice(0, 300)}`,
    );
  ...
  await step(
    ...
    () =>
      routed(
        ...
        `DOC-029r2 contracts-b C18 child ${tag}`,
  ...
  await step(
    ...
    () =>
      routed(
        ...
        "Open the successor",
        `DOC-029r2 contracts-b C18 successor ${tag}`,
  ...
  await step(
    ...
    async () => {
      const created = await actor.api("POST", "/contracts", {
        title: `DOC-029r2 contracts-b C18 lapsed ${tag}`,
        contractTypeId: typeId("NDA"),
        managerId: userId(PEOPLE[role].name),
      });
      ...
      const e = await actor.api("PATCH", `/contracts/${lapsed}`, {
        expiryDate: past,
      ...
      expectThat(
        c.expiryDate === past && proposal > past,
        `expiry ${c.expiryDate} proposal ${proposal}`,
      );
  ...
  await step(
    ...
    async () => {
      ...
      const write = await actor.api("POST", `/contracts/${num}/renewal`, {
