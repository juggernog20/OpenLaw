...
Move the Contract to the intended Signature Status before sending if completion should advance it to Active. The send action itself does not move its Status. The send dialog says that the Contract advances to Active only if it is still in the Signature Stage. [Manual hand-off](manual-signing.md) remains available without a connector.
...
OpenLaw gets status updates from the provider. With **Polling**, which is the default, an update can take about 15 to 20 minutes while the worker runs. With **Webhook**, updates arrive when the provider delivers them, and the polling check still runs as a fallback. The note under a live row names the configured mode. If an Administrator disables the connector while an Envelope is out, the note says that automatic status updates are unavailable. Watch the Envelope row and the header's signing status. **Signed** means the provider reported completion; the executed file can still be filing. Wait for **Executed copy**, open it, and check the returned paper on the original Document chain and its executed designation.
...
**Send for signature** is absent when the connector is unavailable or disabled, no primary Document is set, a live Envelope already exists, or the Contract is archived. Check those conditions before asking an Administrator to inspect the connector.
