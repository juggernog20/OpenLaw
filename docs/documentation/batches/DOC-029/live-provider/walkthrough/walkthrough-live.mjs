...
if (!PASSWORD || !["c43", "c42"].includes(PHASE))
  throw new Error("Set LAB_PASSWORD and PHASE=c43|c42");
...
async function step(id, article, role, method, action, fn, { kindOnFail = "product-bug" } = {}) {
  ...
  try {
    ...
    entry.actual = `${String(error?.message ?? error)
      .split("\n")
      .slice(0, 4)
      .join(" ")}`;
    entry.result = "fail";
  ...
  await context.route(/\/api\/v1\/signing-connectors\/docusign(\?.*)?$/, async (route) => {
    ...
    if (m === "PUT") {
      ...
      if (!body || body.updateMode !== "webhook") {
        log.guard.push({
          at: new Date().toISOString(),
          blocked: "PUT signing connector (not a Webhook refusal check)",
        });
        return route.abort();
      }
      const stripped = { ...body };
      const removed = [];
      for (const k of ["webhookSecret", "privateKey"])
        if (k in stripped) (delete stripped[k], removed.push(k));
      log.guard.push({
        at: new Date().toISOString(),
...
async function findOrCreateContract(who, title, typeName, managerEmail) {
  ...
  const created = await api(who, "POST", "/contracts", {
    title,
...
const hasPrimary = async (who, n) =>
  ((await api(who, "GET", `/contracts/${n}/documents`)).json.documents ?? []).some(
    (d) => d.isPrimary,
  );
...
async function openAiSettings(page) {
  ...
  if ((await provider.count()) && (await provider.getAttribute("aria-expanded")) === "false")
...
const analysisCard = (page) =>
  page.locator("section[aria-labelledby='contract-ai-analysis-heading']");
...
async function c43(browser) {
  ...
  expect(
    before.configured &&
      before.enabled &&
      before.preset === "openrouter" &&
      before.model === EXPECTED_MODEL &&
      before.hasApiKey,
    `unexpected start state ${JSON.stringify(before)}`,
  );
  const writes = [];
  page.on("request", (r) => {
    if (
      ["PUT", "POST", "PATCH", "DELETE"].includes(r.method()) &&
      /\/api\/v1\/(ai-|contracts\/\d+\/analysis)/.test(r.url())
    )
  ...
  await step(
    ...
    "administrator",
    "browser-walkthrough",
    "Open Settings > Organization > AI analysis; check the card order, chip and Request conversion switches",
    async () => {
      ...
      const promptsCollapse = await page
        .getByRole("button", { name: "Field prompts", exact: true })
        .count();
      const sw = [];
      for (const id of [
        "matter-preparation",
  ...
  await step(
    ...
    "administrator",
    "live-provider-check",
    "Expand Provider; check the saved OpenRouter settings; Test connection",
    async (e) => {
      ...
      e.testMs = ms;
      expect(
        preset === "openrouter" && model === EXPECTED_MODEL && keyBlank && keepHint,
        `preset=${preset} model=${model} keyBlank=${keyBlank} keepHint=${keepHint}`,
      );
  ...
  await step(
    ...
    "administrator",
    "live-provider-check",
    "Load models with the stored key; Search models; Refresh models; check that nothing is saved",
    async (e) => {
      const w0 = writes.length;
      const load = page.getByRole("button", { name: "Load models" });
      const canLoad = await load.isEnabled();
      const t0 = Date.now();
      const listed = page.waitForResponse((r) => r.url().endsWith("/api/v1/ai-connector/models"), {
        timeout: 90000,
      });
      await load.click();
      ...
      await wait(300);
      const filtered = (await page.locator("select#ai-model option").allInnerTexts()).map((t) =>
        t.trim(),
      );
      ...
      const listed2 = page.waitForResponse((r) => r.url().endsWith("/api/v1/ai-connector/models"), {
        timeout: 90000,
      });
      ...
      expect(
        selected === EXPECTED_MODEL && selectedAfterRefresh === EXPECTED_MODEL,
        `selection changed ${selected}/${selectedAfterRefresh}`,
      );
      expect(
        after.model === EXPECTED_MODEL &&
          after.updatedAt === before.updatedAt &&
          newWrites.length === 0,
        `saved state changed or writes ${newWrites}`,
      );
  ...
  const NOTICE_TEXT =
    "Extract the notice period for non-renewal or termination as a number of days stated in the paper. Do not infer a period when the paper states none.";
  await step(
    "A04",
    "configure-analysis",
    "administrator",
    "browser-walkthrough",
    "Field prompts: edit the notice-period prompt, press Enter, wait for the save result",
    async () => {
      await page.reload();
      await page.getByRole("heading", { name: "Field prompts" }).waitFor();
      ...
      const stored = (await api(admin, "GET", "/ai-field-prompts")).json.prompts.find(
        (p) => p.slug === NOTICE,
      );
  ...
  await step(
    ...
    "administrator",
    "browser-walkthrough",
    "Field prompts: Shift+Enter inserts a line break; clearing the prompt and leaving restores the saved text",
    async () => {
      const box = page.locator(`#ai-field-prompt-${NOTICE}`);
      const puts = [];
      const l = (r) =>
        r.method() === "PUT" && r.url().includes("/api/v1/ai-field-prompts") && puts.push(1);
      page.on("request", l);
      await box.click();
      await box.press("End");
      ...
      expect(
        afterClear === NOTICE_TEXT && stored.prompt === NOTICE_TEXT,
        `after clear box="${afterClear.slice(0, 40)}" stored="${stored.prompt.slice(0, 40)}"`,
      );
  ...
  await step(
    ...
    "administrator",
    "browser-walkthrough",
    "Field prompts: type in the effective-date prompt, press Escape, reload",
    async (e) => {
      const slug = "effective_date";
      const box = page.locator(`#ai-field-prompt-${slug}`);
      const original = (await api(admin, "GET", "/ai-field-prompts")).json.prompts.find(
        (p) => p.slug === slug,
      );
      ...
      if (stored.overridden) {
        ...
        const back = (await api(admin, "GET", "/ai-field-prompts")).json.prompts.find(
          (p) => p.slug === slug,
        );
  ...
  await step(
    ...
    "administrator",
    "live-provider-check",
    "Create the fictional Contract DOC-029 live analysis, upload its primary Document, wait for the automatic Analysis run",
    async (e) => {
      ...
      if (!(await hasPrimary(admin, c.number)))
        doc = await uploadPrimary(admin, c.number, "doc029-live-analysis.docx");
      ...
      e.autoRun = {
        ...
        providerMs:
          run.startedAt && run.finishedAt
            ...
            : null,
        uploadToFinishMs: Date.now() - uploadedAt,
      };
      ...
      expect(
        run.model === EXPECTED_MODEL && run.preset === "openrouter",
        `run model ${run.preset}/${run.model}`,
      );
  ...
  await step(
    ...
    "administrator",
    "browser-walkthrough",
    "Read the AI analysis card: completion sentence, results, Unverified values",
    async (e) => {
      ...
      e.savedValues = {
        ...
        noticePeriodDays: detail.noticePeriodDays ?? null,
        termType: detail.termType ?? null,
      };
      expect(
        completed && completed.includes(EXPECTED_MODEL),
        `completion sentence missing: ${cardText.slice(0, 300)}`,
      );
      return `Card sentence: "${completed}". Result rows (${rows.length}): ${rows
        .map((r) => r.replace(/\s+/g, " ").trim())
        .join(" || ")
        .slice(
          0,
          900,
        )}. "Unverified" markers on the page: ${unverified}. Saved values read from the Contract: ${JSON.stringify(e.savedValues)}. Accuracy is not claimed.`;
    },
  );
  ...
  await step(
    ...
    "administrator",
    "live-provider-check",
    "Rerun deliberately after the prompt edit: select Run analysis and wait",
    async (e) => {
      await openFields(page, analysisNumber);
      const card = analysisCard(page);
      ...
      await card
        .getByText(/Completed .* on Version \d+ with /)
        .waitFor({ timeout: 30000 })
        .catch(() => {});
      const sentence = (await text(card)).match(
        /(Completed|Failed) .*? on Version \d+ with \S+?\.(?= |$)/,
      )?.[0];
      e.manualRun = {
        ...
        providerMs:
            ...
            ? Date.parse(done.finishedAt) - Date.parse(done.startedAt)
            : null,
        clickToFinishMs: Date.now() - t0,
      };
      const noticeRow = (await card.locator("li").allInnerTexts()).find((r) => /notice/i.test(r));
      expect(
        done.state === "ready" && done.trigger === "manual" && done.model === EXPECTED_MODEL,
        `manual run ${done.trigger}/${done.state}/${done.failure}`,
      );
  ...
  await step(
    ...
    "administrator",
    "browser-walkthrough",
    "Open the evidence sparkle for Notice period (days) on Overview; check the doc panel opens on the cited Version",
    async () => {
      ...
      const names = await sparkles.evaluateAll((els) =>
      ...
      const panel = await page
        ...
        .catch(() => []);
      expect(
        names.length > 0 && aiCalls.length === 0,
        `sparkles=${names.length} aiCalls=${aiCalls.length}`,
      );
  ...
  await step(
    ...
    "administrator",
    "browser-walkthrough",
    "Failure the articles document: Analysis refused for a Contract without a primary Document while the connector passes its test",
    async () => {
      const c = await findOrCreateContract(
        admin,
        "DOC-029 live analysis no paper",
        "NDA",
        MEMBER.email,
      );
  ...
  await step(
    ...
    "administrator",
    "browser-walkthrough",
    "Turn off Use AI analysis; check chip, Test connection, Request conversion switches, Contract card and refusal",
    async (e) => {
      await openAiSettings(page);
      const t0 = Date.now();
      ...
      const runCount = cardCount
        ? await card.getByRole("button", { name: "Run analysis" }).count()
        : 0;
      const unverified = await page.getByText("Unverified", { exact: true }).count();
      ...
      expect(
        res.status() === 200 && !state.enabled && state.model === EXPECTED_MODEL && state.hasApiKey,
        `disable ${res.status()} ${JSON.stringify(state)}`,
      );
      expect(testDisabled && sws.every(Boolean), `test=${testDisabled} switches=${sws}`);
      expect(
        runCount === 0 && refusal.status === 409,
        `run button ${runCount} refusal ${refusal.status}`,
      );
  ...
  await step(
    ...
    "legal_team_member",
    "browser-walkthrough",
    "While off: a Legal Team Member sees no Run analysis and cannot open or change the AI settings",
    async () => {
      const member = await signIn(browser, MEMBER);
      await openFields(member.page, analysisNumber);
      const card = analysisCard(member.page);
      const runOff = (await card.count())
        ? await card.getByRole("button", { name: "Run analysis" }).count()
        : 0;
      ...
      const prompts = await api(member, "PUT", "/ai-field-prompts", {
        slug: NOTICE,
        prompt: "DOC-029 member attempt",
      });
      await member.context.close();
      expect(
        runOff === 0 && apiRes.status === 403 && prompts.status === 403,
        `run=${runOff} api=${apiRes.status} prompts=${prompts.status}`,
      );
  ...
  await step(
    ...
    "administrator",
    "live-provider-check",
    "Turn Use AI analysis on again; Test connection; Legal Team Member sees Run analysis again",
    async (e) => {
      await openAiSettings(page);
      ...
      await until(
        ...
        "Test connection stayed disabled",
        10000,
        300,
      );
      const t0 = Date.now();
      ...
      const member = await signIn(browser, MEMBER);
      await openFields(member.page, analysisNumber);
      const runOn = await analysisCard(member.page)
        .getByRole("button", { name: "Run analysis" })
        .count();
      await member.context.close();
      expect(
        res.status() === 200 && state.enabled && !failed && state.model === EXPECTED_MODEL,
        `enable ${res.status()} failed=${failed}`,
      );
  ...
  await step(
    ...
    "administrator",
    "browser-walkthrough",
    "Load models failure: Custom endpoint with a fictional key against a missing path (not saved)",
    async () => {
      await openAiSettings(page);
      ...
      const res = await listed;
      const err = page
        .getByText("Models could not be loaded")
        .or(page.locator(".text-status-danger-fg"))
        .first();
      ...
      expect(
        res.status() >= 400 &&
          ...
          state.hasApiKey,
        `status=${res.status()} ${JSON.stringify(state)}`,
      );
  ...
  await step(
    ...
    "administrator",
    "browser-walkthrough",
    "Reset the notice-period prompt to default; check the end state",
    async () => {
      await openAiSettings(page);
      const reset = page.getByRole("button", { name: /^Reset .*notice.* to default$/i });
      const put = page.waitForResponse(
      ...
      expect(
        overridden.length === 0 &&
          ...
          state.model === EXPECTED_MODEL &&
          state.hasApiKey,
        `overridden=${overridden} ${JSON.stringify(state)}`,
      );
...
async function c42(browser) {
  ...
  expect(
    before.configured &&
      before.enabled &&
      before.environment === "demo" &&
      before.updateMode === "polling" &&
      before.hasPrivateKey,
    `unexpected start ${JSON.stringify(before)}`,
  );
  const sigWrites = [];
  page.on("request", (r) => {
    if (
      ["PUT", "POST", "PATCH", "DELETE"].includes(r.method()) &&
      /signing-connectors|envelopes/.test(r.url())
    )
      sigWrites.push(`${r.method()} ${new URL(r.url()).pathname}`);
  });
  ...
  await step(
    ...
    "administrator",
    "browser-walkthrough",
    "Open Settings > Organization > Integrations > E-signature; expand DocuSign; check the saved Polling connector",
    async () => {
      await page.goto(`${BASE}/settings/profile`);
      const org = page.getByRole("group", { name: "Organization" });
      await org.waitFor({ timeout: 20000 });
      ...
      expect(
        env === "demo" &&
          ...
          chip &&
          swOn === "true",
        `env=${env} mode=${mode} rsaBlank=${rsaBlank} callback=${callback} hmac=${hmac} chip=${chip} switch=${swOn}`,
      );
  ...
  await step(
    ...
    "administrator",
    "live-provider-check",
    "Test connection against DocuSign Demo",
    async (e) => {
      const t0 = Date.now();
      await page.getByRole("button", { name: "Test connection" }).click();
  ...
  await step(
    ...
    "administrator",
    "browser-walkthrough",
    "Choose Webhook without saving; check the extra fields; Save with the HMAC secret blank",
    async () => {
      ...
      const webhookHint = await page
        .getByText(/^Receives updates as DocuSign delivers them/)
        .isVisible();
      ...
      expect(
        required &&
          missing &&
          sigWrites.length === n &&
          state.updateMode === "polling" &&
          state.updatedAt === before.updatedAt,
        `required=${required} missing=${missing} writes=${sigWrites.slice(n)}`,
      );
  ...
  await step(
    ...
    "administrator",
    "browser-walkthrough",
    "Webhook refusals: http:// callback URL, and an HTTPS URL with a query (secret removed from each request by the guard)",
    async () => {
      const tries = [];
      for (const url of [
        "http://gateway.doc029.example/api/v1/signing/docusign/webhook",
        "https://gateway.doc029.example/api/v1/signing/docusign/webhook?token=x",
        "https://user:pw@gateway.doc029.example/hook",
      ]) {
        ...
        const pane = await page
          .getByText(/invalid|HTTPS|Paste the DocuSign Connect HMAC/i)
          .allInnerTexts();
        tries.push({
          url: url.replace(/\/\/[^@/]*@/, "//***@"),
          status: res.status(),
          api: body.detail ?? body.title ?? null,
          pane: pane.map((t) => t.trim()).filter((t) => t.length < 200),
        });
      }
      const state = safeSig(await sigState(admin));
      await page.reload();
      ...
      const rsaBlank = (await page.getByLabel("RSA private key").inputValue()) === "";
      expect(
        tries.every((t) => t.status === 400),
        JSON.stringify(tries),
      );
      expect(
        state.updateMode === "polling" &&
          !state.hasWebhookSecret &&
          state.updatedAt === before.updatedAt &&
          state.webhookUrlOverride === null,
        `stored changed ${JSON.stringify(state)}`,
      );
  ...
  await step(
    ...
    "administrator",
    "browser-walkthrough",
    "Prepare the fictional Contract DOC-029 live signing with a primary Document containing /sig/; move it to Out for signature",
    async (e) => {
      ...
      if (current.statusName !== "Out for signature") {
        const answered = page.waitForResponse(
          (r) =>
            r.url().endsWith(`/api/v1/contracts/${c.number}`) && r.request().method() === "PATCH",
        );
        await control.click();
        await page
      ...
      e.autoAnalysisOnSigningPaper = auto
        ? { trigger: auto.trigger, state: auto.state, model: auto.model }
        : null;
  ...
  await step(
    ...
    "administrator",
    "browser-walkthrough",
    "Turn off Send for signature from records; check Test connection and the Contract; turn it back on and retest",
    async (e) => {
      await openSigningSettings(page);
      const sw = page.getByRole("switch", { name: "Send for signature from records" });
      const off = page.waitForResponse(
        (r) => r.url().endsWith("/signing-connectors/docusign/disable"),
        { timeout: 20000 },
      );
      await sw.click();
      ...
      if (testEnabledWhileOff) {
        ...
        await bad
          ...
          .catch(() => {});
        testMsg =
          (await page.locator("p[aria-live]").allInnerTexts()).join(" | ").trim() || "(no message)";
      }
      await openApprovals(page, signingNumber);
      const sendWhileOff = await page
        .getByRole("region", { name: "Approvals & signing" })
        .getByRole("button", { name: "Send for signature" })
        .count();
      await openSigningSettings(page);
      const on = page.waitForResponse(
        (r) => r.url().endsWith("/signing-connectors/docusign/enable"),
        { timeout: 20000 },
      );
      ...
      expect(
        offRes.status() === 200 &&
          ...
          state.updateMode === "polling" &&
          state.hasPrivateKey,
        `off=${offRes.status()} on=${onRes.status()} send ${sendWhileOff}/${sendWhileOn} ${JSON.stringify(state)}`,
      );
  ...
  await step(
    ...
    "administrator",
    "browser-walkthrough",
    "Open Send for signature on C-signing; fill Version, Signer name and Subject; Cancel (nothing is sent)",
    async (e) => {
      await openApprovals(page, signingNumber);
      await page
        .getByRole("region", { name: "Approvals & signing" })
      ...
      const versions = (await dialog.getByLabel("Version").locator("option").allInnerTexts()).map(
        (t) => t.trim(),
      );
      ...
      expect(
        sent.length === 0 && (env.envelopes?.length ?? 0) === 0,
        `envelopes ${env.envelopes?.length} writes ${sent}`,
      );
  ...
  await step(
    ...
    "legal_team_member",
    "browser-walkthrough",
    "Legal Team Member cannot open or change the Signing connector but sees Send for signature on the reached Contract",
    async () => {
      const member = await signIn(browser, MEMBER);
      ...
      const send = await member.page
        .getByRole("region", { name: "Approvals & signing" })
        .getByRole("button", { name: "Send for signature" })
        .count();
      await member.context.close();
      expect(
        g.status === 403 && t.status === 403 && send === 1,
        `get=${g.status} test=${t.status} send=${send}`,
      );
...
try {
  ...
  log.abort = String(error?.message ?? error)
    .split("\n")
    .slice(0, 3)
    .join(" ");
