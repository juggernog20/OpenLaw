...
// A new "DOC-029 access" Business User exercises first sign-in, onboarding and link reuse; Jonas and Amara
// use separate contexts for record isolation and shared-record pages.
import { BASE, SEED, mailSince, newContext, sleep, sql, text, waitForLink } from "./lib-r1.mjs";
...
export async function run(log, fx) {
  ...
  const buEmail = `doc029.access.business.${stamp}@helix.example`;
  const buName = `DOC-029 access Business User ${stamp % 100000}`;
  ...
  const check = await bu.page
    .getByText(
      `If ${buEmail} is eligible, a sign-in link is on its way. It expires in 5 minutes and works once.`,
    )
  ...
  const lifetime = sql(
    `select round(extract(epoch from (expires_at - created_at))/60) from verifications where value like '%${buEmail}%' order by created_at desc limit 1`,
  );
  await bu.page.goto(first.link);
  await bu.page.waitForURL(/\/portal/, { timeout: 20000 });
  const onboarding = await bu.page
    .getByText("We need to learn a little about you")
    .first()
    .waitFor({ timeout: 15000 })
    .then(
      () => true,
  ...
  await bu.page.screenshot({
    path: new URL("./r1-business-user-onboarding.png", import.meta.url).pathname,
  });
  ...
  await bu.page.waitForURL((u) => u.pathname === "/portal" || u.pathname === "/portal/", {
    timeout: 20000,
  });
  await sleep(1500);
  ...
  const signOutLogin = await bu.page.waitForURL(/\/portal\/login/, { timeout: 15000 }).then(
    () => true,
    () => false,
  );
  ...
  R(
    "Sign out in the Portal header",
    "Sign out returns to the Portal sign-in page",
    `Returned to /portal/login ${signOutLogin}; opening /portal afterwards -> ${new URL(bu.page.url()).pathname}`,
    signOutLogin && stillOut,
  );

  await bu.page.goto(first.link);
  const expired = await bu.page
  ...
  const back = await bu.page
    .waitForURL((u) => u.pathname === "/portal" || u.pathname === "/portal/", { timeout: 20000 })
    .then(
      () => true,
      () => false,
    );
  const noOnboardingAgain = !(await bu.page
    .getByText("We need to learn a little about you")
    .isVisible()
    .catch(() => false));
  R(
  ...
  {
    const c = await newContext();
    const outsider = `doc029.access.${stamp}@outside-domain.example`;
    const s = await requestPortalLink(c.page, outsider);
    await sleep(6000);
    const n = (await mailSince(outsider, s)).length;
    R(
      "address outside the allowed domains",
      "Neutral Check your email; no link arrives",
      `Check your email shown; messages to ${outsider.replace(String(stamp), "<stamp>")}: ${n}`,
      n === 0,
    );
  ...
  {
    const c = await newContext();
    const pwEmail = `doc029.access.business-pw.${stamp}@helix.example`;
    ...
    const neutral = await c.page
      .getByText("password setup link is on its way. It expires in one hour.")
      .first()
    ...
    fx.created.push(
      `Business User ${pwEmail}, created by Portal password setup (left at onboarding)`,
    );
    await c.context.close();
  }
  ...
  const amaraReq = sql(
    `select number from requests where requester_id='${amaraId}' order by number limit 1`,
  );
  const jonasReq = sql(
    `select number from requests where requester_id='${jonasId}' and status<>'converted' order by number limit 1`,
  );
  const jonasReqTitle = sql(`select title from requests where number=${jonasReq}`);
  const converted = sql(
    `select r.number||'|'||c.number from requests r join contracts c on c.id=r.converted_contract_id join contract_team t on t.contract_id=c.id and t.user_id='${jonasId}' where r.requester_id='${jonasId}' and c.archived_at is null order by r.number limit 1`,
  ).split("|");
  const archivedC = sql(
    `select c.number from contract_team t join contracts c on c.id=t.contract_id where t.user_id='${jonasId}' and c.archived_at is not null order by c.number limit 1`,
  );
  const primaryC = sql(
    `select c.number from contract_team t join contracts c on c.id=t.contract_id where t.user_id='${jonasId}' and c.archived_at is null and c.primary_document_id is not null order by c.number limit 1`,
  );
  const teamMatter = sql(
    `select m.number from matter_team t join matters m on m.id=t.matter_id where t.user_id='${jonasId}' and m.archived_at is null order by m.number limit 1`,
  );
  ...
  R(
    "Portal lists that person's Requests (Jonas)",
    "The Portal opens under Jonas's identity and lists his Requests",
    `Header shows ${SEED.jonas.email} ${header.includes(SEED.jonas.email)}; own Request R-${jonasReq} listed ${ownVisible}`,
    header.includes(SEED.jonas.email) && ownVisible,
  );

  await jp.goto(`${BASE}/portal/requests/${amaraReq}`);
  await sleep(2000);
  const blocked = new URL(jp.url()).pathname;
  const leaked = (await text(jp.locator("main"))).includes(
    "Reference request for a former employee",
  );
  R(
    "another Business User's Request link does not grant access",
    "Amara's Request number returns Jonas to the Portal without showing it",
    `/portal/requests/${amaraReq} -> ${blocked}; Amara's Request title shown ${leaked}`,
    (blocked === "/portal" || blocked === "/portal/") && !leaked,
  );
  ...
  R(
    "Business Users cannot enter the staff Inbox, Contracts, or Matters pages",
    "Staff pages send the Business User to the Portal",
    JSON.stringify(staffPaths),
    Object.values(staffPaths).every((p) => p.startsWith("/portal")),
  );
  ...
  for (
    let i = 0;
    i < 6 && !(await jp.locator(`main a[href$='/portal/contracts/${primaryC}']`).count());
    i++
  ) {
  ...
  const primaryLabel = await jp
    .getByText("Primary Document", { exact: true })
    .first()
    .isVisible()
    .catch(() => false);
  ...
  if (downloadName) {
    const [dl] = await Promise.all([
      jp.waitForEvent("download", { timeout: 20000 }).catch(() => null),
      downloadLink.click(),
    ]);
    downloaded = dl ? dl.suggestedFilename().length > 0 : false;
  }
  const docButton = jp
    .locator("li", { hasText: "Primary Document" })
    .locator("button.text-link")
    .first();
  const docTitle = await text(docButton);
  await docButton.click();
  await sleep(2500);
  const panel = jp.getByRole("complementary", {
    name: new RegExp(`^${docTitle.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}, version \\d+$`),
  });
  ...
  const applets = {};
  for (const n of ["Comments", "History", "Contract team"])
    applets[n] =
      (await jp
        .getByRole("toolbar", { name: "Applets" })
        .getByRole("button", { name: new RegExp(`^${n}( \\(\\d+\\))?$`) })
        .count()) > 0;
  await jp.getByRole("link", { name: "Your Contracts" }).first().click();
  const backToList = await jp.waitForURL(/\/portal\/contracts$/, { timeout: 10000 }).then(
    () => true,
    () => false,
  );
  R(
    ...
    refShown &&
      ...
      reader &&
      downloaded &&
      Object.values(applets).every(Boolean) &&
      backToList,
  );
  ...
  for (
    let i = 0;
    i < 6 && !(await jp.locator(`main a[href$='/portal/matters/${teamMatter}']`).count());
    i++
  ) {
    await jp.getByRole("button", { name: "Show more" }).click();
    await sleep(1500);
  ...
  const mApplets = {};
  for (const n of ["Comments", "History", "Matter team"])
    mApplets[n] =
      (await jp
        .getByRole("toolbar", { name: "Applets" })
        .getByRole("button", { name: new RegExp(`^${n}( \\(\\d+\\))?$`) })
  ...
  R(
    ...
    mText.includes(`M-${teamMatter}`) &&
      mMissing.length === 0 &&
      Object.values(mApplets).every(Boolean),
  );
  ...
  await jp
    .waitForURL(new RegExp(`/portal/contracts/${converted[1]}`), { timeout: 15000 })
    .catch(() => {});
  await sleep(2500);
  const convUrl = new URL(jp.url()).pathname;
  const original = (await text(jp.locator("main"))).includes("Original request");
  R(
    "a converted Request address redirects to its record",
    "R- address opens the new Contract, which shows Original request",
    `/portal/requests/${converted[0]} -> ${convUrl}; Original request section ${original}`,
    convUrl === `/portal/contracts/${converted[1]}` && original,
  );
  ...
  R(
    ...
    `Header shows ${SEED.amara.email} ${aHeader.includes(SEED.amara.email)}; own R-${amaraReq} listed ${aText.includes("Reference request for a former employee")}; Jonas's title on home ${aText.includes(jonasReqTitle.slice(0, 30))}; /portal/requests/${jonasReq} -> ${aBlocked}, title shown ${aLeak}`,
    aHeader.includes(SEED.amara.email) &&
      aText.includes("Reference request for a former employee") &&
      !aText.includes(jonasReqTitle.slice(0, 30)) &&
      aBlocked.replace(/\/$/, "") === "/portal" &&
      !aLeak,
  );
  ...
  // ---- Branches that need an organization-wide policy change: page rendering with a mocked methods answer.
  {
    ...
    const view = async (path, business) => {
      ...
      await c.page.route("**/api/v1/auth/methods", (route) =>
        route.fulfill({ json: methods(business) }),
      );
    ...
    const off = await view("/portal/login", {
      password: true,
      magicLink: false,
      sso: false,
      requireTwoFactor: false,
    });
    const none = await view("/portal/login", {
      password: false,
      magicLink: false,
      sso: false,
      requireTwoFactor: false,
    });
    const exp = await view("/auth/link-expired?portal=1", {
      password: true,
      magicLink: false,
      sso: false,
      requireTwoFactor: false,
    ...
    const offOk =
      !off.buttons.some((b) => /Email me a sign-in link/.test(b)) &&
      off.buttons.some((b) => /Set up or reset your password/.test(b)) &&
      /Password/.test(off.t);
    const noneOk = /Sign-in is unavailable\. Contact your administrator\./.test(none.t);
    const expOk =
      [...exp.links, ...exp.buttons].some((b) => /Back to sign-in/.test(b)) &&
      !/Send link/.test(exp.buttons.join("|"));
    R(
