// DOC-029 round 3 independent walkthrough, run 1, for the "Set up a new OpenLaw instance" guide (first-run).
// Scenario V-C35, role administrator, method browser-walkthrough, app commit 432dba3a.
// The reviewer copied the round 2 script (walkthrough-r2-1.mjs). Only the two-factor step changed: it now expects
// /auth/two-factor/enroll right after Continue, and the Done link back to the wizard Welcome step.
// The reviewer did not write the article.
//
// Run from the repository root, one phase at a time, inside its own network namespace:
//   PHASE=A SCRATCH=... LAB_ADMIN_PASSWORD=... pasta --config-net -T 23311,23411 -- mise exec -- node docs/documentation/batches/DOC-029/first-run/walkthrough-r3-1.mjs
...
if (!PHASE || !PASSWORD || !SCRATCH)
  throw new Error("PHASE, LAB_ADMIN_PASSWORD and SCRATCH are required");
const OUT = path.join(here, "walkthrough-r3-1.json");
const ARTICLE = path.join(root, "docs/user-guides/first-run.md");
const LAB = JSON.parse(
  readFileSync(path.join(root, ".documentation-labs/firstrun-r3/lab.json"), "utf8"),
);

const ADMIN = {
  name: "DOC-029r3 first-run Avery Morgan",
...
const log = existsSync(OUT)
  ...
  : {
      record: "walkthrough-r3-1",
      articleId: "first-run",
      scenario: "V-C35",
      reviewer: "DOC-029 independent walkthrough agent (first-run, round 3)",
      reviewerKind: "agent",
      appCommit: "432dba3a835863091791fbac429a6a66777bdc40",
      runs: [],
    };
const run = {
  phase: PHASE,
...
async function step(action, expected, fn, { continueOnFail = false } = {}) {
  ...
  try {
    ...
    if (currentPage) {
      try {
        const main = (await currentPage.locator("main").first().innerText({ timeout: 3000 }))
          .replace(/\s+/g, " ")
          .slice(0, 400);
        entry.actual += ` Page at failure: ${pathOf(currentPage)}; main text "${main}".`;
        await currentPage.screenshot({
          path: path.join(SCRATCH, `fail-${PHASE}-${run.steps.length}.png`),
        });
      } catch (diagError) {
  ...
  console.log(
    `[${PHASE}] ${entry.result.toUpperCase()} ${action}${entry.result === "fail" ? `: ${entry.actual}` : ""}`,
  );
  save();
...
async function phaseA() {
    ...
    async () => {
      ...
      const res = await other.request.post(`${BASE}/api/v1/auth/setup`, {
        data: {
          ...
          displayName: "DOC-029r3 first-run Second",
  ...
  const bigPng = path.join(SCRATCH, "doc029r3-logo-over-5mb.png");
  const okPng = path.join(SCRATCH, "doc029r3-logo-under-5mb.png");
  const gif = path.join(SCRATCH, "doc029r3-logo.gif");
  ...
  await step(
    ...
    async () => {
      ...
      const [chooserOk] = await Promise.all([
        page.waitForEvent("filechooser"),
        button(page, "Upload").click(),
      ]);
  ...
  const orgName = "DOC-029r3 first-run Harbor Legal";
  await step(
  ...
  await step(
    ...
    async () => {
      ...
      must(
        ...
        `timezone is ${general.general.defaultTimezone}`,
      );
      must(
        typeof general.general.logo === "string" &&
          general.general.logo.startsWith("data:image/png"),
        "logo not saved",
      );
      const decoded = Buffer.byteLength(
        general.general.logo.slice(general.general.logo.indexOf(",") + 1),
        "base64",
      );
  ...
  await step(
    ...
    async () => {
      ...
      must(
        (await nav.getByRole("link", { name: "Email", exact: true }).count()) === 0,
        "an Email settings page exists",
      );
      await expect(p2.getByText("Setup checklist", { exact: true })).toBeVisible();
      const rows = await outstandingRows(p2);
      must(
        (await p2.getByText("Return to setup", { exact: true }).count()) === 0,
        "Return to setup shown on a direct Settings visit",
      );
      await p2.screenshot({ path: path.join(here, "r3-1-a-setup-checklist.png") });
      await p2.close();
      ...
      must(
        review && review.href === null && review.markAsReviewed,
  ...
  await step(
    ...
    async () => {
      ...
      await page.getByLabel("Organization name").fill("DOC-029r3 first-run Unsaved Name");
      await page.reload();
      const where = await expectStep(page, "Your organization", 2);
  ...
  await step(
    ...
    async () => {
      await page.getByLabel("Provider ID", { exact: true }).fill("doc029r3-harbor");
      ...
      await page.getByLabel("Client ID", { exact: true }).fill("doc029r3-first-run-client");
      await page.getByLabel("Client secret", { exact: true }).fill(randomBytes(12).toString("hex"));
      await button(page, "Register provider").click();
      await expect(
        page.getByText("Identity provider doc029r3-harbor is registered."),
      ).toBeVisible();
      const callback = (
      ...
      return `The step showed "Identity provider doc029r3-harbor is registered." and "${callback}". The Single sign-on (SSO) switch became enabled. The issuer was a local OpenID Connect stand-in.`;
    },
  );
  ...
  await step(
    ...
    async () => {
      ...
      // Build 432dba3a navigates straight to enrollment, so wait for that address, but do not fail before observing.
      await page.waitForURL(/\/auth\/two-factor\/enroll$/, { timeout: 15000 }).catch(() => {});
      await page.waitForLoadState("networkidle");
      const obs = [];
      const afterContinue = pathOf(page);
      const onEnroll = /\/auth\/two-factor\/enroll$/.test(new URL(page.url()).pathname);
      obs.push(
        `after Continue the page was ${afterContinue}${onEnroll ? "" : ` showing "${await stepHeading(page).catch(() => "no step")}"`}`,
      );
      const methods = await getJson(context, "/api/v1/auth/methods");
      const me = await getJson(context, "/api/v1/me");
      if (!onEnroll)
        await page.screenshot({ path: path.join(here, "r3-1-a-two-factor-after-continue.png") });
      obs.push(
        `saved legal policy ${JSON.stringify(methods.policy.legal)}; /me twoFactorSetupRequired ${me.user.twoFactorSetupRequired}`,
      );
      ...
      must(
        methods.policy.legal.requireTwoFactor === true &&
          methods.policy.legal.magicLink === true &&
          methods.policy.legal.password === true,
        `legal policy ${JSON.stringify(methods.policy.legal)}`,
      );
      must(
        me.user.twoFactorSetupRequired === true,
        "/me did not report two-factor setup as required",
      );
      // Leaving enrollment is not possible: another app address sends the Administrator back to it.
      ...
      return `Continue opened two-factor setup directly, with "Your organization requires two-factor authentication." and Turn on two-factor. Observed: ${obs.join("; ")}.`;
    },
    { continueOnFail: true },
  );
  ...
  await step(
    ...
    async () => {
      ...
      const link = page.getByRole("link", { name: "Done", exact: true });
      ...
      await expectStep(page, "Your organization", 2);
      must(
        (await page.getByLabel("Organization name").inputValue()) === orgName,
  ...
  await step(
    ...
    async () => {
      ...
      await page.getByLabel("Name", { exact: true }).fill("DOC-029r3 first-run Rowan Lee");
  ...
  await step(
    ...
    async () => {
      await page.getByLabel("Name", { exact: true }).fill("DOC-029r3 first-run Sam Ortiz");
  ...
  await step(
    ...
    async () => {
      await page.getByLabel("Name", { exact: true }).fill("DOC-029r3 first-run Unsent");
  ...
  await step(
    ...
    async () => {
      ...
      await page.screenshot({ path: path.join(here, "r3-1-a-review-step.png") });
  ...
  await step(
    ...
    async () => {
      ...
      await page.screenshot({ path: path.join(here, "r3-1-a-return-to-setup.png") });
      await link.click();
      const where = await expectStep(page, "Review", 9);
  ...
  await step(
    ...
    async () => {
      ...
      await expect(page.getByText("DOC-029r3 first-run Rowan Lee")).toBeVisible();
      await expect(page.getByText("DOC-029r3 first-run Sam Ortiz")).toBeVisible();
      must(
        (await page.getByText("DOC-029r3 first-run Unsent").count()) === 0,
        "unsent invite listed",
      );
      for (const p of [
...
async function phaseB0() {
  ...
  currentPage = page;
  await step(
    "Create the first Administrator on a fresh instance whose environment sets SMTP_URL but not SMTP_FROM",
    "Setup signs the Administrator in and opens Welcome to OpenLaw.",
    async () => {
  ...
  await step(
    ...
    async () => {
      const out = [];
      for (const p of ["/", "/settings/general", "/contracts", "/matters"])
        out.push(await landsOnWizardWelcome(page, p));
      return out.join("; ");
    },
  );
  await step(
    "On Welcome select Skip optional steps while email is not configured",
    "Outbound email opens instead of ending onboarding; it says SMTP_FROM is not set; no Set up later; Continue unavailable.",
    async () => {
      await expectStep(page, "Welcome to OpenLaw", 1);
      ...
      must(
        (await button(page, "Save relay").count()) === 0,
        "Save relay shown for an environment source",
      );
      await expect(button(page, "Continue")).toBeDisabled();
      const onboarding = await getJson(context, "/api/v1/onboarding");
      must(!onboarding.completed, "onboarding completed");
      await page.screenshot({ path: path.join(here, "r3-1-b0-smtp-from-missing.png") });
  ...
  await step(
    ...
    "Setup cannot finish without working outbound email: each action opens Outbound email and onboarding stays open.",
    async () => {
      const out = [];
...
async function phaseB1() {
  ...
  currentPage = page;
  await step(
    "Sign in again after the operator removes email from the environment",
    "The unfinished wizard opens.",
    async () => {
      await signIn(page);
      await page.waitForLoadState("networkidle");
  ...
  await step(
    "While email is unset, open Home and other app pages",
    "Every app page opens the wizard at its Welcome step.",
    async () => {
      const out = [];
      for (const p of ["/", "/settings/users", "/requests"])
        out.push(await landsOnWizardWelcome(page, p));
      return out.join("; ");
    },
  );
  await step(
    "Select Skip optional steps with email unset",
    "Outbound email opens with SMTP server, Port, Connection security, Authentication, Sender email, Sender name (optional) and Save relay; no Set up later; Continue unavailable.",
    async () => {
      await button(page, "Skip optional steps").click();
      const where = await expectStep(page, "Outbound email", 5);
      for (const label of [
  ...
  await step(
    "Enter SMTP server, Port, Connection security, Authentication, Sender name and Sender email, select Save relay, then Send test email",
    "The relay saves, Continue becomes available, and a test email reaches the Administrator.",
    async () => {
      await page.getByLabel("SMTP server", { exact: true }).fill("mailpit");
      await page.getByLabel("Connection security", { exact: true }).selectOption({ label: "None" });
      await page.getByLabel("Port", { exact: true }).fill("1025");
      await page.getByLabel("Authentication", { exact: true }).selectOption({ label: "None" });
      await page
        .getByLabel("Sender name (optional)", { exact: true })
        .fill("DOC-029r3 first-run Harbor Legal");
      await page.getByLabel("Sender email", { exact: true }).fill("legal@harbor.example");
      await button(page, "Save relay").click();
      await expect(
  ...
  await step(
    "Move through the remaining steps with Set up later, and select Set up later on Review",
    "Set up later on Review ends onboarding without recording the review.",
    async () => {
      await button(page, "Continue").click();
  ...
  await step(
    ...
    "The checklist lists the unfinished steps with Settings links; Mark as reviewed removes the Review row.",
    async () => {
      await page.goto(`${BASE}/settings/general`);
      const rows = await outstandingRows(page);
      const labels = rows.map((r) => r.label);
      ...
      await page
        .getByRole("list", { name: "Outstanding setup steps" })
        .getByRole("link", { name: "Business-user portal" })
        .click();
  ...
  await step(
    "Try to reopen the wizard after Set up later on Review",
    "The wizard cannot be reopened.",
    async () => {
      await page.goto(`${BASE}/welcome`);
      await page.waitForLoadState("networkidle");
      must(new URL(page.url()).pathname === "/", `ended at ${page.url()}`);
...
async function phaseB2() {
  ...
  currentPage = page;
  await step(
    "After completion, with SMTP_URL set and SMTP_FROM unset in the environment, open Settings -> Organization -> General",
    "The Setup checklist shows Email as a row without a Settings link.",
    async () => {
      await signIn(page);
      await page.waitForLoadState("networkidle");
      ...
      for (const r of rows.filter((x) => !["Email", "Review seeded types"].includes(x.label)))
        must(r.href?.startsWith("/settings/"), `${r.label} not linked`);
      await page.screenshot({ path: path.join(here, "r3-1-b2-checklist-email-row.png") });
...
async function phaseC() {
  ...
  await step(
    ...
    "Onboarding ends and the app opens without recording the review.",
    async () => {
      await button(page, "Skip optional steps").click();
      await expect(page).toHaveURL(`${BASE}/`);
      await page.waitForLoadState("networkidle");
