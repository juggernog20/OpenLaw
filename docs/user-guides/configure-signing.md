...
5. Follow [electronic signing](electronic-signing.md) with fictional paper and controlled test Signers. Complete the Envelope in DocuSign. In Polling mode, allow the next reconciliation check to run. Then check the **Signed** Envelope, **Executed copy**, and that the returned Document Version is marked as the executed copy on the original Document chain. Check the Contract's Status too. A Contract still in the Signature Stage moves to the first configured live Active Status. A Contract in any other Stage keeps its Status.
...
Turn off **Send for signature from records** to stop offering new sends while retaining the credentials. Reconciliation, accepted signed Webhook deliveries and executed-copy filing continue with the saved credentials. Existing provider sessions are not closed. New OpenLaw launches are refused. This does not void Envelopes already held by DocuSign: Signers may still act there. To withdraw a round, [void its Envelope](electronic-signing.md) while the connector is available.
...
Select **Remove connector** and confirm only when its credentials should be deleted from OpenLaw. Removal is refused while any preparation, uncertain creation or sent Envelope is live, including an externally restored round. A Signed Envelope whose executed copy is pending or failed also retains the connector. Resolve drafts in DocuSign, finish or void sent rounds, and file outstanding executed copies first. Reconnecting after removal requires the RSA private key again, plus the Connect secret for Webhook. Removing the connector does not revoke the integration or its credentials in DocuSign.
...
Finish or resolve outstanding rounds before changing the integration, sending user, or environment. OpenLaw checks changed credentials against the recorded account and environment and refuses changes that would strand outstanding work. Historical rounds without recorded provider identity retain their original integration key, User ID and environment while work remains; secret rotation is still available. Check the returned account name and complete another controlled round after the change.
...
| Save is refused                                   | Supply the RSA private key and required account identifiers. Webhook also requires a Connect secret. A stored secret can be kept by leaving its field blank. If the pane says only that request fields are invalid, check that **Public callback URL** is blank or an HTTPS address without credentials, query parameters or a fragment.                                                                                                                                                                |
...
| Provider status changed but OpenLaw did not       | Check that the worker is running and the saved connector credentials are usable. The connector switch does not stop status accounting. For Polling, allow the next scheduled check and check outbound access. For Webhook, check the public callback address and that DocuSign can reach it over HTTPS. Check the Connect subscription's user coverage, JSON envelope data, and HMAC secret. After correction, use the provider's delivery recovery controls and allow OpenLaw's reconciliation to run. |
...
The normal deployment still uses immediate **Send envelope**. **Continue to DocuSign** is enabled only in a disposable acceptance lab until native editor restrictions pass live. A successful **Test connection** is not that proof. Follow the [electronic signing guide](electronic-signing.md) for the two interfaces and the [operator acceptance procedure](https://github.com/juggernog20/OpenLaw/blob/feat/1170-docusign-preparation/docs/DEPLOYMENT.md#docusign-preparation-acceptance).
