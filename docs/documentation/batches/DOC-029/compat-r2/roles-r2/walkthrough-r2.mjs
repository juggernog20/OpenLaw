...
await step(
  ...
  async () => {
    ...
    check(
      ct.includes("DOC-029r2 roles open contract") &&
        !ct.includes("DOC-029r2 roles confidential contract") &&
        !mt.includes("DOC-029r2 roles confidential matter"),
      "API lists it",
    );
...
await step(
  ...
  async () => {
    ...
    const fullRow = (
      await panel
        .getByRole("listitem")
        .filter({ hasText: "DOC-029r2 roles Full thread note" })
        .first()
        .innerText()
    ).replace(/\s+/g, " ");
    const chipText = text.replace(/DOC-029r2 roles [A-Za-z ]*note/g, "");
    check(
      text.includes("DOC-029r2 roles Full thread note") &&
        !text.includes("DOC-029r2 roles Legal only note") &&
        !text.includes("DOC-029r2 roles Working team note") &&
        audience === 0,
      text.slice(0, 300),
    );
    check(
      /Contract Team/.test(fullRow.replace("DOC-029r2 roles Full thread note", "")) &&
        !/Legal Only|Internal team/.test(chipText),
      `row "${fullRow}"`,
    );
...
await step(
  ...
  "legal_team_member",
  "D5b.tier-labels",
  "In the Comments applet a Full Thread comment shows Contract Team on a Contract and Matter Team on a Matter; Legal Only and older Internal team comments show those labels; Legal chooses Contract Team or Matter Team above New comment, not Legal Only.",
  async () => {
    await go(N, `/contracts/${CO}`);
    ...
    const row = async (body) =>
      (await lp.getByRole("listitem").filter({ hasText: body }).first().innerText())
        .replace(/\s+/g, " ")
        .replace(body, "");
    ...
    check(
      /Legal Only/.test(audience) &&
        ...
        /Legal Only/.test(maudience) &&
        /Matter Team/.test(maudience),
      `audience ${audience} / ${maudience}`,
    );
    check(gBox.y < tBox.y, `Audience y ${gBox.y}, New comment y ${tBox.y}`);
    check(
      /Contract Team/.test(fullRow) && /Legal Only/.test(legalRow) && /Internal team/.test(workRow),
      JSON.stringify({ fullRow, legalRow, workRow }),
    );
...
async function legalComment(page, url, audienceLabel, body) {
  ...
  return (await panel.getByRole("listitem").filter({ hasText: body }).first().innerText())
    .replace(/\s+/g, " ")
    .replace(body, "");
}
...
const activityCount = async (page, entityType, entityId) => {
  ...
  return {
    ...
    firstActor: entries[0]?.actor?.displayName ?? null,
    firstAction: entries[0]?.action ?? null,
...
await step(
  ...
  "The Portal History applet shows Contract Team comment activity and excludes Legal Only and Internal team activity. The Portal excludes legal Fields, Tasks, Key dates, Approvals and signing controls.",
  async () => {
    const stamp = Date.now().toString().slice(-6);
    const h0 = await portalHistory(J, `/portal/contracts/${CO}`);
    const a0 = await activityCount(J, "contract", fx.openContract.id);
    const legalChip = await legalComment(
      D,
      `/contracts/${CO}`,
      "Legal Only",
      `DOC-029r2 roles r2 Legal Only probe ${stamp}`,
    );
    const older = await api(D, "POST", "/comments", {
      entityType: "contract",
      entityId: fx.openContract.id,
      body: `DOC-029r2 roles r2 Internal team probe ${stamp}`,
      visibility: "working_team",
    });
    await sleep(1000);
    const h1x = await portalHistory(J, `/portal/contracts/${CO}`);
    const a1 = await activityCount(J, "contract", fx.openContract.id);
    const teamChip = await legalComment(
      D,
      `/contracts/${CO}`,
      "Contract Team",
      `DOC-029r2 roles r2 Contract Team probe ${stamp}`,
    );
    await sleep(1000);
    ...
    check(
      /Legal Only/.test(legalChip) && /Contract Team/.test(teamChip),
      `${legalChip} / ${teamChip}`,
    );
    check(
      a1.first === a0.first && h1x.top === h0.top && !/^Daniel Okafor/.test(h1x.top),
      `after Legal Only and Internal team: newest entry ${a0.first}->${a1.first}, top row "${h0.top}" -> "${h1x.top}"`,
    );
    check(
      a2.first !== a1.first &&
        a2.firstActor === "Daniel Okafor" &&
        /^comment\./.test(a2.firstAction) &&
        /^Daniel Okafor commented/.test(h2.top),
      `after Contract Team: newest ${a2.firstActor} ${a2.firstAction}, top row "${h2.top}"`,
    );
...
await step(
  RA,
  "business_user",
  "D6b.history-no-record-changes",
  "Record changes do not appear in Portal History; the record details and Fields show the current values.",
  async () => {
    const h0 = await portalHistory(J, `/portal/contracts/${CO}`);
    const a0 = await activityCount(J, "contract", fx.openContract.id);
    await go(N, `/contracts/${CO}`);
    ...
    const fields = (await J.getByRole("region", { name: "Fields" }).innerText()).replace(
      /\s+/g,
      " ",
    );
    ...
    await settle(N);
    check(
      /changed Region/.test(legalHist),
      `Legal History did not record the change: ${legalHist.slice(0, 160)}`,
    );
    check(
      /APAC/.test(overview) && fields.includes(descText),
      `Overview ${overview.slice(0, 160)} / Fields ${fields.slice(0, 160)}`,
    );
    check(
      a1.first === a0.first &&
        h1b.top === h0.top &&
        !/changed/.test(h1b.text) &&
        !a1.actions.includes("contract.updated"),
      `Portal History top "${h0.top}" -> "${h1b.top}": ${h1b.text.slice(0, 160)}; actions ${a1.actions}`,
    );
...
await step(
  ...
  async () => {
    ...
    const comm = await api(
      J,
      "GET",
      `/comments?entityType=contract&entityId=${fx.openContract.id}`,
    );
    await go(N, `/matters/${MO}`);
    await pickPerson(N, "Business Owner", "Jonas Weber");
    ...
    check(
      jRefused && docs.status >= 400 && comm.status >= 400,
      `refused ${jRefused} docs ${docs.status} comments ${comm.status}`,
    );
    check(
...
await step(
  ...
  "business_user",
  "D9.related-no-reach",
  "Legal chooses Matter Team for a message to the business; a Full Thread comment shows Matter Team on a Matter and appears in Portal History; Legal Only stays hidden. A link or parent relation does not grant a Business User access to the related record.",
  async () => {
    await go(N, `/matters/${MO}`);
    ...
    const teamChip = await legalComment(
      ...
      `/matters/${MO}`,
      "Matter Team",
      `DOC-029r2 roles r2 Matter Team note ${stamp}`,
    );
    const legalChip = await legalComment(
      N,
      `/matters/${MO}`,
      "Legal Only",
      `DOC-029r2 roles r2 Matter Legal Only note ${stamp}`,
    );
    ...
    const jRow = (
      await jc
        .getByRole("listitem")
        .filter({ hasText: `DOC-029r2 roles r2 Matter Team note ${stamp}` })
        .first()
        .innerText()
    ).replace(/\s+/g, " ");
    ...
    check(
      /Matter Team/.test(teamChip) &&
        /Legal Only/.test(legalChip) &&
        /Matter Team/.test(jRow) &&
        !jText.includes(`Matter Legal Only note ${stamp}`) &&
        !/Legal Only/.test(jText) &&
        /^Nadia Haddad commented/.test(mh.top),
      JSON.stringify(results.d9labels),
    );
    ...
    check(
      !m.refused &&
        !/DOC-029r2 roles confidential (contract|matter)/.test(main) &&
  ...
  async () => {
    ...
    check(
      !list.includes(`C-${CJ}`) &&
        ...
        /archived/.test(main) &&
        main.includes("DOC-029r2 roles fictional ask from Jonas") &&
        docs === 0 &&
        comments === 0,
      main.slice(0, 300),
    );
...
await step(
  ...
  "legal_team_member",
  "Z0.restore-paging",
  "Fixture: restore the 25 paging Matters archived at the end of round 1.",
  async () => {
    const codes = [];
    for (const m of fx.pagingMatters)
      codes.push((await api(N, "POST", `/matters/${m.number}/restore`, {})).status);
    check(
      codes.every((c) => c < 300 || c === 409),
      codes.join(","),
    );
...
await step(
  ...
  async () => {
    ...
    const rq = await api(R, "POST", "/requests", {
      ...
      departmentId: sales,
      title: `DOC-029r2 roles Ravi request ${stamp}`,
      description: "DOC-029r2 roles fictional ask from Ravi.",
      urgency: "medium",
    });
    ...
    const cv = await api(N, "POST", `/requests/${rq.json.request.number}/convert`, {
      title: `DOC-029r2 roles Ravi converted contract ${stamp}`,
      contractTypeId: nda,
    });
  ...
  async () => {
    ...
    check(
      /not found/.test(before.title) &&
        title === "DOC-029r2 roles Ravi matter" &&
        main.includes(`M-${MR}`) &&
...
await step(
  ...
  async () => {
    const cp = await api(N, "POST", `/contracts/${CR}/counterparties`, {
      name: "DOC-029r2 Roles Fictional Supplier Ltd",
    });
  ...
  async () => {
    ...
    check(
      /Department/.test(mMain) &&
        ...
        fieldsAfter.includes("DOC-029r2 roles description updated by Legal.") &&
        original.includes("DOC-029r2 roles fictional ask from Ravi.") &&
        editable.length === 0,
...
await step(
  ...
  async () => {
    ...
    await dialog
      ...
      .fill("DOC-029r2 roles business note");
    ...
    check(
      posts === 3 &&
        ...
        uploads.every((d) => d.versions[0].note === "DOC-029r2 roles business note"),
...
await step(
  ...
  async () => {
    ...
    const dt = await R.evaluateHandle(() => {
      ...
      d.items.add(
        new File(["DOC-029r2 roles dropped paper"], "doc029-roles-dropped.txt", {
          type: "text/plain",
        }),
      );
      return d;
...
await step(
  ...
  async () => {
    ...
    await api(N, "POST", "/comments", {
      ...
      body: "DOC-029r2 roles Legal reply for Ravi",
    ...
    check(
      pickers === 0 &&
        switched.startsWith("DOC-029r2 roles draft kept") &&
        kept.startsWith("DOC-029r2 roles draft kept") &&
        keptFile &&
...
await step(
  ...
  "Your reply shows Contract Team on a Contract or Matter Team on a Matter (the Full Thread audience); the text below the composer names it; Legal Only and older Internal team messages stay outside this view.",
  async () => {
    const stamp = Date.now().toString().slice(-6);
    const hiddenL = await api(N, "POST", "/comments", {
      entityType: "matter",
      entityId: fx.raviMatter.id,
      body: `DOC-029r2 roles r2 hidden Legal Only ${stamp}`,
      visibility: "legal_only",
    });
    const hiddenW = await api(N, "POST", "/comments", {
      entityType: "matter",
      entityId: fx.raviMatter.id,
      body: `DOC-029r2 roles r2 hidden Internal team ${stamp}`,
      visibility: "working_team",
    });
    const out = {};
    for (const [label, url, module] of [
      ...
      const text = (await panel.innerText()).replace(/\s+/g, " ");
      if (module === "Matter")
        await R.screenshot({ path: path.join(here, "r2-portal-reply-tier-label.png") });
      out[module] = {
        row,
        hint,
        hidden:
          text.includes(`hidden Legal Only ${stamp}`) ||
          text.includes(`hidden Internal team ${stamp}`),
        otherLabels: /Legal Only|Internal team/.test(text),
      };
      check(
        new RegExp(label).test(row) && hint === `Visible to Legal and all ${module} team members.`,
        `${module}: row "${row}", hint "${hint}"`,
      );
    }
    check(
      hiddenL.status < 300 &&
        hiddenW.status < 300 &&
        !out.Matter.hidden &&
        !out.Matter.otherLabels &&
        !out.Contract.otherLabels,
      JSON.stringify(out),
    );
...
await step(
  ...
  "Closing a Matter or Ending a Contract does not remove Portal work; archiving removes the record from the Portal; removing Ravi's membership stops access.",
  async () => {
    ...
    const moved = await api(N, "PATCH", `/matters/${MR}`, {
      statusId: closed.id,
      closingNote: "DOC-029r2 roles fictional closing note.",
    });
    await go(R, "/portal/matters");
    const box = R.getByRole("searchbox", { name: "Search Matters" });
    await box.fill("DOC-029r2 roles Ravi");
    ...
    const ended = await api(N, "PATCH", `/contracts/${CR}`, {
    ...
    const commentEnded = await (
      ...
      .isEditable()
      .catch(() => false);
    const back = await api(N, "PATCH", `/contracts/${CR}`, {
      statusId: cRow.statusId ?? cRow.status?.id,
      overrideSoftGate: true,
    });
    results.g14ended = {
      ended: ended.status,
      endedStatus: endedStatus?.displayName,
      back: back.status,
      openEnded,
      uploadEnded,
      commentEnded,
    ...
    check(
      ended.status < 300 && !openEnded.refused && uploadEnded && commentEnded,
      JSON.stringify(results.g14ended),
    );
    check(
      moved.status < 300 &&
        ...
        !openClosed.refused &&
        uploadClosed &&
        !listArchived.includes("DOC-029r2 roles Ravi matter") &&
