...
const mr = await matterFor(n.page, {
  ...
  departmentId: sales,
  region: "EMEA",
  description: "DOC-029r2 roles fictional Matter description.",
});
...
for (let i = 1; i <= 25; i++) {
  const m = await matterFor(n.page, {
    title: `DOC-029r2 roles paging matter ${String(i).padStart(2, "0")}`,
    matterTypeId: commercial,
    managerId: fx.ids.nadia,
  });
...
const dm = must(
  await api(n.page, "POST", "/matters", {
    title: "DOC-029r2 roles document paging matter",
    matterTypeId: commercial,
    managerId: fx.ids.nadia,
  }),
  "DM",
...
for (let i = 1; i <= 51; i++)
  await upload(
    ...
    `DOC-029r2 roles fictional paging paper ${i}.\n`,
  );
