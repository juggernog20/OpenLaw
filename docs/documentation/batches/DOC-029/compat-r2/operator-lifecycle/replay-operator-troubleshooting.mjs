...
const LC =
  "/tmp/claude-1000/-home-blairwentworth-projects-OpenLaw/7de884dd-eedd-4464-96ea-cc0709f7fe19/scratchpad/lc-diag";
...
const log = readJson(LOG, {
  ...
  baseline: BASELINE,
  script:
    "docs/documentation/batches/DOC-029/compat-r2/operator-lifecycle/replay-operator-troubleshooting.mjs",
...
phases["populate"] = async () => {
  ...
  const people = {
    admin: {
      email: "morgan.diagnose.r2@lifecycle.example",
      displayName: "DOC-029r2 operator-lifecycle Morgan Diagnose",
    },
    reader: {
      email: "rowan.diagnose.r2@lifecycle.example",
      displayName: "DOC-029r2 operator-lifecycle Rowan Diagnose",
  ...
  await step(
    ...
    async () => {
      ...
      for (const title of [
        "DOC-029r2 operator-lifecycle supply agreement",
        "DOC-029r2 operator-lifecycle services agreement",
      ]) {
      ...
      const d1 = (
        await api.upload(
          ...
          Buffer.from("DOC-029r2 operator-lifecycle draft, original.\n"),
          "text/plain",
        )
      ).document;
      const d2 = (
        await api.upload(
          ...
          pdf([
            "DOC-029r2 operator-lifecycle terms, original.",
            "Fictional terms for diagnostics.",
          ]),
          "application/pdf",
        )
      ).document;
  ...
  await step(
    ...
    () => {
      if (sh(`docker inspect ${OCCUPIER_NAME}`, root).code !== 0)
        must(
          `docker run -d --name ${OCCUPIER_NAME} --label openlaw-docs-owner=${PREFIX} -p 127.0.0.1:${OCCUPIED_PORT}:80 caddy:2-alpine caddy respond --listen :80 "DOC-029r2 operator-lifecycle unrelated listener"`,
          root,
        );
  ...
  await step(
    ...
    async () => {
      ...
      form.append(
        "file",
        new File(
          [Buffer.from("DOC-029r2 operator-lifecycle missing bucket\n")],
  ...
  await step(
    ...
    async () => {
      ...
      await api.post("/api/v1/auth/invites", {
        email,
        displayName: "DOC-029r2 operator-lifecycle Nadia Proxy",
        role: "legal_team_member",
      });
      const message = await waitMail(
        (m) => toAddress(m, email) && Date.parse(m.Created) >= since - 2000,
