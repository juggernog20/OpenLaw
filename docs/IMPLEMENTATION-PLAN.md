...
through the signed-in API. M36 added the share register and cap table. **M43 is shipped to dev in PR [#1142](https://github.com/juggernog20/OpenLaw/pull/1142), and every email now sits on one HTML layout.** M41 and M42 are next, with M34 Release after them.
...
- [x] **M38 — Device notifications**
      ...
      _Built:_ [#962](https://github.com/juggernog20/OpenLaw/issues/962), tasks #963–#967. The journey is
      [`59-m38-device-notifications.spec.ts`](../e2e/tests/59-m38-device-notifications.spec.ts).
      It proves staff and Portal enrolment, preferences and revocation against Compose images with
      browser push APIs stubbed. Delivery and notification clicks are covered by handler and service
      worker tests; the browser journey does not prove OS delivery.
  - [x] Schema, per-group Push defaults and Notifier delivery debt; subscriptions bound to sessions
  - [x] VAPID keys, the `notification.push` queue and handler, retries and the lost-work re-ask
  - [x] The service worker without offline caching, manifest, third switch column and staff Devices card
