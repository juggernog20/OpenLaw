...
async function versions(role, person) {
  ...
  const contract = await createContract(
    page,
    role,
    `DOC-029 documents ${label} versions Contract ${stamp}`,
  );
  await addTeam(page, contract.number, PEOPLE.jonas.email);
  const matter = await createMatter(
    page,
    role,
    `DOC-029 documents ${label} versions Matter ${stamp}`,
  );
  ...
  await step(
    ...
    async () => {
      ...
      const ent = await api(page, "POST", "/entities", {
        legalName: `DOC-029 documents ${label} versions Entity ${stamp} Ltd`,
  ...
  await step(
    ...
    async () => {
      const frozen = await createContract(
        page,
        role,
        `DOC-029 documents ${label} archived owner ${stamp}`,
      );
  ...
  const legalOnly = await api(page, "POST", "/comments", undefined, {
    ...
    body: `DOC-029 ${label} Legal only attachment`,
  ...
  const teamNote = await api(page, "POST", "/comments", undefined, {
    entityType: "contract",
    entityId: contractRow.id,
    body: `DOC-029 ${label} team attachment`,
  ...
  await step(
    ...
    async () => {
      ...
      const c = await api(page, "POST", "/comments", undefined, {
        entityType: "matter",
        entityId: m.id,
        body: `DOC-029 ${label} matter attachment`,
      ...
      expect(
        kinds === 0 && docs.some((d) => d.title === `DOC-029 ${label} matter filed ${stamp}`),
        `kinds ${kinds}`,
      );
      expect(
...
async function folders(role, person) {
  ...
  const contract = await createContract(
    page,
    role,
    `DOC-029 documents ${label} folders Contract ${stamp}`,
  );
  const recUrl = `/contracts/${contract.number}`;
  ...
  await step(
    ...
    async () => {
      const matter = await createMatter(
        page,
        role,
        `DOC-029 documents ${label} folders Matter ${stamp}`,
      );
      await openDocumentsTab(page, `/matters/${matter.number}/documents`);
      await docsSection(page).getByRole("button", { name: "Upload", exact: true }).click();
      await chooseFiles(
...
async function previews(role, person) {
  ...
  const contract = await createContract(
    page,
    role,
    `DOC-029 documents ${label} previews Contract ${stamp}`,
  );
  const recUrl = `/contracts/${contract.number}`;
...
async function portal() {
  ...
  const contract = await createContract(
    staff,
    "legal_team_member",
    `DOC-029 documents Portal Contract ${stamp}`,
  );
  const recUrl = `/contracts/${contract.number}`;
  ...
  for (const [name, as] of [
    ...
    const thread = await api(staff, "POST", "/comments", undefined, {
      entityType: "contract",
      entityId: contractId,
      body: `DOC-029 Portal thread file ${as}`,
      visibility: "full_thread",
      file: part(name, as),
    });
    expect(
  ...
  const hidden = await createContract(
    staff,
    "legal_team_member",
    `DOC-029 documents Portal not on team ${stamp}`,
  );
  ...
  const item = await api(staff, "POST", "/knowledge", {
    title: `DOC-029 documents Portal Knowledge ${stamp}`,
  ...
  const req = await api(page, "POST", "/requests", {
    ...
    title: `DOC-029 documents Portal request ${stamp}`,
  ...
  await step(
    ...
    async () => {
      ...
      const attempts = {
        ...
        folder: (await api(page, "POST", `${recUrl}/folders`, { name: "DOC-029 portal folder" }))
          .status,
  ...
  await step(
    ...
    async () => {
      ...
      const staffFolder = await api(staff, "POST", `${recUrl}/folders`, {
        name: `DOC-029 portal target ${stamp}`,
      });
      const targetId = (await folderList(staff, recUrl)).find(
        (f) => f.name === `DOC-029 portal target ${stamp}`,
      )?.id;
...
async function repository(role, person) {
  ...
  const contract = await createContract(
    page,
    role,
    `DOC-029 documents ${label} repository Contract ${stamp}`,
  );
  const recUrl = `/contracts/${contract.number}`;
  ...
  const matter = await createMatter(
    page,
    role,
    `DOC-029 documents ${label} repository Matter ${stamp}`,
  );
  ...
  const types = (await api(page, "GET", "/entities/types")).body.entityTypes;
  const ent = await api(page, "POST", "/entities", {
    legalName: `DOC-029 documents ${label} repository Entity ${stamp} Ltd`,
    entityTypeId: types[0].id,
  });
  expect(ent.status === 201, `fixture entity ${ent.status}`);
  record(role, "entity", ent.body.entity.legalName, ent.body.entity.id);
  ...
  const k = await api(page, "POST", "/knowledge", {
    title: `DOC-029 documents ${label} repository Knowledge ${stamp}`,
    knowledgeTypeId: kType,
  });
  ...
  const ad = await api(page, "POST", "/auto-docs", {
    name: `DOC-029 documents ${label} repository Auto-Doc ${stamp}`,
  });
  expect(
    ad.status === 201,
...
async function archive(role, person) {
  ...
  const contract = await createContract(
    page,
    role,
    `DOC-029 documents ${label} archive Contract ${stamp}`,
  );
  const recUrl = `/contracts/${contract.number}`;
  const tag = `${label.toLowerCase()}-${stamp}`;
  ...
  await step(
    ...
    async () => {
      const kType = (await api(page, "GET", "/knowledge?limit=1")).body.knowledgeItems?.[0]
        ?.knowledgeTypeId;
      const k = await api(page, "POST", "/knowledge", {
        title: `DOC-029 documents ${label} archive Knowledge ${stamp}`,
        knowledgeTypeId: kType,
      });
      expect(k.status === 201, `fixture knowledge ${k.status}`);
