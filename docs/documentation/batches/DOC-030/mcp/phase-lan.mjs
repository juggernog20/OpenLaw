...
const { log, save, step } = createLog("lan", {
  ...
  proxy:
    "loopback-only forward proxy in lib.mjs startLanProxy; the lab port stays bound to 127.0.0.1",
});
...
async function advancedEntry(page, label) {
  ...
  const link = () =>
    label === "MCP"
      ? nav.getByRole("link", { name: "MCP", exact: true }).last()
  ...
  await until(
    async () => (label === "MCP" ? (await count()) > 1 : await link().isVisible()),
    `${label} not in the rail`,
  );
  await link().click();
}
...
await step(
  { ...CMo, page: "/settings/mcp-limits" },
  "Settings → Organization → Advanced → MCP before any pin: Calls per hour per credential defaults to 600",
  "Field shows 600 as Default; OAuth grant lifetime (days) 90",
  async () => {
...
await step(
  { ...CMo, method: "browser-walkthrough", page: "container + /settings/mcp-limits" },
  "Operator sets MCP_RATE_LIMIT_PER_HOUR=3 in the deployment and recreates API and worker; the field shows Read only",
  "Deployment configuration · Read only; the input cannot be changed",
  async () => {
    ...
    expectThat(
      value === "3" &&
        readOnly !== null &&
        /Read only/.test(text) &&
        /Deployment configuration/.test(text),
      text.slice(0, 500),
    );
...
await step(
  { ...CMo, page: "SDK Client" },
  "A refused Client call names the limit and reset time",
  "The fourth call in the hour is refused with the limit and reset time",
  async () => {
    ...
    for (let i = 0; i < 5; i++)
      results.push(
        await callTool(`${LAB.base}/mcp`, { "x-api-key": limitKey }, "openlaw_whoami", {}),
      );
    const texts = results.map(
      (r) =>
        `${r.status}:${toolText(r.body?.result ?? r.body).slice(0, 160) || JSON.stringify(r.body).slice(0, 160)}`,
    );
...
await step(
  { ...CMo, page: "container" },
  "Operator removes the BASE_URL pin and the rate pin, and recreates API and worker",
  "The app answers; Instance address falls back to its default",
  async () => {
    mcplanCompose(["up", "-d", "--no-build", "--no-deps", "app", "worker"], ["overlay-unpin.json"]);
    await waitHealthy(LAB.base);
    const env = mcplanCompose(
      ["exec", "-T", "app", "sh", "-c", 'printf "%s|%s" "$BASE_URL" "$MCP_RATE_LIMIT_PER_HOUR"'],
      ["overlay-unpin.json"],
    );
...
await step(
  { ...M41, page: "/settings/instance" },
  "Administrator sets Application address to http://[::1]:43320 in Settings → Advanced → Instance address; before the restart nothing changes",
  "Settings saved. Restart the API and worker to apply changes.; the MCP Server address still shows the old origin",
  async () => {
...
await step(
  { ...CMo, page: "container" },
  "Operator restarts the API and worker",
  "App answers again",
  async () => {
    operatorRestart();
    await waitHealthy(LAB.base);
...
a = await adminAt(V6);
await step(
  { ...M41, page: "/settings/mcp" },
  "Reachability pill on an IPv6-only address: OAuth Clients on shows Not reachable with HTTPS scheme, IPv4 record and Public IPv4 address",
  "Switch saves (loopback keeps the authorization server); pill names all three checks",
  async () => {
    await gotoMcp(a.page);
    const addr = (await main(a.page)).match(/http\S+\/mcp/)?.[0];
    ...
    expectThat(
      addr === `${V6}/mcp` &&
        status === 200 &&
        checked === "true" &&
        pill === "Not reachable · HTTPS scheme, IPv4 record, Public IPv4 address",
      JSON.stringify({ addr, status, pill }),
    );
...
await step(
  { ...M41, page: "/settings/instance" },
  "Administrator sets the plain-HTTP LAN Application address http://192.168.50.10:43320",
  "Settings saved. Restart the API and worker to apply changes.",
  async () => {
...
await step(
  { ...CMo, page: "container" },
  "Operator restarts the API and worker after the Instance address change",
  "App answers again",
  async () => {
    operatorRestart();
    await waitHealthy(LAB.base);
...
await step(
  { ...M41, page: "/settings/mcp" },
  "Negative: on a plain-HTTP LAN address the OAuth Clients switches stay off and the pill names the failed checks, including HTTPS scheme",
  "PATCH refused; switches off; Not reachable · HTTPS scheme, Public IPv4 address",
  async () => {
    await gotoMcp(a.page);
    const addr = (await main(a.page)).match(/http\S+\/mcp/)?.[0];
    const s1 = await setSwitch(a.page, "Legal Users OAuth Clients", true);
    await pause(800);
    const legal = await a.page
      .getByRole("switch", { name: "Legal Users OAuth Clients" })
      .getAttribute("aria-checked");
    const pill = flat(
      await a.page
    ...
    await pause(800);
    const business = await a.page
      .getByRole("switch", { name: "Business Users OAuth Clients" })
      .getAttribute("aria-checked");
    ...
    expectThat(
      addr === `${LAN}/mcp` &&
        ...
        legal === "false" &&
        business === "false" &&
        /^Not reachable · HTTPS scheme/.test(pill),
      JSON.stringify({ addr, s1, s2, legal, business, pill }),
    );
...
await step(
  { ...M41, page: "/settings/mcp" },
  "Negative: Add Client and Generate secret fail on the LAN address",
  "Add Client Save shows the save failure; Generate secret on the template shows the secret failure",
  async () => {
    ...
    expectThat(
      /could not be saved/.test(addError) && /could not be generated/.test(genError),
      `${addError} | ${genError}`,
    );
...
await step(
  { ...M41, page: "HTTP" },
  "Negative: the well-known documents return 404 on the LAN address",
  "404 for the OAuth and OpenID discovery documents",
  async () => {
    const paths = [
...
let lanKey;
await step(
  { ...M41, page: "/settings/api-keys" },
  "API keys work on the LAN address: the Administrator's own key and an SDK Client at the LAN Server address",
  "Key collected; the Client connects and whoami is Daniel",
  async () => {
    await gotoMcp(a.page);
    const legalKeys = await a.page
    ...
    const secure = await a.page.evaluate(() => ({
      isSecureContext,
    ...
    const afterCopy = {
      ...
      alert: (await ready.getByRole("alert").count())
        ? flat(await ready.getByRole("alert").innerText())
        : null,
      pageErrors: errors,
    };
    ...
    await client.close();
    expectThat(
      /Daniel/.test(who.structuredContent?.person?.displayName ?? ""),
...
await step(
  { ...CMo, page: "/settings/mcp" },
  "Private deployment: API keys work at the private Server address; OAuth stays unavailable there",
  "See the LAN steps above",
  async () => {
    const lanSteps = log.steps.filter((s) => s.phase === "lan" && s.scenario === "V-M41-MCP");
    expectThat(
      lanSteps.every((s) => s.result === "pass"),
      "a LAN step failed",
    );
...
await step(
  { ...M41, page: "/settings/instance" },
  "Administrator sets Application address back to http://127.0.0.1:43320",
  "Settings saved. Restart the API and worker to apply changes.",
  async () => {
...
await step(
  { ...CMo, page: "container" },
  "Operator restarts the API and worker",
  "App answers; well-known documents answer 200 again",
  async () => {
    operatorRestart();
    await waitHealthy(LAB.base);
...
await step(
  { ...CMa, page: "fixture" },
  "Cleanup: revoke the rate-limit and LAN keys; turn Legal Users API keys off again",
  "Keys revoked",
  async () => {
    const list = (await api(a.page, LAB.base, "GET", "/api/v1/mcp-settings/api-keys")).body;
    const mine = list.filter(
      (r) => r.clientName.startsWith("DOC-030 mcp") && r.status === "active",
    );
    for (const r of mine)
      await api(a.page, LAB.base, "POST", `/api/v1/api-key-requests/${r.id}/revoke`);
...
log.runs.lan.articleHashesAtEnd = {
...
console.log(
  `lan done: ${log.steps.filter((s) => s.phase === "lan").length} steps, ${failed.length} failed`,
);
process.exit(0);
