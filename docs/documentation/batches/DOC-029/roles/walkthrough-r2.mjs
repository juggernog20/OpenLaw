...
await step(
  ...
  async () => {
    ...
    const fullRow = (
      await panel
        .getByRole("listitem")
        .filter({ hasText: "DOC-029 roles Full thread note" })
        .first()
        .innerText()
    ).replace(/\s+/g, " ");
    ...
    check(
      /Contract Team/.test(fullRow.replace("DOC-029 roles Full thread note", "")) &&
...
await step(
  ...
  async () => {
    ...
    const legalChip = await legalComment(
      ...
      `/contracts/${CO}`,
      "Legal Only",
      `DOC-029 roles r2 Legal Only probe ${stamp}`,
    );
    const older = await api(D, "POST", "/comments", {
      entityType: "contract",
      entityId: fx.openContract.id,
      body: `DOC-029 roles r2 Internal team probe ${stamp}`,
    ...
    const teamChip = await legalComment(
      ...
      `/contracts/${CO}`,
      "Contract Team",
      `DOC-029 roles r2 Contract Team probe ${stamp}`,
    );
    await sleep(1000);
...
await step(
  ...
  async () => {
    ...
    const teamChip = await legalComment(
      ...
      `/matters/${MO}`,
      "Matter Team",
      `DOC-029 roles r2 Matter Team note ${stamp}`,
    );
    const legalChip = await legalComment(
      N,
      `/matters/${MO}`,
      "Legal Only",
      `DOC-029 roles r2 Matter Legal Only note ${stamp}`,
    );
    ...
    const jRow = (
      await jc
        .getByRole("listitem")
        .filter({ hasText: `DOC-029 roles r2 Matter Team note ${stamp}` })
        .first()
        .innerText()
    ).replace(/\s+/g, " ");
...
await step(
  ...
  async () => {
    ...
    const hiddenL = await api(N, "POST", "/comments", {
      entityType: "matter",
      entityId: fx.raviMatter.id,
      body: `DOC-029 roles r2 hidden Legal Only ${stamp}`,
      visibility: "legal_only",
    });
    const hiddenW = await api(N, "POST", "/comments", {
      entityType: "matter",
      entityId: fx.raviMatter.id,
      body: `DOC-029 roles r2 hidden Internal team ${stamp}`,
      visibility: "working_team",
    });
    const out = {};
