...
export async function run(log, fx) {
  ...
  await bu.page.screenshot({ path: new URL("./r1-business-user-onboarding.png", import.meta.url).pathname });
  ...
  const amaraReq = sql(`select number from requests where requester_id='${amaraId}' order by number limit 1`);
  ...
  const leaked = (await text(jp.locator("main"))).includes("Reference request for a former employee");
  ...
  R(
    ...
    aHeader.includes(SEED.amara.email) && aText.includes("Reference request for a former employee") && !aText.includes(jonasReqTitle.slice(0, 30)) && aBlocked.replace(/\/$/, "") === "/portal" && !aLeak,
  );
