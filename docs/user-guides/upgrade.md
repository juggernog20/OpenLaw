...
3. Review the deployment settings that the target's Compose file changes. Make each decision in `.env` before the target starts:
   ...
   - Set `TRUSTED_PROXIES` to the proxy's own address, such as `127.0.0.1,::1` for a proxy on the same host. Without it, every visitor behind the proxy shares one sign-in rate-limit bucket, and the app logs a warning at start.
