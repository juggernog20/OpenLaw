...
async function mailCount(email, subjectPart, since = null) {
  ...
  return msgs.filter(
    (m) => m.Subject.includes(subjectPart) && (!since || m.Created >= since),
  ).length;
}
...
async function portalOnboarding(page, acct) {
  ...
  for (let i = 0; i < 8 && page.url().includes("/portal/onboarding"); i++) {
    ...
    seen.push(flat(await main.getByText(/^Step \d of \d$/).first().textContent().catch(() => "")));
  ...
  await step(
    ...
    async () => {
      ...
      for (const [key, role, label] of specs) {
        ...
        out.push(`${role} ${acct.displayName}: initial switches match (${Object.keys(states).length} checked)`);
      }
    ...
    async () => {
      ...
      const submit = async (title) => {
        const r = await L.api(bp, "POST", "/requests", {
          ...
          title,
          description: "DOC-030 conversations fixture. Fictional request for the notifications walkthrough.",
          urgency: "medium",
          customFields: {},
        });
      ...
      fx.n = {
        ...
        openRequestId: (await L.api(bp, "GET", `/portal/requests/${open.number}`)).body?.request?.id ?? null,
...
async function leadTimes() {
  ...
  const card = (page) =>
    page.locator("section, div").filter({ has: page.getByRole("heading", { name: "Reminder lead times" }) }).last();
  ...
  if (!only2) await step(
    ...
    async () => {
      ...
      const tooFar = (await input.evaluate((e) => e.validationMessage)) ||
        flat(await l1.getByText(/between 0 and 730/).first().textContent().catch(() => ""));
      ...
      for (const label of copy) {
        ...
        await list.getByText(label, { exact: true }).waitFor({ state: "detached", timeout: 10000 });
      }
      const left = (await list.getByRole("listitem").allInnerTexts()).map(flat);
      const lastDisabled = await list.getByRole("button", { name: "Remove 3 days before" }).isDisabled();
      await l1.reload();
      await l1.getByRole("heading", { name: "Reminder lead times" }).waitFor();
      const afterReload = (
        await l1.getByRole("list", { name: "Your reminder lead times" }).getByRole("listitem").allInnerTexts()
      ).map(flat);
      const swAfter = await l1
        .getByRole("switch", { name: "Use the organization's default lead times" })
        .getAttribute("aria-checked");
      const apiOwn = (await L.api(l1, "GET", "/me/notification-preferences")).body.reminderOffsetDays;
      at(l1);
      expect(
        initial === "true" &&
          ...
          swAfter === "false" &&
          q(apiOwn) === "[3]",
        JSON.stringify({ initial, orgShown, copy, left, lastDisabled, afterReload, swAfter, apiOwn }),
      );
  ...
  await step(
    ...
    async () => {
      ...
      await l2.getByRole("list", { name: "Your reminder lead times" }).getByRole("listitem").first().waitFor();
      const list = (
        await l2.getByRole("list", { name: "Your reminder lead times" }).getByRole("listitem").allInnerTexts()
      ).map(flat);
      const removeButtons = await l2
        .getByRole("list", { name: "Your reminder lead times" })
        .getByRole("button")
        .count();
      const after = (await L.api(l2, "GET", "/me/notification-preferences")).body.reminderOffsetDays;
      at(l2);
      expect(q(mid) !== "null" && after === null && removeButtons === 0 && list.length > 0, JSON.stringify({ mid, after, list }));
...
async function keyDateDialog() {
  ...
  await step(
    ...
    async () => {
      ...
      await l1.getByRole("link", { name: /^Key dates/ }).first().click();
      ...
      for (const cb of await dialog.getByRole("checkbox").all())
        names.push(
          flat(await cb.evaluate((e) => e.labels?.[0]?.textContent ?? e.getAttribute("aria-label") ?? "")),
        );
      const buOffered = names.some((x) => x.includes(fx.fresh.b1.displayName ?? fx.fresh.b1.email));
      const usualBefore = await dialog.getByRole("button", { name: "Use the usual audience" }).count();
...
async function morningReady() {
  await step(
    ...
    async () => {
      ...
      const s = await switchStates(l3, ["Dates approaching In-app", "Dates approaching Push", "Dates Email"]);
      const out = {};
    ...
    async () => {
      ...
      const submit = async (title) => {
        const r = await L.api(j, "POST", "/requests", {
          ...
          departmentId: deps[0].id,
          title,
          description: "DOC-030 conversations fixture. Fictional request for the comments walkthrough.",
          urgency: "medium",
          customFields: {},
        });
        expect(r.status < 300, `submit ${r.status} ${JSON.stringify(r.body).slice(0, 200)}`);
...
async function memberComments() {
  ...
  await step(CA, role,
    ...
    async () => {
      ...
      expect(teamChecked && /Administrators and Legal Team Members/.test(legalText) && internal === 0,
  ...
  await step(CA, role,
    ...
    async () => {
      ...
      const ml = await postComment(p, `${tag} Nadia matter legal only note`, { tier: "Legal Only" });
  ...
  await step(CA, role,
    ...
    async () => {
      ...
      await a.getByRole("listbox", { name: "People and files you can mention" })
        .getByRole("option", { name: new RegExp(fx.fresh.b1.displayName) }).click();
      await a.getByRole("button", { name: "Comment", exact: true }).click();
      ...
      expect(postedAfterCancel === 0 && draftKept.includes(text), "comment posted after Cancel or draft lost");
      await a.getByRole("button", { name: "Comment", exact: true }).click();
      await dialog.waitFor({ timeout: 10000 });
  ...
  await step(CA, role,
    ...
    async () => {
      ...
      await (await chooser).setFiles([fixture("doc029-conv-notice.pdf"), fixture("doc029-conv-schedule.pdf")]);
      ...
      await row.getByRole("button", { name: /doc029-conv-notice\.pdf/ }).first().click();
      const preview = p.getByRole("dialog").filter({ hasText: "doc029-conv-notice.pdf" });
      await preview.waitFor({ timeout: 15000 });
      await p.waitForTimeout(1500);
      const previewButtons = (await preview.getByRole("button").allInnerTexts()).map(flat).filter(Boolean);
      const previewLinks = (await preview.getByRole("link").allInnerTexts()).map(flat).filter(Boolean);
  ...
  await step(CA, role,
    ...
    async () => {
      const a = applet(p);
      await a.getByRole("textbox", { name: "New comment" }).fill(`${tag} six file probe (not posted)`);
      const chooser = p.waitForEvent("filechooser");
      await a.getByRole("button", { name: "Attach files" }).click();
      await (await chooser).setFiles([1, 2, 3, 4, 5, 6].map((k) => fixture(`doc029-conv-extra-${k}.txt`)));
      await p.waitForTimeout(800);
      const listed = await a.getByRole("list", { name: "Files attached to this comment" }).getByRole("listitem").count();
  ...
  await step(CA, role,
    ...
    async () => {
      ...
      const paper = flat(await a.getByRole("list", { name: "Files attached to this comment" }).innerText());
      const rows = await rowWith(a, text).count();
      const filed = await a.getByText(/^Filed to/).count();
      await p.unroute(/\/api\/v1\/comments(\?.*)?$/, handler);
      expect(kept === text && /doc029-conv-schedule\.pdf/.test(paper) && rows === 0, "draft or paper lost, or row posted");
  ...
  await step(CA, role,
    ...
    async () => {
      ...
      await dialog.waitFor();
      await p.waitForTimeout(800);
      const destOptions = (await dialog.getByLabel("Destination").locator("option").allInnerTexts()).map(flat);
      const typeSelect = dialog.getByLabel("Type", { exact: true });
      const typeOptions = (await typeSelect.count()) ? (await typeSelect.locator("option").allInnerTexts()).map(flat) : [];
      ...
      expect(filedText.includes(docName) && stillThere > 0 && onRecord && fileAgain === 0 && filed2Text.includes(docName),
  ...
  await step(CA, role,
    ...
    async () => {
      ...
      await p.getByRole("menuitem", { name: "Edit" }).click();
      await rowWith(a, text).first().getByRole("textbox", { name: "Edit comment" }).fill(`${text} corrected`);
      ...
      expect(!items.some((i) => /Redact/.test(i)) && !afterCancel.includes("draft change") && !/edited/.test(afterCancel) && radiosInEdit === 0 && gone === 0,
  ...
  await step(CA, role,
    ...
    async () => {
      if (!fx.c.bulkPosted) {
        for (let i = 1; i <= 55; i++)
          await comment(ctx.daniel.page, "matter", fx.c.matterId, `${tag} Daniel bulk update ${String(i).padStart(2, "0")}`, "full_thread");
        await comment(ctx.daniel.page, "matter", fx.c.matterId, `${tag} Daniel matter legal only bulk note`, "legal_only");
        fx.c.bulkPosted = true;
        saveFx();
        prep(`Daniel Okafor posted 55 Matter Team comments and one Legal Only comment on M-${fx.c.matter} through the comments API, to give the Matter more than one page of comments.`);
      }
      await p.goto(M);
      ...
      const beforeName = await toolbar.getByRole("button", { name: /^Comments/ }).getAttribute("aria-label");
      const a = await openComments(p);
      ...
      await p.getByRole("toolbar", { name: "Applets" }).getByRole("button", { name: /^Comments/ }).click();
      await p.waitForTimeout(1500);
      const after = flat(await toolbar.getByRole("button", { name: /^Comments/ }).innerText());
      const afterName = await toolbar.getByRole("button", { name: /^Comments/ }).getAttribute("aria-label");
      at(p);
  ...
  await step(CA, role,
    ...
    async () => {
      ...
      const toolbar = p.getByRole("toolbar", { name: "Applets" });
      await toolbar.waitFor();
      const applets = await toolbar.getByRole("button").evaluateAll((els) => els.map((e) => e.getAttribute("aria-label") ?? e.textContent.trim()));
      const a = await openComments(p);
      at(p);
      const labels = flat(await a.getByRole("group", { name: "Audience" }).innerText());
      ...
      const shared = await postComment(p, `${tag} Nadia shared reply on the open Request`, { tier: "Shared with requester" });
      const legal = await postComment(p, `${tag} Nadia legal only Request note`, { tier: "Legal Only" });
      expect(legalFirst && !applets.some((x) => /History/.test(x)), `applets ${applets} legalFirst ${legalFirst}`);
  ...
  await step(CA, role,
    ...
    async () => {
      if (!fx.c.workingTeam) {
        await comment(p, "contract", fx.c.contractId, `${tag} older working team note`, "working_team");
        await comment(p, "request", fx.c.openRequestId, `${tag} older working team request note`, "working_team");
        fx.c.workingTeam = true;
        saveFx();
        prep(`Nadia Haddad posted one working_team comment on C-${fx.c.contract} and one on R-${fx.c.openRequest} through the comments API to stand in for older Working Team comments; the composer does not offer that tier.`);
      }
      await p.goto(C);
      ...
      at(p);
      expect(/Internal team/.test(cs) && /Working team/.test(rs) && !/Internal|Working/.test(cOptions + rOptions), JSON.stringify({ cs, rs, cOptions, rOptions }));
  ...
  await step(CA, role,
    ...
    async () => {
      if (!fx.c.taskId) {
        const r = await L.api(p, "POST", `/contracts/${fx.c.contract}/tasks`, { title: `${tag} prepare the signing pack`, dueDate: utcDate(10) });
        ...
        fx.c.taskId = (listed.tasks ?? listed).find((x) => x.title === `${tag} prepare the signing pack`)?.id;
        saveFx();
        prep(`Nadia Haddad added the Task "${tag} prepare the signing pack" due ${utcDate(10)} on C-${fx.c.contract} through the tasks API.`);
      }
      ...
      const audience = flat(await dialog.locator("p").filter({ hasText: /^Visible to/ }).first().textContent().catch(() => ""));
      ...
      const s = await rowSummary(row);
      at(p);
      expect(radios === 0 && !/Legal Only|Contract Team|Internal team|Working team|Full Thread/.test(s), JSON.stringify({ radios, s }));
  ...
  await step(CA, role,
    ...
    async () => {
      ...
      let block = true;
      const thread = (route) =>
        block && route.request().method() === "GET" && !/unread|mention/.test(route.request().url()) ? route.abort("failed") : route.continue();
      await n.route(/\/api\/v1\/comments\?/, thread);
      await openApplet(n, "Comments");
      const a = applet(n);
      await a.getByText("The conversation could not be read. Reopen the panel to try again.").waitFor({ timeout: 15000 });
      block = false;
      ...
      let blockOlder = true;
      const older = (route) => (blockOlder && /before=|cursor=/.test(route.request().url()) ? route.abort("failed") : route.continue());
      await n.route(/\/api\/v1\/comments\?/, older);
      await a.getByRole("button", { name: "Show older" }).click();
      await a.getByText("The earlier comments could not be read. Try again.").waitFor({ timeout: 15000 });
      ...
      await panel.getByText("The history could not be read. Reopen the panel to try again.").waitFor({ timeout: 15000 });
      blockHist = false;
      ...
      at(n);
      expect(loaded > 0 && kept === loaded && more > kept && hRows > 0, JSON.stringify({ loaded, kept, more, hRows }));
...
async function adminComments() {
  ...
  await step(CA, role,
    ...
    async () => {
      ...
      const legal = await postComment(p, `${tag} Daniel legal only memo`, { tier: "Legal Only", files: ["doc029-conv-legal-memo.pdf"] });
      const s2 = await rowSummary(legal);
      expect(first && /Legal Only/.test(s2) && /doc029-conv-legal-memo\.pdf/.test(s2), `rows ${s2}`);
  ...
  await step(CA, role,
    ...
    async () => {
      ...
      at(p);
      const row = await postComment(p, `${tag} Daniel matter paper`, { tier: "Matter Team", files: ["doc029-conv-schedule.pdf"] });
      const offer = await row.getByRole("button", { name: "File to Matter" }).count();
      await row.getByRole("button", { name: "File to Matter" }).click();
      const dialog = p.getByRole("dialog", { name: "File attachment" });
      await dialog.waitFor();
      const dest = (await dialog.getByLabel("Destination").locator("option").allInnerTexts()).map(flat);
      await dialog.getByRole("button", { name: "Cancel" }).click();
      await dialog.waitFor({ state: "hidden" });
  ...
  await step(CA, role,
    ...
    async () => {
      ...
      const row = await postComment(p, text, { tier: "Shared with requester", files: ["doc029-conv-schedule.pdf"] });
      ...
      expect(fileControls === 0 && gone === 0 && tomb > 0, `file ${fileControls} gone ${gone} tomb ${tomb}`);
...
async function portalComments() {
  ...
  await step(CA, role,
    ...
    async () => {
      ...
      await row.waitFor({ timeout: 15000 });
      const s = await rowSummary(row);
      expect(radios === 0 && shared === 1 && legal === 0 && wt === 0 && s.includes(fx.fresh.b1.displayName) && /doc029-conv-portal-note\.pdf/.test(s),
  ...
  await step(CA, role,
    ...
    async () => {
      ...
      const otherMenu = await rowWith(a, `${tag} Nadia shared reply on the open Request`).first().getByRole("button", { name: "Comment actions" }).count();
      expect(!items.some((i) => /Redact/.test(i)) && otherMenu === 0, `items ${items} other ${otherMenu}`);
  ...
  await step(CA, role,
    ...
    async () => {
      ...
      const download = await dl;
      const same = sha256(await download.path()) === sha256(fixture("doc029-conv-notice.pdf"));
      expect(seen.team && seen.widened && seen.notice && seen.danielTeam && !seen.legalOnly && !seen.danielMemo && !seen.memoFile && !seen.workingTeam && radios === 0 && fileTo === 0 && same,
...
async function portalHistory() {
  ...
  const tag = fx.c.tag;
  const role = "business_user";
  await step(CA, "legal_team_member",
    ...
    async () => {
      ...
      const statuses = (await L.api(ctx.daniel.page, "GET", "/contract-statuses")).body.contractStatuses;
      ...
      if (!fx.c.progressPart1) {
        await call("contract to Internal review", "PATCH", `/contracts/${cn}`, { statusId: sid("internal_review") });
        await call("contract to With counterparty", "PATCH", `/contracts/${cn}`, { statusId: sid("redlining") });
        await call("contract title", "PATCH", `/contracts/${cn}`, { title: `${fx.c.contractTitle} (renamed)` });
        await call("portal-visible field", "PATCH", `/contracts/${cn}`, { customFields: { deal_desk_reference: `DD-${stamp}` } });
        await call("hidden field", "PATCH", `/contracts/${cn}`, { customFields: { governing_law: "England and Wales" } });
        await call("task added", "POST", `/contracts/${cn}/tasks`, { title: `${tag} collect signatures`, dueDate: utcDate(14) });
        await call("task added 2", "POST", `/contracts/${cn}/tasks`, { title: `${tag} draft cover note` });
  ...
  await step(CA, role,
    ...
    async () => {
      ...
      const checks = {
        ...
        visibleField: has(/Deal desk reference|deal desk/i) || has(/changed 2 fields|changed # fields/),
      ...
      expect(checks.stage && !checks.status && checks.visibleField && !checks.hiddenField && checks.taskAdded && checks.taskCompleted && !checks.taskEdited && !checks.taskReordered && !checks.taskRemoved && checks.comments > 0 && checks.comments < staffComments,
  ...
  await step(CA, role,
    ...
    async () => {
      ...
      if (!fx.c.matterClosed) {
        const ms = JSON.parse(`[${psql("select json_build_object('id', id, 'slug', slug) from matter_statuses where archived_at is null").split("\n").join(",")}]`);
        const r = await L.api(n, "PATCH", `/matters/${mn}`, { statusId: ms.find((s) => s.slug === "closed").id, closingNote: `${tag} closing note kept inside Legal` });
        expect(r.status < 300, `close ${r.status} ${JSON.stringify(r.body).slice(0, 200)}`);
        fx.c.matterClosed = true;
        saveFx();
        prep(`Nadia Haddad closed M-${mn} with a closing note through the API after the Matter conversation checks.`);
      }
      ...
      expect(checks.statusMoves >= 2 && checks.visibleField && !checks.hiddenField && !checks.closingNote, JSON.stringify({ checks, entries: entries.slice(0, 12) }));
  ...
  await step(CA, role,
    ...
    async () => {
      ...
      const entries = await historyEntries(j, true);
      at(j);
      const staffRows = await L.api(n, "GET", `/activity?entityType=request&entityId=${fx.c.openRequestId}&limit=100`);
...
async function linkedRedaction() {
  ...
  await step(CA, "legal_team_member",
    ...
    async () => {
      if (!fx.c.hidden) {
        const ct = (await L.api(d, "GET", "/contract-types")).body.contractTypes;
        const r = await L.api(d, "POST", "/contracts", { title: `${tag} confidential side letter`, contractTypeId: ct.find((t) => t.slug === "sales").id });
        expect(r.status < 300, `create ${r.status} ${JSON.stringify(r.body).slice(0, 300)}`);
        const hidden = r.body.contract ?? r.body;
        const conf = await L.api(d, "PATCH", `/contracts/${hidden.number}`, { isConfidential: true });
        expect(conf.status < 300, `confidential ${conf.status}`);
        const link = await L.api(d, "POST", `/contracts/${fx.c.contract}/relations`, { relatedContractNumber: hidden.number, relationType: "related" });
        ...
        saveFx();
        prep(`Daniel Okafor created C-${hidden.number} "${tag} confidential side letter", marked it Confidential (Nadia Haddad is not on its team), and linked C-${fx.c.contract} to it as related, through the API.`);
      }
      ...
      expect(reach >= 400 && nEntries.length > 0 && !nEntries.some((e) => e.includes(hiddenRef) || e.includes("confidential side letter")) && dEntries.some((e) => e.includes("confidential side letter")),
...
async function adminRedact() {
  ...
  await step(CA, role,
    ...
    async () => {
      ...
      const attachmentHref = await target.getByRole("link", { name: "Download doc029-conv-legal-memo.pdf" }).getAttribute("href");
      ...
      const deleted = a.getByRole("listitem").filter({ hasText: "Comment deleted by its author." }).first();
      ...
      expect(before === 200 && after === 404 && tombs >= 3 && docStays, JSON.stringify({ before, after, tombs, docStays }));
  ...
  await step(CA, role,
    ...
    async () => {
      ...
      await openComments(p);
      const row0 = await postComment(p, `${tag} Daniel archive paper`, { tier: "Contract Team", files: ["doc029-conv-schedule.pdf"] });
      ...
      prep(`Daniel Okafor archived C-${fx.c.archive} through the archive API after posting a Contract Team comment with paper in the browser.`);
      await p.goto(`${L.BASE}/contracts/${fx.c.archive}`);
      const a = await openComments(p);
      at(p);
...
async function portalAfter() {
  ...
  await step(CA, role,
    ...
    async () => {
      ...
      await p.waitForTimeout(2000);
      at(p);
      const msg = flat(await p.getByText(/archived/).first().textContent().catch(() => ""));
      ...
      expect(/archived/.test(msg) && comments === 0 && recordRead >= 400, `msg ${msg} comments ${comments} record ${recordRead}`);
  ...
  await step(CA, role,
    ...
    async () => {
      const add = await L.api(d, "POST", `/contracts/${fx.c.contract}/team`, { userId: fx.c.ids.amara });
      ...
      try {
        ...
        restore = await L.api(d, "POST", `/contracts/${fx.c.contract}/team`, { userId: fx.c.ids.jonas });
        await L.api(d, "PATCH", `/contracts/${fx.c.contract}`, { businessOwnerId: fx.c.ids.jonas });
      }
      prep(`Daniel Okafor added Amara Nwosu (Business User) to the C-${fx.c.contract} team through the team API, then removed her again. He then cleared the Business Owner (HTTP ${bo.status}; the API refuses to take the Business Owner off the team), removed the requester from the team (HTTP ${rj.status}), and afterwards restored both.`);
      expect(earlier === 1 && legal === 0 && rm.status < 300 && bo.status < 300 && rj.status < 300 && amaraApi >= 400 && jonasApi >= 400 && jonasComments === 0 && restore.status < 300,
        JSON.stringify({ earlier, legal, amaraApi, jonasApi, jonasComments, restore: restore.status }));
...
async function memberBell() {
  ...
  await step(NT, role,
    ...
    async () => {
      if (!fx.n.bellFill) {
        for (let i = 1; i <= 11; i++)
          await comment(n, "contract", fx.n.contractId, `${tag} bell fill ${String(i).padStart(2, "0")} for Legal Own List`, "full_thread", [acct.id]);
        fx.n.bellFill = true;
        saveFx();
        prep(`Nadia Haddad posted eleven Contract Team comments mentioning Legal Own List on C-${fx.n.contract} through the comments API to give that account more than nine unread bell items.`);
      ...
      const item = dialog.getByRole("link", { name: new RegExp(`Nadia Haddad mentioned you on .*${stamp}`) }).first();
      ...
      await d2.getByRole("link", { name: new RegExp(`Nadia Haddad mentioned you on .*${stamp}`) }).first().click();
      await p.waitForURL(new RegExp(`/contracts/${fx.n.contract}`));
      at(p);
      await p.waitForTimeout(1500);
      const afterSelect = await unreadApi(p);
      const d3 = await openBell(p);
      const againName = flat(await d3.getByRole("link", { name: new RegExp(`mentioned you on .*${stamp}`) }).first().innerText());
      await closeBell(p);
      expect(before > 9 && badge === "9+" && afterOpen === before && afterSelect === before - 1 && !/^Unread/.test(againName),
  ...
  await step(NT, role,
    ...
    async () => {
      if (!fx.n.staffApproval) {
        const r = await L.api(n, "POST", `/contracts/${fx.n.contract}/approvals`, { approverIds: [acct.id] });
        expect(r.status < 300, `approval ${r.status} ${JSON.stringify(r.body).slice(0, 300)}`);
        const rows = r.body.approvals ?? r.body.requests ?? [];
        const mine = rows.find((x) => (x.approverId ?? x.approver?.id) === acct.id && (x.status ?? x.state) === "pending") ?? rows.find((x) => (x.approverId ?? x.approver?.id) === acct.id);
        fx.n.staffApproval = mine?.id ?? null;
        saveFx();
        prep(`Nadia Haddad requested approval of C-${fx.n.contract} from Legal Own List through the approvals API (approval ${fx.n.staffApproval}).`);
        await settleQueue(2000);
      }
      await p.goto(`${L.BASE}/`);
      await p.waitForTimeout(1000);
      ...
      const order = await dialog.evaluate((el) => [...el.querySelectorAll("h3")].map((h) => h.textContent.trim()));
      const markAll = await dialog.getByRole("button", { name: "Mark all read" }).count();
      const note = flat(await dialog.getByText("Mark all read leaves Your approvals in place.").textContent().catch(() => ""));
      ...
      expect(earlier === 1 && markAll === 1 && stillApproval === 1 && markAllAfter === 0 && unreadAfter >= 1 && note && stillCounted >= 1,
        JSON.stringify({ heading, earlier, markAll, unreadAfter, stillApproval, markAllAfter, note, stillCounted }));
  ...
  await step(NT, role,
    ...
    async () => {
      const before = await unreadApi(p);
      const r = await L.api(p, "POST", `/approvals/${fx.n.staffApproval}/decision`, { decision: "approved", note: "DOC-030 fictional approval" });
      prep(`Legal Own List answered approval ${fx.n.staffApproval} through the decision API (HTTP ${r.status}); the approval procedure itself belongs to the contract-approvals guide.`);
      await p.goto(`${L.BASE}/`);
      await p.waitForTimeout(1500);
      const dialog = await openBell(p);
      ...
      expect(r.status < 300 && group === 0 && after < before, JSON.stringify({ status: r.status, body: r.body, group, before, after }));
  ...
  await step(NT, role,
    ...
    async () => {
      if (!fx.n.bellMore) {
        for (let i = 1; i <= 16; i++)
          await comment(n, "contract", fx.n.contractId, `${tag} bell page ${String(i).padStart(2, "0")} for Legal Own List`, "full_thread", [acct.id]);
        fx.n.bellMore = true;
        saveFx();
        prep(`Nadia Haddad posted sixteen more mentions of Legal Own List on C-${fx.n.contract} through the comments API so the bell has more than one page.`);
        await settleQueue(2000);
      }
      await p.goto(`${L.BASE}/`);
      const dialog = await openBell(p);
  ...
  await step(NT, role,
    ...
    async () => {
      ...
      const caption = flat(await l2.getByText(/^These switches change the email only/).textContent());
  ...
  await step(NT, role,
    ...
    async () => {
      ...
      const mentionRound = async (label) => {
        ...
        const id = await comment(n, "contract", fx.n.contractId, `${tag} mention round ${label}`, "full_thread", [acct.id]);
        await settleQueue();
      ...
      const pushWhileOff = await p.getByRole("switch", { name: "Assigned to you Push", exact: true }).getAttribute("aria-checked");
      const r3 = await mentionRound("inapp-off");
      await setSwitch(p, "Assigned to you In-app", true);
      expect(keptOff === "false" && r1.bell === 1 && r1.mail === 0 && r1.commentItems === 0 && r2.bell === 1 && r2.mail === 1 && keptInApp === "false" && r3.bell === 0 && r3.mail === 0,
  ...
  await step(NT, role,
    ...
    async () => {
      ...
      await p.reload();
      const reloaded = await p.getByRole("switch", { name: "New requests Push", exact: true }).getAttribute("aria-checked");
    ...
    async () => {
      ...
      if (!fx.n.bellRequest) {
        ...
        const r = await L.api(b1, "POST", "/requests", {
          requestTypeId: (await L.api(d, "GET", "/request-types")).body.requestTypes.find((t) => t.slug === "legal_question").id,
        ...
        saveFx();
        prep(`The fresh Business User submitted R-${fx.n.bellRequest} through the Requests API for the New requests and Request receipt checks.`);
        await settleQueue(2000);
      }
      await p.goto(`${L.BASE}/`);
      const dialog = await openBell(p);
      const link = await findItem(p, dialog, new RegExp(`new request: DOC-030 conversations new request bell ${stamp}`));
...
async function devices() {
  ...
  const out = {};
  await step(NT, "legal_team_member",
    ...
    async () => {
      ...
      const c1 = await b.newContext({ baseURL: L.BASE, viewport: { width: 1440, height: 900 }, storageState: undefined });
      ...
      const card = p1.locator("div, section").filter({ has: p1.getByRole("heading", { name: "Devices", exact: true }) }).last();
      await p1.getByRole("heading", { name: "Devices", exact: true }).waitFor();
      await p1.waitForTimeout(2000);
      out.support = await p1.evaluate(() => ({ sw: "serviceWorker" in navigator, push: "PushManager" in window, notification: "Notification" in window, permission: typeof Notification !== "undefined" ? Notification.permission : null }));
      out.cardBefore = await devicesCardText(p1);
      out.detailsSwitch = await p1.getByRole("switch", { name: "Show details in notifications" }).getAttribute("aria-checked");
      ...
      if (out.turnOnShown) {
        ...
        out.subscriptions = (await L.api(p1, "GET", "/notifications/subscriptions")).body?.subscriptions?.length ?? null;
        ...
        if (out.revokeButtons) {
          ...
          out.subscriptionsAfterRevoke = (await L.api(p1, "GET", "/notifications/subscriptions")).body?.subscriptions?.length ?? null;
        }
      }
      ...
      out.detailsSaved = (await L.api(p1, "GET", "/me/notification-preferences")).body.showRecordNamesOnDevices;
      ...
      const card2 = p2.locator("div, section").filter({ has: p2.getByRole("heading", { name: "Devices", exact: true }) }).last();
      ...
      out.portalCard = flat(await bp.locator("main").innerText()).split("How we tell you about your work").pop().slice(0, 600);
      out.portalCard = out.portalCard.slice(out.portalCard.indexOf("Devices"));
      results.records.deviceStates = out;
      expect(/Devices/.test(out.cardBefore) && out.detailsSwitch === "true" && out.detailsSaved === false, JSON.stringify(out));
...
async function adminNotifications() {
  ...
  await step(NT, role,
    ...
    async () => {
      ...
      await openStaffNotificationSettings(p, acct.displayName);
      const init = await switchStates(p, ["Activity on your records In-app", "Activity on your records Email", "Activity on your records Push"]);
      const round = async (label, actor = n) => {
        const since = new Date(Date.now() - 2000).toISOString();
        const bm = await mailCount(email, subject);
        const id = await comment(actor, "contract", fx.n.contractId, `${tag} activity round ${label}`, "full_thread");
        await settleQueue();
      ...
      await p.waitForURL(new RegExp(`/contracts/${fx.n.contract}`));
      at(p);
      expect(r1.bell === 1 && r1.mail === 0 && r2.bell === 1 && r2.mail === 1 && own.bell === 0 && own.mail === 0, JSON.stringify({ init, r1, r2, own }));
  ...
  await step(NT, role,
    ...
    async () => {
      ...
      let block = true;
      const handler = (route) =>
        block && route.request().method() === "GET" && !/unread-count/.test(route.request().url()) ? route.abort("failed") : route.continue();
      await p.route(/\/api\/v1\/notifications(\?.*)?$/, handler);
      await bell(p).click();
      const dialog = p.getByRole("dialog", { name: "Notifications" });
      await dialog.getByText("Notifications could not be read. Close this and open it again.").waitFor({ timeout: 10000 });
      block = false;
      ...
      await d2.getByText("The older notifications could not be read. Try again.").waitFor({ timeout: 10000 });
...
async function withMcpEnabled(fn) {
  ...
  prep(`Organization MCP settings before the API key checks: ${q({ enabled: before.enabled, legalApiKeysEnabled: before.legalApiKeysEnabled, businessApiKeysEnabled: before.businessApiKeysEnabled })}. Daniel Okafor set ${q(change)} through the MCP settings API (HTTP ${set.status}) for the API key and MCP attribution checks.`);
  try {
    return await fn();
  } finally {
    if (Object.keys(restore).length) {
      const r = await L.api(d, "PATCH", "/mcp-settings", restore);
      prep(`Daniel Okafor restored the organization MCP settings to ${q(restore)} (HTTP ${r.status}).`);
    }
  }
}
...
async function apiKeysAndVia() {
  ...
  await withMcpEnabled(async () => {
    await step(NT, "administrator",
      ...
      async () => {
        const req = await L.api(l1, "POST", "/api-key-requests", { clientName: "Claude", toolsets: ["comments"], scope: "write", note: "DOC-030 fictional key for the attribution check" });
        expect(req.status === 201, `request ${req.status} ${JSON.stringify(req.body).slice(0, 200)}`);
        fx.n.keyRequest = req.body.id;
        saveFx();
        prep(`Legal Own List requested an API key for the Client "Claude" (comments Toolset, write) through the API key request API (request ${req.body.id}); the request procedure belongs to the Configure MCP guide.`);
        await settleQueue(2000);
        ...
        const section = dialog.getByRole("region", { name: "Your approvals" });
        const row = section.getByRole("listitem").filter({ hasText: fx.fresh.l1.displayName }).first();
        ...
        const stillThere = await section.getByRole("listitem").filter({ hasText: fx.fresh.l1.displayName }).count();
        const dialogText = flat(await a1.getByRole("dialog").last().innerText().catch(() => ""));
        ...
        const status = (await L.api(l1, "GET", "/api-key-requests")).body.requests.find((r) => r.id === fx.n.keyRequest)?.status;
        at(a1);
        expect(buttons.includes("Approve") && buttons.includes("Deny") && stillThere === 0 && status === "active", JSON.stringify({ rowText, buttons, stillThere, status }));
    ...
    await step(NT, "legal_team_member",
      ...
      async () => {
        ...
        prep(`Legal Own List read the one-time key in memory and posted one Contract Team comment on C-${fx.n.contract} through the MCP endpoint's openlaw_comment_post tool (HTTP ${posted.status}); the key was never written down.`);
        await settleQueue(2000);
        const a1 = await freshPage("a1");
        ...
        const bellItems = await itemsMatching(a1, /posted through the Claude client|via/, false, new Date(Date.now() - 300000).toISOString());
        ...
        expect(landed.startsWith("/settings/api-keys") && ok && hist.length > 0 && hist[0].includes(`${fx.fresh.l1.displayName}, via Claude,`),
    ...
    await step(NT, "business_user",
      ...
      async () => {
        const req = await L.api(b1, "POST", "/api-key-requests", { clientName: "Claude", toolsets: ["comments"], scope: "read" });
        expect(req.status === 201, `request ${req.status} ${JSON.stringify(req.body).slice(0, 200)}`);
        fx.n.buKeyRequest = req.body.id;
        saveFx();
        prep(`The fresh Business User requested an API key for "Claude" (comments, read) through the API key request API (request ${req.body.id}).`);
        await settleQueue(2000);
        await a1.goto(`${L.BASE}/`);
        const dialog = await openBell(a1);
        const group = dialog.getByRole("heading", { name: /Your approvals/ });
        await group.waitFor({ timeout: 10000 });
        const row = dialog.getByRole("region", { name: "Your approvals" }).getByRole("listitem").filter({ hasText: fx.fresh.b1.displayName }).first();
        const rowText = flat(await row.innerText());
        await row.getByRole("button", { name: "Deny" }).click();
        await a1.waitForTimeout(2500);
        await closeBell(a1).catch(() => {});
        const status = (await L.api(b1, "GET", "/api-key-requests")).body.requests.find((r) => r.id === fx.n.buKeyRequest)?.status;
        await settleQueue(2000);
        await b1.goto(`${L.BASE}/portal`);
...
async function portalNotifications() {
  ...
  await step(NT, role,
    ...
    async () => {
      await comment(n, "contract", fx.n.contractId, `${tag} Nadia team update for the requester`, "full_thread");
      await settleQueue(2000);
      await p.goto(`${L.BASE}/portal`);
      const name = await bellName(p);
      const dialog = await openBell(p);
      const receipt = await findItem(p, dialog, new RegExp(`received your request DOC-030 conversations new request bell ${stamp}`));
      expect(receipt, "no receipt item");
      const rl = flat(await receipt.innerText());
      const rhref = await receipt.getAttribute("href");
      await closeBell(p);
      const d2 = await openBell(p);
      const rec = await findItem(p, d2, new RegExp(`Nadia Haddad replied on your request ${title}`));
  ...
  await step(NT, role,
    ...
    async () => {
      ...
      at(p);
      const groups = ["Request updates", "Assigned to you", "Activity on your records", "Dates approaching"];
      const names = groups.flatMap((g) => ["In-app", "Email", "Push"].map((c) => `${g} ${c}`));
      const states = await switchStates(p, names);
      const expected = Object.fromEntries(names.map((k) => [k, !/^Activity on your records (Email|Push)$/.test(k)]));
      const diff = names.filter((k) => states[k] !== expected[k]);
      const details = [];
      for (const re of [/^Receipts, replies/, /^Contract team additions/, /^Shared comments/, /^Key-date reminders/])
        details.push(flat(await p.getByText(re).first().textContent().catch(() => "")));
  ...
  await step(NT, role,
    ...
    async () => {
      ...
      const round = async (label) => {
        const since = new Date(Date.now() - 2000).toISOString();
        const bm = (await mailSearch(`to:"${email}"`)).filter((m) => m.Subject.startsWith(subject) && m.Subject.includes(fx.n.openTitle)).length;
        const id = await comment(n, "request", fx.n.openRequestId, `${tag} Nadia reply round ${label}`, "full_thread");
        await settleQueue();
        const am = (await mailSearch(`to:"${email}"`)).filter((m) => m.Subject.startsWith(subject) && m.Subject.includes(fx.n.openTitle)).length;
      ...
      at(p);
      expect(kept === "false" && kept2 === "false" && r1.bell === 1 && r1.mail === 0 && r2.bell === 1 && r2.mail === 1 && r3.bell === 0 && r3.mail === 0,
  ...
  await step(NT, role,
    ...
    async () => {
      const add = await L.api(d, "POST", `/contracts/${fx.n.contract}/team`, { userId: (await L.api(d, "GET", "/users?limit=200")).body.users.find((u) => u.email === L.PEOPLE.amara.email).id });
      expect(add.status < 300, `add amara ${add.status}`);
      ctx.amara ??= await L.portalContext(L.PEOPLE.amara.email);
      await p.goto(`${L.BASE}/portal/settings`);
      await p.getByRole("switch", { name: "Request updates Email", exact: true }).waitFor();
      const prefs = await switchStates(p, ["Activity on your records Email", "Request updates Email"]);
      const subj = `Legal replied on`;
      const once = async (label) => {
        const since = new Date(Date.now() - 2000).toISOString();
        const id = await comment(n, "contract", fx.n.contractId, `${tag} converted reply ${label}`, "full_thread");
        await settleQueue();
        const mine = await aboutComment(p, id, since, true);
        const amara = await aboutComment(ctx.amara.page, id, since, true);
        const mail = (await mailsSince(email, since)).map((m) => m.Subject).filter((s) => s.includes(title));
      ...
      await setSwitch(p, "Request updates Email", true);
      const rm = await L.api(d, "DELETE", `/contracts/${fx.n.contract}/team/${fx.c?.ids?.amara ?? (await L.api(d, "GET", "/users?limit=200")).body.users.find((u) => u.email === L.PEOPLE.amara.email).id}`);
      prep(`Daniel Okafor added Amara Nwosu to the C-${fx.n.contract} team for the comparison and removed her afterwards (HTTP ${rm.status}).`);
      at(p);
      expect(r1.mine.includes("request.replied") && r1.mail.some((s) => s.startsWith(subj)) && r2.mine.includes("request.replied") && r2.mail.length === 0 && r1.amara.includes("comment.posted"),
  ...
  await step(NT, role,
    ...
    async () => {
      ...
      const mid = await comment(n, "contract", fx.n.contractId, `${tag} Nadia mentions the requester`, "full_thread", [acct.id]);
      const lid = await comment(n, "contract", fx.n.contractId, `${tag} Nadia legal only no portal item`, "legal_only");
      await settleQueue();
  ...
  await step(NT, role,
    ...
    async () => {
      const r = await L.api(n, "POST", `/contracts/${fx.n.contract}/approvals`, { approverIds: [acct.id] });
      expect(r.status < 300, `approval ${r.status} ${JSON.stringify(r.body).slice(0, 300)}`);
      const mine = r.body.approvals.find((x) => x.approver?.id === acct.id && x.status === "pending");
      prep(`Nadia Haddad requested approval of C-${fx.n.contract} from the fresh Business User through the approvals API (approval ${mine?.id}).`);
      await settleQueue(2000);
      await p.goto(`${L.BASE}/portal`);
      await p.waitForTimeout(1000);
      const dialog = await openBell(p);
      ...
      const landed = at(p);
      const heading = flat(await p.getByRole("heading", { level: 1 }).first().textContent().catch(() => ""));
      const cancel = await L.api(n, "DELETE", `/approvals/${mine.id}`);
      prep(`Nadia Haddad cancelled that approval request through the approvals API (HTTP ${cancel.status}).`);
      await p.goto(`${L.BASE}/portal`);
      await p.waitForTimeout(1500);
      const d2 = await openBell(p);
      const after = await d2.getByRole("heading", { name: /Your approvals/ }).count();
      await closeBell(p);
      expect(landed === `/portal/approvals/${mine.id}` && earlier === 1 && after === 0, JSON.stringify({ landed, earlier, after }));
  ...
  await step(NT, role,
    ...
    async () => {
      ...
      prep(`Daniel Okafor cleared the Business Owner (HTTP ${bo.status}), removed the fresh Business User from the C-${fx.n.contract} team (HTTP ${rm.status}), and restored both afterwards, through the API, for the Portal bell access check.`);
...
function htmlText(html) {
  return flat(html.replace(/<style[\s\S]*?<\/style>/g, " ").replace(/<[^>]+>/g, " ").replace(/&nbsp;/g, " ").replace(/&rarr;/g, "→").replace(/&amp;/g, "&").replace(/&#39;/g, "'").replace(/&quot;/g, '"').replace(/&hellip;/g, "…"));
}
async function waitMailWith(email, marker, since, timeoutMs = 45000) {
  ...
  while (Date.now() < end) {
    for (const m of await mailsSince(email, since)) {
      ...
      if (!marker || t.includes(marker) || m.Subject.includes(marker)) return { subject: m.Subject, text: htmlText(msg.HTML ?? ""), plain: flat(msg.Text ?? ""), attachments: (msg.Attachments ?? []).length };
    }
    await sleep(800);
  }
  return null;
...
async function commentEmails() {
  ...
  await step(NT, "legal_team_member",
    ...
    async () => {
      ...
      const long = `${tag} long email check. ` + "The fictional supplier asks for a longer notice period and a revised cap on liability for data incidents. ".repeat(4) + "TAIL-MARKER-NOT-IN-EMAIL";
      ...
      const checks = {
        ...
        tier: /Nadia Haddad\s*·?\s*Full Thread/.test(t) || (t.includes("Nadia Haddad") && t.includes("Full Thread")),
      ...
      expect(checks.tier && checks.cut && checks.readFull && !checks.attachmentNamed && checks.attachments === 0 && checks.footer, JSON.stringify({ checks, posted, t: t.slice(0, 1200) }));
  ...
  await step(NT, "legal_team_member",
    ...
    async () => {
      const since = new Date(Date.now() - 1000).toISOString();
      await comment(n, "contract", fx.n.contractId, `${tag} legal only words for the email`, "legal_only", [L1.id]);
      const m1 = await waitMailWith(L1.email, "legal only words for the email", since);
      if (!fx.n.taskId) {
        const t = await L.api(n, "POST", `/contracts/${fx.n.contract}/tasks`, { title: `${tag} email task`, assigneeId: L1.id });
        ...
        saveFx();
        prep(`Nadia Haddad added the Task "${tag} email task" assigned to Legal Own List on C-${fx.n.contract} through the tasks API.`);
      }
      await comment(n, "contract_task", fx.n.taskId, `${tag} task words for the email`, "working_team", [L1.id]);
      const m2 = await waitMailWith(L1.email, "task words for the email", since);
      expect(m1 && m2, `mails ${!!m1} ${!!m2}`);
      const tierOn = (t) => ["Legal Only", "Working Team", "Full Thread"].filter((x) => t.includes(x));
      expect(tierOn(m1.text).includes("Legal Only") && tierOn(m2.text).length === 0, JSON.stringify({ m1: m1.text.slice(0, 500), m2: m2.text.slice(0, 500) }));
  ...
  await step(NT, "business_user",
    ...
    async () => {
      const since = new Date(Date.now() - 1000).toISOString();
      await comment(n, "contract", fx.n.contractId, `${tag} shared words for the requester email`, "full_thread");
      await comment(n, "contract", fx.n.contractId, `${tag} legal only words the requester must not get`, "legal_only");
      const m = await waitMailWith(fx.fresh.b1.email, "shared words for the requester email", since);
      await settleQueue();
  ...
  await step(NT, "legal_team_member",
    ...
    async () => {
      const round = async (label, change) => {
        const since = new Date(Date.now() - 1000).toISOString();
        const id = await comment(n, "contract", fx.n.contractId, `${tag} ${label} original words`, "full_thread", [L1.id]);
        ...
        for (const m of await mailsSince(L1.email, since)) {
          ...
          if (t.includes("You were mentioned")) mails.push({ created: m.Created, words: t.includes(`${label} revised words`) ? "revised" : t.includes(`${label} original words`) ? "original" : "none" });
        }
      ...
      for (let i = 1; i <= 10; i++) {
        ...
        edit = await round(label, (id) => L.api(n, "PATCH", `/comments/${id}`, { body: `${tag} ${label} revised words` }));
      ...
      expect(edits.length > 0 && edit.status < 300 && del.status < 300 && edit.mails.length === 1 && edit.mails[0].words === "revised" && del.mails.length === 1 && del.mails[0].words === "none", JSON.stringify({ edits, edit, del }));
  ...
  await step(NT, "administrator",
    ...
    async () => {
      ...
      await a1.getByRole("navigation", { name: "Settings sections" }).getByRole("group", { name: "Organization" }).getByRole("link", { name: "Notifications" }).click();
      const sw = a1.getByRole("switch", { name: "Include comment words in email" });
      await sw.waitFor();
      at(a1);
      const before = await sw.getAttribute("aria-checked");
      const help = flat(await a1.getByText(/^Applies to everyone in the organization/).textContent().catch(() => ""));
      let mail;
      try {
        if (before === "true") {
          await sw.click();
        ...
        prep(`The fresh Administrator turned "Include comment words in email" off in Settings, Organization, Notifications for one mention email, then turned it back on.`);
        const since = new Date(Date.now() - 1000).toISOString();
        await comment(n, "contract", fx.n.contractId, `${tag} words while the switch is off`, "full_thread", [L1.id]);
        await settleQueue(2000);
        const mails = await mailsSince(L1.email, since);
        mail = mails.length ? { subject: mails[0].Subject, text: htmlText((await L.mailMessage(mails[0].ID)).HTML ?? "") } : null;
      } finally {
        await a1.reload();
      ...
      expect(mail && !mail.text.includes("words while the switch is off") && restored === true, JSON.stringify({ mail, restored }));
...
async function morningCheck() {
  ...
  for (const key of ["l1", "l2", "l3", "a1", "b1"]) {
    ...
    for (const m of await mailsSince(fx.fresh[key].email, since)) {
      ...
      const t = htmlText((await L.mailMessage(m.ID)).HTML ?? "");
      mails.push({ subject: m.Subject, keyDates: [k3, k5, k7].filter((k) => t.includes(k)).map((k) => k.split(" ").slice(-2).join(" ")), sections: ["Approvals", "Tasks", "Dates", "Obligations", "Knowledge", "Intake"].filter((h) => new RegExp(`\\b${h}\\b`).test(t)), portalLink: /\/portal\//.test((await L.mailMessage(m.ID)).HTML ?? ""), viewAll: /View all/.test(t) });
    }
    report[key] = {
      bell: items.map((x) => `${x.eventType}:${x.payload?.label ?? ""}${x.reminderOffsetDays ?? x.payload?.reminderOffsetDays ?? x.payload?.offsetDays ?? ""}`),
      keyDates: [k3, k5, k7].filter((k) => items.some((x) => JSON.stringify(x).includes(k))).map((k) => k.split(" ").slice(-2).join(" ")),
      mails,
    };
  }
  results.records.morningRound = report;
  const rounds = psql("select to_char(created_on, 'HH24:MI:SS') || ' ' || state from pgboss.job where name = 'notification.morning-round' order by created_on desc limit 3");
  await step(NT, "legal_team_member",
    ...
    async () => {
      const r = report;
      expect(r.l1.keyDates.join() === "three-day check,five-day check" || r.l1.keyDates.sort().join() === ["five-day check", "three-day check"].join(), `l1 ${q(r.l1)}`);
      expect(r.a1.keyDates.sort().join() === ["five-day check", "seven-day check"].join(), `a1 ${q(r.a1)}`);
      expect(r.l2.keyDates.length === 0 && r.l2.mails.length === 0, `l2 ${q(r.l2)}`);
      expect(r.l3.keyDates.length === 0 && !r.l3.mails.some((m) => m.sections.includes("Dates")), `l3 ${q(r.l3)}`);
  ...
  await step(NT, "business_user",
    ...
    async () => {
      const r = report.b1;
      expect(r.keyDates.sort().join() === ["five-day check", "seven-day check"].join() && r.mails.length === 1 && r.mails[0].portalLink && !r.mails.some((m) => /daily briefing/i.test(m.subject)), q(r));
  ...
  await step(NT, "administrator",
    ...
    async () => {
      ...
      expect(briefing && briefing.sections.includes("Dates") && briefing.sections.includes("Tasks") && href === "/", q({ r, href }));
...
async function viaHistory() {
  ...
  await step(CA, "legal_team_member",
    ...
    async () => {
      ...
      at(n);
      expect(entries.some((e) => e.startsWith(`${fx.fresh.l1.displayName}, via Claude, commented`)), q(entries));
...
try {
  ...
  results.identities = [
    { role: "administrator", account: "Daniel Okafor (seed) and DOC-030 conversations Admin (fresh)", entry: "password sign-in" },
    { role: "legal_team_member", account: "Nadia Haddad (seed) and DOC-030 conversations Legal accounts (fresh)", entry: "password sign-in" },
    { role: "business_user", account: "Amara Nwosu (seed) and DOC-030 conversations Business (fresh); Jonas Weber was not used because the lab refused his Requests (20 an hour, shared with other agents)", entry: "fresh magic link read from the work2 Mailpit, never stored" },
  ];
