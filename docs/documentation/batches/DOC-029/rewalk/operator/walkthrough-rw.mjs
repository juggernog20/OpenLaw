// DOC-029 independent re-walk of V-C45 (deployment-configuration.md), method container-operation,
// by the DOC-029 independent re-walk agent. Based on operator-install/walkthrough-r1.mjs. The install.md
// steps are replayed only to build the fixture installation; they are not credited to V-C44 here.
// The outbound-email checks are rewritten for the corrected paragraph (phases mail, mail-no-from,
// mail-both).
//
// Run one phase at a time from the documentation worktree root:
//   mise exec -- node docs/documentation/batches/DOC-029/rewalk/operator/walkthrough-rw.mjs <phase>
//
// Private state (the clone, its .env, fixture passwords, browser traces) lives outside the
// repository in WORK. The sanitized log is walkthrough-rw.json beside this script. Every attempt
...
const WORK = "/home/blairwentworth/.cache/doc029-rewalk-operator";
const PRIVATE = path.join(WORK, "private");
const CLONE = path.join(WORK, "openlaw");
const FIX = path.join(WORK, "fixtures");
const PROJECT = "doc029-rewalk-op";
const FX_PROJECT = "doc029-rewalk-op-fx";
const APP_PORT = 23340;
const PROXY_PORT = 23341;
const MAIL_UI = "http://127.0.0.1:23342";
const MINIO_HOST = "http://127.0.0.1:23343";
const AZURITE_HOST = "http://127.0.0.1:23344";
const SPARE_PORT = 23346;
const LOCAL = `http://127.0.0.1:${APP_PORT}`;
const ORIGIN = `https://openlaw-rw.localhost:${PROXY_PORT}`;
const LOG = path.join(here, "walkthrough-rw.json");
const REVIEWER = "DOC-029 independent re-walk agent";
...
const log = existsSync(LOG)
  ...
  : {
      batch: "DOC-029",
      group: "rewalk-operator",
      round: "re-walk",
      basedOn: "docs/documentation/batches/DOC-029/operator-install/walkthrough-r1.mjs",
      reviewer: REVIEWER,
      reviewerKind: "agent",
      appCommit: COMMIT,
...
async function openBrowser() {
  browser ??= await chromium.launch({
    headless: true,
    args: [`--host-resolver-rules=MAP openlaw-rw.localhost 127.0.0.1`],
  });
  return browser;
}
...
openlaw-rw.localhost:${PROXY_PORT} {
...
`,
      ...
      return `Fixture project ${FX_PROJECT} started: Mailpit as the SMTP relay stand-in (relay:1025 on the installation's backend network, UI on 127.0.0.1:23342), MinIO (127.0.0.1:23343), Azurite (127.0.0.1:23344), a separate postgres:16 service reachable only on the backend network as extdb, and caddy:2-alpine on the host network serving https://openlaw-rw.localhost:${PROXY_PORT} with "reverse_proxy 127.0.0.1:${APP_PORT}". Caddy issues the localhost certificate from its internal CA; the browser context accepts that CA, which does not test public certificate issuance.`;
    },
  );
  await step(
    "V-C44",
    ...
    async () => {
      ...
      await page.screenshot({ path: path.join(here, "rw-setup-through-proxy.png") });
      await context.close();
      return `GET ${ORIGIN} answered ${status} through Caddy, and the page shows the "Set up OpenLaw" heading. Screenshot rw-setup-through-proxy.png.`;
    },
  );
};
...
async function inviteThroughSettings(email, displayName, shotName) {
  const s = await signIn(ORIGIN, ADMIN.email, state.adminPassword);
  try {
    const page = s.page;
    ...
    const users = await json(await page.request.get(`${ORIGIN}/api/v1/users`));
...
phases["mail-both"] = async () => {
  await step(
    ...
    async () => {
      ...
      await api.dispose();
      const invitee = `doc029-rw-both-${Date.now()}@doc029-install.example`;
