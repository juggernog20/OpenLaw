...
Generation `created_contract_id` and `created_document_id` are nullable unique FKs with `ON DELETE SET NULL`; Contract `created_by_generation_id` is the nullable unique reverse FK. Document Version `generated_from_generation_id` is indexed and restricts deletion of its Generation. The source CHECK admits an Auto-Doc original only as Version 1, `draft_ours`, `generated`, with its Generation reference and no comparison operands. Deferred triggers lock the Generation and check both Contract links and the original Document's ownership. The remembered original Document can later cease to be primary without losing its provenance. Other generated Versions still require the existing redline operands; Filing extends this in M35/12.
