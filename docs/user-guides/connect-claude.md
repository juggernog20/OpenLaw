...
In OpenLaw, open **Settings → Personal → API keys → Connected Clients**. Business Users open **API keys → Connected Clients** in Portal Settings. Check the Client, Toolsets, Read or Write scope, granted date and last use. Select **Disconnect**, then confirm **Revoke OAuth grant** with **Revoke**. The Client's next call is refused. Connecting again needs new consent.
...
| MCP is off for this organization.                                          | Ask the Administrator to turn on Enable MCP.                                                                                                                                             |
| OAuth Clients are off for your account type.                               | Ask the Administrator to enable your account group's OAuth Clients.                                                                                                                      |
| This Client is not on the organization's Allowed Clients list.             | Ask the Administrator to check the Client identity.                                                                                                                                      |
| This Client is off in the organization's Allowed Clients list.             | Ask the Administrator whether this Client should be enabled.                                                                                                                             |
...
A refused page offers **Deny** alone. An expired or altered request offers no actions. An expired grant needs a fresh connection and consent.
