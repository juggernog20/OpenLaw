...
| E.5 | Signature elements chip          | V12   | done   | keep (conditional) — renders envelope status per CTR-013 when an envelope exists; hidden for manual hand-off contracts | Built in M15/2: the chip sits in the record's sub-bar after the status pill, per **DES-036** clause 11. |
...
| G.R5 | Last renewal   | V12   | done   | keep — last _confirmed_ renewal event from activity_log per CTR-006/007                                                               | Built in M16/4: a fact closing the Contract card's term group, read from the newest `contract.renewal_confirmed` entry. No roll yet prints the X.6 em dash. **DES-043** clause 17. |
...
| H.H2   | Title                   | Both     | done   | change — card becomes **"Approvals & signing"**: auto-derived rows from `contract_approvals` (CTR-012), envelope events (CTR-013), confirmed renewals (CTR-006). No manual event authoring | Took the two-part name in M15/2, when envelope rows joined it (DES-035 §3, **DES-036** §1). Renewal rows follow in M16. |
...
| ID   | Element                            | Where    | Status | Decision                                                                                                                                           | Notes                                                                                                                                                                                                        |
| ---- | ---------------------------------- | -------- | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| I.H1 | Section icon                       | V12      | done   | remove — per X.1                                                                                                                                   | Built in M16/2: the card has no leading icon, per X.1.                                                                                                                                                       |
| I.H2 | Title                              | Both     | done   | keep — title "Term" (renders the CTR-006 term timeline)                                                                                            | Built in M16/2 as **"Term timeline"** — DES-040 gave the Contract card the term fields, so "Term" would name two surfaces. **DES-041** clause 1.                                                             |
| I.H3 | Zoom switcher (Year/Quarter/Month) | Both     | done   | remove — single fit-to-term view in v1; zoom machinery unearned for a bar that spans one contract's life                                           | Built in M16/2: one fit-to-term scale, widened to hold every mark, per **DES-041** clause 11.                                                                                                                |
| I.X1 | Label placement: on-bar vs. gutter | Diverges | done   | keep gutter (V13) — survives narrow containers per DES-012/X.4                                                                                     | Built in M16/2: the gutter names each period and prints its dates, and is the card's readable half. **DES-041** clause 4.                                                                                    |
| I.B1 | Effective date marker              | Both     | done   | keep — `effective_date` per CTR-006                                                                                                                | Built in M16/2 as the first period's own start, printed in the gutter rather than as a marker of its own. **DES-041** clause 4.                                                                              |
| I.B2 | Term 1 bar                         | Both     | done   | keep — initial term: `effective_date → expiry_date` (as of first term) per CTR-006                                                                 | Built in M16/2: the initial term is what is left in front of the first implied boundary. **DES-041** clause 2.                                                                                               |
| I.B3 | Term 2 bar                         | Both     | done   | keep — one bar per confirmed renewal event (activity_log per CTR-006)                                                                              | Decision stands; still not built. M16/4 writes the log entry the bar reads, so the datum now exists — telling an implied period from a confirmed one on one plot is its own decision. **DES-043** clause 18. |
| I.B4 | Term 3 bar                         | Both     | done   | keep — bar count = confirmed renewals + 1; renewals routed to amendment/child/new contracts (CTR-007) render on those records, not extra bars here | Decision stands; still not built, with I.B3. **DES-043** clause 18.                                                                                                                                          |
| I.B5 | Last renewal marker                | V12      | done   | keep — same datum as G.R5                                                                                                                          | Decision stands; still not built on the plot, with I.B3. The datum landed in M16/4, and G.R5 draws it as a fact. **DES-043** clause 18.                                                                      |
| I.B6 | End of contract                    | Both     | done   | keep — `expiry_date` per CTR-006; absent for evergreen                                                                                             | Built in M16/2 as the last period's own end, printed in the gutter; an evergreen term draws an open end and "No end date" instead. **DES-041** clause 7.                                                     |
| I.B7 | Renewal cap                        | Both     | done   | remove — same call as G.R6 (no cap field)                                                                                                          | Built in M16/2: nothing draws one, and the sixty-roll render guard is not a cap. **DES-041** clause 9.                                                                                                       |
| I.B8 | Risk threshold marker              | Both     | done   | remove — undefined concept; the real threshold datum is the derived notice deadline, which gets its own marker per CTR-006                         | Built in M16/2: the notice deadline has the mark, per **DES-041** clause 5.                                                                                                                                  |
| I.X2 | Today line + pill                  | Both     | done   | keep — plus a **notice-deadline marker** (derived, CTR-006) joins the timeline                                                                     | Built in M16/2: the today line keeps its pill at the plot's foot and the deadline takes a pill at its head, so the two can never collide. **DES-041** clause 5.                                              |
| I.X3 | Legend (3 swatches)                | Both     | done   | keep — swatches: initial term / renewals / markers; final copy at build per DES-015                                                                | Built in M16/2: the key names only the fills the plot is using. **DES-041** clause 6.                                                                                                                        |
...
| X.2  | Status visualization (G.R2, H decision col, lifecycle pills)       | done   | change — pills everywhere; value→family mapping (stages, priority, risk, approvals, envelopes, request statuses) lands as a DES addendum at build     | Stages **DES-034**; approvals **DES-035** clause 7; envelopes **DES-036** clause 5. |
