...
export default async function documents(ctx) {
  ...
  const part = (name, as = name) => ({ name: as, mimeType: MIME[path.extname(name)] ?? "application/octet-stream", buffer: fs.readFileSync(path.join(FIX, name)) });
  results.records = [];
  ...
  const contractTypeId = (await admin.api("GET", "/contracts/options")).json.contractTypes.find((t) => t.displayName === "NDA").id;

  async function uploadApi(who, url, name, as) {
    const r = await who.api("POST", url, undefined, { multipart: { kind: "draft_ours", file: part(name, as) } });
    expectThat(r.status === 201, `fixture upload ${r.status} ${JSON.stringify(r.json).slice(0, 200)}`);
    return r.json.document;
  }
  async function addVersion(who, id, name, as) {
    const r = await who.api("POST", `/documents/${id}/versions`, undefined, { multipart: { kind: "draft_theirs", file: part(name, as) } });
  ...
  const getDoc = async (who, recordUrl, id) => (await recordDocs(who, recordUrl, true)).find((d) => d.id === id) ?? null;
  ...
  async function openDocuments(page, url) {
    ...
    if (await broken.isVisible().catch(() => false)) {
      results.reloads = [...(results.reloads ?? []), { url, at: new Date().toISOString(), text: (await page.locator("body").innerText()).replace(/\s+/g, " ").slice(0, 300) }];
  ...
  for (const flow of flows) {
    ...
    const main = await uploadApi(who, `${recUrl}/documents`, "doc029-draft-v1.docx", `archive-main-${tag}.docx`);
    for (const n of [2, 3]) await addVersion(who, main.id, "doc029-draft-v2.docx", `archive-main-${tag}-v${n}.docx`);
    let doc = await getDoc(who, recUrl, main.id);
    const v2 = doc.versions.find((v) => v.versionNumber === 2);
    expectThat((await who.api("POST", `/documents/${main.id}/executed-version`, { versionId: v2.id })).status < 300, "fixture pin");
    if (!(await getDoc(who, recUrl, main.id)).isPrimary) expectThat((await who.api("POST", `/documents/${main.id}/primary`, {})).status < 300, "fixture primary");
    const b1 = await uploadApi(who, `${recUrl}/documents`, "doc029-bulk-a.txt", `archive-bulk-1-${tag}.txt`);
    const b2 = await uploadApi(who, `${recUrl}/documents`, "doc029-bulk-b.txt", `archive-bulk-2-${tag}.txt`);
    const r1 = await uploadApi(who, `${recUrl}/documents`, "doc029-bulk-c.txt", `archive-repo-${tag}.txt`);
    flow.contract = contract;
    flow.recUrl = recUrl;
    flow.main = main;
    flow.b1 = b1;

    await step(role, who.name, `${recUrl}/documents`, "Archive a Document steps 1-3: open the Documents tab, the Document's Actions menu, Archive; check it leaves the list; turn on Show archived", "The Document leaves the normal list and reappears marked Archived with Show archived on; its 3 Versions, files, primary mark and executed designation stay.", async () => {
      ...
      const downloads = [];
      for (const v of d.versions) downloads.push((await who.api("GET", `/documents/${main.id}/versions/${v.id}/download`)).status);
      const search = await who.api("GET", `/documents?q=${encodeURIComponent(`archive-main-${tag}`)}`);
      const inRepo = JSON.stringify(search.json ?? {}).includes(main.id);
      expectThat(items.includes("Archive") && d.archivedAt && d.versions.length === 3 && d.isPrimary && d.versions.find((v) => v.isExecuted)?.versionNumber === 2 && downloads.every((s) => s === 200), `archived ${q({ items, a: d.archivedAt, n: d.versions.length, p: d.isPrimary, downloads })}`);
    ...
    await step(role, who.name, `${recUrl}/documents`, "Restore a Document steps 1-3 on the owning record: Show archived, the row's Actions menu, Restore", "The Document is live again with its 3 Versions, its primary mark and its executed designation.", async () => {
      ...
      expectThat(!d.archivedAt && d.isPrimary && d.versions.length === 3 && d.versions.find((v) => v.isExecuted)?.versionNumber === 2, `restored ${text}`);
    ...
    await step(role, who.name, `${recUrl}/documents`, "Archive several live Documents: select their checkboxes and Archive in the selection bar; then, with Show archived on, select both and Restore in the selection bar", "Both leave the list together and return together.", async () => {
      ...
      const live = [await waitDoc(who, recUrl, b1.id, (x) => !x.archivedAt), await waitDoc(who, recUrl, b2.id, (x) => !x.archivedAt)];
      await showArchived(page, false);
      expectThat(live.every((d) => !d.archivedAt), "bulk restore");
    ...
    await step(role, who.name, "/documents", "Restore a Document step 1 and 2 in the Documents repository: Filter, Show archived, then the repository's Restore action", "Without Show archived the archived Document is not listed; with it, the row is marked Archived and Restore makes it live.", async () => {
      expectThat((await who.api("POST", `/documents/${r1.id}/archive`, {})).status === 200, "fixture archive");
      ...
      await page.waitForLoadState("networkidle").catch(() => {});
      await sleep(1000);
      const rowsBefore = (await page.getByRole("row").allInnerTexts()).map((t) => t.replace(/\s+/g, " "));
      ...
      expectThat(!before && liveListed && /Archived/.test(rowText) && !d.archivedAt, q({ before, liveListed, rowText, archived: d.archivedAt }));
    ...
    await step(role, who.name, `${recUrl}/documents`, "If restoration is refused because the owning record is archived: archive the Contract (setup), try to restore its archived Document, restore the Contract first, then restore the Document", "While the Contract is archived there is no Show archived or change control and restoring is refused; after restoring the Contract the Document restores.", async () => {
      expectThat((await who.api("POST", `/documents/${b1.id}/archive`, {})).status === 200, "fixture doc archive");
      expectThat((await who.api("POST", `${recUrl}/archive`, {})).status === 200, "fixture record archive");
      await openDocuments(page, `${recUrl}/documents`);
      ...
      for (const trigger of await docsSection(page).getByRole("button", { name: /^Actions for / }).all()) {
        await trigger.click();
        seen.push(...(await page.getByRole("menu").getByRole("menuitem").allInnerTexts()).map((t) => t.trim()));
        await page.keyboard.press("Escape");
      }
      const changing = seen.filter((t) => /Archive|Restore|Delete|Add version|Move to folder/.test(t)).length;
      const refused = await who.api("POST", `/documents/${b1.id}/restore`, {});
      expectThat((await who.api("POST", `${recUrl}/restore`, {})).status === 200, "fixture record restore");
      await openDocuments(page, `${recUrl}/documents`);
      await showArchived(page, true);
      await chooseMenu(page, b1.title, "Restore");
      const d = await waitDoc(who, recUrl, b1.id, (x) => !x.archivedAt);
      await showArchived(page, false);
      expectThat(sw === 0 && upload === 0 && changing === 0 && refused.status === 409 && !d.archivedAt, q({ sw, upload, changing, refused: refused.status }));
    ...
    await step(role, who.name, `${recUrl}/documents`, "Restore a Document: a second session restores the same archived Document first; this page selects Restore on its stale row, reads the message and reloads", "A stale-state message shows; after reload the Document is live.", async () => {
      expectThat((await who.api("POST", `/documents/${b2.id}/archive`, {})).status === 200, "fixture archive");
      await openDocuments(page, `${recUrl}/documents`);
      await showArchived(page, true);
      ...
      const alert = docsSection(page).locator('[aria-live="polite"], [role="alert"]').filter({ hasText: /archived|restore|could not|already/i });
    ...
    await step(role, who.name, `/knowledge/<id>`, "Archive a Document step 1 in a Knowledge Item's Documents section: Archive from the row's Actions, Show archived, Restore", "The Knowledge Documents section archives and restores the same way.", async () => {
      const kType = (await who.api("GET", "/knowledge?limit=1")).json.knowledgeItems?.[0]?.knowledgeTypeId;
      const k = await who.api("POST", "/knowledge", { title: `DOC-030 contracts-c archive Knowledge ${label} ${STAMP}`, knowledgeTypeId: kType });
      ...
      await uploadApi(who, `/knowledge/${kid}/documents`, "doc029-services-text.pdf", `knowledge-a-${tag}.pdf`);
      const d2 = await uploadApi(who, `/knowledge/${kid}/documents`, "doc029-bulk-a.txt", `knowledge-b-${tag}.txt`);
    ...
    if (role === "legal_team_member") {
      await step(role, who.name, `${recUrl}/documents`, "Before you start and Permanently delete: a Legal Team Member looks for Delete in the row menu and the selection bar, and tries a direct deletion", "No Delete in the row menu or selection bar; direct deletion is refused and the Document stays.", async () => {
        await openDocuments(page, `${recUrl}/documents`);
        const items = await menuItems(page, main.title);
        await page.getByRole("checkbox", { name: `Select ${b1.title}` }).click();
        const barDelete = await bar(page, 1).getByRole("button", { name: "Delete", exact: true }).count();
        const barButtons = (await bar(page, 1).getByRole("button").allInnerTexts()).map((t) => t.trim());
        await bar(page, 1).getByRole("button", { name: "Clear selection" }).click();
        const del = await who.api("DELETE", `/documents/${main.id}`, { confirmTitle: main.title });
        const d = await getDoc(who, recUrl, main.id);
        expectThat(!items.includes("Delete") && barDelete === 0 && del.status === 403 && d?.versions.length === 3, q({ items, barDelete, del: del.status }));
      ...
      await step("legal_team_member", `${PEOPLE.mei.name} (Business User on the Contract team)`, `/portal/contracts/${contract.number}`, "Before you start: a Business User on the Contract team reads its Documents in the Portal and tries archive, restore and delete", "No archive, restore, delete or Show archived controls; all three requests are refused.", async () => {
        const team = await who.api("POST", `/contracts/${contract.number}/team`, { userId: idOf(PEOPLE.mei.email) });
        ...
        try {
          ...
          await mei.page.getByRole("heading", { name: "Documents" }).first().waitFor({ timeout: 30000 });
          ...
          const del = await mei.api("DELETE", `/documents/${main.id}`, { confirmTitle: main.title });
          const stillArchived = (await getDoc(who, recUrl, b2.id))?.archivedAt;
          expectThat(!/\bArchive\b|\bRestore\b|\bDelete\b|Show archived/.test(text) && archive.status >= 400 && restore.status >= 400 && del.status >= 400 && stillArchived && listsLive, q({ archive: archive.status, restore: restore.status, del: del.status, listsLive }));
    ...
    await step(role, who.name, `${recUrl}/documents`, "Permanently delete steps 1-4: Actions, Delete; read Delete this document?; try Delete with the field empty and with the Document name; Cancel", "The dialog names the Document and its 3 Versions; Delete stays disabled until the word delete is typed; Cancel keeps the Document.", async () => {
      await openDocuments(page, `${recUrl}/documents`);
      ...
      const d = await getDoc(who, recUrl, main.id);
      expectThat(text.includes(main.title) && /3 versions/.test(text) && disabledEmpty && disabledWrong && d?.versions.length === 3, `text ${text} disabled ${disabledEmpty}/${disabledWrong}`);
    ...
    await step(role, who.name, `${recUrl}/documents`, "If deletion fails: someone renames the Document while the Delete dialog is open; type delete and select Delete", "OpenLaw refuses the deletion with a message and the Document stays under its new name.", async () => {
    ...
    await step(role, who.name, `${recUrl}/documents`, "Permanently delete steps 3-4 on an archived Document: Show archived, archive the primary Document first, Actions > Delete, type delete, Delete; check the row, Version downloads, primary reference and activity", "The row is gone (archived rows included); every Version download fails; no other Document becomes primary; the activity record of the deletion remains.", async () => {
      await openDocuments(page, `${recUrl}/documents`);
      await chooseMenu(page, main.title, "Archive");
      await rowOf(page, main.title).waitFor({ state: "hidden", timeout: 20000 });
      await showArchived(page, true);
      ...
      const downloads = [];
      for (const v of before.versions) downloads.push((await who.api("GET", `/documents/${main.id}/versions/${v.id}/download`)).status);
      const docs = await recordDocs(who, recUrl, true);
      const activity = await who.api("GET", `/activity?entityType=contract&entityId=${contractId}`);
      ...
      expectThat(!docs.some((d) => d.id === main.id) && downloads.every((s) => s === 404) && !docs.some((d) => d.isPrimary) && mention, q({ downloads, primary: docs.filter((d) => d.isPrimary).length, activity: activity.status, mention }));
    ...
    await step(role, who.name, `${recUrl}/documents`, "Permanently delete several Documents: select their checkboxes, Delete in the selection bar, type delete", "The bulk dialog asks for the word delete and removes both.", async () => {
      const c1 = await uploadApi(who, `${recUrl}/documents`, "doc029-bulk-c.txt", `archive-del-1-${tag}.txt`);
      const c2 = await uploadApi(who, `${recUrl}/documents`, "doc029-bulk-d.txt", `archive-del-2-${tag}.txt`);
      await openDocuments(page, `${recUrl}/documents`);
      ...
      await dialog.waitFor({ state: "hidden", timeout: 20000 });
      const docs = await recordDocs(who, recUrl, true);
      expectThat(disabled && !docs.some((d) => d.id === c1.id || d.id === c2.id), `disabled ${disabled}`);
    ...
    await step(role, who.name, `${recUrl}/documents`, "You cannot delete just one Version: open an earlier Version's actions and the Version rows' Type column", "No Version-level Delete; the Version rows carry a Type control for correcting a Version's Document type; a single-Version delete request is refused.", async () => {
      const d = await uploadApi(who, `${recUrl}/documents`, "doc029-draft-v1.docx", `archive-keep-${tag}.docx`);
      ...
      await page.keyboard.press("Escape");
      const typeSelects = await page.getByLabel(new RegExp(`^Type of version \\d+ of ${d.title.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}$`)).count();
      ...
      expectThat(!items.some((i) => /Delete/.test(i)) && del.status >= 400 && after.versions.length === 2 && typeSelects >= 1 && header >= 1, q({ items, del: del.status, typeSelects, header }));
  ...
  await step("administrator", PEOPLE.daniel.name, "docs/user-guides/archive-and-delete-documents.md", "Guide text check: the labels walked are in the reviewed content", "The guide names Actions, Archive, Show archived, Archived, Restore, Filter, Delete, Delete this document? and the Type column.", async () => {
    const text = articleText(ARTICLE);
    for (const label of ["**Actions**", "**Archive**", "**Show archived**", "**Archived**", "**Restore**", "**Filter**", "**Delete**", "**Delete this document?**", "**Type**", '**Type "delete" to confirm**'])
