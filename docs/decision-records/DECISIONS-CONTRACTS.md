...
- **Consequences** — `documents` schema (Documents grill) must support: contract primary-doc designation, version ordering, version kind, generated-artifact provenance. _Settled in M11/4: the primary-doc designation is **`contracts.primary_document_id`**, an FK on the contract rather than a flag on each document — one column is the exactly-one rule stated as a shape, where a flag needs a partial unique index to say the same thing. The first document uploaded takes it; there is no clear, so it moves or it stays._ Grill-plan F.3 unblocked (renders CTR-008 fields); K.H3/K.H4 stay DOC-gated but now have the contract-side contract to satisfy.
