...
export default async function templates(ctx) {
  ...
  // ------------------------------------------------------------ fixture setup (API)
  const fieldIds = {};
  for (const [key, displayName] of [["fOpt", T.fOpt], ["fReq", T.fReq], ["fRec", T.fRec]]) {
    const f = must(await admin.api("POST", "/fields", { displayName, moduleScope: "matter", fieldType: "text", fieldTag: "business" }), displayName).field;
    fieldIds[key] = f;
  }
  const type = must(await admin.api("POST", "/matter-types", { displayName: T.type }), T.type).matterType;
  const current = must(await admin.api("GET", `/matter-types/${type.id}/form`), "form").form;
  const pins = current.slice(0, 2);
  const builtin = Object.fromEntries(current.filter((n) => n.kind === "row" && n.id === n.rowRef).map((n) => [n.rowRef, n]));
  const fieldRow = (f, { required = false, intake = false } = {}) => ({ kind: "row", id: f.id, rowRef: f.slug, fieldType: "text", onIntakeForm: intake, isRequired: required, visibleOnPortal: true });
  const formRows = (withRecord = true) => [
    ...
    ...["department", "region", "needed_by"].map((k) => ({ ...builtin[k], onIntakeForm: false, isRequired: false })),
  ...
  const rowFlags = savedForm.filter((n) => n.kind === "row").map((n) => ({ rowRef: n.rowRef, onIntakeForm: n.onIntakeForm, isRequired: n.isRequired }));
  const rt = must(await admin.api("POST", "/request-types", { displayName: T.requestType }), T.requestType).requestType;
  must(await admin.api("PATCH", `/request-types/${rt.id}`, { targetModule: "matter", targetTypeId: type.id }), "request type target");
  ...
  results.fixtures.fields = Object.fromEntries(Object.entries(fieldIds).map(([k, f]) => [k, { slug: f.slug, displayName: f.displayName }]));
  ...
  const requests = {};
  {
    ...
    const departmentId = onboarding.departmentId ?? must(await admin.api("GET", "/departments"), "departments").departments[0].id;
    for (const role of ["administrator", "legal_team_member"]) {
      ...
      requests[role] = { number: created.number, title, createdAt: created.createdAt ?? new Date().toISOString() };
    }
  ...
  async function openTemplates(page) {
    ...
    await page.getByRole("navigation", { name: "Matters panes" }).getByRole("link", { name: "Templates" }).click();
  ...
  const fieldBox = (scope, name) => scope.getByRole("textbox", { name: new RegExp(`^${name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}`) });
  ...
  async function readTemplate(page) {
    ...
    for (let i = 1; i <= nTasks; i++)
      tasks.push([
        ...
        await page.getByRole("combobox", { name: `Task ${i} role` }).locator("option:checked").innerText(),
      ]);
    const dates = [];
    ...
    return {
      ...
      priority: await page.getByRole("combobox", { name: "Priority" }).locator("option:checked").innerText(),
      risk: await page.getByRole("combobox", { name: "Risk" }).locator("option:checked").innerText(),
  ...
  const optionsOf = (loc) => loc.evaluate((el) => Array.from(el.options).map((o) => o.textContent.trim()));
  ...
  async function pageReadsOnce(page, number) {
    ...
    const titleValue = await page.getByRole("textbox", { name: /^Title\b/ }).first().inputValue().catch(() => "");
  ...
  const sortLike = (actual, expected) => expected.map((e) => actual.find((a) => a[0] === e[0]) ?? null);
  const S = {};

  // ------------------------------------------------------------ steps
  await step("administrator", PEOPLE.daniel.name, "API (fixture setup)", "Before you start: a live Matter type with the Fields the template should fill (fixture setup through the API)", "The DOC-030 Matter type is live; Priority and Risk are On intake form, so both are creation Rows; the optional note is On intake form, the required note is Required for creation, and the record note is neither.", async () => {
  ...
  await step("administrator", PEOPLE.daniel.name, "/settings/matters/templates", "Create and edit a template, steps 1-7: profile menu > Settings > Matters > Templates; Matter type; Add template (Name, Description); Edit; Matter defaults; Custom field defaults; Tasks (Add task, offsets, Matter Manager / Unassigned, reorder); Key dates (Add key date); Save template; reopen and confirm", "The template saves, and after reload all four areas hold the entered values in the chosen order.", async () => {
    ...
    await A.getByRole("button", { name: new RegExp(`^Reorder ${T.task3.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}, position`) }).focus();
    ...
    expectThat(t.priority === "Low" && t.risk === "Critical" && t.prefix.trim() === T.prefix, `defaults ${q(t)}`);
    expectThat(t.fOpt === "Template optional default" && t.fReq === "Template required default" && t.fRec === "Template record default", `field defaults ${q(t)}`);
    ...
    S.template = must(await admin.api("GET", "/matter-templates?includeArchived=true"), "templates").matterTemplates.find((x) => x.name === T.name);
  ...
  await step("administrator", PEOPLE.daniel.name, "template editor", "Offsets: enter Task 1 due offset 3651 and save; then clear Key date 1's offset and save (Recover a failed save: correct invalid offsets)", "Both saves are refused with an error naming the problem; after reload the template is unchanged.", async () => {
  ...
  await step("administrator", PEOPLE.daniel.name, "/matters (Create matter dialog), /matters/<n>", "Test both creation paths, direct creation: choose the type, then the template in Matter template; read the hint, the title, the creation Rows, and Matter Manager; clear the optional Field; Create; check Overview, Tasks, Task assignments and Key dates", "The dialog says the template adds 3 Tasks and 2 Key dates, suggests the prefix title, shows Priority Low and Risk Critical and the Field defaults in the creation Rows; the record-only Field is not shown but its default goes on the Matter; the cleared optional Field stays empty; dates count from the UTC creation date; Manager Tasks go to the Matter Manager; the Task without an offset has no due date.", async () => {
    ...
    expectThat(suggested.title.trim() === T.prefix && suggested.priority === "Low" && suggested.risk === "Critical" && suggested.manager === PEOPLE.daniel.name, `suggested ${q(suggested)}`);
    expectThat(suggested.fOpt === "Template optional default" && suggested.fReq === "Template required default" && suggested.fRecShown === 0, `fields ${q(suggested)}`);
    ...
    expectThat(m.priority === "low" && m.risk === "critical", `priority/risk ${m.priority}/${m.risk}`);
    expectThat((cf[fieldIds.fOpt.slug] ?? null) === null || cf[fieldIds.fOpt.slug] === "", `optional ${q(cf)}`);
    expectThat(cf[fieldIds.fReq.slug] === "Template required default" && cf[fieldIds.fRec.slug] === "Template record default", `fields ${q(cf)}`);
    expectThat(q(sortLike(snap.tasks, expectedTasks(anchor, PEOPLE.daniel.name))) === q(expectedTasks(anchor, PEOPLE.daniel.name)), `tasks ${q(snap.tasks)}`);
    expectThat(q(sortLike(snap.dates, expectedDates(anchor))) === q(expectedDates(anchor)), `dates ${q(snap.dates)}`);
    expectThat(reads.tasks.includes(T.task1) && reads.dates.includes(T.date1) && reads.overview.includes(title), "pages do not show the template content");
  ...
  await step("administrator", PEOPLE.daniel.name, "/matters (Create matter dialog)", "Direct creation with explicit values: clear the Required for creation Field and select Create; then fill it, set Priority to Medium and Matter Manager to Unassigned, and Create", "Creation is refused while the required Row is empty; the explicit Priority wins over the template default; Matter Manager Tasks stay unassigned.", async () => {
    const d = await openCreate(A);
    await d.getByLabel("Matter type").selectOption({ label: T.type });
    await d.getByLabel("Matter template").selectOption({ label: T.name });
    await sleep(500);
    ...
    expectThat(m.priority === "medium" && m.risk === "critical" && m.manager === null || (m.priority === "medium" && !m.manager?.id), `matter ${q({ p: m.priority, r: m.risk, mgr: m.manager })}`);
    expectThat(snap.tasks.length === 3 && snap.tasks.every((t) => t[2] === null), `tasks ${q(snap.tasks)}`);
  ...
  for (const [role, who] of [["administrator", admin], ["legal_team_member", nadia]]) {
    const R = requests[role];
    await step(role, who.name, `/inbox/${R.number} (Convert dialog)`, `Test both creation paths, Request conversion: open R-${R.number}, Triage > Convert to matter, confirm the configured Matter type, choose the template in Matter template, review the creation Rows${role === "administrator" ? ", fill Risk Medium" : ", leave Risk empty"}, and select Convert to matter`, "The dialog shows the target Form's creation Rows; the optional Field starts from the Request's answer, the required Field from the template default; Priority starts from the Request's Urgency (High), not the template's Low; Risk starts empty; no carried or not-carried lists; no template title prefix.", async () => {
      const page = who.page;
      ...
      const before = { priority: await checked(d.getByLabel(/^Priority\b/)), fReq: await fieldBox(d, T.fReq).inputValue() };
      await d.getByLabel("Matter template").selectOption({ label: T.name });
      await sleep(800);
      ...
      expectThat(reviewed.title === R.title && reviewed.priority === "High" && reviewed.risk === "Not set", `reviewed ${q(reviewed)}`);
      expectThat(reviewed.fOpt === `Requester answer ${role}` && reviewed.fReq === "Template required default" && reviewed.fRecShown === 0, `fields ${q(reviewed)}`);
      expectThat(!lists, "carry lists shown");
      if (role === "administrator") await d.getByLabel(/^Risk\b/).selectOption({ label: "Medium" });
      await page.screenshot({ path: path.join(here, `${role}-convert-matter-template.png`), fullPage: false });
      ...
      expectThat(detail.request.convertedRecord?.module === "matter", `outcome ${q(detail.request.convertedRecord)}`);
      ...
      expectThat(role === "administrator" ? m.risk === "medium" : m.risk === null, `risk ${m.risk}`);
      expectThat(m.manager?.displayName === who.name, `manager ${q(m.manager)}`);
      expectThat(cf[fieldIds.fOpt.slug] === `Requester answer ${role}` && cf[fieldIds.fReq.slug] === "Template required default", `fields ${q(cf)}`);
      expectThat(q(sortLike(snap.tasks, expectedTasks(anchor, who.name))) === q(expectedTasks(anchor, who.name)), `tasks ${q(snap.tasks)}`);
      expectThat(q(sortLike(snap.dates, expectedDates(anchor))) === q(expectedDates(anchor)), `dates ${q(snap.dates)}`);
      expectThat(reads.tasks.includes(T.task1) && reads.dates.includes(T.date1), "pages do not show the template content");
  ...
  await step("administrator", PEOPLE.daniel.name, "template editor, /matters", "Change or archive a template: rename Task 1 and change its offset to 3, save; compare the earlier Matter and a new Matter", "The earlier Matter keeps its Tasks; a new Matter uses the changed Task.", async () => {
    await A.goto(S.templateUrl);
    await A.getByRole("heading", { name: "Matter defaults" }).waitFor();
    ...
    expectThat(q(directNow) === q(S.directSnapshot.tasks), `existing Matter changed: ${q(directNow)}`);
    const d = await openCreate(A);
    await d.getByLabel("Matter type").selectOption({ label: T.type });
    await d.getByLabel("Matter template").selectOption({ label: T.name });
    await sleep(500);
    await d.getByLabel(/^Title\b/).fill(`${T.prefix} ${G} after change matter ${STAMP}`);
    const posted = A.waitForRequest((r) => r.method() === "POST" && /\/api\/v1\/matters$/.test(r.url()));
    await d.getByRole("button", { name: "Create" }).click();
    ...
    expectThat(t1 && t1[1] === utcDatePlus(snap.matter.createdAt, 3), `new tasks ${q(snap.tasks)}`);
  ...
  await step("administrator", PEOPLE.daniel.name, "/settings/matters/templates, /matters", "Archive the template (Archive control, Archive template), check creation, then Show archived > Restore", "The archived template is not offered and a creation that names it is refused with a clear message; Restore brings its saved definition back.", async () => {
    await openTemplates(A);
    ...
    const refused = await admin.api("POST", "/matters", { ...S.createBody, title: `${G} refused archived template ${STAMP}` });
    await openTemplates(A);
    await A.getByRole("switch", { name: "Show archived" }).click();
    await A.getByRole("button", { name: `Restore ${T.name}`, exact: true }).click();
    await A.getByRole("button", { name: `Archive ${T.name}`, exact: true }).waitFor({ timeout: 15000 });
    await A.getByRole("link", { name: `Edit ${T.name}` }).click();
    await A.getByRole("heading", { name: "Matter defaults" }).waitFor();
    await sleep(800);
    ...
    await d.getByRole("button", { name: "Cancel" }).click();
    expectThat(!archivedOptions.includes(T.name) && refused.status >= 400 && restoredOptions.includes(T.name), q({ archivedOptions, refused: refused.status, restoredOptions }));
    expectThat(restored.tasks.length === 3 && restored.dates.length === 2 && restored.fReq === "Template required default", `restored ${q(restored)}`);
  ...
  await step("administrator", PEOPLE.daniel.name, "/matters (Create matter dialog)", "A template whose Matter type is unavailable: a second, unused DOC-030 Matter type with its own template is archived (setup through the API); open Create matter and try that template; then restore the type", "The archived type and its template are not offered, and a creation naming them is refused.", async () => {
    const t2 = must(await admin.api("POST", "/matter-types", { displayName: `${T.type} unused` }), "second type").matterType;
    const tpl2 = must(await admin.api("POST", "/matter-templates", { matterTypeId: t2.id, name: `${T.name} unused`, defaultPriority: "low" }), "second template").matterTemplate;
    let d = await openCreate(A);
    ...
    try {
      ...
      await d.getByRole("button", { name: "Cancel" }).click();
      const refused = await admin.api("POST", "/matters", { title: `${G} refused archived type ${STAMP}`, matterTypeId: t2.id, templateId: tpl2.id });
      expectThat(before.includes(`${T.type} unused`) && tplBefore.includes(`${T.name} unused`), "second type or template not offered while live");
      expectThat(!types.includes(`${T.type} unused`) && refused.status >= 400, q({ offered: types.includes(`${T.type} unused`), refused: refused.status }));
  ...
  await step("administrator", PEOPLE.daniel.name, "template editor", "A Field detached from the template's type: remove the record-only Field's Row from the type Form (setup through the API), open the template editor, then attach it again", "The editor names the Field and its retained saved value; after reattaching, the default is an editable control again.", async () => {
    must(await admin.api("PUT", `/matter-types/${type.id}/form`, { form: formRows(false) }), "detach");
    await A.goto(S.templateUrl);
    await A.getByRole("heading", { name: "Custom field defaults" }).waitFor();
    await sleep(1000);
    const note = (await A.getByText(/is no longer attached to this Matter type/).textContent()).trim();
    must(await admin.api("PUT", `/matter-types/${type.id}/form`, { form: formRows(true) }), "reattach");
    await A.reload();
    await A.getByRole("heading", { name: "Custom field defaults" }).waitFor();
    await sleep(1000);
    const value = await fieldBox(A, T.fRec).inputValue();
    expectThat(note.includes(T.fRec) && note.includes("Template record default") && value === "Template record default", `note ${note}, value ${value}`);
    return `After the Row was removed, the editor read ${q(note)}. After the Row was added again, the default control shows ${q(value)}.`;
  });

  await step("administrator", PEOPLE.daniel.name, "docs/user-guides/matter-templates.md", "Guide text check: the steps walked are the ones in the reviewed content", "The guide names Matter template, On intake form, Required for creation, Convert to matter, Archive template, Show archived and Restore.", async () => {
    const text = articleText(ARTICLE);
    for (const label of ["**Matter template**", "**On intake form**", "**Required for creation**", "**Convert to matter**", "**Archive template**", "**Show archived**", "**Restore**", "**Save template**"])
      expectThat(text.includes(label), `guide lacks ${label}`);
    return "All eight labels are in the reviewed guide text.";
  });

  results.records = { direct: S.direct, converted: { administrator: S.converted_administrator, legal_team_member: S.converted_legal_team_member } };
