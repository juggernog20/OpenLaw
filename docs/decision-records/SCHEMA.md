...
| `answer_style`                 | text (enum) | not null, default `sentence`; `few_words` \| `sentence` \| `full_clause`, checked by the database; organization Answer style for Contract text Fields, migration `0156` (**CTR-008**) |
...
- `entity_holdings` — (`owner_entity_id`, `owned_entity_id`, `ownership_percent`, `source` (`manual|register`), timestamps), compound PK on the pair; row CHECKs prohibit self-ownership and hold percentage to 0–100. Longer cycles remain application-enforced; soft ≤100% aggregate validation per owned Entity backs the v1 org chart (ENT-003).
- `entity_share_classes` — `entity_id`, `name` (1–100, unique per Entity among live rows, case-insensitive), `authorized` bigint nullable, `par_value` bigint minor units nullable with `par_value_currency`, `votes_per_share` numeric(10,4) default 1, `rights` text nullable, `position`, `archived_at`, timestamps (ENT-011).
- `entity_shareholders` — per issuer: `entity_id`, `kind` (`entity|individual`), `holder_entity_id` FK (required for `entity`, unique per issuer), `name` (required for `individual`), timestamps. A Holder row exists because an entry names it; balances are never stored (ENT-011).
- `entity_share_entries` — `entity_id`, `entry_no` (per Entity, never reused), `kind` (`allotment|transfer|buyback|cancellation|conversion`), `effective_on`, `share_class_id`, `to_share_class_id` (conversion only), `quantity` > 0, `from_holder_id`, `to_holder_id`, `price_per_share` minor units nullable with `price_currency`, `consideration`, `distinctive_numbers`, `resolution_ref`, `note`, `recorded_by`, timestamps. Per-kind CHECKs pin which of from and to may be null (ENT-011).
...
| `reminder_recipient_ids`   | jsonb       | not null, default `[]`; deduplicated person IDs from the current record owners and team, including Business Users; empty uses both owners and all members, nonempty selects a subset revalidated at delivery (**NOT-004**, #807, #900) |
| `created_at`, `updated_at` | timestamptz |                                                                                                                                                                                                                                        |
...
- `owning_department` and `region`, nullable text; built-in Contract classification shown in Overview for every Contract Type (**CTR-025**).
...
| `created_at`, `updated_at` | timestamptz |                                                                                                                                                                                                                   |
| `archived_at`              | timestamptz | soft delete                                                                                                                                                                                                       |
| `search_vector`            | tsvector    | stored generated English FTS: title and R-number weight A, description weight B, fixed status weight C; Type and Requester names remain query-time joins (**DOC-009**, M25/2)                                     |
...
> **DD-028, built in M39:** Contract, Matter and Entity types own Forms. Record-type Field joins carry Row switches and Branch membership. Contract and Matter types also have built-in Rows and one Default type each. Migration 0159 re-keys stored Request answers and removes the ten shadow Fields, `fields.built_in_key`, `fields.field_tag`, `request_type_fields`, and `request_types.form_field_order`.
...
Unread tracking (CMT-004, confirmed by CMT-009): `comment_last_read` (`user_id` FK → `users.id` with no delete action, `entity_type`, `entity_id`, `read_at`, compound PK on the first three). One watermark per reader per record — where that person had read to, not a receipt per comment. The badge counts comments on the record that pass the viewer's tier predicate, are not the viewer's own, are neither soft-deleted nor redacted, and were created after `read_at`. Hidden-tier counts never leak, because the count is taken over the same filtered set the thread is read at. **No row means everything visible is unread**, not zero: a reader who has never opened the panel has read none of it. Opening the panel writes the row, and only when the thread was actually delivered. DES-079 adds nullable `full_thread_read_at` and makes `read_at` nullable. A Portal read advances only the Full Thread watermark. Full Thread counts use the later of both markers; private tiers use `read_at` alone. Conversion carries both markers onto the resulting record.
...
`auto_doc_assignment_rules`: `id`; `auto_doc_id` FK with cascade delete; nonnegative `display_order` with an ordered lookup index; `field_slug`; checked `operator` (`equals | is_one_of | is_set | is_not`); nullable `value` jsonb; `legal_owner_id` FK → `users.id`. Settings, not form definition: edited in place under the Auto-Doc row lock, audited. `auto_docs.default_legal_owner_id` is nullable and clears on user deletion. New Generation Contract snapshots include the resolved `legalOwnerId` (nullable); older snapshots without the property read as unassigned.
...
M35/7 builds the pair, person, answers, state, Word reference, failure, and timestamps. A ready Generation requires a Word reference; a failed Generation requires a JSON object with nonempty string `code` and `detail`. M35/8 adds the PDF and email columns. M35/9 adds the Contract columns below. Both version references are indexed for ownership checks and deletion checks.

M35/8 adds Generation snapshots `formats`, `cover_note`, and `display_values`, plus a positive `attempt` counter, `pdf_file_ref`, `email_state`, `email_sent_at`, and `email_failure`. `display_values` keeps the Entity names printed by the saved answers. Ready now requires a Word reference and, for `pdf` or `both`, a PDF reference. Email state is checked against `not_requested | pending | sent | failed | unconfigured`. Only `sent` has a sent time. Only `failed` and `unconfigured` have an email failure, a JSON object with nonempty string `code` and `detail`. Sent and unconfigured require a ready Generation. Existing rows default to Word-only and not requested; new submissions explicitly copy formats and request email. The state/email-state index supports recovery sweeps. Retry keeps the snapshots and pair, increments the attempt, and clears the previous output and failure references.

M35/9 adds nullable `title_pattern` and `fixed_entity_id` settings. Form map metadata includes optional `valueCurrency` and `valueCadence` for the built-in Value destination. The immutable Generation `contract_snapshot` holds the target Type, resolved title, Entity, Business Owner, Owning department, region, Counterparty name, catalog values, Value, dates, and term. Untargeted Generations have no Contract snapshot.

Generation `created_contract_id` and `created_document_id` are nullable unique FKs with `ON DELETE SET NULL`; Contract `created_by_generation_id` is the nullable indexed reverse FK; M35/12 permits multiple Contracts from distinct Filings. Document Version `generated_from_generation_id` is indexed and restricts deletion of the origin it names (see the M35 erasure note below, which moved both provenance references off `auto_doc_generations`). The source CHECK admits an Auto-Doc original only as Version 1, `draft_ours`, `generated`, with its Generation reference and no comparison operands. Deferred triggers lock the Generation and check both Contract links and the original Document's ownership. The remembered original Document can later cease to be primary without losing its provenance. A Filing extends the same generated Version shape to its separately recorded Document; generated redlines retain their existing comparison operands.
...
`auto_doc_filings`: `id`; `generation_id` FK → `auto_doc_generations`; exactly one `matter_id` or `contract_id`; nullable unique `document_id` FK → `documents` with `ON DELETE SET NULL`, preserving history after erasure; checked `format` (`docx | pdf`); `created_contract` boolean; `filed_by`; `created_at`. A partial unique index permits one birth Filing per created Contract. Many Contracts may cite the same Generation through separate Filings; the Generation's unique automatic Contract pointer remains separate. Deferred provenance checks require every generated Version to belong to the original automatic Document or a recorded Filing on its named owner. A Filing Document's Version 1 must cite that Generation.

**Built in M35/12:** `auto_doc_generations.requested_filing` holds an optional accepted destination, allowed format, and stable Filing id; `filing_failure` holds an actionable failure. These fields let the delivery worker complete a PDF Filing after the generating HTTP request ends. Successful Filing clears both within the Document transaction. Source Form labels and Entity display values are projected with Generation answers for Generate again.
...
**Built in M35 (2026-09-14, [#873](https://github.com/juggernog20/OpenLaw/issues/873)), ADO-012 erasure:** `auto_doc_generation_origins` holds one column, `id`, and nothing else. It is the provenance tombstone. `auto_doc_generations.id` references it, and migration 0129 backfills one row per existing Generation, repoints `contracts.created_by_generation_id` (`ON DELETE SET NULL`) and `document_versions.generated_from_generation_id` (no action) at it, and drops the two restrictive foreign keys that named `auto_doc_generations` directly. Administrator erasure can then delete every Generation while a created Contract and a generated Document Version keep an id that says the paper was generated and holds nothing that says what was in it. The generated Version CHECK is unchanged, because the reference it requires still resolves.

Erasure through `DELETE /api/v1/auto-docs/:id` locks the `auto_docs` row, its Generations, and its template `document_versions` `FOR UPDATE`, then deletes `auto_doc_filings`, `auto_doc_acknowledgements`, `auto_doc_generations`, the template `documents` row, and the `auto_docs` row, and destroys every collected blob inside the same transaction before the commit, in DOC-010's order. `auto_doc_form_versions`, `auto_doc_assignment_rules`, `auto_doc_audience_users`, `auto_doc_audience_departments`, and `auto_doc_template_scans` go with their cascades. `auto_doc_filings.document_id` already cleared on Document erasure, so a Filed Document erased separately on its own record leaves its Filing row behind without it.

`activity_log.entity_type` gains `auto_doc`. Verbs: `auto_doc.created`, `.updated`, `.published`, `.unpublished`, `.archived`, `.restored`, `.generated`, `.generation_retried`, `.filed`, `.acknowledged`, `.form_saved`, `.template_uploaded`, `.hard_deleted`. The erasure entry carries the Generation count and the Version count only.
