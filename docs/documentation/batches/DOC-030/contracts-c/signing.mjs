...
import { PEOPLE, articleText, expectThat, pdf, q, sha, sleep, until } from "./lib.mjs";
...
export default async function signing(ctx) {
  ...
  async function liveNote(page) {
    ...
    return (
      texts
        ...
        .find((t) => /status (is checked|arrives)|Automatic status updates/.test(t)) ?? null
    );
  }
  ...
  async function sendEnvelope(page, number, { versionLabel, signers, subject }) {
    ...
    const sent = page.waitForResponse(
      (r) =>
        r.url().endsWith(`/api/v1/contracts/${number}/envelopes`) &&
  ...
  const signersFor = (role, round) => [
    {
      name: `DOC-030 Signer One ${round}`,
      email: `signer.one.${role}.${round}.${STAMP}@counterparty.example`,
    },
    {
      name: `DOC-030 Signer Two ${round}`,
      email: `signer.two.${role}.${round}.${STAMP}@counterparty.example`,
    },
  ];
  ...
  async function main() {
    ...
    await step(
      "administrator",
      PEOPLE.daniel.name,
      "/settings/integrations/e-signature (API read)",
      "Read the lab's starting Signing connector",
    ...
    for (const flow of flows) {
      ...
      flow.title = title;
      flow.v1 = pdf(`DOC-030 ${flow.role} primary version 1`, [
        "Signature of Signer One: /sig/",
        "Signature of Signer Two: /sig/",
      ]);
      ...
      const noPaper = await api(flow.sender, "POST", "/contracts", {
        title: `DOC-030 contracts-c electronic-signing ${flow.role} no paper ${STAMP}`,
        contractTypeId: typeId,
      });
    ...
    for (const flow of flows) {
      await step(
        ...
        `/contracts/${flow.number}/approvals`,
        "If signing is unavailable: open Approvals with no connector configured, before and after a primary Document exists",
        "Send for signature is absent with no connector, with or without a primary Document.",
        async () => {
          ...
          await addVersion(
            ...
            flow.v2,
            "redline_theirs",
            "DOC-030 agreed round",
          );
          await upload(
            flow.sender,
            flow.number,
            flow.supportName,
            pdf(`DOC-030 ${flow.role} supporting`),
            "draft_ours",
            "DOC-030 supporting paper",
          );
          await openApprovals(flow.sender.page, flow.number);
    ...
    await step(
      ...
      "Setup (configure-signing owns these steps): save and enable a Webhook-mode DocuSign connector that points at the stand-in, then Test connection",
      "The connector saves, is enabled, and Test connection reaches the stand-in.",
      async () => {
        ...
        const saved = page.waitForResponse(
          (r) =>
            r.url().endsWith("/api/v1/signing-connectors/docusign") &&
    ...
    for (const flow of flows) {
      await step(
        flow.role,
        flow.sender.person.name,
        `/contracts/${flow.noPaperNumber}/approvals, /contracts/${flow.number}/approvals`,
        "With the connector enabled, open Approvals on a Contract with no primary Document and on one with a primary Document",
        "Send for signature is absent with no primary Document and shown with one.",
        async () => {
    ...
    await step(
      ...
      PEOPLE.daniel.name,
      "/settings/integrations/e-signature",
      "If signing is unavailable: turn Send for signature from records off, read both Contracts' Approvals, then turn it on again",
      "With the connector disabled, Send for signature is absent on both Contracts; enabled again, it returns.",
      async () => {
    ...
    for (const flow of flows) {
      ...
      await step(
        role,
        sender.person.name,
        `/contracts/${number}/documents (API download)`,
        "Before you start (/sig/): the paper to send carries /sig/ marks before its Document Version is uploaded",
        "The guide paragraph is present, and Version 1 downloads with its two /sig/ marks.",
        async () => {
          const text = articleText(ARTICLE);
          expectThat(
            text.includes("Type `/sig/` where each signature goes.") &&
              text.includes("If the paper has no `/sig/`, DocuSign puts no Sign Here field"),
            "paragraph missing",
          );
          ...
          const dl = await page.request.get(
            `${BASE}/api/v1/documents/${flow.documentId}/versions/${v1.id}/download`,
          );
          const bytes = await dl.body();
          const marks = (bytes.toString("latin1").match(/\/sig\//g) ?? []).length;
          expectThat(
            dl.status() === 200 && sha(bytes) === sha(flow.v1) && marks === 2,
            `download ${dl.status()}, marks ${marks}`,
          );
      ...
      await step(
        ...
        sender.person.name,
        `/contracts/${number}`,
        "Before you start: move the Contract to the Signature Status Out for signature",
        "The Contract is in the Signature Stage.",
        async () => {
      ...
      await step(
        ...
        "Send the Envelope steps 1-3 and 5: Approvals > Send for signature on Approvals & signing; read the send dialog; Version; empty and duplicate Signers; Add signer and Remove signer; Cancel",
        "The dialog says the Contract advances to Active only if it is still in the Signature Stage; Version lists only the primary Document's Versions with the current one selected; empty and duplicate Signers are refused; Cancel sends nothing.",
        async () => {
          ...
          const signersHelp = await dialog
            ...
            .count();
          const subjectHelp = await dialog
            .getByText("Signers see this on the invitation. Left blank, it names this contract.")
            .count();
          expectThat(
            dialogText.includes(SEND_NOTE),
            `send note missing: ${dialogText.slice(0, 300)}`,
          );
          expectThat(
          ...
          await fillSigners(dialog, [
            ...
            {
              name: "DOC-030 Unwanted Row",
              email: `unwanted.${role}.${STAMP}@counterparty.example`,
            },
          ]);
          ...
          await until(
            async () =>
              ...
              (await dialog.getByRole("alert").innerText()).trim() !== emptyAlert,
            "no duplicate alert",
          );
      ...
      await step(
        ...
        sender.person.name,
        `/contracts/${number}/approvals`,
        "If sending fails: send during a provider outage, read the error, reload the Envelope list, then send Version 1 with a blank Subject (Send the Envelope steps 2-5)",
        "The outage shows an error and records no round; the retry shows an Out for signature row with Version, Signers and sent time; the provider receives only the primary PDF with subject C-<number> <title>; the Status does not move; the note under the live row names Webhook mode.",
        async () => {
          ...
          expectThat(
            duringOutage === 0 && rowsAfterReload === 0,
            "a round exists after the outage",
          );
          ...
          expectThat(
            provider.subject === `C-${number} ${flow.title}`,
      ...
      await step(
        ...
        sender.person.name,
        `/contracts/${number}/approvals`,
        "One live Envelope: read Approvals while the round is out and try a second send directly",
        "Send for signature is absent and a second send is refused.",
        async () => {
          await openApprovals(page, number);
          const count = await sendButton(page).count();
          const second = await api(sender, "POST", `/contracts/${number}/envelopes`, {
            documentVersionId: (await signingState(sender, number)).primaryDocument.versions[0].id,
      ...
      await step(
        role,
        `${PEOPLE.daniel.name}, ${sender.person.name}`,
        `/settings/integrations/e-signature, /contracts/${number}/approvals`,
        "Follow completion: an Administrator disables the connector while the Envelope is out; the sender reads the note under the live row; the Administrator enables it again",
        "With the connector disabled, the note says automatic status updates are unavailable; enabled again, it names Webhook.",
        async () => {
          await setEnabled(daniel, false);
          await openApprovals(page, number);
      ...
      await step(
        ...
        `/contracts/${number}/approvals`,
        `Withdraw: an unrelated Legal Team Member reads the round; the sender opens its actions; the ${flow.voiderWhy} selects Void envelope, tries an empty Reason, cancels, then voids with a Reason`,
        "The unrelated member has no envelope actions and cannot void; the void dialog needs a Reason; Cancel keeps the round; a Voided round frees a new send and does not move the Status.",
        async () => {
          ...
          const unrelatedVoid = await api(flow.unrelated, "POST", `/envelopes/${envelopeId}/void`, {
            reason: "DOC-030 should be refused",
          });
          await openApprovals(page, number);
          ...
          await dialog.waitFor();
          const explains = await dialog
            .getByText(
              "The signers can no longer sign this round. The contract can be sent again straight after.",
            )
            .count();
          ...
          const voided = vp.waitForResponse((r) =>
            r.url().endsWith(`/envelopes/${envelopeId}/void`),
          );
          await dialog.getByRole("button", { name: "Void envelope" }).click();
      ...
      await step(
        ...
        sender.person.name,
        `/contracts/${number}/approvals`,
        "Decline: send a new round (current Version 2), the provider reports Declined with a reason",
        "The Declined row records its reason, the Voided round stays in history, nothing is filed, the Contract is not ended or archived, and another send is allowed.",
        async () => {
          const before = (await standin("state")).envelopes.length;
          ...
          const sent = await sendEnvelope(page, number, {
            ...
            subject: `DOC-030 contracts-c ${role} decline round`,
          });
          expectThat(sent.status === 201, `send answered ${sent.status}`);
          const provider = await newestProviderEnvelope(before);
          expectThat(
            provider.documentSha256 === sha(flow.v2),
            "decline round did not send the current Version 2",
          );
          expectThat(
            provider.subject === `DOC-030 contracts-c ${role} decline round`,
            `subject ${provider.subject}`,
          );
      ...
      await step(
        ...
        `/contracts/${number}/approvals`,
        "If another sender won: both people open the send dialog; the first sends; the second sends, reads the refusal, and reloads",
        "The second send is refused in the dialog and reloading shows the winner's live round; no second round exists.",
        async () => {
          const before = (await standin("state")).envelopes.length;
          await openApprovals(page, number);
          ...
          const sent = await sendEnvelope(page, number, {
            ...
            subject: `DOC-030 contracts-c ${role} completion round`,
          });
      ...
      await step(
        role,
        sender.person.name,
        `/contracts/${number}/approvals, /contracts/${number}/documents`,
        "Follow completion: the provider reports completion; watch the row, wait for Executed copy, open it, and check the Documents tab and Contract Status",
        "The row shows Signed then Executed copy; the executed PDF equals the provider output and is filed once as the pinned executed Version on the primary Document chain; the supporting Document is untouched; the Contract moves to Active; a repeated delivery adds nothing.",
        async () => {
          ...
          expectThat(
            executed.isExecuted && executed.isCurrent && chain.length === 3,
      ...
      await step(
        ...
        sender.person.name,
        `/contracts/${number}`,
        "Follow completion: someone moves the Contract to another Stage while the round is out; the provider then completes it",
        "Filing still pins the executed Version, and completion does not overwrite that Stage.",
        async () => {
          await moveStatus(page, number, "Out for signature");
          const before = (await standin("state")).envelopes.length;
          await openApprovals(page, number);
          ...
          await until(
            async () => (await signingState(sender, number)).envelopes[0].executedFetch === "ready",
            "never filed",
            120000,
            1500,
          );
      ...
      await step(
        ...
        sender.person.name,
        `/contracts/${number}/approvals`,
        "If voiding is refused because the provider already completed the round: void, read the refusal, reload, and inspect the recorded outcome",
        "The dialog shows the refusal; after the provider's feed arrives, reloading shows Signed.",
        async () => {
          const before = (await standin("state")).envelopes.length;
          await openApprovals(page, number);
          ...
          expectThat(
            refusal.length > 0 && reloadedAfterFeed.includes("Signed"),
      ...
      await step(
        ...
        sender.person.name,
        `/contracts/${number}/approvals`,
        "If the row says the executed copy could not be filed: the provider completes a round whose executed file cannot be fetched",
        "The Signed row says the executed copy could not be filed, and no Version is created.",
        async () => {
          await standin("faults", { combinedRefused: true });
          try {
            const before = (await standin("state")).envelopes.length;
            const versionsBefore = (await paper(sender, number)).find(
              (d) => d.id === flow.documentId,
            ).versions.length;
            await openApprovals(page, number);
            ...
            await until(
              async () =>
                (await signingState(sender, number)).envelopes[0].executedFetch === "failed",
            ...
            const versionsAfter = (await paper(sender, number)).find(
              (d) => d.id === flow.documentId,
            ).versions.length;
            expectThat(
              row.includes("Signed") &&
      ...
      await step(
        role,
        `${PEOPLE.ravi.name} (Business User on the Contract team)`,
        `/portal (API reads)`,
        "Negative: a Business User on the Contract team tries to read and send Envelopes",
        "Both are refused.",
        async () => {
      ...
      await step(
        ...
        sender.person.name,
        `/contracts/${number}/approvals`,
        "If signing is unavailable: archive the Contract and open Approvals",
        ...
        async () => {
          ...
          try {
            ...
            const direct = await api(sender, "POST", `/contracts/${number}/envelopes`, {
              documentVersionId:
                (await signingState(sender, number)).primaryDocument?.versions?.[0]?.id ?? "x",
    ...
    ctx.results.contracts = flows.map((f) => ({
      role: f.role,
      number: f.number,
  ...
  async function polling() {
    ...
    await step(
      ...
      PEOPLE.daniel.name,
      "/settings/integrations/e-signature",
      "Setup: switch Signing updates to Polling and save",
      "The connector stays enabled in Polling mode.",
      async () => {
        const page = daniel.page;
        await openConnector(page);
    ...
    for (const flow of flows) {
      ...
      const created = await api(flow.who, "POST", "/contracts", {
        title: flow.title,
        contractTypeId: typeId,
      });
      flow.number = created.json.contract.number;
      ...
      await upload(
        flow.who,
        flow.number,
        `doc030-polling-${flow.role}.pdf`,
        flow.bytes,
        "draft_ours",
        "DOC-030 polling round",
      );
      await step(
        flow.role,
        flow.who.person.name,
        `/contracts/${flow.number}/approvals`,
        "Polling: move to Out for signature, send an Envelope, and read the note under the live row",
        "The note under the live row says status is checked by polling about every 15–20 minutes.",
        async () => {
          await moveStatus(flow.who.page, flow.number, "Out for signature");
          const before = (await standin("state")).envelopes.length;
          await openApprovals(flow.who.page, flow.number);
          const sent = await sendEnvelope(flow.who.page, flow.number, {
            signers: signersFor(flow.role, "p1"),
          });
          expectThat(sent.status === 201, `send ${sent.status}`);
          await sent.dialog.waitFor({ state: "hidden" });
          await openApprovals(flow.who.page, flow.number);
    ...
    for (const flow of flows) {
      ...
      flow.refused = await deliver(flow.providerId, "completed", {
        completedAt: new Date().toISOString(),
      });
    }
    for (const flow of flows) {
      await step(
        ...
        flow.who.person.name,
        `/contracts/${flow.number}/approvals`,
        "Polling: the provider completes the round and no webhook is accepted; watch the row until Signed and Executed copy",
        "The row reaches Signed within about 15 to 20 minutes while the worker runs; the executed copy files and the Contract moves to Active.",
        async () => {
          await until(
            async () =>
              (await signingState(flow.who, flow.number)).envelopes[0].status === "signed",
          ...
          await until(
            async () =>
              (await signingState(flow.who, flow.number)).envelopes[0].executedFetch === "ready",
            "never filed",
            300000,
            5000,
          );
          ...
          const exec = await card(flow.who.page)
          ...
          expectThat(
            row.includes("Signed") && exec === 1 && c.stage === "active",
            `${row} ${c.statusName}`,
          );
