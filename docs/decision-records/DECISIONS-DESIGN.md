...
The card has a 44px header, a 40px column-heading strip and the tree. The header carries the type name as a caption and the root controls at the right: **Attach Field**, **Create Field**, **Add condition** and **Preview intake form**. They wrap onto a second header line when the card is narrow. Only a Branch has a footer, with its own three controls below its children. There is no Save form button. Each completed switch, reorder, attach, detach or condition edit commits the whole valid tree through the existing Form write. An incomplete new Branch or condition stays local until it is complete; Escape discards that draft. Existing condition values commit on blur or Enter, selects on change. Escape restores the prior value. The edited control carries the SET-003 pending/saved/error micro-state.
...
Children sit 24px inside the left rule. Only the name side gains indentation; switch and trailing columns keep the root's alignment. A nested Branch repeats the rule and indent, without a new card border. **Add row into branch**, below the children, opens an Attach Field / Create Field menu scoped to that Branch. Moving an existing Row uses drag onto the Branch's child area, or the grip's menu. The root controls append at root; the Branch action appends inside that Branch. The Branch overflow offers Add condition inside and Remove branch; its grip carries the depth moves like any Row. Remove branch keeps its children in place under the parent, in order, and removes only its conditions. The menu states "Keep children here" beside that removal. Validate references before committing the resulting tree.

**4. Foot actions are always at the end of the tree.** Left to right: **Attach Field**, **Create Field**, **Add condition**; **Preview intake form** at the right. Buttons are 32px high with 8px gaps, 13px labels and 16px icons. Attach Field opens the live, unattached Fields menu for this module, with name and 12px type caption, not a second catalog. Already attached Fields and all built-ins are absent. If nothing is available, the menu says "All Fields are attached" and still offers Create Field. Attach creates a Record Row with all three switches off. The Administrator can turn on Visible on Portal before enabling intake. There is no retired Field tag to inherit.

Create Field opens the existing DES-021 Field editor dialog without the retired tag control. A successful catalog create is followed by attachment at the chosen parent. A failed attachment keeps the created Field available and names the failure beside Create Field; retry must attach that Field, not create another. Add branch appends a local empty Branch and focuses its first Row select. An Entity Form with no Fields has a 52px caption row, "No Fields attached", above these same three actions.
