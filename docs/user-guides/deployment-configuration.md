...
For [Claude Code with OAuth](connect-claude.md#connect-claude-code), also forward the discovery, authorization, sign-in and consent paths in the publicly reachable profile below. Keep them private. Enable Claude Code in Allowed Clients and OAuth Clients for the person's account group. The browser and Client reach OpenLaw over the office network or VPN; the browser returns to Claude Code's loopback callback on the same device. The API needs outbound access to Claude Code's published identity. The public-address pill may warn on this private deployment.
