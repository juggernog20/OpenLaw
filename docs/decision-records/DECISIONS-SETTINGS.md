...
| **MTR-011**  | Matters Settings → Fields                                                            | Admin      | Custom-field catalog view scoped to `matter` fields per **CTR-016**: add / rename / describe / archive; field type immutable (9 types incl. `entity`); DD-015 business\|legal tag; options editor for selects                                                                                                                                                                                                                                                                                                                                                                      |
...
| **INT-002**  | Intake Settings → Request Types                                                      | Admin      | List + per-type form editor: target matter/contract type; attach catalog fields (target-module scope, or Contract/Matter scope when undecided); required flags; display order                                                                                                                                                                                                                                                                                                                                                                                                      |
| **CTR-016**  | Contracts Settings → Fields                                                          | Admin      | Catalog view scoped to `contract` fields; per-field `ai_prompt` editor (CTR-008); same machinery as the matters view                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| **CTR-016**  | Contracts Settings → Types → [type]                                                  | Admin      | Attach / detach Contract fields; per-type display order; `is_required` toggle (hard-enforced per MTR-014 rule)                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **MTR-011**  | Matters Settings → Types → [type]                                                    | Admin      | Attach / detach Matter fields to the type; per-type display order                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| **CTR-008**  | ~~Organization → Integrations → AI analysis~~ → **Organization → AI analysis**       | Admin      | Four collapsed cards: Provider, Answer style, Conversion draft prompts and Contract analysis prompts, with the open Request conversion switches card after Provider once a connector is saved; seven core prompts and five conversion prompts, editable semantics, with the code-enforced format sentence available from a label tooltip (2026-09-21); prompted catalog Fields and their answer style stay under Contracts → Fields (CTR-008 addendum 2026-09-20). _Placement superseded by **SET-008**: the pane is an Organization section of its own, not an Integrations tab._ |
| **KNW-001**  | Knowledge Settings → Types                                                           | Admin      | List view; seeds: template, precedent, playbook, article; MTR-001 machinery                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| **ENT-001**  | Entities Settings → Types / Types → [type]                                           | Admin      | List view; add / rename / reorder / archive; `other` row protected; per-type attachment editor for Entity Fields with order and required flags                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **ENT-001**  | Entities Settings → Officer Roles                                                    | Admin      | Shared taxonomy pane; seeds: director, ceo, cfo, secretary, other; `other` protected; archive guard counts and reassigns current and resigned officers                                                                                                                                                                                                                                                                                                                                                                                                                             |
| **ENT-001**  | Entities Settings → Fields                                                           | Admin      | Catalog view scoped to `entity` fields per **CTR-016**'s shared `fields` catalog: add / rename / describe / archive; same machinery as the Matters/Contracts views                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
- **Consequences** — The wizard is a real v1 build surface ~~(one flow, ~6 steps)~~ _(superseded by the M33/6 addendum: nine steps including the welcome splash)_. Wizard completion state stored per-org; skipped steps resurface as a Settings checklist card until done.
...
- **Addendum (2026-09-05, #701)** — The checklist card this decision promised lands at Settings → Organization → General, above the Organization card. It reads the per-step state from `GET /api/v1/onboarding`, the route the wizard reads, so the two cannot disagree. Each outstanding row links to the pane that finishes it. Two rows have no pane. Email is set by the environment or by the wizard's own step (TECH-011), so its row is plain text. Review has no setting behind it at all: only its own mark finishes it, and the wizard's Finish was the one place that wrote it, so an Administrator who chose "Set up later" would carry the row for good. The review row therefore carries a "Mark as reviewed" action that calls `POST /api/v1/onboarding/reviewed` in place. The card renders nothing once every step is done.

### Addendum (2026-09-05, M33/6, #702): two connector steps and nine steps in all

SET-008 moved AI analysis out of Integrations into its own Organization section. The wizard now gives each Settings destination its own step. E-signature configures the signing connector at `/settings/integrations/e-signature`; AI analysis configures the AI connector at `/settings/ai-analysis`. This supersedes the single optional-integrations step and the original estimate of about six steps.

The built flow has nine steps, including its welcome splash: Welcome to OpenLaw → Your organization → Authentication → Business-user portal → Outbound email → Invite your team → E-signature → AI analysis → Review. Every step remains skippable. Finish on Review records `onboarding_reviewed_types_at` before completing the wizard; skipping Review completes the wizard without that acknowledgement. Neither timestamp can be reset.
...
## SET-008 — AI analysis is an Organization section of its own, not an Integrations tab
...
### SET-008 implementation note, 2026-09-20, M37/6, #961

M37 replaces the earlier single Prompts card with Conversion draft prompts and Contract
analysis prompts, and adds Answer style beside Provider. All four cards start collapsed.
The Request conversion switches card is unchanged: it stays open between Provider and
Answer style and appears once a connector is saved. The Answer style radios save immediately to the connector; they require a configured
provider. The prompt rows edit extraction instructions and show the type's fixed format
sentence below them. Contract catalog Field prompts and Answer style overrides remain
under Contracts → Fields. This updates the CTR-008 row above and supersedes the earlier
reconciliation that described an always-open Field prompts card.
