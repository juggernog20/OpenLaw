...
export async function runC19(ctx, step) {
  ...
  await step(
    ...
    async () => {
      ...
      await dialog.getByRole("textbox", { name: "Title" }).fill(`DOC-029r2 contracts-b C19 cancelled ${tag}`);
  ...
  await step(
    ...
    async () => {
      ...
      await dialog.getByRole("textbox", { name: "Add a note" }).fill("DOC-029r2 initial note for the reviewer.");
      ...
      expectThat(first && first.visibility === "working_team" && detailsText.includes("DOC-029r2 initial note") && detailsText.includes("doc029-services-agreement.pdf"), `comment ${JSON.stringify(first)?.slice(0, 300)}; details ${detailsText.slice(0, 300)}`);
  ...
  await step(
    ...
    async () => {
      ...
      await dialog.getByRole("textbox", { name: "Title" }).fill(title);
      await dialog.getByRole("textbox", { name: "Add a note" }).fill("DOC-029r2 note that fails once.");
      let failed = 0;
  ...
  await step(
    ...
    async () => {
      ...
      const find = (j) => JSON.stringify(j).match(/\{[^{}]*"label":"DOC-029r2 price review[^{}]*\}/)?.[0] ?? "";
  ...
  await step(
    ...
    async () => {
      ...
      const w1 = await actor.api("POST", `/contracts/${num}/tasks`, { title: "DOC-029r2 archived write" });
      const w2 = await actor.api("POST", `/contracts/${num}/key-dates`, { date: isoPlusDays(5), label: "DOC-029r2 archived write" });
