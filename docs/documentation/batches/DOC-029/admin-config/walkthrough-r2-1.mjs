// DOC-029 round 2 independent browser walkthrough for the admin-config group:
// types-statuses-fields (V-C38), request-forms (V-C40), and reminders-and-audit (V-C41).
// Written by the DOC-029r2 independent walkthrough agent (admin-config, round 1). It adapts
// walkthrough-r1.mjs to the current article text (select Save or Save status to add a row,
// Cancel to discard a draft, and the Default fields and Custom Fields cards) and records what
// the admin2 documentation lab built from 57e77e38 showed.
// Run from the repository root:
//   LAB_PASSWORD=... mise exec -- node docs/documentation/batches/DOC-029/admin-config/walkthrough-r2-1.mjs
// Optional: SECTIONS=types,forms,reminders,audit,access (default: all).
...
const OUT = process.env.OUT ?? path.join(here, "walkthrough-r2-1.json");
const SHOTS = process.env.SHOTS ?? os.tmpdir();
const SECTIONS = (process.env.SECTIONS ?? "types,forms,reminders,audit,access").split(",");
const stamp = Date.now().toString(36);
const G = `DOC-029r2 admin-config walk`;
const sha256 = (file) => createHash("sha256").update(readFileSync(file)).digest("hex");
const ARTICLES = ["types-statuses-fields", "request-forms", "reminders-and-audit"];
...
const results = {
  ...
  walkthroughRound: 1,
  issues: [745, 747],
  walkthroughReviewer: "DOC-029r2 independent walkthrough agent (admin-config, round 1)",
  reviewerKind: "agent",
  ...
  previousWalkthrough: "docs/documentation/batches/DOC-029/admin-config/walkthrough-r1.json",
  labProject: PROJECT,
  images: {
...
function row(page, name) {
  return page
    .getByRole("listitem")
    .filter({ has: page.getByRole("button", { name: `Rename ${name}`, exact: true }) });
}
...
// The article: select the add button, enter a name, and select Save.
async function addListRow(page, button, inputLabel, name, saveLabel = "Save") {
...
async function typesSection(admin, member) {
  ...
  await step(
    SC,
    role,
    "Add and maintain types, steps 1-2: open Settings > Organization > Matters > Types, select Add type, enter a name, select Save; select Cancel to discard a draft",
    "Save stays unavailable while the name is blank. Cancel discards the draft and creates no row. Save adds the new type row to the Matter types list.",
    async () => {
      await openSettings(admin, "Organization", "Matters");
      await admin
        .getByRole("navigation", { name: "Matters panes" })
        .getByRole("link", { name: "Types" })
        .click();
      await admin.getByRole("heading", { name: "Matter types" }).waitFor();
      // Cancel check: a named draft, then Cancel.
      const cancelName = `${G} Cancelled draft ${stamp}`;
      await admin.getByRole("button", { name: "Add type", exact: true }).click();
      const input = admin.getByRole("textbox", { name: "New type name" });
      const saveBlank = await admin.getByRole("button", { name: "Save", exact: true }).isDisabled();
      const cancelVisible = await admin
        .getByRole("button", { name: "Cancel", exact: true })
        .isVisible();
      await input.fill(cancelName);
      const saveFilled = await admin.getByRole("button", { name: "Save", exact: true }).isEnabled();
      ...
      const cancelledRows = await admin
        ...
        .count();
      expectThat(
        saveBlank && cancelVisible && saveFilled,
        `Save blank disabled ${saveBlank}, Cancel visible ${cancelVisible}, Save enabled with a name ${saveFilled}`,
      );
      expectThat(cancelledRows === 0, "Cancel created a row");
      ...
      results.records.push(
        { kind: "matter type", name: typeName },
        { kind: "matter type", name: replacementType },
      );
      return `Profile menu > Settings opened ${new URL(admin.url()).pathname}. Add type opened a name box with Save and Cancel; Save was disabled while the box was blank (${saveBlank}) and enabled after a name was entered (${saveFilled}). Cancel on the draft ${q(cancelName)} closed the box, and after reload no row had that name. Add type with the name ${q(typeName)} and Save added the row ${q(usage)}. A second type ${q(replacementType)} was added the same way for the archive check.`;
    },
  );
  ...
  await step(
    SC,
    role,
    "Step 3: rename the row with Rename; open Edit, change the description, leave the field, check the saved result; the Slug stays fixed",
    "Rename changes the display name. Edit shows a fixed Slug and saves the description on blur.",
    async () => {
      ...
      expectThat(
        saved === "DOC-029 fictional supplier assessment type.",
        `description after reload ${q(saved)}`,
      );
  ...
  await step(
    ...
    "The row moves one position and the order persists after reload.",
    async () => {
      await admin.getByRole("link", { name: "All types" }).click();
      await admin.getByRole("heading", { name: "Matter types" }).waitFor();
      const before = await reorderPosition(admin, typeRenamed);
      await admin
        .getByRole("button", { name: new RegExp(`^Reorder ${escapeRe(typeRenamed)}, position`) })
        .focus();
      await admin.keyboard.press("ArrowUp");
      await until(
        async () => (await reorderPosition(admin, typeRenamed)) === before - 1,
        "row did not move up",
      );
      await pause(1200);
      await admin.reload();
  ...
  await step(
    SC,
    role,
    "Step 1-2 for the other modules: Contracts, Entities, and Knowledge Types each accept Add type; Edit exists only where the article says it may",
    "Contracts and Entities rows have Edit; Knowledge rows have none.",
    async () => {
      const out = [];
      for (const [link, heading, pane] of [
        ...
        await admin
          .getByRole("navigation", { name: pane })
          .getByRole("link", { name: "Types" })
          .click();
        ...
        out.push(
          `${link}: added ${q(name)} with Save, Edit control ${edit ? "present" : "absent"}`,
        );
  ...
  await step(
    SC,
    role,
    "Create a Field, first paragraph: Contracts and Matters Fields show Default fields as locked, read-only rows (collapsed at first, expand on the heading) above an expanded Custom Fields card with the add controls; Entities Fields has no Default fields card",
    "Default fields starts collapsed; selecting its heading shows locked rows with no Rename, Archive, or reorder controls. Custom Fields starts expanded and holds Add field. Entities Fields shows one Fields card.",
    async () => {
      const out = [];
      for (const [link, pane] of [
        ...
        await custom.getByRole("button", { name: "Add field" }).waitFor({ timeout: 15000 });
        const order = await defaults.evaluate(
          (d, c) => !!(d.compareDocumentPosition(c) & Node.DOCUMENT_POSITION_FOLLOWING),
          await custom.elementHandle(),
        );
        const toggle = defaults.getByRole("button", { name: "Default fields" });
        const startExpanded = await toggle.getAttribute("aria-expanded");
        const customExpanded = await custom
          .getByRole("button", { name: "Custom Fields" })
          .getAttribute("aria-expanded");
        const rowsBefore = await defaults.getByRole("listitem").count();
        await toggle.click();
        await until(
          async () => (await toggle.getAttribute("aria-expanded")) === "true",
          "Default fields did not expand",
        );
        const rows = (await defaults.getByRole("listitem").allInnerTexts()).map((t) =>
          t.replace(/\s+/g, " ").trim(),
        );
        const locks = await defaults
          .getByRole("img", { name: /: built-in field, read-only here$/ })
          .count();
        const controls = await defaults.getByRole("list").getByRole("button").count();
        const inputs =
          (await defaults.getByRole("textbox").count()) +
          (await defaults.getByRole("switch").count());
        expectThat(order, `${link}: Default fields is not above Custom Fields`);
        expectThat(
          startExpanded === "false" && rowsBefore === 0,
          `${link}: Default fields started expanded=${startExpanded} with ${rowsBefore} rows`,
        );
        expectThat(customExpanded === "true", `${link}: Custom Fields expanded=${customExpanded}`);
        expectThat(
          rows.length > 0 && locks === rows.length && controls === 0 && inputs === 0,
          `${link}: rows ${rows.length}, locks ${locks}, row buttons ${controls}, inputs ${inputs}`,
        );
        out.push(
          `${link} > Fields: Default fields sits above Custom Fields, starts with aria-expanded=${startExpanded} and no visible rows; Custom Fields starts aria-expanded=${customExpanded} with Add field. Selecting the Default fields heading showed ${rows.length} rows (${q(rows.slice(0, 5))}...), each with a "<name>: built-in field, read-only here" lock image, and ${controls} row buttons and ${inputs} text boxes or switches`,
        );
      }
      await openSettings(admin, "Organization", "Entities");
      await admin
        .getByRole("navigation", { name: "Entities panes" })
        .getByRole("link", { name: "Fields" })
        .click();
      await admin.getByRole("button", { name: "Add field" }).waitFor({ timeout: 15000 });
      const entityDefaults = await admin.getByRole("region", { name: "Default fields" }).count();
      const entityCustom = await admin.getByRole("region", { name: "Custom Fields" }).count();
      expectThat(entityDefaults === 0, "Entities Fields shows Default fields");
      out.push(
        `Entities > Fields has ${entityDefaults} Default fields card and ${entityCustom} Custom Fields card; Add field is on its Fields card`,
      );
      return out.join(". ") + ".";
    },
  );
  ...
  await step(
    SC,
    role,
    "Create a Field, steps 1-5: Matters > Fields > Add field with Name, Description, Type, Scope, Tag, and Options",
    "The Type list matches the article order; the new Field row shows its Type, Scope, and Tag.",
    async () => {
      await openSettings(admin, "Organization", "Matters");
      await admin
        .getByRole("navigation", { name: "Matters panes" })
        .getByRole("link", { name: "Fields" })
        .click();
      await admin.getByRole("button", { name: "Add field" }).waitFor();
      await admin.getByRole("button", { name: "Add field" }).click();
      const d = admin.getByRole("dialog", { name: "Add field" });
      const types = (await d.getByLabel("Type").locator("option").allInnerTexts()).filter(
        (t) => t !== "Type…",
      );
      const expectedTypes = [
        "Text",
      ...
      expectThat(
        /Single select/.test(rowText) && /Matter/.test(rowText) && /Legal/.test(rowText),
        `row ${rowText}`,
      );
      ...
      const typeText = (
        await e.getByText("The field type is immutable after creation.").textContent()
      ).trim();
  ...
  await step(
    SC,
    role,
    "Attach Fields and set requiredness, steps 1-3: Types > Edit > Attach field, turn on Required, reorder the attached Fields",
    "Both Fields attach; Required saves; keyboard reorder changes the attached order.",
    async () => {
      ...
      await req.click();
      await until(
        async () => (await req.isChecked()) && (await req.isEnabled()),
        "Required did not save",
      );
      await pause(800);
      ...
      await admin
        .getByRole("button", { name: new RegExp(`^Reorder ${escapeRe(fieldTwo)}, position`) })
        .focus();
      await admin.keyboard.press("ArrowUp");
      await pause(1500);
      await admin.reload();
      ...
      const after = await admin
        .getByRole("list", { name: "Attached fields" })
        .getByRole("button", { name: /^Reorder / })
        .evaluateAll((els) => els.map((e) => e.getAttribute("aria-label")));
      const checked = await admin
        .getByRole("checkbox", { name: `${fieldName} required` })
        .isChecked();
  ...
  await step(
    SC,
    role,
    "Step 4: create a fictional Matter of this type, first leaving the required Field empty, then with a valid value",
    "Creation is refused without the required Field and accepted with it; the record holds the value.",
    async () => {
      ...
      expectThat(
        main.includes(fieldName) && main.includes("Privacy"),
        "Overview does not show the Field value",
      );
  ...
  await step(
    SC,
    role,
    "Global Fields: promote the Field to Global in Edit, attach it to a Contract type, then try to narrow it back to Matter",
    "Promotion saves. Narrowing is refused while the Contract type attaches it and allowed after Detach.",
    async () => {
      await openSettings(admin, "Organization", "Matters");
      await admin
        .getByRole("navigation", { name: "Matters panes" })
      ...
      await until(
        async () =>
          (await admin
            ...
            .getByText(fieldName)
            .count()) === 0,
        "Field still attached to the Contract type",
      );
      await openSettings(admin, "Organization", "Matters");
      await admin
        .getByRole("navigation", { name: "Matters panes" })
  ...
  await step(
    SC,
    role,
    "Detach and Archive keep stored values: detach the Field from the Matter type, archive it in the catalog, then Show archived and Restore",
    "The definition stays in the catalog after Detach; the Matter keeps its stored value; Archive hides the Field; Restore brings it back.",
    async () => {
      await admin.goto(S.matterTypeUrl);
      await admin.getByRole("heading", { name: "Attached fields" }).waitFor();
      ...
      await admin
        ...
        .click();
      await admin.getByRole("button", { name: `Rename ${fieldName}`, exact: true }).waitFor();
      const inCatalog = await admin
        .getByRole("button", { name: `Rename ${fieldTwo}`, exact: true })
        .count();
      ...
      await until(
        async () =>
          (await admin
            .getByRole("button", { name: `Rename ${fieldName}`, exact: true })
            .count()) === 0,
        "archived Field still listed",
      );
      ...
      const attachedWhileArchived = await (async () => {
        await admin.goto(S.matterTypeUrl);
        await admin.getByRole("heading", { name: "Attached fields" }).waitFor();
        return (await admin.getByRole("list", { name: "Attached fields" }).innerText()).replace(
          /\s+/g,
          " ",
        );
      })();
      ...
      await admin
        .getByRole("button", { name: `Archive ${fieldName}`, exact: true })
        .waitFor({ timeout: 15000 });
      // The switch disappears when no archived Field is left.
      if (await admin.getByRole("switch", { name: "Show archived" }).count())
        await admin.getByRole("switch", { name: "Show archived" }).click();
  ...
  await step(
    SC,
    role,
    "Archive an in-use type: read the usage count, try without a replacement, then choose the live replacement; Restore does not move the Matter back",
    "The dialog names the usage count; archive without a replacement is refused; with a replacement the Matter moves; Restore leaves it on the replacement.",
    async () => {
      await openSettings(admin, "Organization", "Matters");
      ...
      await admin.goto(`${BASE}/matters/${S.matterNumber}`);
      await admin.getByRole("combobox", { name: "Matter type" }).waitFor();
      const typeAfterRestore = await admin
        .getByRole("combobox", { name: "Matter type" })
        .locator("option:checked")
        .innerText();
      expectThat(
        typeAfterRestore === replacementType,
        `after restore the Matter type is ${typeAfterRestore}`,
      );
  ...
  await step(
    SC,
    role,
    "Configure Statuses: add a Contract Status with a Stage, rename it, and check the protected and in-use rows",
    "The new row keeps its Stage after rename; no control changes the Stage; Draft, Active, and Expired are protected; an in-use Status cannot be archived from the dialog.",
    async () => {
      await openSettings(admin, "Organization", "Contracts");
      await admin
        .getByRole("navigation", { name: "Contracts panes" })
        .getByRole("link", { name: "Statuses" })
        .click();
      ...
      const locks = [];
      for (const n of ["Draft", "Active", "Expired"])
        locks.push(
          await admin
      ...
      const inUse = admin
        .getByRole("listitem")
        .filter({ hasText: /Stage: \w+\s*[1-9]\d* contracts?/ })
        .filter({ has: admin.getByRole("button", { name: /^Archive / }) })
        .first();
      const inUseName = (
        await inUse.getByRole("button", { name: /^Rename / }).getAttribute("aria-label")
      ).replace(/^Rename /, "");
      ...
      expectThat(
        disabled && selects === 0,
        "in-use Contract Status dialog allowed archive or offered reassignment",
      );
  ...
  await step(
    SC,
    role,
    "Configure Statuses: add Matter Statuses in the Open Category with and without a group, rename, change the group on the row, and archive an in-use Status with a same-Category replacement",
    "A new Open Status uses In progress unless another group is chosen; rename keeps Category and group; the archive replacement lists only same-Category Statuses and the Matter moves.",
    async () => {
      await openSettings(admin, "Organization", "Matters");
      await admin
        .getByRole("navigation", { name: "Matters panes" })
      ...
      await admin.getByRole("button", { name: "Save status", exact: true }).click();
      await admin
        .getByRole("button", { name: `Rename ${a}`, exact: true })
        .waitFor({ timeout: 15000 });
      await admin.getByRole("button", { name: "Add status" }).click();
      await admin
        .getByRole("combobox", { name: "New status category" })
      ...
      await admin.getByRole("button", { name: "Save status", exact: true }).click();
      await admin
        .getByRole("button", { name: `Rename ${b}`, exact: true })
        .waitFor({ timeout: 15000 });
      await renameRow(admin, a, aRenamed);
      await admin.reload();
      await admin.getByRole("button", { name: `Rename ${aRenamed}`, exact: true }).waitFor();
      const aGroup = await admin
        .getByRole("combobox", { name: `Group for ${aRenamed}` })
        .locator("option:checked")
        .innerText();
      const bGroup = await admin
        .getByRole("combobox", { name: `Group for ${b}` })
        .locator("option:checked")
        .innerText();
      expectThat(
        defaultGroup === "In progress" && bGroup === "In progress",
        `default group ${defaultGroup}/${bGroup}`,
      );
      ...
      const locks = [
        await admin
          .getByRole("img", { name: "Open is system-protected and can't be archived" })
          .count(),
        await admin
          .getByRole("img", { name: "Closed is system-protected and can't be archived" })
          .count(),
      ];
      expectThat(
        locks.every((c) => c === 1),
        "Open/Closed not protected",
      );
      ...
      const after = await api(admin, "GET", `/api/v1/matters/${S.matterNumber}`);
      expectThat(
        bId && after.body?.matter?.statusId === bId,
        "Matter did not move to the replacement Status",
      );
      results.records.push(
        { kind: "matter status (archived)", name: aRenamed },
        { kind: "matter status", name: b },
      );
      return `Category choices ${q(cats)}; with Open chosen the group select offered ${q(groups)} and started at ${q(defaultGroup)}. ${q(a)} was added with Waiting and Save status; ${q(b)} was added without choosing a group and reads ${q(bGroup)}. After rename and reload ${q(aRenamed)} keeps group ${q(aGroup)}. Changing ${q(b)}'s row group to Open persisted after reload. Open and Closed show the protected lock. M-${S.matterNumber} was moved to ${q(aRenamed)} from the record's status control (${moved}). Its Archive dialog read ${q(dtext.slice(0, 160))} with Archive status disabled until a replacement was chosen; replacements offered ${q(options)} (no Closed-Category Status). Choosing ${q(b)} archived it and the Matter now carries ${q(b)}.`;
    },
  );
  ...
  await step(
    SC,
    role,
    "Maintain Officer roles: Add role, Rename, reorder; archive an in-use role that has a current and a resigned Officer, choose the replacement and confirm Archive role; Restore does not reverse it",
    "The usage count includes the resigned Officer; both entries move to the replacement; Other cannot be archived; Restore leaves them on the replacement.",
    async () => {
      await openSettings(admin, "Organization", "Entities");
      await admin
        .getByRole("navigation", { name: "Entities panes" })
        .getByRole("link", { name: "Officer roles" })
        .click();
      ...
      await admin
        .getByRole("button", { name: new RegExp(`^Reorder ${escapeRe(roleRenamed)}, position`) })
        .focus();
      await admin.keyboard.press("ArrowUp");
      await until(
        async () => (await reorderPosition(admin, roleRenamed)) === before - 1,
        "role did not move",
      );
      const otherLock = await admin
        .getByRole("img", { name: "Other is system-protected and can't be archived" })
        .count();
      ...
      const created = admin.waitForResponse(
        (r) => r.request().method() === "POST" && /\/api\/v1\/entities$/.test(r.url()),
      );
      ...
      for (const [person, resigned] of [
        ...
        await officersRegion
          .getByLabel("Role", { exact: true })
          .selectOption({ label: roleRenamed });
        ...
        if (resigned) {
          ...
          const saved = admin.waitForResponse(
            (r) => r.request().method() === "PATCH" && /\/officers\//.test(r.url()),
          );
      ...
      await admin
        .getByRole("button", { name: `Archive ${roleRenamed}`, exact: true })
        .waitFor({ timeout: 15000 });
      ...
      await region
        ...
        .first()
        .waitFor();
      const currentOnly = await region
        .getByRole("combobox", { name: / Role$/ })
        .evaluateAll((els) => els.map((e) => e.getAttribute("aria-label")));
      expectThat(currentOnly.length === 1, `current Officers ${q(currentOnly)}`);
      await region.getByRole("checkbox", { name: "Show former" }).check();
      await pause(1500);
      const roles = await region
        .getByRole("combobox", { name: / Role$/ })
        .evaluateAll((els) =>
          els.map((e) => `${e.getAttribute("aria-label")}: ${e.selectedOptions[0]?.textContent}`),
        );
      expectThat(/2 officers/.test(usage), `usage before archive ${usage}`);
      expectThat(
        roles.length === 2 && roles.every((r) => r.endsWith(`: ${replacement}`)),
        `officer roles ${q(roles)}`,
      );
...
async function openIntake(admin, pane = "Request types") {
  ...
  await admin
    .getByRole("navigation", { name: "Intake panes" })
    .getByRole("link", { name: pane })
    .click();
...
async function submitPortal(page, slugPath, { title, fill = async () => {} }) {
  ...
  await page
    .getByRole("textbox", { name: "Description (required)" })
    .fill("Fictional DOC-029 walkthrough request. No real work is needed.");
  await fill(page);
...
async function formsSection(admin, business) {
  ...
  await step(
    SC,
    role,
    "Before you start: live destination types and Fields (fixtures created through Settings)",
    "A Contract type with the Global Deal value (USD) Field, a Matter type with a Global Entity Field, and a Global User Field exist.",
    async () => {
      ...
      await listed.click();
      await until(
        async () => (await listed.getAttribute("aria-checked")) === "true",
        "Portal-listed did not turn on",
      );
  ...
  await step(
    SC,
    role,
    "Create the request type, steps 1-4: Settings > Intake > Request types > Add request type, display name, Save (and Cancel discards a draft), Edit, Description saved on leaving the field; the list Target column and the editor have no Target control",
    "The new row shows Target: No target; the editor has Display name, Description, and Target turnaround, and no Target or Slug control; the Description persists after reload.",
    async () => {
      await openIntake(admin);
      // Cancel discards a draft.
      const cancelName = `${G} Cancelled request type ${stamp}`;
      await admin.getByRole("button", { name: "Add request type" }).click();
      await admin.getByRole("textbox", { name: "New request type name" }).fill(cancelName);
      await admin.getByRole("button", { name: "Cancel", exact: true }).click();
      await admin
        .getByRole("textbox", { name: "New request type name" })
      ...
      const cancelled = await admin
        .getByRole("button", { name: `Rename ${cancelName}`, exact: true })
        .count();
      expectThat(cancelled === 0, "Cancel created a request type");
      for (const name of [F.contractForm, F.matterForm, F.targetForm]) {
        await addListRow(admin, "Add request type", "New request type name", name);
      ...
      const descs = {
        [F.contractForm]: "Use this DOC-029 form for a supplier contract review.",
        [F.matterForm]: "Use this DOC-029 form for supplier advice that is not a contract.",
        [F.targetForm]: "DOC-029 form bound to a Contract type that will be archived.",
      };
      const urls = {};
      let controls = null;
      for (const name of [F.contractForm, F.matterForm, F.targetForm]) {
      ...
      S.formUrls = urls;
      return `Cancel on the draft ${q(cancelName)} closed the name box and created no row (checked after reload). Add request type with a display name and Save created ${q(F.contractForm)}, ${q(F.matterForm)} and ${q(F.targetForm)}. The list row reads ${q(rowText)}. Edit opened each editor; the Description saved on Tab and read ${q(saved)} after reload. Editor labels ${q(controls.labels)}; Target controls ${controls.target}, Slug controls ${controls.slug}.`;
    },
  );
  ...
  await step(
    SC,
    role,
    "Publish an estimated turnaround: 36501 is refused, 5 saves on leaving the control",
    "36501 shows an error and is not saved; 5 is saved and persists after reload.",
    async () => {
      ...
      await pause(1500);
      const refusal =
        (await admin.locator("main").innerText()).match(
      ...
      const saved = await admin
        .getByRole("spinbutton", { name: "Target turnaround (business days)" })
        .inputValue();
  ...
  await step(
    SC,
    role,
    "Choose the form fields, steps 1-3: Attach field, Required, reorder with the arrow keys; No target offers only Global Fields; the fixed basics; a User Field cannot be required, an Entity Field can",
    "The No target menu lists only Global Fields; Required persists; the order changes; Department is a required basic; the User Field's Required is refused; the Entity Field's Required saves.",
    async () => {
      await admin.goto(S.formUrls[F.contractForm]);
      ...
      expectThat(
        menu.length > 0 && nonGlobal.length === 0,
        `non-global menu items ${q(nonGlobal)}`,
      );
      const basics = await admin
        .getByRole("list", { name: "Basics are always on the form" })
        .getByRole("checkbox")
        .evaluateAll((els) =>
          els.map(
            (e) =>
              `${e.getAttribute("aria-label")}: ${e.getAttribute("aria-checked") ?? e.checked}`,
          ),
        );
      ...
      const order = await admin
        ...
        .getByRole("button", { name: /^Reorder / })
        .evaluateAll((els) =>
          els.map((e) => e.getAttribute("aria-label").replace(/^Reorder (.*), position.*$/, "$1")),
        );
      const reqAfter = await admin
        .getByRole("checkbox", { name: "Deal value (USD) required" })
        .isChecked();
      expectThat(
        reqAfter && q(order) === q(["Deal value (USD)", "Needed by", "Country"]),
        `order ${q(order)} required ${reqAfter}`,
      );
      // Seeded targeted types: read their Attach field menus only.
      const seeded = {};
      for (const [name, label] of [
        ["Vendor onboarding", "Contract · Vendor"],
        ["Employment question", "Matter · Employment"],
      ]) {
        await openIntake(admin);
        await admin.getByRole("button", { name: `Edit ${name}`, exact: true }).click();
        await admin.getByRole("heading", { name: "Form fields" }).waitFor();
        ...
        seeded[`${name} (${label})`] = {
          global: items.filter((t) => /· global$/.test(t)).length,
          moduleScoped: items.filter((t) => !/· global$/.test(t)),
        };
      }
      expectThat(
        seeded["Vendor onboarding (Contract · Vendor)"].moduleScoped.some((t) =>
          /Governing law|Jurisdiction|Liability cap/.test(t),
        ),
        "Contract target menu lacks Contract Fields",
      );
      expectThat(
        seeded["Employment question (Matter · Employment)"].moduleScoped.some((t) =>
          /External counsel|Regulator|Budget/.test(t),
        ),
        "Matter target menu lacks Matter Fields",
      );
      ...
      await ent.click();
      await until(
        async () => (await ent.isChecked()) && (await ent.isEnabled()),
        "Entity Required did not save",
      );
      ...
      const userRowText = (
        await admin
          .getByRole("list", { name: "Form fields" })
          .getByRole("listitem")
          .filter({ hasText: F.userField })
          .innerText()
      ).replace(/\s+/g, " ");
      await admin.reload();
      const usrAfter = await admin
        .getByRole("checkbox", { name: `${F.userField} required` })
        .isChecked();
      const entAfter = await admin
        .getByRole("checkbox", { name: `${F.entityField} required` })
        .isChecked();
  ...
  await step(
    SC,
    role,
    "Offer guidance before submission, steps 1-4: Deflection links > Add link with an External address (refuse an address without https://) and with a Knowledge item; Placement on each form; Edit; reorder",
    "An address without http:// or https:// is refused; both links save with the chosen placement; Edit changes the label; a handle moves with the arrow keys.",
    async () => {
      ...
      await d
        ...
        .fill("example.com/doc-029-supplier-checklist");
      ...
      await d
        .getByRole("textbox", { name: "Address" })
        .fill("https://example.com/doc-029-supplier-checklist");
      await d.getByRole("button", { name: "Add link" }).click();
      await d.waitFor({ state: "hidden", timeout: 15000 });
      await admin.getByRole("button", { name: "Add link" }).click();
      ...
      const kiOptions = (
        await d.getByRole("combobox", { name: "Knowledge item" }).locator("option").allInnerTexts()
      ).filter((t) => !/Choose an item/.test(t));
      await d
        .getByRole("combobox", { name: "Knowledge item" })
        .selectOption({ label: "How to ask legal for help" });
      ...
      const extRow = (
        await admin
          .getByRole("listitem")
          .filter({ has: admin.getByRole("button", { name: `Edit ${F.extLabel}`, exact: true }) })
          .innerText()
      ).replace(/\s+/g, " ");
      const kiRow = (
        await admin
          .getByRole("listitem")
          .filter({ has: admin.getByRole("button", { name: `Edit ${F.kiLabel}`, exact: true }) })
          .innerText()
      ).replace(/\s+/g, " ");
      const before = await reorderPosition(admin, F.kiLabel);
      await admin
        .getByRole("button", { name: new RegExp(`^Reorder ${escapeRe(F.kiLabel)}, position`) })
        .focus();
      await admin.keyboard.press("ArrowUp");
      await until(
        async () => (await reorderPosition(admin, F.kiLabel)) === before - 1,
        "link did not move",
      );
      await pause(1200);
      await admin.reload();
      ...
      results.records.push(
        { kind: "deflection link", name: F.extLabel },
        { kind: "deflection link", name: F.kiLabel },
      );
  ...
  await step(
    SC,
    "business_user",
    "Check the result on the Portal: request-type cards, Before you submit, Fields, order, and required markers; submit with a required answer missing, then with valid answers",
    "Both cards show the description and estimated turnaround; each form shows its own guidance; Fields appear in the configured order with required markers; a missing required answer is refused; valid answers create a Request.",
    async () => {
      currentPage = business;
      await business.goto(`${BASE}/portal`);
      await business
        .getByRole("list", { name: "Request types" })
        .getByRole("link", { name: new RegExp(escapeRe(F.matterForm)) })
        .waitFor({ timeout: 20000 });
      const cards = await business
        .getByRole("list", { name: "Request types" })
        .getByRole("link")
        .allInnerTexts();
      ...
      expectThat(
        /Estimated turnaround: 5 business days/.test(contractCard ?? "") &&
          /supplier contract review/.test(contractCard ?? ""),
        `contract card ${contractCard}`,
      );
      await business
        .getByRole("list", { name: "Request types" })
        .getByRole("link", { name: new RegExp(escapeRe(F.contractForm)) })
        .click();
      ...
      const detailLabels = labels
        .slice(labels.findIndex((l) => /Urgency/.test(l)) + 1)
        .map((l) => l.replace(/\*/g, "").replace(/\s+/g, " ").trim());
      expectThat(
        extHref === "https://example.com/doc-029-supplier-checklist",
        `external href ${extHref}`,
      );
      expectThat(
        q(detailLabels.slice(0, 3)) === q(["Deal value (USD) (required)", "Needed by", "Country"]),
        `detail labels ${q(detailLabels)}`,
      );
      await business.screenshot({
        path: path.join(SHOTS, "r2-1-portal-contract-form.png"),
        fullPage: true,
      });
      // Missing required answer.
      await business
        .getByRole("textbox", { name: "Title (required)" })
        .fill(`${G} Supplier contract request ${stamp}`);
      await business
        .getByRole("textbox", { name: "Description (required)" })
        .fill("Fictional DOC-029 walkthrough request. No real work is needed.");
      ...
      await pause(1500);
      const missing =
        (await business.locator("main").innerText()).match(
          /[^\n]*Deal value \(USD\)[^\n]*required[^\n]*/,
        )?.[0] ?? null;
      const stillForm =
        /\/portal\/new\//.test(business.url()) &&
        (await business.getByRole("button", { name: "Submit request" }).count()) === 1;
      ...
      const countryOptions = (
        await business.getByRole("combobox", { name: "Country" }).locator("option").allInnerTexts()
      ).map((t) => t.trim());
      S.countryAnswer = countryOptions.find((o) => o && !/^(Not set|Choose|Select)/.test(o));
      await business
        .getByRole("combobox", { name: "Country" })
        .selectOption({ label: S.countryAnswer });
      await business.getByRole("button", { name: "Submit request" }).click();
      ...
      results.records.push({
        kind: "request",
        name: `${G} Supplier contract request ${stamp}`,
        number: S.contractRequest,
      });
      // Matter form.
      await business.goto(`${BASE}/portal`);
      await business
      ...
      const extOnMatter = await mguide
        .getByRole("link", { name: new RegExp(escapeRe(F.extLabel)) })
        .count()
        .catch(() => 0);
      const mlabels = (await business.locator("main label").allInnerTexts()).map((l) =>
      ...
      expectThat(
        kiCount === 1 && extOnMatter === 0,
        `guidance on matter form: ki ${kiCount}, ext ${extOnMatter}`,
      );
      expectThat(
        mlabels.includes(`${F.entityField} (required)`),
        `matter form labels ${q(mlabels)}`,
      );
      const liveEntities = ((await api(admin, "GET", "/api/v1/entities")).body?.entities ?? []).map(
        (e) => e.legalName ?? e.name,
      );
      const offered = entityOptions.filter((o) => o !== "Not set");
      expectThat(
        offered.includes(F.listedEntity) &&
          offered.every((o) => o.startsWith(`${G} Portal supplier`)),
        `Portal Entity choices ${q(entityOptions)}`,
      );
  ...
  await step(
    SC,
    "business_user",
    "Submit the Matter form with a valid Entity answer",
    ...
    async () => {
      ...
      await business
        ...
        .fill("Fictional DOC-029 walkthrough request. No real work is needed.");
      await business
        .getByRole("combobox", { name: "Urgency (required)" })
        .selectOption({ label: "High" });
      const entityControl = business.getByLabel(new RegExp(`^${escapeRe(F.entityField)}`));
      S.portalEntityChoice = S.portalEntityOptions.find(
        (o) => o && !/Not set|Choose|Select|None|—/.test(o),
      );
  ...
  await step(
    SC,
    role,
    "Have Legal convert the Contract Request: a blank Type is refused; the attached Field carries; the answer the destination type does not attach is listed under Does not carry into the contract",
    "Convert without a type shows Pick a contract type.; with the Contract type chosen, Deal value (USD) carries, Counterparty name is listed as not carrying, and the Contract is created with the value.",
    async () => {
      currentPage = admin;
      const d = await openConvert(admin, S.contractRequest, "contract");
      const typeBefore = await d
      ...
      await d
        .getByRole("combobox", { name: "Contract type" })
        .selectOption({ label: F.contractType });
      await pause(1500);
      const text = (await d.innerText()).replace(/\s+/g, " ");
      const notCarry = text.match(/Does not carry into the contract.{0,160}/)?.[0] ?? null;
      expectThat(
        notCarry && /Country/.test(notCarry),
        `not-carry section ${notCarry}; dialog ${text.slice(0, 600)}`,
      );
      await d.getByRole("button", { name: "Convert to contract" }).click();
      ...
      const contractNumber =
        req.body?.request?.convertedRecord?.module === "contract"
          ? req.body.request.convertedRecord.number
          : null;
      ...
      expectThat(
        cText.includes("125000") && cText.includes(F.contractType),
        "Contract lacks the type or Deal value",
      );
      results.records.push({
        kind: "contract",
        number: contractNumber,
        from: `R-${S.contractRequest}`,
      });
  ...
  await step(
    SC,
    role,
    "If a configured destination type is archived, select a live type during conversion (fixture: the Target set through the API, because the editor has no Target control)",
    "The Target column names the configured Contract type; after that type is archived, conversion does not use it or switch module; a live type must be chosen and the Request becomes a Contract of that type.",
    async () => {
      const types = await api(admin, "GET", "/api/v1/request-types");
      const rt = (types.body?.requestTypes ?? []).find((t) => t.displayName === F.targetForm);
      const ct = (await api(admin, "GET", "/api/v1/contract-types")).body?.contractTypes?.find(
        (t) => t.displayName === F.targetType,
      );
      expectThat(rt && ct, "fixture ids not found");
      const patch = await api(admin, "PATCH", `/api/v1/request-types/${rt.id}`, {
      ...
      const number = await submitPortal(business, `/portal`, {
        ...
        await business
          .getByRole("list", { name: "Request types" })
          .getByRole("link", { name: new RegExp(escapeRe(F.targetForm)) })
          .click();
        await business
          .getByRole("textbox", { name: "Title (required)" })
          .fill(`${G} Archived target request ${stamp}`);
        await business
          .getByRole("textbox", { name: "Description (required)" })
          .fill("Fictional DOC-029 walkthrough request. No real work is needed.");
        await business.getByRole("button", { name: "Submit request" }).click();
      ...
      results.records.push({
        kind: "request",
        name: `${G} Archived target request ${stamp}`,
        number,
      });
      currentPage = admin;
      await openSettings(admin, "Organization", "Contracts");
      ...
      await admin.goto(`${BASE}/inbox/${number}`);
      const overview = (await admin.getByRole("region", { name: "Overview" }).innerText()).replace(
        /\s+/g,
        " ",
      );
      ...
      const contract = await api(admin, "GET", `/api/v1/contracts/${contractNumber}`);
      expectThat(
        JSON.stringify(contract.body).includes(F.contractType),
        "contract type is not the chosen live type",
      );
      ...
      await admin
        .getByRole("button", { name: `Archive ${F.targetType}`, exact: true })
        .waitFor({ timeout: 15000 });
  ...
  await step(
    SC,
    role,
    "Archive a request type: the dialog reports no usage and no replacement; the Portal drops the card and the form; the Request stays readable; Show archived > Restore offers it again",
    "Archive removes the card and form from the Portal; the earlier Request still opens for the Requester; Restore brings the card back.",
    async () => {
      currentPage = business;
      await business.goto(`${BASE}/portal`);
      await business
      ...
      await business
        .getByRole("textbox", { name: "Title (required)" })
        .fill(`${G} Open request on archived form ${stamp}`);
      await business
        .getByRole("textbox", { name: "Description (required)" })
        .fill("Fictional DOC-029 walkthrough request. No real work is needed.");
      await business.getByRole("button", { name: "Submit request" }).click();
      ...
      results.records.push({
        kind: "request (left open)",
        name: `${G} Open request on archived form ${stamp}`,
        number: openNumber,
      });
      currentPage = admin;
      await openIntake(admin);
      ...
      expectThat(
        cardGone && formLanded !== formPath,
        `card gone ${cardGone}, form landed ${formLanded}`,
      );
      expectThat(
        reqText.includes(`${G} Open request on archived form ${stamp}`),
      ...
      await admin
        .getByRole("button", { name: `Archive ${F.targetForm}`, exact: true })
        .waitFor({ timeout: 15000 });
      currentPage = business;
      let cardBack = 0;
      ...
      expectThat(
        enabledSelects === 0 && /not used by any requests/.test(dtext),
        `archive dialog: ${dtext}`,
      );
  ...
  await step(
    SC,
    role,
    "Check the result: have Legal convert the Matter form Request to a Matter and check the intended answer on the new record",
    "The Convert dialog lists no answer under Does not carry into the matter, and the new Matter holds the Entity answer and the Request's title.",
    async () => {
      currentPage = admin;
      const d = await openConvert(admin, S.matterRequest, "matter");
      await d.getByRole("combobox", { name: "Matter type" }).selectOption({ label: F.matterType });
      await pause(1500);
      const templates = await d
        .getByRole("combobox", { name: "Matter template" })
        .locator("option")
        .allInnerTexts()
        .catch(() => []);
      const text = (await d.innerText()).replace(/\s+/g, " ");
      const notCarry = text.match(/Does not carry into the matter.{0,160}/)?.[0] ?? null;
      ...
      const entityValue =
        Object.entries(cf).find(([k]) => k.includes("supplier_entity"))?.[1] ?? null;
      await admin.goto(`${BASE}/matters/${number}`);
      await pause(2000);
      const main = (await admin.locator("main").innerText()).replace(/\s+/g, " ");
      const shown = main.includes(F.entityField) && main.includes(S.portalEntityChoice);
      expectThat(!notCarry, `an answer stayed behind: ${notCarry}`);
      expectThat(
        entityValue && m.title === S.matterRequestTitle,
        `matter title ${q(m.title)}, custom fields ${q(cf)}`,
      );
      expectThat(shown, "the Matter page does not show the Entity answer");
      results.records.push({ kind: "matter", from: `R-${S.matterRequest}`, number });
      return `Convert R-${S.matterRequest} to a matter with Matter type ${q(F.matterType)} (template choices ${q(templates)}, left at the default) showed no "Does not carry into the matter" list. Convert created M-${number}; its title is ${q(m.title)}, the API holds a value for ${q(F.entityField)}, and the Matter page shows ${q(F.entityField)} with ${q(S.portalEntityChoice)}.`;
    },
  );
  ...
  await step(
    SC,
    role,
    "Remove the walkthrough's deflection links and turn off Portal-listed on its Entities",
    "Remove deletes each link at once with no archive; the Knowledge Item stays published; the Entities leave Portal pickers.",
    async () => {
      currentPage = admin;
      await openIntake(admin, "Deflection links");
      const removed = [];
      ...
      const older = await admin
        .getByRole("button", { name: new RegExp(`^Remove ${escapeRe(G)} `) })
        .evaluateAll((els) => els.map((e) => e.getAttribute("aria-label").replace(/^Remove /, "")));
      ...
      const leftovers = await admin
        ...
        .count();
      currentPage = business;
      await business.goto(`${BASE}${S.kiPath}`);
      ...
      for (const e of list.filter((x) => (x.legalName ?? "").startsWith(`${G} Portal supplier`))) {
        ...
        if ((await sw.getAttribute("aria-checked")) === "true") {
          await sw.click();
          await until(
            async () => (await sw.getAttribute("aria-checked")) === "false",
            "Portal-listed did not turn off",
          );
        }
        entities.push(e.legalName);
      }
      expectThat(
        leftovers === 0 && archivedSwitch === 0,
        `links left ${leftovers}, archive switch ${archivedSwitch}`,
      );
      return `Removed this run's links ${q([F.extLabel, F.kiLabel])} and ${removed.length - 2} links left by earlier development runs of this script; after reload no ${G} link remains and the pane has no Show archived switch. The Knowledge Item still opens on the Portal with heading ${q(kiStill)}. Portal-listed is now off for ${q(entities)}.`;
    },
  );
}
...
async function addLeadTime(page, days) {
  ...
  const open = page
    .getByRole("listitem")
    .filter({ has: page.getByRole("spinbutton", { name: "days before the date" }) });
  if (await open.count()) return (await open.innerText()).replace(/\s+/g, " ").trim();
  return null;
}
// A refused draft stays open; Cancel closes it (Add lead time is unavailable while a draft is open).
async function closeAddRow(page) {
  const cancel = page.getByRole("button", { name: "Cancel", exact: true });
  if (await cancel.count()) {
    await cancel.click();
    await page
      .getByRole("spinbutton", { name: "days before the date" })
      .waitFor({ state: "detached", timeout: 10000 });
  }
}
...
async function setTimezone(page, zone) {
  ...
  await page
    .getByRole("option", { name: new RegExp(escapeRe(zone.split("/").pop().replace(/_/g, " "))) })
    .first()
    .click();
  await pause(1500);
...
async function addKeyDate(
  page,
  matterNumber,
  { date, event, extraLead = null, onlyRecipient = null },
) {
  ...
  if (extraLead !== null) {
    await d
      .getByRole("spinbutton", { name: "Additional lead time (days before)" })
      .fill(String(extraLead));
...
async function remindersSection(admin, browser) {
  ...
  await step(
    SC,
    role,
    "Set reminder lead times, steps 1-2: profile menu > Settings > Organization > Notifications opens Reminder lead times; Personal > Notifications is a different page",
    "Organization > Notifications shows Reminder lead times; Personal > Notifications shows the person's own preferences.",
    async () => {
  ...
  await step(
    SC,
    role,
    "Steps 3-5: Add lead time 9 with Save, Cancel discards a draft, refuse a duplicate and 731, reorder with the arrow keys, reload and confirm; check the 20-entry limit and that the last entry cannot be removed",
    "9 days before is added; a duplicate and 731 are refused; the reordered list persists; a 21st entry is refused; the last entry has no working Remove.",
    async () => {
      await openSettings(admin, "Organization", "Notifications");
      ...
      expectThat(!afterCancel.includes("33 days before"), `Cancel saved 33: ${q(afterCancel)}`);
      const addAlerts = await addLeadTime(admin, 9);
      await admin
        .getByRole("button", { name: /^Reorder 9 days before, position/ })
      ...
      expectThat(
        listAfterDup.filter((l) => l === "9 days before").length === 1,
        `duplicate added: ${q(listAfterDup)}`,
      );
      ...
      await admin.keyboard.press("ArrowUp");
      await until(
        async () => (await reorderPosition(admin, "9 days before")) === before - 1,
        "lead time did not move",
      );
      await pause(1500);
      await admin.reload();
      const reloaded = await leadTimes(admin);
      expectThat(reloaded.indexOf("9 days before") === before - 2, `reloaded ${q(reloaded)}`);
      await admin.screenshot({ path: path.join(SHOTS, "r2-1-reminder-lead-times.png") });
      // 20-entry limit.
      const extra = [];
      ...
      expectThat(
        after21.length === 20 && !after21.includes("90 days before"),
        `21st entry saved: ${after21.length}`,
      );
  ...
  await step(
    SC,
    role,
    "Check delivery, part 1: prepare two eligible fictional Legal Team Members in different saved timezones, a Matter for each, and Key dates at chosen distances",
    "Invitations activate; timezones save; Key dates show the lead times each will use.",
    async () => {
      const morning = ZONES.find((z) => localHour(z) >= 9 && localHour(z) <= 22);
      const early = ZONES.find((z) => localHour(z) < 7);
      expectThat(
        morning && early,
        `no zone pair for this hour: ${q(ZONES.map((z) => [z, localHour(z)]))}`,
      );
      const users = {};
      for (const [key, zone] of [
        ...
        users[key] = {
          ...
          ctx,
          page,
          id: me.body?.user?.id ?? me.body?.id,
          localHour: localHour(zone),
      ...
      const plan = [
        ...
        {
          event: `${G} KD own three to admin only ${stamp}`,
          days: 3,
          extraLead: 3,
          onlyRecipient: ADMIN.displayName,
          expect: false,
        },
      ];
      const reminds = [];
      for (const k of plan) {
        ...
        k.remind = await addKeyDate(admin, r.matter, {
          date: k.date,
          event: k.event,
          extraLead: k.extraLead ?? null,
          onlyRecipient: k.onlyRecipient ?? null,
        });
        reminds.push(
          `${k.event.replace(`${G} `, "").replace(` ${stamp}`, "")} on ${k.date}: ${k.remind}`,
        );
      }
      const e = users.early;
      const earlyKd = {
        event: `${G} KD early seven ${stamp}`,
        days: 7,
        date: localDate(e.zone, 7),
      };
  ...
  await step(
    SC,
    role,
    "Check delivery, part 2: run the morning round in the lab worker, then check each colleague's bell and briefing email",
    "The past-08:00 colleague gets one reminder for each listed Key date, including both Key dates on the same date and the Key date's own lead time; nothing for 11 days or for the Key date that selects only another recipient. The before-08:00 colleague gets nothing yet.",
    async () => {
      ...
      const observed = {};
      for (const k of S.keyDatePlan)
        observed[k.event] = items
          .filter(
            (n) => n.eventType === "date.key_date_approaching" && n.payload?.label === k.event,
          )
          .map((n) => `offset ${n.payload.offsetDays}`);
      // Briefing mail.
      let mail = null;
      for (let i = 0; i < 40 && !mail; i++) {
        const search = await fetch(
          `${MAIL}/api/v1/search?query=${encodeURIComponent(`to:"${r.email}"`)}`,
        ).then((x) => x.json());
      ...
      const earlySearch = await fetch(
        `${MAIL}/api/v1/search?query=${encodeURIComponent(`to:"${e.email}"`)}`,
      ).then((x) => x.json());
      const earlyMails = (earlySearch.messages ?? []).filter((m) =>
        /briefing/i.test(m.Subject),
      ).length;
      ...
      expectThat(
        wrong.length === 0,
        `unexpected reminder outcome for ${q(wrong.map((k) => k.event))}; bell ${q(observed)}; mail ${q(mailHits)}`,
      );
      expectThat(
        !earlyHasIt && earlyMails === 0,
        `early colleague was served: bell ${earlyHasIt}, mails ${earlyMails}`,
      );
  ...
  await step(
    SC,
    role,
    "Restore the organization lead time list and confirm it after reload",
    "The list matches the list recorded before the walkthrough.",
    async () => {
      await openSettings(admin, "Organization", "Notifications");
      let list = await leadTimes(admin);
      for (const l of list)
        if (!S.originalLeadTimes.includes(l)) {
          await admin.getByRole("button", { name: `Remove ${l}`, exact: true }).click();
          await admin
            .getByRole("button", { name: `Remove ${l}`, exact: true })
            .waitFor({ state: "detached", timeout: 10000 });
        }
      ...
      for (let i = 0; i < S.originalLeadTimes.length; i++) {
        ...
        while (pos - 1 > i) {
          ...
          await admin.keyboard.press("ArrowUp");
          await until(
            async () => (await reorderPosition(admin, label)) === pos - 1,
            "restore reorder failed",
          );
          pos -= 1;
        }
      }
      ...
      expectThat(
        q(final) === q(S.originalLeadTimes),
        `final ${q(final)} vs ${q(S.originalLeadTimes)}`,
      );
      expectThat((await leadTimes(admin)).length > 0, "empty list");
      expectThat(
        !lastOutcome.startsWith("Remove clicked") || /list \["/.test(lastOutcome),
        "last entry removed",
      );
...
async function auditRows(page) {
  ...
  return (await table.getByRole("row").allInnerTexts())
    .slice(1)
...
async function auditSection(admin, member) {
  ...
  await step(
    SC,
    role,
    "Find a change in the Audit log, steps 1-2: Settings > Security > Audit log; combine Person, Action, and Record to find the walkthrough's lead time changes",
    "Entries by Daniel Okafor for org_settings.updated on System show the before and after lead times, including 9 days.",
    async () => {
      ...
      await admin
        ...
        .selectOption({ label: "org_settings.updated" });
      ...
      expectThat(
        rows.length > 0 && rows.every((r) => r.startsWith(ADMIN.displayName)),
        `rows ${q(rows.slice(0, 3))}`,
      );
      expectThat(nine.length > 0, "no lead time entry with 9 days");
      S.auditPath = pathName;
      await admin.screenshot({ path: path.join(SHOTS, "r2-1-audit-log-filtered.png") });
  ...
  await step(
    SC,
    role,
    "Steps 3-5: narrow with From and To in the browser's local days and with Search; read event, record, audience, and time; Show older; Clear filters",
    "Today's bounds keep the entries; a range that ends yesterday removes them; Search narrows to the walkthrough's Matter type; Show older adds rows; Clear filters resets every filter.",
    async () => {
      ...
      const yText = (await admin.getByText("No entry matches these filters.").count())
        ? "No entry matches these filters."
        : null;
      expectThat(todayRows.length > 0, "no rows today");
      expectThat(
        yRows.length === 0 || yRows.every((r) => /^No /.test(r)),
        `rows for yesterday ${q(yRows.slice(0, 2))}`,
      );
      ...
      const cleared = {
        ...
        action: await admin
          ...
          .innerText(),
        record: await admin
          .getByRole("combobox", { name: "Record" })
          .locator("option:checked")
          .innerText(),
      ...
      expectThat(
        cleared.person === "Anyone" &&
          cleared.action === "Any action" &&
          cleared.record === "Any record" &&
          !cleared.from &&
          !cleared.to &&
          !cleared.search,
        `cleared ${q(cleared)}`,
      ...
      await older.click();
      await until(
        async () => (await admin.getByRole("table").getByRole("row").count()) - 1 > firstPage,
        "Show older added no rows",
      );
      ...
      expectThat(
        searched.length > 0 && searched.every((r) => r.includes(G)),
        `search rows ${q(searched.slice(0, 3))}`,
      );
  ...
  await step(
    SC,
    role,
    "Export CSV applies the current filters",
    "The downloaded file has a header and only entries that match the filters.",
    async () => {
      await admin
        .getByRole("combobox", { name: "Action" })
        .selectOption({ label: "org_settings.updated" });
      ...
      expectThat(
        body.length > 0 &&
          body.every(
            (l) => l.includes("org_settings.updated") || l.includes("organization settings"),
          ),
        `csv rows ${q(body.slice(0, 2))}`,
      );
      await admin.getByRole("button", { name: "Clear filters" }).click();
  ...
  await step(
    SC,
    "legal_team_member+administrator",
    "Entries about a Confidential Matter appear only when the Administrator is in its audience, in the page and in Export CSV",
    "While Nadia Haddad's Confidential Matter excludes Daniel Okafor, Search finds none of its entries and the CSV has none; after Nadia adds Daniel to the Matter team, its entries appear.",
    async () => {
      currentPage = member;
      ...
      const csvHiddenHits = csvHidden.lines
        .slice(1)
        .filter((l) => l.includes(title) || l.includes(`M-${number}`)).length;
      expectThat(
        hiddenEntries.length === 0 && csvHiddenHits === 0,
        `hidden entries shown: page ${hiddenEntries.length}, csv ${csvHiddenHits}`,
      );
      ...
      await until(
        async () => (await api(admin, "GET", `/api/v1/matters/${number}`)).status === 200,
        "Daniel still cannot read the Matter",
      );
      currentPage = admin;
      await admin.getByRole("button", { name: "Clear filters" }).click();
      await admin.getByRole("searchbox", { name: "Search" }).fill(title);
      ...
      const csvShownHits = csvShown.lines
        .slice(1)
        .filter(
          (l) => l.includes(title) || l.includes(`M-${number}`) || l.includes("matter.created"),
        ).length;
      expectThat(
        shown.length > 0 && csvShownHits > 0,
        `entries still hidden: page ${shown.length}, csv ${csvShownHits}`,
      );
      await admin.getByRole("button", { name: "Clear filters" }).click();
...
// =====================================================================
// Negative access checks (V-C38, V-C40, V-C41)
// =====================================================================
async function accessSection(member, business) {
  const pages = [
    [
      ...
      "POST",
      "/api/v1/matter-types",
      { displayName: `${G} refused type ${stamp}` },
    ],
    [
      "V-C38",
      "/settings/matters/fields",
      "POST",
      "/api/v1/fields",
      {
        displayName: `${G} refused field ${stamp}`,
        fieldType: "text",
        moduleScope: "matter",
        fieldTag: "business",
      },
    ...
    [
      ...
      { displayName: `${G} refused request type ${stamp}` },
    ],
    ["V-C41", "/settings/reminders", "PUT", "/api/v1/org/reminder-offsets", { offsets: [1] }],
    ["V-C41", "/settings/audit-log", "GET", "/api/v1/audit-log", null],
  ];
  for (const [who, page, label] of [
    ["legal_team_member", member, MEMBER.displayName],
    ["business_user", business, BUSINESS.displayName],
  ]) {
    currentPage = page;
    const me = await api(page, "GET", "/api/v1/me");
    ...
    for (const [sc, url, method, apiUrl, body] of pages) {
      await step(
        sc,
        who,
        `Negative: ${label} (${who}) opens ${url} and calls ${method} ${apiUrl}`,
        "The Settings page does not open for this role and the API refuses the call.",
        async () => {
...
try {
  ...
  await step(
    ...
    "The app opens with the Administrator's name in the header.",
    async () => {
  ...
  if (process.env.DEBUG)
    admin.on("request", (r) => {
      if (r.url().includes("/api/") && r.method() !== "GET")
        console.log(new Date().toISOString(), "REQ", r.method(), r.url());
    });
  ...
  await step(
    "all",
    "legal_team_member",
    "Sign in as the seeded Legal Team Member in a separate browser context",
    "The app opens with the Legal Team Member's name in the header.",
    async () => {
  ...
  await step(
    "all",
    "business_user",
    "Sign in as the seeded Business User with a fresh emailed sign-in link in a separate browser context",
    "The Portal opens.",
    async () => {
