// DOC-030 compatibility replay (set compat), article configure-signing (V-C42), credential-free checks only. Copy of
// DOC-029/compat-r2/live-provider-r2/credential-free-checks-r2.mjs. DOC-030 changes: repository root depth, default lab URL
// (the separate compatcmp lab, 43345), output path. S04 and S07 read the HMAC and keep/rotate hints from the field's
// help icon instead of text under the field (see fieldHelp). No step, value or other check changed. No request goes to DocuSign.
...
const BASE = process.env.LAB_APP_URL ?? "http://127.0.0.1:43345";
const ADMIN = {
...
const OUT = process.env.OUT ?? path.join(here, "configure-signing-replay.json");
...
// DOC-030 harness change: at 067c1646 the E-signature pane shows field hints behind a
// "More information" help icon beside the label (#1040), not as text under the field.
// Read the hint the way a reader does: focus the field's help icon and read the tooltip.
async function fieldHelp(page, fieldId) {
  const labelId = await page.locator(`label[for="${fieldId}"]`).getAttribute("id");
  const button = page.locator(`button[aria-describedby^="${labelId} "]`);
...
try {
  ...
  await step(
    ...
    async () => {
      ...
      const hmacHint = ((await fieldHelp(page, "ds-webhook-secret")) ?? "").startsWith(
        "Required. OpenLaw checks it on every delivery",
      );
      ...
      return `Webhook shows Public callback URL and Connect HMAC secret with the help-icon hint "Required. OpenLaw checks it on every delivery...". Webhook hint: "${hint}". With the HMAC secret blank the browser refused the submit and no request was sent.`;
    },
  );
  ...
  await step(
    ...
    async () => {
      ...
      const keepHint =
        (await fieldHelp(page, "ds-private-key")) ===
        "Leave blank to keep the current value. Paste a new one to rotate.";
      ...
      return "The chip reads Connected and Send for signature from records is on. After reload the RSA private key input is blank and its help icon shows the keep/rotate hint. Test connection is enabled. No Webhook URL row in Polling mode.";
    },
  );
