...
| [convert-gaps](independent-convert-gaps.json)                                 | V-C40                         |            2 |               1 |
...
| [repair-archived-target](independent-repair-archived-target.json)             | V-C40                         |            4 |               0 |
...
The independent pass now holds 119 passing steps and 55 retained failed attempts across 63 records; the table above is generated from those records. Every failure is a reviewer helper mistake, never a relabelled product result: asynchronous required-checkbox state that a plain `check()` could not settle; a submit control named **Create** rather than "Create matter"; select values read as page text; the key-dates envelope key; row-action controls that are buttons in one pane and links in another; the request detail path; a Portal form that confirms in place instead of navigating; the audit Search box colliding with the global Search; an **Export CSV** link read as a button; a member picker whose checkboxes carry no aria-label; and duplicate fixtures left by interrupted attempts.

The reclassification arithmetic was checked against the commits rather than recalled. Measured from `b8265f8f`, which held 55 records at 121 passing and 37 failed steps: 18 steps that had been recorded as passes are now `fail`, and 8 new records contribute 16 passing steps and no failures, which gives 121 − 18 + 16 = 119 and 37 + 18 + 0 = 55. Seventeen of those 18 were relabelled in the first repair pass; the eighteenth is the archived-target conversion step described below. Reclassifications made earlier than `b8265f8f`, during the walkthroughs themselves, are already inside that baseline's 37 and are not counted again here.

Each relabelled step keeps its original text and timestamps and gains a `supersededBy` link to the record that establishes the claim properly. What they had in common was a sentence standing in for an assertion: audit row counts that printed −1 for an empty table; two audit date-bound steps whose captured query did not correspond to the inputs they described, one a transient hybrid taken between two keystrokes; a form attachment list read from an envelope key the endpoint does not answer, ending in `[]` against the step's own opening sentence; an approver-group membership write that used `userIds` where the route takes `memberIds`, so its 400 proved request validation rather than the role rule, alongside a six-name picker sample presented as the whole offer; two group rosters read as `approverName`, leaving the skipped and restored people as `null`; a request-type step claiming a stable identity while recording `requestTypeId unchanged: false`; an `isDisabled()` call on static text recorded as `disabled=null`; a template step recording `Tasks card (false)` yet passing; a section reading that captured the Overview for both Tasks and Key dates; five Portal steps whose "no Request was created" clause matched an earlier probe's summary; and the archived-target step, which counted a picker, sampled `isDisabled()` on the confirm control and then cancelled without ever choosing a type, so its "disabled=false until a live type is chosen" asserted nothing at all.
...
The eight `repair-*` records re-establish every superseded claim, and each of their steps throws when the stated outcome is not observed. The audit bounds come from an awaited response whose query carries exactly the instants the two date controls were set to, checked in both directions; the group checks name people by id and compare whole rosters before and after; the picker list is verified complete against the user list rather than sampled, with a well-formed `memberIds` write drawing the real 422 role refusal; the Portal refusal is proved against a unique summary no other Request carries; the field type is shown to be static text with no control at all; the template anchor is asserted offset by offset against the new Matter's own creation instant; and the archived target is carried through to the end — a blank picker is refused by the inline message "Pick a contract type." with no convert request sent and the Request still `new`, then an explicit live type converts it into a Contract of exactly that type, on the reviewer's own fictional type so no shared seed is archived.
