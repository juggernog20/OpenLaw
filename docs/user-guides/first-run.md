...
4. In **Outbound email**, check whether email is set by the deployment environment. If it is unset, enter the **SMTP server**, **Port**, **Connection security**, **Authentication**, and **Sender email** (plus an optional sender name), then select **Save relay**. Select **Send test email** and check your inbox before inviting anyone. See [email setup and recovery](authentication-and-email.md#set-up-email-before-finishing-the-wizard).
