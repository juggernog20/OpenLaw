...
export default async function matterWork(ctx) {
  ...
  await step(
    ...
    async () => {
      ...
      await d.getByLabel("Title").fill("DOC-029r2 work cancelled task");
  ...
  await step(
    ...
    async () => {
      await tasksRegion().getByRole("button", { name: "Add Task" }).click();
      const d = page.getByRole("dialog", { name: "Add a Task" });
      await d.getByLabel("Title").fill("DOC-029r2 work review task");
      await d.getByLabel("Description").fill("Fictional task description.");
      await stagePerson(d.getByRole("button", { name: "Assignee" }), "Tom Iwu", "Use this person");
      await d.getByLabel("Due date").fill(day(10));
      await d.getByLabel("Add a note").fill("DOC-029r2 fictional initial note.");
      await d.getByRole("button", { name: "Add Task" }).click();
      await d.waitFor({ state: "hidden", timeout: 20000 });
      const t = await tasks();
      const review = t.tasks.find((x) => x.title === "DOC-029r2 work review task");
      const team = await teamNames();
      const comments = (
      ...
      expectThat(
        !recordComments.some((c) => JSON.stringify(c).includes("DOC-029r2 fictional initial note")),
        "note on record conversation",
      );
      return `Saved "DOC-029r2 work review task": assignee ${review.assigneeName}, due ${review.dueDate}; team now ${q(team)}. The Task conversation holds ${comments.length} comment with visibility ${comments[0].visibility}; the record conversation (${recordComments.length} comments) does not contain the note.`;
    },
  );
  ...
  await step(
    ...
    async () => {
      await api(page, "POST", `/matters/${s.w}/tasks`, { title: "DOC-029r2 work plain task" });
      await api(page, "POST", `/matters/${s.w}/tasks`, {
        title: "DOC-029r2 work empty task",
        dueDate: day(5),
      });
      await gotoTab("tasks");
      const rowButton = tasksRegion().getByRole("button", {
        name: /^Change assignee for DOC-029r2 work plain task/,
      });
      await stagePerson(rowButton, "Ines Duarte", "Add to team and assign");
      await page.waitForTimeout(800);
      const assigned = (await tasks()).tasks.find((x) => x.title === "DOC-029r2 work plain task");
      ...
      await page
        ...
        .click();
      await page.waitForTimeout(800);
      const cleared = (await tasks()).tasks.find((x) => x.title === "DOC-029r2 work plain task");
      const teamAfterClear = await teamNames();
      await tasksRegion()
        .getByRole("button", { name: "DOC-029r2 work plain task", exact: true })
        .click();
      ...
      const afterClose = (await tasks()).tasks.find((x) => x.title === "DOC-029r2 work plain task");
  ...
  await step(
    ...
    async () => {
      ...
      await tasksRegion()
        .getByRole("checkbox", { name: "Complete Task: DOC-029r2 work review task" })
        .click();
      await page.waitForTimeout(1200);
      const hidden = (await tasksRegion().getByText("DOC-029r2 work review task").count()) === 0;
      const count = (
        await tasksRegion()
          .getByText(/of \d+ done/)
          .innerText()
      ...
      const reopenBox = tasksRegion().getByRole("checkbox", {
        name: "Reopen Task: DOC-029r2 work review task",
      });
      const shown = await reopenBox.isVisible();
      await reopenBox.click();
      await page.waitForTimeout(1200);
      const reopened = (await tasks()).tasks.find((x) => x.title === "DOC-029r2 work review task");
      await tasksRegion()
        .getByRole("button", { name: "Actions for DOC-029r2 work plain task" })
        .click();
      await page.getByRole("menuitem", { name: "Edit Task" }).click();
      const details = page.getByRole("dialog", { name: "Task details" });
      await details.getByLabel("Title").fill("DOC-029r2 work renamed task");
      await details.getByRole("button", { name: "Save" }).click();
      await details.waitFor({ state: "hidden" });
      await tasksRegion()
        .getByRole("button", { name: "DOC-029r2 work renamed task", exact: true })
        .waitFor();
      const countAfter = (
        await tasksRegion()
          .getByText(/of \d+ done/)
  ...
  await step(
    ...
    async () => {
      await gotoTab("tasks");
      await tasksRegion()
        .getByRole("button", { name: "DOC-029r2 work review task", exact: true })
        .click();
      const details = page.getByRole("dialog", { name: "Task details" });
      await details.waitFor();
      const audienceText = (await details.getByRole("group", { name: "Audience" }).innerText())
        .replace(/\s+/g, " ")
        .trim();
      await details.getByText("DOC-029r2 fictional initial note.").waitFor();
      await details.getByRole("contentinfo").getByRole("button", { name: "Close" }).click();
      await details.waitFor({ state: "hidden" });
      const confirm = page
      ...
      await tasksRegion()
        .getByRole("button", { name: "Actions for DOC-029r2 work empty task" })
        .click();
      ...
      await tasksRegion()
        .getByRole("button", { name: "Actions for DOC-029r2 work review task" })
        .click();
      await page.getByRole("menuitem", { name: "Remove Task" }).click();
      await page.waitForTimeout(600);
      if (
      ...
      expectThat(
        !afterEmpty.includes("DOC-029r2 work empty task") &&
          afterHeld.includes("DOC-029r2 work review task") &&
          refusal.length > 0,
  ...
  await step(
    ...
    async () => {
      ...
      await tasksRegion().getByRole("button", { name: "Add Task" }).click();
      const d = page.getByRole("dialog", { name: "Add a Task" });
      await d.getByLabel("Title").fill("DOC-029r2 work retry task");
      await d.getByLabel("Add a note").fill("DOC-029r2 retry note.");
      let blocked = 0;
      ...
      const matches = (await tasks()).tasks.filter((x) => x.title === "DOC-029r2 work retry task");
      const comments = (
  ...
  await step(
    ...
    async () => {
      ...
      await d.getByLabel("Event").fill("DOC-029r2 work filing");
      ...
      await d.getByRole("button", { name: "Add date" }).click();
      await d.waitFor({ state: "hidden" });
      const saved = (await dates()).find((x) => x.label === "DOC-029r2 work filing");
      expectThat(
  ...
  await step(
    ...
    async () => {
      const past = await api(page, "POST", `/matters/${s.w}/key-dates`, {
        date: day(-2),
        label: "DOC-029r2 work overdue",
      });
      const tomOnly = await api(page, "POST", `/matters/${s.w}/key-dates`, {
        date: day(60),
        label: "DOC-029r2 work Tom only",
      ...
      await d.waitFor({ state: "hidden" });
      const filing = (await dates()).find((x) => x.label === "DOC-029r2 work filing");
      await datesRegion()
        .getByRole("button", { name: "Actions for DOC-029r2 work Tom only" })
        .click();
      await page.getByRole("menuitem", { name: "Edit date" }).click();
      await d
        .getByText("Some selected recipients have left the team and will not be reminded.")
      ...
      await d.getByRole("button", { name: "Save" }).click();
      await d.waitFor({ state: "hidden" });
      const cleared = (await dates()).find((x) => x.label === "DOC-029r2 work filing");
      expectThat(
  ...
  await step(
    ...
    async () => {
      ...
      await d.waitFor({ state: "hidden" });
      await datesRegion()
        .getByRole("button", { name: "Actions for DOC-029r2 work overdue edited" })
        .click();
      ...
      const kept = (await dates()).some((x) => x.label === "DOC-029r2 work overdue edited");
      await datesRegion()
        .getByRole("button", { name: "Actions for DOC-029r2 work overdue edited" })
        .click();
      await page.getByRole("menuitem", { name: "Remove date" }).click();
      ...
      await confirm.waitFor({ state: "hidden" });
      await page.waitForTimeout(800);
      const removed = !(await dates()).some((x) => x.label === "DOC-029r2 work overdue edited");
      const openDeadline = (await matterByNumber(page, s.w)).matter.nextDeadline;
      const close = await api(page, "PATCH", `/matters/${s.w}`, {
        statusId: closedId,
        closingNote: "DOC-029r2 closed for the Key date check.",
      });
      await gotoTab("key-dates");
      ...
      await d.getByRole("button", { name: "Save" }).click();
      await d.waitFor({ state: "hidden" });
      const closedNote = (await dates()).find((x) => x.label === "DOC-029r2 work filing").note;
  ...
  await step(
    ...
    async () => {
      ...
      const overdue = await api(page, "POST", `/matters/${s.w2}/tasks`, {
        title: "DOC-029r2 work overdue task",
        dueDate: day(-3),
      });
      await api(page, "POST", `/matters/${s.w2}/key-dates`, {
        date: day(4),
        label: "DOC-029r2 work upcoming date",
      });
      ...
      expectThat(
        byTitle >= 1 && overdue.status === 201 && deadline?.label === "DOC-029r2 work overdue task",
        `${byTitle} ${q(deadline)}`,
      );
