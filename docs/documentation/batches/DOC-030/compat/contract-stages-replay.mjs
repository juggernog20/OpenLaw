// DOC-030 compatibility replay (set compat), article contract-stages. Copy of DOC-029/compat-r2/contracts-a-r2/replay-r2.mjs,
// itself adapted from DOC-029/contracts-a/walkthrough-r1.mjs. DOC-030 changes: lab ports (work2 43300/48425),
// repository root depth, output path, reviewer label, record names DOC-029r2 to DOC-030 compat, default ONLY=contract-stages, lab manifest name from LAB_NAME (default work2),
// the Portal sign-in waits out the sign-in link budget (see magicSession),
// and the fixture Field attachment runs only for create-contract (see the DOC-030 note at the fixtures).
// DOC-029 round 2 header:
...
const MAIL = process.env.LAB_MAIL_URL ?? "http://127.0.0.1:48425";
const PASSWORD = process.env.LAB_PASSWORD;
if (!PASSWORD) throw new Error("LAB_PASSWORD is required");
const OUT = process.env.OUT ?? path.join(here, "contract-stages-replay.json");
const ONLY = (process.env.ONLY ?? "contract-stages").split(",");
...
const lab = JSON.parse(
  readFileSync(
    path.join(root, `.documentation-labs/${process.env.LAB_NAME ?? "work2"}/lab.json`),
    "utf8",
  ),
);
const results = {
...
// DOC-030 harness change: at 067c1646 the app allows 3 sign-in link requests per email address in
// 15 minutes (TECH-032). When the Portal says so, wait for the window to pass instead of failing.
...
async function magicSessionOnce(email, displayName) {
  ...
  const limited = page.getByText("Too many sign-in link requests. Try again later.");
  await page
  ...
  if (await limited.isVisible()) {
    await ctx.close();
    console.log(`sign-in link budget spent for ${displayName}; waiting 60 s`);
    await sleep(60000);
    throw new Error(`sign-in link budget spent for ${displayName}`);
  }
  let link;
...
async function cleanupFixtures() {
  ...
  for (const [label, base, key] of lists) {
    ...
    for (const row of rows.filter(
      (x) => /^DOC-030 compat contracts-a /.test(x.displayName ?? x.name ?? "") && !x.archivedAt,
    )) {
...
}
{
  const f1 = await S.daniel.api("POST", "/fields", {
    displayName: `DOC-030 compat contracts-a Required number ${stamp}`,
    moduleScope: "contract",
    fieldType: "number",
    fieldTag: "business",
  });
  const f2 = await S.daniel.api("POST", "/fields", {
    displayName: `DOC-030 compat contracts-a Notes ${stamp}`,
    moduleScope: "contract",
    fieldType: "long_text",
    fieldTag: "business",
  });
  const t = await S.daniel.api("POST", "/contract-types", {
    displayName: `DOC-030 compat contracts-a Required type ${stamp}`,
  });
  must(
  ...
  // DOC-030 harness change: only create-contract uses the Fields on the fixture type. At 067c1646
  // the per-type attach endpoint is gone (the type Form replaced it, M39), so the attachment runs
  // only when create-contract is selected. contract-stages uses the fixture Status alone.
  if (ONLY.includes("create-contract")) {
  ...
  const st = await S.daniel.api("POST", "/contract-statuses", {
    displayName: `DOC-030 compat contracts-a review ${stamp}`,
    stage: "review",
  });
  ...
  const g = await S.daniel.api("POST", "/approver-groups", {
    name: `DOC-030 compat contracts-a sign-off ${stamp}`,
...
async function quickContract(s, role, label, extra = {}) {
  const title = `DOC-030 compat contracts-a ${role} ${label} ${stamp}`;
...
async function createContract(role, A, O) {
  ...
  const sc = "V-C14";
  const page = A.page;
  const title = `DOC-030 compat contracts-a ${role} C14 ${stamp}`;
  let number;
  ...
  await step(
    ...
    async () => {
      ...
      await d.getByLabel("Title").fill(`DOC-030 compat contracts-a ${role} refusal ${stamp}`);
      await d.getByRole("button", { name: "Create", exact: true }).click();
      ...
      const made = (list.json.contracts ?? []).filter(
        (c) => c.title === `DOC-030 compat contracts-a ${role} refusal ${stamp}`,
      );
  ...
  await step(
    ...
    async () => {
      const main = page.getByRole("main");
      const newName = `DOC-030 compat contracts-a ${role} Counterparty ${stamp}`;
  ...
  await step(
    ...
    async () => {
      ...
      if (role === "legal_team_member")
        await page.screenshot({ path: path.join(here, "doc030-c14-change-contract-type.png") });
  ...
  await step(
    ...
    async () => {
      ...
      const post = await S.jonas.api("POST", "/contracts", {
        title: `DOC-030 compat contracts-a portal refusal ${stamp}`,
  ...
  await step(
    ...
    async () => {
      ...
      for (const [name, stage] of [
        ...
        const region = (
          await page
            .getByRole("region", { name: new RegExp(`DOC-030 compat contracts-a ${role} C15`) })
            .innerText()
        ).replace(/\s+/g, " ");
...
async function contractApprovals(role, A, O) {
  ...
  await step(
    ...
    async () => {
      ...
      const text = (await dialog.innerText()).replace(/\s+/g, " ");
      if (role === "legal_team_member")
        await page.screenshot({ path: path.join(here, "doc030-c16-move-past-approval.png") });
      await dialog.getByRole("button", { name: "Cancel" }).click();
      await dialog.waitFor({ state: "hidden" });
      await sleep(800);
      must(
...
async function manualSigning(role, A) {
  ...
  const draftBytes = pdf(`DOC-030 compat contracts-a ${role} draft ${stamp}`);
  const execBytes = pdf(
    `DOC-030 compat contracts-a ${role} executed ${stamp} signed by both parties`,
  );
  const laterBytes = pdf(`DOC-030 compat contracts-a ${role} later round ${stamp}`);
    ...
    async () => {
      ...
      try {
        const rendered = await reader
          .getByText(`DOC-030 compat contracts-a ${role} executed ${stamp} signed by both parties`)
  ...
  await step(
    ...
    async () => {
      ...
      if (role === "legal_team_member")
        await docsCard().screenshot({
          path: path.join(here, "doc030-c17-executed-designation.png"),
        });
...
try {
  for (const { role, A, O } of plan) {
    ...
    if (ONLY.includes("contract-stages"))
      await contractStages(role, A, O, `DOC-030 compat contracts-a renamed by ${role} ${stamp}`);
