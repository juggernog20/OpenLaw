...
**2026-09-11 amendment:** The thread still moves once with tiers preserved. DD-023 makes the converted Request address redirect to the record. Current team membership determines Business User Full Thread access, including old Request aliases. Being the original Requester grants no independent record access. The full-app converted Request has no thread or composer.
...
**The watermarks move with the rows, and only where they cannot clobber a real read state.** A `comment_last_read` row is one reader's place in one record's conversation (CMT-009). Left on the Request's pair after the comments left it, every reader's badge would count the whole thread as news and announce as new what they read yesterday. So each row is re-keyed onto the record's pair — unless that reader already holds one there, in which case theirs is a real place in a real conversation and the Request's must not overwrite it. The Request's rows then go, because the pair they name is not a comment target any more. **It is the address they follow rather than the rows**, so a Request whose thread was empty has its watermarks re-keyed too: once the back-link is written nothing reads a watermark on the Request's pair again, and one rule is better than two. **The conflict arm is unreachable in the Contract conversion written here** and is recorded anyway: conversion creates a new record, so nobody can already hold a watermark on it. The M22 Matter arm inherited the same rule; the close addendum below records that it remains defensive there too. A future move onto an existing record is the case that can exercise it.

**A comment racing the conversion cannot strand.** The conversion holds the Request's row `FOR UPDATE` while it moves the thread, and the `request` arm's resolve holds the same row in share mode. Every thread write resolves through that arm, so the two serialize: a post that overlaps the conversion either commits first and the move carries its row, or waits out the conversion and is answered the record's pair. Without the second lock a post could resolve the Request's pair just before the move and insert just after it, leaving a comment on a pair no address answers again. A plain read takes the share lock for one statement and gives it straight back, which at this product's scale costs nothing worth naming.
...
**The activity entries stay where they were written.** A `comment.posted` entry on the Request records what somebody did on the Request, and the log is append-only (DD-017): rewriting the entity of an appended row is the one thing that table does not do. The Request's feed goes on saying what was said there, the record's feed carries the entries written on it, and the move itself is a **new** entry — `request.thread_moved` on the Request, naming the record by its C-###, at the record tier and carrying no count. The count is left out on purpose: how many comments moved is how much was said at every tier, and that entry is one a Contributor reads (DD-016). A Request whose thread was empty writes no entry at all.

### Addendum (2026-08-23, M22 close, [#474](https://github.com/juggernog20/OpenLaw/issues/474)) — the thread follows a matter too

The re-parent now accepts a module-aware record reference and moves the same rows onto a Matter pair, with tiers, mentions, revisions, attachments, tombstones, and watermarks untouched. The Request audience arm follows `converted_matter_id` through the Matter audience just as it follows a Contract through the Contract audience: staff take the Matter's rooms; the Requester retains Full Thread at the stable portal address; and their replies land on the Matter. A Full Thread Matter reply raises the existing requester event once, beside the Matter roster event and with the actor excluded.

The watermark conflict branch remains defensive rather than newly reachable: both conversion arms create a new record, so no reader can already hold a destination watermark through an ordinary conversion. M22 proves that the generic branch works for a second module; a future move onto an existing record is still the case that can exercise the conflict.
...
### Addendum, 2026-09-24: an email may carry a comment's words, read at send time
...
**The tiers still hold, twice.** The composer's refusal is the first gate — a mention whose addressee hears no such tier is a 400 — and the seam re-applies the comment's tier over the audience it read, which is the gate no later call site can forget. The re-application is the requester side's: a Member+ hears every tier on every Request, so the seam skips the check on the Inbox side rather than asking a question whose answer is fixed. Both halves live behind the one seam either way, which is the property that matters — no call site decides a tier — and the group-5 arm is where the tier can narrow anything at all.

> **Corrected at the M21 close ([#423](https://github.com/juggernog20/OpenLaw/issues/423)).** This paragraph first said the second gate "is asked anyway", by analogy with the wall being asked on a record with no wall. The seam does not ask it: `requestReachedBy` evaluates the tier only when the side is the requester's. The behaviour is the same and the reason was wrong, which matters because that reason is what a later group-1 event on a Request would be built against.
...
## CMT-011 — Paper arrives with the form or with a comment, and a Member+ files it onto the record (extends CMT-001)

- **Status** — Accepted
- **Date** — 2026-08-22
- **Context** — CMT-001 gave the thread an address that follows the work, so a requester's words go on reaching Legal after a conversion. Their paper does not. Promotion is a one-time copy taken inside the disposition transaction (the INT-002 M21/10 addendum), and a file attached to a Request afterwards sits where nobody reads it ([#438](https://github.com/juggernog20/OpenLaw/issues/438)). The loop that breaks is the ordinary one: Legal drafts, the business sends the draft to the counterparty, the counterparty marks it up and sends it back, and the business has nowhere to put it. Comments carry no attachments today and the code says so in as many words. This record decides that they should, and what happens to one after it arrives.
- **Decision** —
  - **Paper arrives with the form, or with a comment, and there is no third way in.** A Request's own attachments stay what the INT-002 M20/6 addendum made them, because there is no thread before there is a Request. Everything after that moment arrives on the thread. A business user learns one act rather than two, which is the whole reason to prefer this over a second upload control on the portal: they should not have to know which kind of file they are holding before they can send it.
  - **An attachment takes its comment's tier by being part of the comment.** A Legal Only note keeps its paper legal only. There is no tier of its own to set, to drift, or to leak through — the same reason a tier is immutable after posting (CMT-005). An attachment is part of what was said rather than a thing standing beside it.
  - **A comment attachment is not a document** (DOC-008). It is a stored blob and the name it arrived under, exactly as a request attachment is: no version chain, no folder, no confidentiality flag, and nothing to edit. A comment owns no record, so there is nothing for a document to hang off.
  - **One comment carries at most five attachments.** A bound rather than none, for the M20/6 reason: an upload address a Business User can reach is unbounded disk. Five is generous for a message, and paper that needs more than five files is a filing rather than a remark.
  - **A Member+ files it onto the record, and filing has two destinations** — a new document at the record root, or a new version on a chain already there. The second is the case this record exists for: the counterparty's markup is round four of the agreement, not a file sitting next to it. The first is everything else.
  - **The kind (CTR-014) is named at the moment of filing, by the person filing.** Nobody upstream is asked what the round is. A requester cannot know which round a markup marks up, and asking them would be the classification intake keeps out (DD-018). The filer is looking at the file and at the chain, which is the first moment anybody in the room can answer honestly.
  - **A file is filed once, and the comment says where it went.** The attachment stays on the thread afterwards, because the thread is a record of what was said and filing does not rewrite it. It carries the document and the version it became. Without that, two readers of one thread file one file twice and the chain grows a duplicate round.
  - **Filing from a Legal Only comment proposes a Confidential document** (DD-014), and the filer may say otherwise. The tier said the paper was legal's; a default that dropped the wall on the way to the record would make the tier mean less than it said.
  - **A redact takes the attachment with the text** (CMT-006, CMT-008), and takes the blob with it: what was posted into the wrong record includes the file. **A document already filed from it stays.** It is the record's own by then, put there by a deliberate act, which is the rule promotion already follows — a copy does not answer for its source.
- **Rationale** — The thread is the only address in the product that follows the work, and paper is the one thing it could not carry. Everything else here falls out of that. Keeping the attachment inside the comment means the tier, the corrections, and the audience machinery all apply to it without a second set of rules. Keeping it out of `documents` means a remark with a file on it does not become a record's paper by accident. Putting the filing act in a lawyer's hands is what makes the kind honest, which is the thing promotion could never manage: it copied files at a moment when nobody was reading them.
- **Alternatives considered** — **Promote on arrival, matching CMT-001's re-parent**: rejected, and it was the shape the M21 close guessed at. A file promoted on arrival lands as a new document of an unknown kind with nobody in the room, and the record's document list fills with every scrap anybody attached. **A second upload control on the portal request detail**: rejected, it is two acts for one intention and it leaves the business user deciding which one they are performing. **Refusing post-conversion paper with no replacement**: rejected, it closes the loop the product exists to run. **A tier on the attachment of its own**: rejected, two tiers on one post is a leak with a user interface.
- **Consequences** — `comment_attachments` lands in M21A (TECH-014), with the DOC-012 key rules and the one upload module documents and request attachments already share. Filing is a route on the comment that writes through the one version write path, so a filed round is indistinguishable from an uploaded one once it is on the chain, and it narrates what an upload narrates. The portal composer gains the control, and `POST /requests/{number}/attachments` starts refusing a dispositioned Request in the same milestone (the INT-002 addendum of this date) — a door and the wall beside it, shipped together. CTR-014 gains a correctable kind by the addendum of this date, because a filer names a round somebody else sent. Every thread the audience seam answers gets this at once (CMT-010): Contract, Request, Document, and the Matter arm added in M22.

## Index of decisions

| #       | Decision                                                                       | Status                                                                                                                                                                                                                                                                                                                                                                                                            |
| ------- | ------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| CMT-001 | One comment system; anchored doc comments; thread follows the work             | Accepted (extended by CMT-010 and CMT-011; the re-parent built by the M21/11 addendum)                                                                                                                                                                                                                                                                                                                            |
...
| CMT-006 | Prior comment text lives in `comment_revisions`, not the activity log          | Accepted (extended by CMT-008; comment words in email read at send time by the 2026-09-24 addendum)                                                                                                                                                                                                                                                                                                               |
