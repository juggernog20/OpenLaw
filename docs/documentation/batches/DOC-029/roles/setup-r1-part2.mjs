...
const mr = await matterFor(n.page, { title: "DOC-029 roles Ravi matter", matterTypeId: commercial, managerId: fx.ids.nadia, departmentId: sales, region: "EMEA", description: "DOC-029 roles fictional Matter description." });
fx.raviMatter = { number: mr.number, id: mr.id };

fx.pagingMatters = [];
for (let i = 1; i <= 25; i++) {
  const m = await matterFor(n.page, { title: `DOC-029 roles paging matter ${String(i).padStart(2, "0")}`, matterTypeId: commercial, managerId: fx.ids.nadia });
...
const dm = must(await api(n.page, "POST", "/matters", { title: "DOC-029 roles document paging matter", matterTypeId: commercial, managerId: fx.ids.nadia }), "DM").matter;
must(await api(n.page, "POST", `/matters/${dm.number}/team`, { userId: fx.ids.ravi }), "DM team");
fx.docPagingMatter = { number: dm.number, id: dm.id };
for (let i = 1; i <= 51; i++) await upload(n.page, "matters", dm.number, `doc029-roles-page-${String(i).padStart(2, "0")}.txt`, `DOC-029 roles fictional paging paper ${i}.\n`);
