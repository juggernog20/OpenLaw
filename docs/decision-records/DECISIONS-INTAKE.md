...
- **Decision** — A **Conversion draft** is a durable, actor-scoped proposal before ordinary conversion. An Administrator opts into Matter preparation through the AI connector's independently persisted switch, off by default and subordinate to connector enablement. Settings are checked at scheduling, execution, publication and acceptance. Opening Convert to matter shows “Getting matter ready…” before the editable Convert dialog. Cancelling or continuing manually remains available; only submitting the dialog dispositions the Request.
  ...
  - Broadly visible proposals use only Full Thread messages and public scalar Request answers. Legal Only and Working Team messages, Legal Fields and reference Fields are collected with their source identities but excluded from this provider call. This conservative rule prevents restricted text contaminating a synthesized public value even when the provider gives it a public citation. Authorized triagers see an omission notice; no restricted observations are persisted in proposals. Attachments are disclosed as unread until #827. Inputs are bounded to 200 public sources and 180,000 characters, with exclusions disclosed.
