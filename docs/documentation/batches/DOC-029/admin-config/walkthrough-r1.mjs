...
const PROJECT = process.env.LAB_PROJECT ?? "openlaw-docs-41255c61-admin";
...
const results = {
  ...
  round: 1,
  issues: [745, 747],
  walkthroughReviewer: "DOC-029 independent walkthrough agent (admin-config, round 1)",
  reviewerKind: "agent",
...
async function typesSection(admin, member) {
  ...
  await step(
    SC,
    role,
    "Add and maintain types, step 1-2: open Settings > Organization > Matters > Types, select Add type, enter a name, press Enter",
    "The new type row appears in the Matter types list.",
    async () => {
...
async function formsSection(admin, business) {
  ...
  await step(
    SC,
    role,
    "Create the request type, steps 1-4: Settings > Intake > Request types > Add request type with Enter, Edit, Description saved on leaving the field; the list Target column and the editor have no Target control",
    ...
    async () => {
      ...
      for (const name of [F.contractForm, F.matterForm, F.targetForm]) {
        ...
        const input = admin
          .getByRole("textbox")
          .filter({ hasNot: admin.locator("[aria-label='Search']") })
          .last();
        await input.fill(name);
        await input.press("Enter");
        await admin
  ...
  await step(
    ...
    async () => {
      ...
      await business.screenshot({
        path: path.join(SHOTS, "r1-portal-contract-form.png"),
        fullPage: true,
      });
  ...
  await step(
    ...
    async () => {
      ...
      const older = await admin
        .getByRole("button", { name: /^Remove DOC-029 admin-config / })
...
async function matterSnapshot(admin, number) {
  ...
  const dates = await admin
    ...
    .getByRole("row")
    .evaluateAll((els) => els.slice(1).map((e) => e.innerText.replace(/\s+/g, " ").trim()));
...
function taskFacts(apiTasks) {
  ...
  return list.map((t) => [
    t.title,
    t.dueDate ?? t.dueOn ?? null,
    t.assignee?.displayName ?? t.assigneeName ?? null,
  ]);
}
...
async function templatesSection(admin, business) {
  ...
  await step(
    SC,
    role,
    "Before you start: a live Matter type with the Fields the template should fill",
    "The type has an optional Text Field, a required Text Field, and the Entity Field the Matter form collects.",
    async () => {
      ...
      S.templateTypeUrl = await createTypeWithFields(admin, "matter", T.type, [
        T.region,
        T.counsel,
        F.entityField,
      ]);
  ...
  await step(
    SC,
    role,
    "Create and edit a template, steps 1-7: Templates > Matter type > Add template (Name, Description); Edit; Matter defaults; Custom field defaults; Tasks with offsets, roles and reorder; Key dates; Save template; reopen and confirm",
    ...
    async () => {
      ...
      await d
        ...
        .fill("DOC-029 fictional onboarding checklist for a supplier.");
      ...
      await admin
        .getByRole("textbox", { name: T.counsel, exact: true })
        .fill("DOC-029 Fictional Counsel LLP");
      ...
      await admin
        .getByRole("button", { name: new RegExp(`^Reorder ${escapeRe(T.task3)}, position`) })
        .focus();
      await admin.keyboard.press("ArrowUp");
      await pause(500);
      ...
      const regionDefault = await admin
        .getByRole("textbox", { name: T.region, exact: true })
        .inputValue();
      const counselDefault = await admin
        .getByRole("textbox", { name: T.counsel, exact: true })
        .inputValue();
      expectThat(
        t.priority === "Low" && t.risk === "Medium" && t.prefix.trim() === "Supplier -",
        `defaults ${q(t)}`,
      );
      expectThat(
        regionDefault === "EMEA default" && counselDefault === "DOC-029 Fictional Counsel LLP",
        "Field defaults not saved",
      );
      expectThat(
        q(t.tasks) ===
          q([
      ...
      await admin.screenshot({
        path: path.join(SHOTS, "r1-matter-template-editor.png"),
        fullPage: true,
      });
  ...
  await step(
    SC,
    role,
    "Offsets are whole numbers from 0 to 3650; a Key date needs an offset; the editor names the problem",
    "3651 and a blank Key date offset are refused with an error, and the saved template stays unchanged.",
    async () => {
  ...
  await step(
    SC,
    role,
    "Test both creation paths, direct creation: Create matter with the template; suggested title, Priority, Risk and Fields; Matter Manager starts as the person who opens it; clear the optional Field; check Overview, Tasks, assignments and Key dates",
    "Dialog shows the template defaults; the Matter keeps the cleared optional Field empty; Tasks and Key dates use the Matter's UTC creation date; Matter Manager Tasks go to the manager; the Task with no offset has no due date.",
    async () => {
      await admin.goto(`${BASE}/matters`);
      await admin
        .getByRole("button", { name: /Create matter|New matter/ })
      ...
      await pause(800);
      const templateOptions = await d
        .getByLabel("Matter template")
        .locator("option")
        .allInnerTexts();
      ...
      expectThat(
        suggested.title.trim() === "Supplier -" &&
          suggested.priority === "Low" &&
          suggested.risk === "Medium" &&
          suggested.manager === ADMIN.displayName,
      ...
      const posted = admin.waitForRequest(
      ...
      results.records.push({
        kind: "matter",
        name: `Supplier - ${G} Direct matter ${stamp}`,
        number,
      });
      ...
      const regionValue =
        Object.entries(cf).find(([k]) => k.includes("template_region"))?.[1] ?? null;
      const counselValue =
        Object.entries(cf).find(([k]) => k.includes("template_counsel"))?.[1] ?? null;
      const expectTasks = [
        [T.task1, utcDatePlus(anchor, 2), ADMIN.displayName],
        [T.task3, utcDatePlus(anchor, 5), ADMIN.displayName],
        [T.task2, null, null],
      ];
      const expectDates = [
      ...
      expectThat(
        snap.m.priority === "low" && snap.m.risk === "medium",
        `priority/risk ${snap.m.priority}/${snap.m.risk}`,
      );
      expectThat(
        (regionValue === null || regionValue === "") &&
          counselValue === "DOC-029 Fictional Counsel LLP",
        `fields ${q(cf)}`,
      );
      expectThat(q(tfSorted) === q(expectTasks), `tasks ${q(tf)} expected ${q(expectTasks)}`);
      expectThat(
        q(dfSorted.map((x) => x && [x[0], String(x[1]).slice(0, 10)])) === q(expectDates),
        `dates ${q(df)} expected ${q(expectDates)}`,
      );
  ...
  await step(
    SC,
    role,
    "Direct creation with Matter Manager set to Unassigned",
    "Matter Manager Tasks stay unassigned.",
    async () => {
      await admin.goto(`${BASE}/matters`);
      await admin
        .getByRole("button", { name: /Create matter|New matter/ })
      ...
      results.records.push({
        kind: "matter",
        name: `Supplier - ${G} Unassigned matter ${stamp}`,
        number,
      });
      const snap = await matterSnapshot(admin, number);
      const tf = taskFacts(snap.apiTasks);
  ...
  await step(
    "V-C39,V-C40",
    role,
    "Test both creation paths, Request conversion: convert the Matter form Request with the Matter type and template; check carried answers, defaults, Priority, Risk, title, Manager, Tasks and Key dates",
    "The Entity answer carries; template Field defaults fill the rest; Priority stays the Request's Urgency, Risk stays unset, the title has no template prefix; the converting person is Matter Manager and gets the Manager Tasks; dates use the new Matter's UTC creation date.",
    async () => {
      const d = await openConvert(admin, S.matterRequest, "matter");
      const priorityBefore = await d
        .getByRole("combobox", { name: "Priority" })
        .locator("option:checked")
        .innerText();
      ...
      expectThat(
        snap.m.priority === "high" && snap.m.risk === null,
        `priority/risk ${snap.m.priority}/${snap.m.risk}`,
      );
      ...
      expectThat(
        Object.values(cf).includes("DOC-029 Fictional Counsel LLP") &&
          Object.values(cf).includes("EMEA default"),
        `template defaults ${q(cf)}`,
      );
      expectThat(q(tfSorted) === q(expectTasks), `tasks ${q(tf)} expected ${q(expectTasks)}`);
      expectThat(
        q(dfSorted.map((x) => x && [x[0], String(x[1]).slice(0, 10)])) === q(expectDates),
  ...
  await step(
    SC,
    role,
    "Change or archive a template: edit and save; existing Matters keep their Tasks and dates; a new Matter uses the change; Archive template removes it from creation; Show archived > Restore brings it back",
    "The earlier Matter is unchanged; a new Matter shows the edited Task; the archived template is not offered and cannot be used; Restore makes it available.",
    async () => {
      await admin.goto(S.templateUrl);
      await admin.getByRole("heading", { name: "Matter defaults" }).waitFor();
      ...
      const directNow = taskFacts(
        (await api(admin, "GET", `/api/v1/matters/${S.directMatter}/tasks`)).body,
      );
      ...
      results.records.push({
        kind: "matter",
        name: `Supplier - ${G} After change matter ${stamp}`,
        number,
      });
      const m = (await api(admin, "GET", `/api/v1/matters/${number}`)).body.matter;
      ...
      expectThat(
        newTask1 && newTask1[1] === utcDatePlus(m.createdAt, 3),
        `new Matter tasks ${q(newTasks)}`,
      );
      // Archive.
      await openTemplates(admin, T.type);
      ...
      expectThat(
        ...
        `archived template offered ${q(archivedOptions)}`,
      );
      ...
      await admin
        .getByRole("button", { name: `Archive ${T.name}`, exact: true })
        .waitFor({ timeout: 15000 });
      await admin.goto(`${BASE}/matters`);
      await admin
        .getByRole("button", { name: /Create matter|New matter/ })
  ...
  await step(
    SC,
    role,
    "A Field detached from the template's type: the editor names its retained saved value; reattach restores it",
    "After Detach the editor says the Field is no longer attached and shows the retained value; after reattach the default is editable again.",
    async () => {
      ...
      await pause(1000);
      const note = (
        await admin.getByText(/is no longer attached to this Matter type/).textContent()
      ).trim();
      await admin.goto(S.templateTypeUrl);
      await admin.getByRole("button", { name: "Attach field" }).click();
      ...
      expectThat(
        /EMEA default/.test(note) && value === "EMEA default",
...
async function remindersSection(admin, browser) {
  ...
  await step(
    SC,
    role,
    "Steps 3-5: Add lead time 9 with Enter, refuse a duplicate and 731, reorder with the arrow keys, reload and confirm; check the 20-entry limit and that the last entry cannot be removed",
...
async function auditSection(admin, member) {
  ...
  await step(
    ...
    async () => {
      ...
      expectThat(
        searched.length > 0 && searched.every((r) => r.includes("DOC-029 admin-config")),
