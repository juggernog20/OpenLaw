...
import { createLog, expectThat, flat, LAB, MCP_URL, PEOPLE, PUBLIC, PW_PATH, ROOT } from "./lib.mjs";
...
const S = { article: "deployment-configuration", scenario: "V-M41-PUBLIC", role: "operator", method: "container-operation", page: "Tailscale Funnel 443" };
...
let ingress = [];
await step(S, "Tailscale state after the move: Funnel on 443 → 127.0.0.1:43330; 8443 off; tcp 5901 unchanged", "funnel status shows only the 443 Funnel and the 5901 entry", async () => {
  const { execFileSync } = await import("node:child_process");
  const st = execFileSync("tailscale", ["funnel", "status"], { encoding: "utf8" });
  const ok = /https:\/\/omarchy\.tail0a8904\.ts\.net \(Funnel on\)\s*\n\|-- \/ proxy http:\/\/127\.0\.0\.1:43330/.test(st) && !/:8443/.test(st) && /:5901 \(tailnet only\)/.test(st);
  expectThat(ok, st);
...
const undici = await import(path.join(ROOT, "node_modules/.pnpm/undici@7.29.1/node_modules/undici/index.js"));
await step(S, "Public DNS and certificate for the 443 origin", "Public A records; TLS verified", async () => {
  const cf = await (await fetch(`https://cloudflare-dns.com/dns-query?name=${HOST}&type=A`, { headers: { accept: "application/dns-json" } })).json();
  ingress = (cf.Answer ?? []).filter((a) => a.type === 1).map((a) => a.data);
  const r = await undici.fetch(`${PUBLIC}/readyz`, { dispatcher: new undici.Agent({ connect: { lookup: (_h, o, cb) => (o?.all ? cb(null, [{ address: ingress[0], family: 4 }]) : cb(null, ingress[0], 4)) } }) });
...
const outside = (p, init = {}) =>
  undici.fetch(`${PUBLIC}${p}`, { ...init, dispatcher: new undici.Agent({ connect: { lookup: (_h, o, cb) => (o?.all ? cb(null, [{ address: ingress[0], family: 4 }]) : cb(null, ingress[0], 4)) } }) });

await step(S, "Discovery paths on the 443 origin return JSON with the new issuer", "Each well-known path 200 JSON; issuer https://omarchy.tail0a8904.ts.net/api/auth", async () => {
  const out = [];
  let bad = 0;
  for (const p of ["/.well-known/oauth-authorization-server", "/.well-known/oauth-authorization-server/api/auth", "/.well-known/openid-configuration", "/.well-known/openid-configuration/api/auth", "/.well-known/oauth-protected-resource", "/.well-known/oauth-protected-resource/mcp", "/api/auth/jwks"]) {
    ...
    const good = r.status === 200 && type.includes("json") && j && (!j.issuer || j.issuer === `${PUBLIC}/api/auth`) && (!j.resource || j.resource === MCP_URL);
    if (!good) bad++;
    out.push(`${p} → ${r.status} ${type} (${j?.issuer ? `issuer ${j.issuer}` : j?.resource ? `resource ${j.resource}` : j?.keys ? `${j.keys.length} key(s)` : "?"})`);
  }
  expectThat(bad === 0, out.join("; "));
  return out.join("; ");
});

await step(S, "Unauthenticated /mcp on the 443 origin returns 401 with WWW-Authenticate", `401; resource_metadata="${PUBLIC}/.well-known/oauth-protected-resource"`, async () => {
  const r = await outside("/mcp", {
    method: "POST",
    headers: { "content-type": "application/json", accept: "application/json, text/event-stream" },
    body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "initialize", params: { protocolVersion: "2025-06-18", capabilities: {}, clientInfo: { name: "DOC-030 claude-live probe", version: "1" } } }),
  });
  const www = r.headers.get("www-authenticate") ?? "";
  await r.text();
  expectThat(r.status === 401 && www.includes(`resource_metadata="${PUBLIC}/.well-known/oauth-protected-resource"`), `${r.status} ${www}`);
  return `POST /mcp initialize without credentials → ${r.status}; WWW-Authenticate: ${www.slice(0, 110)}…`;
});

await step({ ...S, role: "administrator", method: "browser-walkthrough", article: "configure-mcp", scenario: "V-M41-C59 prerequisite (configure-mcp Enable OAuth Clients)", page: "/settings/mcp", secrets: [PASSWORD] }, "Re-read Server address and the pill as Daniel Okafor on the 443 origin", `Server address ${MCP_URL}; pill text recorded`, async () => {
  ...
  try {
    ...
    await page.getByRole("navigation", { name: "Settings sections" }).getByRole("link", { name: "MCP", exact: true }).first().click();
    ...
    const pill = flat(await page.getByRole("status").filter({ hasText: /Not reachable|Reachable/ }).innerText());
    const sw = {};
    for (const n of ["Enable MCP", "Legal Users OAuth Clients", "Business Users OAuth Clients", "Enable Claude", "Enable Claude Code"]) sw[n] = await page.getByRole("switch", { name: n, exact: true }).getAttribute("aria-checked");
    await page.screenshot({ path: new URL("./settings-mcp-pill-443.png", import.meta.url).pathname });
    expectThat(addr === MCP_URL && Object.values(sw).every((v) => v === "true"), JSON.stringify({ addr, pill, sw }));
