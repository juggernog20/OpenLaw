...
Analysis can start automatically when the target's text becomes ready or a ready Document Version is marked as the executed copy. To request a run yourself, open **Fields** and select **Run analysis** in the section header. The Contract actions menu also offers **Run analysis** from any section. The control says **Running…** while the run is under way. The record updates when the run finishes, while preserving typed drafts.
