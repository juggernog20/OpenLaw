...
const S = { article: "deployment-configuration", scenario: "V-M41-PUBLIC", role: "operator", method: "container-operation", page: "installation directory" };
const keys = () =>
  Object.fromEntries(
...
const containerEnv = (service, key) => {
  ...
  return sh("docker", ["--context", "default", "inspect", id, "--format", "{{range .Config.Env}}{{println .}}{{end}}"]).out
    .split("\n")
    .find((l) => l.startsWith(`${key}=`))
    ?.slice(key.length + 1);
};
const createdAt = (service) => sh("docker", ["--context", "default", "inspect", compose(["ps", "-q", service]).trim(), "--format", "{{.Created}}"]).out.trim();

await step({ ...S }, "Publicly reachable 1: set the public HTTPS origin as BASE_URL in .env, no subpath", `.env BASE_URL=${PUBLIC}`, async () => {
...
await step({ ...S }, "Publicly reachable 1: recreate app and worker (docker compose config --quiet; up -d --no-build --pull never)", "Both containers recreated with the new BASE_URL; readyz 200", async () => {
  ...
  const env = { app: containerEnv("app", "BASE_URL"), worker: containerEnv("worker", "BASE_URL") };
  expectThat(before.app !== after.app && before.worker !== after.worker && env.app === PUBLIC && env.worker === PUBLIC, JSON.stringify({ before, after, env }));
...
let gateway;
await step({ ...S }, "Find the trusted proxy address: docker network inspect <project>_openlaw-backend; set TRUSTED_PROXIES to the gateway; up -d --no-build --pull never", "Gateway read; app recreated with TRUSTED_PROXIES; no start warning", async () => {
  const r = sh("docker", ["--context", "default", "network", "inspect", `${LAB.project}_openlaw-backend`, "--format", "{{range .IPAM.Config}}{{.Gateway}} {{.Subnet}}{{end}}"]);
...
await step({ ...S }, "Publicly reachable 2: keep the app port behind the reverse proxy; database and document engine unpublished", "docker compose port app 3000 prints 127.0.0.1:43330; postgres and doc-engine publish nothing", async () => {
  const app = compose(["port", "app", "3000"]).trim();
  const ps = compose(["ps", "--format", "{{.Service}} {{.Publishers}}"]).trim().split("\n");
  const leaks = ps.filter((l) => /^(postgres|doc-engine) /.test(l) && /PublishedPort:[1-9]/.test(l));
