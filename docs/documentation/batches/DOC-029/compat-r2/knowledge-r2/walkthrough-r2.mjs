// DOC-029 round 2 compatibility replay of knowledge/walkthrough-r1.mjs against 57e77e38. Changes: paths, record names (DOC-029r2), reviewer, output name.
...
//   LAB_PASSWORD=... mise exec -- node docs/documentation/batches/DOC-029/compat-r2/knowledge-r2/walkthrough-r2.mjs
...
const FIX = path.join(here, "../../knowledge/fixtures");
...
const results = {
  ...
  group: "knowledge",
  round: 2,
  reviewer: "DOC-029r2 compatibility reviewer (knowledge)",
  reviewerKind: "agent",
  lab: LAB,
...
async function createKnowledge(role, page, account, shared) {
  ...
  const base = `DOC-029r2 knowledge ${account.tag} ${stamp}`;
  const ctx = {};
  ...
  await step(
    ...
    async () => {
      ...
      const homeBefore = await (async () => {
        ...
        return reader
          .getByRole("link", { name: `DOC-029r2 knowledge ${account.tag} ${stamp} supplier pack` })
          .count();
      })();
      ...
      const api = [
        ...
        (
          await apiJson(page, "POST", "/api/v1/knowledge", {
            data: { title: "DOC-029r2 knowledge refused", knowledgeTypeId: shared.liveTypeId },
          })
        ).status,
      ];
      const readerPaths = [];
...
async function cleanup(admin, shared) {
  ...
  for (const row of mine.knowledgeItems.filter(
    (r) => r.title.includes(`DOC-029r2 knowledge`) && r.title.includes(stamp),
  )) {
...
try {
  ...
  shared.archivedTypeName = `DOC-029r2 knowledge retired type ${stamp}`;
  ...
  if (ONLY === "all" || ONLY === "create") {
    ...
    if (created[firstRole]?.guidance)
      await helpSeparation(
        ...
        firstRole,
        `DOC-029r2 knowledge ${ACCOUNTS[firstRole].tag} ${stamp} guidance`,
      );
  }
