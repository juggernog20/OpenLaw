...
Use an Administrator or Legal Team Member account that can read the Document. Have at least two ordinary Document Versions in the same chain. A Generated redline cannot itself be a Comparison operand. See [Document Versions](document-versions.md) to add a Version.
...
Open the owning record's **Documents** section. Open the relevant Document or Version's actions and select **Compare with previous**. The Document reader also offers **Compare** when there is a previous eligible Version. When a ready Comparison exists for that pair, the link shows its change count instead.
