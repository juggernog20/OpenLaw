// DOC-029 round 2 compatibility replay of electronic-signing.md (V-C17-electronic) by the DOC-029r2
// compatibility reviewer (signing-standin). A copy of rewalk/signing/walkthrough-rw.mjs, replayed in full
// on the owned lab "sign-r2" (app commit 57e77e38). Only the lab name, the output path and the record
// names (DOC-029r2 signing-standin ...) changed.
// stand-in copy in this folder keeps the posted Signer tabs so the anchor on the wire can be read).
//
...
const lab = JSON.parse(readFileSync(path.join(root, ".documentation-labs/sign-r2/lab.json"), "utf8"));
...
const OUT = path.join(here, `walkthrough-r2-${PHASE}.raw.json`);
...
const results = {
  ...
  providerMode: "isolated local DocuSign protocol stand-in (rewalk/signing/server.mjs, a copy of signing-standin/fixture/server.mjs that also keeps posted Signer tabs); no real provider account",
  stamp: STAMP,
...
async function step(role, actor, expected, fn) {
  if (blockedRoles.has(role)) {
    const skipped = { role, actor, method: "browser-walkthrough", expected, result: "not-run", actual: "Not run: an earlier step for this role failed.", at: new Date().toISOString() };
    results.steps.push(skipped);
    console.log(`[${role}] NOT-RUN ${expected}`);
    save();
    return skipped;
  }
  const entry = { role, actor, method: "browser-walkthrough", expected, startedAt: new Date().toISOString() };
...
const signing = async (who, number) => (await api(who, "GET", `/contracts/${number}/envelopes`)).json;
const paper = async (who, number) =>
  (await api(who, "GET", `/contracts/${number}/documents?includeArchived=true`)).json.documents;
const contractOf = async (who, number) => (await api(who, "GET", `/contracts/${number}`)).json.contract;

function fictionalPdf(label, marks = "") {
  return Buffer.from(`%PDF-1.4\n% DOC-029r2 signing-standin fictional paper: ${label}${marks}\n1 0 obj << >> endobj\n%%EOF\n`);
}
const SIG_TAB = { documentId: "1", anchorString: "/sig/", anchorUnits: "pixels", anchorXOffset: "0", anchorYOffset: "0", anchorIgnoreIfNotPresent: "true" };
function checkSigTabs(provider) {
  for (const s of provider.signers) {
    const tabs = s.tabs?.signHereTabs ?? [];
    expectThat(tabs.length === 1 && JSON.stringify(tabs[0]) === JSON.stringify(SIG_TAB), `Signer ${s.name} tabs ${JSON.stringify(s.tabs)}`);
  }
  return `each of ${provider.signers.length} Signers carried the same single signHereTabs entry ${JSON.stringify(SIG_TAB)}`;
}
...
async function sendEnvelope(page, number, { versionLabel, signers, subject }) {
  ...
  const sent = page.waitForResponse(
    (r) => r.url().endsWith(`/api/v1/contracts/${number}/envelopes`) && r.request().method() === "POST",
  );
...
const signersFor = (role, round) => [
  { name: `DOC-029 Signer One ${round}`, email: `signer.one.${role}.${round}@counterparty.example` },
  { name: `DOC-029 Signer Two ${round}`, email: `signer.two.${role}.${round}@counterparty.example` },
];
async function newestProviderEnvelope(before) {
  const state = await standin("state");
  expectThat(state.envelopes.length === before + 1, `provider holds ${state.envelopes.length} envelopes, expected ${before + 1}`);
...
async function main(browser) {
  ...
  await step("administrator", PEOPLE.daniel.name, "The lab starts with no Signing connector", async () => {
  ...
  for (const flow of flows) {
    const title = `DOC-029r2 signing-standin ${flow.role} ${STAMP}`;
    ...
    flow.title = title;
    flow.v1 = fictionalPdf(`${flow.role} primary version 1`, " Signature of Signer One: /sig/ Signature of Signer Two: /sig/");
    ...
    const noPaper = await api(flow.sender, "POST", "/contracts", {
      title: `DOC-029r2 signing-standin ${flow.role} no paper ${STAMP}`,
      contractTypeId: typeId,
    });
  ...
  for (const flow of flows) {
    await step(flow.role, flow.sender.person.name, "Send for signature is absent with no connector configured, both before and after a primary Document exists", async () => {
      await openApprovals(flow.sender.page, flow.number);
      const before = await sendButton(flow.sender.page).count();
      const doc = await upload(flow.sender, flow.number, flow.v1Name, flow.v1, "draft_ours", "DOC-029 first round");
      await addVersion(flow.sender, doc.id, flow.v2Name, flow.v2, "redline_theirs", "DOC-029 agreed round");
      await upload(flow.sender, flow.number, flow.supportName, fictionalPdf(`${flow.role} supporting`), "draft_ours", "DOC-029 supporting paper");
      await openApprovals(flow.sender.page, flow.number);
  ...
  await step("administrator", PEOPLE.daniel.name, "Setup: the Administrator saves and enables a Webhook-mode DocuSign connector that points at the stand-in, and Test connection reaches it", async () => {
    ...
    const saved = page.waitForResponse(
      (r) => r.url().endsWith("/api/v1/signing-connectors/docusign") && r.request().method() === "PUT",
    );
    ...
    if (!c.enabled) {
      ...
      await until(async () => (await api(daniel, "GET", "/signing-connectors/docusign")).json.connector.enabled, "connector never enabled");
    }
  ...
  for (const flow of flows) {
    await step(flow.role, flow.sender.person.name, "With the connector enabled, Send for signature stays absent when no primary Document is set and appears when one is", async () => {
  ...
  await step("administrator", PEOPLE.daniel.name, "Setup: the Administrator turns Send for signature from records off, then on again", async () => {
    const page = daniel.page;
    await page.goto(`${BASE}/settings/integrations/e-signature`);
    const docusign = page.getByRole("button", { name: "DocuSign", exact: true });
    await docusign.waitFor({ timeout: 20000 });
    if ((await docusign.getAttribute("aria-expanded")) === "false") await docusign.click();
    await page.getByRole("switch", { name: "Send for signature from records" }).click();
    await until(async () => !(await api(daniel, "GET", "/signing-connectors/docusign")).json.connector.enabled, "connector never disabled");
    const seen = [];
    for (const flow of flows) {
      await openApprovals(flow.sender.page, flow.number);
    ...
    if ((await docusign.getAttribute("aria-expanded")) === "false") await docusign.click();
    await page.getByRole("switch", { name: "Send for signature from records" }).click();
    await until(async () => (await api(daniel, "GET", "/signing-connectors/docusign")).json.connector.enabled, "connector never re-enabled");
    for (const flow of flows) {
      await openApprovals(flow.sender.page, flow.number);
  ...
  for (const flow of flows) {
    ...
    await step(role, sender.person.name, "Before you start (/sig/ paragraph): the paper to send is marked with /sig/ before its Document Version is uploaded; the article paragraph is present in the reviewed bytes", async () => {
      const text = readFileSync(path.join(root, "docs/user-guides/electronic-signing.md"), "utf8");
      ...
      const dl = await sender.page.request.get(`${BASE}/api/v1/documents/${flow.documentId}/versions/${v1.id}/download`);
      ...
      return `Version 1 (${v1.originalFilename}) was uploaded with two /sig/ marks already in the paper; its download answered 200 with the same bytes and ${marks} /sig/ marks. Version 2 has none. The stand-in cannot render placement or white text.`;
    });

    await step(role, sender.person.name, "Before you start: the sender moves the Contract to the Signature Status Out for signature", async () => {
    ...
    await step(role, sender.person.name, "Send the Envelope steps 1-3 and 5: Approvals > Send for signature opens the dialog; Version lists only primary Document Versions with the current one selected; empty and duplicate Signers are refused; Add signer and remove work; Cancel sends nothing", async () => {
      ...
      expectThat(!optionTexts.some((t) => t.includes("supporting")), "supporting Document offered");
      await dialog.getByRole("button", { name: "Send envelope" }).click();
      ...
      await dialog.getByRole("button", { name: "Send envelope" }).click();
      await until(async () => (await dialog.getByRole("alert").count()) > 0 && (await dialog.getByRole("alert").innerText()).trim() !== "", "no duplicate alert");
    ...
    await step(role, sender.person.name, "If sending fails: during a provider outage the dialog shows the error and no round exists; after reload the retry sends Version 1 with a blank Subject; the row shows Out for signature, Version, Signers and sent time; the provider receives only that primary PDF with subject C-<number> <title>; the Status does not move", async () => {
      ...
      const note = await card(page).getByText("Signed, declined, and voided status arrives by webhook. The executed file auto-files and the stage advances to Active.").count();
      ...
      const tabsV1 = checkSigTabs(provider);
      return `Sign Here on the wire for Version 1 (with /sig/): ${tabsV1}; Outage alert: "${outageAlert}"; 0 rounds in app and after reload; retry row: "${rows[0]}"; header Envelope sent; note under live row: "${note ? "Signed, declined, and voided status arrives by webhook. …" : "absent"}"; provider envelope subject "${provider.subject}", ${provider.documentCount} document equal to Version 1 bytes, 2 fictional Signers with the same routingOrder ${provider.signers[0].routingOrder}; Status still ${c.statusName}`;
    });

    await step(role, sender.person.name, "One live Envelope only: Send for signature is absent while it is out, and a direct second send is refused", async () => {
      await openApprovals(page, number);
    ...
    await step(role, `${flow.unrelated.person.name}, ${sender.person.name}, ${flow.voider.person.name}`, `Withdraw: an unrelated Legal Team Member has no envelope actions and cannot void; the sender has the actions; the ${flow.voiderWhy} voids with a required Reason, Cancel keeps the round, and a Voided round frees a new send without moving the Status`, async () => {
      ...
      const unrelatedVoid = await api(flow.unrelated, "POST", `/envelopes/${envelopeId}/void`, { reason: "DOC-029 should be refused" });
      await openApprovals(page, number);
    ...
    await step(role, sender.person.name, "Decline: a provider-reported Declined round records its reason, keeps the Voided round in history, files nothing, does not end or archive the Contract, and allows another send", async () => {
      const before = (await standin("state")).envelopes.length;
      const versionsBefore = (await paper(sender, number)).find((d) => d.id === flow.documentId).versions.length;
      await openApprovals(page, number);
      const sent = await sendEnvelope(page, number, { signers: signersFor(role, "r2"), subject: `DOC-029r2 signing-standin ${role} decline round` });
      ...
      const tabsV2 = checkSigTabs(provider);
      ...
      expectThat(versionsAfter === versionsBefore && c.stage === "signature" && !c.archivedAt && sendAgain === 1 && header === 1, "after decline");
      return `Current Version 2 (no /sig/) sent with the same anchor request (${tabsV2}), so a missing mark is left to anchorIgnoreIfNotPresent; Signed provider decline delivery answered ${webhook}; rows: "${rows[0]}" / "${rows[1]}"; header Envelope declined; primary Document still ${versionsAfter} Versions; Status ${c.statusName}, archivedAt ${c.archivedAt ?? null}; Send for signature shown`;
    });

    await step(role, `${sender.person.name}, ${flow.rival.person.name}`, "If another sender won: with both dialogs open, the second send is refused in the dialog, and reloading shows the winner's live round instead of creating another", async () => {
      const before = (await standin("state")).envelopes.length;
      await openApprovals(page, number);
      ...
      const sent = await sendEnvelope(page, number, { signers: signersFor(role, "r3"), subject: `DOC-029r2 signing-standin ${role} completion round` });
    ...
    await step(role, sender.person.name, "Follow completion: a provider-reported completion shows Signed, then Executed copy; the executed PDF equals the provider's output, is filed once as the pinned executed Version on the primary Document chain, the supporting Document is untouched, and the Contract moves to Active; a repeated delivery adds nothing", async () => {
      ...
      expectThat(executed.kind === "executed" && executed.isExecuted && executed.isCurrent && chain.length === 3, `chain ${JSON.stringify(chain.map((v) => [v.versionNumber, v.kind, v.isExecuted]))}`);
    ...
    await step(role, sender.person.name, "If someone moved the Contract to another Stage before completion, filing still pins the executed Version but completion does not overwrite that Stage", async () => {
      await moveStatus(page, number, "Out for signature");
      const before = (await standin("state")).envelopes.length;
      await openApprovals(page, number);
      ...
      await until(async () => (await signing(sender, number)).envelopes[0].executedFetch === "ready", "never filed", 120000, 1500);
    ...
    await step(role, sender.person.name, "If voiding is refused because the provider already completed the round: the dialog shows the refusal; after the provider's feed arrives, reloading shows the recorded Signed outcome", async () => {
      const before = (await standin("state")).envelopes.length;
      await openApprovals(page, number);
      ...
      expectThat(refusal.startsWith("The provider says this envelope is no longer live") && reloadedAfterFeed.includes("Signed"), `${refusal} / ${reloadedAfterFeed}`);
      await until(async () => (await signing(sender, number)).envelopes[0].executedFetch === "ready", "never filed", 120000, 1500);
    ...
    await step(role, sender.person.name, "If the row says the executed copy could not be filed: a Signed round whose executed file cannot be fetched shows that message and creates no Version", async () => {
      ...
      try {
        ...
        await standin("complete", { id: provider.id });
        const webhook = await deliver(provider.id, "completed", { completedAt: new Date().toISOString() });
        await until(async () => (await signing(sender, number)).envelopes[0].executedFetch === "failed", "never recorded the failure", 120000, 1500);
        await openApprovals(page, number);
    ...
    await step(role, `${PEOPLE.ravi.name} (Business User on the team)`, "Negative: a Business User on the Contract team cannot read or send Envelopes", async () => {
    ...
    await step(role, sender.person.name, "Send for signature is absent on an archived Contract; its recorded rounds stay readable", async () => {
      ...
      try {
        ...
        const actions = await card(page).getByRole("button", { name: /^Actions for the envelope sent on/ }).count();
        const direct = await api(sender, "POST", `/contracts/${number}/envelopes`, {
          documentVersionId: (await signing(sender, number)).primaryDocument?.versions?.[0]?.id ?? "x",
  ...
  results.contracts = flows.map((f) => ({ role: f.role, number: f.number, noPaperNumber: f.noPaperNumber, title: f.title }));
...
async function polling(browser) {
  ...
  const options = (await api(daniel, "GET", "/contracts/options")).json;
  const typeId = options.contractTypes.find((t) => t.displayName === "NDA").id;
  await step("administrator", PEOPLE.daniel.name, "Setup: the Administrator switches Signing updates to Polling", async () => {
    const page = daniel.page;
    await page.goto(`${BASE}/settings/integrations/e-signature`);
    const docusign = page.getByRole("button", { name: "DocuSign", exact: true });
    await docusign.waitFor({ timeout: 20000 });
  ...
  const title = `DOC-029r2 signing-standin polling ${STAMP}`;
  ...
  let providerId;
  let completedAt;
  await step("legal_team_member", PEOPLE.nadia.name, "Polling: the live row still carries the webhook note; a completion reported only at the provider reaches the row without a webhook, within about 15 to 20 minutes while the worker runs", async () => {
    ...
    const note = await card(nadia.page).getByText("Signed, declined, and voided status arrives by webhook. The executed file auto-files and the stage advances to Active.").count();
    const provider = await newestProviderEnvelope(before);
    providerId = provider.id;
    await standin("complete", { id: providerId });
    completedAt = Date.now();
    const refused = await deliver(providerId, "completed", { completedAt: new Date().toISOString() });
    await until(async () => (await signing(nadia, number)).envelopes[0].status === "signed", "polling never reported completion within 30 minutes", 30 * 60 * 1000, 30000);
    const signedAt = Date.now();
    await until(async () => (await signing(nadia, number)).envelopes[0].executedFetch === "ready", "never filed", 300000, 5000);
    await openApprovals(nadia.page, number);
...
try {
  ...
  failures += 1;
  results.fatal = String(error?.message ?? error).split("\n").slice(0, 6).join(" ");
