...
3. In **Business-user portal**, review magic-link sign-in and add each **Allowed email domains** entry using **Add**. Enter a domain such as `helix.example`, without an email username. An empty allowlist admits nobody. OpenLaw checks the list at every Portal sign-in, so an empty list also stops Business Users who signed in before. Continue to save the choices.
