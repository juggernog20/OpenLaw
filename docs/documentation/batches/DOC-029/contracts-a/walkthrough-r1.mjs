...
//   LAB_PASSWORD=... mise exec -- node docs/documentation/batches/DOC-029/contracts-a/walkthrough-r1.mjs
...
const OUT = process.env.OUT ?? path.join(here, "walkthrough-r1.raw.json");
const ONLY = (
...
const results = {
  ...
  group: "contracts-a",
  round: 1,
  walkthroughReviewer: "DOC-029 independent walkthrough agent (contracts-a, round 1)",
  reviewerKind: "agent",
...
async function cleanupFixtures() {
  ...
  for (const [label, base, key] of lists) {
    ...
    for (const row of rows.filter(
      (x) => /^DOC-029 contracts-a /.test(x.displayName ?? x.name ?? "") && !x.archivedAt,
    )) {
...
}
{
  const f1 = await S.daniel.api("POST", "/fields", {
    displayName: `DOC-029 contracts-a Required number ${stamp}`,
    moduleScope: "contract",
    fieldType: "number",
    fieldTag: "business",
  });
  const f2 = await S.daniel.api("POST", "/fields", {
    displayName: `DOC-029 contracts-a Notes ${stamp}`,
    moduleScope: "contract",
    fieldType: "long_text",
    fieldTag: "business",
  });
  const t = await S.daniel.api("POST", "/contract-types", {
    displayName: `DOC-029 contracts-a Required type ${stamp}`,
  });
  must(
  ...
  const st = await S.daniel.api("POST", "/contract-statuses", {
    displayName: `DOC-029 contracts-a review ${stamp}`,
    stage: "review",
  });
  ...
  const g = await S.daniel.api("POST", "/approver-groups", {
    name: `DOC-029 contracts-a sign-off ${stamp}`,
...
async function quickContract(s, role, label, extra = {}) {
  const title = `DOC-029 contracts-a ${role} ${label} ${stamp}`;
...
async function createContract(role, A, O) {
  ...
  const sc = "V-C14";
  const page = A.page;
  const title = `DOC-029 contracts-a ${role} C14 ${stamp}`;
  let number;
  ...
  await step(
    ...
    async () => {
      ...
      await d.getByLabel("Title").fill(`DOC-029 contracts-a ${role} refusal ${stamp}`);
      await d.getByRole("button", { name: "Create", exact: true }).click();
      ...
      const made = (list.json.contracts ?? []).filter(
        (c) => c.title === `DOC-029 contracts-a ${role} refusal ${stamp}`,
      );
  ...
  await step(
    ...
    async () => {
      const main = page.getByRole("main");
      const newName = `DOC-029 contracts-a ${role} Counterparty ${stamp}`;
  ...
  await step(
    ...
    async () => {
      ...
      if (role === "legal_team_member")
        await page.screenshot({ path: path.join(here, "r1-c14-change-contract-type.png") });
  ...
  await step(
    ...
    async () => {
      ...
      const post = await S.jonas.api("POST", "/contracts", {
        title: `DOC-029 contracts-a portal refusal ${stamp}`,
  ...
  await step(
    ...
    async () => {
      ...
      for (const [name, stage] of [
        ...
        const region = (
          await page
            .getByRole("region", { name: new RegExp(`DOC-029 contracts-a ${role} C15`) })
            .innerText()
        ).replace(/\s+/g, " ");
...
async function contractApprovals(role, A, O) {
  ...
  await step(
    ...
    async () => {
      ...
      const text = (await dialog.innerText()).replace(/\s+/g, " ");
      if (role === "legal_team_member")
        await page.screenshot({ path: path.join(here, "r1-c16-move-past-approval.png") });
      await dialog.getByRole("button", { name: "Cancel" }).click();
      await dialog.waitFor({ state: "hidden" });
      await sleep(800);
      must(
...
async function manualSigning(role, A) {
  ...
  const draftBytes = pdf(`DOC-029 contracts-a ${role} draft ${stamp}`);
  const execBytes = pdf(`DOC-029 contracts-a ${role} executed ${stamp} signed by both parties`);
  const laterBytes = pdf(`DOC-029 contracts-a ${role} later round ${stamp}`);
    ...
    async () => {
      ...
      try {
        const rendered = await reader
          .getByText(`DOC-029 contracts-a ${role} executed ${stamp} signed by both parties`)
  ...
  await step(
    ...
    async () => {
      ...
      if (role === "legal_team_member")
        await docsCard().screenshot({ path: path.join(here, "r1-c17-executed-designation.png") });
...
try {
  for (const { role, A, O } of plan) {
    ...
    if (ONLY.includes("contract-stages"))
      await contractStages(role, A, O, `DOC-029 contracts-a renamed by ${role} ${stamp}`);
