 ...
 * #422). A conversion moves the comment rows onto the contract the ask
 * became, and the `request` audience arm follows that back-link — so
 * this card keeps asking for the Request's thread by the Request's own
 * id, and what it is answered is the record's conversation. There is no
 * branch here and no second address: which record the rows hang off is
 * the seam's answer, and a card that had to know would be a second
 * place for CMT-001 to be forgotten.
 *
