...
import { BASE, SEED, api, ensureStaffAccounts, newContext, passwordSignIn, sleep, sql, text } from "./lib-r1.mjs";
...
export async function run(log, fx) {
  ...
  const visible = must(await api(admin.page, "POST", "/contracts", { title: `DOC-029 access search document holder ${stamp}`, contractTypeId: nda, managerId: danielId }), "c1").contract;
  const second = must(await api(admin.page, "POST", "/contracts", { title: `DOC-029 access search second ${term}`, contractTypeId: nda, managerId: danielId }), "c2").contract;
  const hidden = must(await api(admin.page, "POST", "/contracts", { title: `DOC-029 access search confidential ${term}`, contractTypeId: nda, managerId: danielId, isConfidential: true }), "c3").contract;
  const up = await admin.page.request.post(`${BASE}/api/v1/contracts/${visible.number}/documents`, {
    headers: { origin: BASE },
    multipart: { file: { name: `doc029-access-search-${stamp}.pdf`, mimeType: "application/pdf", buffer: pdf(`DOC-029 access fictional clause mentions ${term} only inside this Document.`) } },
  ...
  for (let i = 0; i < 90 && !indexed; i++) {
    ...
    indexed = JSON.stringify(r.body).includes(`doc029-access-search-${stamp}.pdf`) || JSON.stringify(r.body).includes('"kind":"document"');
  ...
  for (const role of ["administrator", "legal_team_member"]) {
    ...
    const readerOpen = (await p.getByRole("complementary").filter({ hasText: `doc029-access-search-${stamp}.pdf` }).count()) > 0 || (await text(p.locator("body"))).includes(`doc029-access-search-${stamp}.pdf`);
    R(
    ...
    if (role === "legal_team_member") await p.screenshot({ path: new URL("./r1-legal_team_member-saved-view.png", import.meta.url).pathname });
    R(
