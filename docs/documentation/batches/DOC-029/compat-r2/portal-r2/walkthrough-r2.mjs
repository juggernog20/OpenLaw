...
async function submitPhase() {
  ...
  await step(
    ...
    async () => {
      ...
      await page
        ...
        .fill(
          "DOC-029r2 portal: Northstar proposes a 60-day evaluation. Counterparty Northstar Evaluation Ltd. Please review the evaluation terms.",
        );
  ...
  await step(
    ...
    async () => {
      ...
      await page
        .getByLabel(/^Description/)
        .fill("DOC-029r2 portal: checking an attachment failure.");
      const [chooser] = await Promise.all([
        page.waitForEvent("filechooser"),
        page.getByRole("button", { name: "Choose files" }).click(),
      ]);
      ...
      await panel
        ...
        .fill(`DOC-029r2 portal: the file that did not attach to R-${failedNumber}.`);
  ...
  await step(
    ...
    async () => {
      ...
      await page
        .getByLabel(/^Description/)
        .fill("DOC-029r2 portal: Legal declines while the file uploads.");
      const [chooser] = await Promise.all([
        page.waitForEvent("filechooser"),
        page.getByRole("button", { name: "Choose files" }).click(),
      ]);
      ...
      await page.route("**/api/v1/requests/*/attachments", async (route) => {
        ...
        must(
          await L.api(nadia, "POST", `/requests/${n}/decline`, {
            reason: "DOC-029r2 portal: declined during upload.",
          }),
          "decline",
        );
        await route.continue();
      ...
      await panel
        .getByRole("textbox", { name: "New comment" })
        .fill(`DOC-029r2 portal: the file that did not attach to R-${n}.`);
      const [fc] = await Promise.all([
        page.waitForEvent("filechooser"),
        panel.getByRole("button", { name: "Attach files" }).click(),
      ]);
  ...
  await step(
    ...
    async () => {
      ...
      await page
        .getByLabel(/^Description/)
        .fill("DOC-029r2 portal: the submission does not reach the server.");
...
async function followPhase() {
      ...
      for (const [key, slug, customFields] of plan) {
        ...
        const r = must(
          await L.api(page, "POST", "/requests", {
            ...
            title,
            description: `DOC-029r2 portal description for ${key}.`,
            urgency: "high",
            customFields,
          }),
          `request ${key}`,
  ...
  await step(
    ...
    async () => {
      ...
      for (const key of ["open", "contract", "matter", "resolved", "declined"]) {
        ...
        legal[key] = {
          full: `DOC-029r2 portal Legal full thread ${tag}`,
          legalOnly: `DOC-029r2 portal Legal only note ${tag}`,
          working: `DOC-029r2 portal Working team note ${tag}`,
    ...
    async () => {
      ...
      must(
        await L.api(nadia, "POST", `/requests/${Q.resolved.number}/resolve`, {
          reply: `DOC-029r2 portal resolution reply ${runStamp}`,
        }),
        "resolve",
      );
      must(
        await L.api(nadia, "POST", `/requests/${Q.declined.number}/decline`, {
          reason: `DOC-029r2 portal decline reason ${runStamp}`,
        }),
        "decline",
      );
    ...
    async () => {
      ...
      for (let i = 1; i <= 55; i++)
        await post(
          ...
          Q.older.id,
          "full_thread",
          `DOC-029r2 portal older marker ${String(i).padStart(2, "0")} ${runStamp}`,
        );
  ...
  for (const key of ["open", "resolved", "declined"]) {
    ...
    await step(
      ...
      async () => {
        ...
        if (key === "declined")
          expect(banner.includes(`DOC-029r2 portal decline reason ${runStamp}`), banner);
    ...
    await step(
      ...
      async () => {
        ...
        if (key === "resolved")
          expect(
            body.includes(`DOC-029r2 portal resolution reply ${runStamp}`),
    ...
    await step(
      ...
      async () => {
        ...
        const body = await text(card);
        for (const want of [
          `DOC-029r2 portal description for ${key}.`,
          q.original,
          "Engineering",
          "High",
        ])
  ...
  for (const key of ["contract", "matter"]) {
    ...
    await step(
      ...
      async () => {
        ...
        expect(
          orig.includes(`DOC-029r2 portal description for ${key}.`),
  ...
  await step(
    ...
    async () => {
      ...
      const panel = await openComments(page);
      await panel
        .getByText(`DOC-029r2 portal older marker 55 ${runStamp}`)
        .waitFor({ timeout: 20000 });
      const before = await panel
        .getByText(new RegExp(`^DOC-029r2 portal older marker \\d\\d ${runStamp}$`))
        .count();
      const hadFirst = await panel
        .getByText(`DOC-029r2 portal older marker 01 ${runStamp}`)
        .count();
...
async function knowledgePhase() {
  ...
  await step(
    ...
    async () => {
      ...
      await page
        .getByLabel(/^Description/)
        .fill("DOC-029r2 portal: the external website did not load.");
      await page.getByRole("button", { name: "Submit request" }).click();
