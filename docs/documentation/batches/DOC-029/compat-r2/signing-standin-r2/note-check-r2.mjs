...
const NOTE = "Signed, declined, and voided status arrives by webhook. The executed file auto-files and the stage advances to Active.";
const STAMP = new Date().toISOString().replace(/[-:TZ.]/g, "").slice(0, 14);
const out = { kind: "compatibility-extra-check", appCommit: lab.sourceCommit, environment: lab.project, appImageId: lab.appImageId, startedAt: new Date().toISOString(), steps: [] };
...
try {
  ...
  const api = async (method, url, data, multipart) => {
    const r = await page.request.fetch(`${BASE}/api/v1${url}`, { method, data, multipart, headers: { origin: BASE }, failOnStatusCode: false });
  ...
  const connector = (await (await adminPage.request.get(`${BASE}/api/v1/signing-connectors/docusign`)).json()).connector;
  await adminContext.close();
  const typeId = (await api("GET", "/contracts/options")).json.contractTypes.find((t) => t.displayName === "NDA").id;
  const title = `DOC-029r2 signing-standin note check ${STAMP}`;
  const number = (await api("POST", "/contracts", { title, contractTypeId: typeId })).json.contract.number;
  await api("POST", `/contracts/${number}/documents`, undefined, { kind: "draft_ours", note: "DOC-029r2 note check", file: { name: "doc029r2-note-check.pdf", mimeType: "application/pdf", buffer: Buffer.from("%PDF-1.4\n% DOC-029r2 fictional paper\n%%EOF\n") } });
  ...
  const sent = page.waitForResponse((r) => r.url().endsWith(`/api/v1/contracts/${number}/envelopes`) && r.request().method() === "POST");
  await dialog.getByRole("button", { name: "Send envelope" }).click();
  const sendStatus = (await sent).status();
  await card.locator('table[aria-labelledby="contract-signing-heading"] tbody tr').filter({ hasText: "Out for signature" }).first().waitFor({ timeout: 20000 });
  ...
  const voided = await api("POST", `/envelopes/${envelopes[0].id}/void`, { reason: "DOC-029r2 note check done" });
  await page.reload();
  await card.waitFor({ timeout: 20000 });
  await card.locator('table[aria-labelledby="contract-signing-heading"] tbody tr').filter({ hasText: "Voided" }).first().waitFor({ timeout: 20000 });
  const noteAfterVoid = await card.getByText(NOTE).count();
  const pass = connector.updateMode === "polling" && sendStatus === 201 && noteBefore === 0 && noteAfterRow === 1 && noteAfterReload === 1 && voided.status === 200 && noteAfterVoid === 0;
  failed = !pass;
  out.steps.push({ role: "legal_team_member", actor: "Nadia Haddad", method: "browser-walkthrough", expected: "In Polling mode, the note under a live row still says that status arrives by webhook; the note is absent with no live round", result: pass ? "pass" : "fail",
  ...
  failed = true;
  out.fatal = String(error?.message ?? error).split("\n").slice(0, 4).join(" ");
} finally {
