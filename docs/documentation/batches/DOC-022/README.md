...
Code review found one further error and corrected it: `configure-analysis` told the reader to expand the **Field prompts** card, but that card is not a disclosure and draws no expand control. The step now says where the card sits and that it is always open. The article's recorded hash was updated with it. The correction changes no action or expected result, and the live-provider blocker recorded above is unchanged.
