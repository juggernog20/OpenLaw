  ...
  await step(
    ...
    async () => {
      ...
      for (const key of ["M1", "M2"]) {
        const r = await actor.api("POST", "/matters", { title: `DOC-029 contracts-b C20 ${key} ${tag}`, matterTypeId, managerId: userId(PEOPLE[role].name) });
  ...
  await step(
    ...
    async () => {
      const matterLink = async (key, matterKey) => {
        ...
        await dialog.getByRole("textbox", { name: "Search by Matter number or title" }).fill(`DOC-029 contracts-b C20 ${matterKey} ${tag}`);
