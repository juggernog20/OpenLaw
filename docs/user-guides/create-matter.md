...
Choose **Department** from the shared list, or leave **No Department** selected. **Priority**, **Risk** and **Description** are Rows of the Form; the dialog draws them only when the type's Form collects them at creation. A template's Priority and Risk defaults land on the created Matter either way. Review **Confidential**. Complete any Entity or person reference Fields your type offers by selecting the intended record, rather than entering an identifier.
