...
Open the C- or M- reference on the Request's **Status** card. Check the Title, Type, Priority, Description, **Department** and **Needed by** Key date. Check **Fields** on a Contract or **Custom fields** on a Matter. The record receives the reviewed Description, including an explicit clear. The Department starts from the Request and can be changed when its Row is collected. The original Request retains its submitted answers, Department and Description.
...
Answers carry when the destination Type has a matching Row. Re-targeting refreshes the collected Rows and lists answers without a matching Row as staying on the Request. Nothing is deleted from the Request.
