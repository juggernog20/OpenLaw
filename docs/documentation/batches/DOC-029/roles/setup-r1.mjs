...
const co = must(await api(d.page, "POST", "/contracts", { title: "DOC-029 roles open contract", contractTypeId: nda, managerId: ids.daniel, owningDepartmentId: sales, region: "EMEA" }), "CO").contract;
fx.openContract = { number: co.number, id: co.id };
const mo = must(await api(d.page, "POST", "/matters", { title: "DOC-029 roles open matter", matterTypeId: commercial, managerId: ids.daniel, departmentId: sales, region: "EMEA" }), "MO").matter;
fx.openMatter = { number: mo.number, id: mo.id };
const cc = must(await api(n.page, "POST", "/contracts", { title: "DOC-029 roles confidential contract", contractTypeId: nda, managerId: ids.nadia, isConfidential: true, matterNumber: mo.number }), "CC").contract;
fx.confContract = { number: cc.number, id: cc.id };
const mc = must(await api(n.page, "POST", "/matters", { title: "DOC-029 roles confidential matter", matterTypeId: commercial, managerId: ids.nadia, isConfidential: true, parentMatterNumber: mo.number }), "MC").matter;
...
const ent = must(await api(n.page, "POST", "/entities", { legalName: "DOC-029 roles confidential entity Ltd", entityTypeId: etypes[0].id }), "E").entity;
...
for (const [visibility, body] of [["legal_only", "DOC-029 roles Legal only note"], ["working_team", "DOC-029 roles Working team note"], ["full_thread", "DOC-029 roles Full thread note"]]) {
...
for (const [who, email, key] of [["Jonas", "jonas.weber@helix.example", "jonas"], ["Ravi", "ravi.menon@helix.example", "ravi"]]) {
  ...
  const rq = must(await api(b.page, "POST", "/requests", { requestTypeId: reviewType, departmentId: sales, title: `DOC-029 roles ${who} request`, description: `DOC-029 roles fictional ask from ${who}.`, urgency: "medium", customFields: { counterparty_name: "DOC-029 Fictional Counterparty Ltd" } }), `request ${who}`).request;
  const converted = must(await api(n.page, "POST", `/requests/${rq.number}/convert`, { title: `DOC-029 roles ${who} converted contract`, contractTypeId: nda }), `convert ${who}`);
