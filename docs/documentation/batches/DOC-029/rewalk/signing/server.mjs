...
// DOC-029 re-walk copy of signing-standin/fixture/server.mjs. One change: each stored Signer also keeps
// the posted `tabs` object, so the walkthrough can read the Sign Here anchor OpenLaw sends. Placement
// inside the paper is still not rendered here.
...
const INTEGRATION_KEY = "doc029-standin-integration-key";
const ACCOUNT_ID = "doc029-standin-account";
const ACCOUNT_NAME = "DOC-029 signing stand-in";
const USER_EMAIL = "integration@standin.example";
const TOKEN = "doc029-standin-access-token";

const envelopes = new Map();
const faults = { outage: false, combinedRefused: false };
const counters = { tokenGrants: 0, envelopePosts: 0 };
let minted = 0;
const idPrefix = `doc029-env-${Date.now()}`;

const executedPdf = (id) =>
  Buffer.from(`%PDF-1.7\n% DOC-029 stand-in executed copy for ${id}\n%%EOF\n`, "utf8");
...
}).listen(PORT, "0.0.0.0", () => console.log(`DOC-029 signing stand-in on ${PORT}`));
