...
const { log, save, step } = createLog("m41", {
  ...
  instanceAddress: "Application address http://127.0.0.1:43320 saved in Settings → Advanced (BASE_URL unpinned by overlay-unpin.json)",
});
const S = (page, role = "administrator") => ({ article: "configure-mcp", scenario: "V-M41-MCP", role, lab: LAB.name, page });
const DAY = 86400000;
...
async function setSwitch(label, want, page = admin) {
  ...
  const resp = page.waitForResponse((r) => /\/api\/v1\/mcp-settings(\/allowed-clients\/[^/]+)?$/.test(new URL(r.url()).pathname) && r.request().method() === "PATCH");
  await sw.click();
...
function clientRow(name) {
  return admin.locator("div.flex.flex-wrap.items-center").filter({ has: admin.getByRole("switch", { name: `Enable ${name}`, exact: true }) }).first();
}
...
const allowed = async () => (await api(admin, BASE, "GET", "/api/v1/mcp-settings/allowed-clients")).body;
const grants = async () => (await api(admin, BASE, "GET", "/api/v1/mcp-settings/oauth-grants")).body;
...
async function consent(page, clientId, { toolsets, write, scope }) {
  ...
  const heading = flat(await page.getByRole("heading", { level: 1 }).first().innerText().catch(() => ""));
  const text = await main(page).catch(async () => flat(await page.locator("body").innerText()));
  const offered = await page.getByRole("checkbox").evaluateAll((els) => els.map((e) => e.closest("label")?.textContent?.trim()));
  const radios = await page.getByRole("radio").evaluateAll((els) => els.map((e) => e.closest("label")?.textContent?.trim().slice(0, 40)));
  ...
  await page.getByRole("radio", { name: write ? "Read and write" : "Read only", exact: true }).check();
  await page.getByRole("button", { name: "Allow", exact: true }).click();
  await until(() => lastCallback, "no callback", 20000);
  expectThat(lastCallback.searchParams.get("state") === state, "state mismatch");
  return { code: lastCallback.searchParams.get("code"), verifier, heading, text: text.slice(0, 300), offered, radios, allowDisabled };
}
...
async function exchange(clientId, secret, c) {
  const t = await token({ grant_type: "authorization_code", client_id: clientId, client_secret: secret, code: c.code, redirect_uri: REDIRECT, code_verifier: c.verifier });
  expectThat(t.status === 200 && t.body?.access_token, `token ${t.status} ${JSON.stringify(t.body)}`);
  return t.body;
}
const bearer = (access) => ({ authorization: `Bearer ${access}` });

// =====================================================================
await step(S("/settings/mcp"), "Enable OAuth Clients 1-3: MCP on; Legal Users and Business Users OAuth Clients on; pill beside Server address", "Switches save; pill appears only while an OAuth switch is on: Not reachable · HTTPS scheme, Public IPv4 address; API keys switches unchanged", async () => {
  await gotoMcp();
  ...
  const pill = flat(await admin.getByRole("status").filter({ hasText: /Not reachable|Reachable/ }).innerText());
  const s2 = await setSwitch("Business Users OAuth Clients", true);
  const keysL = await admin.getByRole("switch", { name: "Legal Users API keys" }).getAttribute("aria-checked");
  const keysB = await admin.getByRole("switch", { name: "Business Users API keys" }).getAttribute("aria-checked");
  const addr = (await main(admin)).match(/http\S+\/mcp/)?.[0];
  const caption = (await main(admin)).match(/OAuth Clients expose[^.]*\./)?.[0];
  expectThat(pillOff === 0 && s1 === 200 && s2 === 200 && pill === "Not reachable · HTTPS scheme, Public IPv4 address", JSON.stringify({ pillOff, s1, s2, pill }));
...
await step(S("/settings/mcp"), "Manage Allowed Clients: card starts open; four seeded Clients enabled; published identities with metadata URL captions, not editable; toggle one off and on", "Claude, Claude Code and ChatGPT Published identity; Microsoft 365 Copilot Registered client with No secret generated", async () => {
  await admin.reload();
  const cardOpen = await admin.getByRole("button", { name: "Allowed Clients", exact: true }).getAttribute("aria-expanded");
  const rows = {};
  for (const n of ["Claude", "Claude Code", "ChatGPT", "Microsoft 365 Copilot"]) {
    rows[n] = {
      text: flat(await clientRow(n).innerText()),
      enabled: await admin.getByRole("switch", { name: `Enable ${n}`, exact: true }).getAttribute("aria-checked"),
  ...
  const dyn = await admin.getByRole("switch", { name: "Dynamic client registration" }).getAttribute("aria-checked");
  expectThat(cardOpen === "true" && Object.values(rows).every((r) => r.enabled === "true") && rows.Claude.edit === 0 && rows["Microsoft 365 Copilot"].edit === 1 && /No secret generated/.test(rows["Microsoft 365 Copilot"].text), JSON.stringify(rows));
...
let copilot = {};
await step(S("/settings/mcp"), "Microsoft 365 Copilot template: Edit → Edit Client with Client name, Client id, Callback URLs (two read-only, one empty); fill the slot and Save; a non-loopback http callback is refused", "Fixed callbacks read-only; slot saves; http://192.168.1.5 callback shows the save failure", async () => {
  ...
  for (let i = 1; i <= 3; i++) {
    ...
    cbs.push({ value: await f.inputValue(), readOnly: (await f.getAttribute("readonly")) !== null });
  }
  ...
  await d.getByLabel("Callback URL 3").fill("https://copilotstudio.fictional.example/consent-redirect");
  ...
  expectThat(cbs[0].readOnly && cbs[1].readOnly && !cbs[2].readOnly && cbs[2].value === "" && /Generate a secret to create the client id\./.test(text) && /could not be saved/.test(refused) && saved[2] === "https://copilotstudio.fictional.example/consent-redirect", JSON.stringify({ cbs, refused, saved }));
...
await step(S("/settings/mcp"), "Template 1-2: Edit → Generate secret; Your Client secret is ready; Copy; Client id; Done", "One-time secret dialog; caption becomes Secret generated; the button becomes Rotate secret", async () => {
  await admin.getByRole("button", { name: "Edit Microsoft 365 Copilot", exact: true }).click();
  const d = admin.getByRole("dialog", { name: "Edit Client" });
  ...
  expectThat(s.secret.length > 16 && s.survived && /Secret generated/.test(caption) && rotate === 1 && idShown === s.clientId && del === 0, JSON.stringify({ caption, rotate, idShown, del }));
...
await step(S("/settings/mcp"), "Add Client: Client name and a loopback callback, Save; the editor stays open as Edit Client; Generate secret", "Editor turns into Edit Client; secret dialog; new Client starts enabled; Delete Client offered", async () => {
  ...
  const enabled = await admin.getByRole("switch", { name: `Enable ${c1Name}` }).getAttribute("aria-checked");
...
let grantA = {};
await step(S("/auth/consent", "legal_team_member"), "A Legal Team Member consents: choose from the requested Toolsets and Read and write (no Administrator approval)", "Consent page lists only requested Toolsets and both scope choices; Allow returns a code; tokens work on /mcp", async () => {
  const c = await consent(ltm, c1.clientId, { toolsets: ["Matters"], write: true, scope: "toolset:matters toolset:contracts write offline_access" });
  ...
  expectThat(tools.status === 200 && tools.names.includes("openlaw_matter_create") && !tools.names.includes("openlaw_contracts_list") && c.offered.length === 2, JSON.stringify({ offered: c.offered, names: tools.names }));
...
await step(S("/settings/mcp"), "Active keys and grants lists the grant with Owner, Client, Toolsets, Scope, Status, Granted, Last used and Expires (default 90 days)", "1 grant; Expires 90 days after Granted", async () => {
  await gotoMcp();
...
await step(S("/settings/mcp"), "Policy on an existing grant: Read-only, the Allowed Client switch and the group's OAuth Clients switch apply to the next request", "Read-only drops write Tools; Client off or group off → 401; back on → 200", async () => {
  ...
  expectThat(ro.status === 200 && !ro.names.includes("openlaw_matter_create") && clientOff.status === 401 && groupOff.status === 401 && back.status === 200, JSON.stringify([ro.names, clientOff.status, groupOff.status, back.status]));
...
let c1Secret2;
await step(S("/settings/mcp"), "Negative: Rotate secret invalidates the old secret at once and keeps the grant", "Old secret refused at the token endpoint; grant still listed; new secret refreshes", async () => {
  ...
  const s = await secretDialog();
  c1Secret2 = s.secret;
  const old = await token({ grant_type: "refresh_token", client_id: c1.clientId, client_secret: c1.secret, refresh_token: grantA.refresh });
  const after = (await grants()).filter((g) => g.clientName === c1Name).length;
  await admin.reload();
  await expandCard("Active keys and grants");
  const row = flat(await admin.getByRole("row").filter({ hasText: c1Name }).innerText());
  const fresh = await token({ grant_type: "refresh_token", client_id: c1.clientId, client_secret: c1Secret2, refresh_token: grantA.refresh });
  const works = fresh.status === 200 ? await rawToolsList(MCP_URL, bearer(fresh.body.access_token)) : { status: null };
  if (fresh.status === 200) grantA = { ...grantA, access: fresh.body.access_token, refresh: fresh.body.refresh_token ?? grantA.refresh };
  expectThat(s.clientId === c1.clientId && old.status >= 400 && before === 1 && after === 1 && fresh.status === 200 && works.status === 200, JSON.stringify({ old, before, after, fresh: fresh.status, works: works.status }));
...
await step(S("/settings/mcp"), "Dynamic client registration: on lets a Client register itself (Registered by the Client); off refuses new registrations and drops the endpoint from discovery but keeps the entry", "Entry enabled with caption Registered by the Client; after off, registration refused and entry unchanged; Delete Client removes it", async () => {
  ...
  const enabled = await admin.getByRole("switch", { name: `Enable ${selfName}` }).getAttribute("aria-checked");
  ...
  const enabledAfter = await admin.getByRole("switch", { name: `Enable ${selfName}` }).getAttribute("aria-checked");
...
await step(S("/settings/mcp"), "Revoke a grant from Active keys and grants: Revoke → Revoke OAuth grant → Revoke", "Next request refused; grant leaves the list", async () => {
  await admin.reload();
  await expandCard("Active keys and grants");
  await admin.getByRole("row").filter({ hasText: c1Name }).getByRole("button", { name: "Revoke", exact: true }).click();
  ...
  const refresh = await token({ grant_type: "refresh_token", client_id: c1.clientId, client_secret: c1Secret2, refresh_token: grantA.refresh });
  const counts = (await main(admin)).match(/\d+ keys? · \d+ grants?/)?.[0];
  expectThat(after.status === 401 && refresh.status >= 400, JSON.stringify([after.status, refresh.status]));
...
let grantD = {};
let grantN = {};
await step(S("/settings/mcp-limits"), "OAuth grant lifetime (days): set 7 in Settings → Organization → Advanced → MCP and Save", "Settings saved. Restart the API and worker to apply changes.; a grant made before the restart still gets 90 days", async () => {
  await gotoMcp();
  await admin.getByRole("button", { name: "Add Client", exact: true }).click();
  ...
  await admin.getByRole("dialog", { name: "Edit Client" }).getByRole("button", { name: "Generate secret", exact: true }).click();
  c2 = await secretDialog();
  const nav = await openSettings(admin);
  if ((await nav.getByRole("link", { name: "MCP", exact: true }).count()) < 2) await nav.getByRole("button", { name: "Advanced" }).click();
  await until(async () => (await nav.getByRole("link", { name: "MCP", exact: true }).count()) > 1, "Advanced MCP not shown");
  ...
  const msg = await admin.getByText("Settings saved. Restart the API and worker to apply changes.").innerText({ timeout: 15000 });
  const pending = (await main(admin)).match(/Saved changes are waiting for a restart[^.]*\./)?.[0] ?? null;
  const c = await consent(admin, c2.clientId, { toolsets: ["Contracts"], write: false, scope: "toolset:contracts offline_access" });
...
await step(S("container", "operator"), "Operator restarts the API and worker so the saved lifetime applies", "App answers again", async () => {
  mcplanCompose(["restart", "app", "worker"], ["overlay-unpin.json"]);
  await waitHealthy(BASE);
  return "docker compose restart app worker; health check passed.";
});
await step(S("/settings/mcp"), "New grants use the lifetime after restart; existing grants keep their expiry; refresh does not extend it", "New grant 7 days; Daniel's earlier grant keeps its 90-day expiry after a refresh", async () => {
  const c = await consent(ltm, c2.clientId, { toolsets: ["Contracts"], write: false, scope: "toolset:contracts offline_access" });
  const t = await exchange(c2.clientId, c2.secret, c);
  ...
  const r = await token({ grant_type: "refresh_token", client_id: c2.clientId, client_secret: c2.secret, refresh_token: grantD.refresh });
  const d2 = (await grants()).find((x) => x.clientName === c2Name && x.owner === "Daniel Okafor");
  await gotoMcp();
  await expandCard("Active keys and grants");
  const rows = (await admin.getByRole("row").filter({ hasText: c2Name }).allInnerTexts()).map(flat);
  ...
  expectThat(nDays === 7 && d.expiresAt === grantD.expiresAt && r.status === 200 && d2.expiresAt === grantD.expiresAt, JSON.stringify({ nDays, d, r: r.status, d2 }));
...
await step(S("/settings/api-keys", "legal_team_member"), "People disconnect their own Clients: Connected Clients on the API keys pane → Disconnect → Revoke OAuth grant → Revoke", "Grant leaves Connected Clients; its next request is refused", async () => {
...
await step(S("/settings/mcp"), "Delete Client removes the entry and its grants, and stops their access", "Daniel's grant for the deleted Client is gone and refused", async () => {
  await gotoMcp();
  ...
  expectThat(before === 1 && after === 0 && refused.status === 401 && !listed, JSON.stringify({ before, after, refused: refused.status }));
  return `Grants for ${c2Name} before ${before}, after ${after}; entry listed ${listed}; Daniel's token now ${refused.status}.`;
});

log.runs.m41.articleHashesAtEnd = { "configure-mcp": articleHash("configure-mcp"), "connect-headless-client": articleHash("connect-headless-client") };
save();
await browser.close();
cb.close();
const failed = log.steps.filter((s) => s.phase === "m41" && s.result !== "pass");
console.log(`m41 done: ${log.steps.filter((s) => s.phase === "m41").length} steps, ${failed.length} failed`);
process.exit(0);
