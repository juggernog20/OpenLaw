...
export async function runC19(ctx, step) {
  ...
  const keyDatesTab = async () => {
    ...
    try {
      ...
      ctx.records.push({ note: `Key dates section for C-${num} did not render within 20 s once; a reload rendered it` });
    }
    await sleep(600);
    return p.getByRole("region", { name: "Key dates" });
  };
  const apiTasks = async () => (await actor.api("GET", `/contracts/${num}/tasks`)).json;
  const teamNames = async () => JSON.stringify((await actor.api("GET", `/contracts/${num}`)).json.team ?? []);
  const taskComments = async (id) => (await actor.api("GET", `/comments?entityType=contract_task&entityId=${id}`)).json?.comments ?? [];
  const kdRows = async () => {
    const region = await keyDatesTab();
    const rows = region.getByRole("row");
    const n = await rows.count();
    const out = [];
    for (let i = 1; i < n; i++) out.push({ text: await text(rows.nth(i)), actions: await rows.nth(i).getByRole("button").count() });
    return out;
  };
  ...
  await step(
    ...
    async () => {
      const created = await actor.api("POST", "/contracts", {
        title: `DOC-029 contracts-b C19 ${tag}`,
      ...
      ctx.records.push({ article: "contract-tasks-and-dates", role, purpose: "Tasks and Key dates", reference: `C-${num}` });
      return `C-${num} "DOC-029 contracts-b C19 ${tag}" with Legal Owner ${PEOPLE[role].name}; team adds ${PEOPLE[other].name} and ${PEOPLE.businessUser.name} (Business User).`;
    },
  );
  ...
  await step(
    ...
    async () => {
      ...
      await dialog.getByRole("textbox", { name: "Title" }).fill(`DOC-029 contracts-b C19 cancelled ${tag}`);
      ...
      expectThat(offered.some((s) => s.includes(PEOPLE[role].name)) && offered.some((s) => s.includes(PEOPLE[other].name)), `offered ${offered.join(", ")}`);
      expectThat(!offered.some((s) => s.includes(PEOPLE.businessUser.name)), `Business User offered: ${offered.join(", ")}`);
      ...
      const deferred = await text(picker.getByText(/Team membership and assignment are saved when you save the task/));
  ...
  await step(
    ...
    async () => {
      ...
      await region.getByRole("button", { name: "Add task" }).click();
      const dialog = p.getByRole("dialog", { name: "Add a task" });
      const title = `DOC-029 contracts-b C19 review notice ${tag}`;
      ...
      await dialog.getByRole("textbox", { name: "Add a note" }).fill("DOC-029 initial note for the reviewer.");
      await dialog.locator("input[type=file]").first().setInputFiles(fix("doc029-services-agreement.pdf"));
      ...
      expectThat(task && task.assigneeName === addCandidate && task.dueDate === isoPlusDays(20), `task ${JSON.stringify(task)}`);
      expectThat(team.includes(addCandidate), `${addCandidate} not on team`);
      expectThat(first && first.visibility === "working_team" && detailsText.includes("DOC-029 initial note") && detailsText.includes("doc029-services-agreement.pdf"), `comment ${JSON.stringify(first)?.slice(0, 300)}; details ${detailsText.slice(0, 300)}`);
  ...
  await step(
    ...
    async () => {
      ...
      await region.getByRole("button", { name: "Add task" }).click();
      const dialog = p.getByRole("dialog", { name: "Add a task" });
      const title = `DOC-029 contracts-b C19 retry ${tag}`;
      await dialog.getByRole("textbox", { name: "Title" }).fill(title);
      await dialog.getByRole("textbox", { name: "Add a note" }).fill("DOC-029 note that fails once.");
      let failed = 0;
  ...
  await step(
    ...
    async () => {
      ...
      await withResponse(p, (r) => r.request().method() !== "GET" && r.url().includes("/tasks/"), () => region.getByRole("checkbox", { name: `Complete task: ${title}` }).click());
      await sleep(800);
  ...
  await step(
    ...
    async () => {
      const retryTitle = `DOC-029 contracts-b C19 retry ${tag}`;
      const emptyTitle = `DOC-029 contracts-b C19 empty ${tag}`;
      ...
      const refusal = tasksText.match(/[^.]*(cannot|could not|conversation|history|comments)[^.]*\./i)?.[0]?.trim() ?? `(no refusal sentence found in: ${tasksText.slice(0, 200)})`;
  ...
  await step(
    ...
    async () => {
      ...
      await withResponse(p, (r) => r.request().method() === "PATCH", () => p.getByRole("dialog", { name: "Assign task" }).getByRole("button", { name: "Unassigned" }).click());
      await sleep(800);
  ...
  await step(
    ...
    async () => {
      ...
      await details.getByRole("button", { name: "Close" }).first().click();
      const recordThread = (await actor.api("GET", `/comments?entityType=contract&entityId=${(await read(num)).id}`)).json;
      const leaked = JSON.stringify(recordThread).includes("DOC-029 initial note");
      expectThat(dragHandles === 0 && moveButtons === 0, `drag ${dragHandles} move ${moveButtons}`);
      expectThat(/Legal only/.test(labels) && /Working team/.test(labels) && !/Full thread/.test(labels), `audiences "${labels}"`);
  ...
  await step(
    ...
    async () => {
      ...
      await dialog.getByRole("textbox", { name: "Event" }).fill(`DOC-029 price review ${tag}`);
      ...
      const row = rows.find((r) => r.text.includes(`DOC-029 price review ${tag}`));
      ...
      expectThat(/45 days before/.test(combined) && thirtyCount === 1 && /60 days before/.test(withSixty) && !/60 days before/.test(afterRemove), `combined "${combined}" / "${withSixty}" / "${afterRemove}"`);
      expectThat(boxesText.includes(PEOPLE[other].name) && !boxesText.includes(PEOPLE.businessUser.name), `recipient list "${boxesText}"`);
      expectThat(row && row.text.includes("Key date") && row.actions === 1, `row ${JSON.stringify(row)}`);
  ...
  await step(
    ...
    async () => {
      ...
      await addKd(isoPlusDays(-10), `DOC-029 past check ${tag}`);
      await addKd(isoPlusDays(10), `DOC-029 near check ${tag}`);
      await addKd(isoPlusDays(200), `DOC-029 remove me ${tag}`);
      ...
      const label = `DOC-029 price review ${tag}`;
      ...
      await dialog.waitFor({ state: "hidden" });
      region = await keyDatesTab();
      await region.getByRole("button", { name: `Actions for DOC-029 remove me ${tag}` }).click();
      ...
      expectThat(!cancelled && idx(`${label} saved`) >= 0 && idx("remove me") < 0, `cancelled ${cancelled}; rows ${texts.join(" | ")}`);
      expectThat(derived.length >= 1 && derived.every((r) => r.actions === 0), `derived ${JSON.stringify(derived)}`);
      expectThat(idx("near check") < idx(`${label} saved`) && idx(`${label} saved`) < idx("Current term expires") && idx("Current term expires") < idx("past check"), `order ${texts.join(" | ")}`);
  ...
  await step(
    ...
    async () => {
      const label = `DOC-029 price review ${tag} saved`;
      ...
      const find = (j) => JSON.stringify(j).match(/\{[^{}]*"label":"DOC-029 price review[^{}]*\}/)?.[0] ?? "";
      const back = await actor.api("POST", `/contracts/${num}/team`, { userId: userId(PEOPLE[other].name) });
      const b = find(before);
      const a = find(after);
      expectThat(/reminderRecipientIds":\["/.test(b) && !/reminderRecipientIds":\["/.test(a), `before ${b} after ${a}`);
  ...
  await step(
    ...
    async () => {
      const overdueTitle = `DOC-029 contracts-b C19 overdue ${tag}`;
      ...
      const readCell = async () => {
        ...
        for (let i = 0; i < 20; i++) {
          const row = p.getByRole("row").filter({ has: p.getByRole("cell", { name: `C-${num}`, exact: true }) });
          if (await row.count()) return { cell: await text(row.getByRole("cell").nth(col)), viewNote };
      ...
      await p.goto(`${BASE}/contracts/${num}/tasks`);
      await withResponse(p, (r) => r.request().method() !== "GET" && r.url().includes("/tasks/"), () => p.getByRole("region", { name: "Tasks" }).getByRole("checkbox", { name: `Complete task: ${overdueTitle}` }).click());
      const toggled = (await apiTasks()).tasks.find((x) => x.title === overdueTitle);
      const second = await readCell();
      const restored = ctx.restoreView ? `re-selected view "${await selectContractsView(p, ctx.restoreView)}" afterwards` : "made no view change";
      expectThat(first.cell.includes("overdue") && toggled.isDone, `first "${first.cell}" done ${toggled.isDone}`);
  ...
  await step(
    ...
    async () => {
      const title = `DOC-029 contracts-b C19 mine ${tag}`;
      const t = await actor.api("POST", `/contracts/${num}/tasks`, { title, assigneeId: userId(PEOPLE[role].name), dueDate: isoPlusDays(2) });
      expectThat(t.status === 201, `task ${t.status}`);
      await p.goto(`${BASE}/`);
      await p.getByRole("region", { name: "Tasks assigned to you" }).waitFor();
      await sleep(1500);
      const regions = await p.getByRole("main").getByRole("region").evaluateAll((els) => els.map((e) => e.querySelector("h2")?.textContent));
      ...
      const homeTask = p.getByRole("region", { name: "Tasks assigned to you" }).getByRole("link", { name: new RegExp(title) });
      const homeTaskText = (await homeTask.count()) ? `Home Tasks assigned to you lists it (${await homeTask.first().getAttribute("href")})` : "Home Tasks assigned to you shows the first rows only and did not include it; View all opens My Tasks";
      ...
      if (await dateLink.count()) {
        ...
        await cal.waitFor();
        await sleep(1500);
        const whole = cal.getByRole("button", { name: "Show whole month" }).or(cal.getByRole("checkbox", { name: "Show whole month" })).or(cal.getByRole("switch", { name: "Show whole month" }));
      ...
      expectThat(/lists "/.test(dateEvidence) && dateEvidence.includes(`/contracts/${num}`), `${dateEvidence}; contracts "${contractsText.slice(0, 300)}"`);
  ...
  await step(
    ...
    async () => {
      ...
      const enabledBoxes = await region.getByRole("checkbox").evaluateAll((els) => els.filter((e) => !e.disabled && e.getAttribute("aria-disabled") !== "true").length);
      ...
      const w1 = await actor.api("POST", `/contracts/${num}/tasks`, { title: "DOC-029 archived write" });
      const w2 = await actor.api("POST", `/contracts/${num}/key-dates`, { date: isoPlusDays(5), label: "DOC-029 archived write" });
      const r = await actor.api("POST", `/contracts/${num}/restore`);
      region = await tasksTab();
      const addAfter = await region.getByRole("button", { name: "Add task" }).count();
      expectThat(addTask === 0 && enabledBoxes === 0 && addDate === 0 && kdActions === 0 && w1.status === 409 && w2.status === 409 && r.status === 200 && addAfter === 1, `addTask ${addTask} boxes ${enabledBoxes} addDate ${addDate} kdActions ${kdActions} w1 ${w1.status} w2 ${w2.status} restore ${r.status} after ${addAfter}`);
