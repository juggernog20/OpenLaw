...
Expand **Provider** if collapsed. Choose **Provider**, then supply the controls it exposes. **Model** is editable for every provider; a prefilled name is a starting value from this OpenLaw build, not proof that your account can use it.
...
| **Azure OpenAI**    | An API key, the full **Deployment endpoint** for [chat completions](https://learn.microsoft.com/en-us/rest/api/microsoft-foundry/azureopenai/chat), and the deployment's model name. Include the complete chat-completions path and any required API-version query; a resource host alone is insufficient. |
...
1. Choose the provider and enter its required endpoint and **Model**.
2. Paste **API key**, then select **Save connector**. The key is encrypted and write-only; the saved value is never displayed.
...
To rotate the key, paste a new **API key**, select **Save connector**, and test while the connector is enabled. Leaving the field blank keeps the stored key, even if you change provider. Paste the correct new provider key when switching accounts; choosing Ollama does not erase a previously stored key.
