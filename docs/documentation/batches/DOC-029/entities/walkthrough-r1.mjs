...
//   LAB_PASSWORD=... mise exec -- node docs/documentation/batches/DOC-029/entities/walkthrough-r1.mjs
...
const ARTICLES = ["entities-and-counterparties", "entity-records", "entity-obligations", "entity-structure-and-access"];
const articleHashes = Object.fromEntries(
  ARTICLES.map((id) => [id, createHash("sha256").update(readFileSync(path.join(ROOT, "docs/user-guides", `${id}.md`))).digest("hex")]),
);
// Lab identity from the helper's manifest; only non-secret identity fields are copied.
const manifest = JSON.parse(readFileSync(path.join(ROOT, ".documentation-labs/entities/lab.json"), "utf8"));
const running = Object.fromEntries(
  ["app", "doc-engine", "worker"].map((service) => [
    service,
    execFileSync("docker", ["inspect", "--format", "{{.Image}}", `${manifest.project}-${service}-1`]).toString().trim(),
  ]),
);
...
const log = {
  ...
  group: "entities",
  round: 1,
  walkthroughReviewer: "DOC-029 independent walkthrough agent (entities, round 1)",
  reviewerKind: "agent",
  ...
  seed: { scale: manifest.seed?.scale, randomSeed: manifest.seed?.randomSeed, completedAt: manifest.seed?.completedAt, ai: manifest.seed?.ai, signing: manifest.seed?.signing },
  articleHashes,
...
async function shot(page, name, list, purpose) {
  ...
  list.push({ path: `docs/documentation/batches/DOC-029/entities/${name}`, purpose, url: new URL(page.url()).pathname, at: new Date().toISOString(), theme: "light (default)", viewport: "1440x1000" });
}
function save() {
  ...
  try {
    ...
    if (process.env.DEBUG_SHOTS) {
      for (const [key, c] of Object.entries(contexts)) {
        ...
        await c.page.screenshot({ path: path.join(process.env.DEBUG_SHOTS, `fail-${log.steps.length}-${key}.png`), fullPage: true }).catch(() => {});
      }
    }
  }
...
async function magicSession(key) {
  ...
  for (let i = 0; i < 40 && !href; i++) {
    ...
    if (fresh) {
      ...
      const match = message.Text.match(/https?:\/\/\S+/g)?.find((u) => /auth|magic|verify|token/i.test(u));
      if (match) {
...
function watch(page) {
  page.__bad = [];
  page.on("response", (r) => {
    if (r.status() >= 400) page.__bad.push(`${r.status()} ${r.request().method()} ${new URL(r.url()).pathname}`);
  });
  page.on("console", (m) => {
...
async function checkErrorScreen(page, url) {
  if (await page.getByText("Something went wrong.").isVisible().catch(() => false)) {
    log.errorScreens.push({ at: new Date().toISOString(), scenario: current.scenario, url: new URL(page.url()).pathname, requested: url, responses: [...page.__bad] });
...
async function typeId(s, name) {
  if (!typeIds) {
    ...
    typeIds = Object.fromEntries((r.json.entities ?? []).map((e) => [e.entityTypeName, e.entityTypeId]));
  }
...
async function fixtureContract(s, title, extra = {}) {
  if (!ndaTypeId) {
    ...
    ndaTypeId = (list.json.contracts ?? []).find((c) => c.contractTypeName === "NDA")?.contractTypeId;
  }
  const r = await api(s, "POST", "/api/v1/contracts", { title, contractTypeId: ndaTypeId, ...extra });
...
async function c31(roleKey) {
  ...
  const legalName = `DOC-029 entities C31 ${who} Aldoria Holdings ${TAG}`;
  let entityId = null;

  await step(R, "Register step 1: open Entities; first view is Calendar; select List", "Entities opens on Calendar; List shows the registry table.", async () => {
    await page.getByRole("navigation", { name: "Primary" }).getByRole("link", { name: "Entities" }).click();
    await settle(page);
    ...
    await page.getByRole("navigation", { name: "Registry view" }).getByRole("link", { name: "List" }).click();
    await settle(page);
  ...
  await step(R, "Register step 2: Search entities by name, then Type/Status/Jurisdiction/Majority owner filters and Clear all", "Search narrows by legal name; each filter exists and narrows; Clear all removes filters.", async () => {
    ...
    expect(names.length >= 1 && names.every((n) => /Helix Software GmbH/.test(n)), `search rows ${q(names)}`);
    await search.fill("");
    await settle(page);
    const total = await page.locator("main table tbody tr").count();
    for (const label of ["Type", "Status", "Jurisdiction", "Majority owner"]) {
      expect(await page.getByRole("combobox", { name: label, exact: true }).isVisible(), `no ${label} filter`);
    }
    await page.getByRole("combobox", { name: "Status", exact: true }).selectOption({ label: "Active" });
    await page.getByRole("combobox", { name: "Type", exact: true }).selectOption({ label: "LLC" });
    await settle(page);
    const filtered = await page.locator("main table tbody tr").allInnerTexts();
    expect(filtered.length > 0 && filtered.length < total && filtered.every((r) => /LLC/.test(r) && /Active/.test(r)), `filtered rows ${filtered.length}/${total}`);
    await page.getByRole("button", { name: /^Clear all/ }).first().click();
    await settle(page);
    const cleared = await page.locator("main table tbody tr").count();
    const typeValue = await page.getByRole("combobox", { name: "Type", exact: true }).inputValue();
  ...
  await step(R, "Register steps 3-5: Add entity with identity details, Status default Active, Register, open the new Entity; creator Grant", "Dialog titled Add entity; Status defaults to Active; Register saves the details; the adder holds a Grant.", async () => {
    ...
    if ((await typeControl.evaluate((e) => e.tagName)) === "SELECT") await typeControl.selectOption({ label: "Corporation" });
    else {
    ...
    expect(await page.getByRole("heading", { level: 1, name: legalName }).isVisible(), "new Entity did not open");
    const values = {};
    for (const label of ["Formation jurisdiction", "Formed on", "Registration no.", "Tax ID", "Registered agent", "Registered address"]) {
      values[label] = await page.getByRole("main").getByLabel(label, { exact: true }).inputValue();
    }
    expect(values["Registration no."] === `ALD-${TAG}` && values["Formed on"] === "2019-03-14", `saved ${q(values)}`);
    const statusSaved = await page.getByRole("main").getByLabel("Status", { exact: true }).locator("option:checked").innerText();
    expect(statusDefault === "Active" && statusSaved === "Active", `Status default ${statusDefault}, saved ${statusSaved}`);
    ...
    expect(manage && names.includes(s.person.name), `Manage access ${manage}; grants ${q(names)}`);
  ...
  await step(R, "Register step 5: change one field and move focus away; a selection saves when chosen; both survive reload; Status Dormant does not archive", "Registered agent saves on blur; Status Dormant saves on selection; no Archived mark; Archive still offered.", async () => {
    ...
    await page.getByRole("main").getByLabel("Status", { exact: true }).selectOption({ label: "Dormant" });
    await settle(page);
    await reload(page);
    const agentAfter = await page.getByRole("main").getByLabel("Registered agent", { exact: true }).inputValue();
    const statusAfter = await page.getByRole("main").getByLabel("Status", { exact: true }).locator("option:checked").innerText();
    const archiveOffered = await page.getByRole("button", { name: "Archive", exact: true }).isVisible();
    const restoreOffered = await page.getByRole("button", { name: "Restore", exact: true }).isVisible();
    expect(agentAfter === "Aldoria Registered Agents Ltd" && statusAfter === "Dormant", `after reload agent=${agentAfter} status=${statusAfter}`);
  ...
  await step(R, "Names are not unique: registering the same legal name again creates another Entity", "A second Register with the same name creates a distinct record; the first is not changed.", async () => {
    await go(page, "/entities?view=list");
    await page.getByRole("button", { name: "Add entity" }).click();
    const dialog = page.getByRole("dialog", { name: "Add entity" });
    await dialog.getByLabel("Legal name").fill(legalName);
    const typeControl = dialog.getByLabel("Entity type");
    if ((await typeControl.evaluate((e) => e.tagName)) === "SELECT") await typeControl.selectOption({ label: "LLC" });
    ...
    const hrefs = await page.locator("main table tbody tr").getByRole("link", { name: legalName }).evaluateAll((a) => a.map((x) => x.getAttribute("href")));
    expect(rows.length === 2 && new Set(hrefs).size === 2, `rows ${q(rows)}`);
    expect(rows.some((r) => /Republic of Aldoria/.test(r) && /Dormant/.test(r)) && rows.some((r) => /Duchy of Kessel/.test(r)), "first record changed");
  ...
  await step(R, "Negative: blank Legal name has a clear outcome", "Register with no Legal name keeps the dialog open with the refusal message and creates nothing.", async () => {
    const probe = `/api/v1/entities?q=${encodeURIComponent("DOC-029 entities")}`;
    ...
    expect(stillOpen && count(before) === count(after), `open=${stillOpen} before=${count(before)} after=${count(after)}`);
  ...
  // Contract parties
  const contract = await fixtureContract(s, `DOC-029 entities C31 ${who} parties contract ${TAG}`);
  const cedar = `DOC-029 entities Cedar Supply ${who} ${TAG}`;
  const birch = `DOC-029 entities Birch Logistics ${who} ${TAG}`;
  const counterparties = page.getByRole("combobox", { name: "Counterparties" });

  await step(R, "Parties step 1: choose Our entity on a Contract; it survives reload", "Our entity saves the chosen Entity.", async () => {
    await go(page, `/contracts/${contract.number}`);
    const our = page.getByRole("combobox", { name: "Our entity" });
    const option = our.locator("option", { hasText: "DOC-029 entities C31" }).filter({ hasText: legalName }).first();
    const value = await option.getAttribute("value");
    await our.selectOption(value);
    await settle(page);
    await reload(page);
    const shown = await page.getByRole("combobox", { name: "Our entity" }).locator("option:checked").innerText();
    expect(shown === legalName, `Our entity after reload ${shown}`);
    return `C-${contract.number} Our entity read ${q(shown)} after a reload.`;
  });

  await step(R, "Parties steps 2-3: type a new name and Create; the first Counterparty is Primary", "Create adds the new Counterparty to the Contract with the Primary mark.", async () => {
    ...
    expect(detail.names.length === 1 && detail.primary === cedar && shownPrimary === 1, `parties ${q(detail)} primary marks ${shownPrimary}`);
  ...
  await step(R, "Parties step 4: add another Counterparty and Make primary", "Make primary moves the Primary mark to the chosen Counterparty.", async () => {
  ...
  await step(R, "Picker negatives: an exact existing name in another case offers no Create; a Counterparty on the Contract is not offered again", "No Create row for a case variant; the attached party is not listed.", async () => {
  ...
  await step(R, "Parties step 5: Take name off the contract; next remaining takes Primary; shared record remains; final removal leaves none", "Removing the Primary passes the mark; removal does not delete the shared Counterparty; removing the last leaves no Counterparties.", async () => {
    ...
    expect(afterFirst.primary === cedar && !afterFirst.names.includes(birch), `after removing primary ${q(afterFirst)}`);
  ...
  await step(R, "Recovery: an archived Entity is not offered for a new selection; the existing Contract keeps its reference", "Archive the chosen Entity; Our entity keeps showing it on C-n; another Contract does not offer it.", async () => {
    await go(page, `/entities/${entityId}`);
    await page.getByRole("button", { name: "Archive", exact: true }).click();
    await settle(page);
    const other = await fixtureContract(s, `DOC-029 entities C31 ${who} second contract ${TAG}`);
    await go(page, `/contracts/${contract.number}`);
    const kept = await page.getByRole("combobox", { name: "Our entity" }).locator("option:checked").innerText();
    await go(page, `/contracts/${other.number}`);
    const options = await page.getByRole("combobox", { name: "Our entity" }).locator("option").allInnerTexts();
    const offered = options.filter((o) => o.includes(legalName));
    const values = await page.getByRole("combobox", { name: "Our entity" }).locator("option").evaluateAll((os) => os.map((o) => o.value));
    expect(kept.includes(legalName) && offered.length === 1 && !values.includes(entityId), `kept ${q(kept)} offered ${q(offered)} archived id offered ${values.includes(entityId)}`);
...
async function c31BusinessUsers() {
  ...
  for (const key of ["business_user_ravi", "business_user_magic"]) {
    const label = key === "business_user_magic" ? "business_user (Jonas Weber, magic link)" : "business_user (Ravi Menon, magic link)";
    await step(label, "Negative: a Business User cannot use the Member+ Entities destination", "No Entities navigation item; /entities sends the reader away; the Entities API refuses.", async () => {
      ...
      const r = await api(s, "GET", "/api/v1/entities");
      expect(nav === 0 && !landed.startsWith("/entities") && r.status === 403, `nav=${nav} landed=${landed} api=${r.status}`);
...
async function c32(roleKey) {
  ...
  const R = roleKey;
  const who = R === "administrator" ? "A" : "L";
  const main = await fixtureEntity(s, `DOC-029 entities C32 ${who} main ${TAG}`, { jurisdiction: "Republic of Aldoria" });
  const owned = await fixtureEntity(s, `DOC-029 entities C32 ${who} owned ${TAG}`);
  const third = await fixtureEntity(s, `DOC-029 entities C32 ${who} third ${TAG}`);
  const coOwner = await fixtureEntity(s, `DOC-029 entities C32 ${who} co-owner ${TAG}`);
  const officer = `Ines Castell ${who} ${TAG}`;
  const other = R === "administrator" ? "Nadia Haddad" : "Daniel Okafor";
  const body = page.getByRole("main");

  await step(R, "Officers steps 1-3: Directors & Officers, Add director or officer, name, Role, Appointed on, Linked user, Add", "The Officer row saves name, role, date and linked user.", async () => {
    ...
    expect(row.name === officer && row.role === "Director" && row.appointed === "2022-05-09" && row.user === other, q(row));
    return `After Add and a reload the row read ${q(row)}.`;
  });

  await step(R, "Officers step 4: correct text and move focus away; choose another role saves", "Name edit saves on blur; Role selection saves at once.", async () => {
    const card = page.getByRole("region", { name: "Directors & Officers" });
  ...
  await step(R, "Negative: a resignation date before the appointment date is refused", "Refusal message; the appointment date is unchanged and no resignation is stored.", async () => {
    const card = page.getByRole("region", { name: "Directors & Officers" });
    await card.getByLabel(`${officer} Resigned on`).fill("2021-01-01");
    await blurTo(page, "Directors & Officers");
    const message = await page.getByText("The resignation date cannot be before the appointment date.").first().isVisible();
    await reload(page);
    ...
    expect(message && appointed === "2022-05-09" && resigned === "", `message=${message} appointed=${appointed} resigned=${resigned}`);
  ...
  await step(R, "Officers step 5: Resigned on retains a resignation; Show former reads former Officers; clearing the date returns the Officer", "The resigned Officer leaves the current list, returns with Show former, and clearing the date restores it as current.", async () => {
    const card = page.getByRole("region", { name: "Directors & Officers" });
    ...
    expect(currentCount === 0 && former === "2024-08-31" && back === 1, `current=${currentCount} former=${former} back=${back}`);
  ...
  await step(R, "Officer note: Linking a user does not grant that person Entity access", "The linked user gains no Grant on the Entity.", async () => {
  ...
  await step(R, "Officer note: Remove name deletes the Officer entry", "The Officer is absent even with Show former.", async () => {
    const card = page.getByRole("region", { name: "Directors & Officers" });
  ...
  const obligationLabel = `DOC-029 entities C32 ${who} licence renewal ${TAG}`;
  await step(R, "Registrations steps 1-3: Add registration with Jurisdiction, Registration number, Registered agent, Status Lapsed; Formation jurisdiction unchanged", "The row saves; the Entity's Formation jurisdiction stays the same.", async () => {
    ...
    expect(row.number === `NV-${TAG}` && row.status === "Lapsed" && formation === "Republic of Aldoria", `${q(row)} formation=${formation}`);
    return `The Registration ${jur} saved ${q(row)}. Formation jurisdiction still read ${q(formation)}.`;
  });

  await step(R, "Registrations step 3: edit text and move focus away; a Status selection saves immediately", "Agent saves on blur; Withdrawn saves on selection.", async () => {
    const card = page.getByRole("region", { name: "Registrations" });
  ...
  await step(R, "Registrations step 4 and note: Remove jurisdiction registration detaches linked Obligations and keeps them", "The Obligation linked to the Registration remains with no Registration link.", async () => {
    ...
    const registration = kept ? await page.getByLabel(`${obligationLabel} registration`).locator("option:checked").innerText() : null;
    expect(gone && kept === 1 && registration === "None", `gone=${gone} kept=${kept} registration=${registration}`);
  ...
  await step(R, "Holdings steps 1-4: Ownership tab, Add Holding, This Entity owns, Entity, Ownership percent, Add; Owners and Owned Entities on both records", "Holding appears under Owned Entities on the owner and under Owners on the owned Entity.", async () => {
    ...
    expect(ownedList.includes(owned.legalName) && owners.includes(main.legalName) && pct === "60", `owned=${q(ownedList)} owners=${q(owners)} pct=${pct}`);
  ...
  await step(R, "Holdings: Owns this Entity direction; over-100 total warns but saves; correcting the row on blur clears it", "Second owner at 60% saves with 'Ownership totals 120% for name.'; changing a row to 40 saves and clears the warning.", async () => {
    ...
    expect(warned && saved === "60" && corrected === "40" && still === 0, `warned=${warned} saved=${saved} corrected=${corrected} warnings=${still}`);
  ...
  await step(R, "Negative: an out-of-range row percentage is refused without changing the saved value", "150 is refused; the row still reads 40 after reload.", async () => {
    ...
    expect(value === "40" && other === "60" && alerts.length > 0, `value=${value} other=${other} alerts=${q(alerts)}`);
    return `Entering 150 showed ${q(alerts)}. After a reload the row still read ${value} and the other Holding still read ${other}.`;
  });

  await step(R, "Holdings note: the Entity cannot own itself, duplicate a directional Holding, or create a loop", "Self and existing related Entities are not offered; a loop is refused with a message.", async () => {
    await go(page, `/entities/${owned.id}/ownership`);
    await page.getByRole("button", { name: "Add Holding" }).click();
    const dialog = page.getByRole("dialog", { name: "Add Holding" });
    const combo = dialog.getByRole("combobox", { name: "Entity" });
    await combo.fill(`DOC-029 entities C32 ${who}`);
    const options = await dialog.getByRole("listbox", { name: "Entity matches" }).getByRole("option").allInnerTexts();
    ...
    return `On ${owned.legalName} the picker for "DOC-029 entities C32 ${who}" offered ${q(options)}: not itself and not ${main.legalName}, which already holds it. Adding ${third.legalName} as owner of ${main.legalName} (closing a loop through ${owned.legalName}) was refused with ${q(message)} and no Owner row was added.`;
  });

  await step(R, "Holdings step 5: Remove name removes the Holding; neither Entity is deleted", "The Holding leaves both records; both Entities still open.", async () => {
    await go(page, `/entities/${owned.id}/ownership`);
  ...
  await step(R, "Documents: a new Entity has no pre-created folders; Upload, Choose files, Kind Executed, Upload gives a v1 row", "Empty Documents tab; the upload lands as v1 on this Entity only.", async () => {
    ...
    const [chooser] = await Promise.all([page.waitForEvent("filechooser"), dialog.getByRole("button", { name: /Choose files/ }).click()]);
    await chooser.setFiles(path.join(here, "fixtures", "doc029-entities-certificate.txt"));
    await dialog.getByLabel("Kind").selectOption({ label: "Executed" });
    await dialog.getByRole("button", { name: "Upload", exact: true }).click();
    await dialog.waitFor({ state: "hidden", timeout: 20000 });
    await reload(page);
    const rowLocator = page.locator("main table tbody tr", { hasText: "doc029-entities-certificate" }).first();
    const kind = await rowLocator.locator("select").first().locator("option:checked").innerText().catch(() => null);
    ...
    expect(empty && / v1$/.test(row) && kind === "Executed" && sibling === 0, `empty=${empty} row=${q(row)} sibling=${sibling}`);
  ...
  await step(R, "Archive step 1: Archive saves immediately; Archived mark; editable controls unavailable", "Archived mark and restore note; Legal name disabled; add buttons absent.", async () => {
    await go(page, `/entities/${main.id}`);
    await page.getByRole("button", { name: "Archive", exact: true }).click();
    await settle(page);
    await reload(page);
    ...
    expect(mark && note && nameDisabled && addOfficer === 0 && addRegistration === 0 && addObligation === 0, q({ mark, note, nameDisabled, addOfficer, addRegistration, addObligation }));
  ...
  await step(R, "Archive: the archived Entity leaves ordinary registry and Holding picker results", "Not in List search without Show archived; not offered in Add Holding.", async () => {
    await go(page, "/entities?view=list");
    ...
    await page.getByRole("button", { name: "Add Holding" }).click();
    const dialog = page.getByRole("dialog", { name: "Add Holding" });
    await dialog.getByRole("combobox", { name: "Entity" }).fill(`DOC-029 entities C32 ${who}`);
    const options = await dialog.getByRole("listbox", { name: "Entity matches" }).getByRole("option").allInnerTexts();
    await page.keyboard.press("Escape");
    await page.keyboard.press("Escape");
    expect(hidden === 0 && !options.includes(main.legalName), `hidden=${hidden} options=${q(options)}`);
  ...
  await step(R, "Archive steps 2-3: List, Show archived, open the Entity, Restore; facts, relationships, Documents and Obligation remain; editing returns", "Show archived lists it; Restore brings back editing with data intact.", async () => {
    await go(page, "/entities?view=list");
    ...
    expect(listed === 1 && editable && addOfficer === 1 && formation === "Republic of Aldoria" && ownedList.includes(owned.legalName) && doc > 0 && obligation === 1, q({ listed, editable, addOfficer, formation, ownedList, doc, obligation }));
...
async function c52(roleKey) {
  ...
  const R = roleKey;
  const who = R === "administrator" ? "A" : "L";
  const entity = await fixtureEntity(s, `DOC-029 entities C52 ${who} entity ${TAG}`);
  const annual = `DOC-029 entities C52 ${who} annual return ${TAG}`;
  const oneOff = `DOC-029 entities C52 ${who} one-off licence ${TAG}`;
  const unassigned = `DOC-029 entities C52 ${who} unassigned review ${TAG}`;
  const scrap = `DOC-029 entities C52 ${who} scrap schedule ${TAG}`;
  ...
  await api(s, "POST", `/api/v1/entities/${entity.id}/registrations`, { jurisdiction: jur, registrationNumber: `KS-${TAG}`, status: "active" });
  ...
  await step(R, "Add steps 1-4: recurring Obligation with Registration, Assignee, Matter and Note", "The row shows the due date, recurrence and links.", async () => {
    ...
    const matterOptions = await dialog.getByLabel("Matter", { exact: true }).locator("option").allInnerTexts();
    ...
    expect(row.due === "2025-09-30" && row.repeat === "12" && row.registration.startsWith(jur) && row.assignee === me && row.matter === matterLabel, q(row));
    return `After Add obligation and a reload the row read ${q(row)}.`;
  });

  await step(R, "Add step 2: a blank recurrence makes a one-off", "The one-off row has an empty recurrence.", async () => {
    const dialog = await openAdd();
  ...
  await step(R, "Negative: recurrence outside 1 to 1,200 months and a blank Label are refused", "1201 and 0 keep the dialog open with a message; a blank Label does not submit; nothing is created.", async () => {
    const outcomes = [];
    for (const months of ["1201", "0"]) {
      const dialog = await openAdd();
      await dialog.getByLabel("Label", { exact: true }).fill(`DOC-029 entities C52 ${who} bad recurrence ${months} ${TAG}`);
    ...
    await dialog.getByRole("button", { name: "Cancel" }).click();
    await reload(page);
    const labels = await page.locator("main table tbody tr input[aria-label$=' label']").evaluateAll((els) => els.map((e) => e.value));
    const leaked = labels.filter((l) => /bad recurrence/.test(l));
    expect(outcomes.every((o) => o.open && o.message) && blankOpen && blankMessage && leaked.length === 0, q({ outcomes, blankOpen, blankMessage, labels }));
    return `Outcomes ${q(outcomes)}. A blank Label kept the dialog open (${q(blankMessage)}). After a reload the Entity listed ${q(labels)}.`;
  });

  await step(R, "Add step 5: edit an open row; text saves on focus change; a selection saves immediately", "Note saves on blur; Assignee Unassigned saves at once; both survive reload.", async () => {
    ...
    await settle(page);
    expect(row.note === "Renew before the end of November." && row.assignee === "Unassigned", q(row));
    return `After a reload the one-off had note ${q(row.note)} and assignee ${row.assignee}; the assignee was then set back to ${me}.`;
  });

  await step(R, "Calendar steps 1-2: Calendar, Due-date list puts overdue open Obligations first; Entity and Assignee filters with Apply", "Overdue rows lead; the filter narrows to this Entity and assignee.", async () => {
    await go(page, "/entities");
    const dueTexts = await page.locator("main table tbody tr td:first-child").allInnerTexts();
    const overdueFlags = await page.locator("main table tbody tr td:first-child").evaluateAll((tds) => tds.map((td) => /status-severe/.test(td.className)));
    const firstNotOverdue = overdueFlags.indexOf(false);
    const orderOk = firstNotOverdue === -1 || overdueFlags.slice(firstNotOverdue).every((f) => !f);
    const ourRow = page.locator("main table tbody tr", { hasText: annual });
    const ourOverdue = await ourRow.locator("td").first().evaluate((td) => /status-severe/.test(td.className)).catch(() => null);
    await page.getByRole("combobox", { name: "Entity", exact: true }).selectOption({ label: entity.legalName });
    await page.getByRole("combobox", { name: "Assignee", exact: true }).selectOption({ label: me });
    await page.getByRole("button", { name: "Apply" }).click();
    await settle(page);
    const rows = await page.locator("main table tbody tr").allInnerTexts();
    expect(orderOk && overdueFlags.some(Boolean) && ourOverdue === true && rows.length === 2 && rows.every((r) => r.includes(entity.legalName)) && /2025/.test(rows[0]), q({ orderOk, ourOverdue, rows }));
  ...
  await step(R, "Calendar step 3: Month, Previous month, Next month, Today, and Due-date list keep the filters", "The Entity and Assignee filter stays in the address across each change.", async () => {
    const trail = [];
    const keep = () => {
  ...
  await step(R, "Calendar step 2: From/To narrow by date; Include completed exists", "From 2026-11-01 to 2026-11-30 leaves only the one-off.", async () => {
  ...
  await step(R, "Calendar step 4: no results shows Clear all; opening an Obligation goes to its Entity", "An empty window shows 'No obligations match' and Clear all; the row link opens the Entity's Obligations tab.", async () => {
    ...
    await page.getByRole("button", { name: "Clear all" }).or(page.getByRole("link", { name: "Clear all" })).first().click();
    await settle(page);
    ...
    await settle(page);
    const landed = new URL(page.url()).pathname;
    expect(empty && !/from=|entity=/.test(cleared) && landed === `/entities/${entity.id}/obligations`, q({ empty, cleared, landed }));
  ...
  await step(R, "Home: open Obligations assigned to you, subject to access; reading does not file or advance", "Home's Entity obligations section lists the overdue item; afterwards it is still due 2025-09-30 and still offers Mark filed.", async () => {
    await go(page, "/");
  ...
  await step(R, "Home: Administrators also see unassigned Obligations on Entities they can reach; others do not", R === "administrator" ? "An unassigned overdue Obligation appears on the Administrator's Home." : "An unassigned Obligation does not appear on the Legal Team Member's Home.", async () => {
    const dialog = await openAdd();
    ...
    await dialog.waitFor({ state: "hidden" });
    await go(page, "/");
    const items = await page.getByRole("region", { name: "Entity obligations" }).getByRole("listitem").allInnerTexts().catch(() => []);
  ...
  await step(R, "Mark filed steps 1-3: Mark filed dialog explanation and Filed on default; filing 2026-10-05 advances the annual to 2027-09-30; History records it", "Explanation names 12 months; Filed on starts at today's local date; the new due date is 2027-09-30; History shows the filing.", async () => {
    ...
    await settle(page);
    const history = (await page.getByText(new RegExp(`filed.*${annual}|${annual}.*filed`)).allInnerTexts()).map((t) => t.replace(/\s+/g, " ").trim());
    await page.keyboard.press("Escape");
    expect(/12 months/.test(explanation) && defaultDate === browserToday && row.due === "2027-09-30" && history.length > 0, q({ explanation, defaultDate, browserToday, row, history }));
  ...
  await step(R, "One-off filing: keeps its due date, shows Filed, leaves open Obligations, cannot be filed again, row read-only; Include completed finds it", "Filed one-off stays 2026-11-20 with Filed; no Mark filed; controls disabled; calendar shows it only with Include completed.", async () => {
    ...
    await reload(page);
    const row = await obligationRow(page, oneOff);
    const filedPill = await page.locator("main table tbody tr", { has: page.getByLabel(`${oneOff} label`) }).getByText(/^Filed /).innerText();
    ...
    await page.getByRole("button", { name: "Apply" }).click();
    await settle(page);
    const completedRow = await page.locator("main table tbody tr", { hasText: oneOff }).allInnerTexts();
    expect(row.due === "2026-11-20" && markFiled === 0 && disabled && open === 0 && completedRow.length === 1 && /Filed/.test(completedRow[0]), q({ row, filedPill, markFiled, disabled, open, completedRow }));
  ...
  await step(R, "Recovery: correct an open recurring row's date; Delete removes the Obligation without filing", "The due date correction saves on blur; Delete removes the scrap Obligation.", async () => {
    await go(page, `/entities/${entity.id}/obligations`);
    const dialog = await openAdd();
    ...
    expect(corrected === "2027-07-31" && gone && annualKept === "2027-09-30", q({ corrected, gone, annualKept }));
    return `The scrap schedule's due date saved as ${corrected}. Delete ${scrap} removed it; ${annual} still read ${annualKept}.`;
  });

  await step(R, "Negative: an invalid due date correction is not saved", "Clearing the date box restores the saved date; nothing changes.", async () => {
  ...
  await step(R, "Archiving the Entity removes its Obligations from the calendar and Home until restored", "After Archive the annual Obligation is absent from Home and the calendar; after Restore it returns.", async () => {
    // Make the annual overdue again so it leads Home.
    await page.getByLabel(`${annual} due date`).fill("2025-09-30");
    await blurTo(page, "Obligations");
    await go(page, "/");
    const before = (await page.getByRole("region", { name: "Entity obligations" }).getByRole("listitem").allInnerTexts()).some((i) => i.includes(annual));
    await go(page, `/entities/${entity.id}`);
    await page.getByRole("button", { name: "Archive", exact: true }).click();
    await settle(page);
    await go(page, "/");
    const homeAfter = (await page.getByRole("region", { name: "Entity obligations" }).getByRole("listitem").allInnerTexts().catch(() => [])).some((i) => i.includes(annual));
    await go(page, "/entities");
    const calendarAfter = await page.locator("main table tbody tr", { hasText: annual }).count();
    const entityOption = await page.getByRole("combobox", { name: "Entity", exact: true }).locator("option", { hasText: entity.legalName }).count();
    await go(page, `/entities/${entity.id}`);
    await page.getByRole("button", { name: "Restore", exact: true }).click();
    await settle(page);
    await go(page, "/entities");
    const calendarRestored = await page.locator("main table tbody tr", { hasText: annual }).count();
    expect(before && !homeAfter && calendarAfter === 0 && entityOption === 0 && calendarRestored === 1, q({ before, homeAfter, calendarAfter, entityOption, calendarRestored }));
  ...
  await step(R, "Negative: a change on an archived Entity is refused (unauthorized/archived outcome)", "The row controls are disabled while archived and a direct write is refused with a message.", async () => {
    await go(page, `/entities/${entity.id}`);
    await page.getByRole("button", { name: "Archive", exact: true }).click();
    await settle(page);
    await go(page, `/entities/${entity.id}/obligations`);
    ...
    const write = await api(s, "POST", `/api/v1/entities/${entity.id}/obligations/${id}/file`, { filedOn: "2026-10-06" });
    await go(page, `/entities/${entity.id}`);
    await page.getByRole("button", { name: "Restore", exact: true }).click();
    await settle(page);
    expect(disabled && markFiled === 0 && write.status >= 400, q({ disabled, markFiled, status: write.status }));
...
async function obligationRow(page, label) {
  ...
  const selected = async (field) => page.getByLabel(`${label} ${field}`).locator("option:checked").innerText();
  return {
...
async function c52BusinessUsers() {
  ...
  const entity = await fixtureEntity(legal, `DOC-029 entities C52 business refusal ${TAG}`);
  const s = await magicSession("business_user_magic");
  await step("business_user (Jonas Weber, magic link)", "Negative: an unauthorized change has a clear outcome", "A Business User's Obligation write is refused.", async () => {
    const r = await api(s, "POST", `/api/v1/entities/${entity.id}/obligations`, { label: "DOC-029 entities C52 business refusal", nextDueOn: "2027-01-31" });
...
async function c53(roleKey) {
  ...
  const prefix = `DOC-029 entities C53 ${who}`;
  ...
  const hold = async (owned, owner, pct) => {
    const r = await api(s, "POST", `/api/v1/entities/${owned.id}/holdings`, { direction: "owner", relatedEntityId: owner.id, ownershipPercent: pct });
  ...
  const c2 = await fixtureContract(s, `${prefix} confidential contract ${TAG}`, { isConfidential: true });
  ...
  // ---------------- share capital ----------------
  await step(R, "Share capital steps 1-2, 4: Authorized shares saves on focus change, Issued shares on Enter; both survive reload", "10000 and 7500 persist.", async () => {
  ...
  await step(R, "Share capital step 4: Escape abandons an unsaved edit", "Typing 999 then Escape restores 7500 and nothing is saved.", async () => {
  ...
  await step(R, "Negative: negative and fractional share counts are refused without replacing the saved value", "-5 and 12.5 show 'Enter a whole number of zero or more.'; 10000 stays.", async () => {
    const seen = [];
    for (const bad of ["-5", "12.5"]) {
      ...
      seen.push({ bad, typed, message: await page.getByText("Enter a whole number of zero or more.").isVisible() });
      await reload(page);
    }
    const stored = plain(await body.getByLabel("Authorized shares", { exact: true }).inputValue());
    expect(seen.every((x) => x.message && x.typed === x.bad) && stored === "10000", q({ seen, stored }));
    return `Attempts ${q(seen)}. After each reload Authorized shares read ${stored}.`;
  });

  await step(R, "Share capital step 2: leave a value blank when unknown", "Clearing Issued shares saves a blank value.", async () => {
  ...
  await step(R, "Share capital step 3 and note: Par value unavailable until Currency; amount with the currency's decimals; too many decimals refused; changing Currency keeps the amount", "Par value disabled with no Currency; GBP 1.25 saves; 1.255 refused; EUR keeps 1.25.", async () => {
    ...
    const currencyNow = currencyTag === "SELECT" ? await body.getByLabel("Currency", { exact: true }).locator("option:checked").innerText() : await body.getByLabel("Currency", { exact: true }).innerText();
  ...
  const readChart = async (p = page) =>
    p.evaluate(() => {
      ...
      const nodes = [...region.querySelectorAll("svg g[data-restricted], svg g")].filter((g) => g.querySelector(":scope > a > rect, :scope > rect"));
  ...
  await step(R, "Chart steps 1-2: Chart draws the Holdings; primary owner solid, others dashed; highest percentage is primary even below 50%; an Entity with no Holdings in a separate row", "P→S 75% solid, M→S 25% dashed, P→S2 40% solid, M→S2 35% dashed; lone Entity unconnected on the last row.", async () => {
    ...
    expect(e1?.kind === "primary" && !e1.dashed && e1.percent === "75%" && e2?.kind === "secondary" && e2.dashed && e2.percent === "25%" && e3?.kind === "primary" && e3.percent === "40%" && e4?.kind === "secondary" && e4.percent === "35%" && lone?.unconnected && lone.y > maxOther, q({ e1, e2, e3, e4, lone, maxOther }));
  ...
  await step(R, "Chart step 3: drag pans, wheel zooms; with the chart focused arrows pan, plus zooms, zero fits; Fit to window resets", "Each input changes the view; zero and Fit to window return to the fitted view.", async () => {
    ...
    expect(!same(start, dragged) && wheeled.zoom !== dragged.zoom && +arrowed.pan[0] === +fitted.pan[0] + 40 && +zoomed.zoom > +arrowed.zoom && same(zeroed, fitted) && same(refitted, fitted), q({ start, dragged: dragged.pan, wheeled: wheeled.zoom, fitted: [fitted.pan, fitted.zoom], arrowed: arrowed.pan, zoomed: zoomed.zoom, zeroed: [zeroed.pan, zeroed.zoom], refitted: [refitted.pan, refitted.zoom] }));
  ...
  await step(R, "Chart step 4: single click or Space highlights the ownership chain; Clear highlight or Escape removes it; double-click or Enter opens", "Click highlights S's chain; Clear highlight clears; Space on a node highlights and Escape clears; double-click and Enter open the Entity.", async () => {
    const node = (name) => chart.getByRole("link", { name: `Open ${name}` });
    await node(S.legalName).click();
    await wait(300);
    const clicked = await readChart();
    const chain = clicked.nodes.filter((n) => n.highlighted === "true").map((n) => n.name).sort();
    ...
    expect(q(chain) === q(expectedChain) && clearVisible && /view=chart/.test(urlAfterClick) && cleared && spaced.includes(M.legalName) && escaped && dbl === `/entities/${P.id}` && entered === `/entities/${U.id}`, q({ chain, clearVisible, urlAfterClick, cleared, spaced, escaped, dbl, entered }));
  ...
  // ---------------- linked work ----------------
  await step(R, "Linked Contracts: the Contracts tab lists Contracts through Our entity; rows and counts reflect the reader's reach", "The comparison reader sees only the open Contract and a count of 1; the Confidential Contract contributes neither row nor count.", async () => {
    await go(page, `/entities/${P.id}/contracts`);
    const actorRows = await page.getByRole("main").getByRole("link").allInnerTexts();
    const actorBadge = await page.getByRole("navigation", { name: "Entity sections" }).getByRole("link", { name: /^Contracts/ }).innerText();
    ...
    const readerBadge = (await readerTab.getAttribute("aria-label")) ?? (await readerTab.locator("[role=img]").getAttribute("aria-label").catch(() => null)) ?? (await readerTab.innerText());
    ...
    expect(joinedReader.includes(c1.title) && !joinedReader.includes(c2.title) && readerImg === "1 linked Contract" && c2direct.status >= 400, q({ actorRows, actorBadge, readerRows, readerImg, c2: c2direct.status }));
  ...
  await step(R, "Linked Matters tab: shows relationships only through saved Entity-valued Fields", "The Matters tab renders its linked-record list (no Entity-valued Matter Field exists in this lab).", async () => {
  ...
  await step(otherKey, `Before Grants: ${otherKey === "administrator" ? "an Administrator can manage access while the Entity is not Confidential" : "a Legal Team Member without a Grant sees the switch unavailable and no Manage access"}`, otherKey === "administrator" ? "Manage access is offered on the open Entity added by the Legal Team Member." : "Switch disabled; no Manage access button on the open Entity added by the Administrator.", async () => {
  ...
  await step(R, "Grant steps 1-3: Manage access, Confidential access, Person, Grant access; the list includes the person and you; only live Legal Team Members and Administrators are offered", "Priya Raman appears in the list with the actor; Business Users and archived users are not offered.", async () => {
    ...
    if (R === "legal_team_member") await shot(page, "c53-confidential-access-dialog.png", log.shots, "V-C53: Confidential access after Nadia Haddad grants Priya Raman");
    const offeredBusiness = options.filter((n) => ["Ravi Menon", "Jonas Weber", "Amara Nwosu", "Ade Balogun", "Gabriel Santos"].includes(n));
    expect(listed.includes("Priya Raman") && listed.includes(s.person.name) && offeredBusiness.length === 0, q({ listed, offeredBusiness }));
    await page.keyboard.press("Escape");
  ...
  await step(R, "Grant step 4: turn on Confidential — restrict to the access list; check the saved result", "The switch is on after reload.", async () => {
    ...
    const banner = await page.getByText("Confidential entity — only people with an access grant can access this entity.").isVisible().catch(() => false);
  ...
  await step("legal_team_member (Priya Raman, comparison reader with a Grant)", `V-C53 ${who}: a Grant gives the named person access`, "Priya opens the Confidential Entity.", async () => {
  ...
  await step(otherKey, `V-C53 ${who}: without a Grant, ${otherKey === "administrator" ? "an Administrator" : "a Legal Team Member"} cannot reach the Confidential Entity; Holding shows Restricted Entity; chart shows Confidential Entity; no self-grant`, "Direct open fails; parent's Owned Entities shows Restricted Entity without a link; chart node reads Confidential Entity with no link; self-grant refused.", async () => {
    const op = o.page;
    ...
    if (R === "legal_team_member") {
      ...
      await wait(400);
      await shot(op, "c53-admin-without-grant-chart.png", log.shots, `V-C53: ${o.person.name} (Administrator, no Grant) searches the chart for ${P.legalName}; the Confidential sub-Entity is a Confidential Entity box`);
    }
    ...
    const self = await api(o, "POST", `/api/v1/entities/${S.id}/grants`, { userId: (await api(o, "GET", "/api/v1/me")).json?.user?.id ?? (await api(o, "GET", "/api/v1/me")).json?.id });
    expect(!opens && direct.status === 404 && /Restricted Entity/.test(owned) && secretLinks === 0 && restrictedNodes.length >= 1 && restrictedNodes.every((n) => n.name === "Confidential Entity" && !n.href) && !named && listed === 0 && self.status >= 400, q({ opens, direct: direct.status, owned, secretLinks, restrictedNodes, named, listed, self: self.status }));
  ...
  await step(otherKey, `V-C53 ${who}: an Officer user link and a linked Contract do not grant access`, "After being linked as an Officer, the user still cannot open the Entity; a reachable Contract signed by it shows Restricted Entity.", async () => {
    await go(page, `/entities/${S.id}`);
    const card = page.getByRole("region", { name: "Directors & Officers" });
    await card.getByRole("button", { name: "Add director or officer" }).click();
    ...
    const ourText = (await our.count()) ? await our.locator("option:checked").innerText() : (await o.page.getByText("Restricted Entity").first().innerText().catch(() => ""));
    ...
    expect(direct.status === 404 && (/Restricted Entity/.test(ourText) || pageHasRestricted > 0) && leaks === 0, q({ direct: direct.status, ourText, pageHasRestricted, leaks }));
  ...
  await step(R, "Grant step 5: Remove name withdraws access; the relationship is preserved", "Priya leaves the list, cannot open the Entity, and sees Restricted Entity for the Holding; the actor still sees the Holding.", async () => {
    await page.getByRole("button", { name: "Manage access" }).click();
  ...
  await step(R, "Negative: removing the last live person from a Confidential Entity is refused", "Remove self as the only grantee shows the refusal and keeps the Grant.", async () => {
    await go(page, `/entities/${S.id}`);
    await page.getByRole("button", { name: "Manage access" }).click();
    ...
    await page.keyboard.press("Escape");
    expect(/Grant another person access before removing the last person/.test(message) && still === 1, q({ message, still }));
    return `Removing ${s.person.name}, the only person in the list, showed ${q(message)} and the Grant stayed.`;
  });

  await step(R, "Turning the Confidential switch off makes the Entity available to all Legal Team Members and Administrators without Grants", "The other role and Priya can open the Entity again.", async () => {
    await page.getByRole("switch", { name: "Confidential — restrict to the access list" }).click();
    await settle(page);
    await reload(page);
  ...
  if (R === "administrator") {
    await step(R, "Negative: turning on Confidential with no live person in the list is refused", "After the Administrator removes the only Grant on an open Entity, the switch refuses with a message.", async () => {
      ...
      await page.getByRole("switch", { name: "Confidential — restrict to the access list" }).click();
      await settle(page);
      const message = await page.getByText("Grant at least one person access before making this entity confidential.").isVisible();
      await reload(page);
      const on = await page.getByRole("switch", { name: "Confidential — restrict to the access list" }).isChecked();
    ...
    await step(R, "Grant note: Grants let their holder change the flag and Grants; a Legal Team Member who removes their own Grant on an open Entity loses that control", "After removing the own Grant, the switch is unavailable and Manage access is gone.", async () => {
      await go(page, `/entities/${U.id}`);
      await page.getByRole("button", { name: "Manage access" }).click();
      await grantsDialog.getByRole("button", { name: `Remove ${s.person.name}` }).click();
      await grantsDialog.getByText("No grants yet.").waitFor();
