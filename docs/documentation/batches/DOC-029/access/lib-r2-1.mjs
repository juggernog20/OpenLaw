// Shared helpers for the DOC-029 round 2 access independent walkthrough (walkthrough round 1).
...
/** Fixture: invite and activate a "DOC-029 access" staff account for each role not already in fx.accounts. */
...
// ---------- Sign-in branding (round 2: AuthLayout shows the organization name and logo) ----------
/** Reads the public branding answer and checks that the name shows above the given heading text on the current page. */
export async function brandingAbove(page, headingText) {
  const api = await fetch(`${BASE}/api/v1/org/branding`).then((r) => r.json()).catch(() => null);
  ...
  const headLoc = page.getByRole("heading", { name: headingText, exact: true }).first();
  const headBox = await headLoc.boundingBox().catch(() => null);
  ...
  const above = !!(nameBox && headBox && nameBox.y + nameBox.height <= headBox.y && (!formBox || nameBox.y + nameBox.height <= formBox.y));
  return { apiName: name, apiLogo: api?.logo ? "saved" : "none", nameShown: !!nameBox, above, logoCount };
}

/** Browser-only rendering with a mocked branding answer: a saved logo, and a saved logo that fails to load. */
export async function brandingMockRender(path, headingText) {
  const png = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg==";
  const run = async (answer) => {
    const c = await newContext();
    await c.page.route("**/api/v1/org/branding", (route) => route.fulfill({ json: answer }));
    await c.page.goto(`${BASE}${path}`);
    await c.page.getByRole("heading", { name: headingText, exact: true }).first().waitFor({ timeout: 15000 });
    await sleep(1200);
    const img = c.page.getByRole("img", { name: "Organization logo" });
    const logo = (await img.count()) > 0 ? await img.first().isVisible() : false;
    const imgBox = logo ? await img.first().boundingBox() : null;
    const headBox = await c.page.getByRole("heading", { name: headingText, exact: true }).first().boundingBox();
    const body = await text(c.page.locator("main"));
    await c.context.close();
    return { logo, logoAbove: !!(imgBox && headBox && imgBox.y + imgBox.height <= headBox.y), body };
  };
  const saved = await run({ name: "DOC-029r2 Fictional Org", logo: `data:image/png;base64,${png}` });
  const broken = await run({ name: "DOC-029r2 Fictional Org", logo: "data:image/png;base64,AAAA" });
  const noName = await run({ name: "", logo: null });
  return {
    savedLogoShownAbove: saved.logo && saved.logoAbove && saved.body.startsWith("DOC-029r2 Fictional Org"),
    brokenLogoHidden: !broken.logo && broken.body.startsWith("DOC-029r2 Fictional Org"),
    noNameShowsOpenLaw: noName.body.startsWith("OpenLaw") && !noName.body.includes("Powered by OpenLaw"),
  };
}
