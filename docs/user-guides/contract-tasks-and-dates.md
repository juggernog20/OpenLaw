...
2. Enter **Title** and, if useful, **Description**. Choose **Assignee** or **Unassigned**, and set **Due date** if needed. Under **Comments & attachments**, you can add an initial note and files. The initial note uses the **Working team** audience.
3. Select **Add task** and check the new row. **Cancel** before saving creates neither the Task nor a staged team addition.
4. Select the Task's checkbox to complete it. A completed Task leaves the list until you turn on **Show completed**. Select the checkbox again to reopen it. Check the done count and saved row.
...
The current Contract Task screen has no reordering control. Complete, edit, assign, or remove Tasks using the offered controls rather than assuming that dragging changes their saved order.
...
4. Leave the recipient checkboxes clear to use the usual audience: the Legal Owner and the Contract team. The checkboxes list active team members and never Business Users. Select people to send this Key date's reminders only to those people. Their current access and notification preferences still apply; selecting someone grants no access.
...
Each Key date gets its own reminder, including when several on one Contract share a date. The reminder names its event. A lead time present in both the global and additional lists fires only once for that Key date. Selected recipients who leave the team or become inactive are excluded; an empty eligible selection does not fall back to everyone. If a selected recipient has left the team, the dialog says so and offers **Use the usual audience**. Otherwise, clear all recipient checkboxes to return to the default recipients.
