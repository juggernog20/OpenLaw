...
// The "DOC-029r2 access" fresh accounts change their own preferences; the other fresh account is the unchanged control.
// Jonas (Business User) checks that the View Business Portal control is absent.
import { deflateSync } from "node:zlib";
import { BASE, SEED, api, ensureStaffAccounts, newContext, passwordSignIn, sleep, sql, text, waitForLink } from "./lib-r2.mjs";
...
const saved = (scope) => scope.getByText("Saved", { exact: true }).first().waitFor({ timeout: 10000 }).then(() => true, () => false);
...
export async function run(log, fx) {
  ...
  const prefs = (id) => sql(`select display_name||'|'||coalesce(timezone,'')||'|'||theme||'|'||(image is not null) from users where id='${id}'`);
  ...
  for (const role of ["administrator", "legal_team_member"]) {
    ...
    const emailReadOnly = (await emailInput.count()) === 0 || (await emailInput.isDisabled().catch(() => false)) || (await emailInput.getAttribute("readonly").catch(() => null)) !== null;
    const bodyText = await text(p.locator("main"));
    const emailValue = (await emailInput.count()) ? await emailInput.inputValue().catch(() => "") : "";
    const roleNote = bodyText.includes("Roles are managed in Organization → Users.") && (emailValue === acct.email || bodyText.includes(acct.email));
    R(
      ...
      personalGroup && nameSaved && nameAfter === edited && headerName && tzSaved && /Tokyo/.test(tzAfter) && tzCleared && dbTz === "null" && emailReadOnly && roleNote,
    );
    ...
    await chooser.setFiles({ name: "doc029r2-access-avatar.png", mimeType: "image/png", buffer: small });
    const photoSaved = await saved(p);
    await sleep(1000);
    const imageStored = sql(`select image is not null from users where id='${acct.userId}'`) === "t";
    const big = png(700, 700, true);
    await fileInput.setInputFiles({ name: "doc029r2-access-too-large.png", mimeType: "image/png", buffer: big });
    const bigError = await p.getByText("The change could not be saved. Try again.").first().waitFor({ timeout: 8000 }).then(() => true, () => false);
    R(
    ...
    const htmlTheme = await p.evaluate(() => document.documentElement.getAttribute("data-theme") || document.documentElement.className);
    ...
    R(
      ...
      themes.Warm === "true/warm" && themes.Dark === "true/dark" && themes.Light === "true/light" && darkKept && stillSignedIn && otherBefore === otherAfter,
    );
    ...
    await sleep(2500);
    const banner = await p.getByText("Viewing as business user").first().isVisible().catch(() => false);
    const bannerText = await p.getByText("You’re viewing your Portal work. Submissions, edits, and replies are real.").isVisible().catch(() => false);
    ...
    await p.getByRole("link", { name: "Profile", exact: true }).click();
    await p.waitForURL(/\/settings\/profile/);
    const card = await p.getByText("Password & two-factor").waitFor({ timeout: 15000 }).then(() => true, () => false);
    ...
    const wrongMsg = await d.getByText("The password could not be changed. Check your current password and use at least 8 characters.").waitFor({ timeout: 10000 }).then(() => true, () => false);
    await d.getByLabel("Current password").fill(acct.password);
    await d.getByLabel("New password").fill("short1");
    await d.getByRole("button", { name: "Save" }).click();
    await sleep(2000);
    const shortMsg = await d.getByText("The password could not be changed. Check your current password and use at least 8 characters.").isVisible().catch(() => false);
    await d.getByLabel("New password").fill(newPw);
    await d.getByRole("button", { name: "Save" }).click();
    ...
    const oldRefused = await (async () => {
      ...
      const r = await t.page.getByText("Check your email and password.").waitFor({ timeout: 10000 }).then(() => true, () => false);
  ...
  {
    const email = `doc029r2.access.nopassword.${stamp}@helix.example`;
    const displayName = `DOC-029r2 access No Password Member ${stamp % 100000}`;
    const inv = await api(admin.page, "POST", "/auth/invites", { email, displayName, role: "legal_team_member" });
    ...
    const inApp = await c.page.waitForURL((u) => !u.pathname.startsWith("/auth"), { timeout: 20000 }).then(() => true, () => false);
    ...
    log.record(A, "legal_team_member", "Password & two-factor is absent for an account without a password", "Profile shows Password & two-factor only when the account has a password", `Invite ${inv.status}; email-link sign-in reached the app ${inApp}; Profile without a Password & two-factor card ${absent}`, inv.status === 201 && inApp && absent);
    await c.context.close();
  }
  ...
  // Business Users do not have the View Business Portal control.
  {
    ...
    for (let i = 0; i < 4 && !ok; i++) {
      ...
      const since = Date.now();
      await c.page.getByRole("button", { name: "Send link" }).click();
      const m = await waitForLink(SEED.jonas.email, since, { subjectRe: /Sign in/, linkRe: /magic-link/ });
      if (!m?.link) continue;
      await c.page.goto(m.link);
      ok = await c.page.waitForURL((u) => u.pathname.startsWith("/portal") && !u.pathname.startsWith("/portal/login"), { timeout: 20000 }).then(() => true, () => false);
    }
    const res = {};
    ...
    const noControl = (await c.page.getByText("View as business user").count()) === 0 && (await c.page.getByText("Viewing as business user").count()) === 0;
    log.record(A, "business_user", "Business Users do not have the View Business Portal control", "A Business User cannot open the staff Settings view control", `Signed in ${ok}; ${JSON.stringify(res)}; View as business user control or banner shown ${!noControl}`, ok && Object.values(res).every((x) => x.startsWith("/portal")) && noControl);
    await c.context.close();
  }
