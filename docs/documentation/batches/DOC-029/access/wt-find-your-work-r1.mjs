...
// Seeded Daniel and Nadia read Home as it is; the "DOC-029 access" fresh accounts carry the reviewer's Task fixtures.
import {
  BASE,
  SEED,
  api,
...
export async function run(log, fx) {
  ...
  const home = must(
    await api(admin.page, "POST", "/contracts", {
      title: `DOC-029 access Home contract ${stamp}`,
      contractTypeId: nda,
  ...
  const conf = must(
    await api(admin.page, "POST", "/contracts", {
      title: `DOC-029 access confidential contract ${stamp}`,
      contractTypeId: nda,
      managerId: uid(SEED.daniel.email),
      isConfidential: true,
    }),
  ...
  fx.created.push(
    `Contract C-${home.number} DOC-029 access Home contract ${stamp}`,
    `Confidential Contract C-${conf.number} DOC-029 access confidential contract ${stamp} (Daniel's team only)`,
  );
  ...
  for (const [role, email] of [
    ...
    const title = `DOC-029 access Home task ${role} ${stamp}`;
    must(
  ...
  for (const [role, seed] of [
    ...
    for (const name of ORDER)
      if (await p.locator("main").getByRole("heading", { level: 2, name, exact: true }).count())
        present.push(name);
    const domOrder = await p
      .locator("main h2")
    ...
    R(
      ...
      ordered &&
        ...
        docsOk &&
        backHome &&
        present.includes("Tasks assigned to you"),
    );
    ...
    const taskInDates =
      datesRows.includes(seededTask[role]) || datesRows.includes(`"number":${home.number}`);
    ...
    R(
      "Task due dates do not feed Dates approaching; View all opens Your dates",
      "The reviewer Task's due date is absent from Dates approaching and Your dates; View all opens Your dates",
      `Task or its Contract in Dates approaching ${taskInDates}; Your dates opened without the Task ${yourDates}`,
    ...
    R(
      "Help in the header opens product instructions and its search searches guides",
      "Help opens from Home; its search finds the guide, not records",
      `Help URL ${new URL(p.url()).pathname}${new URL(p.url()).search.slice(0, 60)}; result summary "${(helpText.match(/\d+ results?/) ?? ["none"])[0]}"; guide found ${helpFound}; reviewer Contract title in Help results ${recordLeak}`,
      helpFound && !recordLeak,
    );
    ...
    R(
      "unavailable record pages",
      "Contract not found or Matter not found says the record does not exist or you cannot open it; Back to Contracts or Back to Matters returns to the list",
  ...
  for (const role of ["administrator", "legal_team_member"]) {
    ...
    const empty = await p
      .getByText("No open Tasks assigned to you.")
      .waitFor({ timeout: 15000 })
      .then(
        () => true,
        () => false,
    ...
    await p.goto(`${BASE}/`);
    await sleep(2500);
    const noCard = !(await p
      .getByRole("region", { name: "Tasks assigned to you" })
      .isVisible()
      .catch(() => false));
    const welcome = await p
      .getByText("Welcome to OpenLaw")
      .isVisible()
      .catch(() => false);
    const freshSections = (must(await api(p, "GET", "/home"), "home fresh").sections ?? []).map(
      (x) => x.type,
    );
    R(
      "empty Tasks list and absent Home card",
      "No open Tasks assigned to you. on Your Tasks; no Tasks assigned to you card on Home",
      `Empty message ${empty}; Home Tasks card absent ${noCard}; Home sections for this account: ${freshSections.join(", ") || "none"}; Welcome to OpenLaw shown ${welcome}`,
      empty && noCard && (freshSections.length > 0 || welcome),
    );

    const titles = [];
    for (let i = 0; i < 55; i++) {
      const dated = i < 53;
      const title = `DOC-029 access Task ${String(i + 1).padStart(2, "0")} ${dated ? "dated" : "undated"} ${role} ${stamp}`;
      must(
        await api(admin.page, "POST", `/contracts/${home.number}/tasks`, {
          title,
          assigneeId: acct.userId,
          addToTeam: true,
          dueDate: dated ? iso(5 + (53 - i)) : null,
        }),
        "fresh task",
      );
    ...
    const toggle = p
      ...
      .or(p.getByRole("button", { name: "Show completed" }));
    ...
    if (role === "administrator")
      await p.screenshot({
        path: new URL("./r1-administrator-your-tasks-show-completed.png", import.meta.url).pathname,
      });
    await p.getByRole("checkbox", { name: `Reopen Task: ${t1}` }).click();
    const reopened = await p
      .getByText(`Reopened: ${t1}`)
      .waitFor({ timeout: 10000 })
      .then(
        () => true,
        () => false,
    ...
    const hideToggle = p
      ...
      .or(p.getByRole("button", { name: "Hide completed" }));
    ...
    R(
      "Your Tasks: Task title opens its owning record; Home in the app navigation returns",
      "Task title opens the Contract; Home returns",
