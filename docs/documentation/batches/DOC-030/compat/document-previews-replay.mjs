// DOC-030 compatibility replay (set compat), article document-previews (V-C28). Copy of DOC-029/compat-r2/documents-r2/walkthrough-r2.mjs.
// DOC-030 changes: helpers from ./document-previews-replay/lib.mjs, default SECTIONS=previews,portal (the V-C28 sections),
// lab name from LAB_NAME (default work2), reviewer label, task, app commit, output path, record names DOC-029r2 to "DOC-030 compat".
...
import {
  ...
  root,
  sleep,
} from "./document-previews-replay/lib.mjs";

const HERE = path.dirname(new URL(import.meta.url).pathname);
const OUT = process.env.OUT ?? path.join(HERE, "document-previews-replay.json");
const SECTIONS = (process.env.SECTIONS ?? "previews,portal").split(",");
...
function labStatus() {
  try {
    ...
    const project = text.match(/(openlaw-docs-[a-z0-9]+-[a-z0-9-]+?)-app-1/)?.[1] ?? null;
...
async function versions(role, person) {
  ...
  const contract = await createContract(
    page,
    role,
    `DOC-030 compat documents ${label} versions Contract ${stamp}`,
  );
  await addTeam(page, contract.number, PEOPLE.jonas.email);
  const matter = await createMatter(
    page,
    role,
    `DOC-030 compat documents ${label} versions Matter ${stamp}`,
  );
  ...
  const newName = `DOC-030 compat ${label} supply draft ${stamp}`;
  await step(
    A,
    S,
    role,
  ...
  await step(
    ...
    async () => {
      ...
      const ent = await api(page, "POST", "/entities", {
        legalName: `DOC-030 compat documents ${label} versions Entity ${stamp} Ltd`,
  ...
  await step(
    ...
    async () => {
      const frozen = await createContract(
        page,
        role,
        `DOC-030 compat documents ${label} archived owner ${stamp}`,
      );
  ...
  const legalOnly = await api(page, "POST", "/comments", undefined, {
    ...
    body: `DOC-030 compat ${label} Legal only attachment`,
  ...
  const teamNote = await api(page, "POST", "/comments", undefined, {
    entityType: "contract",
    entityId: contractRow.id,
    body: `DOC-030 compat ${label} team attachment`,
  ...
  let filedTitle = `DOC-030 compat ${label} filed paper ${stamp}`;
  await step(
    A,
    S,
    role,
  ...
  await step(
    ...
    async () => {
      ...
      const c = await api(page, "POST", "/comments", undefined, {
        entityType: "matter",
        entityId: m.id,
        body: `DOC-030 compat ${label} matter attachment`,
      ...
      await dialog
        ...
        .fill(`DOC-030 compat ${label} matter filed ${stamp}`);
      await dialog.getByRole("button", { name: "File", exact: true }).click();
      await dialog.waitFor({ state: "hidden", timeout: 20000 });
      const docs = await recordDocs(page, `/matters/${matter.number}`);
      expect(
        kinds === 0 &&
          docs.some((d) => d.title === `DOC-030 compat ${label} matter filed ${stamp}`),
        `kinds ${kinds}`,
      );
      expect(
...
async function folders(role, person) {
  ...
  const contract = await createContract(
    page,
    role,
    `DOC-030 compat documents ${label} folders Contract ${stamp}`,
  );
  const recUrl = `/contracts/${contract.number}`;
  const parent = `DOC-030 compat ${label} Parent`;
  const child = `DOC-030 compat ${label} Child`;
  const other = `DOC-030 compat ${label} Other`;
  ...
  await step(
    ...
    async () => {
      const matter = await createMatter(
        page,
        role,
        `DOC-030 compat documents ${label} folders Matter ${stamp}`,
      );
      await openDocumentsTab(page, `/matters/${matter.number}/documents`);
      await docsSection(page).getByRole("button", { name: "Upload", exact: true }).click();
      await chooseFiles(
...
async function previews(role, person) {
  ...
  const contract = await createContract(
    page,
    role,
    `DOC-030 compat documents ${label} previews Contract ${stamp}`,
  );
  const recUrl = `/contracts/${contract.number}`;
...
async function portal() {
  ...
  const contract = await createContract(
    staff,
    "legal_team_member",
    `DOC-030 compat documents Portal Contract ${stamp}`,
  );
  const recUrl = `/contracts/${contract.number}`;
  ...
  for (const [name, as] of [
    ...
    const thread = await api(staff, "POST", "/comments", undefined, {
      entityType: "contract",
      entityId: contractId,
      body: `DOC-030 compat Portal thread file ${as}`,
      visibility: "full_thread",
      file: part(name, as),
    });
    expect(
  ...
  const hidden = await createContract(
    staff,
    "legal_team_member",
    `DOC-030 compat documents Portal not on team ${stamp}`,
  );
  ...
  const item = await api(staff, "POST", "/knowledge", {
    title: `DOC-030 compat documents Portal Knowledge ${stamp}`,
  ...
  record(
    "legal_team_member",
    "knowledge_item",
    `DOC-030 compat documents Portal Knowledge ${stamp}`,
    kid,
  );
  ...
  const req = await api(page, "POST", "/requests", {
    ...
    title: `DOC-030 compat documents Portal request ${stamp}`,
  ...
  record(role, "request", `DOC-030 compat documents Portal request ${stamp}`, `R-${reqNumber}`);
  ...
  await step(
    ...
    async () => {
      ...
      const attempts = {
        ...
        folder: (
          await api(page, "POST", `${recUrl}/folders`, { name: "DOC-030 compat portal folder" })
        ).status,
  ...
  await step(
    ...
    async () => {
      ...
      const staffFolder = await api(staff, "POST", `${recUrl}/folders`, {
        name: `DOC-030 compat portal target ${stamp}`,
      });
      const targetId = (await folderList(staff, recUrl)).find(
        (f) => f.name === `DOC-030 compat portal target ${stamp}`,
      )?.id;
      ...
      const create = await api(page, "POST", `${recUrl}/folders`, {
        name: "DOC-030 compat portal attempt",
      });
...
async function hiddenPaper() {
  ...
  const { context, page } = await staffContext(PEOPLE.daniel);
  const title = `DOC-030 compat documents hidden owner ${stamp}`;
...
async function repository(role, person) {
  ...
  const contract = await createContract(
    page,
    role,
    `DOC-030 compat documents ${label} repository Contract ${stamp}`,
  );
  const recUrl = `/contracts/${contract.number}`;
  const cpName = `DOC-030 compat ${label} Northwind ${stamp}`;
  ...
  const matter = await createMatter(
    page,
    role,
    `DOC-030 compat documents ${label} repository Matter ${stamp}`,
  );
  ...
  const types = (await api(page, "GET", "/entities/types")).body.entityTypes;
  const ent = await api(page, "POST", "/entities", {
    legalName: `DOC-030 compat documents ${label} repository Entity ${stamp} Ltd`,
    entityTypeId: types[0].id,
  });
  expect(ent.status === 201, `fixture entity ${ent.status}`);
  record(role, "entity", ent.body.entity.legalName, ent.body.entity.id);
  ...
  const k = await api(page, "POST", "/knowledge", {
    title: `DOC-030 compat documents ${label} repository Knowledge ${stamp}`,
    knowledgeTypeId: kType,
  });
  ...
  const ad = await api(page, "POST", "/auto-docs", {
    name: `DOC-030 compat documents ${label} repository Auto-Doc ${stamp}`,
  });
  expect(
    ad.status === 201,
  ...
  record(
    role,
    "auto_doc",
    `DOC-030 compat documents ${label} repository Auto-Doc ${stamp}`,
    autoDocId,
  );
  ...
  await step(
    ...
    async () => {
      ...
      await applyChoice(page, "Folder", [`DOC-030 compat ${label} Repo folder`]);
      ...
      return `Record with Search choices "C-${contract.number}" narrowed the list to the Contract's ${mine.length} live Documents, and the Filter menu then offered Folder. Folder "DOC-030 compat ${label} Repo folder" showed only "${filedDocx.title}"; Record root showed the Documents outside folders. With Owner Knowledge, and with Owner Auto-Docs, the Filter menu offered no Folder.`;
    },
  );
  ...
  await step(
    ...
    async () => {
      ...
      const target = aside.getByText(`DOC-030 compat ${label} Repo folder`, { exact: true });
      ...
      return `Dragging the "${draggable.title}" row showed the Move to folder panel for C-${contract.number} with None and "DOC-030 compat ${label} Repo folder"; dropping on the folder filed the Document there (read-back).`;
    },
  );
...
async function archive(role, person) {
  ...
  const contract = await createContract(
    page,
    role,
    `DOC-030 compat documents ${label} archive Contract ${stamp}`,
  );
  const recUrl = `/contracts/${contract.number}`;
  const tag = `${label.toLowerCase()}-${stamp}`;
  ...
  await step(
    ...
    async () => {
      const kType = (await api(page, "GET", "/knowledge?limit=1")).body.knowledgeItems?.[0]
        ?.knowledgeTypeId;
      const k = await api(page, "POST", "/knowledge", {
        title: `DOC-030 compat documents ${label} archive Knowledge ${stamp}`,
        knowledgeTypeId: kType,
      });
      expect(k.status === 201, `fixture knowledge ${k.status}`);
