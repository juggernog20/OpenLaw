...
const LAB = JSON.parse(
  readFileSync(path.join(root, ".documentation-labs/firstrun-r2/lab.json"), "utf8"),
);
...
async function phaseA() {
    ...
    async () => {
      ...
      const res = await other.request.post(`${BASE}/api/v1/auth/setup`, {
        data: {
          ...
          displayName: "DOC-029r2 first-run Second",
  ...
  await step(
    ...
    async () => {
      ...
      await expect(
        page.getByText("Identity provider doc029r2-harbor is registered."),
      ).toBeVisible();
      const callback = (
  ...
  await step(
    ...
    async () => {
      ...
      obs.push(
        `after Continue the page was ${afterContinue}${onEnroll ? "" : ` showing "${await stepHeading(page)}" (${await progress(page)})`}`,
      );
      const methods = await getJson(context, "/api/v1/auth/methods");
      const me = await getJson(context, "/api/v1/me");
      if (!onEnroll)
        await page.screenshot({ path: path.join(here, "r2-1-a-two-factor-after-continue.png") });
      obs.push(
      ...
      if (!onEnroll) {
        ...
        await page.waitForTimeout(1500);
        obs.push(
          `Continue on Your organization then showed ${pathOf(page)} "${await stepHeading(page)}"`,
        );
        await page.reload();
        await page.waitForLoadState("networkidle");
  ...
  await step(
    ...
    async () => {
      ...
      must(
        (await page.getByText("DOC-029r2 first-run Unsent").count()) === 0,
  ...
  await step(
    ...
    async () => {
      ...
      await page
        ...
        .fill("DOC-029r2 first-run Harbor Legal");
      await page.getByLabel("Sender email", { exact: true }).fill("legal@harbor.example");
      await button(page, "Save relay").click();
      await expect(
