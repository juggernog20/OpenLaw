...
describe("the scope rule, in both destination modules (INT-002)", () => {
  ...
  it("rejects legacy attachments outside the scope but lets the destination change", async () => {
    ...
    expect((await typeBySlug(type.slug)).targetModule).toBe("matter");
  });
...
describe("the strand refusal on a target change (INT-002)", () => {
  it("changes destination without consulting retired Request attachments", async () => {
    const type = await addType("Repoint probe");
    const fieldId = await createField("Old question", "contract");
    await attach(type.id, { fieldId });
    const result = await setTarget(type.id, { targetModule: "matter" });
...
describe("the DD-017 activity trail", () => {
  ...
  it("records a destination change despite retired attachments", async () => {
    ...
    expect((await setTarget(probe.id, { targetModule: "matter" })).statusCode).toBe(200);
...
it("ignores retired companion-attach keys without attaching to a destination", async () => {
  const type = await addType("Retired companion");
  const fieldId = await createField("Retired companion field", "contract");
