...
export default async function createMatter(ctx) {
  ...
  await step(
    ...
    async () => {
      ...
      await d.getByLabel("Description").fill("DOC-029r2 fictional description.");
      await d
  ...
  await step(
    ...
    async () => {
      ...
      await tasksRegion.getByText("DOC-029r2 alpha evidence review").waitFor();
      const assigneeButton = await tasksRegion
        .getByRole("button", { name: /^Change assignee for DOC-029r2 alpha evidence review/ })
        .getAttribute("aria-label");
      const tasks = (await api(page, "GET", `/matters/${n}/tasks`)).json.tasks;
      const review = tasks.find((x) => x.title === "DOC-029r2 alpha evidence review");
      const discussion = tasks.find((x) => x.title === "DOC-029r2 alpha discussion");
      ...
      await kd.getByText("DOC-029r2 alpha filing").waitFor();
      const dates = (await api(page, "GET", `/matters/${n}/key-dates`)).json.deadlines;
      const kick = dates.find((x) => x.label === "DOC-029r2 alpha kickoff");
      const filing = dates.find((x) => x.label === "DOC-029r2 alpha filing");
      expectThat(
