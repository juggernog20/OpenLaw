...
const ADMIN_LAB = {
  ...
  project: "openlaw-docs-41255c61-admin",
};
const AUTH_LAB = {
...
async function org() {
  ...
  const people = {
    a: {
      name: `DOC-029 admin-org Avery ${stamp}`,
      email: `doc029-org-a-${stamp}@helix.example`,
  ...
  await step(
    ...
    async () => {
      ...
      const inv = await api(n.page, app, "POST", "/api/v1/auth/invites", {
        email: `doc029-org-denied-${stamp}@helix.example`,
        displayName: "DOC-029 admin-org denied",
        role: "legal_team_member",
      });
      await n.ctx.close();
      expect(
  ...
  await step(
    A,
    "administrator",
    "Update organization details: Organization name saves on Enter, Escape restores, leaving the field saves",
    "Saved status; Escape restores the previous name before saving; values survive reload",
    async () => {
      const { page } = admin;
      ...
      expect(
        afterEscape === draft && afterEscapeReload === draft,
  ...
  await step(
    A,
    "administrator",
    "Logo: Upload a supported image; a rejected file shows an error and can be retried",
    "PNG saves; unsupported type and oversize files are refused",
    async () => {
      const { page } = admin;
      ...
      writeFileSync(
        path.join(fx, "doc029-not-an-image.txt"),
        "DOC-029 admin-org fixture: not an image\n",
      );
      writeFileSync(
        path.join(fx, "doc029-logo-too-large.png"),
        Buffer.concat([png, Buffer.alloc(300 * 1024)]),
      );
      await page.goto(`${app}/settings/general`);
      ...
      expect(
        typeof saved === "string" && saved.startsWith("data:image/png"),
        `saved logo ${String(saved).slice(0, 30)}`,
      );
  ...
  await step(
    ...
    async () => {
      ...
      const d = await invite(
        page,
        "DOC-029 admin-org invalid",
        "doc029-org-invalid-address",
        "Legal team member",
      );
      await sleep(800);
      ...
      const apiTry = await api(page, app, "POST", "/api/v1/auth/invites", {
        ...
        displayName: "DOC-029 admin-org invalid",
        role: "legal_team_member",
      });
  ...
  await step(
    ...
    async () => {
      ...
      const created = await api(page, app, "POST", "/api/v1/contracts", {
        title: `DOC-029 admin-org offboarding contract ${stamp}`,
      ...
      results.records = [
        ...(results.records ?? []),
        `Contract ${contractNumber} "DOC-029 admin-org offboarding contract ${stamp}"`,
      ];
      people.a.id = aId;
...
async function groups() {
  ...
  const members = [
    {
      key: "m1",
      name: `DOC-029 admin-org Approver One ${stamp}`,
      email: `doc029-grp-one-${stamp}@helix.example`,
      password: newPassword(),
    },
    {
      key: "m2",
      name: `DOC-029 admin-org Approver Two ${stamp}`,
      email: `doc029-grp-two-${stamp}@helix.example`,
      password: newPassword(),
    },
  ];
  ...
  await step(
    ...
    async () => {
      ...
      const create = await api(n.page, app, "POST", "/api/v1/approver-groups", {
        name: `DOC-029 admin-org denied ${stamp}`,
        memberIds: [],
      });
      await n.ctx.close();
      expect(
  ...
  await step(
    ...
    async () => {
      ...
      await dialog
        ...
        .fill("DOC-029 admin-org walkthrough group. Use for reviewer checks only.");
...
async function emailStored() {
  ...
  await step(
    ...
    "Welcome wizard opens; Skip optional steps on Welcome opens Outbound email while email is unset",
    async () => {
      const { page } = admin;
  ...
  await step(
    A3,
    "administrator",
    "Outbound email: an SMTP relay URL that does not begin with smtp:// or smtps:// is refused",
    "Refused; source stays unset",
    async () => {
      const { page } = admin;
  ...
  await step(
    A3,
    "administrator",
    "Save relay, then Send test email to the signed-in Administrator",
    "Saved; URL not shown again; test message arrives from the stored From address",
    async () => {
      const { page } = admin;
      ...
      expect(
        /Outbound email is set in the app\. Mail is sent from/.test(text) &&
          !text.includes("smtp://mailpit") &&
          !inputs.some((v) => v.includes("smtp://")),
        text.slice(0, 300),
      );
  ...
  await step(
    A3,
    "administrator",
    "Replace relay with a relay that cannot deliver; the test fails; replace again and retest",
    "Saved relay alone does not prove delivery; a corrected relay delivers",
    async () => {
      const { page } = admin;
      ...
      const failure = await page
        .getByText("The test email could not be sent.")
  ...
  await step(
    ...
    async () => {
      ...
      expect(
        source === "unset" && cont && n === 0 && reset.status >= 400,
    ...
    async () => {
      ...
      if (await cont.isDisabled()) {
        const r = await api(page, app, "POST", "/api/v1/auth/invites", {
          email: invitee,
          displayName: "DOC-029 Auth Incomplete Invite",
          role: "legal_team_member",
        });
        await sleep(4000);
...
async function emailRestored() {
  ...
  await step(
    ...
    async () => {
      ...
      const m = await waitMail(mail, AUTH_ADMIN.email, since, null);
      expect(
        /Outbound email is set in the app\. Mail is sent from DOC-029 Stored/.test(text) &&
  ...
  await step(
    A3,
    "administrator",
    "Replace relay with a URL that does not begin with smtp:// or smtps://",
    "The save is refused with a message; the stored relay stays active",
    async () => {
      const { page } = admin;
      await page.getByRole("button", { name: "Replace relay" }).click();
      await page.getByLabel("SMTP relay URL").fill("http://mailpit:1025");
      ...
      const added =
        ...
        after.match(/(The relay could not be saved\.[^]*?\.|[^.]*smtp:\/\/[^.]*\.)/)?.[0];
      ...
      expect(
        settings.data?.source === "app" && !alerts.some((x) => /Relay saved/.test(x)),
        `source ${settings.data?.source}; alerts ${alerts}`,
      );
  ...
  await step(
    ...
    async () => {
      ...
      for (const [email, name] of [
        [authAccounts.legal, "DOC-029 Auth Legal"],
        [authAccounts.ssoStaff, "DOC-029 Auth SSO Staff"],
        [authAccounts.offDomainStaff, "DOC-029 Auth Off-domain Staff"],
      ]) {
...
async function twoFactor() {
  ...
  await step(
    A3,
    "administrator",
    "With Require two-factor authentication on under Legal User Authentication, an Administrator without an authenticator must set one up before continuing",
    "Sign-in leads to enrolment with the organization requirement; enrolment then reaches the app",
    async () => {
      const { page } = admin;
