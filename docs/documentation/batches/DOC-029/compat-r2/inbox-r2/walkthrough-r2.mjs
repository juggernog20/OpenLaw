// DOC-029r2 inbox group, round 2 compatibility replay of the round 1 script: independent browser walkthrough of
//   docs/user-guides/triage-requests.md (V-C12) and docs/user-guides/convert-request.md (V-C13)
// Replayed on the work2 lab built from app commit 57e77e386be31b2a319f7143dd54d00123e65efe (round 1 ran on 3fa407e3).
// Adapted by the DOC-029r2 compatibility reviewer (inbox): lab project, record prefix and output paths only.
//
// Prerequisites, run once from the worktree root:
//   mise exec -- node docs/documentation/batches/DOC-029/compat-r2/inbox-r2/setup-config-r2.mjs
//   mise exec -- node docs/documentation/batches/DOC-029/compat-r2/inbox-r2/setup-requests-r2.mjs
// Run (LAB_PASSWORD set in the environment):
//   LAB_PASSWORD=... mise exec -- node docs/documentation/batches/DOC-029/compat-r2/inbox-r2/walkthrough-r2.mjs
...
const fx = JSON.parse(readFileSync(path.join(here, "fixtures-r2.json"), "utf8"));
const reqs = JSON.parse(readFileSync(path.join(here, "requests-r2.json"), "utf8")).requests;
...
const log = {
  ...
  group: "inbox",
  round: 2,
  replayOf: "docs/documentation/batches/DOC-029/inbox/walkthrough-r1.mjs",
  issues: [745, 747],
  independentReview: true,
  walkthroughReviewer: "DOC-029r2 compatibility reviewer (inbox)",
  reviewerKind: "agent",
...
async function step(article, role, name, expected, fn) {
  ...
  console.log(`[${role}] ${entry.result.toUpperCase()} ${article}: ${name}${entry.result === "fail" ? ` -- ${entry.actual}` : ""}`);
  save();
  return entry;
}
...
const shot = async (page, name) => {
  await page.screenshot({ path: path.join(here, `r2-${name}.png`) });
  return `docs/documentation/batches/DOC-029/compat-r2/inbox-r2/r2-${name}.png`;
};
...
async function staffPage(role) {
  ...
  contexts[role] = await browser.newContext({ viewport: { width: 1440, height: 1000 }, acceptDownloads: true });
...
async function portalPage(key) {
  ...
  contexts[key] = await browser.newContext({ viewport: { width: 1280, height: 900 }, acceptDownloads: true });
...
const getReq = async (n, role = "administrator") => (await api[role].get(`/api/v1/requests/${n}`)).body;
...
async function postComment(page, tierLabel, text, file) {
  ...
  if (tierLabel) await page.locator("label").filter({ hasText: new RegExp(`^\\s*${tierLabel}\\s*$`) }).click();
...
async function portalText(page, n) {
  ...
  if ((await chat.count()) > 0 && !(await page.getByRole("textbox", { name: "New comment" }).isVisible().catch(() => false))) {
    await chat.first().click();
    await page.getByRole("textbox", { name: "New comment" }).waitFor({ timeout: 10000 }).catch(() => {});
    await pause(1200);
  }
...
async function commentsOn(entityType, entityId, role = "administrator") {
  const { body } = await api[role].get(`/api/v1/comments?entityType=${entityType}&entityId=${entityId}&limit=100`);
...
async function triage(role) {
  ...
  await step(A, role, "Open Inbox: Requests tab first, built-in view Status: New, ordered by urgency then age", "Requests tab is the current tab; Status: New chip; rows sorted by urgency then oldest first", async () => {
    await page.goto(`${BASE}/inbox`);
    await page.getByRole("button", { name: "Status: New" }).waitFor({ timeout: 20000 });
    const current = await page.locator('a[aria-current="page"]').filter({ hasText: /^Requests \(/ }).count();
    expectThat(current === 1, "Requests tab is not the current tab");
    const tabs = await page.locator("main a").filter({ hasText: /^(Requests|Unassigned contracts) \(/ }).allInnerTexts();
    const refs = (await page.locator("tbody tr td:first-child").allInnerTexts()).map((t) => t.trim()).filter((t) => /^R-\d+$/.test(t));
    ...
    expectThat(rows.every((r) => r.status === "new"), "a non-New row is listed");
    for (let i = 1; i < rows.length; i++) {
      const a = rows[i - 1], b = rows[i];
      const ok = URGENCY_RANK[a.urgency] > URGENCY_RANK[b.urgency] || (a.urgency === b.urgency && a.createdAt <= b.createdAt);
      expectThat(ok, `order broken between ${a.ref} (${a.urgency}, ${a.createdAt}) and ${b.ref} (${b.urgency}, ${b.createdAt})`);
    }
  ...
  await step(A, role, `Open ${T.reference} and read Description, Form responses, Attachments, Converts to, Requester, Department, Urgency`, "All named facts visible on the Request page", async () => {
    ...
    for (const needle of ["Description", "Form responses", "Attachments", "Converts to", "Contract · DOC-029r2 inbox Supplier agreement", "Requester", "Jonas Weber", "Department", "Engineering", "Urgency", "High", "Northwind Fictional Supplies", "Budget note stays on the Request", T.files[0].name, T.files[1].name]) {
      expectThat(text.includes(needle), `missing ${needle}`);
    }
    return `Opened from the list row link. Page shows Requester Jonas Weber, Converts to Contract · DOC-029r2 inbox Supplier agreement, Department Engineering, Urgency High, Description, two Form responses, and Attachments ${T.files.map((f) => f.name).join(", ")}.`;
  });

  await step(A, role, "Select attachment filenames: PDF displays, Word file is prepared for reading, Download saves the original bytes", "Viewer shows PDF and Word text; Download bytes equal the stored original", async () => {
    const detail = await getReq(T.number, role);
    const out = [];
    for (const att of detail.attachments) {
      const original = await api[role].request("GET", `/api/v1/requests/${T.number}/attachments/${att.id}`, {});
      await page.getByRole("button", { name: att.filename, exact: true }).click();
      const viewer = page.getByRole("dialog");
      const expectedText = att.filename.endsWith(".pdf") ? "Fictional attachment" : "Fictional Word attachment";
      await viewer.getByText(expectedText, { exact: false }).first().waitFor({ timeout: 60000 });
      const [download] = await Promise.all([page.waitForEvent("download"), viewer.getByRole("link", { name: "Download" }).click()]);
      const bytes = readFileSync(await download.path());
      const direct = await page.request.get(`${BASE}/api/v1/requests/${T.number}/attachments/${att.id}`);
      const directBytes = Buffer.from(await direct.body());
      expectThat(sha(bytes) === sha(directBytes), `download of ${att.filename} differs from the stored original`);
      expectThat(bytes.length === T.files.find((f) => f.name === att.filename).bytes, `download of ${att.filename} differs in size from the submitted file`);
      if (att.filename.endsWith(".pdf")) await shot(page, `${role}-viewer-pdf`);
      await page.keyboard.press("Escape");
      await viewer.waitFor({ state: "hidden" });
      out.push(`${att.filename} opened in the viewer with its text and downloaded ${bytes.length} bytes identical to the submitted file`);
      void original;
    }
    return out.join("; ") + ".";
  });

  await step(A, role, `Assign ${T.reference} from the list row: Search people, choose ${them}, Save assignment; status stays New; the other legal session still has Triage`, "Row and Request page show the saved assignee; status New; other session sees Triage", async () => {
    await page.goto(`${BASE}/inbox`);
    const row = await inboxRow(page, T.reference);
    ...
    const listed = (await dialog.locator("fieldset label").allInnerTexts()).map((t) => t.replace(/^[A-Z]{2}\s*/, "").trim());
    expectThat(!listed.some((n) => /Ravi Menon|Jonas Weber|Amara Nwosu/.test(n)), `non-legal person offered: ${listed.join(", ")}`);
    ...
    await page.getByRole("button", { name: `Reassign ${T.reference}: ${them}` }).first().waitFor();
    const r = (await getReq(T.number)).request;
    expectThat(r.assignee?.displayName === them && r.status === "new", "assignment not saved or status changed");
  ...
  await step(A, role, "Portal shows the saved triage assignee as Owner", `Jonas's Portal page reads Owner: ${them}`, async () => {
  ...
  await step(A, role, "Reassign from the avatar control; Cancel leaves the saved assignment; Unassigned clears it and the Portal shows Not assigned yet", "Cancel no change; Unassigned saves null; Portal Owner: Not assigned yet", async () => {
    await gotoRequest(page, T.number);
    ...
    const { text } = await portalText(jonas, T.number);
    expectThat(text.includes("Owner: Not assigned yet"), "Portal does not read Owner: Not assigned yet");
  ...
  await step(A, role, "If it does not work: people list failure offers Retry; a failed save shows the error and keeps the saved assignee", "Retry reloads people; failed save leaves the prior assignee", async () => {
    await gotoRequest(page, T.number);
    let failNext = true;
  ...
  await step(A, role, "Clarify through Comments: audience starts on Legal only; Shared with requester reaches Jonas; Legal only does not; Request stays New", "Default Legal only; shared text on Portal; legal-only text absent; status new", async () => {
    await gotoRequest(page, T.number);
    await openComments(page);
    const checked = await page.locator('input[name="comment-tier"]:checked').getAttribute("value");
    expectThat(checked === "legal_only", `composer starts on ${checked}`);
    const legalText = `DOC-029r2 inbox ${role} legal only note ${Date.now()}`;
    const sharedText = `DOC-029r2 inbox ${role} question for the requester ${Date.now()}`;
  ...
  await step(A, role, "Triage menu offers Convert to contract, Convert to matter, Resolve request without converting, and no Decline", "Exactly the three actions", async () => {
    ...
    await page.keyboard.press("Escape");
    expectThat(JSON.stringify(items) === JSON.stringify(["Convert to contract", "Convert to matter", "Resolve request without converting"]), `menu: ${items.join(" | ")}`);
    return `Menu items: ${items.join(" | ")}.`;
  });

  await step(A, role, `Resolve ${Rz.reference}: blank note refused; Cancel leaves it undecided with nothing posted`, "Refusal message; status new; no comment", async () => {
    await gotoRequest(page, Rz.number);
    await page.getByRole("button", { name: "Triage", exact: true }).click();
    await page.getByRole("menuitem", { name: "Resolve request without converting" }).click();
    const dialog = page.getByRole("dialog", { name: `Resolve ${Rz.reference} without converting` });
    await dialog.waitFor();
    ...
    const refusal = (await dialog.getByText("Explain why this request is being resolved without converting.").first().innerText()).trim();
    await dialog.getByRole("textbox").fill("DOC-029r2 inbox note typed then cancelled");
    await dialog.getByRole("button", { name: "Cancel" }).click();
    await dialog.waitFor({ state: "hidden" });
    ...
    expectThat(!comments.some((c) => (c.body ?? "").includes("typed then cancelled")), "cancelled note posted");
  ...
  let resolveNote;
  await step(A, role, `Assign ${Rz.reference} to ${them}, then resolve it as ${me} with a note: Status card Resolved, note in Comments, assignee kept read-only, controls gone`, "Assignment does not reserve; Resolved; note in Comments; no Assign/Triage", async () => {
    await gotoRequest(page, Rz.number);
    ...
    resolveNote = `DOC-029r2 inbox ${role} resolution: no contract is needed, use the standard purchase order ${Date.now()}`;
    await page.getByRole("button", { name: "Triage", exact: true }).click();
    await page.getByRole("menuitem", { name: "Resolve request without converting" }).click();
    const dialog = page.getByRole("dialog", { name: `Resolve ${Rz.reference} without converting` });
    const notice = await dialog.getByText("This goes on the request's thread, and to the requester by email.").count();
    ...
    expectThat((await page.getByRole("button", { name: "Triage", exact: true }).count()) === 0, "Triage still offered");
    expectThat((await page.getByRole("button", { name: new RegExp(`^(Re)?[Aa]ssign ${Rz.reference}`) }).count()) === 0, "Assign still offered");
    expectThat((await page.getByRole("img", { name: them }).count()) > 0, "assignee not shown read-only");
    await openComments(page);
    await page.getByText(resolveNote).first().waitFor();
    const text = await mainText(page);
    expectThat(text.includes("Attachments") && text.includes("Form responses") && text.includes(Rz.files[0].name), "resolved Request lost submitted information or attachments");
  ...
  await step(A, role, `Requester reads the resolution note on the Portal, receives the email, and a later reply does not reopen ${Rz.reference}`, "Note on Portal; email arrives; status stays resolved after reply", async () => {
    const { text } = await portalText(jonas, Rz.number);
    expectThat(text.includes(resolveNote), "note missing on Portal");
    const mail = await waitForMail(PEOPLE.business_user.email, new RegExp(`resolved.*R-${Rz.number}\\b|R-${Rz.number}\\b.*resolved`, "i"), log.resolveTimes[Rz.reference], 60000);
    const replyText = `DOC-029r2 inbox Jonas thanks for ${Rz.reference} ${Date.now()}`;
  ...
  await step(A, role, `Lost race on ${Rc.reference}: ${them} resolves while the dialog is open; dialog states their outcome with Close; the losing note is not posted`, "Somebody else already resolved this request.; Close; only the winning note", async () => {
    await gotoRequest(page, Rc.number);
    await page.getByRole("button", { name: "Triage", exact: true }).click();
    await page.getByRole("menuitem", { name: "Resolve request without converting" }).click();
    const dialog = page.getByRole("dialog", { name: `Resolve ${Rc.reference} without converting` });
    const winning = `DOC-029r2 inbox ${otherRole} winning resolution ${Date.now()}`;
    const losing = `DOC-029r2 inbox ${role} losing resolution ${Date.now()}`;
    ...
    await dialog.getByRole("button", { name: "Resolve request" }).click();
    await dialog.getByText("Somebody else already resolved this request.").waitFor({ timeout: 15000 });
    ...
    await dialog.waitFor({ state: "hidden" });
    await page.locator('[aria-labelledby="inbox-request-outcome-heading"]').getByText("Resolved").first().waitFor({ timeout: 15000 });
    const comments = await commentsOn("request", Rc.id);
    const bodies = comments.map((c) => c.body ?? "");
    expectThat(bodies.some((b) => b.includes(winning)), "winning note missing");
  ...
  await step(A, role, "Find decided Requests: Status filter Resolved lists them; removing the Status filter lists all outcomes", `${Rz.reference} appears under Resolved and without the Status filter; not in the New view`, async () => {
    await page.goto(`${BASE}/inbox`);
    await page.getByRole("button", { name: "Status: New" }).waitFor();
    expectThat((await (await inboxRow(page, Rz.reference)).count()) === 0, "resolved Request still in the New view");
  ...
  await step(A, role, "Historical Declined Request shows its reason and no Assign or Triage controls", "Status: Declined lists a seeded declined Request; its Status card shows Declined and the reason", async () => {
    await page.goto(`${BASE}/inbox`);
    ...
    expectThat(r.status === "declined" && r.declinedReason, "listed Request is not declined with a reason");
    ...
    expectThat(cardText.includes(r.declinedReason.replace(/\s+/g, " ").slice(0, 30)), "reason not on Status card");
    expectThat((await page.getByRole("button", { name: "Triage", exact: true }).count()) === 0, "Triage offered on declined");
    expectThat((await page.getByRole("button", { name: /^(Re)?[Aa]ssign R-/ }).count()) === 0, "Assign offered on declined");
...
async function convert(role) {
  ...
  await step(A, role, `Before converting ${Cc.reference} and ${Cm.reference}: assign ${them} for triage and post a Legal only note and a Shared with requester message with paper`, "Comments posted with attachments; Jonas sees only the shared one", async () => {
    const out = [];
    for (const R of [Cc, Cm]) {
      ...
      const legalFile = path.join(here, "fixtures", `r2-${role}-${R.number}-legal-paper.pdf`);
      const sharedFile = path.join(here, "fixtures", `r2-${role}-${R.number}-shared-paper.pdf`);
      const { makePdf } = await import("../../../../../../scripts/seed/files.mjs");
      const { mkdirSync } = await import("node:fs");
      mkdirSync(path.join(here, "fixtures"), { recursive: true });
      writeFileSync(legalFile, makePdf(`DOC-029r2 inbox legal paper ${R.reference}`, ["Fictional internal note paper."]));
      writeFileSync(sharedFile, makePdf(`DOC-029r2 inbox shared paper ${R.reference}`, ["Fictional paper shared with the requester."]));
      const legalText = `DOC-029r2 inbox ${role} legal only before conversion ${R.reference}`;
      const sharedText = `DOC-029r2 inbox ${role} shared before conversion ${R.reference}`;
      ...
      expectThat(text.includes(sharedText) && !text.includes(legalText), `Portal reach wrong on ${R.reference}`);
      const detail = await getReq(R.number, role);
      originals[R.number] = [];
      for (const att of detail.attachments) {
        const res = await page.request.get(`${BASE}/api/v1/requests/${R.number}/attachments/${att.id}`);
        originals[R.number].push({ id: att.id, filename: att.filename, sha: sha(Buffer.from(await res.body())) });
      }
      out.push(`${R.reference}: assignee ${them}; Legal only and Shared with requester comments with PDFs posted; Portal shows only the shared one`);
    }
    return out.join("; ") + ".";
  });

  await step(A, role, `Open ${Cc.reference} Triage > Convert to contract with preparation off: manual Convert dialog opens directly`, "No Getting contract ready, no Description box, no Unverified markers; Title, Type, Priority from Urgency, Counterparty, Needed by, carried Field, stay list, required Field, archived Entity note", async () => {
    const r = await api.administrator.get("/api/v1/ai-connector");
    const c = r.body.connector;
    expectThat(!c.contractPreparation && !c.matterPreparation && !c.contractConversionAnalysis, "AI switches are not all off in this lab");
    ...
    expectThat((await dialog.getByLabel("Description").count()) === 0, "Description box present in manual conversion");
    expectThat(!text.includes("Unverified"), "Unverified marker present");
    expectThat((await dialog.getByLabel(/^Title/).inputValue()) === Cc.title, "Title not seeded from the Request");
    expectThat((await dialog.locator("#convert-type").inputValue()) === fx.contractType.id, "configured Contract type not selected");
    expectThat((await dialog.locator("#convert-priority").inputValue()) === "high", "Priority not High from Urgency");
    expectThat((await dialog.getByLabel(F.carry.name).inputValue()).startsWith("Northwind Fictional Supplies"), "carried answer missing");
    expectThat(text.includes("Does not carry into the contract") && text.includes(F.stay.name), "stay list missing");
    expectThat(text.includes(`${fx.entities.old.name} is archived. Pick a live entity to convert.`), "archived Entity note missing");
    expectThat(text.includes("Counterparty") && text.includes("Needed by"), "Counterparty or Needed by missing");
  ...
  await step(A, role, "Refusals: empty Title, empty required Field, and the archived reference are refused by name; Cancel leaves the Request undecided", "Named refusals; status new after Cancel", async () => {
    const dialog = dialogFor(page, Cc.reference, "contract");
  ...
  let contractNumber;
  let typeChangeNote = "";
  await step(A, role, `Convert ${Cc.reference}: replace the archived Entity with a live one, answer the required Field, set Needed by, and convert once`, "One Contract; Request Converted with C- link and Converted by; no Triage", async () => {
    await openConvert(page, Cc.number, "contract");
    const dialog = dialogFor(page, Cc.reference, "contract");
    await dialog.waitFor();
    await dialog.getByLabel(F.required.name).fill(`CC-${role}-001`);
    ...
    if (keptAfterTypeChange !== `CC-${role}-001`) await dialog.getByLabel(F.required.name).fill(`CC-${role}-001`);
    ...
    expectThat(detail.request.status === "converted" && detail.request.convertedRecord?.module === "contract", "not converted to a contract");
    contractNumber = detail.request.convertedRecord.number;
    log.records.push({ role, request: Cc.reference, record: `C-${contractNumber}` });
    const cardText = (await card.innerText()).replace(/\s+/g, " ");
    expectThat(cardText.includes(`C-${contractNumber}`) && cardText.includes(`Converted by ${me}`), `Status card: ${cardText}`);
    const list = (await api[role].get(`/api/v1/contracts?q=${encodeURIComponent(Cc.title)}&limit=10`)).body.contracts ?? [];
  ...
  await step(T, role, `Converted ${Cc.reference} in the Inbox keeps assignee, Description and Form responses, shows no Attachments card or Comments, and has no Assign or Triage`, "Detail as described for a converted Request", async () => {
    await gotoRequest(page, Cc.number);
    const text = await mainText(page);
    expectThat(text.includes("Description") && text.includes("Form responses"), "Description or Form responses missing");
    expectThat(!text.includes("Attachments"), "Attachments card shown");
    expectThat((await page.getByRole("button", { name: /^Comments( \(\d+\))?$/ }).count()) === 0, "Comments applet present");
    expectThat((await page.getByRole("img", { name: them }).count()) > 0, "assignee not kept");
    expectThat((await page.getByRole("button", { name: "Triage", exact: true }).count()) === 0, "Triage offered");
    const link = page.locator('[aria-labelledby="inbox-request-outcome-heading"]').getByRole("link", { name: `C-${contractNumber}` });
  ...
  await step(A, role, `Open C-${contractNumber} from the Status card and check Title, Type, Priority, Description, Department, Needed by Key date, Fields, owners, team, Risk`, "Values as the guide states", async () => {
    await page.locator('[aria-labelledby="inbox-request-outcome-heading"]').getByRole("link", { name: `C-${contractNumber}` }).click();
    ...
    expectThat(c.owningDepartment === req.department, `Department ${c.owningDepartment} vs ${req.department}`);
    ...
    if (fx.contractTypeDefaultPerson) expectThat(team.includes(fx.contractTypeDefaultPerson), "Contract Type default person not on team");
    ...
    expectThat(nb && JSON.stringify(nb).includes(neededBy), `Needed by Key date missing: ${JSON.stringify(kd).slice(0, 200)}`);
    expectThat(req.customFields[F.stay.slug] && req.department && req.description, "Request lost its answers");
    const overview = await mainText(page);
    expectThat(overview.includes("Legal Owner") && overview.includes(me) && overview.includes("Business Owner") && overview.includes("Jonas Weber"), "owners not visible");
    expectThat(overview.includes("Current") && overview.includes("Requester"), "Current / Requester switch missing");
    await page.getByRole("link", { name: /^Fields/ }).click();
    await page.waitForLoadState("networkidle").catch(() => {});
    await pause(1000);
    const fieldsText = await mainText(page);
    expectThat(fieldsText.includes(F.carry.name) && fieldsText.includes(F.required.name), "Fields tab lacks the Fields");
  ...
  await step(A, role, `C-${contractNumber} Documents: each Request attachment is a root Document at Version 1 with its bytes; the first is primary; comment paper is not filed; comments keep tiers and authors`, "Two Documents; primary set; comments moved with tiers", async () => {
    ...
    expectThat(primary.length === 1 && JSON.stringify(primary[0]).includes(orig[0].filename), `primary: ${JSON.stringify(details)}`);
    expectThat(!names.some((n) => /legal-paper|shared-paper/.test(n ?? "")), "comment paper became a Document");
  ...
  await step(A, role, `After conversion the original ${Cc.reference} attachment links stop working (staff and Portal)`, "404 on original attachment routes", async () => {
    const out = [];
    for (const o of originals[Cc.number]) {
      const staff = await page.request.get(`${BASE}/api/v1/requests/${Cc.number}/attachments/${o.id}`);
      const portal = await jonas.request.get(`${BASE}/api/v1/portal/requests/${Cc.number}/attachments/${o.id}`);
  ...
  await step(A, role, `Portal: Jonas's R- address redirects to C-${contractNumber}, the Request leaves Your requests, Original request is shown; Amara is refused`, "Redirect; not in Your requests; Original request; Amara refused", async () => {
    await jonas.goto(`${BASE}/portal/requests/${Cc.number}`);
    await jonas.waitForURL((u) => !u.pathname.endsWith(`/requests/${Cc.number}`), { timeout: 15000 });
    ...
    const listed = (await jonas.request.get(`${BASE}/api/v1/portal/requests?limit=200`)).json ? await (await jonas.request.get(`${BASE}/api/v1/portal/requests?limit=200`)).json() : {};
    ...
    const amaraRecord = await amara.request.get(`${BASE}/api/v1/portal/contracts/${contractNumber}`);
    expectThat(!amaraText.includes(Cc.title) && amaraRecord.status() >= 400, "Amara could read it");
  ...
  let matterNumber;
  await step(A, role, `${Cm.reference} Convert to matter: choose the Matter template, review defaults (carried answer wins, template default fills), explicitly clear the archived Entity, answer required Field`, "Template shows defaults; No template removes them; conversion succeeds", async () => {
    ...
    expectThat(carry.startsWith("Northwind Fictional Supplies"), `carried answer replaced by template: ${carry}`);
    ...
    await dialog.waitFor({ state: "hidden", timeout: 30000 });
    await page.locator('[aria-labelledby="inbox-request-outcome-heading"]').getByText("Converted").first().waitFor({ timeout: 15000 });
  ...
  await step(A, role, `M-${matterNumber}: Title and Priority kept, Risk unset, Matter Manager and Business Owner, Custom fields, template Tasks and Key dates from the creation date in UTC, Documents without primary`, "Values as the guide states", async () => {
    ...
    expectThat(body.team.some((t) => t.displayName === "Jonas Weber"), "Requester not on team");
    expectThat(m.customFields[F.carry.slug]?.startsWith("Northwind Fictional Supplies"), "carried value lost");
    ...
    expectThat(kd.some((d) => /Needed by/i.test(JSON.stringify(d)) && JSON.stringify(d).includes(neededBy)), "Needed by Key date missing");
    ...
    await pause(1500);
    const text = await mainText(page);
    expectThat(text.includes("Custom fields") && text.includes(F.carry.name), "Custom fields on Overview missing");
    expectThat(text.includes("Current") && text.includes("Requester"), "Current / Requester switch missing");
    await shot(page, `${role}-matter-overview`);
    await jonas.goto(`${BASE}/portal/requests/${Cm.number}`);
    await jonas.waitForURL((u) => u.pathname.includes(`/matters/${matterNumber}`), { timeout: 15000 });
    await pause(1500);
    expectThat(/Original request/i.test(await jonas.locator("main").innerText()), "Original request missing on the Portal matter");
  ...
  await step(A, role, `Re-target ${Crt.reference} (configured for Contract): Convert to matter, check the destination, switch with Convert to contract instead and back, then convert once`, "Matter type must be chosen; one Matter created", async () => {
    ...
    let refusal = "";
    if (!typeValue) {
      await dialog.getByLabel(F.required.name).fill("CC-retarget").catch(() => {});
      await dialog.getByRole("button", { name: "Convert to matter", exact: true }).click();
    ...
    log.records.push({ role, request: Crt.reference, record: `M-${detail.request.convertedRecord.number}` });
  ...
  await step(A, role, `Lost race on ${Crace.reference}: ${them} converts first; the dialog states the outcome and record with Close; no second Contract`, "Somebody else already converted this request.; It became C-n.; one record", async () => {
    ...
    await page.route(`**/api/v1/requests/${Crace.number}/convert`, async (route) => {
      await api[otherRole].post(`/api/v1/requests/${Crace.number}/convert`, { title: Crace.title, customFields: { [F.required.slug]: "CC-race-winner" } });
      await route.continue();
    });
    await dialog.getByRole("button", { name: "Convert to contract", exact: true }).click();
    ...
    const again = await api[role].request("POST", `/api/v1/requests/${Crace.number}/convert`, { json: { title: Crace.title }, expect: [409] });
  ...
  await step(A, role, `${Cst.reference}: a refused template default is recovered by choosing No template and answering the required Field`, "Refusal names the Field; Request stays new; then converts", async () => {
    ...
    expectThat((await getReq(Cst.number)).request.status === "new", "refused conversion decided the Request");
    await dialog.locator("#convert-template").selectOption({ label: "No template" });
    await pause(600);
    await dialog.getByLabel(F.required.name).fill("CC-stale");
    await dialog.getByRole("button", { name: "Convert to matter", exact: true }).click();
...
async function access() {
  ...
  await step("triage-requests", "business_user", "Business Users cannot open or triage the Inbox", "Inbox redirects to the Portal; staff Request list and convert API refused", async () => {
    const out = [];
    for (const key of ["business_user", "second_business_user"]) {
      ...
      const conv = await bp.request.post(`${BASE}/api/v1/requests/${n}/convert`, { data: { title: "DOC-029r2 inbox refused" }, headers: { origin: BASE } });
      expectThat(landed !== "/inbox" && !landed.startsWith("/inbox/"), `${key} opened the Inbox`);
      expectThat(list.status() === 403 && detail.status() >= 400 && conv.status() >= 400, `${key} API ${list.status()} ${detail.status()} ${conv.status()}`);
      out.push(`${PEOPLE[key].name}: /inbox landed on ${landed}; staff list API ${list.status()}, staff Request detail ${detail.status()}, convert ${conv.status()}`);
    }
