...
import { BASE, SEED, api, ensureStaffAccounts, newContext, passwordSignIn, sleep, sql, text, waitForLink } from "./lib-r1.mjs";
...
export async function run(log, fx) {
  ...
  for (const role of ["administrator", "legal_team_member"]) {
    ...
    await chooser.setFiles({ name: "doc029-access-avatar.png", mimeType: "image/png", buffer: small });
    ...
    await fileInput.setInputFiles({ name: "doc029-access-too-large.png", mimeType: "image/png", buffer: big });
    ...
    if (role === "administrator") await p.screenshot({ path: new URL("./r1-administrator-portal-view-banner.png", import.meta.url).pathname });
