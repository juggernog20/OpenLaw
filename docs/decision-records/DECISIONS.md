...
For a **Business User, the team row is the only record grant**. Business Owner assignment, being the original Requester, being named in a comment, and being on a related record do not independently grant access. Changing Business Owner does not remove an existing team row. Removing a Business User's row removes their Portal access on the next read, including through old Request addresses, comments, Documents, downloads, and notifications. Every write also checks current membership.
