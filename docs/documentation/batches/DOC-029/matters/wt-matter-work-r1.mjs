// V-C24 steps for "Manage Matter Tasks, Key dates, and relationships" (matter-work.md).
import { expectThat, q, matterByNumber } from "./lib-r1.mjs";
...
export default async function matterWork(ctx) {
  ...
  await step(
    ...
    async () => {
      ...
      await d.getByLabel("Title").fill("DOC-029 work cancelled task");
  ...
  await step(
    "Maintain Tasks: Add Task with a staged person, due date and Add a note saves the Task, membership and assignment; the note uses Working team",
    "Saving the Task saves both team membership and assignment; the initial note posts with the Working team audience on the Task conversation.",
    async () => {
      await tasksRegion().getByRole("button", { name: "Add Task" }).click();
      const d = page.getByRole("dialog", { name: "Add a Task" });
      await d.getByLabel("Title").fill("DOC-029 work review task");
      await d.getByLabel("Description").fill("Fictional task description.");
      await stagePerson(d.getByRole("button", { name: "Assignee" }), "Tom Iwu", "Use this person");
      await d.getByLabel("Due date").fill(day(10));
      await d.getByLabel("Add a note").fill("DOC-029 fictional initial note.");
      await d.getByRole("button", { name: "Add Task" }).click();
      await d.waitFor({ state: "hidden", timeout: 20000 });
      const t = await tasks();
      const review = t.tasks.find((x) => x.title === "DOC-029 work review task");
      const team = await teamNames();
      const comments = (
      ...
      expectThat(
        comments.length === 1 && comments[0].visibility === "working_team",
        q(comments.map((c) => c.visibility)),
      );
      expectThat(
        !recordComments.some((c) => JSON.stringify(c).includes("DOC-029 fictional initial note")),
        "note on record conversation",
      );
      return `Saved "DOC-029 work review task": assignee ${review.assigneeName}, due ${review.dueDate}; team now ${q(team)}. The Task conversation holds ${comments.length} comment with visibility ${comments[0].visibility}; the record conversation (${recordComments.length} comments) does not contain the note.`;
    },
  );
  ...
  await step(
    ...
    async () => {
      await api(page, "POST", `/matters/${s.w}/tasks`, { title: "DOC-029 work plain task" });
      await api(page, "POST", `/matters/${s.w}/tasks`, {
        title: "DOC-029 work empty task",
        dueDate: day(5),
      });
      await gotoTab("tasks");
      const rowButton = tasksRegion().getByRole("button", {
        name: /^Change assignee for DOC-029 work plain task/,
      });
      await stagePerson(rowButton, "Ines Duarte", "Add to team and assign");
      await page.waitForTimeout(800);
      const assigned = (await tasks()).tasks.find((x) => x.title === "DOC-029 work plain task");
      ...
      await page
        ...
        .click();
      await page.waitForTimeout(800);
      const cleared = (await tasks()).tasks.find((x) => x.title === "DOC-029 work plain task");
      const teamAfterClear = await teamNames();
      await tasksRegion()
        .getByRole("button", { name: "DOC-029 work plain task", exact: true })
        .click();
      ...
      const afterClose = (await tasks()).tasks.find((x) => x.title === "DOC-029 work plain task");
  ...
  await step(
    ...
    async () => {
      ...
      expectThat(
        q(order) ===
          q(["DOC-029 work empty task", "DOC-029 work review task", "DOC-029 work plain task"]),
        q(order),
      );
      await tasksRegion()
        .getByRole("checkbox", { name: "Complete Task: DOC-029 work review task" })
        .click();
      await page.waitForTimeout(1200);
      const hidden = (await tasksRegion().getByText("DOC-029 work review task").count()) === 0;
      const count = (
        await tasksRegion()
          .getByText(/of \d+ done/)
          .innerText()
      ...
      const reopenBox = tasksRegion().getByRole("checkbox", {
        name: "Reopen Task: DOC-029 work review task",
      });
      const shown = await reopenBox.isVisible();
      await reopenBox.click();
      await page.waitForTimeout(1200);
      const reopened = (await tasks()).tasks.find((x) => x.title === "DOC-029 work review task");
      await tasksRegion()
        .getByRole("button", { name: "Actions for DOC-029 work plain task" })
        .click();
      await page.getByRole("menuitem", { name: "Edit Task" }).click();
      const details = page.getByRole("dialog", { name: "Task details" });
      await details.getByLabel("Title").fill("DOC-029 work renamed task");
      await details.getByRole("button", { name: "Save" }).click();
      await details.waitFor({ state: "hidden" });
      await tasksRegion()
        .getByRole("button", { name: "DOC-029 work renamed task", exact: true })
        .waitFor();
      const countAfter = (
        await tasksRegion()
          .getByText(/of \d+ done/)
  ...
  await step(
    "Maintain Tasks: Task details conversation audiences; Remove Task on an empty Task and on a Task with conversation history",
    "Task comments offer Legal only and Working team; Remove Task removes an empty Task; a Task with a conversation cannot be removed.",
    async () => {
      await gotoTab("tasks");
      await tasksRegion()
        .getByRole("button", { name: "DOC-029 work review task", exact: true })
        .click();
      const details = page.getByRole("dialog", { name: "Task details" });
      await details.waitFor();
      const audienceText = (await details.getByRole("group", { name: "Audience" }).innerText())
        .replace(/\s+/g, " ")
        .trim();
      await details.getByText("DOC-029 fictional initial note.").waitFor();
      await details.getByRole("contentinfo").getByRole("button", { name: "Close" }).click();
      await details.waitFor({ state: "hidden" });
      const confirm = page
      ...
      await tasksRegion()
        .getByRole("button", { name: "Actions for DOC-029 work empty task" })
        .click();
      ...
      await tasksRegion()
        .getByRole("button", { name: "Actions for DOC-029 work review task" })
        .click();
      await page.getByRole("menuitem", { name: "Remove Task" }).click();
      await page.waitForTimeout(600);
      if (
      ...
      expectThat(
        audienceText.includes("Legal only") &&
          audienceText.includes("Working team") &&
          !audienceText.includes("Full thread"),
        audienceText,
      );
      expectThat(
        !afterEmpty.includes("DOC-029 work empty task") &&
          afterHeld.includes("DOC-029 work review task") &&
          refusal.length > 0,
        `${q(afterHeld)} ${q(refusal)}`,
      );
      return `Task details for the review Task shows the initial note and Audience ${q(audienceText)}. Remove Task on the empty Task (confirmation dialog shown: ${emptyConfirm}) removed its row (remaining ${q(afterEmpty)}). Remove Task on the review Task showed ${q(refusal)} and the Task stayed.`;
    },
  );
  ...
  await step(
    ...
    async () => {
      ...
      await tasksRegion().getByRole("button", { name: "Add Task" }).click();
      const d = page.getByRole("dialog", { name: "Add a Task" });
      await d.getByLabel("Title").fill("DOC-029 work retry task");
      await d.getByLabel("Add a note").fill("DOC-029 retry note.");
      let blocked = 0;
      ...
      const matches = (await tasks()).tasks.filter((x) => x.title === "DOC-029 work retry task");
      const comments = (
      ...
      return `With the comment request failed once by the reviewer's browser route (a network stand-in), the dialog stayed open with ${q(message)} and Retry note & attachments. Retry closed the dialog; ${matches.length} Task has that title and it holds ${comments.length} comment.`;
    },
  );

  await step(
    "Maintain Key dates: Add date refusals, reminder lead times and recipient choices",
    "Missing Date or Event is refused; Reminders show global lead times and the combined schedule; Add lead time and Remove change the schedule; recipients list the team without Business Users; the saved date keeps chosen lead times and recipients.",
    async () => {
      ...
      await d.getByLabel("Event").fill("DOC-029 work filing");
      ...
      await d.getByRole("button", { name: "Add date" }).click();
      await d.waitFor({ state: "hidden" });
      const saved = (await dates()).find((x) => x.label === "DOC-029 work filing");
      expectThat(
      ...
      expectThat(!peopleText.includes("Jonas Weber") && peopleText.includes("Tom Iwu"), peopleText);
      expectThat(
      ...
      return `Add date with nothing showed ${q(noDate)}; with only a Date ${q(noEvent)}. ${q(global)}. 731 left Add lead time disabled: ${tooFarDisabled}. After adding 3: ${q(withThree)}; after Remove: ${q(withoutThree)}. Reminder group text ${q(peopleText.slice(peopleText.indexOf("With no selection")))}; no Jonas Weber listed. Saved ${saved.date} "DOC-029 work filing" with lead times ${q(saved.reminderOffsetDays)} and ${saved.reminderRecipientIds.length} recipients; the row has no assignee field.`;
    },
  );
  ...
  await step(
    ...
    async () => {
      ...
      await page.reload();
      await datesRegion().getByRole("button", { name: "Actions for DOC-029 work filing" }).click();
      ...
      await d.waitFor({ state: "hidden" });
      const filing = (await dates()).find((x) => x.label === "DOC-029 work filing");
      await datesRegion()
        .getByRole("button", { name: "Actions for DOC-029 work Tom only" })
        .click();
      await page.getByRole("menuitem", { name: "Edit date" }).click();
      await d
        .getByText("Some selected recipients have left the team and will not be reminded.")
      ...
      await d.getByRole("button", { name: "Save" }).click();
      await d.waitFor({ state: "hidden" });
      const tomDate = (await dates()).find((x) => x.label === "DOC-029 work Tom only");
      await datesRegion().getByRole("button", { name: "Actions for DOC-029 work filing" }).click();
      await page.getByRole("menuitem", { name: "Edit date" }).click();
      await d.getByRole("checkbox", { name: account.name }).uncheck();
      await d.getByRole("button", { name: "Save" }).click();
      await d.waitFor({ state: "hidden" });
      const cleared = (await dates()).find((x) => x.label === "DOC-029 work filing");
      expectThat(
  ...
  await step(
    ...
    async () => {
      await gotoTab("key-dates");
      await datesRegion().getByRole("button", { name: "Actions for DOC-029 work overdue" }).click();
      await page.getByRole("menuitem", { name: "Edit date" }).click();
      const d = page.getByRole("dialog", { name: "Edit Key date" });
      await d.getByLabel("Event").fill("DOC-029 work overdue edited");
      await d.getByRole("button", { name: "Save" }).click();
      await d.waitFor({ state: "hidden" });
      await datesRegion()
        .getByRole("button", { name: "Actions for DOC-029 work overdue edited" })
        .click();
      ...
      const kept = (await dates()).some((x) => x.label === "DOC-029 work overdue edited");
      await datesRegion()
        .getByRole("button", { name: "Actions for DOC-029 work overdue edited" })
        .click();
      await page.getByRole("menuitem", { name: "Remove date" }).click();
      ...
      await confirm.waitFor({ state: "hidden" });
      await page.waitForTimeout(800);
      const removed = !(await dates()).some((x) => x.label === "DOC-029 work overdue edited");
      const openDeadline = (await matterByNumber(page, s.w)).matter.nextDeadline;
      const close = await api(page, "PATCH", `/matters/${s.w}`, {
        statusId: closedId,
        closingNote: "DOC-029 closed for the Key date check.",
      });
      await gotoTab("key-dates");
      const listed = await datesRegion().getByText("DOC-029 work filing").isVisible();
      await datesRegion().getByRole("button", { name: "Actions for DOC-029 work filing" }).click();
      await page.getByRole("menuitem", { name: "Edit date" }).click();
      await d.getByLabel("Note").fill("Edited while closed.");
      await d.getByRole("button", { name: "Save" }).click();
      await d.waitFor({ state: "hidden" });
      const closedNote = (await dates()).find((x) => x.label === "DOC-029 work filing").note;
  ...
  await step(
    ...
    async () => {
      ...
      await d.getByRole("textbox").fill(String(s.p));
      ...
      await d.getByRole("textbox").fill(String(s.w));
      ...
      return `Set parent > search ${s.p} > M-${s.p} > Cancel left parent ${q(afterCancel)}. Confirming Set parent showed Parent ${q(parentRow)} and the button became Change parent. M-${s.p} lists M-${s.w} under Children. On M-${s.p}, Set parent to M-${s.w} showed ${q(loop)}; Cancel left its parent ${q(pParent)}.`;
    },
  );
  ...
  await step(
    ...
    async () => {
      ...
      await d.getByRole("textbox").fill(String(s.r));
  ...
  await step(
    ...
    async () => {
      ...
      const overdue = await api(page, "POST", `/matters/${s.w2}/tasks`, {
        title: "DOC-029 work overdue task",
        dueDate: day(-3),
      });
      await api(page, "POST", `/matters/${s.w2}/key-dates`, {
        date: day(4),
        label: "DOC-029 work upcoming date",
      });
