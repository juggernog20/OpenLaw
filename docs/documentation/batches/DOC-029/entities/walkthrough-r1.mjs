...
const manifest = JSON.parse(
  readFileSync(path.join(ROOT, ".documentation-labs/entities/lab.json"), "utf8"),
);
...
async function shot(page, name, list, purpose) {
  ...
  list.push({
    path: `docs/documentation/batches/DOC-029/entities/${name}`,
    purpose,
...
async function c31(roleKey) {
  ...
  await step(
    ...
    async () => {
      ...
      const option = our
        .locator("option", { hasText: "DOC-029 entities C31" })
...
async function c32(roleKey) {
  ...
  const R = roleKey;
  const who = R === "administrator" ? "A" : "L";
  const main = await fixtureEntity(s, `DOC-029 entities C32 ${who} main ${TAG}`, {
  ...
  await step(
    ...
    async () => {
      ...
      for (const months of ["1201", "0"]) {
        ...
        await dialog
          .getByLabel("Label", { exact: true })
          .fill(`DOC-029 entities C52 ${who} bad recurrence ${months} ${TAG}`);
  ...
  await step(
    ...
    async () => {
      const r = await api(s, "POST", `/api/v1/entities/${entity.id}/obligations`, {
        label: "DOC-029 entities C52 business refusal",
