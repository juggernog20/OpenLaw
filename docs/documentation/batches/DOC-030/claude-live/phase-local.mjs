...
const MCP = `${BASE}/mcp`;
const CLAUDE = { name: "Claude", id: "https://claude.ai/oauth/mcp-oauth-client-metadata", redirect: "https://claude.ai/api/mcp/auth_callback" };
...
const S = (role, page) => ({ article: "connect-claude", scenario: "V-M41-C59", role, method: "browser-walkthrough", page, secrets: [PASSWORD] });
...
async function ctx() {
  ...
  await c.route("https://claude.ai/**", (route) => {
    ...
    return route.fulfill({ status: 200, contentType: "text/plain", body: "DOC-030 captured Claude callback" });
  });
  return c;
}
async function mail(address, since) {
  for (let i = 0; i < 60; i++) {
    const r = await (await fetch(`${MAIL}/api/v1/search?query=${encodeURIComponent(`to:"${address}"`)}&limit=5`)).json();
    const m = (r.messages ?? []).find((x) => new Date(x.Created).getTime() >= since - 1500 && /sign in/i.test(x.Subject));
...
async function signIn(page, role) {
  ...
  const link = text?.match(/https?:\/\/[^\s<>"')\]]*magic-link\/verify[^\s<>"')\]]*/)?.[0]?.replace(/[.,]+$/, "");
  expectThat(link, "no magic link");
  await page.goto(link);
  await page.waitForURL(/\/portal/, { timeout: 30000 });
}
const pages = {};
for (const role of CC_ONLY ? ["administrator", "legal_team_member"] : ["administrator", "legal_team_member", "business_user"]) {
...
async function gotoMcp() {
  ...
  await admin.getByRole("navigation", { name: "Settings sections" }).getByRole("link", { name: "MCP", exact: true }).first().click();
...
async function token(clientId, code, redirect, verifier) {
  const r = await fetch(`${BASE}/api/auth/oauth2/token`, {
    ...
    body: new URLSearchParams({ grant_type: "authorization_code", client_id: clientId, code, redirect_uri: redirect, code_verifier: verifier, resource: MCP }),
  });
  let body = null;
  try {
    body = await r.json();
...
async function toolsList(access) {
  const r = await fetch(MCP, {
    method: "POST",
    headers: { "content-type": "application/json", accept: "application/json, text/event-stream", "mcp-protocol-version": "2025-06-18", authorization: `Bearer ${access}` },
    body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "tools/list", params: {} }),
  });
  const text = await r.text();
  let names = [];
  try {
    const j = JSON.parse(text.includes("data:") ? text.split("\n").filter((l) => l.startsWith("data:")).map((l) => l.slice(5)).join("") : text);
...
if (!CC_ONLY) {
// =====================================================================
await step(S("administrator", "/settings/mcp"), "Setup as Daniel Okafor: Enable MCP, Legal Users and Business Users OAuth Clients on; Claude and Claude Code enabled", "Switches save", async () => {
  await gotoMcp();
  const r = [await setSwitch("Enable MCP", true), await setSwitch("Legal Users OAuth Clients", true), await setSwitch("Business Users OAuth Clients", true)];
  ...
  const en = [await admin.getByRole("switch", { name: "Enable Claude", exact: true }).getAttribute("aria-checked"), await admin.getByRole("switch", { name: "Enable Claude Code", exact: true }).getAttribute("aria-checked")];
  expectThat(r.every((x) => x === 200 || x === "unchanged") && en.every((x) => x === "true"), JSON.stringify({ r, en }));
...
for (const role of ["administrator", "legal_team_member", "business_user"]) {
  await step(S(role, "/auth/consent"), `Create an OAuth grant as ${PEOPLE[role].name} on the real consent page with Claude's published client ID; capture the code from the redirect to claude.ai's callback; exchange it with PKCE as a public client`, "Allow redirects to https://claude.ai/api/mcp/auth_callback with code and state; token 200; tools/list 200 with Workspace Tools", async () => {
    const page = pages[role];
    page.context().captured.length = 0;
    const a = authorizeUrl(CLAUDE.id, CLAUDE.redirect, "toolset:workspace toolset:contracts write offline_access");
    await page.goto(a.url);
    const c = await consentPage(page);
    await allow(page, ["Workspace"], false);
    const cb = await until(() => page.context().captured.find((u) => u.startsWith(CLAUDE.redirect)), "no callback to claude.ai", 20000);
    const u = new URL(cb);
    expectThat(u.searchParams.get("state") === a.state && u.searchParams.get("code"), "state or code missing");
...
const all = async () => { expectThat(Object.keys(grants).length === 3, `only ${Object.keys(grants).length} grants`); return Object.fromEntries(await Promise.all(Object.entries(grants).map(async ([r, a]) => [r, (await toolsList(a)).status]))); };

await step(S("administrator", "/settings/mcp"), "Existing grants: Daniel turns MCP off → every grant's next call refused; back on → accepted", "All three 401 while off; 200 after", async () => {
  await gotoMcp();
  ...
  expectThat(Object.values(during).every((s) => s === 401) && Object.values(after).every((s) => s === 200), JSON.stringify({ during, after }));
  return `Enable MCP off → ${off}; tools/list per grant ${JSON.stringify(during)}. On → ${on}; ${JSON.stringify(after)}.`;
});
await step(S("legal_team_member", "/settings/mcp"), "Existing grants: Daniel turns Legal Users OAuth Clients off → Nadia's and Daniel's next calls refused, Jonas unaffected; back on", "Legal 401, Business 200; all 200 after", async () => {
  const off = await setSwitch("Legal Users OAuth Clients", false);
  const during = await all();
  const on = await setSwitch("Legal Users OAuth Clients", true);
  const after = await all();
  expectThat(during.administrator === 401 && during.legal_team_member === 401 && during.business_user === 200 && Object.values(after).every((s) => s === 200), JSON.stringify({ during, after }));
  return `Legal Users OAuth Clients off → ${off}; ${JSON.stringify(during)}. On → ${on}; ${JSON.stringify(after)}.`;
});
await step(S("business_user", "/settings/mcp"), "Existing grants: Daniel turns Business Users OAuth Clients off → Jonas's next call refused, Legal Users unaffected; back on", "Business 401, Legal 200; all 200 after", async () => {
  const off = await setSwitch("Business Users OAuth Clients", false);
  const during = await all();
  const on = await setSwitch("Business Users OAuth Clients", true);
  const after = await all();
  expectThat(during.business_user === 401 && during.administrator === 200 && during.legal_team_member === 200 && Object.values(after).every((s) => s === 200), JSON.stringify({ during, after }));
  return `Business Users OAuth Clients off → ${off}; ${JSON.stringify(during)}. On → ${on}; ${JSON.stringify(after)}.`;
});
await step(S("administrator", "/settings/mcp"), "Existing grants: Daniel turns the Claude Allowed Client off → every Claude grant's next call refused; back on", "All three 401 while off; 200 after", async () => {
  await expandCard("Allowed Clients");
...
await step(S("legal_team_member", "/auth/consent"), "Refusal table: a Client removed after the person started. Daniel adds a registered Client; Nadia opens its consent page; Daniel deletes the Client; Nadia reloads. A new request is then refused before sign-in", "This Client is not on the organization's Allowed Clients list. with Deny alone; new request HTTP 400 Client is not on the enabled Allowed Clients list.", async () => {
  ...
  const resp = await fresh.goto(authorizeUrl(clientId, "http://127.0.0.1:53999/callback", "toolset:workspace").url);
  const freshText = flat(await fresh.locator("body").innerText());
  await nadia.screenshot({ path: new URL("./consent-client-removed.png", import.meta.url).pathname });
  expectThat(before.allow === 1 && after.text.includes("This Client is not on the organization's Allowed Clients list.") && after.allow === 0 && after.deny === 1 && resp.status() === 400 && freshText.includes("Client is not on the enabled Allowed Clients list."), JSON.stringify({ before: before.heading, after: after.text.slice(0, 400), a: after.allow, d: after.deny, fresh: resp.status(), freshText }));
...
async function disconnectCC(page) {
  ...
  await page.getByRole("banner").getByRole("button", { name: PEOPLE.legal_team_member.name }).click();
...
for (const host of ["localhost", "127.0.0.1"]) {
  await step(S("legal_team_member", "/auth/consent"), `Claude Code loopback callback http://${host}:<port>/callback: consent, Allow, the browser returns to the loopback listener, the code exchanges with PKCE`, "Consent heading Claude Code …; callback received on the loopback port; token 200; tools/list 200", async () => {
    const nadia = pages.legal_team_member;
    ...
    let remembered = "";
    if (host === "localhost" && (await disconnectCC(nadia))) remembered = "Setup: Nadia's earlier Claude Code grant from a previous run of this script was disconnected in Connected Clients first. ";
    if (host === "127.0.0.1") {
      ...
      const rt = await token(CODE.id, lastCallback.searchParams.get("code"), redirect, r.verifier);
    ...
    const tl = t.status === 200 ? await toolsList(t.body.access_token) : { status: null, names: [] };
    expectThat(c.heading === "Claude Code wants to work in OpenLaw as you" && lastCallback.searchParams.get("state") === a.state && t.status === 200 && tl.status === 200, JSON.stringify({ c: c.heading, t: t.status, tl: tl.status }));
...
await step(S("legal_team_member", "/auth/consent"), "Connect the same Client again: with Nadia's live Claude Code grant (Workspace, Read only), a new request with prompt=consent (as Claude's own requests carry) shows the consent page; new choices replace the earlier Toolsets and scope", "Consent page shown; after Allow the one grant row holds Contracts and write", async () => {
  const { execFileSync } = await import("node:child_process");
  const q = (sql) => execFileSync("docker", ["--context", "default", "exec", "openlaw-docs-80ceef9e-claudelocal-postgres-1", "psql", "-U", "openlaw", "-d", "openlaw", "-At", "-F", "|", "-c", sql], { encoding: "utf8" }).trim();
  const rows = () => q("select array_to_string(g.toolsets, ','), g.scope, g.revoked_at is null from oauth_grants g join users u on u.id=g.person_id join allowed_clients c on c.id=g.allowed_client_id where u.email='nadia.haddad@helix.example' and c.name='Claude Code'");
  const before = rows();
  const nadia = pages.legal_team_member;
  const redirect = `http://127.0.0.1:${PORT}/callback`;
  const a = authorizeUrl(CODE.id, redirect, "toolset:workspace toolset:contracts write offline_access", { prompt: "consent" });
  lastCallback = undefined;
  await nadia.goto(a.url);
  const c = await consentPage(nadia);
  ...
  const t = await token(CODE.id, lastCallback.searchParams.get("code"), redirect, a.verifier);
  const after = rows();
  expectThat(c.heading === "Claude Code wants to work in OpenLaw as you" && t.status === 200 && after === "contracts|write|t", JSON.stringify({ before, after, t: t.status }));
...
await step(S("legal_team_member", "/api/auth/oauth2/authorize"), "Negative: Claude Code with a non-loopback or wrong-path callback is refused before consent", "http://192.168.1.5:<port>/callback and http://localhost:<port>/other refused", async () => {
  const out = [];
  for (const redirect of [`http://192.168.1.5:${PORT}/callback`, `http://localhost:${PORT}/other`]) {
    const p = await (await ctx()).newPage();
    const resp = await p.goto(authorizeUrl(CODE.id, redirect, "toolset:workspace").url);
    out.push({ redirect: redirect.replace(String(PORT), "<port>"), status: resp.status(), url: new URL(p.url()).pathname, text: flat(await p.locator("body").innerText()).slice(0, 120) });
  }
  expectThat(out.every((o) => o.status >= 400 && !o.url.startsWith("/auth/")), JSON.stringify(out));
...
console.log(`local done: ${mine.length} steps, ${mine.filter((s) => s.result !== "pass").length} failed`);
