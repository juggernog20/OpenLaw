...
const ADMIN_LAB = { app: "http://127.0.0.1:23300", mail: "http://127.0.0.1:23400", project: "openlaw-docs-41255c61-admin2" };
...
async function groups() {
  ...
  const members = [
    { key: "m1", name: `DOC-029r2 admin-org Approver One ${stamp}`, email: `doc029r2-grp-one-${stamp}@helix.example`, password: newPassword() },
    { key: "m2", name: `DOC-029r2 admin-org Approver Two ${stamp}`, email: `doc029r2-grp-two-${stamp}@helix.example`, password: newPassword() },
  ];
  ...
  await step(A, "legal_team_member", "Unauthorized configuration: a Legal Team Member cannot open or change Approver groups", "Redirected; API refuses", async () => {
    ...
    const create = await api(n.page, app, "POST", "/api/v1/approver-groups", { name: `DOC-029r2 admin-org denied ${stamp}`, memberIds: [] });
    await n.ctx.close();
  ...
  await step(A, "administrator", "Create a group: profile menu, Settings, Contracts, Approver groups, Add group", "Picker lists only Administrators and Legal team members; saved members show on Edit", async () => {
    ...
    await dialog.getByLabel("Description").fill("DOC-029r2 admin-org walkthrough group. Use for reviewer checks only.");
  ...
  await step(A, "administrator", "Clean up: archive the replay group", "Group archived; no other change", async () => {
    const { page } = admin;
    await page.goto(`${app}/settings/contracts/approver-groups`);
    await page.getByRole("button", { name: `Archive ${groupName}` }).click();
    await page.getByRole("dialog", { name: `Archive ${groupName}` }).getByRole("button", { name: "Archive group" }).click();
    await page.getByRole("button", { name: `Archive ${groupName}` }).waitFor({ state: "hidden" });
    const g = (await api(page, app, "GET", "/api/v1/approver-groups?includeArchived=true")).data?.approverGroups?.find((x) => x.name === groupName);
