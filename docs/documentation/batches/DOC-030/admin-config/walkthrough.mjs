...
async function step(scenario, role, action, expected, fn, method = "browser-walkthrough") {
  ...
  try {
    entry.page = currentPage
      ? new URL(currentPage.url()).pathname + new URL(currentPage.url()).search
      : null;
  } catch {}
...
const record = (kind, value, extra = {}) =>
  results.records.push({ section, kind, name: value, ...extra });
    ...
    async () => {
      ...
      for (const f of fixed) {
        const lock = await admin
          .getByRole("img", { name: `${f} is fixed. Contract workflows depend on it.` })
          .count();
        ...
        const archive = await admin
          ...
          .count();
        expectThat(
          lock === 1 && rename === 0 && archive === 0,
          `${f}: lock ${lock}, rename ${rename}, archive ${archive}`,
        );
        out.push(f);
      }
      const others = (await admin.getByRole("button", { name: /^Rename / }).allInnerTexts()).map(
        flat,
      );
...
async function submittedNumber(page, since) {
  ...
  const mine = list.filter(
    (x) => new Date(x.createdAt ?? x.submittedAt ?? 0).getTime() >= since - 5000,
  );
...
async function formsSection(admin, browser, contexts) {
  ...
  await step(
    ...
    async () => {
      ...
      expectThat(
        S.msaId && S.goneId && S.advisoryId,
        `ids ${S.msaId} ${S.goneId} ${S.advisoryId} ${q(mt.body)}`,
      );
      const deal = await api(admin, "POST", "/api/v1/fields", {
        displayName: F.dealValue,
        moduleScope: "contract",
        fieldType: "number",
      });
      const scope = await api(admin, "POST", "/api/v1/fields", {
        displayName: F.matterScope,
        moduleScope: "matter",
        fieldType: "single_select",
        options: ["Employment", "Privacy", "Tax"],
      });
      expectThat(
        deal.status < 300 && scope.status < 300,
        `fields ${deal.status} ${q(deal.body)} ${scope.status} ${q(scope.body)}`,
      );
      ...
      const expiry = {
        ...cf.find((n) => n.rowRef === "expiry_date"),
        onIntakeForm: true,
        isRequired: true,
      ...
      const form = cf
        ...
        .map((n) =>
          ["term_type", "value", "counterparties"].includes(n.rowRef)
            ? { ...n, onIntakeForm: true, visibleOnPortal: true }
            : n,
        );
      ...
      const mput = await api(admin, "PUT", `/api/v1/matter-types/${S.advisoryId}/form`, {
        form: mf,
      });
      expectThat(mput.status < 300, `matter form put ${mput.status} ${q(mput.body)}`);
      for (const [k, v] of [
        ["contract type", F.msa],
        ["contract type", F.gone],
        ["matter type", F.advisory],
        ["contract field", F.dealValue],
        ["matter field", F.matterScope],
      ])
        record(k, v);
  ...
  await step(
    ...
    async () => {
      ...
      await admin
        ...
        .fill(name(SC, "Cancelled request type"));
      await admin.getByRole("button", { name: "Cancel", exact: true }).click();
      ...
      const cancelled = await admin
        .getByRole("button", { name: `Rename ${name(SC, "Cancelled request type")}`, exact: true })
        .count();
      ...
      const dest = await admin
        .getByRole("combobox", { name: "Default destination" })
        .evaluate((e) => e.selectedOptions[0]?.textContent);
      const destType = await admin
        .getByRole("combobox", { name: "Default matter type" })
        .evaluate((e) => e.selectedOptions[0]?.textContent)
        .catch(() => null);
      const desc = admin.getByRole("textbox", { name: "Description" });
      await desc.fill(
        "Ask Legal to review a supplier contract. Fictional DOC-030 walkthrough form.",
      );
      await desc.press("Tab");
      await pause(1500);
      await admin.reload();
      const saved = await admin.getByRole("textbox", { name: "Description" }).inputValue();
      expectThat(
        cancelled === 0 && dest === "Matter" && destType === "Default",
        `cancelled ${cancelled}, dest ${dest}, type ${destType}`,
      );
  ...
  await step(
    ...
    async () => {
      ...
      await dest.selectOption({ label: "Contract" });
      await pause(1200);
      const clearedBack = await admin
        .getByRole("combobox", { name: "Default contract type" })
        .evaluate((e) => e.selectedOptions[0]?.textContent);
      await admin
        .getByRole("combobox", { name: "Default contract type" })
        .selectOption({ label: F.msa });
      await pause(1500);
      await admin.reload();
      ...
      expectThat(
        first === "Default" && initial === "Default" && after === F.msa,
        `first ${first}, initial ${initial}, after ${after}`,
      );
      expectThat(
        cleared === "Default" && clearedBack === "Default",
        `cleared ${cleared} ${clearedBack}`,
      );
  ...
  await step(
    ...
    async () => {
      ...
      const refusal = flat(
        await admin
          .getByText(/Enter a whole number from 0 to 36,500/)
          .first()
          .innerText()
          .catch(() => ""),
      );
  ...
  await step(
    ...
    async () => {
      ...
      const controls =
        (await card.getByRole("textbox").count()) +
        (await card.getByRole("switch").count()) +
        (await card.getByRole("combobox").count());
      expectThat(
        /^Title/.test(items[0]) &&
          /^Department/.test(items[1]) &&
          /^Urgency/.test(items[2]) &&
          /^Attachments/.test(items.at(-1)),
        `items ${q(items)}`,
      );
      ...
      const refusal = flat(
        (await d.innerText())
          .split("\n")
          .filter((l) => /is required|^Fill /.test(l.trim()))
          .join(" | "),
      );
      await admin.keyboard.press("Escape");
      await d.waitFor({ state: "hidden" });
      const focus = await admin.evaluate(
        () =>
          document.activeElement?.getAttribute("aria-label") ?? document.activeElement?.textContent,
      );
      expectThat(
        controls === 0 && dText.includes(F.contractForm) && /supplier contract/.test(dText),
        `controls ${controls}; preview ${dText.slice(0, 200)}`,
      );
      expectThat(
        fixed > 0 && evergreen === 0 && refusal,
        `fixed ${fixed} evergreen ${evergreen} refusal ${refusal}`,
      );
  ...
  await step(
    ...
    async () => {
      await admin
        .getByRole("region", { name: "Intake form" })
      ...
      expectThat(
        admin.url().includes(S.msaId) && heading.includes(F.msa),
        `url ${admin.url()} header ${heading}`,
      );
      const builtins = [];
      for (const b of ["Counterparties", "Effective date", "Expiry date", "Term type", "Value"])
        builtins.push(`${b}: ${(await formRow(admin, b).count()) ? "present" : "absent"}`);
  ...
  await step(
    ...
    async () => {
      ...
      await pause(1200);
      const defaultCard = flat(
        await admin.getByRole("region", { name: "Intake form" }).innerText(),
      );
      const editHref = await admin
        .getByRole("region", { name: "Intake form" })
        .getByRole("link", { name: "Edit form" })
        .getAttribute("href");
      await admin
        .getByRole("combobox", { name: "Default contract type" })
        .selectOption({ label: F.gone });
      await admin
        .getByRole("textbox", { name: "Description" })
        .fill("Fictional form whose destination type will be archived.");
      ...
      await admin
        .getByRole("textbox", { name: "Description" })
        .fill("Ask Legal for advice. Fictional DOC-030 walkthrough form.");
      await admin.getByRole("textbox", { name: "Description" }).press("Tab");
      await admin
        .getByRole("combobox", { name: "Default matter type" })
        .selectOption({ label: F.advisory });
      await pause(1500);
      await admin
        .getByRole("region", { name: "Intake form" })
        .getByText(F.matterScope)
        .waitFor({ timeout: 15000 });
      const card = (
        await admin
          .getByRole("region", { name: "Intake form" })
      ...
      expectThat(
        editHref?.includes(def?.id),
        `module-only Edit form ${editHref}, default ${def?.id}`,
      );
  ...
  await step(
    ...
    async () => {
      ...
      await d
        ...
        .fill("example.com/doc-030-supplier-checklist");
      ...
      const refusal = flat(
        await d
          .getByText(/Enter a full web address/)
          .first()
          .innerText()
          .catch(() => ""),
      );
      await d
        .getByRole("textbox", { name: "Address" })
        .fill("https://example.com/doc-030-supplier-checklist");
      await d.getByRole("button", { name: "Add link" }).click();
      await d.waitFor({ state: "hidden", timeout: 15000 });
      await admin.getByRole("button", { name: "Add link" }).first().click();
      ...
      const kis = (await kiSel.locator("option").allInnerTexts())
        .map(flat)
        .filter((o) => o && !/^Choose/.test(o) && !/\(archived\)$/.test(o));
      ...
      const pos = async () =>
        Number(
          (
            await admin
              .getByRole("button", {
                name: new RegExp(`^Reorder ${escapeRe(F.kiLabel)}, position`),
              })
              .getAttribute("aria-label")
          ).match(/position (\d+)/)[1],
        );
      ...
      expectThat(
        refusal && after === before - 1,
        `refusal ${refusal}, positions ${before} ${after}`,
      );
  ...
  const buState =
    process.env.BU_STATE && existsSync(process.env.BU_STATE) ? process.env.BU_STATE : undefined;
  const bctx = await browser.newContext({
    viewport: { width: 1280, height: 900 },
    storageState: buState,
  });
  contexts.push(bctx);
  const business = await bctx.newPage();
  S.business = business;
  async function businessSignIn() {
    if (buState && (await business.request.get(`${BASE}/api/v1/me`)).ok())
      return "reused a Portal session from an earlier magic-link sign-in in this walk";
  ...
  await step(
    ...
    async () => {
      ...
      for (const u of ["/settings/contracts/types", "/settings/intake/request-types"]) {
        ...
        expectThat(
          !new URL(business.url()).pathname.startsWith("/settings/contracts") &&
            !new URL(business.url()).pathname.startsWith("/settings/intake"),
          `Business User stayed on ${business.url()}`,
        );
      }
      const { list, link } = await portalCard(business, F.contractForm);
      const card = flat(await link.innerText());
      expectThat(
        /Estimated turnaround: 5 business days/.test(card) && /supplier contract/.test(card),
        `card ${card}`,
      );
      await link.click();
      await business
        .getByRole("heading", { name: F.contractForm, level: 1 })
      ...
      await business.screenshot({
        path: path.join(SHOTS, "v-c40-portal-contract-form.png"),
        fullPage: true,
      });
      await business
        .getByRole("textbox", { name: /^Title/ })
        .fill(name(SC, "Supplier contract request"));
      await business.getByRole("button", { name: "Submit request" }).click();
      await pause(1500);
      const refusal = flat(
        (await business.locator("main").innerText())
          .split("\n")
          .filter((l) => /is required|^Fill /.test(l.trim()))
          .join(" | "),
      );
      ...
      const deptOpts = (
        ...
        .map(flat)
        .filter((o) => o && !/^(Choose|Select|Not set)/.test(o));
      ...
      const urgOpts = (
        ...
        .filter((o) => o && !/^(Choose|Select|Not set)/.test(o));
      if (urgOpts.length)
        await urg.selectOption({
          label: urgOpts.find((o) => /Medium|Normal/.test(o)) ?? urgOpts[0],
        });
      ...
      await pause(500);
      const expiryFixed = (await business.locator("main label").allInnerTexts())
        .map(flat)
        .filter((l) => /^Expiry date/.test(l));
      await termSel.selectOption({ value: "evergreen" });
      await pause(500);
      const expiryEvergreen = (await business.locator("main label").allInnerTexts())
        .map(flat)
        .filter((l) => /^Expiry date/.test(l));
      expectThat(
        expiryFixed.length === 1 && expiryEvergreen.length === 0,
        `Portal Expiry date: fixed ${q(expiryFixed)}, evergreen ${q(expiryEvergreen)}`,
      );
      if (termOpts.includes("evergreen")) {
        results.productBugs.push({
          ...
          expected:
            "Not set, Fixed, Auto-renew, Evergreen (the labels the Form builder preview and the Create contract dialog show)",
          observed: q(termOpts),
      ...
      await business.getByRole("button", { name: "Submit request" }).click();
      await business
        .getByText("Thanks! Your request has been submitted to legal.")
        .waitFor({ timeout: 20000 });
  ...
  await step(
    ...
    async () => {
      ...
      await link.click();
      await business
        .getByRole("heading", { name: F.matterForm, level: 1 })
        .waitFor({ timeout: 20000 });
      const guide = business.getByRole("region", { name: /Before you submit/ });
      ...
      await business
        .getByLabel(new RegExp(`^${escapeRe(F.matterScope)}`))
        .selectOption({ label: "Privacy" });
      const since = Date.now();
      await business.getByRole("button", { name: "Submit request" }).click();
      await business
        .getByText("Thanks! Your request has been submitted to legal.")
      ...
      await business
        ...
        .getByRole("link", { name: new RegExp(escapeRe(F.kiLabel)) })
        .click();
      await business.waitForLoadState("networkidle").catch(() => {});
      await pause(1000);
      const kiPath = new URL(business.url()).pathname;
      const kiHeading = flat(
        await business
  ...
  await step(
    ...
    async () => {
      ...
      const d = admin.getByRole("dialog", {
        name: new RegExp(`^Convert R-${S.contractRequest} to a contract`),
      });
      await d.waitFor({ timeout: 15000 });
      const type = await d
        .getByLabel("Contract type")
        .evaluate((e) =>
          e.tagName === "SELECT" ? e.selectedOptions[0]?.textContent : e.textContent,
        );
      const dText = flat(await d.innerText());
      await d.getByRole("button", { name: "Convert to contract" }).click();
      const conv = await until(
        async () =>
          (await api(admin, "GET", `/api/v1/requests/${S.contractRequest}`)).body?.request
            ?.convertedRecord,
        "Request not converted",
        30000,
      );
      ...
      const pageLink = flat(
        await admin
          .locator("main")
          .getByRole("link", { name: new RegExp(`C-${S.convertedContract}`) })
          .first()
          .innerText()
          .catch(() => ""),
      );
      const statusCard = flat(
        await admin
      ...
      const shown = await admin
        .locator("main")
        .getByRole("textbox", { name: new RegExp(`^${escapeRe(F.dealValue)}`) })
        .inputValue()
        .catch(async () =>
          admin
            .locator("main")
            .getByRole("spinbutton", { name: new RegExp(`^${escapeRe(F.dealValue)}`) })
            .inputValue()
            .catch(() => null),
        );
      expectThat(
        flat(type) === F.msa && Number(deal) === 125000 && term === "evergreen",
        `type ${type}, deal ${deal}, term ${term}`,
      );
  ...
  await step(
    ...
    async () => {
      ...
      const d = admin.getByRole("dialog", {
        name: new RegExp(`^Convert R-${S.matterRequest} to a matter`),
      });
      await d.waitFor({ timeout: 15000 });
      const type = await d
        .getByLabel("Matter type")
        .evaluate((e) =>
          e.tagName === "SELECT" ? e.selectedOptions[0]?.textContent : e.textContent,
        );
      ...
      const conv = await until(
        async () =>
          (await api(admin, "GET", `/api/v1/requests/${S.matterRequest}`)).body?.request
            ?.convertedRecord,
        "Request not converted",
        30000,
      );
      ...
      expectThat(
        flat(type) === F.advisory &&
          m?.customFields?.[S.scopeSlug] === "Privacy" &&
          m?.matterTypeName === F.advisory,
        `type ${type}, value ${q(m?.customFields)}`,
      );
  ...
  await step(
    ...
    async () => {
      ...
      await link.click();
      await business
        .getByRole("heading", { name: F.goneForm, level: 1 })
        .waitFor({ timeout: 20000 });
      await business
        .getByRole("textbox", { name: /^Title/ })
        .fill(name(SC, "Retired destination request"));
      const dept = business.getByLabel(/^Department/).first();
      const deptOpts = (
        await dept
          .locator("option")
      ...
      if (listed) {
        ...
        await business.screenshot({
          path: path.join(SHOTS, "v-c40-portal-archived-destination.png"),
          fullPage: true,
        });
      }
      const apiRead = await api(business, "GET", `/api/v1/portal/request-types`);
      const portalType = (apiRead.body?.requestTypes ?? apiRead.body ?? []).find?.(
        (t) => t.displayName === F.goneForm,
      );
      const detail = portalType
        ? await api(
            business,
            "GET",
            `/api/v1/portal/request-types/${portalType.slug ?? portalType.id}`,
          )
        : null;
      S.goneObserved = {
        listRow,
        ...
        listed,
        opened,
        detail: detail ? `${detail.status} ${q(detail.body?.detail ?? detail.body?.title)}` : null,
      };
      const questions = listed
        ? await business.getByRole("button", { name: "Submit request" }).count()
        : 0;
      ...
      if (listed && questions === 0) {
        results.productBugs.push({
          ...
          scenario: SC,
          title:
            "The Portal lists a Request type whose destination type is archived, and opening it shows the generic error page",
          ...
          expected:
            "Either the Portal stops listing the form, or it opens the Default type's Form (INT-002 archived target type addendum: an archived target type reads as no type), or it names the problem.",
          ...
          source:
            "apps/api/src/lib/intake-form.ts readIntakeTree (400 This Request type needs a destination type.); apps/web/src/routes/portal-request-form.tsx loader throws for any non-404 failure; apps/api/src/modules/portal/routes.ts lists the type",
  ...
  await step(
    ...
    async () => {
      ...
      const convertsTo = flat(
        await admin
          .locator("main")
          .getByText(/Converts to/)
          .first()
          .locator("..")
          .innerText()
          .catch(() => ""),
      ...
      const type = await typeSel.evaluate((e) =>
        e.tagName === "SELECT" ? e.selectedOptions[0]?.textContent : e.textContent,
      );
      const offered = (
        await typeSel
      ...
      await admin.keyboard.press("Escape");
      expectThat(
        /to a contract/.test(title) && !offered.includes(F.gone),
        `title ${title}, offered archived ${offered.includes(F.gone)}`,
      );
  ...
  await step(
    ...
    async () => {
      ...
      const listedAfter = await list
        .getByRole("link", { name: new RegExp(escapeRe(F.goneForm)) })
        .count();
      const req = (await api(admin, "GET", `/api/v1/requests/${S.goneRequest}`)).body;
      const reqType =
        req?.request?.requestTypeName ??
        req?.requestTypeName ??
      ...
      await admin
        .getByRole("button", { name: `Archive ${F.goneForm}`, exact: true })
        .waitFor({ timeout: 15000 });
      expectThat(
        /used by 1 request/.test(text) && listedAfter === 0,
        `text ${text}, listed ${listedAfter}`,
      );
...
async function runMorningRound(admin) {
  ...
  if (r.status === 200)
    return {
      how: "POST /api/v1/notifications/morning-round (the lab's dev overlay trigger, which runs the cron's own round)",
      summary: r.body,
    };
...
console.log(JSON.stringify({id, state: job?.state, output: job?.output ?? null})); await b.stop({graceful:false, wait:false}); process.exit(0);`;
  const out = execFileSync(
    ...
    .pop();
  return {
    how: `pg-boss job notification.morning-round queued in ${PROJECT}-worker-1 (trigger route answered ${r.status})`,
...
async function setTimezone(page, zone) {
  ...
  await page
    .getByRole("option", { name: new RegExp(escapeRe(city)) })
    .first()
    .click();
  await pause(1500);
...
async function addKeyDate(page, matterNumber, { date, event, extraLead = null, recipient = null }) {
  ...
  await page
    .getByRole("button", { name: "Add date" })
  ...
  const boxes = await d
    ...
    .evaluateAll((els) =>
      els.map(
        (e) =>
          `${e.getAttribute("aria-label") ?? e.parentElement?.textContent}: ${e.checked ?? e.getAttribute("aria-checked")}`,
      ),
    );
  if (recipient) {
  ...
  try {
    await step(
      ...
      async () => {
        ...
        const headings = (await admin.locator("main").getByRole("heading").allInnerTexts()).map(
          flat,
        );
        const note = flat(
          await admin
            .getByText(/Keep at least one lead time/)
            .first()
            .innerText(),
        );
    ...
    await step(
      ...
      async () => {
        ...
        await admin
          .getByRole("button", { name: new RegExp(`^Reorder ${label}, position`) })
        ...
        expectThat(
          listNow.filter((l) => l === label).length === 1 && !listNow.includes("731 days before"),
          `list ${q(listNow)}`,
        );
    ...
    await step(
      ...
      async () => {
        ...
        const help = flat(
          await admin.getByText(/Applies to everyone in the organization/).innerText(),
        );
        await sw.click();
        await until(
          async () => (await sw.getAttribute("aria-checked")) === "false",
          "switch did not turn off",
        );
        await pause(1200);
        await admin.reload();
        const persisted = await admin
          .getByRole("switch", { name: "Include comment words in email" })
        ...
        await until(
          ...
          "switch did not turn back on",
        );
        await pause(1200);
        await admin.goto(`${BASE}/settings/audit-log`);
        await admin.getByRole("searchbox", { name: "Search" }).fill("comment");
        await pause(2000);
        const rows = (await admin.getByRole("table").getByRole("row").allInnerTexts())
          .slice(1)
          .map(flat)
          .filter((r) => /Daniel Okafor/.test(r) && /comment/i.test(r));
        await admin.getByRole("button", { name: "Clear filters" }).click();
        expectThat(
          start === "true" && persisted === "false",
          `start ${start}, persisted ${persisted}`,
        );
        expectThat(
          rows.length > 0 && /Administrator/.test(rows[0]),
          `audit rows ${q(rows.slice(0, 2))}`,
        );
    ...
    await step(
      ...
      async () => {
        ...
        for (const [key, zone] of [
          ...
          const mts = (await api(admin, "GET", "/api/v1/matter-types")).body?.matterTypes ?? [];
          const m = await api(admin, "POST", "/api/v1/matters", {
            title: name(SC, `Reminder matter ${key}`),
            matterTypeId: mts.find((t) => t.displayName === "Default").id,
            managerId: users[key].id,
          });
        ...
        for (const k of plan) {
          ...
          const res = await addKeyDate(admin, r.matter, {
            date: k.date,
            event: k.event,
        ...
        const observed = {};
        for (const k of plan)
          observed[k.event] = items.filter((n) => JSON.stringify(n).includes(k.event)).length;
        ...
        const bellButton = await r.page
          .getByRole("banner")
          .getByRole("button", { name: /^Notifications/ })
          .getAttribute("aria-label");
        const wrong = plan.filter((k) =>
          k.expect ? observed[k.event] !== 1 : observed[k.event] !== 0,
        );
        expectThat(
          wrong.length === 0,
          `bell ${q(observed)}; mail ${q(mailHits)}; round ${q(round)}`,
        );
        expectThat(
          !earlyHas && !earlyMail,
          `early colleague served: bell ${earlyHas}, mail ${!!earlyMail}`,
        );
    ...
    await step(
      ...
      async () => {
        ...
        const lastState =
          (await lastRemove.count()) === 0
            ? "no Remove control"
            : `Remove disabled=${await lastRemove.isDisabled()} aria-disabled=${await lastRemove.getAttribute("aria-disabled")}`;
        if ((await lastRemove.count()) && (await lastRemove.isEnabled())) {
          await lastRemove.click();
          await pause(1200);
        }
        const stillOne = (await leadTimes(admin)).length;
        const put = await api(admin, "PUT", "/api/v1/org/reminder-offsets", {
          offsets: S.originalOffsets,
        });
        await admin.reload();
...
async function auditRows(page) {
  ...
  await table
    .or(page.getByText(/^No (entry|Tool calls) match/))
    .first()
    .waitFor();
...
async function auditSection(admin, browser, contexts) {
  ...
  await step(
    ...
    async () => {
      ...
      const tabs = (
        await admin
          .getByRole("navigation", { name: "Audit log panes" })
          .getByRole("link")
          .allInnerTexts()
      ).map(flat);
      ...
      const actions = (
        await admin.getByRole("combobox", { name: "Action" }).locator("option").allInnerTexts()
      ).map(flat);
      const action = await selectContaining(admin, "Action", /matter type|matter_type/i);
      const records = (
        await admin.getByRole("combobox", { name: "Record" }).locator("option").allInnerTexts()
      ).map(flat);
      const rows = await auditRows(admin);
      expectThat(
        rows.length > 0 && rows.every((r) => r.startsWith(A.name)),
  ...
  await step(
    ...
    async () => {
      ...
      await until(
        async () => (await admin.getByRole("table").getByRole("row").count()) - 1 > first,
      ...
      await admin.getByRole("button", { name: "Clear filters" }).click();
      expectThat(
        todayRows.length > 0 && yRows.length === 0,
        `today ${todayRows.length}, yesterday ${yRows.length}`,
      );
      expectThat(!cleared.from && !cleared.search, `cleared ${q(cleared)}`);
      const exports = searched.filter((r) => !r.includes(G));
      expectThat(
        searched.length > 0 && exports.every((r) => /exported the audit log/.test(r)),
        `search ${q(exports.slice(0, 2))}`,
      );
  ...
  await step(
    ...
    async () => {
      ...
      await admin.getByRole("button", { name: "Clear filters" }).click();
      expectThat(
        body.length > 0 && body.every((l) => l.includes(G) || l.startsWith('"') || /V-C38/.test(l)),
        `csv ${q(body.slice(0, 2))}`,
      );
  ...
  await step(
    ...
    async () => {
      ...
      const m = await api(member, "POST", "/api/v1/matters", {
        title,
        matterTypeId: (mts.find((t) => t.displayName === "Default") ?? mts[0])?.id,
        isConfidential: true,
        managerId: me?.user?.id ?? me?.id,
      });
      ...
      const hidden = (await auditRows(admin)).filter(
        (r) => r.includes(title) || r.includes(`M-${number}`),
      );
      const csvHidden = (await exportCsv(admin)).lines
        .slice(1)
      ...
      const csvShown = (await exportCsv(admin)).lines
        .slice(1)
        .filter((l) => l.includes(title) || l.includes(`M-${number}`)).length;
      await admin.getByRole("button", { name: "Clear filters" }).click();
      expectThat(
        hidden.length === 0 && csvHidden === 0,
        `hidden entries shown ${hidden.length}/${csvHidden}`,
      );
      expectThat(
        shown.length > 0 && csvShown > 0,
        `entries still hidden ${shown.length}/${csvShown}`,
      );
  ...
  await step(
    ...
    async () => {
      ...
      const out = [];
      for (const [u, a] of [
        ["/settings/audit-log", "/api/v1/audit-log"],
        ["/settings/audit-log/tool-calls", "/api/v1/audit-log/tool-calls"],
      ]) {
...
async function toolCallsSection(admin, browser, contexts) {
  ...
  try {
    await step(
      ...
      async () => {
        const needs =
          !policyBefore.enabled || !policyBefore.legalApiKeysEnabled || policyBefore.readOnly;
        if (needs) {
          const p = await api(admin, "PATCH", "/api/v1/mcp-settings", {
            enabled: true,
        ...
        const mod = await import(
          path.join(
            ROOT,
            "node_modules/.pnpm/@modelcontextprotocol+client@2.0.0/node_modules/@modelcontextprotocol/client/dist/index.mjs",
          )
        );
        client = new mod.Client({ name: clientName, version: "1" });
        await client.connect(
          new mod.StreamableHTTPClientTransport(new URL("/mcp", BASE), {
    ...
    await step(
      ...
      async () => {
        ...
        const rows = await until(
          ...
          "no Tool call row for the Client",
          20000,
        );
        await admin.getByRole("textbox", { name: "From" }).fill(browserToday(-1));
        await admin.getByRole("textbox", { name: "To" }).fill(browserToday(-1));
        ...
        expectThat(
          /Nadia Haddad/.test(text) &&
            /openlaw_whoami/.test(text) &&
            /(Success|Error|Refused|Pending)/.test(text) &&
            /\d+ ms/.test(text),
          `rows ${text}`,
        );
    ...
    await step(
      ...
      async () => {
        ...
        if (S.mcpMatterTitle) {
          ...
          expectThat(
            new RegExp(`Nadia Haddad, via ${escapeRe(clientName)}`).test(via),
            `activity ${q(rows.slice(0, 2))}`,
          );
        }
    ...
    await step(
      ...
      async () => {
        ...
        if (mine?.id)
          revoked = `POST /api/v1/api-key-requests/{id}/revoke answered ${(await api(admin, "POST", `/api/v1/api-key-requests/${mine.id}/revoke`, {})).status}`;
        const p = await api(admin, "PATCH", "/api/v1/mcp-settings", {
          enabled: policyBefore.enabled,
          legalApiKeysEnabled: policyBefore.legalApiKeysEnabled,
          readOnly: policyBefore.readOnly,
        });
        const now = (await api(admin, "GET", "/api/v1/mcp-settings")).body;
        expectThat(
          now.enabled === policyBefore.enabled &&
            now.legalApiKeysEnabled === policyBefore.legalApiKeysEnabled,
          `policy now ${now.enabled}/${now.legalApiKeysEnabled}`,
        );
...
async function autoDocSection(browser, contexts) {
  ...
  for (const [role, person] of [
    ["legal_team_member", PEOPLE.legal_team_member],
      ...
      async () => {
        ...
        if (role === "administrator")
          await page.screenshot({
            path: path.join(SHOTS, "v-c56-admin-template-pane.png"),
            fullPage: true,
          });
    ...
    await step(
      ...
      async () => {
        ...
        expectThat(
          listed.includes("Counterparty legal name") && controls.every((c) => c.endsWith(":1")),
          `controls ${q(controls)}; list ${q(listed)}`,
        );
    ...
    await step(
      ...
      async () => {
        ...
        expectThat(
          after !== before && /when/i.test(after + rowText),
          `block ${before} -> ${after}`,
        );
    ...
    await step(
      ...
      async () => {
        ...
        return `Refusals: ${Object.entries(out)
          ...
          .join(
            "; ",
          )}. The pane still reads ${q(p)}. Fixtures are fictional files built by fixtures/build-fixtures.py.`;
      },
    );
    ...
    await step(
      ...
      async () => {
        ...
        if (role === "administrator")
          await page.screenshot({ path: path.join(SHOTS, "v-c56-admin-publish-refusal.png") });
    ...
    await step(
      ...
      async () => {
        ...
        if (role === "legal_team_member")
          await page.screenshot({
            path: path.join(SHOTS, "v-c56-member-orphaned-field.png"),
            fullPage: true,
          });
    ...
    await step(
      ...
      async () => {
        ...
        const dialog = page.getByRole("dialog", { name: "Publish" });
        const files = (
          await dialog.getByLabel("File version").locator("option").allInnerTexts()
        ).map(flat);
        const forms = (
          await dialog.getByLabel("Form version").locator("option").allInnerTexts()
        ).map(flat);
        const caption = flat(
        ...
        const newer = flat(
          await page.getByText(/newer than the currently published/).textContent(),
        );
