...
- **Decision** —
  ...
  - **Per-person caps, counted from the database.** Two fills run at once per process (`MAX_CONCURRENT_FILLS`); later fills wait in arrival order. A person may hold five pending Generations (`MAX_PENDING_GENERATIONS_PER_PERSON`); a sixth answers 429. A person may submit twenty Requests an hour (`MAX_REQUESTS_PER_HOUR`) and attach 256 MiB an hour (`MAX_REQUEST_ATTACHMENT_BYTES_PER_HOUR`); either quota answers 429 with the number named. `request_attachments.byte_size` records each upload's length so the byte quota is a sum over the last hour. A per-person advisory lock serialises each count with its write, so a burst cannot read the same count and all pass.
