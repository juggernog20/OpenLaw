// DOC-030 compatibility replay (set compat), article compare-versions (V-C29). Copy of DOC-029/compat-r2/analysis-standin-r2/walkthrough-r2.mjs.
// DOC-030 changes: harness from ./compare-versions-replay/harness.mjs, lab manifest compatcmp (a separate lab, because the
// compare step pauses the worker, which the shared work2 rules forbid), default PARTS=compare,archived-pair (the V-C29 parts),
// output path, reviewer label, task, and record names DOC-029r2 to "DOC-030 compat", and providerMode (no stand-in; the compare parts use no AI). No step, selector or check changed.
// DOC-029 round 2 compatibility replay, group analysis-standin. Copy of analysis-standin/walkthrough-r1.mjs.
// Changed only: lab manifest path (analysis-r2), output name, reviewer and round labels, and record names (DOC-029r2).
...
//   mise exec -- node docs/documentation/batches/DOC-029/compat-r2/analysis-standin-r2/walkthrough-r2.mjs
// Optional: ROLES=administrator,legal_team_member PARTS=analysis,compare OUT=<path>
import { readFileSync, writeFileSync } from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import * as h from "./compare-versions-replay/harness.mjs";

const here = path.dirname(fileURLToPath(import.meta.url));
const OUT = process.env.OUT ?? path.join(here, "compare-versions-replay.json");
const ROLES = (process.env.ROLES ?? "administrator,legal_team_member").split(",");
const PARTS = (process.env.PARTS ?? "compare,archived-pair").split(",");
...
const labJson = JSON.parse(
  readFileSync(path.join(h.root, ".documentation-labs/compatcmp/lab.json"), "utf8"),
);
...
const { results, step } = h.makeRecorder({
  ...
  providerMode:
    "None. The compatcmp lab has AI disabled and no stand-in; the compare and archived-pair parts make no AI call.",
...
async function analysisSetup() {
  ...
  const type = await h.ok(
    admin.post("/contract-types", {
      displayName: `DOC-030 compat analysis-standin Analysis type ${stamp}`,
    }),
  );
  ...
  for (const [key, displayName, fieldType, prompt] of [
    [
      "location",
      `DOC-030 compat Service location ${stamp}`,
      "text",
      "Extract the city where the services are performed.",
    ],
    ["tier", `DOC-030 compat Service tier ${stamp}`, "number", "Extract the numeric service tier."],
    ["absent", `DOC-030 compat Audit clause ${stamp}`, "text", "Extract the audit clause summary."],
  ]) {
  ...
  const holder = await h.ok(
    admin.post("/contracts", {
      title: `DOC-030 compat analysis-standin Counterparty holder ${stamp}`,
      contractTypeId: analysisType,
    }),
  );
  supplierName = `DOC-030 compat Analysis Supplier ${stamp} Ltd`;
  await h.ok(
    admin.post(`/contracts/${holder.contract.number}/counterparties`, { name: supplierName }),
  );
  setupNote(
    `Administrator API: Contract type ${q(`DOC-030 compat analysis-standin Analysis type ${stamp}`)} with three prompted Fields (text location, number tier, text audit clause), and one live Counterparty ${q(supplierName)} created on holder Contract C-${holder.contract.number}.`,
  );
}
...
function paper(marker, version) {
  const v1 = version === 1;
  return [
    `DOC-030 compat fictional services agreement, reference ${marker}.`,
...
async function analysisWalk(role) {
  ...
  const mk = async (suffix) =>
    (
      await h.ok(
        api.post("/contracts", {
          title: `DOC-030 compat analysis-standin ${suffix} ${tag}`,
          contractTypeId: analysisType,
          managerId: null,
        }),
      )
    ...
    async () => {
      ...
      try {
        ...
        await page
          ...
          .fill("DOC-030 compat unsaved draft text");
      } finally {
        await h.standin("/control/pause", { paused: false });
      }
      ...
      const locationShown = await page
        ...
        .inputValue();
      h.expectThat(
        shown === "DOC-030 compat unsaved draft text" && locationShown === "Muscat",
        `draft ${q(shown)} location ${q(locationShown)}`,
      );
      return `C-${draftContract} (separate Contract): typed "DOC-030 compat unsaved draft text" into ${fieldSlugs.absent.label} without leaving the control while the card read Running…. When the card updated to ${q(status)}, the typed draft was still in the control and the written ${fieldSlugs.location.label} showed ${q(locationShown)}.`;
    },
  );
  ...
  await step(
    ...
    async () => {
      ...
      await editKd("Price review", async (dialog) => {
        await dialog.getByLabel(/^Note/).fill("DOC-030 compat note only");
      });
      await h.sleep(1000);
  ...
  await step(
    ...
    async () => {
      ...
      await page.getByRole("menuitem", { name: "Edit date" }).click();
      const dialog = page.getByRole("dialog");
      await dialog.getByLabel(/^Note/).fill("DOC-030 compat check the option notice");
      await dialog.getByRole("button", { name: /^Save/ }).click();
...
async function conversionWalk(role) {
  ...
  await step(
    ...
    async () => {
      ...
      const needle = `DOC-030 compat Portal Supplier ${short(role)} ${stamp} GmbH`;
      const submitted = await h.ok(
        sessions.business_user.api.post("/requests", {
          requestTypeId: legalQuestion.id,
          departmentId: departments[0].id,
          title: `DOC-030 compat analysis-standin Request ${tag}`,
          description: `Please review the fictional services agreement with ${needle}. It is a DOC-030 compat walkthrough record.`,
          urgency: "low",
        }),
      );
      const request = {
      ...
      let filling;
      try {
        const body = { title: `DOC-030 compat analysis-standin Converted ${tag}` };
...
async function archivedNewPairWalk(role) {
  ...
  const number = (
    await h.ok(
      api.post("/contracts", {
        title: `DOC-030 compat analysis-standin Archived pair ${short(role)} ${stamp}`,
  ...
  await h.ok(
    api.upload(`/contracts/${number}/documents`, {
      ...
      buffer: Buffer.from("DOC-030 compat download-only primary placeholder"),
    }),
  );
  let doc = (
    await h.ok(
...
async function compareWalk(role) {
  ...
  const number = (
    await h.ok(
      api.post("/contracts", {
        title: `DOC-030 compat analysis-standin Compare ${tag}`,
        contractTypeId: otherType.id,
      }),
    )
  ).contract.number;
  ...
  const para = (days, cadence) => [
    "DOC-030 compat fictional payment terms.",
