...
const { log, save, step: logStep } = createLog("browser", { lab: LAB.name, publicOrigin: PUBLIC, chromiumHostResolverRule: `MAP ${HOST} ${ingress[0]}` });
const secrets = [PASSWORD];
const S = (role, page, extra = {}) => ({ article: "connect-claude", scenario: "V-M41-C59", role, page, secrets, ...extra });
const SA = (page) => ({ article: "configure-mcp", scenario: "V-M41-C59 prerequisite (configure-mcp Enable OAuth Clients)", role: "administrator", page, secrets });
const main = async (page) => flat(await page.locator("main").innerText().catch(async () => page.locator("body").innerText()));
  ...
  return logStep(sel, name, expected, async (entry) => {
    for (;;) {
      ...
      try {
        ...
        return attempts > 1 ? `${out} (attempt ${attempts}; earlier attempts hit net::ERR_NETWORK_CHANGED in Chromium on this host)` : out;
      } catch (error) {
...
async function setSwitch(label, want) {
  ...
  const resp = admin.waitForResponse((r) => /\/api\/v1\/mcp-settings(\/allowed-clients\/[^/]+)?$/.test(new URL(r.url()).pathname) && r.request().method() === "PATCH");
  await sw.click();
...
await step(SA("/settings/mcp"), "Sign in as Daniel Okafor at the public origin; Settings → Organization → MCP; read Server address", `Server address ${MCP_URL}`, async () => {
  await adminSignIn();
  await gotoMcp();
...
let pill = "";
await step(SA("/settings/mcp"), "Enable OAuth Clients 1-3: Enable MCP on; Legal Users and Business Users OAuth Clients on; read the pill beside Server address", "Switches save 200; pill Reachable", async () => {
  const pillBefore = await admin.getByRole("status").filter({ hasText: /Not reachable|Reachable/ }).count();
  ...
  await pause(500);
  pill = flat(await admin.getByRole("status").filter({ hasText: /Not reachable|Reachable/ }).innerText());
  ...
  const apiView = sh("docker", ["--context", "default", "exec", `${LAB.project}-app-1`, "node", "-e", `require("dns").lookup("${HOST}",{all:true},(e,a)=>console.log(JSON.stringify(a)))`]).out.trim();
  const shared = /"100\.(6[4-9]|[7-9]\d|1[01]\d|12[0-7])\./.test(apiView);
  expectThat([mcp, legal, business].every((s) => s === 200 || s === "unchanged") && (pill === "Reachable" || (pill === "Not reachable · Public IPv4 address" && shared)), JSON.stringify({ mcp, legal, business, pill, apiView }));
  await admin.screenshot({ path: new URL("./settings-mcp-pill.png", import.meta.url).pathname, fullPage: false });
...
await step(SA("/settings/mcp"), "Enable OAuth Clients 4: Allowed Clients card; Claude and Claude Code enabled", "Enable Claude and Enable Claude Code aria-checked true; both Published identity", async () => {
  await expandCard("Allowed Clients");
  const rows = {};
  for (const n of ["Claude", "Claude Code", "ChatGPT", "Microsoft 365 Copilot"]) {
    rows[n] = await admin.getByRole("switch", { name: `Enable ${n}`, exact: true }).getAttribute("aria-checked");
  }
  ...
  expectThat(rows.Claude === "true" && rows["Claude Code"] === "true" && claude.metadataUrl === CLAUDE.clientId && code.metadataUrl === CLAUDE_CODE.clientId, JSON.stringify({ rows, claude, code }));
...
const discovery = await (await fetch(`${LAB.loopback}/.well-known/oauth-authorization-server`, { headers: { host: `${HOST}:8443` } })).json();
...
async function readConsent(page) {
  ...
  const radios = await page.getByRole("radio").evaluateAll((els) => els.map((e) => e.closest("label")?.textContent?.trim().replace(/\s+/g, " ")));
...
for (const role of ["administrator", "legal_team_member"]) {
  await step(S(role, "/auth/consent"), `Consent 1-5 as ${PEOPLE[role].name}: start from Claude's authorization request signed out; sign in with password; back on the consent page; read it; Allow gate; do not Allow`, "Login page first; consent after sign-in; heading Claude wants to work in OpenLaw as you; Published identity https://claude.ai/oauth/mcp-oauth-client-metadata; Allowed Client pill; name, account type, email; What Claude may use; Nothing is selected for you.; How far Claude may go; Read only and Read and write; no Guide checkbox; Allow disabled until a Toolset and a scope", async () => {
    ...
    const checks = {
      ...
      person: c.text.includes(PEOPLE[role].name) && c.text.includes(PEOPLE[role].email) && new RegExp(role === "administrator" ? "Administrator" : "Legal team member", "i").test(c.text),
      ...
      radios: c.radios.some((r) => /^Read only/.test(r)) && c.radios.some((r) => /^Read and write/.test(r)),
    ...
    await p.screenshot({ path: new URL(`./consent-claude-${role}.png`, import.meta.url).pathname });
    expectThat(Object.values(checks).every(Boolean), JSON.stringify({ checks, c: { ...c, text: c.text.slice(0, 600) }, gate }));
...
await step(S("business_user", "/auth/consent"), "Consent 1-5 as Jonas Weber: start signed out; select Business Portal sign-in; request a magic link; open it; back on the consent page; read it; Allow gate; do not Allow", "Business Portal sign-in keeps the query; the magic link returns to consent; Business User account type; no Administration; Allow gate holds", async () => {
  const p = await newPage();
  ...
  expectThat(link && link.startsWith(PUBLIC), `magic link origin ${link ? new URL(link).origin : "none"}`);
  await p.goto(link);
  const c = await readConsent(p);
  ...
  await p.screenshot({ path: new URL("./consent-claude-business_user.png", import.meta.url).pathname });
  expectThat(Object.values(checks).every(Boolean), JSON.stringify({ checks, text: c.text.slice(0, 600), gate }));
...
await step(S("legal_team_member", "/auth/consent"), "Claude Code: an authorization request with Claude Code's published identity and a loopback callback reaches consent with the Claude Code name", "Heading Claude Code wants to work in OpenLaw as you; legends use Claude Code; Published identity https://claude.ai/oauth/claude-code-client-metadata", async () => {
  const p = pages.legal_team_member;
  await p.goto(authorizeUrl(CLAUDE_CODE, "http://localhost:53682/callback"));
  const c = await readConsent(p);
  const ok = c.heading === "Claude Code wants to work in OpenLaw as you" && c.text.includes("What Claude Code may use") && c.text.includes("How far Claude Code may go") && c.text.includes(`Published identity ${CLAUDE_CODE.clientId}`) && c.allowDisabled;
...
// ---------- negative checks ----------
await step(S("legal_team_member", "/auth/consent"), "Negative: Read and write is absent when the organization is read-only (Daniel sets Read-only, Nadia starts a fresh request, Daniel puts it back)", "Only Read only offered under Read-only; both again after", async () => {
  await gotoMcp();
  ...
  await p.goto(authorizeUrl(CLAUDE, CLAUDE.redirect, discovery.scopes_supported.filter((s) => s !== "write").join(" ")));
  const noWrite = await readConsent(p);
  expectThat(ro.radios.length === 1 && /^Read only/.test(ro.radios[0]) && back.radios.some((r) => /^Read and write/.test(r)) && noWrite.radios.length === 1, JSON.stringify({ ro: ro.radios, back: back.radios, noWrite: noWrite.radios }));
...
await step(S("legal_team_member", "/auth/consent"), "Negative: an altered consent link shows the expired-or-changed message with no actions", "This consent request has expired or changed. Start again from your Client.; no Allow, no Deny", async () => {
  const p = pages.legal_team_member;
  await p.goto(authorizeUrl(CLAUDE, CLAUDE.redirect));
  await readConsent(p);
  ...
  const c = await readConsent(p);
  expectThat(c.text.includes("This consent request has expired or changed. Start again from your Client.") && c.allowCount === 0 && c.denyCount === 0, JSON.stringify({ ...c, text: c.text.slice(0, 400) }));
  return `Changed the scope parameter of a live consent URL. Page: "${c.text.slice(0, 300)}". Allow ${c.allowCount}, Deny ${c.denyCount}.`;
});

await step(S("legal_team_member", "/settings/mcp + /auth/consent"), "Negative: disabled Client. Mid-flow: Daniel turns Claude off, Nadia reloads consent. New flow: a fresh request with Claude off. Daniel turns Claude back on", "Mid-flow: This Client is off in the organization's Allowed Clients list., Deny alone. New flow: HTTP 400 Client is not on the enabled Allowed Clients list. before any sign-in page", async () => {
  const p = pages.legal_team_member;
  await p.goto(authorizeUrl(CLAUDE, CLAUDE.redirect));
  await readConsent(p);
  await gotoMcp();
  ...
  const ok = mid.text.includes("This Client is off in the organization's Allowed Clients list.") && mid.allowCount === 0 && mid.denyCount === 1 && freshStatus === 400 && freshText.includes("Client is not on the enabled Allowed Clients list.");
  expectThat(ok, JSON.stringify({ mid: mid.text.slice(0, 400), a: mid.allowCount, d: mid.denyCount, freshStatus, freshText }));
...
await step(S("business_user", "/settings/mcp + /auth/consent"), "Negative: disabled account group. Daniel turns Business Users OAuth Clients off; Jonas reloads consent; Daniel turns it back on", "OAuth Clients are off for your account type.; Deny alone", async () => {
  ...
  expectThat(c.text.includes("OAuth Clients are off for your account type.") && c.allowCount === 0 && c.denyCount === 1, JSON.stringify({ text: c.text.slice(0, 400), a: c.allowCount, d: c.denyCount }));
...
await step(S("legal_team_member", "/settings/mcp + /auth/consent"), "Negative: disabled account group for Legal Users. Daniel turns Legal Users OAuth Clients off; Nadia reloads consent; back on", "OAuth Clients are off for your account type.; Deny alone", async () => {
  const p = pages.legal_team_member;
  await p.goto(authorizeUrl(CLAUDE, CLAUDE.redirect));
  await readConsent(p);
...
await step(S("administrator", "/settings/mcp + /auth/consent"), "Refusal table: MCP off. Daniel opens a consent page, turns MCP off, reloads; turns MCP back on", "MCP is off for this organization.; Deny alone", async () => {
  ...
  const pillAfter = flat(await admin.getByRole("status").filter({ hasText: /Not reachable|Reachable/ }).innerText());
  expectThat(c.text.includes("MCP is off for this organization.") && c.allowCount === 0 && c.denyCount === 1 && pillAfter === pill, JSON.stringify({ text: c.text.slice(0, 400), a: c.allowCount, d: c.denyCount, pillAfter }));
...
for (const role of ["administrator", "legal_team_member", "business_user"]) {
  await step(S(role, "/auth/consent"), `Negative: an expired consent link (the first consent URL for ${PEOPLE[role].name}, reopened more than ten minutes later)`, "This consent request has expired or changed. Start again from your Client.; no Allow, no Deny", async () => {
    ...
    const c = await readConsent(p);
    expectThat(c.text.includes("This consent request has expired or changed. Start again from your Client.") && c.allowCount === 0 && c.denyCount === 0, JSON.stringify({ text: c.text.slice(0, 400), a: c.allowCount, d: c.denyCount }));
...
await step(SA("/settings/mcp"), "State after the checks: MCP on, both OAuth Clients switches on, Read-only off, Claude and Claude Code enabled; no grants exist", "Settings restored; zero OAuth grants", async () => {
  ...
  const flags = { mcpEnabled: s.mcpEnabled ?? s.enabled, legal: s.mcpLegalOAuthClientsEnabled ?? s.legalOAuthClientsEnabled, business: s.mcpBusinessOAuthClientsEnabled ?? s.businessOAuthClientsEnabled, readOnly: s.mcpReadOnly ?? s.readOnly };
  expectThat(pick("Claude") && pick("Claude Code") && grants.length === 0, JSON.stringify({ flags, grants: grants.length }));
...
console.log(`browser done: ${mine.length} steps, ${mine.filter((s) => s.result !== "pass").length} failed; pill "${pill}"`);
void sh;
