...
At a content width below 960px, reflow each Row into name/actions above labelled switches. The tree remains one tree with its left rules; omit the shared heading strip once every switch has its visible label. Condition controls wrap in Row, Operator, Value order with the same 8px gap. Deep nesting must not reduce a control below its usable width; allow the tree region to scroll horizontally when indentation alone exceeds that space. The page itself must not gain horizontal scrolling. Footer buttons wrap with 8px gaps. The 960px threshold and the 320px alternative inspector use the existing spacing scale; they introduce no named token. Use content-container queries, not another viewport breakpoint. Warm and Dark use the same semantic tokens and dimensions.
