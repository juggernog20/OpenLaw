// DOC-029 independent re-walk of electronic-signing.md (V-C17-electronic) by the DOC-029 independent
// re-walk agent. Based on signing-standin/walkthrough-r1.mjs, replayed in full on a new lab "sign-rw",
// with added checks for the Before you start /sig/ paragraph (Version 1 carries /sig/ text; the
...
const lab = JSON.parse(readFileSync(path.join(root, ".documentation-labs/sign-rw/lab.json"), "utf8"));
...
const OUT = path.join(here, `walkthrough-rw-${PHASE}.raw.json`);
...
function fictionalPdf(label, marks = "") {
  return Buffer.from(`%PDF-1.4\n% DOC-029 signing-standin fictional paper: ${label}${marks}\n1 0 obj << >> endobj\n%%EOF\n`);
}
...
async function main(browser) {
  ...
  for (const flow of flows) {
    ...
    await step(role, sender.person.name, "Decline: a provider-reported Declined round records its reason, keeps the Voided round in history, files nothing, does not end or archive the Contract, and allows another send", async () => {
      ...
      await openApprovals(page, number);
      const sent = await sendEnvelope(page, number, { signers: signersFor(role, "r2"), subject: `DOC-029 signing-standin ${role} decline round` });
    ...
    await step(role, `${sender.person.name}, ${flow.rival.person.name}`, "If another sender won: with both dialogs open, the second send is refused in the dialog, and reloading shows the winner's live round instead of creating another", async () => {
      ...
      const sent = await sendEnvelope(page, number, { signers: signersFor(role, "r3"), subject: `DOC-029 signing-standin ${role} completion round` });
