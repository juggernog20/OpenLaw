...
| `MCP_RATE_LIMIT_PER_HOUR`       | no       | MCP calls per hour per credential (DD-029, SET-014). Defaults to 600. A set value pins the Settings → Advanced key. Nothing enforces the limit until the MCP server ships.                                                                                                                                       |
