...
function writeCounter(page, pattern) {
  ...
  page.on("request", (request) => {
    if (["PUT", "POST", "DELETE", "PATCH"].includes(request.method()) && pattern.test(request.url()))
      seen.push(`${request.method()} ${new URL(request.url()).pathname}`);
  });
  return seen;
}
const invalidFields = (page, ids) =>
  page.evaluate((list) => list.filter((id) => document.getElementById(id)?.validity.valueMissing), ids);
...
try {
  ...
  const api = adminContext.request;
  {
    ...
    if (ai.connector.configured)
      throw new Error("An AI connector already exists in this lab. These checks need the unconfigured state.");
  }
  const aiWrites = writeCounter(page, /\/api\/v1\/ai-connector/);

  await step("A01", "administrator", "browser-walkthrough", "Open Settings > Organization > AI analysis; check the page before a connector is saved", async () => {
    await page.goto(`${BASE}/settings`);
    const railLink = page.locator('nav a[href="/settings/ai-analysis"]');
    await railLink.waitFor({ timeout: 20000 });
    const group = await page.getByRole("group", { name: "Organization" }).locator('a[href="/settings/ai-analysis"]').count();
    ...
    const disclosure = await page.getByRole("heading", { name: "Field prompts" }).getByRole("button").count();
    const conversion = await page.getByRole("heading", { name: "Request conversion" }).count();
    const link = await page.getByRole("link", { name: "Contracts → Fields" }).getAttribute("href");
    const order = (await page.locator("h2").allInnerTexts()).map((t) => t.trim());
    expect(
      (await railLink.innerText()).trim() === "AI analysis" && group === 1 && prompts === 7 && disclosure === 0 && conversion === 0 && link === "/settings/contracts/fields",
  ...
  await step("A02", "administrator", "browser-walkthrough", "Expand Provider; check presets, the default form and the Provider card intro", async () => {
    ...
    const keyRequired = await page.getByText("Required for this provider. The key is write-only and encrypted at rest.").isVisible();
    ...
    const intro = await page.getByText(/^Connect your AI provider/).first().innerText().catch(() => "(intro not found)");
    const expected = ["Anthropic", "OpenAI", "Azure OpenAI", "Gemini", "OpenRouter", "Ollama", "Custom endpoint"];
    expect(
      expected.every((o) => options.includes(o)) && options.length === 7 && preset === "anthropic" && keyRequired && loadDisabled && testDisabled && loadEnabledWithKey,
  ...
  await step("A03", "administrator", "browser-walkthrough", "Select Save connector with no API key", async () => {
  ...
  await step("A04", "administrator", "browser-walkthrough", "Check the OpenRouter, Azure OpenAI, Ollama and Custom endpoint forms", async () => {
    const facts = [];
    await page.locator("#ai-preset").selectOption("openrouter");
    facts.push(`OpenRouter: model "${await page.locator("#ai-model").inputValue()}", Base URL shown=${(await page.locator("#ai-base-url").count()) > 0}, Load models shown=${await page.getByRole("button", { name: "Load models" }).isVisible()}`);
    ...
    facts.push(`Azure OpenAI: Deployment endpoint=${azureLabel}, manual model input=${azureManual}, Load models buttons=${azureLoad}`);
    await page.locator("#ai-preset").selectOption("ollama");
    const ollamaOptional = await page.getByText("Ollama does not require an API key.").isVisible();
    ...
    facts.push(`Custom endpoint: Protocol options=${protocols.join(" / ")}, Base URL shown=${(await page.locator("#ai-base-url").count()) > 0}`);
    expect(
      azureLabel && azureManual && azureLoad === 0 && ollamaOptional && !ollamaBase &&
        ["Anthropic Messages", "OpenAI-compatible chat completions", "Gemini"].every((p) => protocols.includes(p)),
      facts.join("; "),
    );
    return facts.join(". ");
  });

  await step("A05", "administrator", "browser-walkthrough", "Custom endpoint: save a Base URL with embedded credentials", async () => {
  ...
  await step("A06", "administrator", "automated-test", "API refusals for incomplete or invalid AI saves, and Test with no connector", async () => {
    const put = (data) => api.put(`${BASE}/api/v1/ai-connector`, { data });
    const noKey = await put({ preset: "openrouter", model: "x" });
    const ftp = await put({ preset: "custom", protocol: "openai_chat_completions", baseUrl: "ftp://127.0.0.1/v1", apiKey: FICTION.aiKey, model: "x" });
    const noModel = await put({ preset: "custom", protocol: "openai_chat_completions", baseUrl: FICTION.closedEndpoint, apiKey: FICTION.aiKey, model: " " });
    const azureList = await api.post(`${BASE}/api/v1/ai-connector/models`, { data: { preset: "azure_openai", baseUrl: "https://example.test/x", apiKey: FICTION.aiKey } });
    const test = await api.post(`${BASE}/api/v1/ai-connector/test`);
    const detail = async (r) => `${r.status()} "${(await r.json().catch(() => ({}))).detail ?? "validation"}"`;
    const out = [
    ...
    const state = await (await api.get(`${BASE}/api/v1/ai-connector`)).json();
    expect([noKey, ftp, noModel, azureList, test].every((r) => r.status() === 400) && !state.connector.configured, out.join("; "));
    return `${out.join(". ")}. No connector was created.`;
  });

  await step("A07", "administrator", "browser-walkthrough", "Load models against a closed loopback endpoint (Model list cannot be loaded)", async () => {
    await page.reload();
    await page.getByRole("button", { name: "Provider", exact: true }).click();
    await page.locator("#ai-preset").selectOption("custom");
    ...
    const manual = await page.getByRole("button", { name: "Enter model ID manually" }).isVisible();
    const state = await (await api.get(`${BASE}/api/v1/ai-connector`)).json();
    expect(!state.connector.configured && manual, `configured=${state.connector.configured} manual=${manual}`);
  ...
  await step("A08", "administrator", "browser-walkthrough", "Save the fictional Custom endpoint; check Connected, the switch, Request conversion; Test connection fails; turn off at once", async () => {
    await page.getByRole("button", { name: "Enter model ID manually" }).click();
    await page.locator("#ai-model").fill(FICTION.model);
    await page.getByRole("button", { name: "Save connector" }).click();
    ...
    for (const id of ["matter-preparation", "contract-preparation", "contract-conversion-analysis"])
      switches.push(`${id}: checked=${await page.locator(`#${id}`).getAttribute("aria-checked")}, enabled=${await page.locator(`#${id}`).isEnabled()}`);
    const order = (await page.locator("h2").allInnerTexts()).map((t) => t.trim());
    ...
    expect(chip && state.connector.enabled && switches.every((s) => s.includes("checked=false")), `chip=${chip} enabled=${state.connector.enabled} ${switches}`);
  ...
  await step("A09", "administrator", "browser-walkthrough", "Turned off: Test connection and switches disabled; save a change while off; API refusals", async () => {
    await page.reload();
    await page.getByRole("button", { name: "Provider", exact: true }).click();
    const hint = await page.getByText(/^Off since /).innerText().catch(() => "(no Off since hint)");
    const testDisabled = await page.getByRole("button", { name: "Test connection" }).isDisabled();
    const switchesDisabled = [];
    for (const id of ["matter-preparation", "contract-preparation", "contract-conversion-analysis"])
      switchesDisabled.push(await page.locator(`#${id}`).isDisabled());
    const keep = await page.getByText("Leave blank to keep the current key. Paste a new one to rotate.").isVisible();
    const keyBlank = (await page.locator("#ai-api-key").inputValue()) === "";
    await page.getByRole("button", { name: "Enter model ID manually" }).click().catch(() => undefined);
    ...
    expect(
      testDisabled && switchesDisabled.every(Boolean) && keep && keyBlank && !state.connector.enabled && state.connector.model.endsWith("-2") && state.connector.hasApiKey !== false,
  ...
  await step("A10", "administrator", "browser-walkthrough", "Changed destination requires a new API key", async () => {
    await page.locator("#ai-base-url").fill("http://127.0.0.1:10/v1");
    const required = await page.getByText("Required for this provider. The key is write-only and encrypted at rest.").isVisible();
    const before = aiWrites.length;
    await page.getByRole("button", { name: "Save connector" }).click();
    await wait(800);
  ...
  await step("A11", "administrator", "browser-walkthrough", "Field prompt: Enter saves, Shift+Enter inserts a line break, clearing restores, Reset to default, maxlength", async () => {
    await page.reload();
    ...
    expect(
      withBreak === "DOC-029r2 fictional prompt\nsecond line" && savedText && afterClear === withBreak && afterReset === original && maxLength === "2000",
  ...
  await step("A12", "administrator", "browser-walkthrough", "Known issue #887 re-check: Escape in a Field prompt (the article makes no Escape claim)", async () => {
    const box = page.locator("textarea[id^=ai-field-prompt-]").first();
    const id = await box.getAttribute("id");
    const original = await box.inputValue();
    await box.click();
    ...
    if (stored) {
      ...
      await page.getByRole("button", { name: /^Reset .* to default$/ }).first().waitFor({ state: "detached", timeout: 10000 });
    }
  ...
  await step("A13", "administrator", "browser-walkthrough", "Follow Contracts → Fields; open Add field and find Scope and AI prompt (cancel without saving)", async () => {
  ...
  await step("A14", "administrator", "browser-walkthrough", "Turn Use AI analysis on and off again; Remove connector", async () => {
    ...
    const conversion = await page.getByRole("heading", { name: "Request conversion" }).count();
    const state = await (await api.get(`${BASE}/api/v1/ai-connector`)).json();
    expect(testEnabled && !state.connector.configured && conversion === 0, `test=${testEnabled} configured=${state.connector.configured} conversion=${conversion}`);
  ...
  await step("R01", "legal_team_member", "browser-walkthrough", "Open AI analysis settings as a Legal Team Member (Settings is unavailable row)", async () => {
    ...
    const prompts = await memberContext.request.put(`${BASE}/api/v1/ai-field-prompts`, { data: { slug: "term_type", prompt: null } });
    expect(organization === 0 && ai.status() === 403 && promptsRead.status() === 403 && prompts.status() === 403, `org=${organization} ai=${ai.status()} promptsRead=${promptsRead.status()} prompts=${prompts.status()}`);
  ...
  await step("OB01", "operator", "browser-walkthrough", "Signed out, open /documentation/configure-analysis and follow the deployment configuration link", async () => {
    ...
    await op.getByRole("heading", { name: "Preserve and rotate encryption keys" }).waitFor({ timeout: 20000 });
  ...
  await step("O01", "operator", "container-operation", "App and worker share OPENLAW_SECRET_KEY; no provider credential or proxy in the environment", async () => {
    const envOf = (svc) => {
      const out = docker(["inspect", "--format", "{{range .Config.Env}}{{println .}}{{end}}", `${PROJECT}-${svc}-1`]);
      return Object.fromEntries(out.split("\n").filter(Boolean).map((l) => [l.slice(0, l.indexOf("=")), l.slice(l.indexOf("=") + 1)]));
    };
    const app = envOf("app");
    const worker = envOf("worker");
    const h = (v) => createHash("sha256").update(v ?? "").digest("hex");
    const same = !!app.OPENLAW_SECRET_KEY && h(app.OPENLAW_SECRET_KEY) === h(worker.OPENLAW_SECRET_KEY);
    const credNames = [...Object.keys(app), ...Object.keys(worker)].filter((n) => /^(AI_|OPENROUTER|OPENAI|ANTHROPIC|GEMINI|AZURE)/i.test(n));
  ...
  await step("O02", "operator", "container-operation", "App and worker logs contain no fictional key or bearer text", async () => {
    const logs = (svc) => {
      try {
        return execFileSync("docker", ["logs", "--since", log.startedAt, `${PROJECT}-${svc}-1`], { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"], maxBuffer: 64 * 1024 * 1024 });
      } catch (e) {
    ...
    for (const svc of ["app", "worker"]) {
      const text = logs(svc);
      counts[svc] = { lines: text.split("\n").length, fictionalKey: text.split(FICTION.aiKey).length - 1, bearer: (text.match(/bearer /gi) ?? []).length };
    }
    expect(counts.app.fictionalKey === 0 && counts.worker.fictionalKey === 0 && counts.app.bearer === 0 && counts.worker.bearer === 0, JSON.stringify(counts));
