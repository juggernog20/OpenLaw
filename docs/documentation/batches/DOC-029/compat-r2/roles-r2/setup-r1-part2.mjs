// Second fixture pass for the DOC-029r2 roles walkthrough (round 1): the Portal paging and Contributor-guide records.
...
async function upload(page, module, number, name, text) {
  ...
  if (res.status() >= 300) throw new Error(`upload ${name}: ${res.status()} ${JSON.stringify(json).slice(0, 300)}`);
...
async function matterFor(page, body) {
  const found = must(await api(page, "GET", `/matters?q=${encodeURIComponent(body.title)}`), "find").matters.find((m) => m.title === body.title);
...
const mr = await matterFor(n.page, { title: "DOC-029r2 roles Ravi matter", matterTypeId: commercial, managerId: fx.ids.nadia, departmentId: sales, region: "EMEA", description: "DOC-029r2 roles fictional Matter description." });
fx.raviMatter = { number: mr.number, id: mr.id };

fx.pagingMatters = [];
for (let i = 1; i <= 25; i++) {
  const m = await matterFor(n.page, { title: `DOC-029r2 roles paging matter ${String(i).padStart(2, "0")}`, matterTypeId: commercial, managerId: fx.ids.nadia });
...
const dm = must(await api(n.page, "POST", "/matters", { title: "DOC-029r2 roles document paging matter", matterTypeId: commercial, managerId: fx.ids.nadia }), "DM").matter;
must(await api(n.page, "POST", `/matters/${dm.number}/team`, { userId: fx.ids.ravi }), "DM team");
fx.docPagingMatter = { number: dm.number, id: dm.id };
for (let i = 1; i <= 51; i++) await upload(n.page, "matters", dm.number, `doc029-roles-page-${String(i).padStart(2, "0")}.txt`, `DOC-029r2 roles fictional paging paper ${i}.\n`);

// Primary Document on Ravi's converted Contract, uploaded by Legal.
const primary = await upload(n.page, "contracts", fx.raviRequest.converted.convertedRecord.number, "doc029-roles-supporting-v1.txt", readFileSync(path.join(here, "fixtures", "doc029-roles-supporting-v1.txt"), "utf8"));
fx.raviPrimary = { id: primary.id, v1: primary.versions[0].id };
writeFileSync(OUT, JSON.stringify(fx, null, 2));
console.log(JSON.stringify({ raviMatter: fx.raviMatter, dm: fx.docPagingMatter, paging: fx.pagingMatters.map((m) => m.number), primary: fx.raviPrimary }));
await browser.close();
