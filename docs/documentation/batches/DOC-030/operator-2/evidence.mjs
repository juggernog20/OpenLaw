...
const sha = (id) =>
  createHash("sha256")
    .update(readFileSync(`docs/user-guides/${id}.md`))
    .digest("hex");
...
const last = (article) =>
  r2
    .filter((s) => s.article === article && s.result === "pass")
    .map((s) => s.at)
    .sort()
    .pop();
...
const prev = (id) => {
  ...
  return e.previousEvidence && e.appCommit === COMMIT
    ? e.previousEvidence
    : { appCommit: e.appCommit, contentSha256: e.contentSha256, verifiedAt: e.verifiedAt };
};
const reviewSources = (id) => review.articles.find((a) => a.articleId === id)?.sources ?? [];
const author = {
  install:
    "Codex documentation author; corrections by DOC-029 and DOC-029r2 source-review author agents (operator-install); DOC-030 corrections by DOC-030 source-review author agent (shared-files) and DOC-030 source-review author agent (operator-2)",
  upgrade:
    "Codex documentation author; corrections by DOC-029 and DOC-029r2 source-review author agents (operator-lifecycle); DOC-030 corrections by DOC-030 source-review author agent (operator-2)",
  "backup-and-restore":
    "Codex documentation author; corrections by DOC-029 source-review author agent (operator-lifecycle); DOC-030 corrections by DOC-030 source-review author agent (operator-2)",
...
function record(
  ...
  scenario,
  coverage,
  { buildId, environment, prerequisites, expected, actual, limitations },
) {
  return {
    articleId: id,
    ...
    sources: [
      ...new Set([
        ...
        "docs/user-guides/deployment-configuration.md",
        `${dir}/technical-review.json`,
        ...logRef,
        ...reviewSources(id).filter((s) => !s.startsWith(`docs/user-guides/${id}.md`)),
      ]),
    ],
    scenarios: [
      {
...
const out = {
  ...
  upgrade: record("upgrade", "V-C46", "C46", {
    ...
    environment:
      "openlaw-doc030-opup2 behind a same-host Caddy proxy at http://127.0.0.1:24651 (round 2); openlaw-doc030-opup and openlaw-doc030-oprecover (round 1)",
    prerequisites: [
      "Starting build d1d098ba installed with install.md's files and populated through the API: Administrator, Legal Team Member, Contributor, Entity, a Contract Field and a global Field on a Contract type and a Matter type, three Contracts (one Confidential), a Matter, two Documents with two Versions each, a saved relay with a password and an AI connector key.",
      "Target 067c1646 selected by the guide; pre-upgrade coherent backup retained.",
    ],
    expected:
      "Accounts, permissions, records, files and required encrypted configuration survive the supported upgrade; recovery is a restore of the pre-upgrade backup into a separate target, not a downgrade.",
    actual:
      "Round 2 on openlaw-doc030-opup2 (starting build published 0.0.0.0:24650): recorded revision, images, project, COMPOSE_FILE and local storage; Settings → Advanced does not exist on the starting build. git status/fetch/rev-parse; git checkout --detach 067c1646 and build without changing the 4 running containers; app and worker resolve to the same image. Step 3: port 127.0.0.1:24650; docker network inspect read gateway 192.168.64.1 before the upgrade and TRUSTED_PROXIES was set to it; default ceilings; VAPID unset. Outstanding jobs checked; backup steps 1 to 4 with app and worker kept stopped. Start: the first migration line was 'migrations: reconciled 0090_onboarding_reviewed_types; continuing with pending migrations'; the worker restarted once on the missing advanced_settings column and stayed running; port 127.0.0.1:24650; readiness in 6 s; journal 91 rows before. Step 2: the guide's remoteAddress command counted 437 requests from 127.0.0.1 and none from the gateway, and wrong passwords from 127.0.0.2 did not refuse a correct sign-in from 127.0.0.1. Administrator and Legal Team Member signed in in the browser; C-1 to C-3, M-1 and the Entity matched; the Legal Team Member and the former Contributor (now Business User) were refused Confidential C-3 (404 and 403). The global Field became one Contract Field (original slug) and one Matter copy with the same name and values. Four Version hashes matched; a new upload was processed; Send test email delivered through the saved relay; Test connection said 'Connection successful.'; System status Refresh showed API and Worker Running and Current. An old tab navigated without the reload notice. Round 1 recovery: the pre-upgrade backup restored into openlaw-doc030-oprecover on d1d098ba matched hashes and access, and a post-backup upload was absent.",
    limitations: [
      "The starting build has no business_owner_id column and no Request type intake questions, so the Business Owner backfill and the intake-to-Form move had no data to rewrite; Document types on Versions and the AI Saved key move were observed.",
      "An old browser tab did not show 'This part of OpenLaw was updated. Reload to continue.'; the guide says it can.",
      "Immediately after a restart System status can list a stale API row for up to a minute (product bug in the log).",
    ],
  }),
  "backup-and-restore": record("backup-and-restore", "V-C47", "C47", {
    ...
    environment:
      "openlaw-doc030-opup2 (source) and openlaw-doc030-oprestore2 (target), round 2; openlaw-doc030-opup and openlaw-doc030-oprestore, round 1",
    prerequisites: [
      "Populated source after the V-C46 upgrade with File uploads 150 and S3 Document storage saved in Settings → Advanced, a Document stored only in the owned MinIO bucket, a sealed relay password, AI key and generated VAPID pair.",
      "Retained AUTH_SECRET and OPENLAW_SECRET_KEY in a private secret store; a separate empty target project.",
    ],
    expected:
      "Records and permissions work, file hashes match, and required encrypted configuration is usable with the retained key; missing or wrong keys and missing files are detected; an archive alone is not a restore.",
    actual:
    ...
    limitations: [
      "The backup used the bundled Postgres and local volume; object-store snapshots were not taken. The restored target read the source's live MinIO bucket because its .env left the storage keys empty, which the guide warns about.",
      "No two-factor enrollment, Signing connector, SSO provider or browser push subscription existed in the fixture; their restore symptoms were not observed.",
      "The 'Emailed links or sign-in go to the source's address' row was not exercised because the source's BASE_URL was pinned.",
    ],
  }),
