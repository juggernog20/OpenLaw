...
const SECTIONS = (process.env.SECTIONS ?? "setup,folders,repository,knowledge,business,teardown").split(",");
...
const ARTICLES = { "document-folders": "V-C27", "document-repository": "V-C30", "create-knowledge": "V-C33" };
...
const results = {
  ...
  articleHashes: Object.fromEntries(
    Object.keys(ARTICLES).map((id) => [id, sha(fs.readFileSync(path.join(root, "docs/user-guides", `${id}.md`)))]),
  ),
  fixtures: [
    ...["doc029-bulk-a.txt", "doc029-bulk-b.txt", "doc029-bulk-c.txt", "doc029-bulk-d.txt", "doc029-services-text.pdf", "doc029-scan.pdf", "doc029-draft-v1.docx", "doc029-schedule.csv"].map((f) => ({
      path: `docs/documentation/batches/DOC-029/documents/fixtures/${f}`,
      sha256: sha(fs.readFileSync(dfix(f))),
    })),
    ...["doc029-policy-a.pdf", "doc029-policy-b.pdf", "doc029-playbook.docx", "doc029-playbook-v2.docx", "doc029-checklist.pdf"].map((f) => ({
...
async function setup() {
  ...
  await step(
    ...
    async () => {
      ...
      await page.getByRole("navigation", { name: "Settings sections" }).getByRole("link", { name: "Documents", exact: true }).click();
      ...
      results.settingsChanged.push({ setting: "Matter Document type list", added: MATTER_TYPE, id: t.id, restore: "archive at teardown" });
...
async function teardown() {
  ...
  for (const t of (r.body.documentTypes ?? []).filter((x) => !x.archivedAt && x.displayName.startsWith("DOC-030 documents-2 Matter type"))) {
    const a = await api(page, "POST", `/documents/types/matter/${t.id}/archive`, {});
    const entry = results.settingsChanged.find((s) => s.id === t.id) ?? { setting: "Matter Document type list", added: t.displayName, id: t.id };
...
async function folders(role, person) {
  ...
  currentPage = page;
  const contract = await createContract(page, role, `DOC-030 documents-2 ${label} folders Contract ${stamp}`);
  ...
  const loose = await uploadApi(page, `${recUrl}/documents`, dfix("doc029-bulk-a.txt"), { as: `doc030-${label.toLowerCase()}-loose-a.txt` });
  const loose2 = await uploadApi(page, `${recUrl}/documents`, dfix("doc029-bulk-b.txt"), { as: `doc030-${label.toLowerCase()}-loose-b.txt` });

  await step(A, role, "Create and maintain folders steps 1-2: Documents tab, New folder, Name, Save; an empty folder has no expand control", "The folder row appears without an expand control", async () => {
    ...
    const rowText = tidy(await page.getByRole("row").filter({ has: page.getByRole("button", { name: `Actions for the ${parent} folder` }) }).innerText());
  ...
  await step(A, role, "Create and maintain folders steps 2-3: New subfolder and Rename from Actions for the … folder; a folder with contents gets an expand control", "The subfolder is created inside; the parent gains an expand/collapse control; Rename changes the name", async () => {
    ...
    const toggle = await page.getByRole("button", { name: new RegExp(`^(Collapse|Expand) ${parent}$`) }).count();
    ...
    expect(c && p && c.parentId === p.id && toggle === 1, `child ${!!c} parent link ${c?.parentId === p?.id} toggle ${toggle}`);
  ...
  await step(A, role, "Folder name rules: a case-only duplicate sibling, a blank name, a slash, '.' and '..' are refused", "Each attempt shows a refusal in the dialog and creates nothing", async () => {
  ...
  await step(A, role, "Create and maintain folders step 3: Move, Move into, None; a folder cannot move into itself or a descendant", "Move into omits the folder and its descendant, the server refuses a cycle, and None returns a folder to the top level", async () => {
    await docsSection(page).getByRole("button", { name: "New folder" }).click();
    ...
    const underOther = folderPath(list, list.find((f) => f.name === child)) === `${other}/${child}`;
  ...
  await step(A, role, "Create and maintain folders step 4: Actions, Move to folder, File in, Move; None moves it out", "The Document is filed in the folder and then moved back to the record", async () => {
    await openDocuments(page, `${recUrl}/documents`);
  ...
  await step(A, role, "Create and maintain folders step 4: drag a Document row onto a folder row, then onto Drop here to move out of folders", "Dragging files the Document into the folder and back out", async () => {
    await openDocuments(page, `${recUrl}/documents`);
    ...
    await until(async () => folderOf(await getDoc(page, recUrl, loose2.id)) === p.id, "drag onto folder row did not file the Document", 8000);
    await openDocuments(page, `${recUrl}/documents`);
    ...
    await until(async () => !folderOf(await getDoc(page, recUrl, loose2.id)), "drop target did not move the Document out", 8000);
  ...
  await step(A, role, "Create and maintain folders step 4: select checkboxes and Move in the selection bar", "Both selected Documents move into the chosen folder", async () => {
    await openDocuments(page, `${recUrl}/documents`);
    ...
    expect(docs.every((d) => folderOf(d) === p.id), "bulk move did not file both");
  ...
  await step(A, role, "Folder Delete dissolves the folder: contents move to its parent or the record; nothing is deleted", "The confirmation says where contents go; the Documents stay live on the record", async () => {
    await openDocuments(page, `${recUrl}/documents`);
    ...
    const both = [loose.id, loose2.id].every((id) => docs.some((d) => d.id === id && !folderOf(d)));
  ...
  await step(A, role, "Import several files step 3: Type starts on No type, or on the type chosen in the upload dialog; a Contract import always shows Type", "The Import dialog's Type starts on No type after a plain pick and on the chosen type after one was picked in Upload document", async () => {
    await openDocuments(page, `${recUrl}/documents`);
    let composer = await uploadDialog(page);
    await chooseFiles(page, composer.getByRole("button", { name: /Choose files$/ }), [dfix("doc029-bulk-a.txt"), dfix("doc029-bulk-b.txt")]);
    ...
    await dialog.getByRole("button", { name: "Cancel" }).click();
    await dialog.waitFor({ state: "hidden" });
    expect(plainStart === "No type" && options[0] === "No type" && carried === "Executed", `plain ${plainStart} carried ${carried} options ${options}`);
  ...
  await step(A, role, "Import several files steps 1-5 and Recover a partial import: Destination Record root, Type, a real oversize file and a dropped connection; counts, row Retry, Done", "Good files stay as v1 Documents of the chosen type; the oversize row is named without Retry; the dropped row has Retry and a retry sends only it; Done closes", async () => {
    await openDocuments(page, `${recUrl}/documents`);
    const big = oversize();
    let dropOnce = true;
    ...
    await page.route(routeUrl, async (route) => {
      ...
      if (route.request().method() === "POST" && dropOnce && head.includes('filename="doc029-bulk-c.txt"')) {
        dropOnce = false;
    ...
    await chooseFiles(page, composer.getByRole("button", { name: /Choose files$/ }), [dfix("doc029-bulk-a.txt"), dfix("doc029-bulk-b.txt"), dfix("doc029-bulk-c.txt"), big]);
    ...
    const typeHelp = await page.evaluate(() => document.getElementById("batch-type-help")?.textContent ?? null);
    const typeDescribedBy = await dialog.getByLabel("Type", { exact: true }).getAttribute("aria-describedby");
    ...
    const retryDropped = await settled.getByRole("button", { name: "Retry doc029-bulk-c.txt" }).count();
    const retryBig = await settled.getByRole("button", { name: "Retry doc030-oversize.bin" }).count();
    const retryAll = await settled.getByRole("button", { name: /^Retry \d+ files?$/ }).allInnerTexts();
    ...
    await until(async () => /Imported 3 of 4 files/.test(tidy(await page.getByRole("dialog").first().innerText().catch(() => ""))), "retry did not settle at 3 of 4", 60000);
    ...
    expect(/Destination/.test(confirmText) && /Record root/.test(confirmText) && /Type/.test(confirmText), `confirm ${confirmText}`);
    expect(midAdded === 2 && retryDropped === 1 && retryBig === 0 && added.length === 1 && fresh.length === 3, `mid ${midAdded} retryDropped ${retryDropped} retryBig ${retryBig} added ${added.length} fresh ${fresh.length}`);
    expect(fresh.every((d) => d.versions.length === 1 && typeOf(d.versions[0]) === "Draft · theirs"), `types ${fresh.map((d) => typeOf(d.versions[0]))}`);
  ...
  await step(A, role, "Import step 5: when every file uploads the dialog closes after the list refreshes", "The dialog closes by itself and both files are listed", async () => {
    const before = (await recordDocs(page, recUrl)).length;
    const composer = await uploadDialog(page);
    await chooseFiles(page, composer.getByRole("button", { name: /Choose files$/ }), [dfix("doc029-bulk-c.txt"), dfix("doc029-bulk-d.txt")]);
    ...
    const rows = await docsSection(page).getByRole("button", { name: /^Actions for doc029-bulk-d/ }).count();
    expect(after.length === before + 2 && rows >= 1, `added ${after.length - before} rows ${rows}`);
  ...
  await step(A, role, "Recover a partial import: Cancel remaining stops queued work; uploaded files remain; final counts are shown", "Queued files are cancelled, finished uploads stay on the record, and the dialog shows the final counts", async () => {
    const routeUrl = `**/api/v1/contracts/${contract.number}/documents`;
    await page.route(routeUrl, async (route) => {
    ...
    await chooseFiles(page, composer.getByRole("button", { name: /Choose files$/ }), files.map(dfix));
  ...
  let tree;
  await step(A, role, "Keep a local folder structure: Choose folder keeps nested paths; the folder picker drops empty folders; a matching path is reused", "The top-level folder and its nested child are created with their files; the empty folder is not; a second import reuses the path", async () => {
    tree = makeTree(label);
    await openDocuments(page, `${recUrl}/documents`);
    let composer = await uploadDialog(page);
  ...
  await step(A, role, "Import steps 1-2 and Keep structure: drop a local folder on a folder row; Destination is that folder; a dropped tree recreates empty folders", "The Import dialog names the folder as Destination and the empty folder is recreated", async () => {
    await openDocuments(page, `${recUrl}/documents`);
    const folderRow = page.getByRole("row").filter({ has: page.getByRole("button", { name: `Actions for the ${other} folder` }) });
  ...
  await step(A, role, "Import step 3: a Matter import shows Type when the Matter list has a type; the chosen type is applied", `Import 2 files on a Matter shows Type with No type and "${MATTER_TYPE}"; both new Versions carry it`, async () => {
    ...
    const matter = await createMatter(page, role, `DOC-030 documents-2 ${label} folders Matter ${stamp}`);
    ...
    expect(start === "No type" && opts.includes(MATTER_TYPE) && docs.length === 2 && docs.every((d) => typeOf(d.versions[0]) === MATTER_TYPE), `start ${start} opts ${opts} types ${docs.map((d) => typeOf(d.versions[0]))}`);
  ...
  await step(A, role, "Import step 3: an Entity import shows Type only when the Entity list has a type", "The Type control is present exactly when the Entity type list is not empty", async () => {
    const active = await listTypes("entity");
    const ent = await createEntity(page, role, `DOC-030 documents-2 ${label} folders Entity ${stamp} Ltd`);
    await openDocuments(page, `/entities/${ent.id}/documents`);
    const composer = await uploadDialog(page);
    await chooseFiles(page, composer.getByRole("button", { name: /Choose files$/ }), [dfix("doc029-bulk-a.txt"), dfix("doc029-bulk-b.txt")]);
    const dialog = page.getByRole("dialog", { name: "Import 2 files" });
    await dialog.waitFor();
    await dialog.getByLabel("Type", { exact: true }).waitFor({ timeout: 8000 }).catch(() => {});
    await page.waitForLoadState("networkidle").catch(() => {});
    const count = await dialog.getByLabel("Type", { exact: true }).count();
    const dest = await dialog.getByText("Destination").count();
    await dialog.getByRole("button", { name: "Cancel" }).click();
    expect(dest === 1 && count === (active.length > 0 ? 1 : 0), `type ${count} active ${active.length}`);
    if (active.length > 0)
      results.limitations.push({
        article: A,
        claim: "A Matter or Entity import shows Type only when an Administrator has added a type to that module's list.",
...
async function hiddenPaper() {
  ...
  const doc = await uploadApi(page, `/contracts/${c.number}/documents`, dfix("doc029-services-text.pdf"), { as: `repo-hidden-${stamp}.pdf` });
...
async function menuNames(page) {
  ...
  const names = (await pop.getByRole("button").allInnerTexts()).map((t) => tidy(t.replace(/[✓]/g, "")));
...
async function repository(role, person) {
  ...
  const contract = await createContract(page, role, `DOC-030 documents-2 ${label} repository Contract ${stamp}`);
  const recUrl = `/contracts/${contract.number}`;
  ...
  const ctypes = Object.fromEntries((await listTypes("contract")).map((t) => [t.displayName, t.id]));
  const tag = `${label.toLowerCase()}-${stamp}`;
  const rootPdf = await uploadApi(page, `${recUrl}/documents`, dfix("doc029-services-text.pdf"), { as: `repo-root-${tag}.pdf`, documentTypeId: ctypes["Draft · ours"] });
  const v2 = await api(page, "POST", `/documents/${rootPdf.id}/versions`, undefined, { documentTypeId: ctypes["Draft · theirs"], file: part(dfix("doc029-scan.pdf"), `repo-root-${tag}-v2.pdf`) });
  expect(v2.status === 201, `fixture version ${v2.status} ${JSON.stringify(v2.body).slice(0, 160)}`);
  const filedDocx = await uploadApi(page, `${recUrl}/documents`, dfix("doc029-draft-v1.docx"), { as: `repo-folder-${tag}.docx`, documentTypeId: ctypes["Executed"], folderId });
  const archivable = await uploadApi(page, `${recUrl}/documents`, dfix("doc029-bulk-a.txt"), { as: `repo-archive-${tag}.txt` });
  const draggable = await uploadApi(page, `${recUrl}/documents`, dfix("doc029-bulk-b.txt"), { as: `repo-drag-${tag}.txt` });
  const matter = await createMatter(page, role, `DOC-030 documents-2 ${label} repository Matter ${stamp}`);
  const matterDoc = await uploadApi(page, `/matters/${matter.number}/documents`, dfix("doc029-services-text.pdf"), { as: `repo-matter-${tag}.pdf` });
  const ent = await createEntity(page, role, `DOC-030 documents-2 ${label} repository Entity ${stamp} Ltd`);
  const entityDoc = await uploadApi(page, `/entities/${ent.id}/documents`, dfix("doc029-services-text.pdf"), { as: `repo-entity-${tag}.pdf` });
  const kType = (await api(page, "GET", "/knowledge/type-options")).body.knowledgeTypes.find((t) => t.displayName === "Precedent");
  const k = await api(page, "POST", "/knowledge", { title: `DOC-030 documents-2 ${label} repository Knowledge ${stamp}`, knowledgeTypeId: kType.id });
  ...
  const knowledgeDoc = await uploadApi(page, `/knowledge/${k.body.knowledgeItem.id}/documents`, dfix("doc029-services-text.pdf"), { as: `repo-knowledge-${tag}.pdf` });
  const ad = await api(page, "POST", "/auto-docs", { name: `DOC-030 documents-2 ${label} repository Auto-Doc ${stamp}` });
  ...
  const tpl = await api(page, "POST", `/auto-docs/${autoDocId}/template`, undefined, { file: part(dfix("doc029-draft-v1.docx"), `repo-autodoc-${tag}.docx`) });
  expect([200, 201].includes(tpl.status), `fixture template ${tpl.status} ${JSON.stringify(tpl.body).slice(0, 200)}`);
  ...
  await step(A, role, "Search and narrow step 1: Documents from navigation; Recent shows up to five Documents by current-Version upload, not by opening", "Recent lists at most five in newest-current-Version order, and opening an older Document does not change it", async () => {
    await page.goto(`${BASE}/`);
    ...
    const read = async () => {
      let ui, apiTitles;
      for (let i = 0; i < 4; i++) {
        ui = (await recent.getByRole("link").evaluateAll((els) => els.map((e) => e.getAttribute("aria-label")))).map((t) => String(t).replace(/^Open recent Document /, ""));
    ...
    expect(first.ui.length <= 5 && JSON.stringify(first.ui) === JSON.stringify(first.apiTitles), `ui ${first.ui} api ${first.apiTitles}`);
    ...
    expect(JSON.stringify(second.ui) === JSON.stringify(second.apiTitles) && (!second.apiTitles.includes(filedDocx.title) || first.apiTitles.includes(filedDocx.title)), `after ${second.ui}`);
  ...
  await step(A, role, "Search and narrow step 2: Filter, Owner, Contracts/Matters/Entities/Auto-Docs/Knowledge, Apply", "Owner offers the five kinds and each narrows to paper of that owner kind", async () => {
    ...
    for (const [choice, present, absent] of expectations) {
      ...
      expect(has(rows, present) && absent.every((t) => !has(rows, t)), `${choice}: ${rows.slice(0, 4)}`);
  ...
  await step(A, role, "Search and narrow step 3: Record with Search choices; Folder becomes available for one Contract, with Record root; no Folder for Knowledge or Auto-Docs", "Record narrows to one Contract; Folder picks the folder or Record root", async () => {
  ...
  await step(A, role, "Search and narrow steps 4-5: Format and Kind take several values; different filters must all match; Kind describes the current Version; Counterparty, Uploader; Uploaded From/To includes both dates; remove one filter; Clear all; Recent hidden while filtered", "Any-of within a filter, all-of across filters, current-Version kind, inclusive dates, and Recent only without filters", async () => {
    ...
    expect(currentKindOnly && anyOf && rows.length - 1 === 1 && has(rows, filedDocx.title), `kind rows ${rows} currentKindOnly ${currentKindOnly} anyOf ${anyOf}`);
    ...
    await page.getByRole("button", { name: /^Uploaded:/ }).first().click();
    ...
    expect(/Matches any selected value/.test(fmtHint) && /Includes both dates/.test(dateHint) && recentWhileFiltered === 0 && recentAfter === 1 && stillEmpty && widened, `hint ${fmtHint.slice(0, 60)} date ${dateHint.slice(0, 60)} recent ${recentWhileFiltered}/${recentAfter} widened ${widened}`);
  ...
  await step(A, role, "Open the matching paper: selecting the name opens the owning record with the current Version; expand the chain for an earlier round", "The record opens with the reader on v2 and the chain lists v1", async () => {
    await applyChoice(page, "Record", [`C-${contract.number} · ${contract.title}`], `C-${contract.number}`);
  ...
  await step(A, role, "Search and narrow closing text: global search reads only the latest Version; an earlier Version is read from the owning record's Documents tab", "Header Search finds the Document by a word in its latest Version and not by a word only in the earlier Version; the chain on the record still opens the earlier Version", async () => {
    ...
    const newText = await until(async () => {
      const t = await search(newWord);
      return t.includes(doc.title) ? t : null;
    }, "the latest Version's word did not find the Document", 90000);
  ...
  await step(A, role, "Missing or archived: a hidden record's Document does not appear through a filter, the list, or a direct link", role === "administrator" ? "An Administrator reaches every Contract, so the Confidential Contract's paper is listed" : "The Confidential Contract outside this reader's team is absent from Record and Counterparty choices and the list, and its address is refused", async () => {
    ...
    await page.waitForLoadState("networkidle").catch(() => {});
    const pageText = tidy(await page.getByRole("main").innerText().catch(() => "")).slice(0, 120);
    await page.goto(`${BASE}/documents`);
    if (role === "administrator") {
      expect(recordChoices.includes(hidden.title) && listed && direct.status === 200, `admin ${recordChoices.slice(0, 80)} listed ${listed} direct ${direct.status}`);
    ...
    expect(!recordChoices.includes(hidden.title) && !cpChoices.includes(hidden.cpName) && !listed && direct.status === 404, `legal record ${recordChoices.slice(0, 80)} cp ${cpChoices.slice(0, 80)} listed ${listed} direct ${direct.status}`);
  ...
  await step(A, role, "Missing or archived: Filter, Show archived includes archived Documents; the offered Restore brings one back", "The archived row appears marked Archived with Restore; Restore makes it live", async () => {
  ...
  await step(A, role, "Missing or archived: an empty result after filters; Clear all widens again", "The empty answer is shown and Clear all returns the whole list", async () => {
    await applyChoice(page, "Record", [`C-${contract.number} · ${contract.title}`], `C-${contract.number}`);
    await applyChoice(page, "Format", ["Email"]);
    let rows = await repoRows(page);
    const emptyText = (tidy(await page.getByRole("main").innerText()).match(/No documents match these filters\.( Clear filters to return to the whole list\.)?/) ?? [""])[0];
    await clearAll(page);
    rows = await repoRows(page);
    const recent = await page.getByRole("region", { name: "Recent documents" }).count();
    expect(emptyText && rows.length > 1 && recent === 1, `empty ${emptyText} rows ${rows.length} recent ${recent}`);
  ...
  await step(A, role, "Before you start: drag a live Contract Document row onto a Move to folder target inside its owning record", "The Move to folder panel names the record's folders and the drop files the Document", async () => {
    await applyChoice(page, "Record", [`C-${contract.number} · ${contract.title}`], `C-${contract.number}`);
    ...
    await until(async () => folderOf(await getDoc(page, recUrl, draggable.id)) === folderId, "repository drag did not file the Document", 8000);
    await clearAll(page);
...
const folderButton = (page, name) => folderTree(page).getByRole("button", { name: new RegExp(`^${escapeRe(name)}\\s*\\d*$`) });
...
async function readKDocs(page, id, includeArchived = false) {
  const r = await api(page, "GET", `/knowledge/${id}/documents${includeArchived ? "?includeArchived=true" : ""}`);
...
async function openKnowledge(page) {
  await page.goto(`${BASE}/`);
  await page.getByRole("navigation").getByRole("link", { name: "Knowledge", exact: true }).first().click();
  await page.waitForURL(/\/knowledge$/);
  await itemsRegion(page).waitFor();
}
async function selectFolder(page, name) {
  await folderButton(page, name).click();
  await until(async () => (await folderButton(page, name).getAttribute("aria-current")) === "page", `folder ${name} did not become selected`);
  await sleep(600);
}
...
async function listTitles(page) {
  ...
  return region.locator("tbody tr").evaluateAll((rows) => rows.map((row) => (row.querySelector("td")?.innerText ?? "").split("\n")[0].trim()));
}
async function addKFolder(page, name) {
  ...
  await dialog.waitFor({ state: "hidden" });
  const folder = await until(async () => (await readKFolders(page)).find((row) => row.name === name), `folder ${name} not saved`);
...
const rowOfAction = (page, title) => page.getByRole("button", { name: `Actions for ${title}`, exact: true }).locator("xpath=ancestor::*[self::tr or self::li][1]");
...
async function knowledgeFixtureType() {
  ...
  results.settingsChanged.push({ setting: "Knowledge type list", added: name, id, state: "archived fixture", restore: "delete at teardown" });
...
async function knowledge(role, person) {
  ...
  await step(A, role, "Before you start: open Knowledge from the navigation", "Knowledge opens with New, All Knowledge and the Knowledge items list", async () => {
  ...
  await step(A, role, "Organize the library step 2: Add folder, Folder name, Add folder at top level; then a child under the selected folder", "With All Knowledge selected the folder is top-level; with a folder selected the new folder is its child", async () => {
    ...
    record(role, "knowledge_folders", `${base} parent, ${base} child`, `${ctx.parent.id}, ${ctx.child.id}`);
  ...
  await step(A, role, "Start with files steps 1-2: New, New from files; Folder starts at the selected folder, or at Library", "Folder reads Library with All Knowledge selected and the selected folder otherwise", async () => {
    await selectAll(page);
    ...
    expect(rootLabel === "Library" && selLabel.includes(`${base} parent`), `root ${rootLabel} selected ${selLabel}`);
    return `With All Knowledge selected, New from files showed Folder "Library". With "${base} parent" selected it started at "${selLabel}".`;
  });

  await step(A, role, "Start with files steps 2-3: Drop files here or choose files, two files, a Type, Create drafts", "Each file creates a separate draft item named after the file with its own primary Document; the first item opens and the other is in Knowledge", async () => {
    ...
    const heading = await until(async () => {
      const t = tidy(await page.locator("#page-title").innerText().catch(() => ""));
      return t.startsWith(base) ? t : null;
    }, "record heading did not render");
    const m = await markers(page);
    const first = await readItem(page, firstId);
    expect(m.draft && heading === first.title && [`${base} policy A.pdf`, `${base} policy B.pdf`].includes(first.title), `draft ${m.draft} heading ${heading} title ${first.title}`);
    expect(first.state === "draft" && first.audience === "legal_only" && first.folderId === ctx.parent.id && first.knowledgeTypeName === "Precedent" && first.primaryDocument && first.documentCount === 1, `first ${JSON.stringify(first).slice(0, 200)}`);
    const primaryControl = await page.getByRole("button", { name: `Open preview of ${first.primaryDocument.title}` }).count();
    expect(primaryControl === 1, "Primary document control does not name the file");
    await page.locator("section[aria-labelledby='page-title']").getByRole("link", { name: "Knowledge" }).click();
    await page.waitForURL(/\/knowledge$/);
    await selectFolder(page, `${base} parent`);
    ...
    expect(otherRow && otherRow.state === "draft" && otherRow.primaryDocument && otherRow.documentCount === 1, "second item not a separate draft with its own primary");
  ...
  await step(A, role, "Start with files step 4: Title saves on Enter and on leaving the field; Type and Folder save on change", "Each value is saved and survives a reload", async () => {
    ...
    await until(async () => (await readItem(page, ctx.itemA.id)).title === `${base} policy A renamed`, "Enter did not save the title");
    await title.fill(`${base} policy A reviewed`);
    await page.locator("#knowledge-record-type").focus();
    await until(async () => (await readItem(page, ctx.itemA.id)).title === `${base} policy A reviewed`, "leaving the field did not save the title");
    await page.locator("#knowledge-record-type").selectOption({ label: "Playbook" });
    await until(async () => (await readItem(page, ctx.itemA.id)).knowledgeTypeName === "Playbook", "type did not save");
    await page.locator("#knowledge-record-folder").selectOption(ctx.child.id);
    await until(async () => (await readItem(page, ctx.itemA.id)).folderId === ctx.child.id, "folder did not save");
    await page.reload();
    await page.locator("#knowledge-record-type").waitFor({ timeout: 20000 });
    const after = {
    ...
    expect(after.title === `${base} policy A reviewed` && after.type === "Playbook" && after.folder === ctx.child.id, `after ${q(after)}`);
  ...
  await step(A, role, "Start with guidance steps 1-2: New, New Knowledge Item, Title, Type, Folder, Attach documents with no type choice for the files; Create item; a failed upload offers Retry failed uploads", "No Document type control; the item exists after the failed upload; Retry failed uploads uploads the file; the item is Draft and Legal Only; the file shows the item's Knowledge type", async () => {
    ...
    await chooseFiles(page, dialog.getByRole("button", { name: "Attach documents", exact: true }).and(dialog.locator("button")), [docx]);
    ...
    await dialog.getByText("Record created. Some documents could not be uploaded.").waitFor({ timeout: 20000 });
    ...
    expect(item.state === "draft" && item.audience === "legal_only" && m.draft && audience === "Legal Only" && item.primaryDocument?.title && item.documentCount === 1, `item ${JSON.stringify(item).slice(0, 200)}`);
    expect(rowTypeText === "Article" && typeOf(docs[0].versions[0]) === "Article", `document row type ${rowTypeText} read-back ${typeOf(docs[0].versions[0])}`);
    ctx.guidance = item;
  ...
  await step(A, role, "Start with guidance step 2: Continue after a failed upload keeps the item", "Continue opens the created item without the failed file", async () => {
    await page.goto(`${BASE}/knowledge`);
    await itemsRegion(page).waitFor();
    await page.getByRole("button", { name: "New", exact: true }).click();
    await page.getByRole("menuitem", { name: "New Knowledge Item" }).click();
    ...
    await chooseFiles(page, dialog.getByRole("button", { name: "Attach documents", exact: true }).and(dialog.locator("button")), [pdf]);
    await page.route("**/api/v1/knowledge/*/documents", (route) => (route.request().method() === "POST" ? route.abort("failed") : route.continue()));
    await dialog.getByRole("button", { name: "Create item" }).click();
    ...
    expect(item.title === `${base} continue` && item.documentCount === 0 && !item.primaryDocument, "continue item wrong");
  ...
  await step(A, role, "Start with guidance steps 3-4: Add guidance, Markdown, Preview saves on blur and renders, Edit returns to the source; raw HTML is not rendered", "Headings, lists, emphasis, code and links render; raw HTML does not become HTML; the source survives a reload", async () => {
    ...
    const source = [`## ${base} supplier checks`, "", "Use this **fictional** list before *onboarding* a supplier.", "", "- Check the `vendor-id` field", "- Read the [supplier policy](https://example.com/doc030-policy)", "", "<b>raw-html-doc030</b>"].join("\n");
    await editor.fill(source);
    await guidance.getByRole("button", { name: "Preview" }).click();
    await until(async () => (await readItem(page, ctx.guidance.id)).body === source, "Preview blur did not save the guidance");
    ...
    expect(rendered.strong.includes("fictional") && rendered.em.includes("onboarding") && rendered.li === 2 && rendered.code.includes("vendor-id") && rendered.links.includes("https://example.com/doc030-policy") && !rendered.rawB, `rendered ${q(rendered)}`);
    ...
    expect((await page.locator("#knowledge-body").inputValue()) === source, "source lost after reload");
  ...
  await step(A, role, "Add Documents step 1: upload a new Document in the item's Documents section", "Upload adds a new v1 Document; the first Document stays primary", async () => {
    await docsSection(page).getByRole("button", { name: "Upload", exact: true }).click();
    const dialog = page.getByRole("dialog", { name: "Upload document" });
    ...
    expect(docxDoc.isPrimary && !pdfDoc.isPrimary && pdfDoc.versions.length === 1, "first Document is no longer primary");
  ...
  await step(A, role, "Add Documents step 2: Actions, Set as primary; check the Primary mark and the Primary document control", "The chosen Document shows Primary and is named under Primary document", async () => {
    ...
    await page.getByRole("button", { name: `Open preview of ${ctx.pdfTitle}` }).waitFor({ timeout: 15000 });
    const docs = await readKDocs(page, ctx.guidance.id);
    const rowText = tidy(await rowOfAction(page, ctx.pdfTitle).innerText());
    expect(docs.find((d) => d.id === ctx.pdfDocId).isPrimary && !docs.find((d) => d.id === ctx.docxDocId).isPrimary && /Primary/.test(rowText) && noSetOnPrimary === 0, `row ${rowText} noSet ${noSetOnPrimary}`);
  ...
  await step(A, role, "Add Documents step 3: Open preview reads the primary's current Version; the other Document keeps its own Version history", "The reader opens on the primary's current Version; a new Version on the supporting Document stays on its own chain", async () => {
    ...
    expect(pdfDoc.isPrimary && pdfDoc.versions.length === 1, "primary changed by supporting version");
  ...
  await step(A, role, "Add Documents closing text: archiving the primary Document preserves Versions and designation and picks no replacement; restore returns it", "The archived primary leaves the ordinary list, no other row becomes Primary, and restore returns it with its designation", async () => {
    ...
    await until(async () => (await page.getByRole("button", { name: `Actions for ${ctx.pdfTitle}` }).count()) === 0, "archived Document still in the ordinary list");
    const docxRow = tidy(await rowOfAction(page, ctx.docxTitle).innerText());
    const archived = (await readKDocs(page, ctx.guidance.id, true)).find((d) => d.id === ctx.pdfDocId);
    expect(!/\bPrimary\b/.test(docxRow) && archived.archivedAt && archived.isPrimary && archived.versions.length === 1, `docxRow ${docxRow} archived ${!!archived.archivedAt} primary ${archived.isPrimary}`);
    await page.getByLabel("Show archived").click();
    await chooseMenu(page, ctx.pdfTitle, "Restore");
    await until(async () => (await readKDocs(page, ctx.guidance.id)).some((d) => d.id === ctx.pdfDocId), "restore did not return the Document");
    await page.reload();
    await page.locator("#knowledge-record-type").waitFor({ timeout: 20000 });
  ...
  await step(A, role, "Add Documents closing text: an item's Documents use a flat list", "The Documents section has no New folder control, and a folder pick imports its file at the item root with no Document folder", async () => {
    await openRecord(page, ctx.guidance.id);
    ...
    fs.copyFileSync(kfix("doc029-checklist.pdf"), path.join(dir, "inner", `${base} folder pick.pdf`));
    ...
    const importButton = page.getByRole("dialog").getByRole("button", { name: /^Import 1 file$/ });
  ...
  await step(A, role, "Organize the library step 1: All Knowledge, a folder with its descendants, and the Type, State, Audience, Author and Format filters", "A folder lists its descendants; each filter narrows the list; clearing restores it", async () => {
    await page.goto(`${BASE}/knowledge`);
    await itemsRegion(page).waitFor();
    for (const l of ["Type", "State", "Audience", "Author", "Format"]) await page.getByRole("combobox", { name: l, exact: true }).first().waitFor();
    await selectFolder(page, `${base} parent`);
    let titles = await listTitles(page);
    expect(titles.includes(ctx.itemA.title) && titles.includes(ctx.itemB.title) && titles.includes(`${base} guidance`), `parent folder list ${q(titles)}`);
    await selectFolder(page, `${base} child`);
    titles = await listTitles(page);
    expect(titles.includes(ctx.itemA.title) && !titles.includes(ctx.itemB.title), `child list ${q(titles)}`);
    await selectFolder(page, `${base} parent`);
    await page.getByRole("combobox", { name: "Type", exact: true }).first().selectOption({ label: "Precedent" });
    await sleep(800);
    titles = await listTitles(page);
    expect(titles.includes(ctx.itemB.title) && !titles.includes(ctx.itemA.title), `type filter ${q(titles)}`);
    await page.getByRole("combobox", { name: "State", exact: true }).first().selectOption({ label: "Published" });
    await sleep(800);
    titles = await listTitles(page);
    ...
    await sleep(800);
    await selectFolder(page, `${base} parent`);
    await page.getByRole("combobox", { name: "Author", exact: true }).first().selectOption({ label: person.name });
    await sleep(800);
    titles = await listTitles(page);
    expect(titles.includes(ctx.itemB.title), `author ${q(titles)}`);
    await page.getByRole("combobox", { name: "Format", exact: true }).first().selectOption({ label: "Word" });
    await sleep(800);
    titles = await listTitles(page);
    expect(!titles.includes(ctx.itemB.title), `format Word kept the PDF item ${q(titles)}`);
    await page.getByRole("combobox", { name: "Format", exact: true }).first().selectOption({ label: "PDF" });
    await sleep(800);
    titles = await listTitles(page);
    expect(titles.includes(ctx.itemB.title), `format PDF ${q(titles)}`);
    await page.getByRole("combobox", { name: "Audience", exact: true }).first().selectOption({ label: "Everyone" });
    await sleep(800);
    titles = await listTitles(page);
    ...
    await selectFolder(page, `${base} parent`);
    titles = await listTitles(page);
    expect(titles.includes(ctx.itemB.title) && titles.includes(ctx.itemA.title), "items did not return after clearing");
  ...
  await step(A, role, "Organize the library step 3: Rename or move selected folder excludes the folder and its descendants; Save folder renames or moves", "Parent folder omits the folder and its child; Save folder renames or moves it", async () => {
    await selectAll(page);
    ...
    const options = await dialog.getByLabel("Parent folder").locator("option").evaluateAll((els) => els.map((e) => e.value));
    expect(!options.includes(ctx.parent.id) && !options.includes(ctx.child.id) && options.includes(ctx.target.id), "parent choices include the folder or its descendant");
    ...
    await dialog.getByRole("button", { name: "Save folder" }).click();
    await dialog.waitFor({ state: "hidden" });
    await until(async () => (await readKFolders(page)).find((r) => r.id === ctx.child.id)?.parentId === ctx.target.id, "child folder did not move");
    await selectFolder(page, `${base} target`);
    const titles = await listTitles(page);
    expect(titles.includes(ctx.itemA.title) && (await readItem(page, ctx.itemA.id)).folderId === ctx.child.id, "moved folder's item not listed under the new parent");
  ...
  await step(A, role, "Organize the library step 4: up/down controls change sibling order; move an item by changing its Folder", "Move up then Move down changes the saved order and returns it", async () => {
    await selectFolder(page, `${base} target`);
    const orderOf = async () => {
      const siblings = (await readKFolders(page)).filter((r) => r.parentId === null).sort((a, b) => a.displayOrder - b.displayOrder);
  ...
  await step(A, role, "Organize the library step 5: Delete selected folder, read the confirmation, Delete folder; items and child folders move to its parent", "The confirmation says nothing is deleted; the child folder and the item remain with their Documents", async () => {
    await selectFolder(page, `${base} target`);
    ...
    expect(/Nothing is deleted\./.test(text) && !fl.some((r) => r.id === ctx.target.id) && fl.find((r) => r.id === ctx.child.id)?.parentId === null && item.folderId === ctx.child.id && item.documentCount === 1 && !item.archivedAt, `text ${text}`);
  ...
  await step(A, role, "Recover from an unavailable choice: Manage types… beside Type is for Administrators; archived types are unavailable for new selection", "Only the Administrator sees Manage types…; the archived type is absent from the record and both create dialogs", async () => {
    await openRecord(page, ctx.itemA.id);
    const manageCount = await page.getByRole("link", { name: "Manage types…" }).count();
    expect(manageCount === (role === "administrator" ? 1 : 0), `Manage types… count ${manageCount}`);
    ...
    await dialog.getByRole("button", { name: "Cancel" }).click();
    for (const list of [recordOptions, createOptions, filesOptions]) expect(!list.includes(archivedType.name) && list.includes("Playbook"), `options ${list}`);
    let target = null;
    if (role === "administrator") {
      await openRecord(page, ctx.itemA.id);
  ...
  await step(A, role, "Recover from an unavailable choice: an archived Knowledge Item cannot be edited or receive uploads; Restore makes it editable", "Archived disables Title, Type, Folder and guidance, hides Upload, and a direct upload is refused; Restore re-enables them", async () => {
    await openRecord(page, ctx.guidance.id);
    ...
    const disabled = {};
    for (const id of ["knowledge-record-title", "knowledge-record-type", "knowledge-record-folder", "knowledge-record-audience"]) disabled[id] = await page.locator(`#${id}`).isDisabled();
    disabled.guidanceEditor = await page.locator("#knowledge-body").isDisabled({ timeout: 3000 }).catch(() => null);
    const uploadCount = await page.getByRole("button", { name: "Upload", exact: true }).count();
    const refused = await api(page, "POST", `/knowledge/${ctx.guidance.id}/documents`, undefined, { file: part(kfix("doc029-checklist.pdf")) });
    expect(Object.values(disabled).every((v) => v !== false) && uploadCount === 0 && refused.status >= 400, `disabled ${q(disabled)} upload ${uploadCount} direct ${refused.status}`);
    await page.getByRole("button", { name: "Knowledge Item actions" }).click();
    ...
    await until(async () => !(await page.locator("#knowledge-record-title").isDisabled()), "title still disabled after restore");
  ...
  if (role === ROLES[0]) {
    await step(A, role, "Before you start: OpenLaw Help search does not search Knowledge Items", "Help finds this guide by its title but not a Knowledge Item by its title", async () => {
      await page.goto(`${BASE}/help`);
      const box = page.getByRole("searchbox", { name: "Search documentation" }).or(page.getByLabel("Search documentation")).first();
      await box.fill("Create and organize Knowledge Items");
      await box.press("Enter");
      await page.getByRole("link", { name: "Create and organize Knowledge Items" }).first().waitFor({ timeout: 15000 });
...
async function business() {
  ...
  const contract = await createContract(staff, "legal_team_member", `DOC-030 documents-2 Portal Contract ${stamp}`);
  const recUrl = `/contracts/${contract.number}`;
  await addTeam(staff, contract.number, PEOPLE.jonas.email);
  const seedDoc = await uploadApi(staff, `${recUrl}/documents`, dfix("doc029-bulk-a.txt"), { as: `doc030-portal-seed-${stamp}.txt` });
  const tf = await api(staff, "POST", `${recUrl}/folders`, { name: `DOC-030 portal target ${stamp}` });
  expect(tf.status === 201, `fixture folder ${tf.status}`);
  const targetId = (await folderList(staff, recUrl)).find((f) => f.name === `DOC-030 portal target ${stamp}`).id;
  const kItem = knowledgeItems.legal_team_member?.itemA?.id ?? knowledgeItems.administrator?.itemA?.id ?? (await api(staff, "GET", "/knowledge?limit=1")).body.knowledgeItems[0].id;
  const { page } = await portalContext(PEOPLE.jonas);
  currentPage = page;
  const section = () => page.getByRole("region", { name: "Documents", exact: true });

  await step("document-folders", role, "Before you start: Business Users upload several files at the record root in the Portal, but cannot create folders, move Documents or import a folder structure", "Upload documents takes two files to the record root; no Choose folder or New folder; the folder and move requests are refused", async () => {
    ...
    await chooseFiles(page, dialog.getByRole("button", { name: "Drop files or choose files" }), [dfix("doc029-bulk-c.txt"), dfix("doc029-bulk-d.txt")]);
    await dialog.getByRole("button", { name: "Upload", exact: true }).click();
    await dialog.waitFor({ state: "hidden", timeout: 30000 });
    const docs = await recordDocs(staff, recUrl);
    const added = docs.filter((d) => /doc029-bulk-[cd]/.test(d.versions[0]?.originalFilename ?? ""));
    const sectionText = tidy(await section().innerText());
    const create = await api(page, "POST", `${recUrl}/folders`, { name: "DOC-030 portal attempt" });
    const move = await api(page, "PATCH", `/documents/${seedDoc.id}`, { folderId: targetId });
    const after = await getDoc(staff, recUrl, seedDoc.id);
    expect(folderPicker === 0 && !/Choose folder|New folder/.test(dialogText + sectionText) && added.length === 2 && added.every((d) => !folderOf(d)) && create.status >= 400 && move.status >= 400 && !folderOf(after), `picker ${folderPicker} added ${added.length} create ${create.status} move ${move.status}`);
  ...
  await step("document-repository", role, "Before you start and Missing: Business Users read paper through the Portal, not the staff repository", "The staff /documents address does not open the repository, and the repository read is refused", async () => {
    await page.goto(`${BASE}/documents`);
    await sleep(2000);
    const landed = new URL(page.url()).pathname;
    const read = await api(page, "GET", "/documents?limit=5");
    expect(!landed.startsWith("/documents") && read.status >= 400, `landed ${landed} read ${read.status}`);
    return `Jonas's Portal session asking for /documents ended on ${landed}; the staff repository read answered ${read.status}.`;
  });

  await step("create-knowledge", role, "Before you start: Business Users cannot author or browse the staff Knowledge library", "The staff /knowledge and record addresses do not open; the staff Knowledge list, record and create requests are refused", async () => {
    await page.goto(`${BASE}/knowledge`);
    await sleep(1500);
    ...
    const statuses = [
      ...
      (await api(page, "POST", "/knowledge", { title: "DOC-030 documents-2 refused", knowledgeTypeId: typeId })).status,
    ];
    expect(!listPath.startsWith("/knowledge") && !recordPath.startsWith("/knowledge") && statuses.every((s) => s === 403 || s === 404), `list ${listPath} record ${recordPath} api ${statuses}`);
...
console.log(`steps ${results.steps.length}, passed ${passed}, failed ${results.steps.length - passed}`);
