...
**6. Keyboard changes order and depth separately.** A grip is a focusable button named "Move {name}". Arrow Up / Down moves one place among siblings, moving a Branch with its descendants. At a boundary it stays put. Announce name, new position, sibling count and parent in a polite live region; focus follows the same grip. Pinned Rows cannot be crossed. Enter or Space opens the grip's menu; the Row overflow exposes the same **Move into** and **Move out** pair. Move into opens a labelled Branch chooser, named by condition sentence and parent path. It excludes self and descendants and marks invalid destinations with their reason. Move out puts the node immediately after its former parent Branch. Root nodes show Move out unavailable with "Already at the root". Neither horizontal Arrow keys nor indentation alone changes depth. Escape closes a menu and returns focus to its trigger.
