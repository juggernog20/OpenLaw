...
export async function run(log, fx) {
  ...
  const home = must(await api(admin.page, "POST", "/contracts", { title: `DOC-029 access Home contract ${stamp}`, contractTypeId: nda, managerId: uid(SEED.daniel.email) }), "contract").contract;
  const conf = must(await api(admin.page, "POST", "/contracts", { title: `DOC-029 access confidential contract ${stamp}`, contractTypeId: nda, managerId: uid(SEED.daniel.email), isConfidential: true }), "confidential").contract;
  fx.created.push(`Contract C-${home.number} DOC-029 access Home contract ${stamp}`, `Confidential Contract C-${conf.number} DOC-029 access confidential contract ${stamp} (Daniel's team only)`);
  ...
  for (const role of ["administrator", "legal_team_member"]) {
    ...
    if (role === "administrator") await p.screenshot({ path: new URL("./r1-administrator-your-tasks-show-completed.png", import.meta.url).pathname });
