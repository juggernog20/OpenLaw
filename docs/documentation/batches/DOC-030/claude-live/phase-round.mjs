...
if (!PASSWORD || !PEOPLE[role])
  throw new Error(
    "usage: phase-round.mjs verify|disconnect|sessions|refused <role> …; LAB_PASSWORD set",
  );
...
const { log, save, step } = createLog(`${tag}-${cmd}`, {
  lab: LAB.name,
  publicOrigin: PUBLIC,
  client,
});
const secrets = [PASSWORD];
const S = (page, method = "browser-walkthrough") => ({
...
function appRequests(sinceMs) {
  ...
  for (const l of lines) {
    ...
    if (j.msg === "incoming request")
      reqs.set(j.reqId, {
        t: j.time,
        method: j.req.method,
        path: j.req.path.split("?")[0],
        ip: j.req.remoteAddress,
      });
    else if (j.msg === "request completed" && reqs.has(j.reqId))
      reqs.get(j.reqId).status = j.res?.statusCode;
  }
...
const grantRows = () =>
  psql(
    ...
    .filter(Boolean)
    .map((l) => {
      const [id, name, toolsets, scope, grantedAt, expiresAt, lastUsedAt, revokedAt, revokedBy] =
        l.split("|");
      return { id, name, toolsets, scope, grantedAt, expiresAt, lastUsedAt, revokedAt, revokedBy };
    });
const toolCalls = (sinceIso) =>
  psql(
    `select t.created_at, t.tool, t.outcome, t.client_name, u.display_name, u.email from mcp_tool_calls t join users u on u.id=t.person_id where t.created_at >= ${q(sinceIso)} order by t.created_at`,
  )
    .split("\n")
    .filter(Boolean)
    .map((l) => {
...
try {
  if (cmd === "verify") {
    ...
    const report = arg4 ?? "";
    await step(
      S("claude.ai", "live-provider-check"),
      `The person connected ${client} as ${person.name} and asked "Use OpenLaw to tell me who I am, then find and read the guide Connect Claude."`,
      "The person's report of what Claude answered",
      async (entry) => {
    ...
    await step(
      S("server: oauth_grants, app log", "live-provider-check"),
      `Grant row for ${person.name} and ${client}; the vendor's requests reached the lab from Anthropic's egress range`,
      "One live grant with the chosen Toolsets and scope; requests from 160.79.104.0/21",
      async () => {
        const g = grantRows();
        ...
        const identity = psql(
          `select c.metadata_url, oc.redirect_uris::text, oc.token_endpoint_auth_method from allowed_clients c left join oauth_clients oc on oc.client_id = c.metadata_url where c.name=${q(client)}`,
        );
        const token = rs
          .filter((r) => r.path === "/api/auth/oauth2/token")
          .map((r) => `${r.status} from ${anthropic(r.ip) ? "Anthropic" : r.ip}`);
    ...
    await step(
      S("server: mcp_tool_calls"),
      `Tool calls since ${since} ran as ${person.name} via ${client}, including openlaw_whoami, openlaw_docs_search and openlaw_docs_read`,
      "Rows with person = the signed-in person, clientName = the Client, outcome ok",
      async () => {
        ...
        expectThat(
          mine.length > 0 &&
            tools.includes("openlaw_whoami") &&
            mine.every((c) => c.outcome !== "pending"),
    ...
    await step(
      S("/settings/api-keys (or /portal/settings/api-keys)"),
      `As ${person.name}: open Connected Clients as the guide says and read the ${client} row`,
      "Row with Client, Toolsets, Read or Write, Granted date, Last used date and Disconnect",
      async () => {
        const how = await signIn();
        const region = await openConnectedClients();
        const text = flat(await region.innerText());
        const btn = await region
          .getByRole("button", { name: `Disconnect ${client}`, exact: true })
          .count();
        await page.screenshot({
          path: new URL(`./connected-clients-${role}.png`, import.meta.url).pathname,
        });
    ...
    await step(
      S(role === "business_user" ? "/portal/help/connect-claude" : "/help/connect-claude"),
      "Compare the Guide text with Help: the article Help shows versus the bundled article openlaw_docs_read returns",
      "Same article text",
      async () => {
        const bundled = JSON.parse(
          execFileSync(
            ...
            [
              ...
              "node",
              "-e",
              "const b=require('/app/apps/api/dist/documentation.json');const a=b.articles.find(a=>a.id==='connect-claude');console.log(JSON.stringify({text:a.text,sha:a.contentSha256,unverified:a.unverified,edition:b.edition.id}))",
            ],
            { encoding: "utf8" },
          ),
        );
        await page.goto(
          `${PUBLIC}${role === "business_user" ? "/portal/help" : "/help"}/connect-claude`,
        );
        await page.waitForLoadState("networkidle").catch(() => {});
        ...
        const norm = (s) =>
          flat(s)
            ...
            .replace(/\( /g, "(")
            .trim();
        const sentences = norm(bundled.text)
          .split(/(?<=\.)\s/)
        ...
        expectThat(
          found / sentences.length > 0.9,
          `${found}/${sentences.length}; missing ${JSON.stringify(missing.slice(0, 6))}`,
        );
    ...
    await step(
      S("/settings/api-keys (or /portal/settings/api-keys)"),
      `As ${person.name}: Connected Clients → Disconnect ${client} → Revoke OAuth grant → Revoke`,
      "The row leaves the list; the grant is revoked by this person",
      async (entry) => {
        const how = await signIn();
        const region = await openConnectedClients();
        ...
        const g = grantRows();
        expectThat(
          !after.includes(`Disconnect ${client}`) &&
            g[0]?.revokedAt &&
            g[0]?.revokedBy === person.name,
          JSON.stringify({ after, g }),
        );
    ...
    await step(
      S("/auth/consent"),
      `Reproduce the consent offer for ${person.name}: a fresh authorization request with Claude's published identity asking for all 13 Toolsets and write; stop at consent`,
      "Offered Toolsets = requested ∩ ceiling ∩ Toolsets with a Tool for this account type",
      async () => {
        ...
        const radios = await page
          ...
          .evaluateAll((els) =>
            els.map(
              (e) =>
                e.closest("label")?.querySelector("span, div")?.textContent?.trim() ??
                e.closest("label")?.textContent?.trim().slice(0, 20),
            ),
          );
        const who = flat(await page.locator("body").innerText()).match(
          /Jonas Weber.{0,60}|Daniel Okafor.{0,60}|Nadia Haddad.{0,60}/,
        )?.[0];
    ...
    await step(
      S("/settings/profile"),
      `As ${person.name}: Settings → Personal → Profile → Sign out other devices, then Sign out, so every OpenLaw browser session of this person ends`,
      "Other sessions revoked by the app; this session signed out; no live session left",
      async () => {
        const count = () =>
          Number(
            psql(
              `select count(*) from sessions s join users u on u.id=s.user_id where u.email=${q(person.email)} and s.expires_at > now()`,
            ),
          );
        const before = count();
        await signIn();
        ...
        expectThat(
          status === 200 && afterOthers === 1 && afterAll === 0,
          JSON.stringify({ before, status, afterOthers, afterAll }),
        );
    ...
    await step(
      S("claude.ai", "live-provider-check"),
      `After Disconnect, the person asked ${client} the same question again`,
      "The person's report: OpenLaw refused or asked to reconnect",
      async (entry) => {
        entry.personObservation = report;
        return `The person's observation, relayed by the coordinator: "${report}"`;
      },
    ...
    await step(
      S("server: app log, mcp_tool_calls", "live-provider-check"),
      "The Client's next calls after Disconnect are refused",
      "Every /mcp request after the revoke answers 401; no Tool call rows after the revoke",
      async () => {
        const rs = appRequests(Date.parse(revokedAt));
        const mcp = rs.filter((r) => r.path === "/mcp");
        const calls = toolCalls(revokedAt).filter(
          (c) => c.email === person.email && c.clientName === client,
        );
        expectThat(
          mcp.length > 0 && mcp.every((r) => r.status === 401) && calls.length === 0,
