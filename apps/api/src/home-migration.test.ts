...
async function existingRows(db: Db): Promise<unknown> {
  const snapshot = await db.execute<{ rows: unknown }>(sql`
    select jsonb_build_object(
      ...
      'contract', (select to_jsonb(c.*) - 'search_vector' - 'ai_unverified' - 'business_owner_id' - 'analysis_human_fields' - 'created_by' - 'owning_department_id' - 'region' - 'created_by_generation_id'
