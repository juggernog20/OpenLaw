...
import * as h from "./harness-r2.mjs";
...
const labJson = JSON.parse(
  readFileSync(path.join(h.root, ".documentation-labs/analysis-r2/lab.json"), "utf8"),
);
...
const { results, step } = h.makeRecorder({
  ...
  round: 2,
  walkthroughReviewer: "DOC-029r2 compatibility reviewer (analysis-standin)",
  reviewerKind: "agent",
...
async function analysisSetup() {
  ...
  const type = await h.ok(
    admin.post("/contract-types", {
      displayName: `DOC-029r2 analysis-standin Analysis type ${stamp}`,
    }),
  );
  ...
  for (const [key, displayName, fieldType, prompt] of [
    [
      "location",
      `DOC-029r2 Service location ${stamp}`,
      "text",
      "Extract the city where the services are performed.",
    ],
    ["tier", `DOC-029r2 Service tier ${stamp}`, "number", "Extract the numeric service tier."],
    ["absent", `DOC-029r2 Audit clause ${stamp}`, "text", "Extract the audit clause summary."],
  ]) {
  ...
  const holder = await h.ok(
    admin.post("/contracts", {
      title: `DOC-029r2 analysis-standin Counterparty holder ${stamp}`,
      contractTypeId: analysisType,
    }),
  );
  supplierName = `DOC-029r2 Analysis Supplier ${stamp} Ltd`;
  await h.ok(
    admin.post(`/contracts/${holder.contract.number}/counterparties`, { name: supplierName }),
  );
  setupNote(
    `Administrator API: Contract type ${q(`DOC-029r2 analysis-standin Analysis type ${stamp}`)} with three prompted Fields (text location, number tier, text audit clause), and one live Counterparty ${q(supplierName)} created on holder Contract C-${holder.contract.number}.`,
  );
}
...
function paper(marker, version) {
  const v1 = version === 1;
  return [
    `DOC-029r2 fictional services agreement, reference ${marker}.`,
...
async function analysisWalk(role) {
  ...
  const mk = async (suffix) =>
    (
      await h.ok(
        api.post("/contracts", {
          title: `DOC-029r2 analysis-standin ${suffix} ${tag}`,
          contractTypeId: analysisType,
          managerId: null,
        }),
      )
    ...
    async () => {
      ...
      try {
        ...
        await page
          ...
          .fill("DOC-029r2 unsaved draft text");
      } finally {
        await h.standin("/control/pause", { paused: false });
      }
      ...
      const locationShown = await page
        ...
        .inputValue();
      h.expectThat(
        shown === "DOC-029r2 unsaved draft text" && locationShown === "Muscat",
        `draft ${q(shown)} location ${q(locationShown)}`,
      );
      return `C-${draftContract} (separate Contract): typed "DOC-029r2 unsaved draft text" into ${fieldSlugs.absent.label} without leaving the control while the card read Running…. When the card updated to ${q(status)}, the typed draft was still in the control and the written ${fieldSlugs.location.label} showed ${q(locationShown)}.`;
    },
  );
  ...
  await step(
    ...
    async () => {
      ...
      await editKd("Price review", async (dialog) => {
        await dialog.getByLabel(/^Note/).fill("DOC-029r2 note only");
      });
      await h.sleep(1000);
  ...
  await step(
    ...
    async () => {
      ...
      await page.getByRole("menuitem", { name: "Edit date" }).click();
      const dialog = page.getByRole("dialog");
      await dialog.getByLabel(/^Note/).fill("DOC-029r2 check the option notice");
      await dialog.getByRole("button", { name: /^Save/ }).click();
...
async function conversionWalk(role) {
  ...
  await step(
    ...
    async () => {
      ...
      const needle = `DOC-029r2 Portal Supplier ${short(role)} ${stamp} GmbH`;
      const submitted = await h.ok(
        sessions.business_user.api.post("/requests", {
          requestTypeId: legalQuestion.id,
          departmentId: departments[0].id,
          title: `DOC-029r2 analysis-standin Request ${tag}`,
          description: `Please review the fictional services agreement with ${needle}. It is a DOC-029r2 walkthrough record.`,
          urgency: "low",
        }),
      );
      const request = {
      ...
      let filling;
      try {
        const body = { title: `DOC-029r2 analysis-standin Converted ${tag}` };
...
async function archivedNewPairWalk(role) {
  ...
  const number = (
    await h.ok(
      api.post("/contracts", {
        title: `DOC-029r2 analysis-standin Archived pair ${short(role)} ${stamp}`,
  ...
  await h.ok(
    api.upload(`/contracts/${number}/documents`, {
      ...
      buffer: Buffer.from("DOC-029r2 download-only primary placeholder"),
    }),
  );
  let doc = (
    await h.ok(
...
async function compareWalk(role) {
  ...
  const number = (
    await h.ok(
      api.post("/contracts", {
        title: `DOC-029r2 analysis-standin Compare ${tag}`,
        contractTypeId: otherType.id,
      }),
    )
  ).contract.number;
  ...
  const para = (days, cadence) => [
    "DOC-029r2 fictional payment terms.",
