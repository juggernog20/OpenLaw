...
- Turning on On intake form turns on Visible on Portal in the same write. The Portal switch then refuses an attempt to turn it off, with a reason beside the control. This follows #1012 and supersedes the proposed requirement to turn on Portal visibility first. Locked switches remain focusable with `aria-disabled` and an accessible explanation.
