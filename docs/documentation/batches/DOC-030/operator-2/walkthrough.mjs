...
function guideFailure(article, stepName, expected, observed) {
  log.guideFailures.push({ article, step: stepName, expected, observed, at: new Date().toISOString() });
  saveLog();
}
...
phases["support-up"] = async () => {
  await step(
    ...
    () => {
      ...
      out.push(`${MAIL_NAME}: Mailpit v1.30, UI 127.0.0.1:24601, joins each project's backend network as op2-mail`);
      ...
      out.push(`${AI_NAME}: OpenAI-compatible stand-in (not a live provider) that answers 200 only for the expected key, joins backend networks as op2-ai`);
...
phases["inst-prereq"] = async () => {
  ...
  await step(
    {
      ...INSTALL,
      action: "Before you start: check Git, OpenSSL, docker version, docker compose version and docker context show",
      command: "git --version; openssl version; docker version; docker compose version; docker context show",
      critical: true,
    },
    () => {
...
phases["inst-source"] = async () => {
  ...
  await step(
    ...
    () => {
      ...
      const clone = existsSync(p.dir) ? null : must(`git clone ${REPO} openlaw`, p.home, { timeout: 900_000 });
  ...
  await step(
    {
      ...INSTALL,
      action: "Prepare step 2: copy .env.example to .env with the grouped commands; a second run refuses to overwrite",
      command: envCommand,
      expected: "The grouped commands refuse to overwrite an existing .env and stop before changing its keys.",
      critical: true,
    },
    () => {
      ...
      check(mode === "600" && a?.length === 44 && k?.length === 44, `mode ${mode}, key lengths ${a?.length} ${k?.length}`);
      check(second.code !== 0 && hash1 === hash2, `second run exit ${second.code}, unchanged ${hash1 === hash2}`);
  ...
  await step(
    {
      ...INSTALL,
      action: "Prepare step 3: edit .env with origin, port, project name, proxy address (each setting once)",
      command:
    ...
    () => {
      ...
      const commentedInExample = ["BASE_URL", "PORT", "TRUSTED_PROXIES", "SETUP_TOKEN", "COMPOSE_PROJECT_NAME"].filter((k) =>
        new RegExp(`^#\\s*${k}=`, "m").test(example),
      );
  ...
  await step(
    { ...INSTALL, action: "Prepare step 4: create compose.operator.yml beside compose.yml", critical: true },
    () => {
      writeFileSync(
  ...
  await step(
    { ...INSTALL, action: "Prepare step 5: save both generated keys in the secret store, apart from backups", critical: true },
    () => {
      secrets.inst = { AUTH_SECRET: envGet(p, "AUTH_SECRET"), OPENLAW_SECRET_KEY: envGet(p, "OPENLAW_SECRET_KEY") };
      saveSecrets();
      const shellOverrides = Object.keys(process.env).filter((k) =>
        /^(COMPOSE_|BASE_URL$|PORT$|APP_BIND$|DATABASE_URL$|AUTH_SECRET$|OPENLAW_SECRET_KEY$|SMTP_)/.test(k),
      );
...
phases["inst-build"] = async () => {
  ...
  await step(
    ...
    () => {
      ...
      attachSupport(p);
      const ps = must("docker compose ps --format '{{.Service}} {{.State}}'", p.dir).stdout.trim().split("\n").sort();
      state.instBuildS = Math.round(build.ms / 1000);
      log.images.install = { app: `openlaw-local:${COMMIT} ${app}`, engine: `openlaw-engine-local:${COMMIT} ${eng}` };
  ...
  await step(
    {
      ...INSTALL,
      action: "Build step 3: curl --fail /readyz on the chosen port; worker running; document engine healthy",
    ...
    async () => {
      ...
      check(worker?.state === "running" && eng?.health === "healthy", `worker ${worker?.state}, engine ${eng?.health}`);
      state.instReadyAt = new Date().toISOString();
      const elapsed = Math.round((Date.parse(state.instReadyAt) - Date.parse(state.installStartedAt)) / 1000);
  ...
  await step(
    ...
    async () => {
      ...
      const lanIp = must("ip -4 -o addr show scope global | awk '{print $4}' | cut -d/ -f1 | head -1", root).stdout.trim();
...
function setupTokenFromLogs(p) {
  ...
  const at = logs.map((l, i) => (l.includes("First-run setup is open. Paste this setup token into the setup screen:") ? i : -1)).filter((i) => i >= 0);
  if (!at.length) return { token: null, count: 0, logs };
  const last = at[at.length - 1];
  const token = logs.slice(last + 1, last + 4).map((l) => l.trim()).find((l) => /^[A-Za-z0-9_-]{20,}$/.test(l));
  ...
  try {
    await step(
      ...
      async () => {
        ...
        const msg = "The setup token is missing or wrong. Copy it from the server log, or from SETUP_TOKEN.";
    ...
    await step(
      ...
      async () => {
        ...
        state.instSetupElapsedS = Math.round((Date.parse(state.instSetupAt) - Date.parse(state.installStartedAt)) / 1000);
    ...
    await step(
      {
        ...INSTALL,
        action: "Work through the welcome steps to Outbound email; save and test the relay; invite a colleague; Finish",
        expected: "Continue stays unavailable until email is configured; the test email and the invitation arrive; Finish enters the app.",
        critical: true,
      },
      async () => {
        ...
        const test = await waitMail((m) => toAddress(m, "avery.morgan@doc030-install.example") && m.Subject === "OpenLaw test email" && since(m, t0));
        ...
        await page.getByLabel("Email", { exact: true }).fill("rowan.operator@doc030-install.example");
        await page.getByRole("button", { name: "Legal team member", exact: true }).click();
        await page.getByRole("button", { name: "Send invite" }).click();
        const invite = await waitMail((m) => toAddress(m, "rowan.operator@doc030-install.example") && since(m, t1));
...
phases["inst-checks"] = async () => {
  ...
  try {
    await step(
      { ...INSTALL, role: "operator", action: "Check the working installation: sign in through the intended origin", critical: true },
      () => {
    ...
    await step(
      {
        ...INSTALL,
        action: "Create a fictional Contract, upload a small supported Document, download it again, check its processing result",
        critical: true,
      },
      async () => {
        const created = await createContractInBrowser(page, ORIGIN, "DOC-030 operator-2 install check contract");
        const body = pdf(["DOC-030 operator-2 install check.", "Processing check words."]);
        const up = await uploadInBrowser(page, ORIGIN, created.number, "doc030-install-check.pdf", body);
        const got = await s.request("GET", `/api/v1/documents/${up.documentId}/versions/${up.versionId}/download`);
        ...
        check(text.state === "ready" && /Processing check words/.test(text.text), `text ${text.state}`);
  ...
  await step(
    { ...INSTALL, action: "Check the working installation: the test invitation's link is usable", critical: false },
    async () => {
      const msg = await (await import("./op-lib.mjs")).mailMessages().then((all) => all.find((m) => m.ID === state.instInviteId));
      ...
      const b = await browserSignIn(ORIGIN, "rowan.operator@doc030-install.example", password("instColleague"));
  ...
  await step(
    { ...INSTALL, action: "Check the working installation: the worker processes new work after a restart", command: "docker compose restart worker" },
    async () => {
      ...
      try {
        ...
        const up = await uploadInBrowser(s2.page, ORIGIN, state.instContract.number, "doc030-install-after-restart.pdf", body);
  ...
  await step(
    { ...INSTALL, action: "Record the source revision, resolved images, project name, origin and storage location" },
    () => {
      const rev = must("git rev-parse HEAD", p.dir).stdout.trim();
      const vol = must(`docker volume inspect ${p.project}_openlaw-files --format '{{.Name}} {{.Mountpoint}}'`, root).stdout.trim();
...
phases["inst-trust"] = async () => {
  ...
  async function probe(label) {
    ...
    const wrong = [];
    for (let i = 0; i < 4; i += 1)
      wrong.push((await viaProxy("POST", "/api/auth/sign-in/email", { json: { email: "nobody@doc030-install.example", password: "not-the-password" }, localAddress: "127.0.0.2" })).status);
    const right = await viaProxy("POST", "/api/auth/sign-in/email", { json: { email: admin, password: password("instAdmin") }, localAddress: "127.0.0.1" });
    const seen = lines(
      ...
      .filter(Boolean);
    return { label, wrongFrom127_0_0_2: wrong, rightFrom127_0_0_1: right.status, remoteAddressesTheAppLogged: [...new Set(seen)] };
  }
  await step(
    {
      ...INSTALL,
      action: "TRUSTED_PROXIES=127.0.0.1,::1 with a proxy on the same host gives each client its own sign-in bucket",
      expected:
  ...
  await step(
    {
      ...
      action: "Control: TRUSTED_PROXIES set to the backend network gateway address the app actually sees",
    },
    async () => {
      const gw = must(`docker network inspect ${p.project}_openlaw-backend --format '{{(index .IPAM.Config 0).Gateway}}'`, root).stdout.trim();
...
phases["inst-negatives"] = async () => {
  ...
  await step(
    {
      ...INSTALL,
      action: "Negative: missing secret (blank AUTH_SECRET) is refused by Compose; the retained key recovers",
      expected: "Troubleshooting row: Compose refuses the command when either key is missing.",
    },
    async () => {
      const original = envGet(p, "AUTH_SECRET");
      const before = psAll(p).map((r) => r.id).sort().join();
      envSet(p, "AUTH_SECRET", "");
      const q = sh("docker compose config --quiet", p.dir);
      const u = sh("docker compose up -d --no-build --pull never", p.dir);
      const after = psAll(p).map((r) => r.id).sort().join();
      envSet(p, "AUTH_SECRET", original);
      const q2 = sh("docker compose config --quiet", p.dir);
      const ready = await waitReady(p).then(() => 200).catch(() => 0);
      check(q.code !== 0 && u.code !== 0 && before === after && q2.code === 0 && ready === 200, `config ${q.code} up ${u.code} unchanged ${before === after} config2 ${q2.code} ready ${ready}`);
  ...
  await step(
    {
      ...INSTALL,
      action: "Negative: occupied port leads to the documented recovery (ps --all shows created; choose an unused port)",
      expected:
    ...
    async () => {
      ...
      const plain = sh("docker compose ps --format '{{.Service}} {{.State}}'", p.dir).stdout.trim().split("\n").filter(Boolean);
      const down = await http(`http://127.0.0.1:${p.port}/readyz`, { timeoutMs: 3000 });
      const other = await http(`http://127.0.0.1:${OCCUPIED_PORT}/`);
      const err = lines(u.stderr, /allocated|already in use|bind/i, 1).map((l) => l.replace(/[0-9a-f]{12,}/g, "…"));
      envSet(p, "PORT", String(p.port));
      const u2 = sh("docker compose up -d --no-build --pull never", p.dir, { timeout: 300_000 });
      const ready = await waitReady(p).then(() => 200).catch(() => 0);
      check(
        u.code !== 0 && all.includes("app created") && all.includes("worker created") && !plain.some((l) => /^(app|worker) /.test(l)) && down.status !== 200 && other.status === 200 && u2.code === 0 && ready === 200,
        JSON.stringify({ u: u.code, all, plain, down: down.status, other: other.status, u2: u2.code, ready }),
      );
      return { upExit: u.code, error: err, psAll: all, plainPs: plain, readyzWhileFailed: down.status, unrelatedListener: other.status, recovery: `PORT=${p.port} restored; same up exit ${u2.code}; readyz ${ready}` };
    },
  );
  await step(
    {
      ...INSTALL,
      action: "Negative: unavailable dependency (database stopped) fails readiness; restoring it recovers",
      expected: "Troubleshooting row: /healthz works but /readyz fails; restore connectivity, then verify a record read and write.",
    },
    async () => {
      ...
      const write = await s.request("PATCH", `/api/v1/contracts/${state.instContract.number}`, { description: `DOC-030 operator-2 write check ${new Date().toISOString()}` });
      await s.context.close();
      void api;
      check(h.status === 200 && r.status !== 200 && read.status === 200 && write.status === 200, `healthz ${h.status} readyz ${r.status} read ${read.status} write ${write.status}`);
...
function keepKeys(p) {
  secrets[p.name] = { ...(secrets[p.name] ?? {}), AUTH_SECRET: envGet(p, "AUTH_SECRET"), OPENLAW_SECRET_KEY: envGet(p, "OPENLAW_SECRET_KEY") };
  saveSecrets();
}
...
const U = PROJECTS.up;
const PEOPLE = {
  admin: { email: "morgan.baseline@doc030-upgrade.example", displayName: "DOC-030 operator-2 Morgan Baseline" },
  reader: { email: "rowan.reader@doc030-upgrade.example", displayName: "DOC-030 operator-2 Rowan Reader", role: "legal_team_member" },
  contributor: { email: "casey.contributor@doc030-upgrade.example", displayName: "DOC-030 operator-2 Casey Contributor", role: "contributor" },
};

phases["up-baseline"] = async () => {
  const p = U;
  await step(
    { ...UPGRADE, action: "Fixture: install the starting build d1d098ba with the installation guide's files", critical: true },
    () => installFiles(p, BASELINE).join("; "),
  );
  await step(
    { ...UPGRADE, action: "Fixture: build and start the starting build; readiness", command: "docker compose config --quiet; docker compose build app doc-engine; docker compose up -d --no-build --pull never", critical: true },
    async () => {
      keepKeys(p);
...
phases["up-populate"] = async () => {
  ...
  await step(
    { ...UPGRADE, method: "automated-test", action: "Fixture: first Administrator (no setup token on this build) and a saved relay with a password", critical: true },
    async () => {
      const setup = await api.get("/api/v1/auth/setup");
      if (setup.needsSetup) await api.post("/api/v1/auth/setup", { ...PEOPLE.admin, password: password("upAdmin") }, { accept: [201] });
      ...
      const mail = await waitMail((m) => toAddress(m, PEOPLE.admin.email) && m.Subject === "OpenLaw test email" && since(m, t0));
  ...
  await step(
    { ...UPGRADE, method: "automated-test", action: "Fixture: invite and activate a Legal Team Member and a Contributor", critical: true },
    async () => {
      const out = [];
      for (const key of ["reader", "contributor"]) {
        const person = PEOPLE[key];
        const t0 = Date.now();
        const created = await api.post("/api/v1/auth/invites", { email: person.email, displayName: person.displayName, role: person.role });
        ...
        await new Api(p.base).post("/api/auth/reset-password", { newPassword: password(`up-${key}`), token });
  ...
  await step(
    { ...UPGRADE, method: "automated-test", action: "Fixture: Entity, a Contract Field, a global Field on a Contract type and a Matter type, three Contracts (one Confidential), a Matter", critical: true },
    async () => {
      const entityTypes = (await api.get("/api/v1/entity-types")).entityTypes;
      const entity = (await api.post("/api/v1/entities", { legalName: "DOC-030 operator-2 Holdings Ltd", entityTypeId: entityTypes[0].id, jurisdiction: "England and Wales", status: "active" })).entity;
      ...
      const cfield = (await api.post("/api/v1/fields", { displayName: "DOC-030 operator-2 reference", moduleScope: "contract", fieldType: "text", fieldTag: "business" })).field;
      const gfield = (await api.post("/api/v1/fields", { displayName: "DOC-030 operator-2 shared code", moduleScope: "global", fieldType: "text", fieldTag: "business" })).field;
      await api.post(`/api/v1/contract-types/${ctype.id}/fields`, { fieldId: cfield.id, isRequired: false });
      await api.post(`/api/v1/contract-types/${ctype.id}/fields`, { fieldId: gfield.id, isRequired: false });
      await api.post(`/api/v1/matter-types/${mtype.id}/fields`, { fieldId: gfield.id, isRequired: false });
      ...
      for (const [title, confidential] of [
        ...
        const created = (await api.post("/api/v1/contracts", { title, contractTypeId: ctype.id, isConfidential: confidential })).contract;
        const updated = (
          await api.patch(`/api/v1/contracts/${created.number}`, {
            entityId: entity.id,
            customFields: { [cfield.slug]: `ref-${created.number}`, [gfield.slug]: `shared-C${created.number}` },
            isConfidential: confidential,
          })
        ).contract;
        inv.contracts.push({ number: updated.number, id: updated.id, title: updated.title, isConfidential: updated.isConfidential, entity: updated.entity?.legalName ?? null, customFields: updated.customFields });
      }
      const matter = (await api.post("/api/v1/matters", { title: "DOC-030 operator-2 regulatory advice", matterTypeId: mtype.id })).matter;
      const m2 = (await api.patch(`/api/v1/matters/${matter.number}`, { customFields: { [gfield.slug]: `shared-M${matter.number}` } })).matter;
      inv.matters.push({ number: matter.number, id: matter.id, title: matter.title, customFields: m2.customFields });
      return { contracts: inv.contracts.map((c) => `${c.number} confidential=${c.isConfidential} values=${JSON.stringify(c.customFields)}`), matter: `${matter.number} ${JSON.stringify(m2.customFields)}`, globalField: gfield.slug };
    },
  );
  await step(
    { ...UPGRADE, method: "automated-test", action: "Fixture: two Documents with original and later Versions (one PDF), Contributor membership", critical: true },
    async () => {
      const [first, , confidential] = inv.contracts;
      const d1 = (await api.upload(`/api/v1/contracts/${first.number}/documents`, "doc030-op2-draft.txt", Buffer.from("DOC-030 operator-2 draft, original version.\n"), "text/plain")).document;
      await api.upload(`/api/v1/documents/${d1.id}/versions`, "doc030-op2-draft-v2.txt", Buffer.from("DOC-030 operator-2 draft, later version.\n"), "text/plain");
      const d2 = (await api.upload(`/api/v1/contracts/${confidential.number}/documents`, "doc030-op2-settlement.pdf", pdf(["DOC-030 operator-2 settlement, original.", "The parties settle."]), "application/pdf")).document;
      await api.upload(`/api/v1/documents/${d2.id}/versions`, "doc030-op2-settlement-v2.pdf", pdf(["DOC-030 operator-2 settlement, later.", "The parties settle in full."]), "application/pdf");
      for (const [contract, doc] of [[first, d1], [confidential, d2]]) {
        const listed = (await api.get(`/api/v1/contracts/${contract.number}/documents`)).documents.find((row) => row.id === doc.id);
        const versions = [];
        for (const v of listed.versions) {
          ...
          versions.push({ id: v.id, versionNumber: v.versionNumber, sha256: got.sha256, textState: text.state });
        }
        inv.documents.push({ id: doc.id, contract: contract.number, title: listed.title, versions });
      }
      await api.post(`/api/v1/contracts/${first.number}/team`, { userId: inv.people.contributor.id, role: "contributor" }, { accept: [201] });
      return inv.documents.map((d) => `${d.title}: ${d.versions.map((v) => `v${v.versionNumber} ${v.sha256.slice(0, 12)} ${v.textState}`).join(", ")}`);
    },
  );
  state.people = PEOPLE;
  saveState();
...
phases["up-populate2"] = async () => {
  ...
  await step(
    { ...UPGRADE, method: "automated-test", action: "Fixture: AI connector (custom endpoint stand-in) with a saved key; its test passes on the starting build", critical: true },
    async () => {
      const r = await api.put("/api/v1/ai-connector", { preset: "custom", protocol: "openai_chat_completions", baseUrl: "http://op2-ai:8080/v1", apiKey: secrets.ai.key, model: "doc030-standin" });
  ...
  await step(
    { ...UPGRADE, method: "automated-test", action: "Fixture: record baseline reach, Request types and schema facts", critical: true },
    async () => {
      const reach = {};
      for (const key of ["reader", "contributor"]) {
        ...
        reach[key] = {};
        for (const c of inv.contracts) reach[key][c.number] = (await s.raw("GET", `/api/v1/contracts/${c.number}`)).status;
      }
      inv.baselineReach = reach;
      const conf = inv.contracts.find((c) => c.isConfidential).number;
      check(reach.reader[conf] !== 200 && reach.contributor[conf] !== 200, "Confidential reachable at baseline");
      ...
      inv.boColumn = psql(p, "select count(*) from information_schema.columns where table_name='contracts' and column_name='business_owner_id'");
      inv.journalRows = psql(p, "select count(*) from drizzle.__drizzle_migrations");
      saveState();
      return { reach, requestTypes: inv.requestTypes, requestTypeFields: inv.requestTypeFields, contractsBusinessOwnerColumn: inv.boColumn, journalRows: inv.journalRows };
    },
  );
  state.people = PEOPLE;
  saveState();
...
async function runBackup(p, backupDir, { meta, restart, pre }) {
  ...
  await step({ ...meta, action: "Backup step 1: create a fresh private backup directory", command: 'BACKUP_DIR=...; umask 077; mkdir -p "$BACKUP_DIR"; chmod 700 "$BACKUP_DIR"', critical: true }, () => {
  ...
  await step({ ...meta, action: "Backup step 2: stop app and worker, keep Postgres running", command: "docker compose stop app worker; docker compose ps --all", critical: true }, () => {
    must("docker compose stop app worker", p.dir, { timeout: 180_000 });
    const s = Object.fromEntries(psAll(p).map((r) => [r.service, r.state]));
    check(s.app === "exited" && s.worker === "exited" && s.postgres === "running", JSON.stringify(s));
    const rows = sh("docker compose exec -T postgres psql -U openlaw -d openlaw -At -c 'select count(*) from runtime_status'", p.dir);
    return { ...s, runtimeStatusRowsAfterStop: rows.code === 0 ? rows.stdout.trim() : "table absent on this build" };
  });
  await step({ ...meta, action: "Backup step 3: dump the database, archive the file volume, record source and images", command: 'pg_dump --format=custom > "$BACKUP_DIR/database.dump"; docker compose run -T --rm --no-deps app tar -czf - -C /var/lib/openlaw/files . > "$BACKUP_DIR/files.tar.gz"; git rev-parse HEAD > app-source.txt; docker compose images --format json > images.json', critical: true }, () => {
    const out = [];
    ...
    if (pre) writeFileSync(path.join(backupDir, "pre-upgrade-revision.txt"), `${pre}\n`, { mode: 0o600 });
  ...
  await step({ ...meta, action: "Backup step 4: list both archives and record hashes", command: "pg_restore --list; tar -tzf; sha256sum > SHA256SUMS", critical: true }, () => {
    must('docker compose exec -T postgres pg_restore --list < "$BACKUP_DIR/database.dump" > "$BACKUP_DIR/database-contents.txt"', p.dir, { env });
    must('tar -tzf "$BACKUP_DIR/files.tar.gz" > "$BACKUP_DIR/file-contents.txt"', p.dir, { env });
    must('(cd "$BACKUP_DIR" && sha256sum database.dump files.tar.gz > SHA256SUMS)', p.dir, { env });
    const entries = readFileSync(path.join(backupDir, "database-contents.txt"), "utf8").split("\n").filter((l) => l && !l.startsWith(";")).length;
    const files = readFileSync(path.join(backupDir, "file-contents.txt"), "utf8").split("\n").filter((l) => l && !l.endsWith("/")).length;
  ...
  if (!restart) return;
  await step({ ...meta, action: "Backup step 5: restart app and worker; confirm readiness and an actual operation", command: "docker compose up -d --no-build --pull never", critical: true }, async () => {
  ...
  await step({ ...meta, action: "Copy the backup to its retained location and verify its hashes there", critical: true }, () => {
    const retained = `${backupDir}-retained`;
    must(`umask 077; mkdir -p "${retained}"; cp -p "$BACKUP_DIR"/* "${retained}/"`, p.dir, { env });
...
phases.upgrade = async () => {
  ...
  await step(
    { ...UPGRADE, action: "Before you start: record source revision, image identities, project, file list, storage configuration and Settings → Advanced", critical: true },
    async () => {
      const rev = must("git rev-parse HEAD", p.dir).stdout.trim();
      ...
      return {
        ...
        storage: { STORAGE_DRIVER: env.STORAGE_DRIVER || "(unset: local)", STORAGE_PATH: env.STORAGE_PATH },
  ...
  await old.page.waitForLoadState("networkidle").catch(() => {});
  await step(
    { ...UPGRADE, action: "Prepare step 1: fetch the source and inspect local changes", command: "git status --short; git fetch origin; git rev-parse HEAD", critical: true },
    () => {
  ...
  await step(
    { ...UPGRADE, action: "Prepare step 2: select the target, update OPENLAW_BUILD_COMMIT, validate and build without recreating the running containers", command: `git checkout --detach ${COMMIT}; docker compose config --quiet; docker compose build app doc-engine`, critical: true },
    () => {
      must(`git checkout --detach ${COMMIT}`, p.dir);
      ...
      const b = must("docker compose build app doc-engine", p.dir, { timeout: 3_000_000 });
      const after = psAll(p);
      const unchanged = state.upgradeBefore.containers.every((x) => after.some((a) => a.id === x.id && a.state === x.state));
      check(unchanged, "containers changed during the build");
      const services = JSON.parse(must("docker compose config --format json", p.dir).stdout).services;
      check(services.app.image === services.worker.image, "app and worker resolve to different images");
      log.images.upgradeTarget = { app: `${services.app.image} ${inspectImage(services.app.image)}`, engine: `${services.engine?.image ?? services["doc-engine"].image} ${inspectImage(services["doc-engine"].image)}`, postgres: services.postgres.image };
  ...
  await step(
    { ...UPGRADE, action: "Before you start: check outstanding signing and processing work before the pause", command: "psql: pg-boss job states; contract_envelopes by status" },
    () => {
      const jobs = psql(p, "select state, count(*) from pgboss.job group by state order by state").replace(/\n/g, "; ");
      const env = sh(`docker compose exec -T postgres psql -U openlaw -d openlaw -At -c "select status, count(*) from contract_envelopes group by status"`, p.dir);
  ...
  await runBackup(p, backupDir, { meta: { ...UPGRADE, article: "upgrade (step 4 calls backup-and-restore)" }, restart: false, pre: BASELINE });
  state.preUpgradeBackup = backupDir;
  saveState();
  await step(
    {
      ...UPGRADE,
      action: "Start step 1: start the target with the retained project and volumes; ps; port; logs",
      command: "docker compose up -d --no-build --pull never; docker compose ps; docker compose port app 3000; docker compose logs --tail=100 app worker",
      expected: "The first app migration line is 'migrations: reconciled 0090_onboarding_reviewed_types; continuing with pending migrations'; the worker can stop on a missing column or table and Compose restarts it; port 127.0.0.1:3000 form by default.",
      critical: true,
    },
    async () => {
      const t0 = Date.now();
      must("docker compose up -d --no-build --pull never", p.dir);
      const ps0 = must("docker compose ps --format '{{.Service}} {{.State}} {{.Status}}'", p.dir).stdout.trim().split("\n");
      const port = must("docker compose port app 3000", p.dir).stdout.trim();
      ...
      const wstate = must(`docker inspect --format '{{.RestartCount}} {{.State.Status}} {{.State.StartedAt}}' ${workerId}`, p.dir).stdout.trim();
      const workerErrors = lines(wlogs, /advanced_settings|runtime_status|does not exist|column|relation/i, 3);
      const journal = psql(p, "select count(*), max(created_at) from drizzle.__drizzle_migrations");
      check(firstMig.includes("migrations: reconciled 0090_onboarding_reviewed_types; continuing with pending migrations"), `first migration line: ${firstMig.slice(0, 200)}`);
      check(port === `127.0.0.1:${p.port}`, `port ${port}`);
      check(wstate.split(" ")[1] === "running", `worker ${wstate}`);
      recordImages("upgrade-after", p);
      return { psRightAfterUp: ps0, port, readyAfterSeconds: readyS, firstMigrationLine: firstMig.trim().slice(0, 160), workerRestartCountAndState: wstate, workerErrorLinesMatchingSchema: workerErrors, journalRowsAndLatest: `${state.inventory.journalRows} before; ${journal} after`, tail100Lines: tail.split("\n").length, errorLikeLinesInTail: tail.split("\n").filter((l) => /"level":50|error/i.test(l)).length };
    },
  );
  await step(
    { ...UPGRADE, role: "administrator", method: "browser-walkthrough", action: "Start step 5: a browser tab opened before the upgrade, used after it", expected: "An old tab can show This part of OpenLaw was updated. Reload to continue." },
    async () => {
      const pageErrors = [];
      ...
      for (const name of ["Matters", "Entities", "Knowledge", "Documents", "Auto-Docs"]) {
        await old.page.getByRole("link", { name, exact: true }).first().click().catch(() => {});
        await old.page.waitForTimeout(2500);
        const text = await bodyText(old.page);
        seen.push(`${name}: ${text.includes("This part of OpenLaw was updated. Reload to continue.") ? "notice shown" : new URL(old.page.url()).pathname}`);
...
async function textWithValues(page) {
  return page.evaluate(() => {
    const values = [...document.querySelectorAll("input, textarea, select")].map((el) => (el.tagName === "SELECT" ? el.options[el.selectedIndex]?.textContent : el.value));
  ...
  for (const [name, route] of [
    ...
    out[name] = await page.evaluate(() =>
      [...document.querySelectorAll("form label")].map((label) => {
        ...
        const value = input ? (input.tagName === "SELECT" ? input.options[input.selectedIndex]?.textContent : input.type === "password" ? "(secret)" : input.value) : "";
...
async function systemStatus(page, base) {
  ...
  return {
    ...
    warning: text.includes("An API or worker heartbeat is missing") ? "An API or worker heartbeat is missing. Check that both services are running." : null,
  };
}
function processRowsOk(status) {
  const api = status.rows.filter((r) => /^API /.test(r));
  const worker = status.rows.filter((r) => /^Worker /.test(r));
  return api.length > 0 && worker.length > 0 && [...api, ...worker].every((r) => r.includes("Running") && r.includes("Current"));
}

async function verifyInstance(p, { meta, label, adminPw, people, modern = true, fieldSplit = true, skipAi = false, only = null }) {
  const want = (k) => !only || only.includes(k);
  const inv = state.inventory;
  const conf = inv.contracts.find((c) => c.isConfidential);
  const admin = await browserSignIn(p.base, people.admin.email, adminPw);
  try {
    if (want("records")) await step({ ...meta, role: "administrator", method: "browser-walkthrough", action: `${label}: Administrator signs in through the origin; representative Contracts, Matter and Entity match the inventory`, critical: true }, async () => {
      ...
      for (const c of inv.contracts) {
        ...
        await admin.page.getByRole("heading", { name: c.title }).first().waitFor({ state: "attached", timeout: 20_000 });
        const r = await admin.request("GET", `/api/v1/contracts/${c.number}`);
        const row = r.body.contract;
        check(row.isConfidential === c.isConfidential && (row.entity?.legalName ?? null) === c.entity, `Contract ${c.number}`);
        check(row.customFields?.[inv.field.slug] === c.customFields[inv.field.slug], `reference Field on ${c.number}: ${JSON.stringify(row.customFields)}`);
        out.contracts.push(`C-${c.number} heading shown; Confidential ${row.isConfidential}; Entity ${row.entity?.legalName}; ${inv.field.displayName} ${row.customFields[inv.field.slug]}`);
      }
      const m = inv.matters[0];
      await admin.page.goto(`${p.base}/matters/${m.number}`);
      await admin.page.getByRole("heading", { name: m.title }).first().waitFor({ state: "attached", timeout: 20_000 });
      const e = await admin.request("GET", `/api/v1/entities/${inv.entity.id}`);
      check(e.status === 200 && e.body.entity.legalName === inv.entity.legalName, `Entity ${e.status}`);
      out.matter = `M-${m.number} heading shown`;
      out.entity = `${inv.entity.legalName} read 200`;
      return out;
    });
    if (fieldSplit && want("fields"))
      await step({ ...meta, role: "administrator", method: "browser-walkthrough", action: `${label}: a Field several modules shared appears once in each module with the same name and saved values` }, async () => {
        const g = inv.globalField;
        const rows = psql(p, `select slug, module_scope from fields where display_name = '${g.displayName}' order by module_scope`).split("\n");
        const m = inv.matters[0];
        const matter = (await admin.request("GET", `/api/v1/matters/${m.number}`)).body.matter;
        const contractValues = [];
        for (const c of inv.contracts) contractValues.push((await admin.request("GET", `/api/v1/contracts/${c.number}`)).body.contract.customFields[g.slug]);
        ...
        const matterValue = Object.entries(matter.customFields ?? {}).find(([k]) => k.startsWith(g.slug));
        check(rows.length === 2 && rows.some((r) => r.endsWith("|contract") && r.startsWith(`${g.slug}|`)) && rows.some((r) => r.endsWith("|matter")), `fields ${rows}`);
        check(matterValue?.[1] === `shared-M${m.number}` && contractValues.every((v, i) => v === `shared-C${inv.contracts[i].number}`), `values ${JSON.stringify(matterValue)} ${contractValues}`);
        check(mText.includes(g.displayName) && mText.includes(`shared-M${m.number}`) && cText.includes(g.displayName), "Fields tabs do not show the name and value");
    ...
    if (want("downloads")) await step({ ...meta, role: "administrator", action: `${label}: download original and later Document Versions and compare hashes with the inventory`, critical: true }, async () => {
      const out = [];
      for (const d of inv.documents)
        for (const v of d.versions) {
          const r = await admin.request("GET", `/api/v1/documents/${d.id}/versions/${v.id}/download`);
    ...
    if (want("upload")) await step({ ...meta, role: "administrator", method: "browser-walkthrough", action: `${label}: a new upload in the browser is processed by the worker` }, async () => {
      const body = pdf([`DOC-030 operator-2 new upload on ${label}.`, "Worker processing check."]);
      const up = await uploadInBrowser(admin.page, p.base, inv.contracts[1].number, `doc030-op2-${p.name}-${Date.now()}.pdf`, body);
      const text = await waitText({ raw: (m, r) => admin.request(m, r) }, up.documentId, up.versionId, 300_000);
      check(text.state === "ready" && /Worker processing check/.test(text.text), `text ${text.state}`);
      state.newUploads = { ...(state.newUploads ?? {}), [p.name]: { id: up.documentId, contract: inv.contracts[1].number } };
      return `${up.how}; processing ready with the uploaded words`;
    });
    if (want("email")) await step({ ...meta, role: "administrator", method: "browser-walkthrough", action: `${label}: an actual use of the saved encrypted relay: Send test email` }, async () => {
      const t0 = Date.now();
      if (!modern) {
      ...
      const mail = await waitMail((m) => toAddress(m, people.admin.email) && m.Subject === "OpenLaw test email" && since(m, t0));
      const page = (await bodyText(admin.page)).match(/(Test email sent[^.]*\.|The test email could not be sent[^.]*\.)/)?.[0];
    ...
    if (!skipAi && want("ai"))
      await step({ ...meta, role: "administrator", method: "browser-walkthrough", action: `${label}: Test connection on the configured AI connector (stand-in endpoint)` }, async () => {
        if (!modern) {
        ...
        await admin.page.getByRole("button", { name: "Test connection" }).first().click({ timeout: 20_000 });
        await admin.page.getByText(/Connection successful\.|The connection test failed/).first().waitFor({ timeout: 30_000 });
        const text = (await bodyText(admin.page)).match(/Connection successful\.|The connection test failed[^.]*\.[^.]*\./)?.[0];
    ...
    if (modern) {
      ...
      if (want("advanced")) await step({ ...meta, role: "administrator", method: "browser-walkthrough", action: `${label}: Settings → Advanced fields and their sources` }, async () => advancedSources(admin.page, p.base));
    }
  } finally {
    await admin.context.close();
  }
  for (const key of want("lower") ? ["reader", "contributor"] : []) {
    await step({ ...meta, role: key === "reader" ? "legal_team_member" : "lower-access", method: "browser-walkthrough", action: `${label}: lower-access account (${key}) signs in; a Confidential record outside its audience stays refused` }, async () => {
      const s = await browserSignIn(p.base, people[key].email, password(`up-${key}`));
      try {
        const reach = {};
        for (const c of inv.contracts) reach[c.number] = (await s.request("GET", `/api/v1/contracts/${c.number}`)).status;
        const portal = s.role === "business_user" ? (await s.request("GET", `/api/v1/portal/contracts/${conf.number}`)).status : null;
        const cd = inv.documents.find((d) => d.contract === conf.number);
        const dl = (await s.request("GET", `/api/v1/documents/${cd.id}/versions/${cd.versions[0].id}/download`)).status;
        await s.page.goto(`${p.base}/contracts/${conf.number}`);
        await s.page.waitForLoadState("networkidle").catch(() => {});
        const visible = await s.page.getByText(conf.title).first().isVisible().catch(() => false);
        const team = psql(p, `select count(*) from contract_team t join contracts c on c.id = t.contract_id where c.number = ${conf.number} and t.user_id = '${inv.people[key].id}'`);
        check(reach[conf.number] !== 200 && dl !== 200 && !visible && (portal === null || portal !== 200), JSON.stringify({ reach, dl, visible, portal }));
        return { role: s.role, landed: s.landed, baselineReach: inv.baselineReach[key], reachNow: reach, portalConfidential: portal, confidentialDownload: dl, confidentialTitleVisible: visible, teamMemberOfConfidential: team, pageText: (await bodyText(s.page)).slice(0, 120) };
      } finally {
        await s.context.close();
      }
    });
  }
  if (want("jobs")) await step({ ...meta, action: `${label}: inspect outstanding jobs and Envelopes`, command: "psql: pg-boss job states; contract_envelopes" }, () => {
    const jobs = psql(p, "select state, count(*) from pgboss.job group by state order by state").replace(/\n/g, "; ");
    const env = sh(`docker compose exec -T postgres psql -U openlaw -d openlaw -At -c "select status, count(*) from contract_envelopes group by status"`, p.dir);
...
phases["up-verify"] = async (only) => {
  const p = U;
  await verifyInstance(p, { meta: UPGRADE, label: "after upgrade", adminPw: password("upAdmin"), people: state.people, only: only ? only.split(",") : null });
  await step({ ...UPGRADE, action: "after upgrade: the data rewrites the guide lists", command: "psql reads" }, () => {
    const inv = state.inventory;
    const types = psql(p, "select dv.version_number || ' ' || coalesce(dt.display_name, '(none)') from document_versions dv left join document_types dt on dt.id = dv.document_type_id order by dv.created_at limit 4").replace(/\n/g, "; ");
    ...
    const backfill = psql(p, "select count(*) from activity_log where payload->>'reason' = 'Business Owner membership backfill'");
...
async function restoreInto(p, backupDir, revision, { meta, keysFrom, extra = {} }) {
  const env = { BACKUP_DIR: backupDir };
  await step({ ...meta, action: `Restore step 1: prepare source ${revision.slice(0, 8)} and its images, retained keys, distinct project, origin and port; no first-run setup`, critical: true }, () => {
    ...
    must("docker compose config --quiet", p.dir);
    const b = must("docker compose build app doc-engine", p.dir, { timeout: 3_000_000 });
    const recorded = JSON.parse(readFileSync(path.join(backupDir, "images.json"), "utf8")).find((i) => i.ContainerName?.includes("-app-"));
    const image = JSON.parse(must("docker compose config --format json", p.dir).stdout).services.app.image;
  ...
  await step({ ...meta, action: "Restore step 2: verify the hashes on the target host and start Postgres", command: '(cd "$BACKUP_DIR" && sha256sum --check SHA256SUMS); docker compose up -d postgres', critical: true }, () => {
    const c = must('(cd "$BACKUP_DIR" && sha256sum --check SHA256SUMS)', p.dir, { env }).stdout.trim().replace(/\n/g, "; ");
    const u = upCommand(p, "docker compose up -d postgres");
    check(u.code === 0, u.stderr.slice(-400));
    return c;
  });
  await step({ ...meta, action: "Confirm the target's database and file store are empty before importing", critical: true }, async () => {
    let tables = "";
    for (let i = 0; i < 60; i += 1) {
      const r = sh(`docker compose exec -T postgres psql -U openlaw -d openlaw -At -c "select count(*) from information_schema.tables where table_schema not in ('pg_catalog','information_schema')"`, p.dir);
      if (r.code === 0) {
    ...
    const files = must("docker compose run -T --rm --no-deps app sh -c 'find /var/lib/openlaw/files -mindepth 1 | wc -l'", p.dir).stdout.trim();
  ...
  await step({ ...meta, action: "Restore step 3: restore the database with --exit-on-error", command: 'docker compose exec -T postgres pg_restore -U openlaw -d openlaw --exit-on-error --no-owner --no-privileges < "$BACKUP_DIR/database.dump"', critical: true }, () => {
    const r = sh('docker compose exec -T postgres pg_restore -U openlaw -d openlaw --exit-on-error --no-owner --no-privileges < "$BACKUP_DIR/database.dump"', p.dir, { env, timeout: 600_000 });
  ...
  await step({ ...meta, action: "Restore step 4: restore the local files through the target app service's volume", command: 'docker compose run -T --rm --no-deps app tar -xzf - -C /var/lib/openlaw/files < "$BACKUP_DIR/files.tar.gz"', critical: true }, () => {
    must('docker compose run -T --rm --no-deps app tar -xzf - -C /var/lib/openlaw/files < "$BACKUP_DIR/files.tar.gz"', p.dir, { env, timeout: 600_000 });
    const files = must("docker compose run -T --rm --no-deps app sh -c 'find /var/lib/openlaw/files -type f | wc -l'", p.dir).stdout.trim();
    return `tar exit 0; files in the target volume ${files}`;
  });
  await step({ ...meta, action: "Restore step 5: start the target and check readiness (controlled relay and AI stand-in reachable under their saved names first)", command: "docker compose up -d --no-build --pull never; docker compose ps", critical: true }, async () => {
    const u = upCommand(p);
    check(u.code === 0, u.stderr.slice(-400));
    attachSupport(p);
    await waitReady(p, 600_000);
    return { readyz: 200, ps: psAll(p).map((r) => `${r.service} ${r.state}`), images: recordImages(`${p.name}-start`, p).containers };
  });
}

phases.recover = async () => {
  const p = PROJECTS.recover;
  const meta = { ...UPGRADE, article: "upgrade (If the upgrade cannot be accepted) + backup-and-restore" };
...
phases["recover-verify"] = async () => {
  const p = PROJECTS.recover;
  const meta = { ...UPGRADE, article: "upgrade (If the upgrade cannot be accepted) + backup-and-restore" };
  await verifyInstance(p, { meta, label: "recovery target", adminPw: password("upAdmin"), people: state.people, modern: false, fieldSplit: false, only: process.argv[3] ? process.argv[3].split(",") : null });
  if (process.argv[3]) return;
  await step({ ...meta, action: "recovery target: work accepted after the backup is absent; the target runs the starting build; the migrated database was never opened by an older image" }, async () => {
    const api = new Api(p.base);
    await api.signIn(state.people.admin.email, password("upAdmin"));
...
phases["br-prepare"] = async () => {
  ...
  try {
    await step({ ...BACKUP, role: "administrator", method: "browser-walkthrough", action: "Fixture through the app: an Administrator saves Settings → Advanced → File uploads → Maximum file size (MiB) = 150", critical: true }, async () => {
      ...
      return { savedNotice: "Settings saved. Restart the API and worker to apply changes.", sourceLabel: hint, systemStatusBeforeRestart: before, afterRestartAppWorker: after };
    });
    await step({ ...BACKUP, role: "administrator", method: "browser-walkthrough", action: "Before you start: record every Settings → Advanced field that shows Saved in OpenLaw, the source revision, images, database major version, project and storage", critical: true }, async () => {
      ...
      const saved = Object.values(sources).flat().filter((l) => l.includes("[Saved in OpenLaw]"));
      check(saved.length === 1 && saved[0].startsWith("Maximum file size (MiB) = 150"), JSON.stringify(saved));
      ...
      state.sourceRecord = { saved, revision: must("git rev-parse HEAD", p.dir).stdout.trim(), pg };
      return { savedInOpenLaw: saved, sourceRevision: state.sourceRecord.revision, images: recordImages("backup-source", p).containers, postgres: pg, project: p.project, storage: sources["Document storage"], sealedVapidPrivateKeyInDatabase: vapid === "t", vapidInEnv: Boolean(envVapid), keysInSecretStore: Boolean(secrets.up?.AUTH_SECRET && secrets.up?.OPENLAW_SECRET_KEY) };
    });
  } finally {
    await s.context.close();
  }
...
phases["br-restore"] = async () => {
  const p = R;
  await step({ ...BACKUP, action: "Review restored destinations before starting app or worker" }, () => {
...
phases["br-negatives"] = async () => {
  ...
  await step({ ...BACKUP, action: "Negative: missing OPENLAW_SECRET_KEY on the target is refused; supplying the retained key recovers", expected: "Startup reports a missing key: supply the retained required keys to both app and worker, then recreate them." }, async () => {
    envSet(p, "OPENLAW_SECRET_KEY", null);
    const q = sh("docker compose config --quiet", p.dir);
    const u = sh("docker compose up -d --no-build --pull never --force-recreate app worker", p.dir);
  ...
  await step({ ...BACKUP, action: "Negative: a wrong OPENLAW_SECRET_KEY with saved Advanced settings: app and worker keep restarting; the retained key recovers", expected: "App and worker keep restarting with Advanced settings cannot be decrypted; supply the retained key and recreate both services." }, async () => {
    ...
    recreate(p);
    await waitReady(p);
    check(appDecrypt.length && workerDecrypt.length && ready.status !== 200, JSON.stringify({ appDecrypt, workerDecrypt, ready: ready.status }));
    return { restartCountsAndStates: states, readyz: ready.status, appLine: appDecrypt, workerLine: workerDecrypt, appNoConfiguredKeyLine: noKey, recovery: "retained key supplied; app and worker recreated; readyz 200" };
  });
  await step({ ...BACKUP, action: "Negative: a wrong OPENLAW_SECRET_KEY with no saved Advanced settings: start log names unreadable columns; saved credentials unavailable; the retained key recovers", expected: "The app log says No configured key opens these stored credentials and names columns such as vapid_private_key; without a VAPID pair in .env it also logs Device notifications are off until the VAPID pair can be read." }, async () => {
    const reset = must("docker compose run -T --rm --no-deps app node apps/api/dist/reset-advanced-settings.js uploads", p.dir).stdout.trim();
    envSet(p, "OPENLAW_SECRET_KEY", randomBytes(32).toString("base64"));
    const t = new Date().toISOString();
    recreate(p);
    await waitReady(p);
    ...
    const read = await s.request("GET", `/api/v1/contracts/${state.inventory.contracts[0].number}`);
    await s.context.close();
    envSet(p, "OPENLAW_SECRET_KEY", good);
    recreate(p);
    await waitReady(p);
    ...
    const mail = await waitMail((m) => toAddress(m, state.people.admin.email) && m.Subject === "OpenLaw test email" && since(m, t0));
    const s2page = await saveUploadLimit(s2.page, p.base, 150);
    await s2.context.close();
    must("docker compose restart app worker", p.dir, { timeout: 240_000 });
    await waitReady(p);
    check(noKey.length && /vapid_private_key/.test(noKey[0]) && vapid.length && email.body?.source === "unset" && test.status >= 400 && read.status === 200 && test2.status === 200 && mail, JSON.stringify({ noKey, vapid, email: email.body?.source, test: test.status, read: read.status, test2: test2.status }));
    return { resetUploadsWithCorrectKey: reset, appLog: [...noKey, ...vapid], emailSettingsSource: email.body?.source, testSend: `${test.status} ${test.body?.detail ?? ""}`, aiTest: `${ai.status} ${ai.body?.detail ?? ""}`.slice(0, 200), unrelatedContractRead: read.status, afterRetainedKey: `test ${test2.status}; delivered`, uploadLimitSavedAgain: s2page };
  });
  await step({ ...BACKUP, action: "Negative: a missing file is detected (Document listed, download fails); reapplying the file backup restores the bytes", expected: "A Document is listed but its download fails: reapply the correct file backup to the isolated target, then compare hashes." }, async () => {
    ...
    must(`docker compose run -T --rm --no-deps app sh -c 'rm -f "/var/lib/openlaw/files/${rel}"'`, p.dir);
    const s = await browserSignIn(p.base, state.people.admin.email, password("upAdmin"));
    const listed = (await s.request("GET", `/api/v1/contracts/${d.contract}/documents`)).body.documents.some((x) => x.id === d.id);
    const missing = await s.request("GET", `/api/v1/documents/${d.id}/versions/${v.id}/download`);
    must('docker compose run -T --rm --no-deps app tar -xzf - -C /var/lib/openlaw/files < "$BACKUP_DIR/files.tar.gz"', p.dir, { env: { BACKUP_DIR: state.sourceBackup } });
    const back = await s.request("GET", `/api/v1/documents/${d.id}/versions/${v.id}/download`);
    await s.context.close();
    check(listed && missing.status !== 200 && back.status === 200 && sha256(back.buffer) === v.sha256, JSON.stringify({ listed, missing: missing.status, back: back.status }));
    return { documentListed: listed, downloadWithFileMissing: `${missing.status} ${missing.body?.detail ?? missing.body?.title ?? ""}`.slice(0, 160), afterReapplyingFiles: `200, SHA-256 ${v.sha256.slice(0, 16)} matches` };
  });
  await step({ ...BACKUP, action: "Negative: a backup archive alone never counts as a restore" }, () => `The backup was accepted only after the restored target ${p.project} signed in, refused the Confidential record to lower-access accounts, matched every Version hash, processed a new upload, sent through the restored relay, passed Test connection with the restored AI key, and showed API and Worker Running and Current.`);
};
...
phases["br-wrongkey"] = async () => {
  ...
  await step(
    {
      ...BACKUP,
      action: "Negative (repeat, observed in detail): a wrong OPENLAW_SECRET_KEY on a target whose dump carries saved Advanced settings",
      expected: "backup-and-restore 'If verification fails': App and worker keep restarting with Advanced settings cannot be decrypted; supply the retained key and recreate both services.",
    },
    async () => {
...
phases["diag-baseline"] = async () => {
  const p = D;
  await step({ ...DIAG, action: "Baseline health before any fault: Start with the observed failure, the three commands", command: "docker compose config --quiet; docker compose ps --all; docker compose logs --since=10m app worker", critical: true }, async () => {
    ...
    const h = await http(`${p.base}/healthz`);
    const r = await http(`${p.base}/readyz`);
    check(cfg.code === 0 && cfg.stdout.trim() === "" && processRowsOk(st), JSON.stringify({ cfg: cfg.code, st }));
    return { configQuiet: `exit ${cfg.code}, no output`, psAll: ps, healthz: h.status, readyz: r.status, systemStatus: st, logLines: logs.split("\n").length };
  });
  await step({ ...DIAG, action: "What logs carry: a request line has the path without its query string and no headers", expected: "A request line carries the path without its query string and no headers." }, async () => {
    const marker = `doc030marker${Date.now()}`;
    await http(`${p.base}/api/v1/contracts?token=${marker}&q=${marker}`, { headers: { authorization: `Bearer ${marker}`, "x-doc030": marker } });
    await sleep(1500);
    const logs = must("docker compose logs --no-log-prefix --since 30s app", p.dir).stdout;
    const line = logs.split("\n").find((l) => l.includes('"path":"/api/v1/contracts"'));
    check(line && !logs.includes(marker), `line ${line?.slice(0, 200)}; marker in logs ${logs.includes(marker)}`);
...
phases["diag-startup"] = async () => {
  const p = D;
  const good = { AUTH_SECRET: secrets.up.AUTH_SECRET, OPENLAW_SECRET_KEY: secrets.up.OPENLAW_SECRET_KEY };
  await step({ ...DIAG, action: "Startup refuses a missing key: Compose refuses config and up for either key; restore", expected: "Compose refuses the command when either key is missing." }, () => {
    const out = {};
    for (const key of Object.keys(good)) {
      const before = psAll(p).map((c) => c.id).sort().join();
      envSet(p, key, null);
      const cfg = sh("docker compose config --quiet", p.dir);
      const up = sh("docker compose up -d --no-build --pull never", p.dir);
      envSet(p, key, good[key]);
      const after = psAll(p).map((c) => c.id).sort().join();
      check(cfg.code !== 0 && up.code !== 0 && before === after, `${key}: ${cfg.code} ${up.code}`);
      out[key] = `config --quiet exit ${cfg.code} (${lines(cfg.stderr, new RegExp(key), 1)[0]?.slice(0, 120)}); up exit ${up.code}; containers unchanged`;
    }
    return out;
  });
  await step({ ...DIAG, action: "Short OPENLAW_SECRET_KEY: config passes; app and worker refuse to start; supply the intended key, config --quiet, recreate, check startup" }, async () => {
    envSet(p, "OPENLAW_SECRET_KEY", "short-key-under-32-chars");
    const cfg = sh("docker compose config --quiet", p.dir);
    const t = new Date().toISOString();
    recreate(p);
    await sleep(25_000);
    const appL = lines(sh(`docker compose logs --no-log-prefix --since ${t} app`, p.dir).stdout, /OPENLAW_SECRET_KEY/, 1);
    const wL = lines(sh(`docker compose logs --no-log-prefix --since ${t} worker`, p.dir).stdout, /OPENLAW_SECRET_KEY/, 1);
    const ready = await http(`${p.base}/readyz`);
    ...
    recreate(p);
    await waitReady(p);
    check(cfg.code === 0 && appL.length && wL.length && ready.status !== 200 && cfg2.code === 0, JSON.stringify({ cfg: cfg.code, appL, wL, ready: ready.status }));
    return { configQuietWithShortKey: cfg.code, readyz: ready.status, restartCounts: st, app: appL, worker: wL, recovery: "intended key supplied; config --quiet exit 0; recreated; readyz 200" };
  });
  await step({ ...DIAG, action: "Short AUTH_SECRET: only a warning is logged and the app starts; restore the retained value" }, async () => {
    envSet(p, "AUTH_SECRET", "short-auth-secret");
    const t = new Date().toISOString();
    recreate(p);
    const ready = await waitReady(p).then(() => 200).catch(() => 0);
    const warn = lines(sh(`docker compose logs --no-log-prefix --since ${t} app`, p.dir).stdout, /secret/i, 2);
  ...
  await step({ ...DIAG, action: "Port is already allocated: failed bind; ps --all shows app and worker created; plain ps omits them; correct PORT and run the same up", expected: "After a failed bind, docker compose ps --all shows the app and worker containers in the created state. Plain docker compose ps does not list them." }, async () => {
    envSet(p, "PORT", String(OCCUPIED_PORT));
    ...
    await waitReady(p);
    check(up.code !== 0 && all.includes("app created") && all.includes("worker created") && !plain.some((l) => /^(app|worker) /.test(l)) && other.status === 200 && up2.code === 0, JSON.stringify({ all, plain }));
    return { upExit: up.code, error: lines(up.stderr, /allocated/, 1).map((l) => l.replace(/[0-9a-f]{12,}/g, "…")), psAll: all, plainPs: plain, unrelatedListener: other.status, recovery: `PORT restored; same up exit ${up2.code}; readyz 200` };
  });
  await step({ ...DIAG, action: "App keeps restarting: a memory ceiling too small; docker inspect OOMKilled; raise APP_MEM_LIMIT and recreate", command: `docker inspect --format '{{.State.OOMKilled}}' "$(docker compose ps --all -q app)"` }, async () => {
    ...
    for (let i = 0; i < 30 && oom !== "true"; i += 1) {
      await sleep(3000);
      oom = sh(`docker inspect --format '{{.State.OOMKilled}}' "$(docker compose ps --all -q app)"`, p.dir).stdout.trim();
      st = restarts(p, "app");
    }
    const ready = await http(`${p.base}/readyz`);
    ...
    recreate(p, "app");
    await waitReady(p);
    const oom2 = sh(`docker inspect --format '{{.State.OOMKilled}}' "$(docker compose ps --all -q app)"`, p.dir).stdout.trim();
    return { withAppMemLimit64m: { oomKilled: oom, restartsAndState: st, readyz: ready.status }, afterRaising: `APP_MEM_LIMIT removed (default 1g); OOMKilled ${oom2}; readyz 200` };
  });
  await step({ ...DIAG, action: "/healthz works but /readyz fails: database stopped; restore; verify a record read and write" }, async () => {
    must("docker compose stop postgres", p.dir);
    await sleep(4000);
    const h = await http(`${p.base}/healthz`);
    const r = await http(`${p.base}/readyz`);
    ...
    const write = await s.request("PATCH", `/api/v1/contracts/${n}`, { description: `DOC-030 operator-2 diag write ${new Date().toISOString()}` });
    await s.context.close();
    check(h.status === 200 && r.status !== 200 && read.status === 200 && write.status === 200, `${h.status} ${r.status} ${read.status} ${write.status}`);
...
phases["diag-cproxy"] = async () => {
  const p = D;
  await step({ ...DIAG, action: "The proxy cannot connect to the app: a proxy in a container answers 502 against 127.0.0.1; APP_BIND to an address it reaches fixes it; restore", command: "docker compose port app 3000" }, async () => {
    const port0 = must("docker compose port app 3000", p.dir).stdout.trim();
    const conf = path.join(WORK, "cproxy.conf");
    writeFileSync(conf, `events {}\nhttp {\n  server {\n    listen 80;\n    location / {\n      proxy_pass http://host.docker.internal:${p.port};\n      proxy_set_header Host $http_host;\n      proxy_set_header X-Forwarded-For $remote_addr;\n      proxy_set_header X-Forwarded-Proto $scheme;\n    }\n  }\n}\n`, { mode: 0o644 });
    sh(`docker rm -f ${CPROXY_NAME}`, root);
    must(`docker run -d --name ${CPROXY_NAME} --label openlaw-doc030-owner=operator-2 --add-host host.docker.internal:host-gateway -p 127.0.0.1:${CPROXY_PORT}:80 -v ${conf}:/etc/nginx/nginx.conf:ro nginx:1.27-alpine`, root);
    await sleep(2000);
    let before;
    ...
    const gw = must("docker network inspect bridge --format '{{(index .IPAM.Config 0).Gateway}}'", root).stdout.trim();
    ...
    sh(`docker rm -f ${CPROXY_NAME}`, root);
    const proxyLog = lines(sh(`docker logs ${CPROXY_NAME} 2>&1`, root).stdout, /upstream|connect/, 1);
    check(port0 === `127.0.0.1:${p.port}` && [502, 504].includes(before.status) && after.status === 200 && port2 === port0, JSON.stringify({ port0, before: before.status, port1, after: after.status }));
    sh(`docker rm -f ${CPROXY_NAME}`, root);
    return { portDefault: port0, containerProxyBefore: before.status, proxyErrorLine: proxyLog.map((l) => l.replace(/^\S+ \S+ /, "")), appBind: `APP_BIND=${gw} (the docker0 bridge address, reachable only from containers and this host)`, portAfter: port1, containerProxyAfter: after.status, restored: `APP_BIND removed; port ${port2}`, notTested: "the host network rule that admits only the proxy" };
  });
};
...
phases["diag-origin"] = async () => {
  ...
  await step({ ...DIAG, role: "administrator", method: "browser-walkthrough", action: "Sign-in fails behind a saved Application address: Instance address shows its source; BASE_URL pins over a saved value; the saved instance setting is removed with the recovery command", expected: "Instance address shows Deployment configuration, Saved in OpenLaw or Default. BASE_URL in .env pins the address over a saved one. If nobody can sign in, remove the saved instance setting." }, async () => {
    const out = {};
    ...
    const tryDefault = await fx.raw("POST", "/api/auth/sign-in/email", { json: { email: state.people.admin.email, password: password("upAdmin") } });
    ...
    out.withoutBaseUrlApi = cur.fields.map((f) => `${f.key} value ${f.value} source ${f.source} locked ${f.locked}`);
    await fx.put("/api/v1/advanced-settings/instance", { version: cur.version, values: { BASE_URL: p.base } });
    must("docker compose restart app worker", p.dir, { timeout: 240_000 });
    await waitReady(p);
    s = await adminLogin();
    ...
    await s.page.getByText("Settings saved. Restart the API and worker to apply changes.").waitFor({ timeout: 15_000 });
    await s.context.close();
    must("docker compose restart app worker", p.dir, { timeout: 240_000 });
    await waitReady(p);
    const api = new Api(p.base);
    const refused = await api.raw("POST", "/api/auth/sign-in/email", { json: { email: state.people.admin.email, password: password("upAdmin") } });
    ...
    recreate(p);
    await waitReady(p);
    const before = await new Api(p.base).raw("POST", "/api/auth/sign-in/email", { json: { email: state.people.admin.email, password: password("upAdmin") } });
    const t = new Date().toISOString();
    const reset = sh("docker compose run -T --rm --no-deps app node apps/api/dist/reset-advanced-settings.js instance", p.dir);
    must("docker compose restart app worker", p.dir, { timeout: 240_000 });
    await waitReady(p);
    const after = await new Api(p.base, "http://localhost:3000").raw("POST", "/api/auth/sign-in/email", { json: { email: state.people.admin.email, password: password("upAdmin") } });
    const atPublic = await new Api(p.base).raw("POST", "/api/auth/sign-in/email", { json: { email: state.people.admin.email, password: password("upAdmin") } });
    ...
    const leaked = secretValues().some((v) => reset.stdout.includes(v)) || reset.stdout.includes(wrong);
    const fx2 = new Api(p.base, "http://localhost:3000");
    await fx2.signIn(state.people.admin.email, password("upAdmin"));
    out.afterReset = (await fx2.get("/api/v1/advanced-settings/instance")).fields.map((f) => `${f.key} = ${f.value} [${f.source}]`);
    const audit = psql(p, "select payload->>'field' || ' ' || (payload->>'new') from activity_log where action = 'org_settings.updated' order by created_at desc limit 1");
    envSet(p, "BASE_URL", p.base);
    recreate(p);
    await waitReady(p);
    ...
    void t;
    check(refused.status !== 200 && reset.code === 0 && after.status === 200 && !leaked && out.afterReset[0].endsWith("[default]"), JSON.stringify(out));
    return out;
  });
};
...
phases["diag-trust"] = async () => {
  const p = I;
  await step({ ...DIAG, action: "Password sign-in refuses people who did not enter a wrong password: the TRUSTED_PROXIES is not set warning; set it to the proxy's own address; the warning is gone; sign in from two client addresses", expected: "Set TRUSTED_PROXIES to the proxy's own address. Recreate the app, check that the warning is gone, and sign in from two client addresses." }, async () => {
    ...
    const probe = async () => {
      const wrongs = [];
      for (let i = 0; i < 4; i += 1) wrongs.push((await viaProxy("POST", "/api/auth/sign-in/email", { json: { email: "nobody@doc030-install.example", password: "nope" }, localAddress: "127.0.0.2" })).status);
      const right = await viaProxy("POST", "/api/auth/sign-in/email", { json: { email: "avery.morgan@doc030-install.example", password: password("instAdmin") }, localAddress: "127.0.0.1" });
    ...
    const seen = [...new Set(sh("docker compose logs --no-log-prefix --since 40s app", p.dir).stdout.split("\n").filter((l) => l.includes("sign-in/email")).map((l) => l.match(/"remoteAddress":"([^"]+)"/)?.[1]).filter(Boolean))];
    ...
    check(warn.length && !warn2.length && set.rightFrom127_0_0_1 === 200, JSON.stringify({ warn, warn2, unset, set, seen }));
    return { warningWhenUnset: warn, withUnset: unset, warningAfterSetting: warn2, withGuideValue: set, remoteAddressSeen: seen };
  });
};

phases["diag-live"] = async () => {
  const p = D;
  await step({ ...DIAG, role: "administrator", method: "browser-walkthrough", action: "Live updates: one person holds at most five streams; a sixth closes the oldest; a change from a second signed-in browser arrives on an open stream" }, async () => {
    const s = await adminLogin();
    ...
    await sleep(6000);
    const posted = await other.request("POST", "/api/v1/comments", { entityType: "contract", entityId: c.id, body: `DOC-030 operator-2 live check ${Date.now()}`, visibility: "working_team" });
    await sleep(6000);
    const streams = await s.page.evaluate(() => window.__d.streams);
    await s.context.close();
    await other.context.close();
    return { streamsInOneTab: streams, commentPosted: posted.status, note: "The app page itself holds its own stream too, so six opened here plus the page's own exceed five for one person; errors count reconnects after the hub closed the oldest streams." };
  });
};

phases["diag-migration"] = async () => {
  const p = D;
  await step({ ...DIAG, action: "Migration failure: a controlled journal fault; the refusal text; both read-only commands; the fixture custodian undoes the fault", expected: "This database cannot apply the migrations it is missing" }, async () => {
    const row = psql(p, "select id, hash from drizzle.__drizzle_migrations order by created_at desc limit 1");
    const [id, hash] = row.split("|");
    psql(p, `update drizzle.__drizzle_migrations set hash = 'doc030diagnosefault' || substr(hash, 20) where id = ${id}`);
    const t = new Date().toISOString();
    recreate(p, "app");
    await sleep(25_000);
    ...
    const ready = await http(`${p.base}/readyz`);
    const c1 = sh(`docker compose exec -T postgres psql -U openlaw -d openlaw -c 'select hash, created_at from drizzle.__drizzle_migrations order by created_at desc limit 5;'`, p.dir);
    const c2 = sh("docker compose run -T --rm --no-deps app node scripts/lint-migration-journal.mjs --hashes", p.dir, { timeout: 180_000 });
    const shipped = c2.stdout.split("\n").find((l) => l.includes(hash))?.trim().slice(0, 120);
    psql(p, `update drizzle.__drizzle_migrations set hash = '${hash}' where id = ${id}`);
    recreate(p, "app");
    await waitReady(p);
    check(refusal.some((l) => l.includes("This database cannot apply the migrations it is missing")) && ready.status !== 200 && c1.code === 0 && c1.stdout.includes("doc030diagnosefault") && c2.code === 0 && shipped, JSON.stringify({ refusal, ready: ready.status, c1: c1.code, c2: c2.code, shipped }));
    return { refusal, readyz: ready.status, psqlCommand: `exit ${c1.code}; lists the faulty row`, lintHashes: `exit ${c2.code}; shipped entry ${shipped}`, undo: "custodian wrote back the recorded hash (not an operator repair); readyz 200" };
  });
  await step({ ...DIAG, action: "accounts.issuer refusals: the shipped 0060 migration against scratch pre-0060 shapes names each case and applies none of the migration" }, () => {
    const sql = must("docker compose run -T --rm --no-deps app cat packages/db/migrations/0060_account_issuer.sql", p.dir).stdout.replace(/--> statement-breakpoint/g, "");
    const run = (label, seed) => {
      const db = `doc030_issuer_${label}`;
      must(`docker compose exec -T postgres psql -U openlaw -d openlaw -c "drop database if exists ${db}" -c "create database ${db}"`, p.dir);
      must(`docker compose exec -T postgres psql -v ON_ERROR_STOP=1 -U openlaw -d ${db}`, p.dir, { input: `create table sso_providers (provider_id text primary key, issuer text not null);\ncreate table accounts (id text primary key, provider_id text not null, account_id text not null, unique (provider_id, account_id));\n${seed}\n` });
      const r = sh(`docker compose exec -T postgres psql -v ON_ERROR_STOP=1 -U openlaw -d ${db}`, p.dir, { input: sql });
      const col = must(`docker compose exec -T postgres psql -U openlaw -d ${db} -At -c "select count(*) from information_schema.columns where table_name='accounts' and column_name='issuer'"`, p.dir).stdout.trim();
      must(`docker compose exec -T postgres psql -U openlaw -d openlaw -c "drop database ${db}"`, p.dir);
      return { exit: r.code, error: lines(r.stderr, /ERROR/, 1), issuerColumnAfter: col };
    };
    const orphan = run("orphan", "insert into accounts values ('a1','credential','u1'),('a2','doc030-retired-idp','subject-1');");
    const dup = run("dup", "insert into sso_providers values ('doc030-idp-a','https://idp.doc030.example'),('doc030-idp-b','https://idp.doc030.example');\ninsert into accounts values ('a1','doc030-idp-a','subject-1'),('a2','doc030-idp-b','subject-1');");
    check(orphan.exit !== 0 && orphan.error.join().includes("Cannot resolve an issuer for accounts under provider(s)") && orphan.issuerColumnAfter === "0", JSON.stringify(orphan));
    check(dup.exit !== 0 && dup.error.join().includes("Two accounts share one 1.7 identity") && dup.issuerColumnAfter === "0", JSON.stringify(dup));
...
phases["diag-0157"] = async () => {
  ...
  const api = new Api(p.base);
  await api.signIn(state.people.admin.email, password("upAdmin"));
  await step({ ...DIAG, method: "automated-test", action: "Fixture on the starting build (recovery target): a Matter-targeted Request type whose form holds a Contract Field. The starting build's API refuses that attachment, so the row is written directly in the database to stand for data an older build may hold.", critical: true }, async () => {
    ...
    const created = await api.post("/api/v1/request-types", { displayName: "DOC-030 operator-2 guarded intake" });
    const rt = created.requestType ?? created.item ?? Object.values(created).find((v) => v && typeof v === "object" && v.id);
    await api.patch(`/api/v1/request-types/${rt.id}`, { targetModule: "matter", targetTypeId: types[0].id });
    const refused = await api.raw("POST", `/api/v1/request-types/${rt.id}/fields`, { json: { fieldId: cf.id, isRequired: false } });
    psql(p, `insert into request_type_fields (request_type_id, field_id, display_order) values ('${rt.id}', '${cf.id}', 1)`);
    state.guarded = { requestTypeId: rt.id, name: rt.displayName, fieldId: cf.id, field: `${cf.slug}:${cf.moduleScope}` };
  ...
  await runBackup(p, dir, { meta: { ...DIAG, article: "operator-troubleshooting (Migration failures: take or preserve a coherent backup)" }, restart: false, pre: BASELINE });
  state.pre0157 = dir;
  saveState();
  await step({ ...DIAG, action: "Upgrade that target to the candidate: the app refuses with 'Cannot migrate the intake form of Request types:'; earlier pending migrations stay applied", expected: "Cannot migrate the intake form of Request types: names each Request type. The message appears inside the failed migration's SQL error. The pending migrations before this one stay applied." }, async () => {
    ...
    check(msg.length && msg[0].includes(state.guarded.name) && Number(after) > Number(before) && ready.status !== 200, JSON.stringify({ msg, before, after, ready: ready.status }));
    return { refusal: msg[0].slice(0, 400), appState: restarts(p, "app"), readyz: ready.status, journalRows: `${before} before; ${after} after the refused start (earlier pending migrations applied)` };
  });
  const f = PROJECTS.migfix;
  await restoreInto(f, dir, BASELINE, { meta: { ...DIAG, article: "operator-troubleshooting (0157 recovery via upgrade.md#if-the-upgrade-cannot-be-accepted)" }, keysFrom: "up" });
  await step({ ...DIAG, role: "administrator", method: "browser-walkthrough", action: "On the starting build in the separate target, re-target the named Request type or remove those Fields from its questions", critical: true }, async () => {
    ...
    try {
      ...
      const r = await s.request("DELETE", `/api/v1/request-types/${state.guarded.requestTypeId}/fields/${state.guarded.fieldId}`);
  ...
  await runBackup(f, dir2, { meta: { ...DIAG, article: "operator-troubleshooting (take a new coherent backup)" }, restart: false, pre: BASELINE });
  await step({ ...DIAG, action: "Repeat the upgrade on the corrected target: migrations complete and the app is ready" }, async () => {
...
phases["diag-mail"] = async () => {
  const p = D;
  const good = secrets.up.OPENLAW_SECRET_KEY;
  await step({ ...DIAG, action: "SMTP_URL pins the environment: the saved relay is ignored; a valid SMTP_FROM; recreate app and worker; verify; then remove the override" }, async () => {
    ...
    await s.context.close();
    check(pinned.source === "env" && test.status === 200 && m1?.From?.Address === "env@doc030-upgrade.example" && back.source !== "env" && test2.status === 200 && m2?.From?.Address === "openlaw@doc030-upgrade.example", JSON.stringify({ pinned: pinned.source, from1: m1?.From?.Address, back: back.source, from2: m2?.From?.Address }));
    return { pinnedSource: pinned.source, deliveredFrom: m1.From.Address, afterRemoval: `source ${back.source}; delivered from ${m2.From.Address}` };
  });
  await step({ ...DIAG, action: "Wrong OPENLAW_SECRET_KEY: the start log names unreadable columns; email reads unset; the test send gives the exact message; an unrelated Contract stays readable; restore the key", expected: "The test email could not be sent. SMTP is not configured — save a relay first." }, async () => {
    envSet(p, "OPENLAW_SECRET_KEY", randomBytes(32).toString("base64"));
    const t = new Date().toISOString();
    recreate(p);
    await sleep(20_000);
    const ready = await http(`${p.base}/readyz`);
    const log1 = lines(sh(`docker compose logs --no-log-prefix --since ${t} app`, p.dir).stdout, /No configured key opens|Advanced settings cannot be decrypted/, 2);
    const s = await adminLogin();
    ...
    check(detail === "The test email could not be sent. SMTP is not configured — save a relay first." && settings.source === "unset" && read.status === 200 && test2.status === 200 && mail, JSON.stringify({ detail, settings, read: read.status }));
    return { readyzWithWrongKey: ready.status, startLog: log1, emailSource: settings.source, testSend: `${test.status} ${detail}`, unrelatedContract: read.status, fileUploadsWithWrongKey: uploads, afterRetainedKey: `test ${test2.status}, delivered; File uploads ${uploads2.join("; ")}` };
  });
};
...
phases["diag-storage"] = async () => {
  ...
  saveSecrets();
  await step({ ...DIAG, action: "Local storage: the file volume belongs to the image's unprivileged user" }, () => {
    const r = must(`docker compose run -T --rm --no-deps app sh -c 'id -u; id -un; stat -c "%U %u %a" /var/lib/openlaw/files'`, p.dir).stdout.trim().split("\n");
    check(r[0] !== "0" && r[2].split(" ")[1] === r[0], JSON.stringify(r));
    return { uid: r[0], user: r[1], volumeOwnerUidMode: r[2] };
  });
  await step({ ...DIAG, role: "administrator", method: "browser-walkthrough", action: "Storage settings name their source; System status shows Active storage; Test connection on Document storage (local)" }, async () => {
    const s = await adminLogin();
    ...
    await s.page.getByText(/Connection test passed\.|The storage test failed/).first().waitFor({ timeout: 30_000 });
    const result = (await bodyText(s.page)).match(/Connection test passed\.|The storage test failed[^.]*\./)?.[0];
    const st = await systemStatus(s.page, p.base);
    await s.context.close();
    check(result === "Connection test passed." && st.activeStorage === "Active storage: local", `${result} ${st.activeStorage}`);
  ...
  await step({ ...DIAG, role: "administrator", method: "browser-walkthrough", action: "Fixture through the app: an owned MinIO; Document storage saved as S3 after Test connection passes; restart; a new upload lands in the bucket", critical: true }, async () => {
    ...
    await s.page.getByRole("button", { name: "Test connection" }).click();
    await s.page.getByText(/Connection test passed\.|The storage test failed/).first().waitFor({ timeout: 30_000 });
    const tested = (await bodyText(s.page)).match(/Connection test passed\.|The storage test failed[^.]*\./)?.[0];
    await s.page.getByRole("button", { name: "Save", exact: true }).click();
    await s.page.getByText("Settings saved. Restart the API and worker to apply changes.").waitFor({ timeout: 15_000 });
    await s.context.close();
    must("docker compose restart app worker", p.dir, { timeout: 240_000 });
    ...
    return { testConnection: tested, fileRefPrefix: ref.split(":")[0], activeStorage: st.activeStorage, storageFields: src.filter((l) => !/secret/i.test(l)) };
  });
  await step({ ...DIAG, action: "Remove saved Advanced settings: storage; files do not move; a Document only the saved values named stays unreadable until the deployment configures that store", expected: "Removing saved storage values does not move files. A Document in a bucket or container that only the saved values named stays unreadable until the deployment configures that store." }, async () => {
    const reset = sh("docker compose run -T --rm --no-deps app node apps/api/dist/reset-advanced-settings.js storage", p.dir);
    must("docker compose restart app worker", p.dir, { timeout: 240_000 });
    await waitReady(p);
    const s = await adminLogin();
    const d = state.s3Doc;
    const s3got = await s.request("GET", `/api/v1/documents/${d.documentId}/versions/${d.versionId}/download`);
    const local = state.inventory.documents[0];
    const localGot = await s.request("GET", `/api/v1/documents/${local.id}/versions/${local.versions[0].id}/download`);
    const st = await systemStatus(s.page, p.base);
    await s.context.close();
    ...
    await waitReady(p);
    const s2 = await adminLogin();
    const again = await s2.request("GET", `/api/v1/documents/${d.documentId}/versions/${d.versionId}/download`);
  ...
  await step({ ...DIAG, role: "administrator", method: "browser-walkthrough", action: "Upload refusal: File uploads saved at 1 MiB; a file above is refused with the app's message; a file below is accepted; restore 150" }, async () => {
    ...
    await (await chooser).setFiles({ name: "doc030-op2-big.txt", mimeType: "text/plain", buffer: Buffer.alloc(1_300_000, 97) });
    ...
    return { over: shown, under: small.how, restored: "File uploads 150 saved again; app and worker restarted" };
  });
};
...
phases["diag-worker"] = async () => {
  ...
  await step({ ...DIAG, role: "administrator", method: "browser-walkthrough", action: "Engine down: the API stays ready; a new PDF does not finish while the engine is stopped; restore; a new controlled PDF is processed" }, async () => {
    ...
    const s = await adminLogin();
    const api = { raw: (m, r) => s.request(m, r) };
    const up = await uploadInBrowser(s.page, p.base, n, "doc030-op2-engine-down.pdf", pdf(["DOC-030 operator-2 engine-down upload."]));
    await sleep(45_000);
    const during = await textState(api, up.documentId, up.versionId);
    const ready = await http(`${p.base}/readyz`);
    const wlog = lines(sh("docker compose logs --since=2m worker", p.dir).stdout, /engine|ECONNREFUSED|ENOTFOUND|fetch failed|unavailable/i, 2);
    must("docker compose start doc-engine", p.dir);
    await sleep(15_000);
    const up2 = await uploadInBrowser(s.page, p.base, n, "doc030-op2-engine-back.pdf", pdf(["DOC-030 operator-2 engine restored.", "Engine back words."]));
    ...
    await s.context.close();
    check(ready.status === 200 && during !== "ready" && t2.state === "ready", JSON.stringify({ during, ready: ready.status, t2: t2.state }));
    return { readyzWhileEngineDown: ready.status, stateAfter45s: during, workerLog: wlog, newPdfAfterRestore: t2.state, earlierUploadLater: later.state };
  });
  await step({ ...DIAG, action: "Engine busy: DOC_ENGINE_MAX_CONCURRENT=1 and DOC_ENGINE_MAX_QUEUED=0; a burst of uploads; the engine answers 503 with Retry-After; processing slows rather than fails; restore" }, async () => {
    ...
    await sleep(15_000);
    const probe = sh(`docker compose exec -T app node -e "Promise.all(Array.from({length:4},()=>fetch('http://doc-engine:8080/healthz').then(r=>r.status))).then(s=>console.log(s.join(',')))"`, p.dir);
    const s = await adminLogin();
    const api = { raw: (m, r) => s.request(m, r) };
    const ups = [];
    for (let i = 0; i < 5; i += 1) ups.push(uploadViaSession(s, n, `doc030-op2-burst-${i}.pdf`, pdf([`DOC-030 operator-2 burst ${i}.`, `Burst words ${i}.`])));
    const refs = await Promise.all(ups);
    const results = [];
    for (const r of refs) results.push((await waitText(api, r.documentId, r.versionId, 600_000)).state);
    await s.context.close();
    ...
    must("docker compose up -d --no-build --pull never --force-recreate doc-engine", p.dir, { timeout: 240_000 });
    return { healthProbeStatuses: probe.stdout.trim(), burstResults: results, busyLogLines: busy, restored: "limits removed; engine recreated" };
  });
};
async function uploadViaSession(s, number, filename, bytes) {
  const r = await s.page.request.post(`${s.base}/api/v1/contracts/${number}/documents`, { headers: { origin: s.base }, multipart: { file: { name: filename, mimeType: "application/pdf", buffer: bytes } } });
...
phases["diag-providers"] = async () => {
  ...
  await step({ ...DIAG, role: "administrator", method: "browser-walkthrough", action: "AI connector refusal: Test connection shows only the HTTP status; the app log line has the provider status and a short redacted reply; restore the provider", expected: "The connection test failed. The provider refused the request with HTTP 401. The app log then has The AI provider refused the connection test." }, async () => {
    ...
    check(shown?.startsWith("The connection test failed. The provider refused the request with HTTP 401.") && line && !keyInLog && again, JSON.stringify({ shown, line: line?.slice(0, 300), keyInLog, again }));
    return { shownInBrowser: shown, logLine: JSON.parse(line)?.msg, providerStatus: JSON.parse(line)?.providerStatus, providerReply: JSON.parse(line)?.providerReply, keyOrKeyPrefixInLog: keyInLog, afterProviderRestored: again };
  });
  await step({ ...DIAG, role: "administrator", method: "browser-walkthrough", action: "Signing: the Signing updates control offers Polling and Webhook" }, async () => {
    const s = await adminLogin();
...
phases["diag-reset-wrongkey"] = async () => {
  const p = D;
  const good = secrets.up.OPENLAW_SECRET_KEY;
  await step({ ...DIAG, action: "Wrong key with saved Advanced settings, and the recovery command under that key", expected: "If settings were saved in Settings → Advanced, a wrong key stops app and worker startup with Advanced settings cannot be decrypted. The recovery command needs the correct key; with a key that cannot open the saved settings it stops with the same message as startup. It removes only that section's saved values." }, async () => {
    ...
    await s.context.close();
    const out = {
      savedBefore: Object.values(before).flat().filter((l) => l.includes("Saved in OpenLaw")),
      ...
      savedAfterRetainedKey: Object.values(after).flat().filter((l) => l.includes("Saved in OpenLaw")),
    };
    guideFailure("operator-troubleshooting", "Startup table 'App keeps restarting' and 'Email and stored credentials' ('If settings were saved in Settings → Advanced, a wrong key stops app and worker startup with Advanced settings cannot be decrypted')", "A wrong OPENLAW_SECRET_KEY with saved Advanced settings stops app and worker with 'Advanced settings cannot be decrypted. Restore the instance encryption key before restarting.'", `Saved: ${out.savedBefore.join("; ")}. With a wrong key app and worker started (${out.states.join(", ")}, readyz ${ready.status}); no 'cannot be decrypted' line; the saved values silently fell back to defaults.`);
    guideFailure("operator-troubleshooting", "Remove saved Advanced settings ('With a key that cannot open the saved settings, it stops with the same message as startup'; 'It removes only that section's saved values')", "Under a wrong key the command stops with 'Advanced settings cannot be decrypted' and removes nothing.", `Under a wrong key 'reset-advanced-settings.js mcp' ${out.resetMcpWithWrongKey}. After the retained key was restored, the other sections' saved values were gone too: before ${out.savedBefore.join("; ")}; after ${out.savedAfterRetainedKey.join("; ") || "none"}.`);
    return out;
  });
};

phases["diag-logscan"] = async () => {
  await step({ ...DIAG, action: "Negative: container logs after every induced fault omit keys, relay, storage and provider credentials, setup tokens and fixture passwords" }, () => {
    const out = {};
    for (const p of [D, I, PROJECTS.restore, PROJECTS.recover, PROJECTS.migfix]) {
      ...
      out[p.project] = { lines: logs.split("\n").length, secretHits: values.filter((v) => logs.includes(v)).length, smtpUrlWithPassword: /smtps?:\/\/[^\s"@/]+:[^\s"@]+@/.test(logs) };
    }
    const hits = Object.values(out).reduce((a, b) => a + b.secretHits + (b.smtpUrlWithPassword ? 1 : 0), 0);
...
async function createContractInBrowser(page, base, title) {
  ...
  const typeShown = await typeSelect.evaluate((el) => (el.tagName === "SELECT" ? el.options[el.selectedIndex]?.textContent : el.textContent)).catch(() => null);
  ...
  return { number, title, how: `Contracts, Create contract, Title "${title}", Contract type showing "${typeShown}", Create opened /contracts/${number}` };
}
...
async function uploadInBrowser(page, base, number, filename, bytes) {
  ...
  await page.getByText(filename.replace(/\.[a-z]+$/, "")).first().waitFor({ timeout: 20_000 }).catch(() => {});
  return { documentId: doc.id, versionId: version.id, sha256: sha256(bytes), how: `Documents tab, Upload, Upload document dialog, Choose files ${filename}, Upload (${response.status()})` };
}
...
export { phases, BASELINE, CPROXY_NAME, CPROXY_PORT, ALT_PORT, UPGRADE, BACKUP, DIAG, attach, containerImage, imageIds, psql, secretValues, textState, rmSync, guideFailure, productBug, MAIL_UI };
