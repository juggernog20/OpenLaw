...
export async function run(log, fx) {
  ...
  await bu.page.screenshot({
    path: new URL("./r2-1-business-user-onboarding.png", import.meta.url).pathname,
  });
  ...
  const amaraReq = sql(
    `select number from requests where requester_id='${amaraId}' and status<>'converted' order by (status='new') desc, number limit 1`,
  );
  ...
  // shared lab, the seeded Administrator creates "DOC-029r2 access" Contracts through the API with Jonas on the team.
  {
    const current = Number(
      sql(
        `select count(*) from contract_team t join contracts c on c.id=t.contract_id where t.user_id='${jonasId}' and c.archived_at is null`,
      ),
    );
    const typeId = sql(
      `select id from contract_types where display_name='NDA' and archived_at is null limit 1`,
    );
    ...
    if (need) {
      ...
      for (let i = 0; i < need; i++) {
        const c = await api(admin.page, "POST", "/contracts", {
          title: `DOC-029r2 access Show more fixture ${i + 1} ${stamp % 100000}`,
          contractTypeId: typeId,
        });
        const n = c.body?.contract?.number;
        const t = n
          ? await api(admin.page, "POST", `/contracts/${n}/team`, { userId: jonasId })
          : { status: 0 };
        made.push(`C-${n}:${c.status}/${t.status}`);
      }
      await admin.context.close();
      fx.created.push(
        `${need} Contracts named DOC-029r2 access Show more fixture, with Jonas Weber on the team (${made.join(", ")})`,
      );
    }
    R(
      "fixture: more than 25 current-team Contracts for Jonas",
      "Enough Contracts exist for Show more to appear",
      `Current-team Contracts before ${current}; created ${need} (${made.join(", ")})`,
      made.every((m) => /:201\/20\d$/.test(m)),
    );
  }
  ...
  R(
    ...
    aHeader.includes(SEED.amara.email) &&
      aText.includes(amaraTitle.slice(0, 30)) &&
