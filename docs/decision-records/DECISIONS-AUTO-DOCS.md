...
## ADO-013 — Template screening, package limits, and per-person caps on Generation and Request writes

- **Status** — Accepted
- **Date** — 2026-09-18
- **Context** — The 2026-09-17 pre-launch security review (M6, M7, M8, M12, M16) found five gaps on the Auto-Doc and Request write paths. A Generation that created a Contract stayed readable by every Member after the Contract went confidential, and its output could be Filed onto a Member's own record. The template upload read only `w:t` text, so a Word file with an external attached template, an OLE link, a macro project, or a DDE field passed through the fill and left the organisation's own SMTP as a finished document. The ZIP guard summed the sizes the central directory declared, which a crafted package can lie about. Template and output reads buffered whatever storage returned. No Portal write path had a per-person bound, so one account could queue every fill worker, every PDF conversion, and every Member notification.
- **Decision** —
  - **A Generation follows its created Contract's audience.** `generationReachScope` in `generation-access.ts` admits a Generation with no created Contract under the Member-wide rule, and a Generation that created a Contract only while the viewer reaches that Contract under DD-014. It applies to the Generation read, the downloads, the list, retry, and the Filing source. Business Users keep the Portal's own rule: their own Generations, under the Auto-Doc's audience. The generating Member is on the created Contract's team and keeps reach.
  - **Every template is screened at upload and again before every fill.** `screenDocxPackage` refuses an external relationship of any type but hyperlink, a `vbaProject` part or relationship, a macro-enabled content type, and a `DDE`, `DDEAUTO`, `INCLUDETEXT`, `INCLUDEPICTURE`, or `IMPORT` field in any Word part, including one whose keyword is split across runs. The upload answers 422 with the part and the reason. The fill runs the same screen on the stored bytes, so a template stored before the screen existed fails its Generation with the same reason instead of filling.
  - **Declared ZIP sizes are not trusted.** `verifyZipPackage` inflates every entry under what the 32 MiB fill ceiling still allows, refuses an entry whose body does not match its declared size, and caps a package at 2000 entries. It runs at upload, in the fill worker before PizZip opens the package, and before the screen.
  - **Storage reads are bounded.** The template read before a fill, the output reads before PDF conversion and email, and the stored-template detection all count bytes against the upload ceiling and abort past it. A stored file over the ceiling fails its Generation or its delivery as a terminal failure, because the blob does not shrink on retry.
  - **Per-person caps, counted from the database.** Two fills run at once per process (`MAX_CONCURRENT_FILLS`); later fills wait in arrival order. Sixteen fills may wait (`MAX_QUEUED_FILLS`); past that a Generation answers 503 with Retry-After before any row is written, because the fill timeout starts only once a slot is held and a queue with no cap is a wait with no bound. A person may hold five pending Generations (`MAX_PENDING_GENERATIONS_PER_PERSON`); a sixth answers 429. A person may submit twenty Requests an hour (`MAX_REQUESTS_PER_HOUR`) and attach 256 MiB an hour (`MAX_REQUEST_ATTACHMENT_BYTES_PER_HOUR`); either quota answers 429 with the number named. `request_attachments.byte_size` records each upload's length so the byte quota is a sum over the last hour. A per-person advisory lock serialises each count with its write, so a burst cannot read the same count and all pass.
- **Rationale** — The answers and the output of a Contract-creating Generation are that Contract's facts, so they take that Contract's audience. A screen at upload alone would leave every template already stored unscreened, so the fill screens too. A guard that trusts a declared size is not a guard. Fixed numbers with a named constant are enough for a first release; the proxy cannot key a limit on the account, and the database already holds what the quotas count.
- **Alternatives considered** — Re-parenting a Generation's audience onto the Auto-Doc: leaves the Contract's facts readable after it goes confidential. Refusing every field: `PAGE`, `MERGEFIELD`, and `HYPERLINK` are ordinary and harmless. Configurable quotas through Advanced settings: more than the job needs today; the constants are one line each to change.
- **Consequences** — Migration 0146 adds `request_attachments.byte_size`, default 0 for rows that predate the quota. `apps/api/src/lib/docx-package.ts` holds `verifyZipPackage` and `screenDocxPackage`; `apps/api/src/lib/bounded-blob.ts` holds the bounded reads; `apps/api/src/lib/auto-doc-fill/semaphore.ts` holds the fill bound. A Generation whose template fails the screen or the size guard records `fill_failed` with the reason. The checked-in fixtures pass the screen unchanged.
...
| ADO-013 | Template screening, package limits, and per-person caps on Generation and Request writes                | Accepted |
