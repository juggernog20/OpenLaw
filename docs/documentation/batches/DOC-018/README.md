...
A separate review seat repeated every reader step in fresh browser contexts against the same immutable lab, after the copy corrections above. It built its own disposable fictional records rather than reusing the author fixtures, which their own recovery checks had already changed or deleted. Infrastructure helpers were reused; the reader steps and assertions were written independently.
...
All 121 independent steps passed. Every step names the role that performed it, the actual observed result, and its own UTC timestamps.
...
The discovery pass reached each guide from the Entity destination's Help, found all four by exact title, followed an outline link, and opened the same guide in the formal reader. Arrival focus lands on the article's H1; Shift+Tab reaches the outline with a visible focus indicator and Enter follows it; the shell's "Skip to content" link targets `#main`. The articles produced no horizontal overflow at 320, 720 and 1440 CSS pixels in Light, Dark and Warm, nor at an emulated 200 percent zoom. The anonymous formal index listed all four guides, offered no sign-in form or password field, and made zero `/api/v1` requests.
