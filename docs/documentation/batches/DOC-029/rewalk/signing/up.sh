#!/usr/bin/env bash
# DOC-029 re-walk: apply the signing stand-in overlay to the owned lab "sign-rw".
# Copy of signing-standin/fixture/up.sh with the lab name changed and server.mjs taken from this folder.
# Usage: up.sh up|down|ps. Run after `lab.mjs up sign-rw` and `lab.mjs seed sign-rw`.
set -euo pipefail
here="$(cd "$(dirname "$0")" && pwd)"
repo="$(cd "$here/../../../../../.." && pwd)"
lab="$repo/.documentation-labs/sign-rw"
