// V-HELP browser walkthrough for the complete edition (guide count from articles.json). Agent check, not human approval.
// Run from the repository root with LAB_PASSWORD set. Writes help.json beside this file.
...
const SHELL_API = new Set([
  ...
  // DOC-030: the header and Portal brand now re-read organization branding (name, logo) on
  // every navigation (lib/organization-branding.ts, 2c028604). It is shell chrome, not a record query.
...
// DOC-030 additions. connect-chatgpt and connect-microsoft-365-copilot stay in review (DOC-031),
// so the development bundle marks them unverified. The badge must render and leave the page usable.
const UNVERIFIED = L.bundle.articles.filter((a) => a.unverified).map((a) => a.id);
const BADGE = L.bundle.validationPending ? "Validation in progress" : "Unverified article";

async function unverifiedBadge(page, base, destination) {
  const out = [];
  const ids = UNVERIFIED.filter((id) =>
    L.bundle.articles.find((a) => a.id === id).destinations.includes(destination),
  );
  L.check(ids.length === 2, `expected two unverified guides in ${destination}, got ${ids}`);
  for (const id of ids) {
    for (const width of [1280, 320]) {
      await page.setViewportSize({ width, height: 800 });
      await page.goto(`${L.BASE}${base}/${id}`);
      await waitTitle(page, L.titleOf(id), `${id} title missing`);
      const badge = page.locator("#docs-main .docs-article-meta .docs-badge");
      L.check((await badge.count()) === 1, `${id} badge count ${await badge.count()}`);
      const text = (await badge.textContent()).trim();
      L.check(text === BADGE, `${id} badge text ${text}`);
      const prose = await page.locator("#docs-main article p").count();
      const outline = await page.locator(".docs-outline a").count();
      L.check(prose > 0, `${id} has no prose`);
      const o = await overflow(page);
      L.check(!o.page && !o.main, `${id} overflows at ${width}px`);
      const box = await badge.boundingBox();
      L.check(box && box.x >= 0 && box.x + box.width <= width + 1, `${id} badge outside viewport`);
      out.push({ id, width, badge: text, paragraphs: prose, outlineLinks: outline });
    }
  }
  await page.setViewportSize({ width: 1280, height: 900 });
  await page.goto(`${L.BASE}${base}?q=${encodeURIComponent("connect chatgpt")}`);
  await page.locator(".docs-result").first().waitFor({ timeout: 15000 });
  const listed = await page.locator(".docs-result").evaluateAll((els) =>
    els.map((e) => ({
      title: e.querySelector("h2")?.textContent?.trim(),
      badge: e.querySelector(".docs-badge")?.textContent?.trim() ?? null,
    })),
  );
  const chatgpt = listed.find((r) => r.title === L.titleOf("connect-chatgpt"));
  L.check(chatgpt?.badge === BADGE, `search result badge ${JSON.stringify(chatgpt)}`);
  const verified = listed.filter((r) => r.badge && !ids.map(L.titleOf).includes(r.title));
  L.check(verified.length === 0, `badge on a verified guide: ${JSON.stringify(verified)}`);
  return { guides: out, searchResults: listed.slice(0, 6) };
}

async function anchorAliases(page, base, eligible) {
  const out = [];
  for (const r of L.bundle.redirects) {
    if (eligible && !eligible.includes(r.to.split("#")[0])) {
      out.push({ from: `${base}/${r.from}`, skipped: "target guide is outside this reader's Help" });
      continue;
    }
    // A fresh document each time, so the alias is resolved on load.
    await page.goto("about:blank");
    const [from, fromAnchor] = r.from.split("#");
    const [to, toAnchor] = r.to.split("#");
    await page.goto(`${L.BASE}${base}/${from}#${fromAnchor}`);
    await page.waitForURL((u) => u.pathname === `${base}/${to}` && u.hash === `#${toAnchor}`, {
      timeout: 15000,
    });
    await waitTitle(page, L.titleOf(to), "alias target title missing");
    await waitFocus(page, (a) => document.activeElement?.id === a, toAnchor, "alias anchor not focused");
    L.check(
      (await page.locator(".docs-notice[role=status]").count()) === 0,
      "missing-section notice on a published alias",
    );
    out.push({ from: `${base}/${r.from}`, to: new URL(page.url()).pathname + new URL(page.url()).hash });
  }
  // Observation only: an alias reached by a same-page hash change (address bar or old in-page link).
  const hops = L.bundle.redirects.filter((r) => !eligible || eligible.includes(r.to.split("#")[0]));
  let samePage = null;
  if (hops.length >= 2) {
    const [a, b] = hops;
    await page.goto(`${L.BASE}${base}/${a.from}`);
    await waitTitle(page, L.titleOf(a.to.split("#")[0]), "alias target title missing");
    await page.waitForTimeout(500);
    await page.evaluate((h) => (location.hash = h), `#${b.from.split("#")[1]}`);
    await page.waitForTimeout(1500);
    samePage = await page.evaluate((target) => {
      const el = document.getElementById(target);
      const top = el?.getBoundingClientRect().top ?? null;
      return {
        hash: location.hash,
        focus: `${document.activeElement?.tagName}#${document.activeElement?.id}`,
        targetTop: top === null ? null : Math.round(top),
        targetInView: top !== null && top >= -2 && top < innerHeight,
      };
    }, b.to.split("#")[1]);
  }
  return { aliases: out, samePageHashChange: samePage };
}
...
async function staffRole(role) {
  ...
  await step(role, "Unverified guides show the validation badge and still read (DOC-031 open)", () =>
    unverifiedBadge(page, "/help", "staff-help"),
  );
  await step(role, "Published anchor aliases land on the renamed section", () =>
    anchorAliases(page, "/help", expected),
  );
  await step(
    role,
...
async function businessUser() {
  ...
  await step(role, "Contextual Help on Portal home, form and Request pages", async () => {
    ...
    // DOC-030: adc4f954 removed the Request page's "Help with this page" link (as 41390ca0 did on
    // the staff Inbox). Header Help carries portal.request instead; the absence is a finding.
    const reqHeader = page.locator('a[aria-label="Help"]').first();
    await reqHeader.waitFor({ timeout: 15000 });
    await page.waitForTimeout(1000);
    const contextualOnRequest = await page.getByRole("link", { name: "Help with this page" }).count();
    const reqHeaderHref = await reqHeader.getAttribute("href");
    await reqHeader.click();
    await page.waitForURL((u) => u.pathname === "/portal/help", { timeout: 15000 });
    const reqTopics = new URL(page.url()).searchParams.getAll("topic");
    L.check(reqTopics.includes("portal.request"), `Request header Help lacks portal.request: ${reqHeaderHref}`);
    L.check(!/\d{2,}|[0-9a-f]{8}-/.test(reqHeaderHref), `record identifier in ${reqHeaderHref}`);
    await page.locator(".docs-result").first().waitFor({ timeout: 15000 });
    const results = await page.locator(".docs-result h2").allTextContents();
    L.check(results.includes(L.titleOf("follow-request")), `Request topic results ${results}`);
    const expectedTopics = ["portal.home", "portal.form"];
    out.forEach((o, i) =>
      L.check(o.href === `/portal/help?topic=${expectedTopics[i]}`, `${o.route} ${o.href}`),
    );
    out.push({
      route: "/portal/requests/:number",
      contextualLinks: contextualOnRequest,
      headerHelp: reqHeaderHref,
      topics: reqTopics,
    });
  ...
  await step(role, "Unverified guides show the validation badge and still read (DOC-031 open)", () =>
    unverifiedBadge(page, "/portal/help", "portal-help"),
  );
  await step(
    role,
    "Unknown article, unknown anchor, unbundled edition and unknown topic recover locally",
...
async function anonymous() {
  ...
  await astep(role, `Signed-out /documentation lists all ${L.TOTAL} guides`, async () => {
    await page.goto(`${L.BASE}/documentation`);
    const heading = await h1(page);
    const count = await indexCount(page);
    L.check(count === L.TOTAL && expected.length === L.TOTAL, `count ${count}`);
    const edition = await page.locator("#edition dd").allTextContents();
    return { heading, count, servedEdition: edition };
  });
  await astep(role, "The served content digest matches the distribution commit's compile", async () => {
    await page.goto(`${L.BASE}/documentation`);
    await page.locator(".docs-collection").first().waitFor({ timeout: 15000 });
    const edition = await page.locator("#edition dd").allTextContents();
    L.check(
      edition.includes(L.bundle.edition.contentDigest),
      `served digest ${edition[3]} differs from local compile ${L.bundle.edition.contentDigest}`,
    );
    return { digest: L.bundle.edition.contentDigest, distribution: edition[2] };
  });
  await astep(role, "Only the two known-open guides carry the validation badge", async () => {
    const badged = [];
    for (const id of expected) {
      await page.goto(`${L.BASE}/documentation/${id}`);
      await h1(page);
      if (await page.locator("#docs-main .docs-article-meta .docs-badge").count()) badged.push(id);
    }
    const unexpected = badged.filter((id) => !UNVERIFIED.includes(id));
    L.check(unexpected.length === 0, `badge on guides the catalog marks verified: ${unexpected}`);
    return { badged };
  });
  ...
  await astep(role, "Unverified guides show the validation badge and still read (DOC-031 open)", () =>
    unverifiedBadge(page, "/documentation", "formal"),
  );
  await astep(role, "Published anchor aliases land on the renamed section", () =>
    anchorAliases(page, "/documentation"),
  );
  await astep(
    role,
    "Unknown article, unknown anchor, unbundled edition and unknown topic recover locally",
