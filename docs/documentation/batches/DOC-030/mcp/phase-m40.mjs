...
function guideBashBlock(address) {
  ...
  const lines = block
    .replaceAll("https://openlaw.company.example/mcp", address)
    .trimEnd()
    .split("\n");
  ...
  return {
    script: [...lines.slice(0, -1), "claude mcp get openlaw"].join("\n"),
    dropped: "claude",
  };
}
...
function claudeCodeFollow(key) {
  ...
  let removed = null;
  if (!/not found|No MCP server/i.test(existing))
    removed = runClaude(["mcp", "remove", "--scope", "user", "openlaw"]);
  ...
  const leaked = files
    ...
    .map((f) => path.relative(CLAUDE_HOME, f));
  ...
  for (const f of files.filter((f) => f.endsWith(".json"))) {
    try {
      ...
      if (server)
        storedHeader = {
          file: path.relative(CLAUDE_HOME, f),
          type: server.type,
          url: server.url,
  ...
  const listWrongKey = runClaude(["mcp", "list"], {
    OPENLAW_API_KEY: "ol_not_a_real_key_for_doc030",
  });
...
async function sdkChecks(key, person) {
  ...
  const search = await client.callTool({
    ...
    arguments: { query: "Connect a headless Client" },
  });
  const hits = (search.structuredContent?.articles ?? search.structuredContent?.results ?? []).map(
    (a) => a.id ?? a.slug,
  );
  const readHeadless = await client.callTool({
    name: "openlaw_docs_read",
    arguments: { id: "connect-headless-client" },
  });
  const readConfigure = await client.callTool({
    name: "openlaw_docs_read",
    arguments: { id: "configure-mcp" },
  });
  await client.close();
  const bearer = await rawToolsList(MCP_URL, { authorization: `Bearer ${key}` });
  expectThat(
    who.structuredContent?.person?.email === person.email,
    `whoami returned ${JSON.stringify(who.structuredContent?.person)}`,
  );
  ...
  expectThat(
    bearer.status === 200 && JSON.stringify(bearer.names) === JSON.stringify(names),
    `Bearer answered ${bearer.status} with ${bearer.names?.length} vs ${names.length} Tools`,
  );
  return {
    names,
    ...
    docsReadUnverified: [
      readHeadless.structuredContent?.unverified,
      readConfigure.structuredContent?.unverified,
    ],
...
function expireInDb(requestId, clientName) {
  ...
  const out = execFileSync(
    "docker",
    ["exec", `${LAB.project}-postgres-1`, "psql", "-U", "openlaw", "-d", "openlaw", "-tAc", sql],
    { encoding: "utf8" },
  );
...
await step(
  ...
  "Open Settings → Organization → MCP, after Integrations and before Advanced",
  "MCP sits after Integrations and before the Advanced group; MCP and every group switch start off",
  async () => {
    ...
    for (const label of [
      ...
      "Legal Users API keys",
      "Business Users OAuth Clients",
      "Business Users API keys",
    ])
      switches[label] = await admin
...
await step(
  as(CM, "legal_team_member", "/settings/mcp"),
  "Only Administrators manage the section: a Legal Team Member has no Organization group and the MCP address sends them to Profile",
  "No Organization group; /settings/mcp lands on /settings/profile",
  async () => {
...
await step(
  as(CH, "legal_team_member", "/settings/api-keys"),
  "Troubleshooting row 'No Request a key button' while MCP and API keys are off",
  "API keys pane has no Request a key button",
  async () => {
...
await step(
  as(CM, "administrator", "/settings/mcp"),
  "Enable API keys 1: turn on the switch beside MCP is off",
  "Title changes to MCP is on; Settings saved.",
  async () => {
    await gotoMcp(admin);
...
await step(
  as(CM, "administrator", "/settings/mcp"),
  "Enable API keys 2: Copy address beside Server address",
  "Clipboard holds the /mcp Server address",
  async () => {
...
await step(
  as(CM, "administrator", "/settings/mcp"),
  "Enable API keys 3: Legal Users API keys on; Business Users API keys on",
  "Each switch saves at once with Settings saved.",
  async () => {
    const a = await toggle(admin, "Legal Users API keys", true);
    const b = await toggle(admin, "Business Users API keys", true);
    const caption = (await main(admin)).match(
      /Administrators and Legal Team Members\.[^.]*\./,
    )?.[0];
...
await step(
  as(CM, "administrator", "/settings/mcp"),
  "Enable API keys 4: expand Toolset ceiling; all 13 start selected; clear one",
  "13 of 13 selected; clearing Administration saves",
  async () => {
    ...
    const labels = await boxes.evaluateAll((els) =>
      els.map((e) => [
        ...
        e.getAttribute("aria-checked") ?? e.getAttribute("data-state"),
      ]),
    );
    ...
    const status = await saved(admin, () =>
      admin.getByRole("checkbox", { name: "Administration", exact: true }).click(),
    );
...
await step(
  as(CM, "administrator", "/settings/mcp"),
  "Enable API keys 6: API key lifetime (days) set to 30 and saved with Enter",
  "Saves on Enter with Settings saved.",
  async () => {
...
await step(
  as(CH, "legal_team_member", "/settings/api-keys"),
  "Request and collect 1-4: Settings → Personal → API keys, Request a key, Toolsets, Scope, Note, Send request",
  "Request an API key dialog; Toolsets limited to the ceiling; nothing selected; Write offered; row Pending approval",
  async () => {
    await apiKeysPane(ltm, "legal_team_member");
    const r = await requestKey(ltm, {
      name: ltmName,
      toolsets: ["Contracts", "Matters"],
      scope: "write",
      note: "DOC-030 fictional task note",
    });
    expectThat(r.status === 201, `POST answered ${r.status}`);
    created.push(r.id);
    keys.ltm = { id: r.id, name: ltmName };
    expectThat(
      !r.offered.includes("Administration") && r.offered.length === 12,
      `offered ${r.offered}`,
    );
    expectThat(r.disabledBefore && r.writeShown, "Send request enabled early or Write missing");
    expectThat(
      /Nothing is selected for you\./.test(r.dialogText) &&
        /Expires 30 days after approval\./.test(r.dialogText),
      r.dialogText,
    );
...
await step(
  as(CM, "administrator", "bell"),
  "Approve a request 1-2: bell → Your approvals, check person, Client, Toolsets, scope; Mark all read leaves it; Approve",
  "The approval shows Nadia Haddad, the Client name, Contracts, Matters and Write; it leaves the group once approved",
  async () => {
...
await step(
  as(CH, "legal_team_member", "/settings/api-keys"),
  "Request and collect 5-6: return to API keys and reload; Your key is ready; Copy; outside click keeps it; Esc closes",
  "Key shown once with Copy and Done; outside click does not close; Esc does; row Active; later read has no key",
  async () => {
    await ltm.reload();
    ...
    keys.ltm.key = r.key;
    expectThat(
      r.key.startsWith("ol_") && r.survivedOutside,
      "no key or dialog closed on outside click",
    );
    expectThat(
      r.href === "/documentation/connect-headless-client" && r.target === "_blank",
      `${r.href} ${r.target}`,
    );
    await ltm.reload();
    await pause(1500);
    ...
    expectThat(
      again === 0 && /Active/.test(row) && !reread.body?.key,
      "dialog came back or key re-readable",
    );
...
await step(
  as(CH, "legal_team_member", "terminal"),
  "Connect Claude Code: run the guide's Bash block literally (address replaced), then claude mcp list",
  "openlaw connects; the saved header keeps ${OPENLAW_API_KEY}; the key is in no file",
  async () => {
    ...
    expectThat(
      JSON.stringify(r.storedHeader?.headers ?? {}).includes("${OPENLAW_API_KEY}") &&
        r.keyInAnyFile.length === 0,
      JSON.stringify(r.storedHeader),
    );
...
let ltmTools;
await step(
  as(CH, "legal_team_member", "SDK Client"),
  "Connect with an SDK Client (Streamable HTTP, x-api-key); Guide Tools read the articles; Bearer also accepted",
  "Granted Tools listed; whoami is Nadia; docs search finds the guide",
  async () => {
    ...
    expectThat(
      [
        ...
        "openlaw_matters_list",
        "openlaw_matter_create",
      ].every((t) => n.includes(t)),
      n.join(","),
    );
    expectThat(
      !n.some((t) => t.startsWith("openlaw_entit") || t.startsWith("openlaw_knowledge")),
      "tools outside the grant listed",
    );
...
await step(
  as(CH, "legal_team_member", "mcp-remote"),
  "Connect Claude Cowork or Claude Desktop: the guide's mcpServers entry through npx -y mcp-remote@0.14.3 (headless), then the --header-file variant",
  "The bridge connects with the key from env; lists Tools; whoami is Nadia",
  async () => {
    ...
    const fileArgs = [
      ...entry.args.slice(0, i),
      "--header-file",
      headerFile,
      ...entry.args.slice(i + 2),
    ];
    ...
    log.runs.m40.bridge = {
      args: entry.args,
      viaEnv: { ...viaEnv },
      viaFile: { ...viaFile, args: fileArgs.map((a) => (a === headerFile ? "<header file>" : a)) },
      noEnv,
    };
    save();
    expectThat(!viaEnv.error && /Nadia/.test(viaEnv.whoami), `env: ${JSON.stringify(viaEnv)}`);
    expectThat(
      !viaFile.error && /Nadia/.test(viaFile.whoami),
      `header file: ${JSON.stringify(viaFile)}`,
    );
...
await step(
  as(CM, "administrator", "/settings/mcp"),
  "Enable API keys 5: Read-only on blocks writes from an existing Write key and hides Write in the request form",
  "Next tools/list drops write Tools; request form hides Write",
  async () => {
    await gotoMcp(admin);
    await expandCard(admin, "Toolset ceiling");
    ...
    ).callTool(MCP_URL, { "x-api-key": keys.ltm.key }, "openlaw_matter_create", {
      matterTypeId: "x",
      answers: {},
    });
    await apiKeysPane(ltm, "legal_team_member");
    ...
    expectThat(
      !after.names.includes("openlaw_matter_create") &&
        back.names.includes("openlaw_matter_create") &&
        radios.length === 1,
      JSON.stringify({ after: after.names, radios }),
    );
...
await step(
  as(CM, "administrator", "/settings/mcp"),
  "Removing a Toolset from the ceiling blocks its Tools even on an existing key",
  "Next tools/list drops Matters Tools; re-adding restores them",
  async () => {
    const s1 = await saved(admin, () =>
      admin.getByRole("checkbox", { name: "Matters", exact: true }).click(),
    );
    const after = await rawToolsList(MCP_URL, { "x-api-key": keys.ltm.key });
    const s2 = await saved(admin, () =>
      admin.getByRole("checkbox", { name: "Matters", exact: true }).click(),
    );
    const back = await rawToolsList(MCP_URL, { "x-api-key": keys.ltm.key });
    expectThat(
      !after.names.some((n) => n.includes("matter")) && back.names.includes("openlaw_matters_list"),
...
await step(
  as(CM, "administrator", "/settings/mcp"),
  "Turning a group's API keys off, or MCP off, blocks that group's next Client request",
  "401 while off; works again when on",
  async () => {
    ...
    const back = await rawToolsList(MCP_URL, { "x-api-key": keys.ltm.key });
    expectThat(
      offKeys.status === 401 && offMcp.status === 401 && back.status === 200,
      JSON.stringify([offKeys.status, offMcp.status, back.status]),
    );
...
await step(
  as(CM, "administrator", "/settings/mcp"),
  "Approval checks the current policy: a Write request under Read-only fails in the card dialog and in the bell; Deny with a note",
  "Card dialog names the reason; bell says The request could not be handled. Try again.; denied row shows the note",
  async () => {
    await apiKeysPane(ltm, "legal_team_member");
    const r = await requestKey(ltm, {
      name: recheckName,
    ...
    expectThat(
      /Read-only|read-only|Write/i.test(cardError) &&
        bellError === "The request could not be handled. Try again." &&
        /Denied/.test(denied) &&
        /DOC-030 fictional denial note/.test(denied),
      JSON.stringify({ cardError, bellError, denied }),
    );
...
await step(
  as(CH, "legal_team_member", "/settings/api-keys"),
  "Troubleshooting 'Pending approval': Cancel request on its row and confirm",
  "Row changes to Cancelled",
  async () => {
...
await step(
  as(CH, "administrator", "/settings/api-keys"),
  "An Administrator's own request approves itself; Your key is ready at once; Copy; Done",
  "Dialog appears immediately; row Active; later read has no key",
  async () => {
    await apiKeysPane(admin, "administrator");
    const r = await requestKey(admin, {
      name: adminName,
      toolsets: ["Contracts", "Entities"],
      scope: "read",
    });
    created.push(r.id);
...
await step(
  as(CH, "administrator", "terminal"),
  "Connect Claude Code with the Administrator's key (guide block)",
  "claude mcp list shows openlaw connected",
  async () => {
...
await step(
  as(CH, "administrator", "SDK Client"),
  "SDK Client with the Administrator's key; Guide Tools",
  "Contracts and Entities read Tools plus Guide Tools; whoami is Daniel",
  async () => {
    const r = await sdkChecks(keys.admin.key, PEOPLE.administrator);
    expectThat(
      r.names.includes("openlaw_contracts_list") &&
        r.names.some((n) => n.includes("entit")) &&
        !r.names.includes("openlaw_matters_list"),
      r.names.join(","),
    );
...
await step(
  as(CH, "business_user", "/portal/settings/api-keys"),
  "Business User: Notification settings gear → API keys → Request a key",
  "Request an API key dialog; Send request; row Pending approval",
  async () => {
    await apiKeysPane(bu, "business_user");
    const r = await requestKey(bu, {
      name: buName,
      toolsets: ["Requests", "Knowledge"],
      scope: "read",
      note: "DOC-030 fictional portal note",
    });
    created.push(r.id);
...
await step(
  as(CM, "administrator", "/settings/mcp"),
  "Approve a request 2: API key requests card, note under the Client name, Approve dialog with Note (Optional)",
  "Card Approve opens a dialog with Note (Optional); the request leaves the card",
  async () => {
    await gotoMcp(admin);
    ...
    await until(
      async () =>
        (await admin.getByRole("row").filter({ hasText: buName }).count()) === 0 ||
        !/Pending/.test(await rowText(admin, buName)),
...
await step(
  as(CH, "business_user", "/portal/settings/api-keys"),
  "Business User returns, reloads, collects the key once",
  "Your key is ready; row Active; later read has no key",
  async () => {
    await bu.reload();
...
await step(
  as(CH, "business_user", "terminal"),
  "Connect Claude Code with the Business User's key (guide block)",
  "claude mcp list shows openlaw connected",
  async () => {
...
await step(
  as(CH, "business_user", "SDK Client"),
  "SDK Client with the Business User's key; Guide Tools",
  "Requests and Knowledge Tools plus Guide Tools; whoami is Jonas",
  async () => {
    const r = await sdkChecks(keys.bu.key, PEOPLE.business_user);
    expectThat(
      r.names.some((n) => n.includes("request")) && !r.names.includes("openlaw_contracts_list"),
      r.names.join(","),
    );
...
await step(
  as(CM, "administrator", "/settings/mcp"),
  "Revoke and inspect 1: expand Active keys and grants; counts and the eight columns",
  "Owner, Client, Toolsets, Scope, Status, Granted, Last used, Expires; our keys listed",
  async () => {
    await gotoMcp(admin);
    const before = await admin
      .getByRole("button", { name: "Active keys and grants", exact: true })
    ...
    expectThat(
      before === "false" &&
        ["Owner", "Client", "Toolsets", "Scope", "Status", "Granted", "Last used", "Expires"].every(
          (h) => heads.map(flat).includes(h),
        ),
...
await step(
  as(CM, "administrator", "/settings/audit-log/tool-calls"),
  "Revoke and inspect 3: Tool calls in the last day → Audit log → Tool calls; columns, From/To, Export CSV",
  "Lists When, Person, Client, Tool, Outcome, Duration with our calls; CSV downloads",
  async () => {
    ...
    const hasFromTo =
      (await admin.getByLabel("From").count()) > 0 && (await admin.getByLabel("To").count()) > 0;
    ...
    const [download] = await Promise.all([
      ...
      admin.getByRole("link", { name: "Export CSV" }).click(),
    ]);
    const file = await download.path();
    const csv = readFileSync(file, "utf8");
    const header = csv.split("\n")[0];
    expectThat(
      ["When", "Person", "Client", "Tool", "Outcome", "Duration"].every((h) => heads.includes(h)) &&
        hasFromTo &&
        csv.includes(ltmName),
      JSON.stringify({ heads, header }),
    );
...
for (const role of ["legal_team_member", "administrator", "business_user"]) {
  const k =
    role === "legal_team_member" ? keys.ltm : role === "administrator" ? keys.admin : keys.bu;
  const page = role === "legal_team_member" ? ltm : role === "administrator" ? admin : bu;
  const sel = { ...CH, role };
  await step(
    { ...sel, page: "fixture" },
    "Fixture setup (not a reader step): move this key's expiry into the past in the work2 database",
    "One api_keys row updated",
    async () => expireInDb(k.id, k.name),
  );
  await step(
    { ...sel, page: "SDK Client" },
    "Negative: an expired key cannot connect; the row says Expired",
    "401 Unauthorized; the API keys row reads Expired",
    async () => {
  ...
  if (role === "legal_team_member") {
    await step(
      { ...CM, role: "administrator", page: "SDK Client" },
      "Negative (V-M40-MCP): an expired key cannot connect",
      "401",
...
await step(
  ...
  async () => {
    const revoked = log.steps.find(
      (s) => s.phase === "m40" && s.step.startsWith("Revoke and inspect 2"),
    );
...
log.runs.m40.articleHashesAtEnd = {
...
console.log(
  `m40 done: ${log.steps.filter((s) => s.phase === "m40").length} steps, ${failed.length} failed`,
);
