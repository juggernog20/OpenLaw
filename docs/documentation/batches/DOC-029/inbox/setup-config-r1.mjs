// DOC-029 inbox round 1: named fixture configuration on the shared work lab.
// Creates only records whose names start with "DOC-029 inbox". Idempotent by name.
// Run: LAB_PASSWORD=... mise exec -- node docs/documentation/batches/DOC-029/inbox/setup-config-r1.mjs
...
const goodTemplate = await ensureTemplate(`${P} Advisory template`, {
  description: "Fictional template for the DOC-029 inbox walkthrough.",
  defaultPriority: "low",
...
const out = {
  note: "Fixture identifiers for the DOC-029 inbox round 1 walkthrough. No credentials.",
...
writeFileSync(path.join(here, "fixtures-r1.json"), JSON.stringify(out, null, 2));
