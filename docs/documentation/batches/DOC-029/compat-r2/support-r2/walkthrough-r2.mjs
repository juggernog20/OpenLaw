...
// The script creates only records named "DOC-029r2 support ..." and changes no organisation settings.
...
const fixture = (f) => readFileSync(path.join(here, "../../support/fixtures", f));
...
function labStatus() {
  try {
    const text = execFileSync(
      "mise",
      ["exec", "--", "node", "scripts/documentation/lab.mjs", "status", "admin2"],
      { cwd: root, encoding: "utf8" },
    );
    const project = text.match(/(openlaw-docs-[0-9a-f]+-admin2)-app-1/)?.[1] ?? null;
...
const results = {
  ...
  reviewer: "DOC-029r2 compatibility reviewer (support)",
  reviewerKind: "agent",
  appUrl: BASE,
  mailUrl: MAIL,
  lab: labStatus(),
...
async function shot(page, name) {
  ...
  return `docs/documentation/batches/DOC-029/compat-r2/support-r2/${name}`;
}
...
const title = (t) => `DOC-029r2 support ${t} ${stamp}`;
...
const FX = { stamp };
{
  ...
  const folderId = must(
    ...
    "folders",
  ).folders.find((f) => f.name === "DOC-029r2 support folder").id;
  ...
  must(
    await N.api("PATCH", `/knowledge/${kItem.id}`, {
      ...
      body: "DOC-029r2 support fictional guidance for the Portal file check.",
    }),
    "k patch",
  );
  ...
  const rq = must(
    await J.api("POST", "/requests", {
      ...
      departmentId: sales,
      title: title("request"),
      description: "DOC-029r2 support fictional ask for a contract review.",
      urgency: "medium",
  ...
  const rq2 = must(
    await J.api("POST", "/requests", {
      ...
      description: "DOC-029r2 support fictional ask left open for the Convert dialog.",
      urgency: "low",
      customFields: { counterparty_name: "DOC-029r2 Fictional Counterparty Ltd" },
    }),
    "request2",
...
if (run("T")) {
  ...
  await step(
    ...
    async () => {
      ...
      await page.getByLabel("Password").fill("DOC-029r2-support-wrong-password");
  ...
  await step(
    ...
    async () => {
      ...
      const screenshot = await shot(p2, "r2-portal-link-expired.png");
  ...
  await step(
    ...
    async () => {
      ...
      const screenshot = await shot(p, "r2-admin-contract-not-found.png");
  ...
  await step(
    ...
    async () => {
      ...
      const screenshot = await shot(p, "r2-member-page-could-not-load.png");
  ...
  await step(
    ...
    async () => {
      ...
      await p
        ...
        .first()
        .fill("DOC-029r2 support administrator confidential")
        .catch(() => {});
  ...
  await step(
    ...
    async () => {
      ...
      const patch = await J.api("PATCH", `/contracts/${FX.shared.number}`, {
        description: "DOC-029r2 support Business User edit attempt",
      });
      ...
      await comments
        ...
        .fill(`DOC-029r2 support Portal reply ${stamp}`);
      await comments
      ...
      await comments
        .getByText(`DOC-029r2 support Portal reply ${stamp}`)
        .last()
  ...
  await step(
    ...
    async () => {
      ...
      dialog = await uploadDialog(
        p,
        FX.uploads.number,
        path.join(here, "../../support/fixtures", "doc029-support-note.txt"),
      );
  ...
  await step(
    ...
    async () => {
      ...
      await dialog
        ...
        .setInputFiles(path.join(here, "../../support/fixtures", "doc029-support-v2.txt"));
      await dialog.getByRole("button", { name: "Upload", exact: true }).click();
      await dialog.waitFor({ state: "hidden", timeout: 30000 });
      await settle(p);
      const rowText = clean(
  ...
  await step(
    ...
    async () => {
      ...
      const dialog = await uploadDialog(
        p,
        FX.uploads.number,
        path.join(here, "../../support/fixtures", "doc029-support-damaged.docx"),
      );
      await dialog.waitFor({ state: "hidden", timeout: 30000 });
      await settle(p, 300);
      const docs = must(
      ...
      const screenshot = await shot(p, "r2-member-preview-failed.png");
  ...
  await step(
    ...
    async () => {
      ...
      const screenshot = await shot(p, "r2-admin-move-past-approval.png");
  ...
  for (const role of [...staffRoles, "business_user"]) {
    ...
    await step(
      ...
      async () => {
        ...
        const screenshot =
          role === "administrator" ? await shot(p, "r2-admin-edition-details.png") : null;
        await p
          .getByRole("navigation", { name: "Documentation" })
