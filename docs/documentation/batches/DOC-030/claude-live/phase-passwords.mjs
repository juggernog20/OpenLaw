...
if (!SEED || !NEW || SEED === NEW)
  throw new Error("Set SEED_PASSWORD and a different LAB_PASSWORD.");
...
const S = {
  ...
  page: "/api/auth (loopback, before exposure)",
  secrets: [SEED, NEW],
};
...
async function signIn(email, password) {
  const r = await fetch(`${LAB.loopback}/api/auth/sign-in/email`, {
    method: "POST",
    headers: headers(),
...
async function adminCall(cookie, method, url) {
  const r = await fetch(`${LAB.loopback}${url}`, {
    method,
...
await step(
  S,
  "Change every password account's password to one fresh random value through the app's change-password endpoint",
  "9 of 9 changed with 200 (an archived account is restored, changed and archived again by the Administrator)",
  async () => {
    const out = [];
    for (const email of ACCOUNTS) {
      ...
      out.push(
        `${email}: archived (seed sign-in 403 USER_ARCHIVED); Daniel Okafor unarchive ${un.status}, own change-password ${changed}, archive again ${ar.status}`,
      );
    }
    expectThat(
      out.every((l) => / 200$|new password 200\)$/.test(l)),
      out.join("; "),
    );
...
await step(
  S,
  "Confirm the published seed password signs in no one, and the new one signs in everyone",
  "Seed password 401 for all 9; new password 200 for the 8 active accounts and 403 USER_ARCHIVED for archived Gabriel Santos",
  async () => {
    const seed = [];
    const fresh = [];
    for (const email of ACCOUNTS) {
    ...
    expectThat(
      seed.every((s) => s === 401) &&
        fresh
          .filter((s, i) => ACCOUNTS[i] !== "gabriel.santos@helix.example")
          .every((s) => s === 200) &&
        fresh[2] === 403,
      JSON.stringify({ seed, fresh }),
    );
