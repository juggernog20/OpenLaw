...
- **Consequences** — #182 closed `wontfix` with the grill trail; #206 and #207 opened. DOC-014 records what the grill forced on storage generally. DOC-012's clause that the interface "stays implementable" by an unlike backend did its job: the seam held, and what fell out was the backend, not the interface.
...
- **Consequences** — The M12 branch grows the derived-store seam and moves rendition writes onto it (noted on #187). #206 builds the router. When DEPLOYMENT.md documents WORM, it must say: split the containers before locking a policy, or rendition rebuilds and hard delete will fail against retention.
