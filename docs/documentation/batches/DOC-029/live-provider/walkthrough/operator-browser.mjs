...
try {
  ...
  for (const [id, article] of [
    ["OB01", "configure-signing"],
    ["OB02", "configure-analysis"],
  ]) {
    const entry = {
      id,
      article,
      ...
      action: `Signed out, open /documentation/${article}, follow the deployment configuration link, find the provider and key sections`,
    };
    try {
      ...
      const heading = await page
        .getByRole("heading", { name: "Preserve and rotate encryption keys" })
        .count();
      if (!(outbound && key && heading))
        throw new Error(`outbound=${outbound} key=${key} heading=${heading}`);
