...
**Avatar contrast addendum (2026-09-10, #756; amends DES-019):** Visible avatar initials use the 4.5:1 small-text contrast floor. They remain decorative in the accessibility tree (`aria-hidden="true"`), because the surrounding control or adjacent text supplies the person's name; that avoids duplicate announcements, but does not make the visible letters easier to read. Warm keeps its terracotta background (`#C97B5C`) and uses its existing dark accent foreground (`#1F2328`) for the initials, giving 4.87:1. Light and Dark already clear this floor. The contrast lint gate now checks avatar initials at 4.5:1 in all three themes. File-type icon squares retain their graphical-object threshold. The Warm foreground in DES-019's original palette is superseded; the uniform-avatar and photo behavior is unchanged.
...
**3. The envelope row is four cells: Signers | Document | Status | Sent.** Each is a fact the seam answers. **Signers** comes first, because "who was asked to sign this" is the question the row exists for, and each signer takes the two-line anatomy the Approver cell already uses — the name, and under it the address the invitation went to. **Document** names what went out with "Version {n}" beneath it, and both halves go to the DES-035 clause 8 em dash together once that version has been erased (DOC-010): the row still says an envelope was sent, which is the fact it is there for. **Sent** is the short date with "by {name}" beneath it, the Decided column's shape. The #1177 amendment on 2026-09-26 names the preparer as "Prepared by {name}" for preparation rounds, including after provider-confirmed sending. A draft says "Not sent". Historical rows keep their recorded sender. Session actors and the shared provider user never replace the preparer.
...
1. ~~**There is no frame, and there will not be one.** No `.pen` file draws an email, because an email is not a screen this product renders. The register above is the decision, and it is DES-015's voice plus the two additions an inbox forces.~~ Superseded by **DES-093**.
2. ~~**The body is plain text.** `MailMessage` carries `text` alone; an HTML alternative is TECH-011's to add, and adding it now would put two copies of every sentence in one file.~~ Superseded by **DES-093**.
3. **`→` and other decorative glyphs are not used**, even though a plain-text message has no Lucide to reach for. DES-008's substance is that glyphs are a system, and a message with no glyph system uses words. **Amended by DES-093 for HTML only:** the label dot, status arrow, step dots and `→` on a link are layout glyphs. The text part still uses words.
...
**1. The sections are cards.** Three `SettingsCard`s follow Provider and the Request conversion switches: ~~**System prompts** (the rules every AI run carries)~~, **Matter and Contract conversion prompts** (the built-in Conversion targets), and **Contract analysis prompts** (the core targets). No card is titled "Prompts" any more. _System prompts is superseded by Answer style in the 2026-09-20 amendment below._
...
### Built addendum (2026-09-18, M36/3–5, [#933](https://github.com/juggernog20/OpenLaw/issues/933)–[#935](https://github.com/juggernog20/OpenLaw/issues/935))
...
**Mobbin was attempted before drawing.** No Mobbin tool was exposed, plugin discovery returned no Mobbin integration, and an initialization request to the configured `https://api.mobbin.com/mcp` endpoint returned HTTP 401, "Missing or invalid Authorization header". No Mobbin screen was available to inspect. The references used are the repository's accepted records and shipped Switch, Dialog, type editor and section-strip components. External form-builder research remains a gap, not a claimed reference.

### Decision

**1. One Form tab, one card, one tree.** The type editor uses DES-032's routed section strip: **Details**, **Form**, then the existing **People** and **Approval defaults** where the type has them. Details keeps DES-022's identity card. Form lives at `/settings/{module}/types/:id/form`. These links replace the editor's taxonomy strip; they do not add another permanent strip. The breadcrumb returns to the types list. The Form tab replaces the Custom Fields and Default Fields cards with one card titled **Form**. Built-in Rows and attached Fields share the same order. A Branch is a nested part of that tree, not another card or navigation pane.

The card has a 44px header, a 40px column-heading strip and the tree. The header carries the type name as a caption and the root controls at the right: **Attach Field**, **Create Field**, **Add condition** and **Preview intake form**. They wrap onto a second header line when the card is narrow. Only a Branch has a footer, with its own two controls below its children. There is no Save form button. Each completed switch, reorder, attach, detach or condition edit commits the whole valid tree through the existing Form write. Only one unfinished Branch exists at a time: **Add condition** and **Add condition inside** are unavailable while one is open, with the reason on the control, because a second draft only deepens a tree that already cannot be saved. Removing an unfinished Branch always works, including when the result lands back on the saved tree (2026-09-22, from live review). An incomplete new Branch or condition stays local until it is complete, and stays silent there: its own editor shows no refusal while the person is still writing it, because unfinished is not wrong. Any other control that cannot save while that Branch is unfinished still says "Complete the Branch condition first" against itself (2026-09-22, from live review). Escape discards that draft. Existing condition values commit on blur or Enter, selects on change. Escape restores the prior value. The edited control carries the SET-003 pending/saved/error micro-state.

**2. Every Row reads across the same columns.** A Row has a 52px minimum height, with a `border-muted` separator. Its columns are:
...
The column headings use their full switch names, 11px semibold, with an accessible column prefix on each control. For example, "Business justification: Required for creation". Touchpoint is a fact, never a select or pill. On intake form wins; otherwise Required for creation gives Creation; otherwise Record. Visible on Portal does not change it.

Title and Type stay first at the root. They draw a presentational lock in the grip column, no grip, no overflow and no detach. Their accessible description states that their position and switches are fixed. The current API fixes On intake form off and Required for creation on for these two Rows. The Portal still collects Title once as a basic and derives Type from the destination. The builder must not turn that projection into a second Title or a requester-editable Type.
...
> Show when all of: Term type is Fixed, Value is greater than 50,000

Match Any changes the lead to "Show when any of:". A new Branch reads "Add a condition" until complete. The pencil opens conditions inline below the header; it does not hide the children. The editor is its own `bg-raised` panel across the card's width, drawn over any ancestor Branch's left rule: it sits inside those Branches' children, so their rules would otherwise run through its controls (2026-09-22, from live review). Match is asked as **AND** or **OR**, not set in advance (2026-09-22, from live review). A Branch with one condition shows no match control at all, because there is nothing to join. **Add another condition** on a Branch that holds exactly one condition first asks "Join the next condition with" and offers AND, OR and Cancel; the answer sets the Branch's match and then adds the empty condition. A later addition does not ask again, because one join covers the whole Branch. Between the first and second condition that join is a 32px select named "Join conditions" with AND and OR, which is how it is changed afterwards. Later joins repeat its word as a 12px muted caption, hidden from assistive technology, since the select above already carries the value. Below it, each condition has visible Row, Operator and Value labels, 32px controls, an 8px gap and a 28px remove button. Their desktop widths are 240px, 192px, flexible with a 160px minimum, and 28px. Editor padding is 12px vertically and 16px horizontally. Conditions have 12px between them. **Add condition** is a 28px text button beneath them. There is no second Save button in the editor.

The Row select contains only Rows above that Branch in depth-first document order. It excludes the Branch's own children and every later Row. A prior sibling Branch's Rows can be listed; their answers count as unanswered when that sibling is hidden, as DD-028 says. The select carries no caption while it has Rows to offer: its own contents say which ones are eligible (2026-09-22, from live review). With no eligible Rows it states "Add a Row above this Branch first" beside the select, and describes the select. A reorder must revalidate every reference before it commits.

| Row type or operator         | Value control                                                                                                                                                                               |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Text, Long text              | Text input.                                                                                                                                                                                 |
| Number                       | Number input; ordering operators compare numbers.                                                                                                                                           |
| Money, including Value       | Amount input with the Row's currency context, if set; no currency comparison. Format the displayed amount with the existing number helpers and convert it to minor units for the evaluator. |
| Currency                     | Currency-code select. No greater-than or less-than operators.                                                                                                                               |
| Date                         | Existing date control; local display, ISO date operand.                                                                                                                                     |
| Boolean                      | Yes / No select.                                                                                                                                                                            |
| Single select, Multi select  | Select from that Row's option labels; multiple choices for **is one of**.                                                                                                                   |
| User, Entity, Counterparties | Existing reached-record picker, showing names and storing identifiers. Portal preview uses Portal options, never the Administrator's wider options.                                         |
| **is set**                   | No Value control and no empty placeholder cell.                                                                                                                                             |

The operator labels are **is**, **is not**, **is one of**, **is set**, **is greater than**, **is less than**. Offer the last two only for number, money and date Rows. Changing Row or operator keeps a compatible operand; otherwise it clears the operand locally and asks for the new value beside that control. It must not save a condition containing a stale operand of the wrong type.

Children sit 24px inside the left rule. Only the name side gains indentation; switch and trailing columns keep the root's alignment. A nested Branch repeats the rule and indent, without a new card border. **Add field into condition**, below the children, opens an Attach Field / Create Field menu scoped to that Branch. Moving an existing Row uses drag onto the Branch's child area, or the grip's menu. The root controls append at root; the Branch action appends inside that Branch. The Branch overflow offers Add condition inside and Remove branch; its grip carries the depth moves like any Row. Remove branch keeps its children in place under the parent, in order, and removes only its conditions. The menu states "Keep children here" beside that removal. Validate references before committing the resulting tree.

**4. The header carries the root's actions; a Branch's own footer carries a smaller set.** Header, left to right after the caption: **Attach Field**, **Create Field**, **Add condition**, then **Preview intake form** at the far right. A Branch's own footer holds **Add field into condition** and **Add condition** only, no standalone Create Field: its Add field into condition menu already has a Create Field item that creates and attaches at that Branch in one step, so a second button beside it would just repeat it (2026-09-22, from live review). Buttons are 32px high with 8px gaps, 13px labels and 16px icons. Attach Field, or Add field into condition inside a Branch, opens the live, unattached Fields menu for this module, with name and 12px type caption, not a second catalog. Already attached Fields and all built-ins are absent. If nothing is available, the menu says "All Fields are attached" and still offers its own Create Field item. Attach creates a Record Row with all three switches off. The Administrator can turn on Visible on Portal before enabling intake. There is no retired Field tag to inherit.

Create Field, whether the header's own button or the item inside a Branch's Add field into condition menu, opens the existing DES-021 Field editor dialog without the retired tag control. A successful catalog create is followed by attachment at the chosen parent. A failed attachment keeps the created Field available and names the failure beside whichever control created it; retry must attach that Field, not create another. Add branch appends a local empty Branch and focuses its first Row select. An Entity Form with no Fields has a 52px caption row, "No Fields attached", above the header's own three actions.

**5. A refusal belongs to the control that caused it.** Keep the last valid tree and the person's focus. Place a 12px `text-danger` reason below the affected control or drop location, expanding the Row beyond 52px as needed. Associate it using `aria-describedby`; announce an attempted refusal without moving focus. Disabled combinations remain discoverable through their labelled wrapper and explanation. These are local preflight refusals, not a failed page save. A server refusal caused by stale data is mapped to the same control; a transport failure uses that action's retry micro-state.

| Trigger                                                                   | Refusal and location                                                                                                                                                                                          |
| ------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Enable intake while Visible on Portal is off                              | Disable intake; "Turn on Visible on Portal first" below it. Do not silently change another switch.                                                                                                            |
| Hide an Intake Field on Portal                                            | Disable Portal off; "Turn off On intake form first" below it.                                                                                                                                                 |
| Require an Intake User Row                                                | Disable Required; "Required for creation is unavailable for Finance reviewer while it is on the intake form" beside Required. Conversely, intake is unavailable on a required User Row until Required is off. |
| Move a referenced Row below its Branch, or move a Branch above its source | Keep order; "Keep Term type above the Branch that uses it" at the target or menu choice.                                                                                                                      |
| Move Title or Type, duplicate a Row, or detach a built-in                 | Do not offer the action. The pin description, Attach menu filtering and built-in action slot explain the constraint.                                                                                          |
| Detach a condition source                                                 | "Used by a Branch condition. Change the condition first" beside detach.                                                                                                                                       |
| Invalid operand or incomplete new condition                               | Reason below Value or Row. Keep the edit local and the saved tree valid.                                                                                                                                      |

**6. Keyboard changes order and depth separately.** A grip is a focusable button named "Move {name}". Arrow Up / Down moves one place among siblings, moving a Branch with its descendants. At a boundary it stays put. Announce name, new position, sibling count and parent in a polite live region; focus follows the same grip. Pinned Rows cannot be crossed. Click, Enter or Space on the grip opens its menu with **Put under a condition…** and **Move out of the condition**; the Row overflow does not repeat them, so a pointer user who drags never meets them, and a built-in Row draws no overflow at all (2026-09-22, from live review). Put under a condition opens a labelled Branch chooser, named by condition sentence and parent path. It excludes self and descendants and marks invalid destinations with their reason. Move out puts the node immediately after its former parent Branch. Root nodes show it unavailable with "Already at the root". The menu anchors on an invisible sibling of the grip: a Radix trigger prevents default on pointer down, and Chromium then never starts the native drag. Neither horizontal Arrow keys nor indentation alone changes depth. Escape closes a menu and returns focus to its trigger.

Pointer drag has the same validation and one commit on drop. A 2px `accent` insertion rule marks sibling placement. Dropping on a Branch's child area moves inside; its outlined target and sentence identify the destination. Invalid targets retain a reason beside that target. Tab reaches every switch, edit control and menu without requiring a drag gesture.

**7. The Request type editor reads the destination Form.** Its identity card keeps display name, description, turnaround and destination. Its second card is **Intake form**, with the resolved destination type's display name as a 12px caption. The 28px eye button in the header is named "Preview intake form", and a secondary **Edit form** button sits to its right and opens the destination type's Form tab. The card body uses 52px name/caption rows, a trailing 12px Required/Optional fact and no switches, grips, detach or edit buttons. Department and Urgency are fixed at the top, Title appears once, and Attachments is fixed last. Type is the destination, not a question. Other built-in and Field Intake Rows follow the destination tree in order. Branches that contain Intake Rows retain their sentence and left rule, including nested headers. This card shows configured children even before answers exist; it is an inventory, not the evaluating preview.

The card has no footer link; the header's Edit form button is the one way to the Form tab. A module-only destination resolves to the module's current Default type and uses its actual display name in the caption. A destination change refreshes the card. No companion-attach offer or independent Request form order remains. Where the card has no optional Intake Rows it still shows the fixed basics; it does not claim the form is empty.

**8. Preview is a dialog containing the Portal form.** The card's eye opens the preview. From the Request type editor, use its display name and description. From a type Form, use a Request type selector only when several point at it; one is selected directly. With none, show the type name and "No Request type uses this Form yet" in the dialog header. Do not invent a Request type or save anything.

Use the shared Dialog at `3xl`, 768px maximum width, with 24px padding. It is fullscreen below the existing 768px viewport breakpoint; above it, keep 16px horizontal and 32px vertical viewport clearance with internal scrolling. Title: **Preview intake form**, 18px semibold. Caption: "Preview only. No Request will be sent." Inside, draw the requester-facing title, description, labels, required marks, controls and attachments with the Portal renderer. No Branch sentence, editing chrome or Touchpoint caption reaches the requester. Value remains one labelled compound control even when its amount, currency and cadence wrap.

The shared evaluator runs on every answer change. In the chosen example, Expiry date and Business justification are absent until Term type is Fixed **and** Value is greater than 50,000. Then both appear at their position in the tree. Changing the answers back hides them and removes their required enforcement. Preview must preserve DD-028's actual empty-value semantics: **is not** can be true before an answer; it must not add a blanket "all conditions answered" gate. A source hidden by another Branch counts as unanswered. Preview does not delete held answers when a Branch hides them.

Submit request is present to exercise client validation, but never posts or uploads. With valid answers it reports "Preview complete. No Request was sent" inside the dialog. Attachment selection may show local filenames only. Close and Escape discard preview answers and return focus to the opening eye or footer button. New answers do not move focus automatically; a polite announcement reports newly shown questions. All real inputs use their normal labels, Portal permission filters and focus order.

**9. Sizes and tokens come from the existing ramp.** This is the complete vocabulary for the chosen content. No CSS token is added. The existing shell keeps its own tokens and dimensions.

| Use               | Existing token or size                                                                                                                                                                                                                                                                                           |
| ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Surface           | `bg-canvas`, `bg-raised`, `bg-section-header`, `bg-control`; Light values from `styles/themes/light.css`.                                                                                                                                                                                                        |
| Text              | `text-primary`, `text-muted`, `text-link`, `text-danger`, `text-on-cta`; older DES prose calls `text-muted` "text-secondary". Placeholder uses `text-placeholder`.                                                                                                                                               |
| Borders and state | `border-default`, `border-muted`, `border-strong`; `accent` for active strip and drop rule; `cta-primary` for on-switch and preview CTA; `status-danger-fg` for invalid-control border. Save micro-states use `text-muted` and `status-danger-fg` through StatusNote.                                            |
| Type              | `font-sans`, self-hosted Inter; `text-xs` 11px/1.45 for column headings, `text-sm` 12px/1.5 for captions and labels, `text-base` 13px/1.5 for names and controls, `text-md` 14px/1.5 for the type title, `text-lg` 18px/1.4 for dialog title. Weights `font-normal` 400, `font-medium` 500, `font-semibold` 600. |
| Layout            | `page-x` 32px, `page-y` and `section-gap` 24px, `card-x` 16px, `card-y` 10px. Tailwind spacing 1/2/3/4/6/8 gives 4/8/12/16/24/32px. Borders are 1px; Branch and insertion rules 2px.                                                                                                                             |
| Geometry          | `rounded-card` and `rounded-button` 6px; switch `rounded-full`. Switch 36 × 20px, 16px thumb, 2px inset, 16px travel, inherited `shadow-sm`. Standalone icons 16px, icon target 28px, all targets at least DES-011's 24px.                                                                                       |
| Rows and controls | Existing 44px plain/header and 52px caption Row anatomy; 40px heading strip from spacing 10. Controls 32px, small actions 28px; textarea/upload minimum 64px. Column widths 36/240/160/192/144/80/64px are spacing-scale layout values, not new named tokens.                                                    |
| Strip and focus   | DES-032 links 36px with 12px horizontal padding and 2px active `accent` rule. `ring-width` 2px and `ring-color` = `accent`, 2px offset; buttons keep the existing `text-link` focus-ring exception.                                                                                                              |
| Dialog and menus  | Existing `3xl` 768px dialog, `bg-black/50` overlay, `shadow-md`; menu `bg-raised`, `border-default`, 6px radius, `shadow-md`, 4px inset, 32px minimum items. Move chooser can use 240px width.                                                                                                                   |

At a content width below 848px, reflow each Row into name/actions above labelled switches. The wide grid is 36px grip, a name column of at least 200px, three 128px switch columns, 80px Touchpoint and 64px actions; the settings pane at a 1280px viewport is about 910px wide, and the first 960px threshold with 160px columns never fit it. The tree remains one tree with its left rules; omit the shared heading strip once every switch has its visible label. Condition controls wrap in Row, Operator, Value order with the same 8px gap. Deep nesting must not reduce a control below its usable width; allow the tree region to scroll horizontally when indentation alone exceeds that space. The page itself must not gain horizontal scrolling. Footer buttons wrap with 8px gaps. The 848px threshold and the 320px alternative inspector use the existing spacing scale; they introduce no named token. Use content-container queries, not another viewport breakpoint. Warm and Dark use the same semantic tokens and dimensions.

### Recorded normalization points

1. The screenshots are design evidence, not a runnable builder or a `.pen` replacement checked into the app. The static before/after preview pair demonstrates the chosen all-match Branch. Evaluator behavior, focus, drag, menus, responsive reflow and submission remain obligations of the UI tickets.
2. The HTML shell is reduced context. Keep the shipped shell in the build. Its 48px mock header and 208px rail do not revise DES-007 or DES-019. The Request identity card is compressed to 400px in the comparison frame; implementation keeps DES-022's 560px identity slot and intrinsic wrapping. Its slug stays read-only and its destination stays selects.
3. Pinned Title and Type draw fixed switch states in the mock. They must be noninteractive with the fixed-state description, not editable switches. Fixed built-in Portal cells mean the Portal's existing built-in rendering policy, not a promise that every built-in is exposed there.
...
6. Existing Switch draws 36 × 20px. Expand its hit area to at least 24px high without growing the pill. The mock adds a border around off switches for contrast; the build uses `border-default` if the current component needs that correction. A disabled reason is a tooltip on the switch, shown on hover and on keyboard focus, never a caption that stays in the Row; the locked switch keeps the not-allowed cursor and the reason as its accessible description at rest.
...
9. M39/4 implementation: a money condition stores an integer in minor units, but its source Row carries no currency to convert a major-unit input. Label the operand as minor units (for example, 5,000 JPY or 50 USD is 5,000 minor units), and use the same units in the Branch caption. The preview still converts the answered Value using its selected currency. This avoids assuming every currency has two decimal places; no currency or conversion rule is added to the condition schema.

### Built addendum, 2026-09-22, M39, #1007 and #1020

The shipped Form tab uses the inline Branch direction. Details, Form, People and Approval defaults use the editor's section links where those sections apply. These corrections record the choices made during implementation:

- Turning on On intake form turns on Visible on Portal in the same write. Visible on Portal then refuses an attempt to turn it off, with the reason as a tooltip on the control. This follows #1012 and supersedes the proposed requirement to turn on Visible on Portal first. Locked switches remain focusable with `aria-disabled` and an accessible explanation.
- Title and Type are fixed Contract and Matter Rows. Entity Forms omit those Rows, On intake form, Visible on Portal, and the intake preview. They retain Required for creation, Touchpoint, catalog Field actions and Branches. The hidden stored Portal value round-trips without a new UI control.
- Match uses labelled native All and Any radio controls rather than the mock's segmented pair. Reference operands use name-labelled selects, with native multiple selection for **is one of**; dates use a native date input. Text, number and date lists use comma-separated input. Currency uses the existing CurrencySelect for a single operand. Changing the source Row resets its operator to **is** and clears the operand, even when the previous value could have been compatible.
- A Branch footer shows **Add field into condition** and **Add condition** only, per point 4's 2026-09-22 amendment; it has no standalone Create Field. **Add condition** adds a nested Branch there, as does **Add condition inside** in the overflow menu. The button says condition, not Branch: the person adds a condition and the Branch is what holds the Rows under it. The condition editor's own second-condition button reads **Add another condition** so the two names never collide. Removal reads **Remove condition** and keeps its children in place (2026-09-22, from live review). The shared heading strip reflows at the specified 848px content width, with 24px indentation per Branch.
- Money operands and Branch captions explicitly say **minor units**. The source Row has no currency, so the builder cannot infer a major-unit conversion. Preview converts the answered Value using its selected currency. The normalization point above records the same correction.
- The Intake form card starts with Title, Department and Urgency and ends with Attachments. Description appears in destination Form order when On intake form is on. The card includes configured Branch children before answers exist; Preview intake form evaluates them as the requester answers. A Request type preview uses that Request type's saved display name and description.
- The builder labels native Rows **Entity**, **Department**, **Renewal period**, and **Notice period**. The switches are **On intake form**, **Required for creation**, and **Visible on Portal**. **Touchpoint** displays Intake, Creation or Record from the switches and is never edited or stored.
- Incomplete conditions stay local. Completed edits save the whole tree. A failed condition write keeps its draft and offers **Retry condition change**; Escape restores the saved condition. If Field creation succeeds but attachment fails, **Retry attaching** followed by the Field name uses the created Field. Removing a Branch keeps its children. A detach or move that would invalidate a condition is refused at that control.
- Preview loads its option lists independently. A failed list leaves successful options available and reports the failure. If Department requirements cannot be read, submission stays disabled. Preview never sends a Request or uploads paper.
...
- **B, tree beside a condition inspector**, mocked in `alternative-inspector.png`. It keeps the tree short, but the 320px inspector takes space from three already-wide switch headings. It also moves the condition away from its children and requires a selected-node state. It loses to A for this settings pane. DES-087 needs a second pane because there is a source document to read; this builder has no such document.
- **A, inline Branch conditions**, chosen in `form.png` and `branch.png`. It can make a long tree longer. The cost is accepted because the condition editor closes to one sentence and the switches retain their alignment.
- **A condition dialog for each Branch.** Not mocked. It hides the Rows above and the children while the Administrator decides what to reference.
- **One card per Row, or separate built-in and Field cards.** Not mocked. Both obscure the single order DD-028 established. A card per Row also repeats the DES-087 layout that the repository already rejected.

### Consequences

The later builder, Request type editor and preview tickets use this record and the attached screenshots as their visual contract. They reuse the Form API and shared evaluator; this task adds no application behavior, schema or tokens. The two HTML directions and four chosen states are preserved as six PNGs. The PR carries each image inline. The Pencil and Mobbin failures are recorded above so a later `.pen` pass can be traced to this fallback without claiming research or editable frames that do not exist.

`CONTEXT.md` already defines Form, Row, Branch, Touchpoint, Intake form and Default type. Only Row's Entity-switch sentence is corrected. UI validation should cover sibling reorder, moves that break references, nested all/any conditions, every switch refusal, create-then-attach recovery, focus restoration and hidden required inputs. Those tests belong with the UI implementation; a test of this prose would not establish that behavior.
...
## DES-093: A2 · Console+ is the shared HTML email layout (extends DES-005; amends DES-051)

- **Status:** Accepted
- **Date:** 2026-09-24
- **Source:** The A2 · Console+ gallery at `~/.cache/openlaw-mocks/email/`, specifically `renderConsolePlus` in `render.js` and the sample content in `data.js`. The gallery stays outside the repo. [M43 #1080](https://github.com/juggernog20/OpenLaw/issues/1080) carries the spec; [#1083](https://github.com/juggernog20/OpenLaw/issues/1083) records this decision.

### Context

Most OpenLaw emails carry one sentence and a link. The reader must open the app to see the record, its status or who acted. The briefing and Auto-Doc delivery have separate, unstyled HTML. M43 chooses A2 · Console+ from the gallery so every sender has one layout to follow.

### Decision

**1. Every email uses one shared HTML layout.** This covers Contract, Matter and Request notifications, the daily briefing, Auto-Doc delivery, account emails and the SMTP test. Each sender supplies its message facts to one layout module. The HTML adds no facts beyond the event payload except the comment words allowed by the NOT-002 and CMT-006 addenda of 2026-09-24.

DES-051 keeps the copy register. The message addresses the reader with `Hello {name},`, then says what happened, then points to the action. The label and headline sit above that greeting. Copy stays terse, second-person and sentence case, with digits, no "please", no emoji and no celebration. Senders author English copy at the call site and print dates in UTC, in `en-US`. The text part keeps its current words and links, except for the comment words governed by NOT-002. It remains an authored part.

**2. The palette is OpenLaw Light.** Use the DES-005 roles below at their light values from `styles/themes/light.css`, resolved into inline styles. There is no organization brand colour or dark-mode variant.

| Role                                                  | Token                              | Light value          |
| ----------------------------------------------------- | ---------------------------------- | -------------------- |
| Outer canvas, head strips, comment fill and why strip | `bg-section-header`                | `#f6f8fa`            |
| Card                                                  | `bg-raised`                        | `#ffffff`            |
| Header                                                | `bg-inverted`                      | `#0d1117`            |
| Card border                                           | `border-default`                   | `#d0d7de`            |
| Inner rules and comment border                        | `border-muted`                     | `#d8dee4`            |
| Legal note rule                                       | `border-strong`                    | `#afb8c1`            |
| Headline, body and record title                       | `text-primary`                     | `#1f2328`            |
| Secondary copy                                        | `text-muted`                       | `#656d76`            |
| Header suffix and later step labels                   | `text-subtle`                      | `#7d8590`            |
| Organization name in header                           | `text-on-inverted`                 | `#f0f6fc`            |
| Links                                                 | `text-link`                        | `#0969da`            |
| Button fill and text                                  | `cta-primary`, `text-on-cta`       | `#1f883d`, `#ffffff` |
| Button border                                         | `status-success-fg`                | `#1a7f37`            |
| Section count badge                                   | `badge-count-bg`, `badge-count-fg` | `#eaeef2`, `#636a73` |

Labels and due dates use the event tone's `status-*-fg`. Pills use the matching background and foreground pair. The reader's own @-mention uses the assigned pair.

| Tone     | Background token and value     | Foreground token and value     |
| -------- | ------------------------------ | ------------------------------ |
| Neutral  | `status-neutral-bg` `#eff1f3`  | `status-neutral-fg` `#57606a`  |
| Success  | `status-success-bg` `#dafbe1`  | `status-success-fg` `#1a7f37`  |
| Warning  | `status-warning-bg` `#fff8c5`  | `status-warning-fg` `#9a6700`  |
| Info     | `status-info-bg` `#ddf4ff`     | `status-info-fg` `#0969da`     |
| Danger   | `status-danger-bg` `#ffebe9`   | `status-danger-fg` `#cf222e`   |
| Severe   | `status-severe-bg` `#fff1e5`   | `status-severe-fg` `#bc4c00`   |
| Assigned | `status-assigned-bg` `#fbefff` | `status-assigned-fg` `#8250df` |

**3. The frame has a slim header and a white card.** The canvas has 32px padding around a 600px container. The card has a 1px `border-default` border and a 6px radius. The header uses `bg-inverted` and 10px 40px padding. It holds a 24px logo with a 5px radius, the organization name in 14px semibold `text-on-inverted`, then `/ Legal` or `/ Legal portal` in `text-subtle`. A missing logo falls back to the OpenLaw mark; a missing name falls back to "OpenLaw". The logo is an inline `cid:` PNG attachment, including the fallback mark.

The body has 32px 40px 8px padding, with 20px side padding on a phone. Blocks sit 24px apart. Use a system sans-serif font stack and a system monospace stack for refs, with no downloaded fonts.

**4. A label and headline identify the event.** The label is a `●` dot followed by 12px semibold text in the event tone. A briefing adds ` · {date}` in `text-muted`. The headline is 22px semibold `text-primary`, with -0.3px letter-spacing. The greeting and body sentences are 14px with a 1.55 line-height.

**5. The record and its action share one card.** A `bg-section-header` head strip shows `{Kind} {REF}`, with the ref in monospace and a status pill on the right. Omit that pill when the card shows a status move. The linked title is 16px semibold `text-primary`. The actor row is a 20px initials avatar and the name in 13px `text-muted`.

Fact rows have a 96px label column in 12px `text-muted` and 13px values. A "Status" row shows an old neutral pill, `→`, and the new pill. A Document row shows an 18px file badge, the name and a version pill. Request steps form a vertical list. Done steps have a green check dot; the current step has an outlined warning dot and a semibold label followed by "· now"; later steps have hollow grey dots.

The bottom row sits below a `border-muted` hairline. It holds the button and a 12px `text-muted` line beside it. The button uses `cta-primary`, a 1px `status-success-fg` border, a 6px radius, 10px 18px padding and 14px semibold white text. On a phone it fills the width and the adjacent line moves below it. Without a record card, the button sits in the body. Account emails also show the fallback link and expiry line.

**6. A comment block replaces the actor row inside the record card.** It has a `bg-section-header` fill, a `border-muted` border, a 6px radius and 12px 14px padding. The author line has a 20px avatar, the name in 13px semibold, then `·` and the Visibility tier, Legal Only, Working Team or Full Thread. Portal emails omit the tier.

The words are 14px. Mark the reader's own @-mention with `status-assigned-bg` and `status-assigned-fg`. A cut comment ends with `…` and "Read the full comment →". NOT-002 and CMT-006 govern which words may be sent, their send-time read, the cut and the organization switch. This layout does not change those rules.

**7. Other message blocks use the same card vocabulary.** A decline reason has a 3px `status-danger-fg` left rule on `bg-section-header`. An Auto-Doc Legal note has a 3px `border-strong` left rule and the label "Note from {org} Legal". The attachments list is bordered; each row has a 28px file badge, the name and "Attached · {size}". These blocks display content the sender already supplies.

**8. The briefing opens with count tiles and then its sections.** Tiles sit three to a row, with a 20px semibold number over a 12px label. Each section head has its name, a `badge-count-bg` / `badge-count-fg` count badge and, when rows are omitted, "View all {n} →" aligned right. The number is the section's total eligible count.

Rows use 14px semibold `text-primary` linked titles, 12px monospace `text-muted` refs that never break, 12px metadata and a right-aligned due date in its tone colour. Date rows have a 100px "when / date" column. The NOT-006 M43/1 addendum owns the HTML delivery anatomy and the unchanged section rules.

**9. The why strip sits inside the card; the sender line sits outside.** The strip uses `bg-section-header`, a `border-muted` top rule, 14px 40px padding and 12px `text-muted`. It carries the sentence appropriate to the sender:

- Notification: "{why} Change what reaches you in your notification settings." The settings link is underlined `text-link`. It opens `/settings/notifications` for staff and `/portal/settings` for Portal readers.
- Security: "This is an automatic security email. Nobody at {org} will ask you for this link."
- System: "Only Administrators can send this email."

The centred sender line is 12px: "{Org} · sent by OpenLaw", or "Sent by OpenLaw" when the organization has no name.

**10. The markup must work in email clients.** Use tables for layout and inline styles. One `<style>` block holds one `@media (max-width:620px)` rule for container width, side padding, stacked cells and the full-width button. Include a hidden preheader and `color-scheme: light`. Escape user-written values. Do not use scripts, web fonts, background images, `calc()`, flex or grid. Images use `cid:` attachments, not `data:` URIs.

DES-051 normalization points 1 and 2 are superseded. Point 3 changes for HTML only: the label dot, the status arrow, the step dots and `→` on a link belong to this layout. The text part still uses words.

### Rationale

A record card lets the reader see the record and act on it in one place. Count tiles make a briefing easy to scan. The header identifies the organization, and the why strip gives the reader a route to their settings. One layout keeps these parts consistent across senders.

### Alternatives considered

- Keep plain text alone or separate HTML per sender. Rejected because the chosen design must reach every email through one layout.
- Build A · Console, B · Letterhead or C · Signal. The gallery retains them for comparison; only A2 · Console+ is selected.
- Add an organization brand colour or a dark-mode variant. Outside M43's scope; the layout uses OpenLaw Light.

### Consequences

M43's build tickets implement this record. This decision does not ship a renderer. The gallery remains outside the repo, and this record carries the anatomy so a builder can work without it. DES-051's register stays in force. NOT-006 records the briefing's HTML changes. Subjects, recipients, timing, preferences, the bell and Push do not change.
...
- ~~S5's faded See all results row is not built for the empty header box. It shows the Saved and Recent groups and Advanced search.~~ Corrected at the M44 close review (PR [#1145](https://github.com/juggernog20/OpenLaw/pull/1145)): the empty header box shows the Saved and Recent groups, then S5's faded See all results row as a disabled option that arrow keys skip and a pointer cannot open, then Advanced search. A typed search retains See all results above Advanced search.
...
| DES-051 | The email copy register (closes DES-015's deferral)                                                                                                                  | Accepted; comment words allowed in email by the 2026-09-24 amendment; DES-093 supersedes normalization points 1–2 and amends point 3 for HTML only |
