...
3. In **Business-user portal**, review magic-link sign-in and add each **Allowed email domains** entry using **Add**. Enter a domain such as `helix.example`, without an email username. An empty allowlist does not admit new Business Users. Continue to save the choices.
