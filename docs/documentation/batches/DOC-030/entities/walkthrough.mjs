...
function productBug(id, summary, reproduction, observed) {
  if (log.productBugs.some((b) => b.id === id)) {
    log.productBugs
      .find((b) => b.id === id)
      .alsoSeen.push({ scenario: current.scenario, observed });
    return;
  }
...
async function session(key) {
  ...
  const s = key === "business_user" ? await portalSession(person) : await passwordSession(person);
  watch(s.page);
...
async function c31Cleanup() {
  ...
  const admin = await session("administrator");
  const using = await admin.api(
    "GET",
    `/entities?limit=100&includeArchived=true&q=${encodeURIComponent(`DOC-030 entities V-C31 ${TAG}`)}`,
  );
...
async function c31(R) {
  ...
  await step(
    ...
    async () => {
      ...
      const jurisdiction = tidy(
        await pop.getByRole("radio").first().locator("xpath=..").innerText(),
      );
      ...
      const chipsAfter = (await bar.getByRole("button").allInnerTexts())
        ...
        .filter((c) => c.includes(":"));
      expect(
        byOwner > 0 && byOwner < total && cleared >= total && chipsAfter.length === 0,
  ...
  await step(
    ...
    async () => {
      ...
      expect(
        shown === legalName && cleared === "Not known yet" && !apiEntity && again === legalName,
        q({ shown, cleared, apiEntity, again }),
      );
  ...
  await step(
    ...
    async () => {
      ...
      await wait(1500);
      const options = (await page.getByRole("listbox").getByRole("option").allInnerTexts()).map(
        tidy,
      );
  ...
  await step(
    ...
    async () => {
      ...
      await wait(1500);
      const offered = (await page.getByRole("listbox").getByRole("option").allInnerTexts()).map(
        tidy,
      );
      await counterparties.fill("");
      expect(
  ...
  await step(
    ...
    async () => {
      ...
      const values = await our()
        .locator("option")
        .evaluateAll((os) => os.map((o) => o.value));
      const offered = (await our().locator("option").allInnerTexts()).filter((o) =>
        o.includes(legalName),
      );
      await go(page, `/entities/${entityId}`);
...
async function c32(R) {
  ...
  await step(
    ...
    async () => {
      ...
      const row = {
        ...
        user: tidy(
          await c.getByLabel(`${officer} Linked user`).locator("option:checked").innerText(),
        ),
      };
      expect(
        row.name === officer &&
  ...
  await step(
    ...
    async () => {
      ...
      const row = tidy(
        await page
          .locator("main table tbody tr", { hasText: "doc030-entities-register-extract" })
          .first()
          .innerText(),
      );
...
async function c52(R) {
  ...
  await s.api("POST", `/entities/${entity.id}/registrations`, {
    jurisdiction: jur,
    registrationNumber: `KS-${TAG}`,
    status: "active",
  });
  fixture(
    `Registration ${jur} on ${entity.legalName}`,
    `POST /entities/:id/registrations as ${me}`,
  );
  await s.api("POST", `/entities/${second.id}/obligations`, {
    label: `${prefix} second table row`,
  ...
  fixture(
    `Confidential Matter M-${matter.number} and Obligation ${restricted} linked to it`,
    `POST /matters and POST /entities/:id/obligations as ${o.person.name}`,
  );
  void matterTypes;
  ...
  await step(
    ...
    async () => {
      ...
      const matterOptions = (
        await dialog.getByLabel("Matter", { exact: true }).locator("option").allInnerTexts()
      ).map(tidy);
      matterLabel = matterOptions.find((x) => x !== "None");
      expect(matterLabel, "no reachable Matter offered");
      const offersConfidential = matterOptions.some((x) =>
        x.includes(`${prefix} confidential matter`),
      );
      ...
      expect(
        api.nextDueOn === "2015-09-30" &&
          api.recurrenceMonths === 12 &&
          api.registration?.jurisdiction === jur &&
          api.assignee?.displayName === me &&
  ...
  await step(
    ...
    async () => {
      ...
      for (const months of ["1201", "0"]) {
        ...
        await dialog
          .getByLabel("Label", { exact: true })
          .fill(`${prefix} bad recurrence ${months}`);
      ...
      expect(
        outcomes.every((x) => x.open && x.message) &&
          blankOpen &&
          blankMessage &&
          leaked.length === 0,
        q({ outcomes, blankOpen, blankMessage, leaked }),
      );
  ...
  await step(
    ...
    async () => {
      ...
      const api = await apiObligation(s, entity.id, oneOff);
      expect(
        api.note === "Renew before the end of November." && api.assignee?.displayName === me,
        q({ row, api: { note: api.note, assignee: api.assignee } }),
      );
  ...
  await step(
    ...
    async () => {
      ...
      const leaks =
        (await page.getByText(`${prefix} confidential matter`).count()) +
        (await page.getByText(`M-${matter.number}`).count());
      await rowMenu(page, restricted, "Edit");
      const dialog = page.getByRole("dialog", { name: "Edit obligation" });
      const selected = tidy(
        await dialog.getByLabel("Matter", { exact: true }).locator("option:checked").innerText(),
      );
      await dialog
        .getByLabel("Note", { exact: true })
        .fill("Edited by the reader who cannot see the Matter.");
      await dialog.getByRole("button", { name: "Save changes" }).click();
      await dialog.waitFor({ state: "hidden" });
      const stored = await apiObligation(o, entity.id, restricted);
      expect(
        row.Matter === "Restricted matter" &&
          leaks === 0 &&
          selected === "Restricted matter" &&
          stored.matter?.id === matter.id &&
          stored.note?.startsWith("Edited by"),
        q({ row, leaks, selected, stored: stored.matter }),
      );
  ...
  await step(
    ...
    async () => {
      ...
      const hits = await page.evaluate(
        ({ x, y }) =>
          [0, 2, 4, 6, 8].map((dx) => {
            const el = document.elementFromPoint(x + dx + 0.5, y);
      ...
      if (centreDrag === start && dragged < start)
        productBug(
          "resize-strip-covered",
          "A table column's resize strip takes the pointer only on its left few pixels; the next heading cell paints over the rest, so a drag that starts on the visible edge line does nothing.",
          "Entity Obligations tab at 1280px; press on the centre of the Note column's heading edge and drag left 30px; then press 2px left of the line and drag again.",
          `Hit test across the strip ${q(hits)}; a drag from the line left the width at ${centreDrag}px; a drag from 2px left of it changed ${start}px to ${dragged}px.`,
        );
      ...
      const elsewhere = Math.round(
        Number(
          await page
            .getByRole("separator", { name: "Width of the Note column" })
            .getAttribute("aria-valuenow"),
        ),
      );
      await page.getByRole("separator", { name: "Width of the Note column" }).focus();
      await page.keyboard.press("Home");
      const reset = Math.round(
        Number(
          await page
            .getByRole("separator", { name: "Width of the Note column" })
            .getAttribute("aria-valuenow"),
      ...
      expect(
        dragged < start &&
          ...
          elsewhere === keyed &&
          reset > keyed &&
          mark.x + mark.width > table.x + table.width - 200,
        q({ start, centreDrag, dragged, keyed, elsewhere, reset, mark, table }),
      );
  ...
  await step(
    ...
    async () => {
      ...
      const rows = (await page.locator("main table tbody tr").allInnerTexts()).map(tidy);
      expect(
        refusal === "End date must be on or after start date." &&
          applyDisabled &&
          hint &&
          rows.length === 1 &&
          rows[0].includes(oneOff),
        q({ refusal, applyDisabled, hint, rows }),
      );
  ...
  await step(
    ...
    async () => {
      ...
      expect(
        empty &&
          !/from=|entity=|assignee=/.test(cleared) &&
          landed === `/entities/${entity.id}/obligations`,
  ...
  await step(
    ...
    async () => {
      ...
      const mark = await page
        ...
        .count();
      expect(
        found && api.nextDueOn === "2015-09-30" && api.completedOn === null && mark === 1,
        q({ items: items.slice(0, 5), api, mark }),
      );
  ...
  await step(
    ...
    async () => {
      ...
      const history = (
        await page
          .getByText(new RegExp(annual.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")))
          .allInnerTexts()
      )
        .map(tidy)
        .filter((t) => /complet|filed/i.test(t));
      await page.keyboard.press("Escape");
      expect(
        /12 months/.test(explanation) &&
          defaultDate === browserToday &&
          api.nextDueOn === "2027-09-30" &&
          history.length > 0,
        q({ explanation, defaultDate, browserToday, api: api.nextDueOn, history }),
      );
  ...
  await step(
    ...
    async () => {
      ...
      const completedRow = (
        await page.locator("main table tbody tr", { hasText: oneOff }).allInnerTexts()
      ).map(tidy);
      const again = await s.api("POST", `/entities/${entity.id}/obligations/${api.id}/file`, {
        filedOn: "2026-10-06",
      });
      expect(
        api.nextDueOn === "2026-11-20" &&
          api.completedOn &&
          /^Completed /.test(row.Obligation?.replace(oneOff, "").trim() ?? "") &&
          buttons === 0 &&
          open === 0 &&
        ...
        q({
          ...
          row,
          buttons,
          api: [api.nextDueOn, api.completedOn],
          open,
          completedRow,
          again,
        }),
  ...
  await step(
    ...
    async () => {
      ...
      expect(
        corrected === "2027-07-31" && confirm === 0 && gone && annualKept === "2027-09-30",
        q({ corrected, confirm, gone, annualKept }),
      );
  ...
  await step(
    ...
    async () => {
      const patch = await s.api(
        "PATCH",
        `/entities/${entity.id}/obligations/${(await apiObligation(s, entity.id, annual)).id}`,
        { nextDueOn: "2015-09-30" },
      );
      ...
      expect(
        before && !homeAfter && calendarAfter === 0 && offered === 0 && restored === 1,
        q({ before, homeAfter, calendarAfter, offered, restored }),
      );
  ...
  await step(
    ...
    async () => {
      ...
      const write = await s.api("POST", `/entities/${entity.id}/obligations/${id}/file`, {
        filedOn: "2026-10-06",
      });
      await go(page, `/entities/${entity.id}`);
      await page.getByRole("button", { name: "Restore", exact: true }).click();
      await settle(page);
      expect(
        add === 0 && mark === 0 && menus === 0 && write.status === 409,
        q({ add, mark, menus, write }),
      );
...
async function c52BusinessUser() {
  ...
  await step(
    ...
    async () => {
      s = await session("business_user");
      const r = await s.api("POST", `/entities/${entity.id}/obligations`, {
        label: "DOC-030 entities V-C52 business refusal",
...
async function c53(R) {
  ...
  await step(
    R,
    page,
    "Chart steps 1-2: Chart draws Holdings, including register Holdings and individual owners; primary owner solid, others dashed; highest percentage is primary even below 50%; an Entity with no Holdings sits in a separate row",
  ...
  await step(
    R,
    page,
    "Linked Contracts: the Contracts tab lists Contracts through Our entity; rows and counts follow the reader's reach",
    "The comparison reader sees only the open Contract and a count of 1; the Confidential Contract adds neither row nor count.",
    async () => {
      ...
      expect(
        readerRows.some((r) => r.includes(c1.title)) &&
          !readerRows.some((r) => r.includes(c2.title)) &&
          readerImg === "1 linked Contract" &&
          c2direct.status >= 400,
        q({ actorRows, readerRows, readerImg, c2: c2direct.status }),
      );
  ...
  await step(
    R,
    page,
    "Linked Matters tab: Matters appear only through saved Entity-valued Fields",
    "The Matters tab renders its linked-record list; the lab has no Entity-valued Matter Field.",
    async () => {
...
try {
  for (const phase of PHASES) {
    ...
    for (const role of ROLES) {
      try {
        ...
        log.steps.push({
          ...
          role,
          method: "browser-walkthrough",
          action: `${phase} fixture or flow aborted`,
          expected: "The phase runs to the end.",
          actual: String(e?.message ?? e).slice(0, 600),
          result: "fail",
          startedAt: new Date().toISOString(),
    ...
    if (phase === "c52")
      await c52BusinessUser().catch((e) => log.notes.push(`c52 business user: ${e.message}`));
  }
} finally {
  save();
  await closeBrowser();
