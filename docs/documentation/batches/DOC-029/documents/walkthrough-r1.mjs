// DOC-029 independent browser walkthrough, documents group, round 1.
// Written by the DOC-029 independent walkthrough agent (documents, round 1) from the article text.
...
import { PEOPLE, staffContext, portalContext, api, close, BASE, MAIL, here, root, sleep } from "./lib-r1.mjs";

const OUT = process.env.OUT ?? path.join(here, "walkthrough-r1.json");
const SECTIONS = (process.env.SECTIONS ?? "versions,folders,previews,portal,repository,archive").split(",");
...
function labStatus() {
  try {
    const text = execFileSync("mise", ["exec", "--", "node", "scripts/documentation/lab.mjs", "status", "work"], {
      cwd: root,
      encoding: "utf8",
    });
    const project = text.match(/(openlaw-docs-[a-z0-9]+-work)-app-1/)?.[1] ?? null;
    const image = (container) =>
      execFileSync("docker", ["inspect", "--format", "{{.Image}}", container], { encoding: "utf8" }).trim();
    return {
      summary: text.split("\n")[0],
      project,
      images: project
    ...
    return { summary: `status unavailable: ${String(error).slice(0, 200)}`, project: null, images: null };
  }
}
...
const results = {
  ...
  group: "documents",
  round: 1,
  walkthroughReviewer: "DOC-029 independent walkthrough agent (documents, round 1)",
  reviewerKind: "agent",
  ...
  articleHashes: Object.fromEntries(
    ARTICLES.map((id) => [id, sha(fs.readFileSync(path.join(root, "docs/user-guides", `${id}.md`)))]),
  ),
  fixtures: fs
    .readdirSync(FIX)
...
async function step(article, scenario, role, action, expected, fn) {
  const entry = { article, scenario, role, method: "browser-walkthrough", action, expected, actual: null, result: "not-run", startedAt: new Date().toISOString(), at: null };
  ...
  try {
    ...
    entry.result = "fail";
    if (SHOT_DIR && currentPage) await currentPage.screenshot({ path: path.join(SHOT_DIR, `fail-${results.steps.length}.png`) }).catch(() => {});
  }
  entry.at = new Date().toISOString();
  console.log(`[${role}] ${entry.result.toUpperCase()} ${article}: ${action}${entry.result === "fail" ? `\n    ${entry.actual}` : ""}`);
  save();
  return entry;
}
...
async function addTeam(page, number, email) {
  const r = await api(page, "POST", `/contracts/${number}/team`, { userId: await userId(page, email) });
  expect(r.status === 201 || r.status === 200 || r.status === 409, `fixture team add ${r.status} ${JSON.stringify(r.body).slice(0, 120)}`);
}
async function createContract(page, role, title, extra = {}) {
  const r = await api(page, "POST", "/contracts", { title, contractTypeId: CONTRACT_TYPE, ...extra });
...
function part(name, as = name) {
  return { name: as, mimeType: MIME[path.extname(name)] ?? "application/octet-stream", buffer: fs.readFileSync(fixture(name)) };
}
...
function watchResponses(page) {
  ...
  page.on("response", (r) => {
    if (r.url().includes("/api/") && r.status() >= 500) seen.push({ status: r.status(), method: r.request().method(), path: new URL(r.url()).pathname, at: new Date().toISOString() });
  });
}
async function openDocumentsTab(page, url) {
  ...
  if (await broken.isVisible().catch(() => false)) {
    ...
    results.pageLoadErrors.push({ path: url, at: new Date().toISOString(), serverErrors: (failingResponses.get(page) ?? []).slice(-5) });
...
function reader(page) {
  return page.getByRole("complementary").filter({ has: page.getByRole("button", { name: "Close the document" }) });
}
const rowOf = (page, title) => page.getByRole("row").filter({ has: page.getByRole("button", { name: `Actions for ${title}`, exact: true }) });
async function shot(page, name) {
  await page.screenshot({ path: path.join(here, name) }).catch(() => {});
  results.screenshots ??= [];
  results.screenshots.push(`docs/documentation/batches/DOC-029/documents/${name}`);
}
const HAND_SET_KINDS = ["Draft · ours", "Draft · theirs", "Redline · theirs", "Redline · ours", "Executed", "Amendment"];
...
async function versions(role, person) {
  ...
  currentPage = page;
  const contract = await createContract(page, role, `DOC-029 documents ${label} versions Contract ${stamp}`);
  await addTeam(page, contract.number, PEOPLE.jonas.email);
  const matter = await createMatter(page, role, `DOC-029 documents ${label} versions Matter ${stamp}`);
  const recUrl = `/contracts/${contract.number}`;
  let first;

  await step(A, S, role, "Upload a new Document steps 1-4: Documents tab, Upload, Choose files with one file, Kind, Note, Upload", "One file creates a Document named from its filename with a v1 row; Kind offers the six hand-set kinds", async () => {
    ...
    const kinds = (await dialog.getByLabel("Kind", { exact: true }).locator("option").allInnerTexts()).map((t) => t.trim());
    expect(HAND_SET_KINDS.every((k) => kinds.includes(k)) && !kinds.includes("Generated redline"), `Kind options ${kinds}`);
    ...
    first = docs[0];
    const row = page.getByRole("row").filter({ has: page.getByRole("button", { name: `Actions for ${first.title}`, exact: true }) });
    await row.waitFor();
  ...
  await step(A, S, role, "Upload step 2 variant: choosing several files opens the bulk import dialog", "The Import dialog replaces the one-file composer; Cancel creates nothing", async () => {
    await docsSection(page).getByRole("button", { name: "Upload", exact: true }).click();
    const dialog = page.getByRole("dialog", { name: "Upload document" });
    await chooseFiles(page, dialog.getByRole("button", { name: /Choose files/ }), [fixture("doc029-bulk-a.txt"), fixture("doc029-bulk-b.txt")]);
  ...
  const newName = `DOC-029 ${label} supply draft ${stamp}`;
  await step(A, S, role, "Upload step 4: Actions menu, Edit details, Save changes the name and description", "Name and description change; the Version chain is untouched", async () => {
    ...
    expect(doc.title === newName && doc.description === "DOC-029 fictional description" && doc.versions.length === 1, JSON.stringify(doc).slice(0, 200));
    first = doc;
  ...
  await step(A, S, role, "Add another Version steps 1-3: Add version, Choose file, Kind, Note, Upload; expand and read the earlier Version", "The Version number increases; the earlier Version stays readable with its note, author and bytes", async () => {
    ...
    await dialog.getByRole("button", { name: "Upload", exact: true }).click();
    await dialog.waitFor({ state: "hidden", timeout: 30000 });
    const row = page.getByRole("row").filter({ has: page.getByRole("button", { name: `Actions for ${newName}`, exact: true }) });
    ...
    expect(doc.versions.length === 2 && v1.note === "DOC-029 round one note" && v1.kind === "draft_ours" && v1.originalFilename === "doc029-draft-v1.docx", "v1 changed");
  ...
  await step(A, S, role, "History note: uploading the same paper as a new Document creates a separate chain", "A second Document appears; the first chain is not merged or changed", async () => {
    await docsSection(page).getByRole("button", { name: "Upload", exact: true }).click();
    const dialog = page.getByRole("dialog", { name: "Upload document" });
    await chooseFiles(page, dialog.getByRole("button", { name: /Choose files/ }), fixture("doc029-draft-v1.docx"));
    await dialog.getByRole("button", { name: "Upload", exact: true }).click();
  ...
  await step(A, S, role, "Correct the Kind control on an existing hand-set Version", "The kind changes; bytes and Executed pin do not; Generated redline is not offered", async () => {
    ...
    expect(v2.kind === "redline_theirs" && !doc.versions.some((v) => v.isExecuted), `v2 ${JSON.stringify(v2).slice(0, 120)}`);
  ...
  let pinnedVersion;
  await step(A, S, role, "Executed pin: Mark as executed copy on an earlier Version; a later upload does not move it; Unmark clears it", "The pin stays on the earlier Version after a newer Version arrives", async () => {
    const earlier = page.getByRole("button", { name: `Actions for version 1 of ${newName}`, exact: true });
    if (!(await earlier.isVisible())) await page.getByRole("button", { name: `Show the 1 earlier version of ${newName}` }).click();
    await earlier.click();
    ...
    await dialog.waitFor({ state: "hidden", timeout: 30000 });
    doc = await getDoc(page, recUrl, first.id);
    const stillOnV1 = doc.versions.find((v) => v.isExecuted)?.versionNumber === 1 && doc.versions.length === 3;
  ...
  await step(A, S, role, "Primary Document: the Primary mark and Make primary move the designation", "Primary moves to the other Document", async () => {
    const docs = await recordDocs(page, recUrl);
    const primary = docs.find((d) => d.isPrimary);
    const other = docs.find((d) => !d.isPrimary);
    expect(primary && other, `primary ${!!primary} other ${!!other}`);
    const primaryRow = page.getByRole("row").filter({ has: page.getByRole("button", { name: `Actions for ${primary.title}`, exact: true }) });
    ...
    await page.getByRole("row").filter({ has: page.getByRole("button", { name: `Actions for ${other.title}`, exact: true }) }).getByText("Primary", { exact: true }).waitFor();
  ...
  await step(A, S, role, "A Matter upload has no Kind control, and a Matter has no Primary mark or Executed pin", "No Kind column or composer Kind, no Make primary or Mark as executed copy", async () => {
    ...
    expect(kindCount === 0 && !header.some((h) => h.trim() === "Kind"), `kind ${kindCount} headers ${header}`);
    expect(!items.includes("Make primary") && !items.includes("Mark as executed copy"), `items ${items}`);
    expect((await docsSection(page).getByLabel(/^Kind of version/).count()) === 0, "kind correction on matter");
    const types = (await api(page, "GET", "/entities/types")).body.entityTypes;
    const ent = await api(page, "POST", "/entities", { legalName: `DOC-029 documents ${label} versions Entity ${stamp} Ltd`, entityTypeId: types[0].id });
    expect(ent.status === 201, `fixture entity ${ent.status}`);
    record(role, "entity", ent.body.entity.legalName, ent.body.entity.id);
    const entDoc = await uploadApi(page, `/entities/${ent.body.entity.id}/documents`, "doc029-draft-v1.docx", { as: `versions-entity-${label.toLowerCase()}-${stamp}.docx` });
    await openDocumentsTab(page, `/entities/${ent.body.entity.id}/documents`);
    const entItems = await menuItems(page, entDoc.title);
    const entPrimary = await rowOf(page, entDoc.title).getByText("Primary", { exact: true }).count();
    expect(!entItems.includes("Make primary") && !entItems.includes("Mark as executed copy") && entPrimary === 0, `entity items ${entItems}`);
  ...
  await step(A, S, role, "Before you start: an archived owning record must be restored before uploading", "The archived record offers no Upload; restoring brings it back; the seam refuses an upload meanwhile", async () => {
    const frozen = await createContract(page, role, `DOC-029 documents ${label} archived owner ${stamp}`);
    ...
    const refused = await api(page, "POST", `/contracts/${frozen.number}/documents`, undefined, { kind: "draft_ours", file: part("doc029-bulk-a.txt") });
    ...
    expect(uploadWhileArchived === 0 && uploadAfter === 1 && refused.status >= 400, `archived ${uploadWhileArchived} after ${uploadAfter} api ${refused.status}`);
  ...
  await step(A, S, role, "Recover from a refusal: a failed upload shows a message and does not alter the chain", "An upload message appears; the Version count stays the same", async () => {
    await openDocumentsTab(page, `${recUrl}/documents`);
    const before = (await getDoc(page, recUrl, first.id)).versions.length;
    await page.route("**/api/v1/documents/*/versions", (route) =>
      route.request().method() === "POST"
        ? route.fulfill({ status: 413, contentType: "application/problem+json", body: JSON.stringify({ type: "about:blank", title: "Payload Too Large", status: 413, detail: "DOC-029 fixture: this file is larger than the upload limit." }) })
        : route.continue(),
    );
    await chooseMenu(page, newName, "Add version");
    const dialog = page.getByRole("dialog", { name: "Add version" });
    ...
    expect(after === before && /larger than the upload limit/.test(alert), `before ${before} after ${after} alert ${alert}`);
  ...
  const legalOnly = await api(page, "POST", "/comments", undefined, { entityType: "contract", entityId: contractRow.id, body: `DOC-029 ${label} Legal only attachment`, visibility: "legal_only", file: part("doc029-services-text.pdf", `doc029-${label.toLowerCase()}-legal-only.pdf`) });
  const teamNote = await api(page, "POST", "/comments", undefined, { entityType: "contract", entityId: contractRow.id, body: `DOC-029 ${label} team attachment`, visibility: "full_thread", file: part("doc029-draft-v2.docx", `doc029-${label.toLowerCase()}-round.docx`) });
  ...
  let filedTitle = `DOC-029 ${label} filed paper ${stamp}`;
  await step(A, S, role, "File a Contract conversation attachment steps 1-4: File to Contract, New Document, Confidential preselected for Legal Only", "A Legal Only attachment files as a new Confidential Document and the comment reads Filed to", async () => {
    await openDocumentsTab(page, `${recUrl}/documents`);
    await openComments();
    const name = `doc029-${label.toLowerCase()}-legal-only.pdf`;
    const item = page.getByRole("list", { name: "Comment attachments" }).getByRole("listitem").filter({ hasText: name });
    ...
    expect(dest === "new_document" && on === "true" && kinds === 1 && doc?.isConfidential && doc.versions.length === 1, `dest ${dest} on ${on} doc ${!!doc}`);
  ...
  await step(A, S, role, "File a Contract conversation attachment: open the preview, File to Contract, New Version on an existing Document with a Note", "The preview offers the same action; the Version is appended with its note", async () => {
    ...
    expect(last.note === "DOC-029 filed round note" && last.originalFilename === name, `last ${JSON.stringify(last).slice(0, 150)}`);
    const filedText = (await item.innerText()).replace(/\s+/g, " ");
  ...
  await step(A, S, role, "File a Matter conversation attachment: File to Matter, no Kind, same Confidential switch label", "The Matter offers File to Matter without Kind; filing creates a Matter Document", async () => {
    const m = (await api(page, "GET", `/matters/${matter.number}`)).body.matter;
    const c = await api(page, "POST", "/comments", undefined, { entityType: "matter", entityId: m.id, body: `DOC-029 ${label} matter attachment`, visibility: "working_team", file: part("doc029-services-text.pdf", `doc029-${label.toLowerCase()}-matter.pdf`) });
    ...
    const dialog = page.getByRole("dialog", { name: "File attachment" });
    const kinds = await dialog.getByLabel("Kind", { exact: true }).count();
    const switchLabel = await dialog.getByRole("switch").getAttribute("aria-label").catch(() => null);
    const labelText = (await dialog.innerText()).match(/Confidential — restrict to the [a-z ]+team/)?.[0];
    await dialog.getByLabel("Document name").fill(`DOC-029 ${label} matter filed ${stamp}`);
    await dialog.getByRole("button", { name: "File", exact: true }).click();
    await dialog.waitFor({ state: "hidden", timeout: 20000 });
    const docs = await recordDocs(page, `/matters/${matter.number}`);
    expect(kinds === 0 && docs.some((d) => d.title === `DOC-029 ${label} matter filed ${stamp}`), `kinds ${kinds}`);
    expect(labelText === "Confidential — restrict to the contract team", `switch label "${labelText ?? switchLabel}"`);
...
async function folders(role, person) {
  ...
  const { context, page } = await staffContext(person);
  currentPage = page;
  const contract = await createContract(page, role, `DOC-029 documents ${label} folders Contract ${stamp}`);
  const recUrl = `/contracts/${contract.number}`;
  const parent = `DOC-029 ${label} Parent`;
  const child = `DOC-029 ${label} Child`;
  const other = `DOC-029 ${label} Other`;
  const loose = await uploadApi(page, `${recUrl}/documents`, "doc029-bulk-a.txt", { as: `doc029-${label.toLowerCase()}-loose-a.txt` });
  const loose2 = await uploadApi(page, `${recUrl}/documents`, "doc029-bulk-b.txt", { as: `doc029-${label.toLowerCase()}-loose-b.txt` });

  await step(A, S, role, "Create and maintain folders steps 1-2: New folder, Name, Save; an empty folder has no expand control", "The folder row appears without an expand control", async () => {
    await openDocumentsTab(page, `${recUrl}/documents`);
    ...
    const rowText = await page.getByRole("row").filter({ has: page.getByRole("button", { name: `Actions for the ${parent} folder` }) }).innerText();
  ...
  await step(A, S, role, "Create and maintain folders step 3: New subfolder and Rename from Actions for the … folder", "The subfolder is created inside; the parent gains an expand control; Rename changes the name", async () => {
    ...
    expect(c && p && c.parentId === p.id && collapse === 1, `child ${!!c} parent link ${c?.parentId === p?.id} collapse ${collapse}`);
  ...
  await step(A, S, role, "Folder name rules: case-only duplicate sibling, blank name, slash, '.' and '..' are refused", "Each attempt shows a refusal and creates nothing", async () => {
  ...
  await step(A, S, role, "Create and maintain folders step 3: Move, Move into, None; a folder cannot move into itself or a descendant", "Move into omits the folder and its descendant, the seam refuses a cycle, and None returns a folder to the top level", async () => {
    await docsSection(page).getByRole("button", { name: "New folder" }).click();
    ...
    const options = (await dialog.getByLabel("Move into").locator("option").allInnerTexts()).map((t) => t.trim());
    await dialog.getByRole("button", { name: "Cancel" }).click();
    expect(options.includes("None") && !options.some((o) => o === parent || o.startsWith(`${parent}/`)) && options.includes(other), `options ${options}`);
    ...
    expect(cycle.status >= 400 && self.status >= 400, `cycle ${cycle.status} self ${self.status}`);
    ...
    await page.getByRole("button", { name: `Expand ${other}` }).click().catch(() => {});
    await folderMenu(page, child, "Move");
    dialog = page.getByRole("dialog", { name: `Move ${child}` });
  ...
  await step(A, S, role, "Create and maintain folders step 4: Move to folder, File in, Move; None moves it out", "The Document is filed in the folder and then moved back to the record", async () => {
    await openDocumentsTab(page, `${recUrl}/documents`);
  ...
  await step(A, S, role, "Create and maintain folders step 4: drag a Document row onto a folder row, then onto Drop here to move out of folders", "Dragging files the Document into the folder and back out", async () => {
    await openDocumentsTab(page, `${recUrl}/documents`);
    const row = page.getByRole("row").filter({ has: page.getByRole("button", { name: `Actions for ${loose2.title}`, exact: true }) });
    const folderRow = page.getByRole("row").filter({ has: page.getByRole("button", { name: `Actions for the ${other} folder` }) });
    ...
    const inner = page.getByRole("row").filter({ has: page.getByRole("button", { name: `Actions for ${loose2.title}`, exact: true }) });
    await inner.waitFor();
  ...
  await step(A, S, role, "Create and maintain folders step 4: select checkboxes and Move in the selection bar", "Both selected Documents move into the chosen folder", async () => {
    await openDocumentsTab(page, `${recUrl}/documents`);
    ...
    expect(docs.every((d) => (d.folderId ?? d.folder?.id) === p.id), "bulk move did not file both");
  ...
  await step(A, S, role, "Folder Delete dissolves the folder: contents move to its parent or the record, nothing is deleted", "The confirmation says where contents go; the Documents stay live on the record", async () => {
    await openDocumentsTab(page, `${recUrl}/documents`);
    ...
    const list = await folderList(page, recUrl);
    const docs = await recordDocs(page, recUrl);
    const both = [loose.id, loose2.id].every((id) => docs.some((d) => d.id === id && !(d.folderId ?? d.folder?.id)));
    expect(!list.some((f) => f.name === parent) && both && /Nothing is deleted/.test(text), `text ${text}`);
    return `The confirmation read "${text.trim()}". After Delete the folder was gone and both Documents were live at the record root.`;
  });

  await step(A, S, role, "Import several files steps 1-5 with a partial failure: Destination Record root, Version kind, counts, Retry", "The good files stay as v1 Documents, the failed row is named with Retry, and Retry sends only the failed file", async () => {
    await openDocumentsTab(page, `${recUrl}/documents`);
    ...
    await page.route(`**/api/v1/contracts/${contract.number}/documents`, async (route) => {
      ...
      if (route.request().method() === "POST" && failOnce && body.includes('filename="doc029-bulk-c.txt"')) {
        failOnce = false;
        return route.fulfill({ status: 503, contentType: "application/problem+json", body: JSON.stringify({ type: "about:blank", title: "Service Unavailable", status: 503, detail: "DOC-029 fixture: the upload service is unavailable." }) });
      }
    ...
    await chooseFiles(page, composer.getByRole("button", { name: /Choose files/ }), names.map(fixture));
    ...
    const retryRow = await settled.getByRole("button", { name: "Retry doc029-bulk-c.txt" }).count();
    const retryAll = await settled.getByRole("button", { name: "Retry 1 file" }).count();
    const mid = await recordDocs(page, recUrl);
    const landed = mid.filter((d) => d.versions[0]?.originalFilename?.startsWith("doc029-bulk-") && (d.title.includes("bulk-a") || d.title.includes("bulk-b")));
    ...
    expect(/Destination/.test(confirmText) && /Record root/.test(confirmText) && /Applied to every file in this import\. Notes are not collected in bulk\./.test(confirmText), `confirm ${confirmText}`);
    expect(retryRow === 1 && retryAll === 1 && added === 3 && landed.every((d) => d.versions.length === 1 && d.versions[0].kind === "draft_theirs"), `retry ${retryRow}/${retryAll} added ${added}`);
  ...
  await step(A, S, role, "Recover a partial import: a size refusal is not offered a retry; Done closes", "The failed row has no Retry, and the good file remains", async () => {
    await page.route(`**/api/v1/contracts/${contract.number}/documents`, async (route) => {
      const body = route.request().postDataBuffer()?.toString("latin1") ?? "";
      if (route.request().method() === "POST" && body.includes('filename="doc029-bulk-d.txt"')) {
        return route.fulfill({ status: 413, contentType: "application/problem+json", body: JSON.stringify({ type: "about:blank", title: "Payload Too Large", status: 413, detail: "DOC-029 fixture: this file is larger than the upload limit." }) });
      }
      return route.continue();
    });
    const before = (await recordDocs(page, recUrl)).length;
    await docsSection(page).getByRole("button", { name: "Upload", exact: true }).click();
    const composer = page.getByRole("dialog", { name: "Upload document" });
    await chooseFiles(page, composer.getByRole("button", { name: /Choose files/ }), [fixture("doc029-bulk-a.txt"), fixture("doc029-bulk-d.txt")]);
    await page.getByRole("dialog", { name: "Import 2 files" }).getByRole("button", { name: "Import 2 files" }).click();
  ...
  await step(A, S, role, "Import step 5: when every file uploads the dialog closes after the list refreshes", "The dialog closes by itself and both files are listed", async () => {
    const before = (await recordDocs(page, recUrl)).length;
    await docsSection(page).getByRole("button", { name: "Upload", exact: true }).click();
    const composer = page.getByRole("dialog", { name: "Upload document" });
    await chooseFiles(page, composer.getByRole("button", { name: /Choose files/ }), [fixture("doc029-bulk-c.txt"), fixture("doc029-bulk-d.txt")]);
  ...
  await step(A, S, role, "Recover a partial import: Cancel remaining stops queued work and uploaded files remain", "Queued files are cancelled; the finished uploads stay on the record; final counts are shown", async () => {
    await page.route(`**/api/v1/contracts/${contract.number}/documents`, async (route) => {
    ...
    await docsSection(page).getByRole("button", { name: "Upload", exact: true }).click();
    const composer = page.getByRole("dialog", { name: "Upload document" });
    const files = ["doc029-bulk-a.txt", "doc029-bulk-b.txt", "doc029-bulk-c.txt", "doc029-bulk-d.txt", "doc029-schedule.csv"];
    await chooseFiles(page, composer.getByRole("button", { name: /Choose files/ }), files.map(fixture));
    await page.getByRole("dialog", { name: "Import 5 files" }).getByRole("button", { name: "Import 5 files" }).click();
    const running = page.getByRole("dialog", { name: "Importing 5 files" });
    await running.waitFor();
    const keepOpen = await running.getByText("Keep this dialog open until the import finishes.").count();
    ...
    const title = await settled.getByRole("heading").first().innerText().catch(async () => (await settled.innerText()).split("\n")[0]);
    const text = (await settled.innerText()).replace(/\s+/g, " ");
    ...
    expect(cancelled > 0 && after - before === landed && keepOpen === 1, `cancelled ${cancelled} landed ${landed} added ${after - before}`);
  ...
  let tree;
  await step(A, S, role, "Keep a local folder structure: Choose folder keeps nested paths; the folder picker drops empty folders; a matching path is reused", "The top-level folder and its nested child are created with their files; the empty folder is not; a second import reuses the path", async () => {
    tree = makeTree();
    await openDocumentsTab(page, `${recUrl}/documents`);
    await docsSection(page).getByRole("button", { name: "Upload", exact: true }).click();
    const composer = page.getByRole("dialog", { name: "Upload document" });
    ...
    await docsSection(page).getByRole("button", { name: "Upload", exact: true }).click();
    await chooseFiles(page, page.getByRole("dialog", { name: "Upload document" }).getByRole("button", { name: /Choose folder/ }), tree.top);
    ...
    expect(/Folder structure is kept/.test(text) && top.length === 1 && nested && !empty && topAfter === 1, `text ${text.slice(0, 120)} top ${top.length} nested ${nested} empty ${empty} again ${topAfter}`);
  ...
  await step(A, S, role, "Import step 1-2 and Keep structure: drop files and a local folder on a folder row; the Destination is that folder; a dropped tree recreates empty folders", "The Import dialog names the folder as Destination and the empty folder is recreated", async () => {
    await openDocumentsTab(page, `${recUrl}/documents`);
    const folderRow = page.getByRole("row").filter({ has: page.getByRole("button", { name: `Actions for the ${other} folder` }) });
    ...
    const list = await folderList(page, recUrl);
    const paths = list.map((f) => folderPath(list, f));
    expect(text.includes(other) && paths.includes(`${other}/${tree.name}/empty-dir`) && paths.includes(`${other}/${tree.name}/sub-a`), `text ${text.slice(0, 160)} paths ${paths.join("|")}`);
  ...
  await step(A, S, role, "Import step 3: a Matter import has no Version kind control", "The Matter batch dialog shows Destination but no Version kind", async () => {
    const matter = await createMatter(page, role, `DOC-029 documents ${label} folders Matter ${stamp}`);
    await openDocumentsTab(page, `/matters/${matter.number}/documents`);
    await docsSection(page).getByRole("button", { name: "Upload", exact: true }).click();
    await chooseFiles(page, page.getByRole("dialog", { name: "Upload document" }).getByRole("button", { name: /Choose files/ }), [fixture("doc029-bulk-a.txt"), fixture("doc029-bulk-b.txt")]);
    const dialog = page.getByRole("dialog", { name: "Import 2 files" });
    await dialog.waitFor();
...
async function previews(role, person) {
  ...
  const contract = await createContract(page, role, `DOC-029 documents ${label} previews Contract ${stamp}`);
  const recUrl = `/contracts/${contract.number}`;
  const up = (name, kind = "draft_ours") => uploadApi(page, `${recUrl}/documents`, name, { kind, as: `${label.toLowerCase()}-${name}` });
  ...
  await step(A, S, role, "Read a managed Document steps 1-4 (PDF): name opens the reader; v… indicator; Previous page, Next page, Zoom out, Zoom in, Find in document; Download; Close the document", "The reader shows the title and v1, the PDF controls work, Download returns the original bytes, and Close returns to the record", async () => {
    await openDocumentsTab(page, `${recUrl}/documents`);
    ...
    for (const name of ["Previous page", "Next page", "Zoom out", "Zoom in", "Find in document"]) {
    ...
    await page.getByRole("complementary", { name: `${pdf.title}, version 1` }).waitFor({ state: "hidden" });
  ...
  await step(A, S, role, "Supported reading surfaces: Word and PowerPoint render as a PDF rendition; Download stays the uploaded file; a preview is not a new Version", "Both open in the PDF reader after preparation and download the original bytes; the chain stays at one Version", async () => {
    const out = [];
    for (const [doc, name] of [[docx, "doc029-draft-v1.docx"], [pptx, "doc029-deck.pptx"]]) {
      ...
      await panel.getByRole("button", { name: "Close the document" }).click();
      out.push(`${doc.title}: rendition ${state}, PDF reader pages shown, download matched the uploaded file`);
    }
    return `${out.join("; ")}. Each chain still had one Version.`;
  });

  await step(A, S, role, "Processing, scans: an image-only PDF stays a scan; Find has no match; OCR text arrives in the background; global search finds it", "Reader Find reports no match, text read-back source is OCR, and global search lists the Document", async () => {
  ...
  await step(A, S, role, "Supported reading surfaces: PNG shows inline; EML shows headers, body and attachment list without remote images; a PDF attachment previews; a Word attachment downloads", "Image inline; email headers and 2 attachments; no remote image request; PDF attachment opens; DOCX attachment is a download", async () => {
    await openDocumentsTab(page, `${recUrl}/documents`);
    ...
    await panel.getByText("DOC-029 fictional counterparty email").first().waitFor({ timeout: 30000 });
    const body = (await panel.innerText()).replace(/\s+/g, " ");
    const frame = panel.frameLocator('iframe[title="Message body"]');
    await frame.getByText("Please find the fictional drafts attached.").waitFor({ timeout: 15000 });
    const remoteImg = await frame.locator("img").evaluateAll((els) => els.map((e) => e.getAttribute("src") ?? "").join(","));
    expect(/From/.test(body) && /To/.test(body) && /Subject/.test(body) && /2 attachments/.test(body), `email ${body.slice(0, 300)}`);
    ...
    await panel.getByRole("button", { name: "Close the document" }).click();
    expect(remote.length === 0 && docxDownload.sha256 === fixtureHash("doc029-draft-v1.docx"), `remote ${remote.length}`);
  ...
  await step(A, S, role, "Supported reading surfaces: other formats download from the name; a reader link shows the download-only card", "CSV and ZIP names download directly; a reader link to one shows the card", async () => {
    await openDocumentsTab(page, `${recUrl}/documents`);
    const before = await page.getByRole("complementary").count();
    const a = await download(page, () => page.getByRole("link", { name: csv.title, exact: true }).click());
    const b = await download(page, () => page.getByRole("link", { name: zip.title, exact: true }).click());
    expect(a.sha256 === fixtureHash("doc029-schedule.csv") && b.sha256 === fixtureHash("doc029-bundle.zip"), "download-only bytes differ");
  ...
  await step(A, S, role, "Processing: Preparing this document for reading… while conversion is pending; the stored original is already downloadable", "The pending message shows and Download returns the uploaded file", async () => {
    const fresh = await up("doc029-draft-v2.docx");
    await page.route("**/api/v1/documents/*/versions/*/rendition", (route) =>
      route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ rendition: { state: "pending" } }) }),
    );
    await openDocumentsTab(page, `${recUrl}/documents`);
  ...
  await step(A, S, role, "Failed previews: the failure message, Download of the original, and recovery by adding a readable Version", "A damaged Word file shows the failure card and downloads; a new Version renders while the failed Version stays in the chain", async () => {
    ...
    const dl = await download(page, () => panel.getByRole("link", { name: "Download" }).first().click());
    ...
    await panel.getByText(/^Page 1 of \d+$/).waitFor({ timeout: 60000 });
    await page.getByRole("button", { name: `Show the 1 earlier version of ${broken.title}` }).click();
    await page.getByRole("button", { name: `${label.toLowerCase()}-doc029-broken.docx`, exact: true }).last().click();
    const v1Panel = page.getByRole("complementary", { name: `${broken.title}, version 1` });
    await v1Panel.getByText("This file could not be prepared for reading here. Download it to read it.").waitFor({ timeout: 30000 });
...
async function portal() {
  ...
  const contract = await createContract(staff, "legal_team_member", `DOC-029 documents Portal Contract ${stamp}`);
  const recUrl = `/contracts/${contract.number}`;
  await addTeam(staff, contract.number, PEOPLE.jonas.email);
  const chain = await uploadApi(staff, `${recUrl}/documents`, "doc029-services-text.pdf", { as: "portal-chain-v1.pdf", note: "Portal round one" });
  const chainV2 = await api(staff, "POST", `/documents/${chain.id}/versions`, undefined, { kind: "draft_theirs", file: part("doc029-draft-v2.docx", "portal-chain-v2.docx") });
  expect(chainV2.status === 201, `fixture version ${chainV2.status}`);
  const gone = await uploadApi(staff, `${recUrl}/documents`, "doc029-bulk-a.txt", { as: "portal-archived.txt" });
  expect((await api(staff, "POST", `/documents/${gone.id}/archive`, {})).status === 200, "fixture archive");
  const contractId = (await api(staff, "GET", recUrl)).body.contract.id;
  for (const [name, as] of [["doc029-services-text.pdf", "portal-thread.pdf"], ["doc029-draft-v1.docx", "portal-thread.docx"], ["doc029-bundle.zip", "portal-thread.zip"]]) {
    const thread = await api(staff, "POST", "/comments", undefined, { entityType: "contract", entityId: contractId, body: `DOC-029 Portal thread file ${as}`, visibility: "full_thread", file: part(name, as) });
    expect(thread.status === 201, `fixture thread ${thread.status} ${JSON.stringify(thread.body).slice(0, 160)}`);
  }
  const hidden = await createContract(staff, "legal_team_member", `DOC-029 documents Portal not on team ${stamp}`);
  // Knowledge shared with the business.
  const kType = (await api(staff, "GET", "/knowledge?limit=1")).body.knowledgeItems?.[0]?.knowledgeTypeId;
  const item = await api(staff, "POST", "/knowledge", { title: `DOC-029 documents Portal Knowledge ${stamp}`, knowledgeTypeId: kType });
  expect(item.status === 201, `fixture knowledge ${item.status}`);
  const kid = item.body.knowledgeItem.id;
  record("legal_team_member", "knowledge_item", `DOC-029 documents Portal Knowledge ${stamp}`, kid);
  const kFirst = await uploadApi(staff, `/knowledge/${kid}/documents`, "doc029-bulk-b.txt", { as: "portal-knowledge-first.txt" });
  const kPrimary = await uploadApi(staff, `/knowledge/${kid}/documents`, "doc029-services-text.pdf", { as: "portal-knowledge-primary.pdf" });
  const patch = await api(staff, "PATCH", `/knowledge/${kid}`, { audience: "everyone", primaryDocumentId: kPrimary.id, body: "DOC-029 fictional guidance." });
  const pub = await api(staff, "POST", `/knowledge/${kid}/publish`, {});
  expect(patch.status === 200 && pub.status === 200, `fixture knowledge patch ${patch.status} publish ${pub.status}`);
  ...
  const legalQuestion = types.body.requestTypes.find((t) => t.slug === "legal_question") ?? types.body.requestTypes[0];
  const me = (await api(page, "GET", "/me")).body.user;
  const req = await api(page, "POST", "/requests", { requestTypeId: legalQuestion.id, departmentId: me.departmentId, title: `DOC-029 documents Portal request ${stamp}`, description: "DOC-029 fictional request.", urgency: "low" });
  expect(req.status === 201, `fixture request ${req.status} ${JSON.stringify(req.body).slice(0, 200)}`);
  const reqNumber = req.body.request.number;
  record(role, "request", `DOC-029 documents Portal request ${stamp}`, `R-${reqNumber}`);
  const att = await api(page, "POST", `/requests/${reqNumber}/attachments`, undefined, { file: part("doc029-schedule.csv", "portal-request-attachment.csv") });
  ...
  await step(A, S, role, "Read files in the Business Portal: Documents section, select a name to read the current Version, download beside it", "The reader opens the current Version and the download returns its bytes", async () => {
    ...
    await panel.getByRole("button", { name: "Close the document" }).click();
    const dl = await download(page, () => section().getByRole("link", { name: `Download ${chain.title}, version 2` }).click());
  ...
  await step(A, S, role, "Read files in the Business Portal: … earlier versions lists earlier Versions; a filename reads that Version", "The earlier Version opens in the reader and downloads", async () => {
    await section().getByRole("button", { name: "1 earlier version" }).click();
    await section().locator("ol").getByRole("button", { name: "portal-chain-v1.pdf", exact: true }).click();
    const panel = page.getByRole("complementary", { name: `${chain.title}, version 1` });
    await panel.waitFor({ timeout: 30000 });
    await panel.getByText(/^Page 1 of \d+$/).waitFor({ timeout: 30000 });
    await panel.getByRole("button", { name: "Close the document" }).click();
    const dl = await download(page, () => section().getByRole("link", { name: `Download ${chain.title}, version 1` }).click());
  ...
  await step(A, S, role, "Read files in the Business Portal: archived Documents and records outside your audience are not listed", "The archived Document is absent; a Contract without team membership is not found", async () => {
    ...
    expect(!archivedListed && direct.status === 404, `archived listed ${archivedListed} direct ${direct.status}`);
  ...
  await step(A, S, role, "Read files in the Business Portal: on your own Request, select a file under Attachments to download it", "The Request attachment downloads with its original bytes", async () => {
  ...
  await step(A, S, role, "Read files in the Business Portal: a comment attachment name opens its preview with Download; Preview unavailable appears for a file with no preview", "The PDF previews and downloads; a ZIP shows the unavailable message with Download; no filing action", async () => {
    await page.goto(`${BASE}/portal/contracts/${contract.number}`);
    await page.getByRole("toolbar", { name: "Applets" }).getByRole("button", { name: /^Comments/ }).click();
    ...
    const pdfDl = await download(page, () => dialog.getByRole("link", { name: "Download" }).click());
    ...
    const docxDl = await download(page, () => dialog.getByRole("link", { name: "Download" }).click());
    await dialog.getByRole("button", { name: "Close attachment preview" }).click();
    dialog = await open("portal-thread.zip");
    await dialog.getByText("Preview unavailable. You can download the original file.").waitFor({ timeout: 30000 });
    await shot(page, "r1-business-portal-preview-unavailable.png");
    const zipDl = await download(page, () => dialog.getByRole("link", { name: "Download" }).click());
    await dialog.getByRole("button", { name: "Close attachment preview" }).click();
    const listFile = await page.getByRole("button", { name: /^File to/ }).count();
    expect(pdfDl.sha256 === fixtureHash("doc029-services-text.pdf") && docxDl.sha256 === fixtureHash("doc029-draft-v1.docx") && zipDl.sha256 === fixtureHash("doc029-bundle.zip") && fileAction === 0 && listFile === 0, `file actions ${fileAction}/${listFile}`);
  ...
  await step(A, S, role, "Read files in the Business Portal: Knowledge lists the primary Document first with Download beside each file and no preview or Version picker; an archived item is no longer available", "Primary first; downloads return bytes; no reader; archived item unavailable", async () => {
    ...
    const rows = (await files.getByRole("listitem").allInnerTexts()).map((t) => t.replace(/\s+/g, " ").trim());
    expect(rows[0]?.startsWith("portal-knowledge-primary.pdf") && rows[1]?.startsWith("portal-knowledge-first.txt"), `order ${rows}`);
    const dl = await download(page, () => files.getByRole("link", { name: "Download portal-knowledge-primary.pdf" }).click());
    const readerButtons = await page.getByRole("button", { name: /portal-knowledge/ }).count();
    expect(dl.sha256 === fixtureHash("doc029-services-text.pdf") && readerButtons === 0, "knowledge download or reader");
    ...
    const stillListed = await page.getByRole("link", { name: "Download portal-knowledge-primary.pdf" }).count();
  ...
  await step(A, S, role, "Before you start: the Portal does not grant access to the staff Documents repository", "The staff repository address and its read are refused", async () => {
    await page.goto(`${BASE}/documents`);
    await page.waitForLoadState("networkidle").catch(() => {});
    const url = page.url();
    const read = await api(page, "GET", "/documents?limit=1");
    expect(!/\/documents$/.test(new URL(url).pathname) || read.status >= 400, `url ${url} read ${read.status}`);
    expect(read.status === 403 || read.status === 404 || read.status === 401, `read ${read.status}`);
  ...
  await step("document-versions", "V-C26", role, "Before you start: Business Users on the team upload Documents and add Versions in the Portal, including a new Version of the primary Document", "Upload documents creates a Document; Add version appends to the primary Document", async () => {
    ...
    const row = section().getByRole("listitem").filter({ has: page.getByRole("button", { name: primaryBefore.title, exact: true }) });
    ...
    await chooseFiles(page, dialog.getByRole("button", { name: "Drop files or choose files" }), fixture("doc029-draft-v1.docx"));
    await dialog.getByRole("button", { name: "Upload", exact: true }).click();
    await dialog.waitFor({ state: "hidden", timeout: 30000 });
    ...
    await chooseFiles(page, dialog.getByRole("button", { name: "Drop files or choose files" }), [fixture("doc029-bulk-c.txt"), fixture("doc029-bulk-d.txt")]);
    await dialog.getByRole("button", { name: "Upload", exact: true }).click();
    await dialog.waitFor({ state: "hidden", timeout: 30000 });
    ...
    expect(primaryAfter.versions.length === primaryBefore.versions.length + 1 && primaryAfter.isPrimary && added.length === 2 && added.every((d) => !d.folderId && !d.folder), `primary ${primaryBefore.versions.length}->${primaryAfter.versions.length} added ${added.length}`);
  ...
  await step("document-versions", "V-C26", role, "Before you start and Recover: Business Users cannot change designations, kinds or folders, archive or delete, and get no filing action", "The Portal shows no such controls and the staff seams refuse them", async () => {
    ...
    const controls = ["Make primary", "Mark as executed copy", "Move to folder", "Archive", "Delete", "New folder", "Edit details"].filter((n) => texts.includes(n));
    const attempts = {
      primary: (await api(page, "POST", `/documents/${other.id}/primary`, {})).status,
      kind: (await api(page, "PATCH", `/documents/${d.id}/versions/${v.id}`, { kind: "executed" })).status,
      executed: (await api(page, "POST", `/documents/${d.id}/executed-version`, { versionId: v.id })).status,
      folder: (await api(page, "POST", `${recUrl}/folders`, { name: "DOC-029 portal folder" })).status,
    ...
    expect(controls.length === 0 && Object.values(attempts).every((s) => s >= 400) && still?.isPrimary && !still.archivedAt, `controls ${controls} attempts ${JSON.stringify(attempts)}`);
  ...
  await step("document-folders", "V-C27", role, "Before you start: Business Users upload several files at the record root but cannot create folders, move Documents or import a folder structure", "No folder controls or folder picker in the Portal; folder and move seams refuse", async () => {
    await section().getByRole("button", { name: "Upload documents" }).click();
    ...
    const staffFolder = await api(staff, "POST", `${recUrl}/folders`, { name: `DOC-029 portal target ${stamp}` });
    const targetId = (await folderList(staff, recUrl)).find((f) => f.name === `DOC-029 portal target ${stamp}`)?.id;
    ...
    const create = await api(page, "POST", `${recUrl}/folders`, { name: "DOC-029 portal attempt" });
    const staffRoute = await api(page, "GET", `${recUrl}/folders`);
    expect(folderPicker === 0 && !/Choose folder|New folder/.test(dialogText) && move.status >= 400 && create.status >= 400, `picker ${folderPicker} move ${move.status} create ${create.status}`);
  ...
  await step("archive-and-delete-documents", "V-C54", role, "Before you start: Business Users cannot archive, restore or delete; they have no managed-Document deletion controls", "No archive, restore or delete controls; the seams refuse", async () => {
    await page.goto(`${BASE}/portal/contracts/${contract.number}`);
    await section().getByRole("heading", { name: "Documents" }).waitFor({ timeout: 30000 });
    ...
    expect(!/Archive|Restore|Delete|Show archived/.test(text) && restore.status >= 400 && archive.status >= 400 && del.status >= 400 && stillArchived, `restore ${restore.status} archive ${archive.status} delete ${del.status}`);
...
async function hiddenPaper() {
  ...
  const { context, page } = await staffContext(PEOPLE.daniel);
  const title = `DOC-029 documents hidden owner ${stamp}`;
  const c = await createContract(page, "administrator", title, { isConfidential: true });
  const doc = await uploadApi(page, `/contracts/${c.number}/documents`, "doc029-services-text.pdf", { as: `repo-hidden-${stamp}.pdf` });
...
async function repoTitles(page) {
  ...
  return (await repoTable(page).getByRole("row").allInnerTexts()).map((t) => t.replace(/\s+/g, " ").trim());
}
async function openFilter(page, name) {
  for (let i = 0; i < 3 && (await page.getByRole("dialog").count()) > 0; i++) await page.keyboard.press("Escape");
...
async function repository(role, person) {
  ...
  currentPage = page;
  const me = (await api(page, "GET", "/me")).body.user;
  const contract = await createContract(page, role, `DOC-029 documents ${label} repository Contract ${stamp}`);
  const recUrl = `/contracts/${contract.number}`;
  const cpName = `DOC-029 ${label} Northwind ${stamp}`;
  ...
  const folder = await api(page, "POST", `${recUrl}/folders`, { name: `DOC-029 ${label} Repo folder` });
  expect(folder.status === 201, `fixture folder ${folder.status}`);
  const folderId = (await folderList(page, recUrl)).find((f) => f.name === `DOC-029 ${label} Repo folder`).id;
  const tag = `${label.toLowerCase()}-${stamp}`;
  const rootPdf = await uploadApi(page, `${recUrl}/documents`, "doc029-services-text.pdf", { as: `repo-root-${tag}.pdf`, kind: "draft_ours" });
  await api(page, "POST", `/documents/${rootPdf.id}/versions`, undefined, { kind: "draft_theirs", file: part("doc029-scan.pdf", `repo-root-${tag}-v2.pdf`) });
  const filedDocx = await uploadApi(page, `${recUrl}/documents`, "doc029-draft-v1.docx", { as: `repo-folder-${tag}.docx`, kind: "executed", folderId });
  const archivable = await uploadApi(page, `${recUrl}/documents`, "doc029-bulk-a.txt", { as: `repo-archive-${tag}.txt` });
  const draggable = await uploadApi(page, `${recUrl}/documents`, "doc029-bulk-b.txt", { as: `repo-drag-${tag}.txt` });
  const matter = await createMatter(page, role, `DOC-029 documents ${label} repository Matter ${stamp}`);
  const matterDoc = await uploadApi(page, `/matters/${matter.number}/documents`, "doc029-services-text.pdf", { as: `repo-matter-${tag}.pdf` });
  const types = (await api(page, "GET", "/entities/types")).body.entityTypes;
  const ent = await api(page, "POST", "/entities", { legalName: `DOC-029 documents ${label} repository Entity ${stamp} Ltd`, entityTypeId: types[0].id });
  expect(ent.status === 201, `fixture entity ${ent.status}`);
  record(role, "entity", ent.body.entity.legalName, ent.body.entity.id);
  const entityDoc = await uploadApi(page, `/entities/${ent.body.entity.id}/documents`, "doc029-services-text.pdf", { as: `repo-entity-${tag}.pdf` });
  const kType = (await api(page, "GET", "/knowledge?limit=1")).body.knowledgeItems?.[0]?.knowledgeTypeId;
  const k = await api(page, "POST", "/knowledge", { title: `DOC-029 documents ${label} repository Knowledge ${stamp}`, knowledgeTypeId: kType });
  ...
  const knowledgeDoc = await uploadApi(page, `/knowledge/${k.body.knowledgeItem.id}/documents`, "doc029-services-text.pdf", { as: `repo-knowledge-${tag}.pdf` });
  const ad = await api(page, "POST", "/auto-docs", { name: `DOC-029 documents ${label} repository Auto-Doc ${stamp}` });
  expect(ad.status === 201, `fixture auto-doc ${ad.status} ${JSON.stringify(ad.body).slice(0, 160)}`);
  const autoDocId = ad.body.autoDoc?.id ?? ad.body.record?.id ?? ad.body.id;
  record(role, "auto_doc", `DOC-029 documents ${label} repository Auto-Doc ${stamp}`, autoDocId);
  const tpl = await api(page, "POST", `/auto-docs/${autoDocId}/template`, undefined, { file: part("doc029-draft-v1.docx", `repo-autodoc-${tag}.docx`) });
  expect(tpl.status === 201 || tpl.status === 200, `fixture template ${tpl.status} ${JSON.stringify(tpl.body).slice(0, 200)}`);
  const autoDocDoc = (await api(page, "GET", "/documents?owner=auto_doc&limit=100")).body.documents.find((d) => d.owner.id === autoDocId);
  ...
  await step(A, S, role, "Search and narrow step 1: Documents from navigation; Recent shows up to five Documents by current-Version upload, not by opening", "Recent lists at most five, in the order of the newest current Versions, and opening an older Document does not change it", async () => {
    ...
    let ui, apiTitles;
    for (let i = 0; i < 4; i++) {
      ui = (await recent.getByRole("link").evaluateAll((els) => els.map((e) => e.getAttribute("aria-label")))).map((t) => t.replace(/^Open recent Document /, ""));
      apiTitles = (await api(page, "GET", "/documents?limit=5")).body.documents.map((d) => d.title);
      if (JSON.stringify(ui) === JSON.stringify(apiTitles)) break;
      await page.reload();
      await recent.waitFor();
    }
    expect(ui.length <= 5 && JSON.stringify(ui) === JSON.stringify(apiTitles), `ui ${ui} api ${apiTitles}`);
    await page.goto(`${BASE}/contracts/${contract.number}/documents?doc=${filedDocx.id}&version=${filedDocx.versions[0].id}`);
    await page.getByRole("complementary", { name: `${filedDocx.title}, version 1` }).waitFor({ timeout: 30000 });
    await page.goto(`${BASE}/documents`);
    await recent.waitFor();
    let after, apiAfter;
    for (let i = 0; i < 4; i++) {
      after = (await recent.getByRole("link").evaluateAll((els) => els.map((e) => e.getAttribute("aria-label")))).map((t) => t.replace(/^Open recent Document /, ""));
      apiAfter = (await api(page, "GET", "/documents?limit=5")).body.documents.map((d) => d.title);
      if (JSON.stringify(after) === JSON.stringify(apiAfter)) break;
      await page.reload();
      await recent.waitFor();
    }
    expect(JSON.stringify(after) === JSON.stringify(apiAfter) && (!apiAfter.includes(filedDocx.title) || apiTitles.includes(filedDocx.title)), `after ${after}`);
  ...
  await step(A, S, role, "Search and narrow step 2: Filter, Owner, Contracts/Matters/Entities/Auto-Docs/Knowledge, Apply", "Owner offers the five kinds and each narrows to paper of that owner kind", async () => {
    const pop = await openFilter(page, "Owner");
    const choices = (await pop.locator("label").allInnerTexts()).map((t) => t.trim());
    await page.keyboard.press("Escape");
    expect(JSON.stringify(choices) === JSON.stringify(["Contracts", "Matters", "Entities", "Auto-Docs", "Knowledge"]), `choices ${choices}`);
    const expectations = [
      ...
      ["Auto-Docs", autoDocDoc.title, [rootPdf.title, matterDoc.title, entityDoc.title, knowledgeDoc.title]],
    ];
    const seen = [];
    for (const [choice, present, absent] of expectations) {
      ...
      const autoDocs = choice === "Auto-Docs" ? (await api(page, "GET", "/documents?owner=auto_doc&limit=100")).body.documents.map((d) => d.title) : null;
      expect((present === null || has(rows, present)) && absent.every((t) => !has(rows, t)), `${choice}: ${rows.slice(0, 4)}`);
      if (autoDocs) expect(rows.slice(1).every((r) => autoDocs.some((t) => r.includes(t))), `auto-doc rows ${rows.slice(0, 3)}`);
      seen.push(`${choice}: ${Math.max(0, rows.length - 1)} rows${present ? `, including ${present}` : ""}`);
    }
    await clearAll(page);
    return `Owner offered ${choices.join(", ")}. ${seen.join("; ")}. Each list left out the fixture paper of the other owner kinds.`;
  });

  await step(A, S, role, "Search and narrow step 3: Record with Search choices; Folder becomes available with Record root", "Record narrows to one Contract; Folder picks the folder or Record root", async () => {
    await applyChoice(page, "Record", [`C-${contract.number} · ${contract.title}`], `C-${contract.number}`);
    let rows = await repoTitles(page);
    expect(mine.every((t) => has(rows, t)) && rows.length - 1 === mine.length, `record rows ${rows.length - 1}`);
    let pop = await openFilter(page);
    const menu = (await pop.getByRole("button").allInnerTexts()).map((t) => t.replace(/[✓]/g, "").trim());
    await page.keyboard.press("Escape");
    expect(menu.includes("Folder"), `menu ${menu}`);
    await applyChoice(page, "Folder", [`DOC-029 ${label} Repo folder`]);
    ...
    const kMenu = (await pop.getByRole("button").allInnerTexts()).map((t) => t.replace(/[✓]/g, "").trim());
    await page.keyboard.press("Escape");
    await clearAll(page);
    await applyChoice(page, "Owner", ["Auto-Docs"]);
    pop = await openFilter(page);
    const aMenu = (await pop.getByRole("button").allInnerTexts()).map((t) => t.replace(/[✓]/g, "").trim());
    await page.keyboard.press("Escape");
    await clearAll(page);
    expect(!kMenu.includes("Folder") && !aMenu.includes("Folder"), `knowledge menu ${kMenu} auto-doc menu ${aMenu}`);
    return `Record with Search choices "C-${contract.number}" narrowed the list to the Contract's ${mine.length} live Documents, and the Filter menu then offered Folder. Folder "DOC-029 ${label} Repo folder" showed only "${filedDocx.title}"; Record root showed the Documents outside folders. With Owner Knowledge, and with Owner Auto-Docs, the Filter menu offered no Folder.`;
  });

  await step(A, S, role, "Search and narrow steps 4-5: Format and Uploader accept several values; different filters must all match; Uploaded From/To includes both dates; remove one filter and Clear all; Recent hides while filtered", "Any-of within a filter, all-of across filters, inclusive dates, and Recent only without filters", async () => {
    const today = new Date().toISOString().slice(0, 10);
    const tomorrow = new Date(Date.now() + 86400000).toISOString().slice(0, 10);
    await applyChoice(page, "Record", [`C-${contract.number} · ${contract.title}`], `C-${contract.number}`);
    const fmtHint = await applyChoice(page, "Format", ["PDF", "Word"]);
    let rows = await repoTitles(page);
    expect(has(rows, rootPdf.title) && has(rows, filedDocx.title) && !has(rows, archivable.title), `format rows ${rows}`);
    const recentWhileFiltered = await page.getByRole("region", { name: "Recent documents" }).count();
    await applyChoice(page, "Kind", ["Draft · ours"]);
    rows = await repoTitles(page);
    const currentKindOnly = !has(rows, rootPdf.title);
    await page.getByLabel("Record filters").getByRole("button", { name: "Remove Kind filter" }).click();
    await sleep(700);
    await applyChoice(page, "Kind", ["Executed"]);
    rows = await repoTitles(page);
    expect(currentKindOnly && rows.length - 1 === 1 && has(rows, filedDocx.title), `kind rows ${rows} currentKindOnly ${currentKindOnly}`);
    ...
    await page.getByLabel("Record filters").getByRole("button", { name: /^Uploaded:/ }).click();
    ...
    await page.getByLabel("Record filters").getByRole("button", { name: "Remove Uploaded filter" }).click();
    await sleep(900);
    rows = await repoTitles(page);
    ...
    expect(/Matches any selected value/.test(fmtHint) && /Includes both dates/.test(dateHint) && recentWhileFiltered === 0 && recentAfter === 1 && stillEmpty && widened, `hint ${fmtHint.slice(0, 60)} recent ${recentWhileFiltered}/${recentAfter} widened ${widened}`);
  ...
  await step(A, S, role, "Open the matching paper: selecting the name opens the owning record with the current Version; expand the chain for an earlier round", "The record opens with the reader on v2 and the chain lists v1", async () => {
    await applyChoice(page, "Record", [`C-${contract.number} · ${contract.title}`], `C-${contract.number}`);
    await repoTable(page).getByRole("link", { name: rootPdf.title, exact: true }).click();
    await page.waitForURL(new RegExp(`/contracts/${contract.number}/documents`), { timeout: 30000 });
    const panel = page.getByRole("complementary", { name: `${rootPdf.title}, version 2` });
    await panel.waitFor({ timeout: 30000 });
    await panel.getByText("v2", { exact: true }).waitFor();
    await page.getByRole("button", { name: `Show the 1 earlier version of ${rootPdf.title}` }).click();
    await page.getByRole("button", { name: rootPdf.title, exact: true }).last().click();
    await page.getByRole("complementary", { name: `${rootPdf.title}, version 1` }).waitFor({ timeout: 30000 });
    await page.goto(`${BASE}/documents`);
  ...
  await step(A, S, role, "Missing or archived: a hidden record's Document does not appear through a filter or direct link", role === "administrator" ? "An Administrator reaches every Contract, so the Confidential Contract's paper is listed" : "The Confidential Contract outside this reader's team is absent from Record choices and lists, and its address is refused", async () => {
    ...
    await page.keyboard.press("Escape");
    const listed = (await api(page, "GET", `/documents?limit=100`)).body.documents.some((d) => d.id === hidden.doc.id);
    ...
    await page.waitForLoadState("networkidle").catch(() => {});
    const pageText = (await page.getByRole("main").innerText().catch(() => "")).replace(/\s+/g, " ").slice(0, 100);
    await page.goto(`${BASE}/documents`);
    if (role === "administrator") {
      expect(choiceText.includes(hidden.title) && listed && direct.status === 200, `admin ${choiceText.slice(0, 80)} listed ${listed} direct ${direct.status}`);
    ...
    expect(!choiceText.includes(hidden.title) && /No matching choices/.test(choiceText) && !listed && direct.status === 404, `legal ${choiceText.slice(0, 80)} listed ${listed} direct ${direct.status}`);
  ...
  await step(A, S, role, "Missing or archived: Filter, Show archived includes archived Documents; the offered Restore brings one back", "The archived row appears with Archived and Restore; Restore makes it live", async () => {
    ...
    await clearAll(page);
    expect(hiddenBefore && /Archived/.test(row) && !live.archivedAt, `before ${hiddenBefore} row ${row}`);
  ...
  await step(A, S, role, "Before you start: drag a live Contract Document row onto a Move to folder target inside its owning record", "The Move to folder panel names the record's folders and the drop files the Document", async () => {
    await applyChoice(page, "Record", [`C-${contract.number} · ${contract.title}`], `C-${contract.number}`);
    ...
    const target = aside.getByText(`DOC-029 ${label} Repo folder`, { exact: true });
    ...
    await clearAll(page);
    expect((doc.folderId ?? doc.folder?.id) === folderId, "repository drag did not file the Document");
    return `Dragging the "${draggable.title}" row showed the Move to folder panel for C-${contract.number} with None and "DOC-029 ${label} Repo folder"; dropping on the folder filed the Document there (read-back).`;
  });
...
async function archive(role, person) {
  ...
  const { context, page } = await staffContext(person);
  currentPage = page;
  const contract = await createContract(page, role, `DOC-029 documents ${label} archive Contract ${stamp}`);
  const recUrl = `/contracts/${contract.number}`;
  const tag = `${label.toLowerCase()}-${stamp}`;
  const main = await uploadApi(page, `${recUrl}/documents`, "doc029-draft-v1.docx", { as: `archive-main-${tag}.docx` });
  for (const n of [2, 3]) await api(page, "POST", `/documents/${main.id}/versions`, undefined, { kind: "draft_theirs", file: part("doc029-draft-v2.docx", `archive-main-${tag}-v${n}.docx`) });
  let doc = await getDoc(page, recUrl, main.id);
  const v2 = doc.versions.find((v) => v.versionNumber === 2);
  expect((await api(page, "POST", `/documents/${main.id}/executed-version`, { versionId: v2.id })).status < 300, "fixture pin");
  if (!(await getDoc(page, recUrl, main.id)).isPrimary) expect((await api(page, "POST", `/documents/${main.id}/primary`, {})).status < 300, "fixture primary");
  const b1 = await uploadApi(page, `${recUrl}/documents`, "doc029-bulk-a.txt", { as: `archive-bulk-1-${tag}.txt` });
  const b2 = await uploadApi(page, `${recUrl}/documents`, "doc029-bulk-b.txt", { as: `archive-bulk-2-${tag}.txt` });

  await step(A, S, role, "Archive a Document steps 1-3: Actions, Archive; it leaves the normal list; Show archived finds its Archived row", "The Document leaves the list, reappears marked Archived, and keeps its Versions, files and designations", async () => {
    await openDocumentsTab(page, `${recUrl}/documents`);
    ...
    const downloads = [];
    for (const v of d.versions) downloads.push((await api(page, "GET", `/documents/${main.id}/versions/${v.id}/download`)).status);
    expect(d.archivedAt && d.versions.length === 3 && d.isPrimary && d.versions.find((v) => v.isExecuted)?.versionNumber === 2 && downloads.every((st) => st === 200), `archived ${JSON.stringify({ a: d.archivedAt, n: d.versions.length, p: d.isPrimary, downloads })}`);
  ...
  await step(A, S, role, "Restore a Document steps 1-3: Show archived, Restore from the row Actions menu; Versions and designations come back", "The Document is live again with its Versions, primary and executed-copy designation", async () => {
    ...
    expect(!d.archivedAt && d.isPrimary && d.versions.length === 3 && d.versions.find((v) => v.isExecuted)?.versionNumber === 2 && /Primary/.test(text), `restored ${text}`);
  ...
  await step(A, S, role, "Archive several live Documents with checkboxes and Archive; restore several archived Documents with Restore in the selection bar", "Both leave the list together and return together", async () => {
    ...
    await showArchived(page, false);
    expect(live.every((d) => !d.archivedAt), "bulk restore");
  ...
  await step(A, S, role, "Restore a Document: refused while the owning record is archived; restore the record first", "The archived record offers no Show archived or Document controls; restore is refused; after restoring the record the Document restores", async () => {
    expect((await api(page, "POST", `/documents/${b1.id}/archive`, {})).status === 200, "fixture doc archive");
    expect((await api(page, "POST", `${recUrl}/archive`, {})).status === 200, "fixture record archive");
    await openDocumentsTab(page, `${recUrl}/documents`);
    const sw = await docsSection(page).getByRole("switch", { name: "Show archived" }).count();
    const upload = await docsSection(page).getByRole("button", { name: "Upload", exact: true }).count();
    const menuItemsSeen = [];
    for (const trigger of await docsSection(page).getByRole("button", { name: /^Actions for / }).all()) {
      await trigger.click();
      menuItemsSeen.push(...(await page.getByRole("menu").getByRole("menuitem").allInnerTexts()).map((t) => t.trim()));
      await page.keyboard.press("Escape");
    }
    const menus = menuItemsSeen.filter((t) => /Archive|Restore|Delete|Add version|Move to folder/.test(t)).length;
    const refused = await api(page, "POST", `/documents/${b1.id}/restore`, {});
    expect((await api(page, "POST", `${recUrl}/restore`, {})).status === 200, "fixture record restore");
    await openDocumentsTab(page, `${recUrl}/documents`);
    await showArchived(page, true);
    ...
    await showArchived(page, false);
    expect(sw === 0 && upload === 0 && menus === 0 && refused.status === 409 && !d.archivedAt, `switch ${sw} menus ${menus} refused ${refused.status}`);
  ...
  await step(A, S, role, "Restore a Document: a stale-state message when someone already restored the same Document", "Restore on a stale row shows a message; reloading shows the live Document", async () => {
    expect((await api(page, "POST", `/documents/${b2.id}/archive`, {})).status === 200, "fixture archive");
    await openDocumentsTab(page, `${recUrl}/documents`);
    await showArchived(page, true);
    ...
    const alert = docsSection(page).locator('[aria-live="polite"]').filter({ hasText: /archived|restore|could not|already/i });
  ...
  if (role !== "administrator") {
    await step(A, S, role, "Before you start and Permanently delete: only an Administrator deletes; this role gets no Delete control and the seam refuses", "No Delete in the row menu or selection bar; direct deletion is refused and the Document stays", async () => {
      await openDocumentsTab(page, `${recUrl}/documents`);
      ...
      const del = await api(page, "DELETE", `/documents/${main.id}`, { confirmTitle: main.title });
      const d = await getDoc(page, recUrl, main.id);
      expect(!items.includes("Delete") && barDelete === 0 && del.status === 403 && d?.versions.length === 3, `items ${items} bar ${barDelete} delete ${del.status}`);
  ...
  await step(A, S, role, "Permanently delete steps 1-4: Actions, Delete; the dialog names the Document and Version count; Delete stays disabled until delete is typed; Cancel keeps it", "Cancel and a wrong word leave the Document; the dialog text matches the guide", async () => {
    await openDocumentsTab(page, `${recUrl}/documents`);
    ...
    await dialog.waitFor({ state: "hidden" });
    const d = await getDoc(page, recUrl, main.id);
    expect(/archive-main/.test(text) && /3 versions/.test(text) && disabledEmpty && disabledWrong && d?.versions.length === 3, `text ${text} disabled ${disabledEmpty}/${disabledWrong}`);
  ...
  await step(A, S, role, "If deletion fails: a rename while the dialog is open makes OpenLaw refuse the deletion", "The deletion is refused with a message and the Document stays", async () => {
  ...
  await step(A, S, role, "Permanently delete: type delete, select Delete; every Version and the primary reference go; no other Document becomes primary; the activity record remains", "The row is gone, Version downloads fail, no replacement primary, activity names the deletion", async () => {
    await openDocumentsTab(page, `${recUrl}/documents`);
    ...
    const downloads = [];
    for (const v of before.versions) downloads.push((await api(page, "GET", `/documents/${main.id}/versions/${v.id}/download`)).status);
    const docs = await recordDocs(page, recUrl, true);
    const activity = await api(page, "GET", `/activity?entityType=contract&entityId=${contractId}`);
    const mention = JSON.stringify(activity.body).includes(main.title);
    expect(!docs.some((d) => d.id === main.id) && downloads.every((st) => st === 404) && !docs.some((d) => d.isPrimary) && mention, `downloads ${downloads} primary ${docs.filter((d) => d.isPrimary).length} activity ${activity.status}/${mention}`);
  ...
  await step(A, S, role, "Permanently delete: several Documents with checkboxes and Delete in the selection bar, also confirmed by typing delete", "The bulk dialog asks for delete and removes both", async () => {
    const c1 = await uploadApi(page, `${recUrl}/documents`, "doc029-bulk-c.txt", { as: `archive-del-1-${tag}.txt` });
    const c2 = await uploadApi(page, `${recUrl}/documents`, "doc029-bulk-d.txt", { as: `archive-del-2-${tag}.txt` });
    await openDocumentsTab(page, `${recUrl}/documents`);
    ...
    await dialog.waitFor({ state: "hidden", timeout: 20000 });
    const docs = await recordDocs(page, recUrl, true);
    expect(disabled && !docs.some((d) => d.id === c1.id || d.id === c2.id), `disabled ${disabled}`);
  ...
  await step(A, S, role, "You cannot delete just one Version", "No Version-level delete control; the seam has no single-Version delete", async () => {
    const d = await uploadApi(page, `${recUrl}/documents`, "doc029-draft-v1.docx", { as: `archive-keep-${tag}.docx` });
    await api(page, "POST", `/documents/${d.id}/versions`, undefined, { kind: "draft_theirs", file: part("doc029-draft-v2.docx") });
    await openDocumentsTab(page, `${recUrl}/documents`);
    await page.getByRole("button", { name: `Show the 1 earlier version of ${d.title}` }).click();
    await page.getByRole("button", { name: `Actions for version 1 of ${d.title}`, exact: true }).click();
    ...
    expect(!items.some((i) => /Delete/.test(i)) && del.status >= 400 && after.versions.length === 2, `items ${items} delete ${del.status}`);
...
async function knowledgeSection(role, page, A, S, label) {
  await step(A, S, role, "Archive a Document step 1: a Knowledge Item's Documents section archives and restores; it has Set as primary and no Document folders", "Archive and Restore work in the Knowledge Documents section; Set as primary is offered; no New folder", async () => {
    const kType = (await api(page, "GET", "/knowledge?limit=1")).body.knowledgeItems?.[0]?.knowledgeTypeId;
    const k = await api(page, "POST", "/knowledge", { title: `DOC-029 documents ${label} archive Knowledge ${stamp}`, knowledgeTypeId: kType });
    expect(k.status === 201, `fixture knowledge ${k.status}`);
    const kid = k.body.knowledgeItem.id;
    record(role, "knowledge_item", k.body.knowledgeItem.title, kid);
    const d1 = await uploadApi(page, `/knowledge/${kid}/documents`, "doc029-services-text.pdf", { as: `knowledge-a-${label.toLowerCase()}-${stamp}.pdf` });
    const d2 = await uploadApi(page, `/knowledge/${kid}/documents`, "doc029-bulk-a.txt", { as: `knowledge-b-${label.toLowerCase()}-${stamp}.txt` });
    ...
    expect(newFolder === 0 && items.includes("Set as primary") && live, `newFolder ${newFolder} items ${items} live ${live}`);
