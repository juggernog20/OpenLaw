...
**Live pair**:
The one Document Version and the one form version that Publish pinned together on an Auto-Doc. Every Generation cites its live pair. Editing either chain changes nothing until the next Publish; Unpublish clears the pair and deletes nothing [ADO-004].
...
An Entity marked Portal-listed so Business Users can pick it by name wherever Legal placed an Entity picker on a Portal request form or Auto-Doc form. Names only; a Confidential Entity is never listed [ENT-010, DD-027].
...
- A **user** belongs to at most one **Department**; a **Contract** has zero or one Owning department [SET-010, CTR-025]
