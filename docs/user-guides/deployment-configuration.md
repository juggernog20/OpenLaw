...
- **System status** shows database availability, the active storage driver, the active document-service address, and one row for each API and worker process. A process with no heartbeat in the last minute shows **No recent heartbeat**, and the page warns that a heartbeat is missing. A worker that stops cleanly, for example with `docker compose stop worker`, removes its own row instead. A process that started with a different configuration from the saved one shows **Restart required** under **Configuration**. Use the connection tests and a real document-processing check to verify the dependent services. A running process alone does not prove them healthy.
...
Ignore the `127.0.0.1` lines. They come from the container's own health check. With a correct value, the other lines show the browsers' addresses. If every other request shows the gateway address, the list does not match the proxy.
...
1. Give the instance a public DNS hostname with a public IPv4 address. Terminate TLS there with a publicly trusted certificate. Serve it on the default HTTPS port, 443. Set that HTTPS origin as `BASE_URL`, without a port or an application subpath. claude.ai did not reach an origin on port 8443. Use the saved **Application address** only when `BASE_URL` is empty. Recreate app and worker after environment changes; restart both after app-saved changes.
...
- Saved **Settings → Advanced** values fall back to the deployment environment or the default, and those fields show **Default**. The instance address becomes `BASE_URL`, or `http://localhost:3000` when `BASE_URL` is empty. Storage uses `STORAGE_DRIVER` from `.env`, or the local driver when it is unset. A Document stored only in a bucket or container that was saved in the app then fails to download.
