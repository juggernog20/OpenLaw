...
async function typesSection(admin, member) {
  ...
  await step(SC, role, "Add and maintain types, steps 1-2: open Settings > Organization > Matters > Types, select Add type, enter a name, select Save; select Cancel to discard a draft", "Save stays unavailable while the name is blank. Cancel discards the draft and creates no row. Save adds the new type row to the Matter types list.", async () => {
    ...
    const cancelVisible = await admin.getByRole("button", { name: "Cancel", exact: true }).isVisible();
    ...
    const cancelledRows = await admin.getByRole("button", { name: `Rename ${cancelName}`, exact: true }).count();
    expectThat(saveBlank && cancelVisible && saveFilled, `Save blank disabled ${saveBlank}, Cancel visible ${cancelVisible}, Save enabled with a name ${saveFilled}`);
  ...
  await step(SC, role, "Step 1-2 for the other modules: Contracts, Entities, and Knowledge Types each accept Add type; Edit exists only where the article says it may", "Contracts and Entities rows have Edit; Knowledge rows have none.", async () => {
    ...
    for (const [link, heading, pane] of [
      ...
      out.push(`${link}: added ${q(name)} with Save, Edit control ${edit ? "present" : "absent"}`);
  ...
  await step(SC, role, "Create a Field, first paragraph: Contracts and Matters Fields show Default fields as locked, read-only rows (collapsed at first, expand on the heading) above an expanded Custom Fields card with the add controls; Entities Fields has no Default fields card", "Default fields starts collapsed; selecting its heading shows locked rows with no Rename, Archive, or reorder controls. Custom Fields starts expanded and holds Add field. Entities Fields shows one Fields card.", async () => {
    const out = [];
    for (const [link, pane] of [["Matters", "Matters panes"], ["Contracts", "Contracts panes"]]) {
      await openSettings(admin, "Organization", link);
      await admin.getByRole("navigation", { name: pane }).getByRole("link", { name: "Fields" }).click();
      ...
      const order = await defaults.evaluate((d, c) => !!(d.compareDocumentPosition(c) & Node.DOCUMENT_POSITION_FOLLOWING), await custom.elementHandle());
      const toggle = defaults.getByRole("button", { name: "Default fields" });
      const startExpanded = await toggle.getAttribute("aria-expanded");
      const customExpanded = await custom.getByRole("button", { name: "Custom Fields" }).getAttribute("aria-expanded");
      ...
      const rows = (await defaults.getByRole("listitem").allInnerTexts()).map((t) => t.replace(/\s+/g, " ").trim());
      ...
      const inputs = (await defaults.getByRole("textbox").count()) + (await defaults.getByRole("switch").count());
      expectThat(order, `${link}: Default fields is not above Custom Fields`);
      expectThat(startExpanded === "false" && rowsBefore === 0, `${link}: Default fields started expanded=${startExpanded} with ${rowsBefore} rows`);
      expectThat(customExpanded === "true", `${link}: Custom Fields expanded=${customExpanded}`);
      expectThat(rows.length > 0 && locks === rows.length && controls === 0 && inputs === 0, `${link}: rows ${rows.length}, locks ${locks}, row buttons ${controls}, inputs ${inputs}`);
      out.push(`${link} > Fields: Default fields sits above Custom Fields, starts with aria-expanded=${startExpanded} and no visible rows; Custom Fields starts aria-expanded=${customExpanded} with Add field. Selecting the Default fields heading showed ${rows.length} rows (${q(rows.slice(0, 5))}...), each with a "<name>: built-in field, read-only here" lock image, and ${controls} row buttons and ${inputs} text boxes or switches`);
    }
    await openSettings(admin, "Organization", "Entities");
    await admin.getByRole("navigation", { name: "Entities panes" }).getByRole("link", { name: "Fields" }).click();
    ...
    out.push(`Entities > Fields has ${entityDefaults} Default fields card and ${entityCustom} Custom Fields card; Add field is on its Fields card`);
    return out.join(". ") + ".";
  });
  ...
  await step(SC, role, "Detach and Archive keep stored values: detach the Field from the Matter type, archive it in the catalog, then Show archived and Restore", "The definition stays in the catalog after Detach; the Matter keeps its stored value; Archive hides the Field; Restore brings it back.", async () => {
    ...
    if (await admin.getByRole("switch", { name: "Show archived" }).count()) await admin.getByRole("switch", { name: "Show archived" }).click();
...
async function formsSection(admin, business) {
  ...
  await step(SC, role, "Create the request type, steps 1-4: Settings > Intake > Request types > Add request type, display name, Save (and Cancel discards a draft), Edit, Description saved on leaving the field; the list Target column and the editor have no Target control", "The new row shows Target: No target; the editor has Display name, Description, and Target turnaround, and no Target or Slug control; the Description persists after reload.", async () => {
    ...
    await admin.getByRole("button", { name: "Cancel", exact: true }).click();
    await admin.getByRole("textbox", { name: "New request type name" }).waitFor({ state: "detached", timeout: 10000 });
    await pause(1000);
    await admin.reload();
    await admin.getByRole("button", { name: /^Rename / }).first().waitFor();
  ...
  await step(SC, "business_user", "Check the result on the Portal: request-type cards, Before you submit, Fields, order, and required markers; submit with a required answer missing, then with valid answers", "Both cards show the description and estimated turnaround; each form shows its own guidance; Fields appear in the configured order with required markers; a missing required answer is refused; valid answers create a Request.", async () => {
    ...
    await business.screenshot({ path: path.join(SHOTS, "r2-1-portal-contract-form.png"), fullPage: true });
  ...
  await step(SC, role, "Check the result: have Legal convert the Matter form Request to a Matter and check the intended answer on the new record", "The Convert dialog lists no answer under Does not carry into the matter, and the new Matter holds the Entity answer and the Request's title.", async () => {
    currentPage = admin;
    ...
    await pause(1500);
    const templates = await d.getByRole("combobox", { name: "Matter template" }).locator("option").allInnerTexts().catch(() => []);
    const text = (await d.innerText()).replace(/\s+/g, " ");
    ...
    expectThat(entityValue && m.title === S.matterRequestTitle, `matter title ${q(m.title)}, custom fields ${q(cf)}`);
...
async function remindersSection(admin, browser) {
  ...
  await step(SC, role, "Steps 3-5: Add lead time 9 with Save, Cancel discards a draft, refuse a duplicate and 731, reorder with the arrow keys, reload and confirm; check the 20-entry limit and that the last entry cannot be removed", "9 days before is added; a duplicate and 731 are refused; the reordered list persists; a 21st entry is refused; the last entry has no working Remove.", async () => {
    ...
    await admin.getByRole("button", { name: "Cancel", exact: true }).click();
    await admin.getByRole("spinbutton", { name: "days before the date" }).waitFor({ state: "detached", timeout: 10000 });
    await pause(1000);
...
async function auditSection(admin, member) {
  ...
  await step(SC, role, "Steps 3-5: narrow with From and To in the browser's local days and with Search; read event, record, audience, and time; Show older; Clear filters", "Today's bounds keep the entries; a range that ends yesterday removes them; Search narrows to the walkthrough's Matter type; Show older adds rows; Clear filters resets every filter.", async () => {
    ...
    expectThat(searched.length > 0 && searched.every((r) => r.includes(G)), `search rows ${q(searched.slice(0, 3))}`);
