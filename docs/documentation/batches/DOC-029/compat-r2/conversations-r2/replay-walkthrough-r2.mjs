...
async function mailSearch(query) {
  const r = await fetch(
    `${L.MAIL}/api/v1/search?query=${encodeURIComponent(query)}&limit=500`,
  ).then((x) => x.json());
  return r.messages ?? [];
}
/** Total messages matching a query, not capped by the page size. */
async function mailTotal(query) {
  const r = await fetch(`${L.MAIL}/api/v1/search?query=${encodeURIComponent(query)}&limit=1`).then(
    (x) => x.json(),
  );
...
async function setupFixtures() {
  ...
  const submit = async (title) => {
    const r = await L.api(j, "POST", "/requests", {
      ...
      title,
      description:
        "DOC-029r2 conversations fixture. Fictional request for the comments walkthrough.",
      urgency: "medium",
      customFields: {},
    });
...
async function memberNotifications() {
  ...
  await step(
    ...
    async () => {
      ...
      if (startUnread < 10) {
        for (let k = 0; k < 10 - startUnread; k++)
          await comment(
            d,
            "contract",
            fx.contractId,
            `${tag} Daniel badge filler ${k + 1}`,
            "full_thread",
          );
        prep(
          `Nadia Haddad had ${startUnread} unread notifications, so Daniel Okafor posted ${10 - startUnread} Contract Team comments on C-${fx.contract.number} through the comments API to take her above nine for the 9+ badge check.`,
        );
      }
      await comment(
        d,
  ...
  await step(
    ...
    async () => {
      const r = await L.api(ctx.jonas.page, "POST", "/requests", {
        ...
        title: `DOC-029r2 conversations new request bell ${stamp}`,
        description: "DOC-029r2 conversations fictional bell check.",
        urgency: "low",
        customFields: {},
      });
      ...
      const link = await findItem(
        p,
        dialog,
        new RegExp(`new request: DOC-029r2 conversations new request bell ${stamp}`),
      );
...
async function portalNotifications() {
  ...
  await step(
    ...
    async () => {
      ...
      const receipt = await findItem(
        p,
        dialog,
        new RegExp(`received your request DOC-029r2 conversations new request bell ${stamp}`),
      );
  ...
  await step(
    NT,
    role,
    "Converted record, the Requester: a shared comment on the Contract converted from his Request reaches him as a reply under Request updates, with the bell item saying replied on your request and the email subject starting Legal replied on, although Activity on your records Email is off",
    "request.replied item (no comment.posted), bell text replied on your request, one Legal replied on mail; the Activity on your records Email switch is off",
    async () => {
      const title = fx.contractTitle;
      await p.goto(`${L.BASE}/portal`);
      await p.getByRole("banner").getByRole("link", { name: "Notification settings" }).click();
      ...
      const bm = (await mailSearch(`to:"${email}"`)).filter(
        (m) => m.Subject.startsWith("Legal replied on") && m.Subject.includes(title),
      ).length;
      ...
      const cid = await comment(
        ...
        "contract",
        fx.contractId,
        `${tag} Nadia shared comment for the Requester`,
        "full_thread",
      );
      await settleQueue();
      const items = await aboutComment(p, cid, since, true);
      const replied = (await mailSearch(`to:"${email}"`)).filter(
        (m) => m.Subject.startsWith("Legal replied on") && m.Subject.includes(title),
      );
      await p.goto(`${L.BASE}/portal`);
      const dialog = await openBell(p);
      const link = dialog
        .getByRole("link", { name: new RegExp(`Nadia Haddad replied on your request ${title}`) })
        .first();
      ...
      results.screenshots = [
        ...(results.screenshots ?? []),
        "r2-portal-bell-converted-contract-reply.png",
      ];
      await link.click();
      await p.waitForURL(new RegExp(`/portal/contracts/${fx.contract.number}`), { timeout: 15000 });
      const landed = new URL(p.url()).pathname;
      expect(
        prefs["Request updates Email"] &&
          !prefs["Activity on your records Email"] &&
          items.length === 1 &&
          items[0].eventType === "request.replied" &&
          replied.length - bm === 1,
        JSON.stringify({
          prefs,
          types: items.map((x) => x.eventType),
          mailDelta: replied.length - bm,
        }),
      );
  ...
  await step(
    NT,
    role,
    "Converted record, the Requester: turn off Email for Request updates in Notification settings; the next shared comment keeps the bell item and sends no Legal replied on mail",
    "Saved; after reload off; one request.replied item; zero Legal replied on mail; switch restored",
    async () => {
      const title = fx.contractTitle;
      await p.goto(`${L.BASE}/portal`);
      await p.getByRole("banner").getByRole("link", { name: "Notification settings" }).click();
      ...
      try {
        const since = new Date(Date.now() - 2000).toISOString();
        const count = async () =>
          (await mailSearch(`to:"${email}"`)).filter(
            (m) => m.Subject.startsWith("Legal replied on") && m.Subject.includes(title),
          ).length;
        ...
        const cid = await comment(
          ...
          "contract",
          fx.contractId,
          `${tag} Nadia shared comment with Request updates Email off`,
          "full_thread",
        );
        await settleQueue();
      ...
      expect(
        kept === "false" &&
          ...
          delta === 0 &&
          allDelta === 0,
        JSON.stringify({ kept, types: items?.map((x) => x.eventType), delta, allDelta }),
      );
  ...
  await step(
    NT,
    role,
    "Team member who did not raise the Request: Contract team addition under Assigned to you; the same shared comment under Activity on your records; Email off gives no mail, Email on gives mail, In-app off stops both",
    "contract.team_added item and You were added to mail; comment.posted with no mail; with Email on one mail; with In-app off no item and no mail",
    async () => {
      const title = fx.contractTitle;
      const aEmail = L.PEOPLE.amara.email;
      const sinceAdd = new Date(Date.now() - 2000).toISOString();
      const addMailBefore = (await mailSearch(`to:"${aEmail}"`)).filter((m) =>
        m.Subject.includes(`You were added to ${title}`),
      ).length;
      ...
      fx.amaraOnTeam = true;
      prep(
        `Daniel Okafor added Amara Nwosu (Business User, not the Requester) to the C-${fx.contract.number} team through the team API for the team-member comparison.`,
      );
      ctx.amara ??= await L.portalContext(L.PEOPLE.amara);
      ctx.amara.context.setDefaultTimeout(15000);
      const a = ctx.amara.page;
      await settleQueue();
      const addItems = (await itemsMatching(a, new RegExp(fx.contractId), true, sinceAdd)).map(
        (x) => x.eventType,
      );
      const addMail =
        (await mailSearch(`to:"${aEmail}"`)).filter((m) =>
          m.Subject.includes(`You were added to ${title}`),
        ).length - addMailBefore;
      ...
      const init = await switchStates(a, [
        "Activity on your records In-app",
        "Activity on your records Email",
      ]);
      const round = async (label) => {
        ...
        const jBefore = (await mailSearch(`to:"${email}"`)).filter((m) =>
          m.Subject.startsWith("Legal replied on"),
        ).length;
        const cid = await comment(
          n,
          "contract",
          fx.contractId,
          `${tag} Nadia team comment round ${label}`,
          "full_thread",
        );
        await settleQueue();
        ...
        const mails =
          newCount > 0
            ? (await mailSearch(`to:"${aEmail}"`)).slice(0, newCount).map((m) => m.Subject)
            : [];
        const jItems = await aboutComment(p, cid, since, true);
        const jReplied =
          (await mailSearch(`to:"${email}"`)).filter((m) =>
            m.Subject.startsWith("Legal replied on"),
          ).length - jBefore;
        return {
          types: items.map((x) => x.eventType),
          mails,
          jonas: { types: jItems.map((x) => x.eventType), repliedMail: jReplied },
        };
      };
      ...
      try {
        ...
        await a.reload();
        kept = await a
          .getByRole("switch", { name: "Activity on your records In-app", exact: true })
          .getAttribute("aria-checked");
      ...
      const ok =
        addItems.includes("contract.team_added") &&
        addMail === 1 &&
        init["Activity on your records In-app"] &&
        !init["Activity on your records Email"] &&
        r1.types.length === 1 &&
        r1.types[0] === "comment.posted" &&
        r1.mails.length === 0 &&
        r1.jonas.types.length === 1 &&
        r1.jonas.types[0] === "request.replied" &&
        r1.jonas.repliedMail === 1 &&
        r2.types.length === 1 &&
        r2.types[0] === "comment.posted" &&
        r2.mails.length === 1 &&
        kept === "false" &&
        r3.types.length === 0 &&
        r3.mails.length === 0;
  ...
  await step(
    NT,
    role,
    "Activity on your records also covers supporting Documents and Contract status changes on the records whose team you are on",
    "Amara gets document.added and contract.status_changed items for C-N",
    async () => {
      const a = ctx.amara.page;
      const since = new Date(Date.now() - 2000).toISOString();
      const up = await L.api(d, "POST", `/contracts/${fx.contract.number}/documents`, undefined, {
        file: {
          name: "doc029-conv-portal-note.pdf",
          mimeType: "application/pdf",
          buffer: readFileSync(fixture("doc029-conv-portal-note.pdf")),
        },
      });
      ...
      const target =
        list.find(
          (s) =>
            s.id !== (cur.statusId ?? cur.status?.id) &&
            !s.archivedAt &&
            s.stage !== "executed" &&
            s.stage !== "signature",
        ) ?? list.find((s) => s.id !== (cur.statusId ?? cur.status?.id));
      const st = await L.api(d, "PATCH", `/contracts/${fx.contract.number}`, {
        statusId: target.id,
      });
      expect(st.status < 300, `status ${st.status} ${JSON.stringify(st.body).slice(0, 200)}`);
      prep(
        `Daniel Okafor uploaded two fictional PDFs (doc029-conv-portal-note.pdf, doc029-conv-schedule.pdf) to C-${fx.contract.number} through the documents API and moved its status to ${q(target.displayName ?? target.label ?? target.name)} through the contract API.`,
      );
      await settleQueue();
      const aTypes = (await itemsMatching(a, new RegExp(fx.contractId), true, since)).map(
        (x) => x.eventType,
      );
      const jTypes = (
        await itemsMatching(p, new RegExp(`${fx.contractId}|${fx.contractRequest}`), true, since)
      ).map((x) => x.eventType);
      await a.goto(`${L.BASE}/portal`);
      const dialog = await openBell(a);
      const texts = (
        await dialog.getByRole("link", { name: new RegExp(fx.contractTitle) }).allInnerTexts()
      )
        .slice(0, 4)
        .map((x) => x.replace(/\s+/g, " ").trim());
      await closeBell(a);
      expect(
        aTypes.includes("document.added") && aTypes.includes("contract.status_changed"),
        JSON.stringify({ aTypes, jTypes }),
      );
  ...
  await step(
    NT,
    role,
    "Portal: a Business User receives no Legal Only comment item and a mention takes Assigned to you, not a second comment item (Amara, team member)",
    "No item for Legal Only; one comment.mentioned for a mention",
    async () => {
      const a = ctx.amara.page;
      const since = new Date(Date.now() - 2000).toISOString();
      const mid = await comment(
      ...
      const lid = await comment(
        ...
        "contract",
        fx.contractId,
        `${tag} Nadia legal only for Amara check`,
        "legal_only",
      );
      await settleQueue();
      ...
      prep(
        `Daniel Okafor removed Amara Nwosu from the C-${fx.contract.number} team after the comparison (HTTP ${rm.status}).`,
      );
      expect(
        m.length === 1 && m[0] === "comment.mentioned" && l.length === 0,
        JSON.stringify({ m, l }),
      );
...
async function morning() {
  ...
  const LA = [
    ...
    "Asia/Dubai",
    "Asia/Tokyo",
  ].find((tz) => localHour(tz) >= 9 && localHour(tz) <= 21);
  const HNL = [
    "Pacific/Honolulu",
    "America/Anchorage",
    "America/Los_Angeles",
    ...
    "Asia/Tokyo",
    "Pacific/Auckland",
  ].find((tz) => localHour(tz) >= 1 && localHour(tz) < 7);
  ...
  await step(
    ...
    async () => {
      ...
      prep(
        `The Member Dates, Member No Date Bell, and Admin No Date Email accounts had their timezone set to ${LA} (local hour ${localHour(LA)}) through the preferences API; Member Before Eight stayed on ${HNL} (local hour ${localHour(HNL)}).`,
      );
      expect(
  ...
  await step(
    ...
    "Accounts in the after-8:00 timezone receive what their switches allow; the before-8:00 account receives nothing; Business User receives no briefing",
    async () => {
    ...
    await step(
      ...
      async () => {
        ...
        const label = MORNING_STAMP
          ? `DOC-029r2 conversations ${MORNING_STAMP} narrowed review`
          : `${tag} narrowed review`;
        const kd = (
          await L.api(d, "GET", `/contracts/${fx.contract.number}/key-dates`)
...
async function shots() {
  ...
  await step(
    NT,
    "business_user",
    "Screenshot: Portal Notification settings with the three groups",
    "Screenshot saved",
    async () => {
...
// ---------- main ----------
let fx = existsSync(path.join(here, "fixtures-r2.json"))
  ? JSON.parse(readFileSync(path.join(here, "fixtures-r2.json"), "utf8"))
  : null;
