...
const LAB = JSON.parse(
  readFileSync(path.join(root, ".documentation-labs/firstrun/lab.json"), "utf8"),
);
...
async function phaseA() {
    ...
    async () => {
      ...
      const res = await other.request.post(`${BASE}/api/v1/auth/setup`, {
        data: {
          ...
          displayName: "DOC-029 first-run Second",
  ...
  await step(
    ...
    async () => {
      ...
      for (const name of [
        ...
        await expect(page.getByRole("switch", { name, exact: true })).toBeVisible();
      await expect(
        page.getByText("No domains allowed yet. Magic-link sign-in is unavailable."),
      ).toBeVisible();
  ...
  await step(
    ...
    async () => {
      const hint = (
        await page.getByText(/Skip it and Contract analysis does not run/).textContent()
      ).trim();
      await button(page, "Set up later").click();
    ...
    async () => {
      const rows = await page.locator("main table tbody tr").evaluateAll((trs) =>
        trs.map((tr) => ({
          ...
          detail: tr.querySelector("th p")?.textContent.trim() ?? undefined,
        })),
      );
      must(rows.length >= 2, "no review rows");
      must(
        rows.some((r) => r.list === "Reminder offsets"),
        "no Reminder offsets row",
      );
  ...
  await step(
    ...
    async () => {
      ...
      must(
        (await page.getByText("DOC-029 first-run Unsent").count()) === 0,
...
async function phaseB0() {
  ...
  const page = await context.newPage();
  await step(
    "Create the first Administrator (or sign in, if an earlier run on this lab created it) on an instance whose environment sets SMTP_URL but not SMTP_FROM",
    "The unfinished wizard opens at Welcome to OpenLaw.",
    async () => {
  ...
  await step(
    ...
    "Every app page opens the wizard.",
    async () => {
      const out = [];
      for (const p of ["/", "/settings/general", "/contracts", "/matters"])
        out.push(await landsOnWizard(page, p));
      return out.join("; ");
    },
  );
  await step(
...
async function phaseB1() {
  ...
  await step(
    ...
    "Outbound email opens with SMTP relay URL, From address and Save relay; no Set up later; Continue unavailable.",
    async () => {
      await button(page, "Skip optional steps").click();
      const where = await expectStep(page, "Outbound email", 5);
  ...
  await step(
    "Save a relay that is not an SMTP URL",
  ...
  await step(
    "Enter SMTP relay URL and From address, select Save relay, then Send test email",
