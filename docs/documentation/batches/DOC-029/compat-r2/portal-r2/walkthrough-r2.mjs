// DOC-029r2 compatibility replay, group "portal", round 2.
// Round 2 replay by the DOC-029r2 compatibility reviewer (portal). Adapted from portal/walkthrough-r1.mjs, written by the DOC-029 independent walkthrough agent (portal, round 1). Only record names, the log header, and file paths changed.
...
// Run from the worktree root after fixtures-r2.mjs:
//   LAB_SEED_PASSWORD=... STATE=<private state file> PHASES=submit,follow,knowledge mise exec -- node docs/documentation/batches/DOC-029/compat-r2/portal-r2/walkthrough-r2.mjs
...
const log = existsSync(OUT)
  ...
  : {
      ...
      group: "portal",
      round: 2,
      walkthroughReviewer: "DOC-029r2 compatibility reviewer (portal)",
      reviewerKind: "agent",
      appCommit: "57e77e386be31b2a319f7143dd54d00123e65efe",
      environment: process.env.LAB_PROJECT ?? "openlaw-docs-41255c61-work2",
...
async function step(article, role, name, expected, fn) {
  const entry = { article, role, method: "browser-walkthrough", step: name, expected, actual: null, result: "not-run", startedAt: new Date().toISOString(), at: null, run: log.runs.length };
  log.steps.push(entry);
  try {
    entry.actual = await fn();
    entry.result = "pass";
  } catch (error) {
    entry.actual = `Check did not hold: ${String(error?.message ?? error).split("\n").slice(0, 3).join(" ")}`;
    entry.result = "fail";
  }
  entry.at = new Date().toISOString();
  console.log(`[${article}/${role}] ${entry.result.toUpperCase()} ${name}${entry.result === "fail" ? " :: " + entry.actual : ""}`);
  save();
  return entry;
}
...
async function submitPhase() {
  ...
  const posts = [];
  page.on("request", (req) => {
    if (req.method() === "POST" && new URL(req.url()).pathname === "/api/v1/requests") posts.push(req.url());
  });
  const title = `DOC-029r2 portal Review the Northstar evaluation terms ${runStamp}`;
  const chosen = `doc029-northstar-terms-${runStamp}.pdf`;
  const dropped = `doc029-northstar-dropped-${runStamp}.pdf`;
  let number = null;

  await step(A, R, "Portal home: read Before you submit, then select the request type", "Home shows request type cards, a Before you submit panel, and Your requests; selecting NDA request opens its form", async () => {
    ...
    await page.getByRole("list", { name: "Request types" }).getByRole("link", { name: /NDA request/ }).click();
  ...
  await step(A, R, "Form labels, asterisks, Department default, and Urgency default", "Title, Description, Attachments, Department, Urgency and the type's questions show; required ones carry an asterisk; Department starts at the user's own Department; Urgency starts at Medium with Low/Medium/High/Critical", async () => {
    const labels = await page.locator("form label").allInnerTexts();
    const clean = labels.map((l) => l.replace(/\s+/g, " ").trim());
    for (const want of ["Title", "Description", "Attachments", "Department", "Urgency", "Counterparty name"]) {
      expect(clean.some((l) => l.startsWith(want)), `label ${want} missing (${clean.join(" | ")})`);
    }
    const starred = clean.filter((l) => l.includes("*"));
    expect(starred.some((l) => l.startsWith("Title")) && starred.some((l) => l.startsWith("Department")) && starred.some((l) => l.startsWith("Counterparty name")), `asterisks: ${starred.join(" | ")}`);
    const notStarred = clean.filter((l) => !l.includes("*"));
    const dept = page.getByLabel("Department");
    const deptText = await dept.evaluate((el) => el.options ? el.options[el.selectedIndex]?.textContent : el.textContent);
    ...
    expect(JSON.stringify(urgOpts) === JSON.stringify(["Low", "Medium", "High", "Critical"]), `Urgency options ${urgOpts}`);
  ...
  await step(A, R, "Submit request with required answers missing", "Submission is refused, the missing questions are named and marked, and no Request is sent", async () => {
    ...
    expect(/Title/.test(said) && /Description/.test(said) && /Counterparty name/.test(said), `alert: ${said}`);
    return `Alert "${said}"; field marks: ${marks.join(" / ")}; POST /api/v1/requests sent: ${posts.length}`;
  });

  await step(A, R, "Attachments: Choose files beyond 20, then remove unintended files", "The form keeps only the files that fit (20) and says so; Remove drops a file from the list", async () => {
    const many = [];
    for (let i = 1; i <= 21; i++) many.push(await pdfFile(`doc029-extra-${String(i).padStart(2, "0")}-${runStamp}.pdf`));
    const [chooser] = await Promise.all([page.waitForEvent("filechooser"), page.getByRole("button", { name: "Choose files" }).click()]);
    ...
    await page.getByRole("button", { name: `Remove doc029-extra-21-${runStamp}.pdf` }).count().then((c) => expect(c === 0, "21st file kept"));
    for (let i = 20; i >= 1; i--) await page.getByRole("button", { name: `Remove doc029-extra-${String(i).padStart(2, "0")}-${runStamp}.pdf` }).click();
  ...
  await step(A, R, "Attachments: select one file with Choose files and drop another onto the form", "Both filenames appear in the list", async () => {
    const [chooser] = await Promise.all([page.waitForEvent("filechooser"), page.getByRole("button", { name: "Choose files" }).click()]);
  ...
  await step(A, R, "Complete the form and Submit request once; keep the confirmation open", "Confirmation names the R- reference, shows Attaching your files while uploads run, reports no failure, and offers Open request", async () => {
    await page.getByLabel(/^Title/).fill(title);
    await page.getByLabel(/^Description/).fill("DOC-029r2 portal: Northstar proposes a 60-day evaluation. Counterparty Northstar Evaluation Ltd. Please review the evaluation terms.");
  ...
  await step(A, R, "Open request and check saved answers and attachments", "The Request shows the title, R- reference, description, Department, Urgency, answer, and both files with their original bytes", async () => {
    ...
    for (const want of ["Northstar proposes a 60-day evaluation", "Procurement", "High", "Northstar Evaluation Ltd", chosen, dropped]) expect(body.includes(want), `missing ${want} in: ${body}`);
    const got = [];
  ...
  await step(A, R, "Check Your requests on the Portal home", "The new Request appears once under Your requests", async () => {
    ...
    await rows.first().waitFor({ timeout: 20000 }).catch(() => {});
    const count = await rows.count();
    expect(count === 1, `rows ${count}`);
    return `Your requests: "${await text(rows.first())}" (1 row).`;
  });

  await step(A, "legal_team_member", "Legal receives the Request to triage; no Contract or Matter exists until conversion", "Staff detail shows the Request as new with no converted record", async () => {
    ...
    expect(r.status === "new" && !r.convertedRecord, `status ${r.status} converted ${JSON.stringify(r.convertedRecord)}`);
  ...
  await step(A, "legal_team_member", "Department: a Requester without a listed Department must choose one", "Department starts empty and submission names Department as missing", async () => {
    ...
    await staff.getByLabel(/^Title/).fill("DOC-029r2 portal department check (not submitted)");
  ...
  await step(A, R, "An attachment fails: the Request still exists and the failure is named", "Confirmation reports the named file did not attach and keeps the R- reference; the Request is saved and the file can go on a reply", async () => {
    await page.goto(`${L.BASE}/portal/new/legal_question`);
    await page.getByLabel(/^Title/).fill(`DOC-029r2 portal attachment failure ${runStamp}`);
    await page.getByLabel(/^Description/).fill("DOC-029r2 portal: checking an attachment failure.");
    const [chooser] = await Promise.all([page.waitForEvent("filechooser"), page.getByRole("button", { name: "Choose files" }).click()]);
    ...
    await panel.getByRole("textbox", { name: "New comment" }).fill(`DOC-029r2 portal: the file that did not attach to R-${failedNumber}.`);
    const [fc] = await Promise.all([page.waitForEvent("filechooser"), panel.getByRole("button", { name: "Attach files" }).click()]);
    await fc.setFiles([await pdfFile(failedName)]);
    await panel.getByRole("button", { name: "Comment", exact: true }).click();
    await panel.getByText(`the file that did not attach to R-${failedNumber}`).waitFor({ timeout: 20000 });
  ...
  await step(A, R, "Legal decides the Request while its files upload", "Confirmation links to a reply on the same Request; the link opens its composer; the submission stays saved", async () => {
    const { page: nadia } = await L.staffContext(L.PEOPLE.nadia);
    const raceName = `doc029-race-${runStamp}.pdf`;
    await page.goto(`${L.BASE}/portal/new/legal_question`);
    await page.getByLabel(/^Title/).fill(`DOC-029r2 portal decided during upload ${runStamp}`);
    await page.getByLabel(/^Description/).fill("DOC-029r2 portal: Legal declines while the file uploads.");
    const [chooser] = await Promise.all([page.waitForEvent("filechooser"), page.getByRole("button", { name: "Choose files" }).click()]);
    await chooser.setFiles([await pdfFile(raceName)]);
    await page.route("**/api/v1/requests/*/attachments", async (route) => {
      const n = Number(new URL(route.request().url()).pathname.split("/")[4]);
      must(await L.api(nadia, "POST", `/requests/${n}/decline`, { reason: "DOC-029r2 portal: declined during upload." }), "decline");
      await route.continue();
    });
    await page.getByRole("button", { name: "Submit request" }).click();
    const heading = page.getByRole("heading", { name: /^Request R-\d+ is with Legal$/ });
    ...
    await panel.getByRole("textbox", { name: "New comment" }).fill(`DOC-029r2 portal: the file that did not attach to R-${n}.`);
    const [fc] = await Promise.all([page.waitForEvent("filechooser"), panel.getByRole("button", { name: "Attach files" }).click()]);
    ...
    const card = await text(page.getByRole("region", { name: "What you submitted" }));
    expect(composerVisible && pill === "Declined" && card.includes("Legal declines while the file uploads"), `composer ${composerVisible} pill ${pill}`);
    await nadia.context().close();
  ...
  await step(A, R, "A form address that does not exist returns to the Portal home", "Unknown form slug redirects to /portal", async () => {
  ...
  await step(A, R, "Submission reports an error without confirmation; Your requests shows no new Request", "Error text shows, answers stay, and no Request was saved", async () => {
    await page.goto(`${L.BASE}/portal/new/legal_question`);
    const t = `DOC-029r2 portal network failure ${runStamp}`;
    await page.getByLabel(/^Title/).fill(t);
    await page.getByLabel(/^Description/).fill("DOC-029r2 portal: the submission does not reach the server.");
    await page.route("**/api/v1/requests", (route) => (route.request().method() === "POST" ? route.abort("failed") : route.continue()));
    await page.getByRole("button", { name: "Submit request" }).click();
    ...
    const mine = must(await L.api(page, "GET", "/portal/requests"), "list").requests.filter((r) => r.title === t).length;
    expect(/could not be submitted/.test(said) && kept === t && mine === 0, `${said} kept=${kept} saved=${mine}`);
...
async function followPhase() {
  ...
  await step(A, R, "Setup: Jonas has Requests to follow (created through the API; the form itself is V-C09)", "Six Requests with one original PDF each", async () => {
    ...
    const engineering = must(await L.api(page, "GET", "/portal/request-types/contract_review"), "form").departments.find((d) => d.displayName === "Engineering");
    const plan = [["open", "contract_review", { counterparty_name: "Northstar Evaluation Ltd" }], ["contract", "contract_review", { counterparty_name: "Northstar Evaluation Ltd" }], ["matter", "employment_question", { country: "Germany" }], ["resolved", "legal_question", {}], ["declined", "legal_question", {}], ["older", "legal_question", {}]];
    for (const [key, slug, customFields] of plan) {
      const title = `DOC-029r2 portal follow ${key} ${runStamp}`;
      const r = must(await L.api(page, "POST", "/requests", { requestTypeId: bySlug[slug].id, departmentId: engineering.id, title, description: `DOC-029r2 portal description for ${key}.`, urgency: "high", customFields }), `request ${key}`).request;
      const original = `doc029-original-${key}-${runStamp}.pdf`;
      must(await L.api(page, "POST", `/requests/${r.number}/attachments`, undefined, { file: await pdfFile(original) }), `attach ${key}`);
      Q[key] = { number: r.number, id: r.id, title, slug, original };
    }
    return Object.entries(Q).map(([k, q]) => `${k}: R-${q.number}`).join("; ");
  });
  ...
  const post = async (entityType, entityId, visibility, body, filename) => {
    ...
    return must(await L.api(nadia, "POST", "/comments", undefined, multipart), `comment ${visibility}`);
  };

  await step(A, "legal_team_member", "Setup: Legal replies at every audience before any decision, and assigns the open Request", "Full thread (with paper), Legal only, and Working team comments post on each Request; R-open is assigned to Nadia", async () => {
    const out = [];
    for (const key of ["open", "contract", "matter", "resolved", "declined"]) {
      const q = Q[key];
      const tag = `${key} ${runStamp}`;
      legal[key] = { full: `DOC-029r2 portal Legal full thread ${tag}`, legalOnly: `DOC-029r2 portal Legal only note ${tag}`, working: `DOC-029r2 portal Working team note ${tag}`, file: `doc029-legal-${key}-${runStamp}.pdf` };
      await post("request", q.id, "full_thread", legal[key].full, legal[key].file);
      await post("request", q.id, "legal_only", legal[key].legalOnly);
      const w = await L.api(nadia, "POST", "/comments", undefined, { entityType: "request", entityId: q.id, body: legal[key].working, visibility: "working_team" });
      out.push(`R-${q.number}: full+file, legal_only, working_team HTTP ${w.status}`);
    }
    must(await L.api(nadia, "PATCH", `/requests/${Q.open.number}/assignee`, { assigneeId: nadiaMe.id }), "assign");
    return out.join("; ") + `; R-${Q.open.number} assigned to ${nadiaMe.displayName}.`;
  });

  await step(A, "legal_team_member", "Setup: Legal converts, resolves, and declines through the API", "Contract and Matter conversions, a resolution with a reply, and a decline with a reason succeed", async () => {
    ...
    const ctype = (ctypes.contractTypes ?? ctypes.types ?? Object.values(ctypes)[0]).find((t) => !t.archivedAt);
    const c = must(await L.api(nadia, "POST", `/requests/${Q.contract.number}/convert`, { title: Q.contract.title, contractTypeId: ctype.id }), "convert contract").request;
    const m = must(await L.api(nadia, "POST", `/requests/${Q.matter.number}/convert`, { title: Q.matter.title }), "convert matter").request;
    const cr = must(await L.api(nadia, "GET", `/contracts/${c.convertedRecord.number}`), "contract read");
    const mr = must(await L.api(nadia, "GET", `/matters/${m.convertedRecord.number}`), "matter read");
    ...
    must(await L.api(nadia, "POST", `/requests/${Q.resolved.number}/resolve`, { reply: `DOC-029r2 portal resolution reply ${runStamp}` }), "resolve");
    must(await L.api(nadia, "POST", `/requests/${Q.declined.number}/decline`, { reason: `DOC-029r2 portal decline reason ${runStamp}` }), "decline");
  ...
  await step(A, "legal_team_member", "Setup: Legal posts later replies with paper after the decisions, and 55 extra replies on R-older", "Later Full thread replies and Legal only notes post on the Requests and the converted records", async () => {
    const targets = [
    ...
    for (const [key, type, id] of targets) {
      legal[key].later = `DOC-029r2 portal Legal later reply ${key} ${runStamp}`;
      legal[key].laterFile = `doc029-legal-later-${key}-${runStamp}.pdf`;
      legal[key].laterNote = `DOC-029r2 portal Legal later legal-only ${key} ${runStamp}`;
      await post(type, id, "full_thread", legal[key].later, legal[key].laterFile);
      await post(type, id, "legal_only", legal[key].laterNote);
    }
    for (let i = 1; i <= 55; i++) await post("request", Q.older.id, "full_thread", `DOC-029r2 portal older marker ${String(i).padStart(2, "0")} ${runStamp}`);
    return `Later replies posted on ${targets.map((t) => `${t[1]} ${t[0]}`).join(", ")}; 55 Full thread markers on R-${Q.older.number}.`;
  });

  await step(A, R, "Your requests lists open, resolved, and declined Requests with Owner; converted ones leave the list", "Rows for R-open, R-resolved, R-declined with Owner text; no rows for the converted Requests", async () => {
    await page.goto(`${L.BASE}/portal`);
    const list = page.getByRole("region", { name: "Your requests" });
    await list.waitFor();
    const rows = {};
    ...
    expect(rows.open && rows.resolved && rows.declined && !rows.contract && !rows.matter, JSON.stringify(rows));
  ...
  for (const key of ["open", "resolved", "declined"]) {
    const q = Q[key];
    await step(A, R, `R-${key}: open from Your requests; check title, reference, status, Owner, and banner`, "Title, R- reference, requester status (Open/Resolved/Declined), Owner line, and banner text; decline reason under the status; reply link on decided Requests only", async () => {
      await page.goto(`${L.BASE}/portal`);
      await page.getByRole("region", { name: "Your requests" }).getByRole("link", { name: new RegExp(q.title) }).click();
      ...
      const replyLink = await main.getByRole("link", { name: "Attach new files to a reply" }).count();
      const want = { open: "Open", resolved: "Resolved", declined: "Declined" }[key];
      expect(pill === want, `pill ${pill}`);
      if (key === "declined") expect(banner.includes(`DOC-029r2 portal decline reason ${runStamp}`), banner);
    ...
    await step(A, R, `R-${key}: Comments shows Legal's shared replies and paper only`, "Full thread replies and their files show; Legal only and Working team notes do not; no audience choice", async () => {
      const panel = await openComments(page);
      const L_ = legal[key];
      await panel.getByText(L_.later).waitFor({ timeout: 20000 }).catch(() => {});
      const body = await text(panel);
      expect(body.includes(L_.full) && body.includes(L_.later), `full thread missing: ${body.slice(0, 300)}`);
      if (key === "resolved") expect(body.includes(`DOC-029r2 portal resolution reply ${runStamp}`), "resolution reply missing");
      expect(!body.includes(L_.legalOnly) && !body.includes(L_.working) && !body.includes(L_.laterNote), "restricted note visible");
      const radios = await panel.getByRole("radio").count();
      const audience = await panel.getByText("Audience", { exact: true }).count();
      const d = await downloadVia(page, panel.getByRole("link", { name: new RegExp(L_.laterFile) }).or(panel.getByRole("button", { name: new RegExp(`Download ${L_.laterFile}`) })).first());
      expect(sha(d.bytes) === sha((await pdfFile(L_.laterFile)).buffer), "Legal file bytes differ");
    ...
    await step(A, R, `R-${key}: What you submitted shows the saved answers and the original attachment`, "Description, original attachment (downloadable, same bytes), Department, Urgency, and answered questions", async () => {
      const card = page.getByRole("region", { name: "What you submitted" });
      const body = await text(card);
      for (const want of [`DOC-029r2 portal description for ${key}.`, q.original, "Engineering", "High"]) expect(body.includes(want), `missing ${want}: ${body}`);
    ...
    await step(A, R, `R-${key}: reply with further paper`, "Comment stays unavailable without a message (also with a file selected); the reply and file appear; the status does not change; Legal reads it", async () => {
      if (key !== "open") {
      ...
      const msg = `DOC-029r2 portal Jonas reply ${key} ${runStamp}`;
      ...
      const d = await downloadVia(page, panel.getByRole("link", { name: new RegExp(replyFile) }).or(panel.getByRole("button", { name: new RegExp(`Download ${replyFile}`) })).first());
      await page.reload();
      await page.getByRole("heading", { level: 1, name: q.title }).waitFor();
      const pill = await text(page.getByRole("main").locator("h1").locator("xpath=following-sibling::span[1]"));
      const panel2 = await openComments(page);
      await panel2.getByText(msg, { exact: true }).first().waitFor({ timeout: 20000 }).catch(() => {});
      const count = await panel2.getByText(msg, { exact: true }).count();
      const staff = must(await L.api(nadia, "GET", `/comments?entityType=request&entityId=${q.id}`), "staff comments").comments.filter((c) => c.body === msg);
      expect(emptyDisabled && fileOnlyDisabled && enabled, `disabled ${emptyDisabled}/${fileOnlyDisabled} enabled ${enabled}`);
      expect(count === 1 && staff.length === 1 && (staff[0].attachments ?? []).some((a) => a.filename === replyFile), `count ${count} staff ${JSON.stringify(staff).slice(0, 200)}`);
      expect(sha(d.bytes) === sha((await pdfFile(replyFile)).buffer), "reply file bytes differ");
      expect(pill === { open: "Open", resolved: "Resolved", declined: "Declined" }[key], `pill ${pill}`);
  ...
  for (const key of ["contract", "matter"]) {
    const q = Q[key];
    const rec = STATE[key];
    await step(A, R, `R-${key}: the converted Request address opens the resulting ${key}`, `Redirect to /portal/${key}s/<n>; Original request shows the ask; Jonas is Business Owner on the team; Comments shows Full thread replies only; a reply posts on the record`, async () => {
      ...
      const orig = await text(main.getByText("Original request").first().locator("xpath=ancestor::section[1]"));
      expect(orig.includes(`DOC-029r2 portal description for ${key}.`), `original: ${orig.slice(0, 300)}`);
      const teamLabel = key === "contract" ? "Contract team" : "Matter team";
      await page.getByRole("toolbar", { name: "Applets" }).getByRole("button", { name: teamLabel }).click();
      ...
      await panel.getByText(L_.later).waitFor({ timeout: 20000 }).catch(() => {});
      const body = await text(panel);
      expect(body.includes(L_.full) && body.includes(L_.later), `full missing: ${body.slice(0, 300)}`);
      expect(!body.includes(L_.legalOnly) && !body.includes(L_.laterNote) && !body.includes(L_.working), "restricted note visible");
      const msg = `DOC-029r2 portal Jonas reply on ${key} ${runStamp}`;
      await panel.getByRole("textbox", { name: "New comment" }).fill(msg);
      await panel.getByRole("button", { name: "Comment", exact: true }).click();
      await panel.getByText(msg).waitFor({ timeout: 20000 });
  ...
  await step(A, R, "Show older loads earlier replies", "The conversation first shows the newest page; Show older adds the earlier replies", async () => {
    await page.goto(`${L.BASE}/portal/requests/${Q.older.number}`);
    const panel = await openComments(page);
    await panel.getByText(`DOC-029r2 portal older marker 55 ${runStamp}`).waitFor({ timeout: 20000 });
    const before = await panel.getByText(new RegExp(`^DOC-029r2 portal older marker \\d\\d ${runStamp}$`)).count();
    const hadFirst = await panel.getByText(`DOC-029r2 portal older marker 01 ${runStamp}`).count();
    await panel.getByRole("button", { name: "Show older" }).click();
    await panel.getByText(`DOC-029r2 portal older marker 01 ${runStamp}`).waitFor({ timeout: 20000 });
    const after = await panel.getByText(new RegExp(`^DOC-029r2 portal older marker \\d\\d ${runStamp}$`)).count();
    expect(before < 55 && hadFirst === 0 && after === 55, `before ${before} first ${hadFirst} after ${after}`);
    return `R-${Q.older.number}: ${before} markers before Show older (marker 01 absent), ${after} after.`;
  });

  await step(A, R, "Recovery: a failed reply keeps its text and files; Comment again posts once", "Error shows, text and file stay; retry posts one reply", async () => {
    await page.goto(`${L.BASE}/portal/requests/${Q.open.number}`);
    const panel = await openComments(page);
    const msg = `DOC-029r2 portal retry reply ${runStamp}`;
    const f = `doc029-retry-${runStamp}.pdf`;
    await panel.getByRole("textbox", { name: "New comment" }).fill(msg);
    const [fc] = await Promise.all([page.waitForEvent("filechooser"), panel.getByRole("button", { name: "Attach files" }).click()]);
    await fc.setFiles([await pdfFile(f)]);
    await page.route("**/api/v1/comments", (route) => (route.request().method() === "POST" ? route.abort("failed") : route.continue()));
    await panel.getByRole("button", { name: "Comment", exact: true }).click();
    ...
    const staff = must(await L.api(nadia, "GET", `/comments?entityType=request&entityId=${Q.open.id}`), "staff").comments.filter((c) => c.body === msg);
    expect(kept === msg && fileKept > 0 && staff.length === 1, `kept ${kept} file ${fileKept} posted ${staff.length}`);
    return `Error "${err}"; text kept, ${f} kept; retry posted ${staff.length} reply.`;
  });

  await step(A, R, "Recovery: the conversation could not be read; close and reopen Comments", "Error text shows; reopening the panel reads the conversation", async () => {
  ...
  await step(A, R, "Clearing triage assignment shows Not assigned yet", "Request page and Your requests show Not assigned yet", async () => {
    must(await L.api(nadia, "PATCH", `/requests/${Q.open.number}/assignee`, { assigneeId: null }), "unassign");
    await page.goto(`${L.BASE}/portal/requests/${Q.open.number}`);
    const own = await text(page.getByRole("main").getByText(/^Owner: /));
    await page.goto(`${L.BASE}/portal`);
    const row = await text(page.getByRole("region", { name: "Your requests" }).getByRole("link", { name: new RegExp(Q.open.title) }));
    expect(own === "Owner: Not assigned yet" && row.includes("Owner: Not assigned yet"), `${own} / ${row}`);
    return `Request page "${own}"; list row "${row}".`;
  });

  await step("follow-request", "business_user", "Negative: a second Business User cannot open Jonas's Requests or files", "Amara's browser returns to /portal; API answers 404 for the Request and its attachment; converted addresses also return to /portal", async () => {
    ...
    const a2 = await amara.request.get(`${L.BASE}/api/v1/portal/requests/${Q.open.number}/attachments/${att.id}`);
    await amara.goto(`${L.BASE}/portal/contracts/${STATE.contract.number}`);
    await amara.waitForURL((u) => u.pathname !== `/portal/contracts/${STATE.contract.number}`, { timeout: 15000 }).catch(() => {});
  ...
  await step(A, R, "Negative: staff addresses are not a substitute for the Portal link", "Jonas's staff API reads are refused and staff record pages do not open", async () => {
  ...
  await step(A, R, "Team row removed: the old Request link no longer opens the live record", "After Legal removes Jonas from the Matter team, the Request address returns to the Portal home", async () => {
    ...
    if (del.status >= 300) throw new Error(`team removal refused: HTTP ${del.status} ${JSON.stringify(del.body).slice(0, 200)}`);
  ...
  await step(A, R, "Archived destination: the Request address shows the original ask and an archived notice", "Notice text, original ask, no Comments applet and no attachment links", async () => {
    const arch = await L.api(nadia, "POST", `/contracts/${STATE.contract.number}/archive`, {});
    if (arch.status >= 300) throw new Error(`archive refused: HTTP ${arch.status} ${JSON.stringify(arch.body).slice(0, 200)}`);
    await page.goto(`${L.BASE}/portal/requests/${Q.contract.number}`);
    await page.getByRole("heading", { level: 1, name: Q.contract.title }).waitFor({ timeout: 20000 });
    const main = page.getByRole("main");
    const notice = await text(main.getByText(/was archived/));
    const card = await text(main.getByRole("region", { name: "What you submitted" }));
    const commentsBtn = await page.getByRole("toolbar", { name: "Applets" }).getByRole("button", { name: "Comments" }).count();
    const fileLinks = await main.getByRole("region", { name: "What you submitted" }).getByRole("link").count();
...
async function knowledgePhase() {
  ...
  await step(A, "legal_team_member", "Setup: the DOC-029 guidance fixture is published for Everyone", "The staff item reads published, Everyone, not archived, with two Documents and a primary", async () => {
    let staff = must(await L.api(nadia, "GET", `/knowledge/${K.guidance.id}`), "staff item").knowledgeItem;
    let republished = false;
    if (staff.state !== "published") {
      must(await L.api(nadia, "POST", `/knowledge/${K.guidance.id}/publish`, {}), "publish");
      republished = true;
      staff = must(await L.api(nadia, "GET", `/knowledge/${K.guidance.id}`), "staff item").knowledgeItem;
    }
    expect(staff.state === "published" && staff.audience === "everyone" && !staff.archivedAt && staff.documentCount === 2 && staff.primaryDocument, JSON.stringify(staff).slice(0, 300));
  ...
  await step(A, R, "Portal home: Before you submit lists only eligible guidance", "Knowledge links point at published Everyone items; external links show their website", async () => {
    await page.goto(`${L.BASE}/portal`);
    const panel = page.getByRole("region", { name: "Before you submit" });
    await panel.waitFor();
    homeLinks = await panel.getByRole("link").evaluateAll((as) => as.map((a) => ({ label: a.childNodes[0]?.textContent?.trim(), href: a.getAttribute("href"), target: a.getAttribute("target"), domain: a.parentElement?.querySelector("span[aria-hidden]")?.textContent ?? null })));
    const listed = must(await L.api(nadia, "GET", "/knowledge?limit=100"), "knowledge").knowledgeItems;
    for (const l of homeLinks.filter((l) => l.href.startsWith("/portal/knowledge/"))) {
      const item = listed.find((k) => k.id === l.href.split("/").at(-1));
      expect(item && item.state === "published" && item.audience === "everyone" && !item.archivedAt, `ineligible link ${l.href}`);
    }
    for (const bad of [K.restricted, K.draft, K.archived]) expect(!homeLinks.some((l) => l.href.includes(bad.id)), "restricted fixture linked");
  ...
  await step(A, R, "A request form shows its own Before you submit links, which can differ from the home", "The form's panel differs from the home panel", async () => {
  ...
  await step(A, R, "Open guidance from Before you submit: same tab, From Legal, title, Guidance, Your requests", "Same-tab navigation to /portal/knowledge/<id>; From Legal above the title; Guidance text; Your requests returns home", async () => {
    await page.goto(`${L.BASE}/portal`);
    const pages = page.context().pages().length;
    const first = homeLinks.find((l) => l.href.startsWith("/portal/knowledge/"));
    await page.getByRole("region", { name: "Before you submit" }).getByRole("link", { name: first.label }).click();
    ...
    const gtext = guidance ? (await text(main.getByRole("region", { name: "Guidance" }))).slice(0, 120) : "";
  ...
  let primaryUrl = null;
  await step(A, R, "A shared Portal Knowledge link: Documents lists the primary first; Download gives the current Version; Guidance shows", "Primary Document first (uploaded after the supporting one); Download bytes equal staff current Version; no version picker or reader", async () => {
    ...
    const clean = names.map((n) => n.replace(/\s+/g, " ").replace(/Download$/, "").trim());
    const item = must(await L.api(page, "GET", `/portal/knowledge/${K.guidance.id}`), "portal item").knowledgeItem;
    const staffItem = must(await L.api(nadia, "GET", `/knowledge/${K.guidance.id}`), "staff item").knowledgeItem;
    const supportingFirstUploaded = item.documents.length === 2;
    expect(supportingFirstUploaded && item.documents[0].id === staffItem.primaryDocument.id && clean[0] === staffItem.primaryDocument.currentVersion.originalFilename, `order ${clean} primary ${staffItem.primaryDocument?.id}`);
    const results = [];
    for (const d of item.documents) {
      ...
      const staffDocs = must(await L.api(nadia, "GET", `/knowledge/${K.guidance.id}/documents`), "staff docs");
      const doc = (staffDocs.documents ?? []).find((x) => x.id === d.id);
      const versions = [...(doc?.versions ?? [])].sort((a, b) => (a.number ?? a.versionNumber) - (b.number ?? b.versionNumber));
      const cur = doc?.currentVersion ?? versions.at(-1);
      if (!cur) throw new Error(`staff version chain not read: ${JSON.stringify(staffDocs).slice(0, 300)}`);
      const staffBytes = await nadia.request.get(`${L.BASE}/api/v1/documents/${d.id}/versions/${cur.id}/download`).then((r) => r.body());
      expect(sha(dl.bytes) === sha(staffBytes), `bytes differ for ${dl.filename}`);
      results.push(`${dl.filename} matched staff current Version ${cur.number ?? cur.versionNumber ?? ""}`.trim());
    }
    ...
    expect(combos === 0 && guidance.includes("Send the latest draft"), `combos ${combos} guidance ${guidance}`);
    return `Documents order: ${clean.join(" | ")}; ${results.join("; ")}; buttons in main: [${buttons.join(", ")}]; version pickers ${combos}; Guidance "${guidance}".`;
  });

  await step(A, R, "A new Version: Download gives the new current file", "After Legal adds a Version, reloading shows the new filename and Download serves the new bytes", async () => {
    const item = must(await L.api(page, "GET", `/portal/knowledge/${K.guidance.id}`), "item").knowledgeItem;
    const primary = item.documents[0];
    const vname = `doc029-primary-v2-${runStamp}.pdf`;
    const up = await L.api(nadia, "POST", `/documents/${primary.id}/versions`, undefined, { file: await pdfFile(vname) });
    if (up.status >= 300) throw new Error(`version upload HTTP ${up.status} ${JSON.stringify(up.body).slice(0, 200)}`);
    await page.reload();
    ...
    expect(sha(dl.bytes) === sha((await pdfFile(vname)).buffer) && sha(saved) === sha(dl.bytes), "new bytes not served");
  ...
  await step(A, R, "Draft, restricted, archived, and unknown items and their files do not open", "Each page returns to /portal; item and file APIs answer 404", async () => {
    const out = [];
    const seedDraft = must(await L.api(nadia, "GET", "/knowledge?limit=100&state=draft"), "drafts").knowledgeItems.find((k) => k.audience === "everyone" && !k.title.startsWith("DOC-029"));
    const cases = [["draft", K.draft.id], ["restricted", K.restricted.id], ["archived", K.archived.id], ["unknown", "01a0aaad-0000-7000-8000-000000000000"]];
    ...
    for (const [label, id] of cases) {
      ...
      if (label !== "unknown" && label !== "seed draft Everyone") {
        const staff = must(await L.api(nadia, "GET", `/knowledge/${id}`), "staff item").knowledgeItem;
        const docId = staff.primaryDocument?.id;
        if (docId) fileStatus = (await page.request.get(`${L.BASE}/api/v1/portal/knowledge/${id}/documents/${docId}/download`)).status();
  ...
  await step(A, R, "A saved download address follows the item's availability", "After Legal unpublishes, the page returns home and the saved address answers 404; republishing restores it", async () => {
    primaryUrl ??= must(await L.api(page, "GET", `/portal/knowledge/${K.guidance.id}`), "item").knowledgeItem.documents[0].currentVersion.downloadUrl;
  ...
  await step(A, R, "A broken external link opens in a new tab and does not block the Request form", "The external link opens a new tab; the original tab stays on the Portal; a Request can still be submitted", async () => {
    await page.goto(`${L.BASE}/portal`);
    const ext = homeLinks.find((l) => l.target === "_blank");
    const [popup] = await Promise.all([page.context().waitForEvent("page"), page.getByRole("region", { name: "Before you submit" }).getByRole("link", { name: new RegExp(ext.label) }).click()]);
    await popup.waitForLoadState("domcontentloaded").catch(() => {});
    const popupUrl = popup.url();
    const popupFailed = await popup.evaluate(() => document.body?.innerText?.slice(0, 80) ?? "").catch((e) => `error: ${e.message}`);
    await popup.close();
    ...
    await page.getByLabel(/^Title/).fill(`DOC-029r2 portal after a broken link ${runStamp}`);
    await page.getByLabel(/^Description/).fill("DOC-029r2 portal: the external website did not load.");
    await page.getByRole("button", { name: "Submit request" }).click();
  ...
  await step(A, R, "The Portal has no staff Knowledge library or editing controls", "Portal navigation offers Requests, Contracts, Matters, Auto-Docs; the staff Knowledge address does not open for a Business User", async () => {
    await page.goto(`${L.BASE}/portal`);
    await page.getByRole("navigation", { name: "Portal" }).getByRole("link").first().waitFor();
    const nav = await page.getByRole("navigation", { name: "Portal" }).getByRole("link").evaluateAll((as) => as.map((a) => a.textContent.trim()));
    await page.goto(`${L.BASE}/knowledge`);
    await page.waitForTimeout(2500);
    const where = new URL(page.url()).pathname;
    const api = await L.api(page, "GET", "/knowledge");
    expect(!nav.some((n) => /Knowledge/i.test(n)) && where.startsWith("/portal") && api.status >= 400, `${nav} ${where} ${api.status}`);
    await page.goto(`${L.BASE}/portal/knowledge/${K.guidance.id}`);
    const editing = await page.getByRole("main").getByRole("button", { name: /Edit|Publish|Archive|Upload/ }).count();
  ...
  await step(A, "anonymous", "Signed out: Everyone guidance is not an anonymous public page", "A browser without a session goes to the Portal sign-in; item and file APIs answer 401", async () => {
  ...
  await step(A, R, "Help search does not search organization Knowledge", "Searching Help for the guidance title finds no organization Knowledge Item", async () => {
...
async function recheckPhase() {
  ...
  const { page } = await L.portalContext(L.PEOPLE.jonas);
  await step("submit-request", "business_user", "Recheck: Check Your requests on the Portal home (after the harness timing fault)", "The Request appears once under Your requests", async () => {
...
async function supplementPhase() {
  ...
  const { page } = await L.portalContext(L.PEOPLE.jonas);
  await step("follow-request", "business_user", "Supplement: Attach new files to a reply opens the Comments composer by itself", "Selecting the banner link on a Declined Request opens Comments with New comment visible, without using the applet bar", async () => {
    await page.goto(`${L.BASE}/portal/requests/${n}`);
    await page.getByRole("link", { name: "Attach new files to a reply" }).click();
  ...
  await step("follow-request", "business_user", "Supplement: selecting a conversation attachment", "The attachment can be downloaded from the conversation", async () => {
    const panel = page.getByRole("complementary", { name: "Comments" });
    await panel.getByRole("list", { name: "Comments" }).waitFor();
    let direct = null;
    const onDl = (d) => (direct = d.suggestedFilename());
    page.on("download", onDl);
    const nameButton = panel.getByRole("button", { name: /^doc029-jonas-reply-declined-/ }).first();
