...
### Built addendum (2026-08-30, M28/4, [#598](https://github.com/juggernog20/OpenLaw/issues/598)) — Knowledge Item is the fourth live owner arm
...
Scope filters the existing matching vectors before ranking. Titles and numbers select weight A, record text selects B and C including query-time joins, and Document contents selects D for Documents only. The exact total and cursor page read the same reached, unarchived hits in one SQL statement. The endpoint accepts `timeZone` through `TimezoneSchema`. ~~Conditions and sorts other than relevance are refused until their compiler tickets land.~~ Superseded by the M44 close addendum below.

### Addendum (2026-09-25, M44 close, [#1100](https://github.com/juggernog20/OpenLaw/issues/1100)): the complete question and relative calendar dates

1. The version 1 question is `{ version, words: { all, phrase, any, none }, scope: { titles, text, contents }, kinds, conditions, match, sort }`. Each condition carries `{ kind, property, operator, value }`, with value omitted for valueless operators. `match` is `all` or `any`; sort is `relevance`, `newest`, `oldest`, `expiry` or `title`. One shared schema validates the endpoint, URL and saved-search config. Each words row allows 200 characters, conditions stop at 20, and a condition must name a selected kind. Empty questions and words without a checked scope are refused.
2. Scope keeps the M25 weights. Titles and numbers select A, record text selects B and C including the query-time joins, and Document contents selects D for Documents only. All three start checked. Field values are not words in these vectors; CTR-016 makes them conditions. Words compile through the existing English full-text parser, including phrase, alternatives and exclusions.
3. Conditions run inside each kind's reached candidate read, before ranking, exact count and keyset paging. Match any ORs conditions within that kind; conditions on different kinds never interact. As built, a Show flag is an ordinary condition: under Match any, `Show archived = Yes` is an OR arm that matches every reached record of that kind. ANDing Show flags regardless of the switch is a possible product follow-up, not the current rule. DD-014 and ENT-004 remain ahead of every result and total, and Business Users receive no search results.
4. Relative dates resolve at each request, never when a question is saved. The web sends the DES-014 display timezone. The server uses request `timeZone`, then the viewer's profile timezone, then UTC. It resolves inclusive calendar-date ranges, not elapsed hours. Weeks run Monday to Sunday. Today, this week, this month, this quarter and this year cover their calendar periods. In the last or next N days includes today and the day N days away; N is an integer from 1 to 3650. Thus the last 90 days covers 91 calendar days. Date Fields inherit these rules.
5. `aq` holds base64url JSON and takes precedence over legacy `q` and `kind`; a simple `q` becomes `words.all`. A URL or saved search reads past removed properties and unavailable Fields, keeping the rest if it remains runnable. The dialog announces dropped conditions; the results page currently announces dropped Fields only. Submitting the header box on a results page changes `words.all` and preserves the rest of the question.
6. The preview uses the same endpoint with limit 10, after 300 ms, and discards stale responses. The endpoint returns `{ results, total, nextCursor }`. All five sorts now run. Expiry puts Contracts first by expiry ascending with nulls last; other kinds follow by relevance. Each sort has a stable row tie-breaker. No migration or index is added, so the upgrade gate remains unchanged and still checks existing records and extracted text.
