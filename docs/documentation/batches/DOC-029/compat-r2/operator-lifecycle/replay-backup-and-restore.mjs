...
const LC =
  "/tmp/claude-1000/-home-blairwentworth-projects-OpenLaw/7de884dd-eedd-4464-96ea-cc0709f7fe19/scratchpad/lc-restore";
...
const PROJECTS = {
  upgrade: {
    ...
    port: 23330,
    subnets: ["10.239.50.0/24", "10.239.51.0/24"],
  },
  recovery: {
    project: `${PREFIX}-recovery`,
    port: 23331,
    subnets: ["10.239.52.0/24", "10.239.53.0/24"],
  },
  restore: {
    project: `${PREFIX}-restore`,
    port: 23332,
    subnets: ["10.239.54.0/24", "10.239.55.0/24"],
  },
};
...
const log = readJson(LOG, {
  ...
  baseline: BASELINE,
  script:
    "docs/documentation/batches/DOC-029/compat-r2/operator-lifecycle/replay-backup-and-restore.mjs",
...
phases["populate"] = async () => {
  ...
  const people = {
    admin: {
      email: "morgan.baseline.r2@lifecycle.example",
      displayName: "DOC-029r2 operator-lifecycle Morgan Baseline",
    },
    reader: {
      email: "rowan.reader.r2@lifecycle.example",
      displayName: "DOC-029r2 operator-lifecycle Rowan Reader",
      role: "legal_team_member",
    },
    contributor: {
      email: "casey.contributor.r2@lifecycle.example",
  ...
  await step(
    ...
    async () => {
      ...
      const write = await api.raw("PATCH", `/api/v1/contracts/${c.number}`, {
        json: {
          description: `DOC-029r2 operator-lifecycle write check ${new Date().toISOString()}`,
        },
      });
      check(
...
phases["diag-worker"] = async () => {
  ...
  await step(
    ...
    async () => {
      ...
      const api = await adminApi(p);
      const body = pdf([
        "DOC-029r2 operator-lifecycle worker-stopped upload.",
  ...
  await step(
    ...
    async () => {
      ...
      const api = await adminApi(p);
      const body = pdf([
        "DOC-029r2 operator-lifecycle engine-down upload.",
      ...
      await sleep(15_000);
      const body2 = pdf([
        "DOC-029r2 operator-lifecycle engine-restored upload.",
