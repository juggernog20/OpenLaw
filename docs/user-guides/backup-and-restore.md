...
| The app log says **No configured key opens these stored credentials**                        | The target has a different `OPENLAW_SECRET_KEY` from the source. The line names each column the key cannot open, such as `advanced_settings`, `smtp_url` or `vapid_private_key`. The app and worker still start, and each of those values reads as empty. Stop app and worker, supply the retained key, and recreate them. Until then, do not save in **Settings → Advanced**, run the saved-settings recovery command, or paste credentials again: each writes over a value the retained key can still open. |
| Fields in **Settings → Advanced** show **Default** instead of the values saved on the source | The target's `OPENLAW_SECRET_KEY` cannot open the saved Advanced configuration, so the app uses the deployment environment or the defaults. Storage falls back to the local driver, and Documents in a store that only the saved values named fail to download. The instance address falls back to `BASE_URL`, or `http://localhost:3000`. Supply the retained key and recreate both services.                                                                                                                |
