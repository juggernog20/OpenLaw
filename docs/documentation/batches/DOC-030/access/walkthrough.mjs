...
const results = {
  ...
  lab: {
    ...
    runningImages: {
      app: image("app"),
      worker: image("worker"),
      "doc-engine": image("doc-engine"),
    },
    seed: LAB.seed,
  },
  browser:
    "Playwright 1.63.0 Chromium, headless, 1440x900, one isolated browser context per person",
...
async function signin() {
  ...
  await step(
    ...
    BU,
    "fixture.expiry-link",
    "Request a sign-in link now for a later check after five minutes.",
  ...
  await step(
    ...
    async () => {
      ...
      return {
        page: "/portal/login",
        actual: `Password form shown ${pw}; Continue with single sign-on ${sso > 0}. Email me a sign-in link changed the card title to Get a sign-in link.`,
      };
    },
  );
  let sent = 0;
  await step(
    ...
    async () => {
      ...
      return {
        page: "/portal/login",
        actual: `Check your email appeared: "${body.replace(newEmail, "<new address>").slice(body.indexOf("Check your email"), body.indexOf("Check your email") + 170)}".`,
      };
    },
  );
  await step(
    ...
    BU,
    "S6.follow-link",
    "Open the sign-in email and follow its link within five minutes.",
    "The Portal opens under that identity; first visit shows Welcome to your Business Portal; Department with Continue, Skip later, Finish opens the Portal.",
    async () => {
      ...
      check(
        header.includes(newName) || header.includes(newEmail) || main.includes(newName),
        `header "${header}"`,
      );
      return {
        page: url,
  ...
  await step(
    ...
    async () => {
      ...
      return {
        page: "/portal/login",
        actual: `Check your email said "If <address> is eligible, a sign-in link is on its way..."; Mailpit held ${n} messages for the address after 8 s.`,
      };
    },
  );
  await step(
    ...
    "An address outside the allowed domains with an existing account receives a link.",
    async () => {
      ...
      const inv = await call(daniel, "POST", "/auth/invites", {
        ...
        displayName: `DOC-030 access partner ${runTag}`,
      ...
      return {
        page: "/portal/login",
        actual: `Fixture: Daniel invited an account at partner-firm.example (not an allowed domain) and changed its role to Business User (${role.status}). Send link showed Check your email and the sign-in email "${m.subject}" arrived; its link opened ${landed}.`,
      };
    },
  );
  ...
  await step(
    ...
    "Sign-in link expired offers Email and Send link; the new link opens the Portal; an old link does not.",
    async () => {
      await bu.page.goto(firstLink);
      ...
      const onboarding = await bu.page
        ...
        .catch(() => false);
      check(
        u.pathname === "/auth/link-expired" && u.searchParams.get("portal") === "1" && !onboarding,
        `${u.pathname}${u.search}`,
      );
      return {
        page: `${u.pathname}${u.search}`,
        actual: `The used link opened ${u.pathname}${u.search}: "${body.slice(body.indexOf("Sign-in link expired"), body.indexOf("Sign-in link expired") + 150)}". Email and Send link sent "${m.subject}"; its link opened /portal without repeating onboarding.`,
      };
    },
  );
  await step(
  ...
  await step(
    ...
    async () => {
      ...
      return {
        page: "/portal",
        actual: `Header "${header.slice(0, 120)}". Jonas's own Request R-${jonasReq[0]} was listed; Amara's R-${amaraReq[0]} was not.`,
      };
    },
  );
  await step(
    ...
    "Another Business User's Request link does not grant access; staff pages are closed.",
    async () => {
      ...
      check(
        !leaked && Object.values(staff).every((p) => p.startsWith("/portal")),
        JSON.stringify({ landed, staff }),
      );
      return {
        page: `/portal/requests/${amaraReq[0]}`,
        actual: `Amara's Request address showed ${landed} without its title. Staff pages: ${JSON.stringify(staff)}.`,
      };
    },
  );
  await step(
    ...
    async () => {
      ...
      check(
        after > before && !archived && /not found/.test(direct),
  ...
  await step(
    ...
    "The Contract page shows these; the Primary Document label marks the primary file; the name opens the current Version; download works; Your Contracts returns to the list.",
    async () => {
      ...
      const viewer = J.getByRole("complementary", {
        name: /doc030-access-jonas-paper\.txt, version \d+/,
      });
      ...
      return {
        ...
        actual: `C-${n} "${fx.jonasConverted.title}" showed ${labels.join(", ")} (Term type is the terms fact). Overview text: "${main.slice(main.indexOf("Overview"), main.indexOf("Overview") + 330)}". Primary Document label shown; the name opened "${viewerName}"; "${dlName}" downloaded "${download.suggestedFilename()}"; Your Contracts returned to the list.`,
      };
    },
  );
  await step(
    ...
    async () => {
      ...
      check(
        main.includes(`M-${teamMatter}`) && missing.length === 0 && notListed,
        JSON.stringify({ missing, notListed }),
      );
      return {
        page: `/portal/matters/${teamMatter}`,
        actual: `Your Matters listed Jonas's team Matters and not M-${fx.mx.number}, whose team does not include him. M-${teamMatter} showed ${labels.join(", ")}: "${main.slice(0, 260)}".`,
      };
    },
  );
  await step(
    ...
    async () => {
      ...
      const hasBar = (label) =>
        J.getByRole("toolbar", { name: "Applets" })
          ...
          .count();
      const c = {
        Comments: await hasBar("Comments"),
        History: await hasBar("History"),
        "Contract team": await hasBar("Contract team"),
      };
      await go(
        J,
      ...
      const m = {
        "Matter team": await J.getByRole("toolbar", { name: "Applets" })
  ...
  await step(
    ...
    "The account keeps its role; it does not act as a Business User.",
    async () => {
      ...
      await c.context.close();
      check(
        me.body?.user?.role === "legal_team_member" || me.body?.role === "legal_team_member",
        JSON.stringify(me.body).slice(0, 200),
      );
      return {
        page: "/portal/login",
        actual: `Nadia's Portal link opened ${landed}; /me role ${me.body?.user?.role ?? me.body?.role}; /contracts stayed at ${staff}.`,
      };
    },
  );
  ...
  await step(
    ...
    async () => {
      ...
      const opened = await p
        .waitForURL(
          (x) => x.pathname.startsWith("/portal") && !x.pathname.startsWith("/portal/login"),
          { timeout: 20000 },
        )
        .then(
          () => true,
...
async function settingVariants({ A, BU, daniel, pwEmail, tfEmail }) {
  ...
  const loginView = async (c) => {
    ...
    return {
      ...
      buttons: (await c.page.locator("main").getByRole("button").allInnerTexts()).map((s) =>
        s.trim(),
      ),
    };
  };
  ...
  try {
    await step(
      ...
      BU,
      "V1.links-off",
      "With email links switched off for the Portal, open the sign-in page; use Set up or reset your password, Send password setup link, follow the one-hour link, set the password, sign in.",
      ...
      async () => {
        ...
        check(
          !v.buttons.includes("Email me a sign-in link") &&
            v.buttons.includes("Sign in") &&
            v.buttons.includes("Set up or reset your password"),
          v.buttons.join("|"),
        );
        ...
        await c.context.close();
        check(
          inPortal && expSend === 0 && /one hour/.test(neutral),
          JSON.stringify({ inPortal, expSend }),
        );
  ...
  try {
    ...
    await step(
      ...
      BU,
      "V2.sso-first",
      "On a page that shows Continue with single sign-on, select Email me a sign-in link; with links off, select Sign in with a password.",
      "Email me a sign-in link leads to Get a sign-in link; Sign in with a password leads to the password form.",
      async () => {
        ...
        await c.context.close();
        check(
          v1.buttons.includes("Continue with single sign-on") &&
            !v2.buttons.includes("Email me a sign-in link") &&
            v2.buttons.includes("Sign in with a password") &&
            pwForm &&
            r2.status === 200,
          JSON.stringify({ v1: v1.buttons, v2: v2.buttons }),
        );
        return {
          page: "/portal/login",
          actual: `SSO on with links on: buttons ${v1.buttons.join(", ")}; Email me a sign-in link opened Get a sign-in link. SSO on with links off: buttons ${v2.buttons.join(", ")}; Sign in with a password opened Email and Password. Continue with single sign-on was not followed: the stand-in provider has no identity provider behind it.`,
        };
      },
    );
  } finally {
  ...
  if (
    ...
    await tfContext.page.waitForURL((u) => u.pathname.replace(/\/$/, "") === "/portal", {
      timeout: 20000,
    });
  }
      ...
      async () => {
        ...
        check(
          enrollUrl.pathname === "/auth/two-factor/enroll" &&
            ...
            challengeUrl.pathname === "/auth/two-factor" &&
            opened &&
            /two-factor\/enroll/.test(pwResult),
          JSON.stringify({
            enroll: enrollUrl.pathname,
            challenge: challengeUrl.pathname,
            opened,
            pwResult,
          }),
        );
        return {
          page: `${enrollUrl.pathname}${enrollUrl.search}`,
  ...
  try {
    await step(
      ...
      async () => {
        ...
        change(
          "organization logo",
          oldLogo ? "saved logo" : null,
          "DOC-030 access 16x16 test logo",
        );
    ...
    change(
      "organization logo restored",
      "DOC-030 access 16x16 test logo",
      oldLogo ? "saved logo" : null,
    );
    if (r.status !== 200)
      results.settingChanges.push({
        module: "signin",
        at: new Date().toISOString(),
        what: "logo restore failed",
        status: r.status,
      });
  }
  ...
  results.settingChanges.push({
    ...
    at: new Date().toISOString(),
    what: "final check",
    policyRestored: JSON.stringify(after.policy.business) === JSON.stringify(original),
    ssoProviderId: after.ssoProviderId,
  });
  save();
}
const state = {};
...
async function historyProbe({ reader, legal, module, number, typed }) {
  ...
  const statuses =
    module === "contract"
      ? (await call(await admin(), "GET", "/contract-statuses")).body.contractStatuses.filter(
          (s) => !s.archivedAt,
        )
      : (await call(await admin(), "GET", "/matter-statuses")).body.matterStatuses.filter(
          (s) => !s.archivedAt,
        );
  const currentStatus = statuses.find((s) => s.id === record.statusId);
  const actions = [];
  const comment = (visibility, body) => () =>
    call(legal, "POST", "/comments", { entityType: module, entityId: id, body, visibility });
  actions.push([
    "Priority change",
    false,
    () =>
      call(legal, "PATCH", `/${base}/${number}`, {
        priority: record.priority === "high" ? "low" : "high",
      }),
  ]);
  actions.push([
    "Risk change",
    false,
    () =>
      call(legal, "PATCH", `/${base}/${number}`, { risk: record.risk === "high" ? "low" : "high" }),
  ]);
  actions.push([
    "Legal Only comment",
    false,
    comment("legal_only", `DOC-030 access Legal Only history probe ${runTag}`),
  ]);
  actions.push([
    "Working Team (Internal team) comment",
    false,
    comment("working_team", `DOC-030 access Working Team history probe ${runTag}`),
  ]);
  if (typed) {
    actions.push([
      ...
      () =>
        call(legal, "PATCH", `/${base}/${number}`, {
          customFields: { [fx.form.hidden.key]: `DOC-030 access hidden change ${runTag}` },
        }),
    ]);
    actions.push([
      ...
      () =>
        call(legal, "PATCH", `/${base}/${number}`, {
          customFields: { [fx.form.visible.key]: `DOC-030 access shown change ${runTag}` },
        }),
    ]);
  }
  actions.push([
    ...
    true,
    async () => {
      const r = await call(legal, "POST", `/${base}/${number}/tasks`, {
        title: `DOC-030 access probe task A ${runTag}`,
        dueDate: "2026-12-15",
      });
  ...
  actions.push([
    ...
    async () => {
      const r = await call(legal, "POST", `/${base}/${number}/tasks`, {
        title: `DOC-030 access probe task B ${runTag}`,
      });
  ...
  actions.push([
    ...
    false,
    () =>
      call(legal, "PATCH", `${tasksBase}/${taskA}`, {
        title: `DOC-030 access probe task A edited ${runTag}`,
      }),
  ]);
  actions.push([
    "Tasks reordered",
    false,
    async () => {
      const all = (await call(legal, "GET", `/${base}/${number}/tasks`)).body.tasks.map(
        (t) => t.id,
      );
      return call(legal, "PUT", `/${base}/${number}/tasks/reorder`, {
        taskIds: [...all].reverse(),
      });
    },
  ]);
  actions.push([
    "Task completed",
    true,
    () => call(legal, "POST", `${tasksBase}/${taskA}/toggle`, {}),
  ]);
  ...
  if (module === "contract") {
    ...
    actions.push([
      ...
      () =>
        call(legal, "PATCH", `/${base}/${number}`, {
          value: { amount: value + 100000, currency: "GBP", cadence: "annually" },
        }),
    ]);
    ...
    actions.push([
      "Expiry date changed",
      true,
      () => call(legal, "PATCH", `/${base}/${number}`, { expiryDate: newExpiry }),
    ]);
    const target =
    ...
    actions.push([
      ...
      true,
      () =>
        call(legal, "PATCH", `/${base}/${number}`, { statusId: target.id, overrideSoftGate: true }),
    ]);
  } else {
    const target =
      statuses.find((s) => s.id !== record.statusId && s.category === "open") ??
      statuses.find((s) => s.id !== record.statusId);
    actions.push([
      `Status move to ${target.displayName}`,
      true,
      () => call(legal, "PATCH", `/${base}/${number}`, { statusId: target.id }),
    ]);
  }
  actions.push([
    "Full Thread (team) comment",
    true,
    comment("full_thread", `DOC-030 access team history probe ${runTag}`),
  ]);
...
async function roles() {
  ...
  await step(
    ...
    "legal_team_member",
    "fixture.reset",
    "Put the roles fixture records in their starting state and create this run's approval requests (API).",
    "Jonas is on no fixture team; the Confidential records hold only their creator; three approval requests name Jonas.",
    async () => {
      const out = [];
      for (const [m, n, u] of [
        ["contracts", CO, fx.ids.jonas],
        ["contracts", CO, fx.ids.amara],
        ["contracts", CO, fx.ids.nadia],
        ["matters", MO, fx.ids.jonas],
        ["contracts", CC, fx.ids.daniel],
        ["contracts", CC, fx.ids.priya],
        ["contracts", CC, fx.ids.jonas],
      ...
      out.push(
        (
          await call(danielApi, "PATCH", `/contracts/${CO}`, {
      ...
      out.push(
        (await call(nadiaApi, "PATCH", `/contracts/${CC}`, { managerId: fx.ids.nadia })).status,
      );
      out.push(
        (await call(nadiaApi, "PATCH", `/matters/${MC}`, { managerId: fx.ids.nadia })).status,
      );
      for (const [k, confidential] of [
        ["open", false],
        ["confidentialPaper", true],
        ["withdraw", false],
      ]) {
        const c = (
          await call(nadiaApi, "POST", "/contracts", {
            title: `DOC-030 access ${k} approval ${runTag}`,
            contractTypeId: fx.types.nda,
        ...
        if (k !== "withdraw") {
          doc = (
            await upload(
              ...
              `doc030-access-${k}-paper-${runTag}.txt`,
              `DOC-030 access fictional ${k} paper.\n`,
            )
          ).body.document;
          if (confidential)
            await call(nadiaApi, "PATCH", `/documents/${doc.id}`, { isConfidential: true });
        }
        const a = (
          await call(nadiaApi, "POST", `/contracts/${c.number}/approvals`, {
        ...
        runApprovals[k] = {
          number: c.number,
          title: c.title,
          file: doc ? `doc030-access-${k}-paper-${runTag}.txt` : null,
          approvalId: (a.approvals ?? [a.approval ?? a])[0]?.id,
        };
      }
  ...
  await step(
    ...
    "administrator",
    "A1.accounts",
    "Open Settings > Users and a Business User's role control.",
    "Account types are Administrator, Legal Team Member and Business User; Contributor is no longer an account type; an Administrator can change an account to Legal Team Member.",
    async () => {
      await go(D, "/settings/users");
      const table = await text(D.getByRole("table").first());
      const roleButton = D.getByRole("button", {
        name: /change the role of ravi\.menon@helix\.example/,
      });
      ...
      return {
        page: "/settings/users",
        actual: `The Role column showed ${["Administrator", "Legal team member", "Business user"].filter((r) => new RegExp(r, "i").test(table)).join(", ")} and no Contributor. Ravi Menon's role control offered "${choices}". Escape closed it with no change.`,
      };
    },
  );
  await step(
    RA,
    "legal_team_member",
    "A2.settings-admin-only",
    "As a Legal Team Member, open Settings > Users.",
    "Organization settings remain Administrator-only.",
    async () => {
      await go(N, "/settings/users");
      const invite = await N.getByRole("button", { name: "Invite user" })
        .isVisible()
        .catch(() => false);
      ...
      return {
        page: "/settings/users",
        actual: `Nadia at /settings/users landed on ${new URL(N.url()).pathname} with no Invite user; an organization settings write answered ${r.status}.`,
      };
    },
  );
  await step(
    ...
    "legal_team_member",
    "B1.open-records",
    "Open a non-Confidential Contract and Matter without a team row.",
    "A Legal Team Member has full legal work on open records.",
    async () => {
      ...
      return {
        page: `/contracts/${CO}`,
        actual: `Nadia (no team row) opened C-${CO} "${c.title}" and M-${MO} "${m.title}".`,
      };
    },
  );
  await step(
    RA,
    "administrator",
    "B2.admin-confidential",
    "Open a Confidential Contract and Matter with no team row, no Legal Owner and no Matter Manager; reload.",
    "Administrator status does not bypass the Confidential rule; a copied link or reload cannot restore access.",
    async () => {
      ...
      return {
        page: `/contracts/${CC}`,
        actual: `Daniel saw "${c.title}" / "This Contract does not exist, or you cannot open it." for C-${CC} on the copied link and after reload; M-${MC} showed "${m.title}".`,
      };
    },
  );
  await step(
    ...
    "administrator",
    "B3.related-records",
    "Open the open parent Matter and the open child Contract whose parent is Confidential.",
    "An unreachable related record shows as Restricted contract or Restricted Matter with no link; a parent's team and Confidential flag do not pass to child Contracts.",
    async () => {
      await go(D, `/matters/${MO}`);
      const main = await text(D.getByRole("main"));
      const links = await D.getByRole("link", {
        name: new RegExp(`${fx.cc.title}|${fx.mc.title}`),
      }).count();
      ...
      check(
        /Restricted Matter/.test(main) && /Restricted contract/.test(main) && links === 0,
        main.slice(0, 300),
      );
      check(
        !child.refused && /Restricted contract/.test(cmain) && parentLink === 0,
        cmain.slice(0, 200),
      );
  ...
  await step(
    ...
    "legal_team_member",
    "B4.add-team-member",
    "As Creator and Legal Owner, open Contract team, select Add team member, choose Daniel in Person, select Add.",
    "Each person appears once with no role picker; the Administrator can then open the Confidential record.",
    async () => {
      ...
      check(
        !c.refused &&
          ...
          rows.filter((r) => r.includes("Daniel Okafor")).length === 1,
        JSON.stringify({ controls, options: options.length, rows }),
      );
      return {
        page: `/contracts/${CC}`,
        actual: `The Add team member dialog had one control, Person, and no role picker; Nadia (already on the team) was not offered. Roster: ${rows.join(" | ")}. Daniel then opened C-${CC} "${c.title}".`,
      };
    },
  );
  await step(
    ...
    "administrator",
    "B5.admin-audience",
    "As an Administrator who can reach the Confidential record, check the audience controls.",
    "An Administrator who can reach the record may change its Confidential audience.",
    async () => {
      await go(D, `/contracts/${CC}`);
      const add = await (
        await applet(D, "Contract team")
      )
      ...
      return {
        page: `/contracts/${CC}`,
        actual: `Daniel (team row, Administrator) had Add team member, the Confidential switch and the Legal Owner control enabled on C-${CC}.`,
      };
    },
  );
  await step(
    RA,
    "legal_team_member",
    "B6.reader-no-audience-right",
    "Add Priya (Legal Team Member, not Creator or Legal Owner) to the team; as Priya, check the audience controls and try to name a new Legal Owner.",
    "Being able to read the record does not give the right to change its audience; naming a new Legal Owner is an audience change.",
    async () => {
      await go(N, `/contracts/${CC}`);
      ...
      check(
        !c.refused && !add && !sw && !removes.some(Boolean) && w.status >= 400,
        JSON.stringify({ add, sw, removes, ownerEnabled, w: w.status }),
      );
      return {
        page: `/contracts/${CC}`,
        actual: `Priya opened C-${CC}. Add team member disabled; Confidential switch disabled; ${removes.length} remove controls, none enabled; Legal Owner control enabled ${ownerEnabled}: ${uiTry}. Naming herself Legal Owner through the record write answered ${w.status} (${JSON.stringify(w.body?.detail ?? w.body?.title ?? "").slice(0, 120)}).`,
      };
    },
  );
  await step(
    RA,
    "legal_team_member",
    "B7.remove-ends-access",
    "Remove Priya and Daniel with each person's remove control.",
    "Removal ends Confidential access on the next read.",
    async () => {
      await go(N, `/contracts/${CC}`);
      ...
      return {
        page: `/contracts/${CC}`,
        actual: `After Nadia removed both rows, Daniel's and Priya's reloads of C-${CC} showed "does not exist, or you cannot open it".`,
      };
    },
  );
  await step(
    ...
    "legal_team_member",
    "B8.legal-owner",
    "Name Daniel Legal Owner on the Confidential Contract in Overview; then Daniel names Nadia again.",
    "A Legal Owner reaches a Confidential Contract without a team row; changing the owner back ends it.",
    async () => {
      await go(N, `/contracts/${CC}`);
      ...
      return {
        page: `/contracts/${CC}`,
        actual: `After Nadia set Legal Owner to Daniel, Daniel opened C-${CC} (roster: ${rows.join(" | ")}). Daniel set Legal Owner back to Nadia; his reload was then refused and Nadia kept access.`,
      };
    },
  );
  await step(
    RA,
    "legal_team_member",
    "B9.matter-manager-and-team",
    "On the Confidential Matter, add and remove Daniel's team row, then name him Matter Manager and change it back.",
    "A team row or Matter Manager admits the person; removal ends access.",
    async () => {
      ...
      return {
        page: `/matters/${MC}`,
        actual: `Daniel opened M-${MC} through a team row, was refused after removal, opened it again as Matter Manager, and was refused after he named Nadia Matter Manager again.`,
      };
    },
  );
  await step(
    RA,
    "legal_team_member",
    "B10.ltm-open-removal-and-document",
    "Add and remove Nadia on the open Contract team; read its Confidential Document each time.",
    "Removing a Legal Team Member from an open record does not end ordinary access; a Document's Confidential flag narrows its audience.",
    async () => {
      ...
      check(
        !after.refused && !before && onTeam && !afterDoc && danielDoc,
        JSON.stringify({ before, onTeam, afterDoc, danielDoc }),
      );
      return {
        page: `/contracts/${CO}/documents`,
        actual: `The Confidential Document doc030-access-confidential-note.txt was hidden from Nadia before, shown while she was on the team, and hidden after removal; she still opened C-${CO}. Daniel (creator, Legal Owner) saw it throughout.`,
      };
    },
  );
  await step(
    RA,
    "administrator",
    "C1.entity-admin-needs-grant",
    "As Administrator and as a Legal Team Member without a Grant, open a Confidential Entity.",
    "Every person needs a Grant for a Confidential Entity; Administrators need one too.",
    async () => {
      ...
      return {
        ...
        actual: `Daniel saw "${e.title}" and Priya saw "${p.title}" for the Confidential Entity; neither has a Grant.`,
      };
    },
  );
  await step(
    ...
    "legal_team_member",
    "C2.entity-grant",
    "As the creator, use Manage access to give Daniel a Grant, then remove it.",
    "The creator has a Grant; a grantee gives and removes Grants with Manage access; a Grant applies to that Entity only.",
    async () => {
      ...
      await dialog.waitFor();
      const creator = await dialog
        .getByRole("button", { name: "Remove Nadia Haddad" })
      ...
      check(
        creator &&
          ...
          other.refused &&
          after &&
          !options.some((o) => /Jonas|Ravi|Amara/.test(o)),
        JSON.stringify({ creator, e, manage, other, after }),
      );
      return {
        page: `/entities/${ENTITY}`,
  ...
  await step(
    ...
    async () => {
      ...
      return {
        page: `/entities/${ENTITY}`,
        actual: `Removing the only Grant was refused (${message.slice(0, 200)}); Nadia still opened the Entity.`,
      };
    },
  );
  await step(
    ...
    "Open a non-Confidential Entity as Administrator and as a Legal Team Member.",
    "An Administrator can manage access on an Entity that is not Confidential.",
    async () => {
      const [oid, oname] = sql(
        "select id||'|'||legal_name from entities where legal_name='Helix Software Group, Inc.' and not is_confidential and archived_at is null limit 1",
      ).split("|");
      const open = { id: oid, legalName: oname };
      await go(D, `/entities/${open.id}`);
      const dm = await D.getByRole("button", { name: "Manage access" })
        .isVisible()
        .catch(() => false);
      await go(N, `/entities/${open.id}`);
      const nm = await N.getByRole("button", { name: "Manage access" })
        .isVisible()
        .catch(() => false);
      check(dm && !nm, JSON.stringify({ dm, nm }));
      return {
        page: `/entities/${open.id}`,
        actual: `On "${open.legalName}" Daniel saw Manage access and Nadia (no Grant) did not. No Grant was changed.`,
      };
    },
  );
  await step(
    ...
    "business_user",
    "D1.full-app-closed",
    "As a Business User, open full-app records, Entities and Settings addresses.",
    "Business Users work through the Portal and cannot open the Entities module.",
    async () => {
      const seen = [];
      for (const url of [
        `/contracts/${CO}`,
      ...
      check(
        seen.every((s) => /-> \/portal/.test(s)) && !/Entities/.test(nav) && api.status >= 400,
        seen.join("; "),
      );
      return {
        page: "/portal",
        actual: `${seen.join("; ")}. Portal navigation: "${nav}". An Entity read answered ${api.status}.`,
      };
    },
  );
  await step(
    ...
    "business_user",
    "D2.not-on-team",
    "Open Portal addresses of records whose teams do not include the Business User.",
    "Business Users reach only non-archived records whose teams include them.",
    async () => {
      const out = [];
      for (const url of [
        `/portal/contracts/${CO}`,
  ...
  await step(
    ...
    "business_user",
    "D3.conversion",
    "Open the Business User's converted Request address.",
    "At conversion the Requester becomes Business Owner and a team member; the old Request address leads to that record.",
    async () => {
      await go(J, `/portal/requests/${fx.jonasConverted.request}`);
      const landed = new URL(J.url()).pathname;
      const rows = await rosterRows(await applet(J, "Contract team"));
      ...
      return {
        page: landed,
        actual: `/portal/requests/${fx.jonasConverted.request} led to ${landed}. Contract team: ${rows.join(" | ")}.`,
      };
    },
  );
  await step(
    ...
    "Open Contract team on the open Contract, Add team member, choose Jonas, Add.",
    "Adding a Business User gives them access to the record in the Portal.",
    async () => {
      ...
      return {
        page: `/portal/contracts/${CO}`,
        actual: `After Nadia added Jonas, he opened /portal/contracts/${CO} "${c.title}".`,
      };
    },
  );
  await step(
    ...
    "business_user",
    "D5.comments",
    "As the later-added Business User, open Comments and reply.",
    "Business Users read and post Full Thread comments with the Contract Team label, see earlier Contract Team comments, and do not see Legal Only or older Internal team comments.",
    async () => {
      ...
      const fullRow = (
        await text(
          panel
            .getByRole("listitem")
            .filter({ hasText: "DOC-030 access Full thread note" })
            .first(),
        )
      ).replace("DOC-030 access Full thread note", "");
      await panel.getByRole("textbox", { name: "New comment" }).fill(reply);
      await panel.getByRole("button", { name: "Comment", exact: true }).click();
      await panel.getByText(reply).waitFor({ timeout: 10000 });
      const mine = (
        await text(panel.getByRole("listitem").filter({ hasText: reply }).first())
      ).replace(reply, "");
      check(
        all.includes("DOC-030 access Full thread note") &&
          !all.includes("DOC-030 access Legal only note") &&
          !all.includes("DOC-030 access Working team note") &&
          radios === 0 &&
          /Contract Team/.test(fullRow) &&
          /Contract Team/.test(mine),
        JSON.stringify({ fullRow, mine, radios }),
      );
      return {
        page: `/portal/contracts/${CO}`,
        actual: `Jonas, added after the three fixture comments, saw the earlier Full Thread comment ("${fullRow.slice(0, 80)}") and neither the Legal Only nor the Working Team comment; no audience picker. His reply row read "${mine.slice(0, 80)}".`,
      };
    },
  );
  await step(
    ...
    "legal_team_member",
    "D6.legal-audience",
    "As Legal, open Comments on the Contract and on a Matter.",
    "Legal chooses Contract Team or Matter Team above New comment; Full Thread rows show Contract Team; Legal Only and older Internal team rows show those labels.",
    async () => {
      await go(N, `/contracts/${CO}`);
      ...
      const row = async (body) =>
        (await text(lp.getByRole("listitem").filter({ hasText: body }).first())).replace(body, "");
      ...
      const maud = await text(
        (await applet(N, "Comments")).getByRole("group", { name: "Audience" }),
      );
      check(
        /Legal Only/.test(audience) &&
          /Contract Team/.test(audience) &&
          /Matter Team/.test(maud) &&
          gy < ty &&
          /Contract Team/.test(full) &&
          /Legal Only/.test(legal) &&
          /Internal team/.test(work),
        JSON.stringify({ audience, maud, full, legal, work }),
      );
      return {
        page: `/contracts/${CO}`,
        actual: `Audience above New comment offered "${audience}" on the Contract and "${maud}" on the Matter. Row labels: Full Thread "${full.slice(0, 60)}"; Legal Only "${legal.slice(0, 60)}"; older Working Team "${work.slice(0, 60)}".`,
      };
    },
  );
  await step(
    ...
    "business_user",
    "D7.portal-history",
    "Legal changes the open Contract one item at a time; the Business User reads Portal History after each.",
    "History shows Contract Team comments, Stage moves, owners, Value, dates, Visible on Portal Fields, and Tasks added or completed; it excludes Legal Only and Internal team comments, Priority, Risk and Fields not Visible on Portal.",
    async () => {
      await call(nadiaApi, "PATCH", `/contracts/${CO}`, { contractTypeId: fx.form.typeA.id });
      const probe = await historyProbe({
        reader: J,
        legal: nadiaApi,
        module: "contract",
        number: CO,
      ...
      const owner = await (async () => {
        ...
        return {
          status: r.status,
          fresh: after.filter((e) => !before.some((b) => b.id === e.id)).length,
        };
      })();
      ...
      return {
        page: `/portal/contracts/${CO}`,
        actual: `${verdict}; Legal Owner change: shown (${owner.fresh} new). Portal History panel: "${probe.panelText.slice(0, 700)}". See roles-portal-history.png.`,
      };
    },
  );
  await step(
    RA,
    "business_user",
    "D8.no-legal-controls",
    "Look for Field, Task, Key date, approval request and signing controls on the Portal record.",
    "The Portal record has no controls for Fields, Tasks, Key dates, approval requests or signing.",
    async () => {
      await go(J, `/portal/contracts/${CO}`);
      const main = await text(J.getByRole("main"));
      const buttons = (await J.getByRole("main").getByRole("button").allInnerTexts())
        .map((s) => s.trim())
        .filter(Boolean);
      const bad = buttons.filter(
        (b) =>
          /Task|Key date|Approv|Sign|Edit|Stage|Status|Move/i.test(b) && !/\.(txt|pdf)$/.test(b),
      );
      const inputs = await J.getByRole("main")
        .locator("input:not([type=file]):not([type=search]), textarea, select")
        .evaluateAll((els) =>
          els
            .filter(
              (e) =>
      ...
      check(
        bad.length === 0 && inputs.length === 0 && !/Key dates|Send for signature/.test(main),
        JSON.stringify({ bad, inputs }),
      );
      return {
        page: `/portal/contracts/${CO}`,
        actual: `No Field inputs (${inputs.length}), no Task, Key date, approval or signing controls; main buttons were: ${[...new Set(buttons)].slice(0, 20).join(", ")}.`,
      };
    },
  );
  await step(
    ...
    "business_user",
    "D9.portal-team-add",
    "In the Portal, open Contract team, Add team member, choose an existing person, Add.",
    "Business Users may add existing people from the Portal on non-Confidential records; there is no remove control.",
    async () => {
      await go(J, `/portal/contracts/${CO}`);
      ...
      return {
        page: `/portal/contracts/${CO}`,
        actual: `Jonas added Amara Nwosu (existing members were not offered); no remove control in the Portal. Nadia saw Amara on the same team in the full app and removed her.`,
      };
    },
  );
  await step(
    ...
    "legal_team_member",
    "D10.business-owner",
    "On Overview, assign Amara as Business Owner; try to remove her; change the Business Owner; remove her; then clear the Business Owner.",
    "Assigning a Business Owner adds that person to the team; change or clear the Business Owner before removing them; reassignment leaves the former owner on the team; removal ends Business User access on the next read, including old links, replies and Documents.",
    async () => {
      ...
      const removeCtl = await (
        await applet(N, "Contract team")
      )
        .getByRole("button", { name: "Take Amara Nwosu off the contract team" })
        .count();
      ...
      const aReply = await pageApi(A, "POST", "/comments", {
        entityType: "contract",
        entityId: fx.co.id,
        body: "DOC-030 access late reply",
      ...
      check(
        rows1.some((r) => /Business Owner/.test(r) && r.includes("Amara Nwosu")) &&
          removeCtl === 0 &&
          refusedRemove.status === 409 &&
          !aOpen.refused,
        JSON.stringify({ rows1, removeCtl, refusedRemove: refusedRemove.status }),
      );
      check(
        rows2.some((r) => r.includes("Amara Nwosu")) &&
          aAfter &&
          aDocs.status >= 400 &&
          aReply.status >= 400 &&
          rows3.some((r) => r.includes("Jonas Weber")),
        JSON.stringify({ rows2, aAfter, aDocs: aDocs.status, aReply: aReply.status, rows3 }),
      );
      return {
        page: `/contracts/${CO}`,
        actual: `Setting Business Owner to Amara added her row: ${rows1.join(" | ")}. Her row had no remove control, and a direct removal answered ${refusedRemove.status} "${refusedRemove.body?.detail}". Amara opened C-${CO} in the Portal (her roster: ${aRows.join(" | ")}). After Business Owner changed to Jonas, Amara stayed on the team (${rows2.join(" | ")}). Nadia then removed her; Amara's reload of the old link showed Contract not found, a Documents read answered ${aDocs.status} and a reply answered ${aReply.status}. Business Owner cleared to Unassigned; Jonas kept his team row (${rows3.join(" | ")}).`,
      };
    },
  );
  await step(
    RA,
    "business_user",
    "D11.matter-team-and-status",
    "Legal adds Jonas to the open Matter and posts with Matter Team and Legal Only; Legal moves its Status.",
    "A Full Thread comment shows Matter Team; Legal Only stays hidden; the Status move appears in Portal History; linked and child Confidential records stay closed.",
    async () => {
      await go(N, `/matters/${MO}`);
      await addTeamMember(N, "Matter team", "Jonas Weber");
      const team = await legalComment(
        N,
        `/matters/${MO}`,
        "Matter Team",
        `DOC-030 access Matter Team note ${runTag}`,
      );
      const legal = await legalComment(
        N,
        `/matters/${MO}`,
        "Legal Only",
        `DOC-030 access Matter Legal Only note ${runTag}`,
      );
      const probe = await historyProbe({
        reader: J,
        legal: nadiaApi,
      ...
      const jRow = (
        await text(
          jc
            .getByRole("listitem")
            .filter({ hasText: `DOC-030 access Matter Team note ${runTag}` })
            .first(),
        )
      ).replace(`DOC-030 access Matter Team note ${runTag}`, "");
      const main = await text(J.getByRole("main"));
      const c = await openRecord(J, `/portal/contracts/${CC}`);
      const child = await openRecord(J, `/portal/matters/${MC}`);
      check(
        /Matter Team/.test(team) &&
          /Legal Only/.test(legal) &&
          /Matter Team/.test(jRow) &&
          !jText.includes(`Matter Legal Only note ${runTag}`) &&
          c.refused &&
          child.refused &&
          !main.includes(fx.cc.title) &&
          !main.includes(fx.mc.title),
        JSON.stringify({ team, legal, jRow }),
      );
      return {
        page: `/portal/matters/${MO}`,
        actual: `Nadia's rows: "${team.slice(0, 50)}" and "${legal.slice(0, 50)}". Jonas saw the Matter Team comment ("${jRow.slice(0, 60)}") and not the Legal Only one. Matter History probe: ${verdict}. Linked C-${CC} showed "${c.title}" and child M-${MC} showed "${child.title}"; the Matter page named neither.`,
      };
    },
  );
  await step(
    RA,
    "business_user",
    "D12.confidential-portal",
    "Legal adds Jonas to the Confidential Contract; Jonas opens Contract team; Legal removes him.",
    "A Business User needs a team row on a Confidential record; Portal team changes on Confidential records stay with Legal.",
    async () => {
      await go(N, `/contracts/${CC}`);
      await addTeamMember(N, "Contract team", "Jonas Weber");
      const c = await openRecord(J, `/portal/contracts/${CC}`);
      ...
      check(
        !c.refused &&
          !add &&
          /Ask Legal to add members to a Confidential record/.test(note) &&
          after,
        note,
      );
      return {
        page: `/portal/contracts/${CC}`,
        actual: `With a team row Jonas opened C-${CC}; Add team member was disabled with "Ask Legal to add members to a Confidential record." After removal his reload showed Contract not found.`,
      };
    },
  );
  await step(
    RA,
    "business_user",
    "D13.approval-exception",
    "Open Approvals in the Portal; open a request; open one whose primary Document is Confidential; Legal withdraws one request and archives one Contract.",
    "The request shows the Contract title and current primary Document, no team row, comments, Fields or other Documents; a Confidential primary Document is not shown to an approver off the team; withdrawal or archiving ends access.",
    async () => {
      await go(J, "/portal");
      await J.getByRole("navigation", { name: "Portal" })
      ...
      const listed = ["open", "confidentialPaper", "withdraw"].every((k) =>
        list.includes(runApprovals[k].title),
      );
      ...
      const ar = await call(
        nadiaApi,
        "POST",
        `/contracts/${runApprovals.confidentialPaper.number}/archive`,
        {},
      );
      ...
      const rs = await call(
        nadiaApi,
        "POST",
        `/contracts/${runApprovals.confidentialPaper.number}/restore`,
        {},
      );
      check(
        listed &&
          packet.includes(runApprovals.open.title) &&
          packet.includes(runApprovals.open.file) &&
          toolbar === 0 &&
          !/Fields|Comments|Contract team/.test(packet) &&
          direct.refused,
        packet.slice(0, 300),
      );
      check(
        /No document attached/.test(cpacket) &&
          !cpacket.includes(runApprovals.confidentialPaper.file),
        cpacket.slice(0, 300),
      );
      check(
        wd.status < 300 &&
          ar.status < 300 &&
          !list2.includes(runApprovals.withdraw.title) &&
          !list2.includes(runApprovals.confidentialPaper.title),
        JSON.stringify({ wd: wd.status, ar: ar.status }),
      );
      return {
        page: "/portal/approvals",
        actual: `Your approvals (Pending) listed the three DOC-030 access requests created for this run naming Jonas. The open request read "${packet.slice(0, 260)}" with no applet bar, Fields, comments or team; /portal/contracts/${runApprovals.open.number} showed "${direct.title}". The request whose primary Document is Confidential read "${cpacket.slice(0, 200)}". Legal withdrew one request (${wd.status}) and archived the other Contract (${ar.status}); both left Your approvals, and the withdrawn request's address read "${wdPage.slice(0, 120)}". Legal restored the Contract afterwards (${rs.status}).`,
      };
    },
  );
  await step(
    RA,
    "business_user",
    "D14.archived-request",
    "Legal archives the converted Contract; the Business User opens Your Contracts, the record link and the old Request link.",
    ...
    async () => {
      ...
      const comments =
        ...
        (await J.getByRole("toolbar", { name: "Applets" }).count());
      const rs = await call(nadiaApi, "POST", `/contracts/${n}/restore`, {});
      check(
        ar.status < 300 &&
          !new RegExp(`\\bC-${n}\\b`).test(list) &&
          direct.refused &&
          /archived/.test(main) &&
          main.includes("DOC-030 access fictional ask from Jonas") &&
          docs === 0 &&
          comments === 0,
        main.slice(0, 300),
      );
      return {
        page: `/portal/requests/${fx.jonasConverted.request}`,
        actual: `Legal archived C-${n} (${ar.status}). Your Contracts no longer listed it; its link showed "${direct.title}". The old Request link read "${main.slice(0, 260)}" with no Documents and no comment controls. Legal restored C-${n} (${rs.status}).`,
      };
    },
  );
}
...
async function contributor() {
  ...
  await step(
    ...
    "legal_team_member",
    "fixture.reset",
    "Put Ravi's fixture records in their starting state (API).",
    "Ravi is on the converted Contract team and absent from the Matter and comparable records.",
    async () => {
      const out = [];
      for (const [m, n, u] of [
        ["matters", MR, fx.ids.ravi],
        ["matters", MR, fx.ids.amara],
        ["matters", fx.mc.number, fx.ids.ravi],
      ])
        out.push((await call(nadiaApi, "DELETE", `/${m}/${n}/team/${u}`)).status);
      out.push((await call(nadiaApi, "POST", `/matters/${MR}/restore`, {})).status);
      const open = (await call(await admin(), "GET", "/matter-statuses")).body.matterStatuses.find(
        (s) => s.slug === "open",
      );
      out.push(
        (
          await call(nadiaApi, "PATCH", `/matters/${MR}`, {
            statusId: open.id,
            confirmReopen: true,
          })
        ).status,
  ...
  await step(
    ...
    BU,
    "G1.before-start",
    "Before Legal adds Ravi, open the Matter; after Legal adds him with Matter team, select Matters in the Portal navigation bar and choose the record; check the title and M- reference; check the converted Contract's team.",
    "The record opens only after Legal adds him; being Business Owner adds you to the team; a copied link to a comparable record does not grant access.",
    async () => {
      ...
      check(
        before.refused &&
          ...
          main.includes(`M-${MR}`) &&
          comparable.refused &&
          rows.some((r) => /Business Owner/.test(r) && r.includes("Ravi Menon")),
        JSON.stringify({ before, title, comparable, rows }),
      );
      return {
        page: `/portal/matters/${MR}`,
        actual: `Before: "${before.title}". After Nadia added Ravi through Matter team, Matters listed the record; it showed "${title}" and M-${MR}. The comparable M-${MX} showed "${comparable.title}". On his converted C-${CR} the team read: ${rows.join(" | ")}.`,
      };
    },
  );
  await step(
    ...
    BU,
    "G2.search",
    "Enter a title or reference in Search, press Enter or select Search; search Contracts by the primary Counterparty.",
    ...
    async () => {
      ...
      check(
        byTitle.includes(`M-${MR}`) &&
          ...
          !byRef.includes(`M-${DM}`) &&
          byCp.includes(`C-${CR}`),
        JSON.stringify({ byTitle, byRef, byCp }).slice(0, 400),
      );
      return {
        page: "/portal/matters",
        actual: `"DOC-030 access Ravi" + Enter listed M-${MR}; "M-${MR}" + Search listed only M-${MR}; Contracts search "Fictional Supplier" listed C-${CR} through its primary Counterparty.`,
      };
    },
  );
  await step(
    ...
    BU,
    "G3.filter",
    "Select Filter, choose a property and value, select Apply; combine with search; remove a chip or use Clear all.",
    "Contracts offer Stage, Type, Legal Owner and Expiry date; Matters offer Status, Type, Matter Manager and open or closed lifecycle.",
    async () => {
      await go(R, "/portal/contracts");
      ...
      check(
        ["Type", "Legal Owner", "Stage", "Expiry date"].every((p) => cprops.includes(p)) &&
          ...
          chips === 1 &&
          cleared === 0,
        JSON.stringify({ cprops, mprops, chips, cleared }),
      );
      return {
        page: "/portal/matters",
        actual: `Contracts Filter offered ${cprops.join(", ")}; Matters Filter offered ${mprops.join(", ")}. Lifecycle showed "${life.slice(0, 80)}". Lifecycle Open with search "DOC-030 access" gave one chip (address ${url}); Clear all removed it.`,
      };
    },
  );
  await step(
    CG,
    BU,
    "G4.sort-columns-more",
    "Select a column heading to sort; use Columns; drag a heading's edge; Show more; Back and a bookmark.",
    "Sorting cycles ascending, descending and default; Columns chooses and reorders; the edge resizes; Show more adds a page; the address keeps search, filters and sort.",
    async () => {
      await go(R, "/portal/contracts");
  ...
  await step(
    ...
    BU,
    "G5.read-record",
    "Read the record details and Fields on the Contract and the Matter; look for edit controls; Legal changes a Visible on Portal Field and the Description; reload.",
    "Department and Region on both; Overview with Value and dates; Visible on Portal Field Rows and Description are read-only; a Row off the Portal is absent; a Row under a false Branch is listed as Not recorded; Legal's change appears on reload; Original request does not change.",
    async () => {
      ...
      await call(nadiaApi, "PATCH", `/contracts/${CR}`, {
        customFields: { [fx.form.visible.key]: newValue },
        description: `DOC-030 access live description ${runTag}`,
      });
      await R.reload();
      await settle(R);
      ...
      const branchRow = fText.slice(
        fText.indexOf(fx.form.branch.name),
        fText.indexOf(fx.form.branch.name) + fx.form.branch.name.length + 20,
      );
      check(
  ...
  await step(
    ...
    BU,
    "G6.documents-read",
    "Read Documents: order, row facts, kind for a Version of an Administrator-added type; select a name to preview; download.",
    "The Primary Document comes first; each row shows current Version, kind, uploader and date; a Version of an Administrator-added type reads General; the name opens a preview; the download control downloads.",
    async () => {
      const pdf = await upload(
        nadiaApi,
        `/contracts/${CR}/documents`,
        `doc030-access-preview-${runTag}.pdf`,
        tinyPdf("DOC-030 access fictional preview paper"),
        "application/pdf",
        { documentTypeId: fx.docType.id },
      );
      ...
      for (let i = 0; i < 10; i++) {
        await region
          .getByRole("button", { name: `doc030-access-preview-${runTag}.pdf` })
          .first()
          .click();
        const viewer = R.getByRole("complementary", {
          name: new RegExp(`doc030-access-preview-${runTag}\\.pdf`),
        });
        await viewer.waitFor({ timeout: 15000 });
        await sleep(2500);
      ...
      await R.getByRole("complementary", {
        name: new RegExp(`doc030-access-preview-${runTag}\\.pdf`),
      })
        .getByRole("button", { name: "Close the document" })
        .click();
      const href = await region
        .getByRole("link", { name: /Download doc030-access-supporting-v1\.txt, version \d+/ })
        .first()
        .getAttribute("href");
      const dl = await R.request.get(new URL(href, BASE).toString());
      check(
        /^Primary Document doc030-access-supporting-v1\.txt/.test(rows[0]) &&
          /Version 1/.test(pdfRow) &&
          /General/.test(pdfRow) &&
          /Nadia Haddad/.test(pdfRow) &&
          rendered > 0 &&
          dl.status() === 200,
        JSON.stringify({ first: rows[0], pdfRow, rendered }),
      );
      return {
        page: `/portal/contracts/${CR}`,
        actual: `First row: "${rows[0].slice(0, 140)}". The PDF uploaded by Legal with the Administrator-added type "${fx.docType.name}" read "${pdfRow.slice(0, 160)}" (kind General). Its name opened the viewer with ${rendered} rendered page element(s); see contributor-portal-preview.png. The download control answered ${dl.status()}.`,
      };
    },
  );
  await step(
    ...
    BU,
    "G7.upload-contract",
    "Select Upload documents; choose New documents under Add as; choose Kind; enter Note (optional); Upload two files with one failure; Retry failed uploads.",
    "Kind starts on General and offers the six fixed Contract types; Kind and Note apply to every file; Legal sees the Kind as the Version's Document type and General as no type; Retry failed uploads retries only the failures.",
    async () => {
      const f1 = file(
        `doc030-access-business-upload-${runTag}.txt`,
        "DOC-030 access fictional business upload.\n",
      );
      const f2 = file(
        `doc030-access-second-upload-${runTag}.txt`,
        "DOC-030 access fictional second upload.\n",
      );
      const f3 = file(
        `doc030-access-general-upload-${runTag}.txt`,
        "DOC-030 access fictional general upload.\n",
      );
      await go(R, `/portal/contracts/${CR}`);
      let failed = false;
      let posts = 0;
        ...
        if (!failed && body.includes(`doc030-access-second-upload-${runTag}.txt`)) {
          ...
          return route.fulfill({
            ...
            body: JSON.stringify({
              ...
              status: 503,
              detail: "DOC-030 injected upload failure.",
            }),
          });
        }
      ...
      await dialog
        ...
        .fill(`DOC-030 access business note ${runTag}`);
      ...
      const typeOf = (d) =>
        d?.versions?.[0]?.documentType?.displayName ??
        d?.versions?.[0]?.documentTypeName ??
        d?.versions?.[0]?.documentTypeId ??
        null;
      ...
      check(
        posts === 3 &&
          ...
          d3 &&
          noteOf(d1) === noteOf(d2) &&
          noteOf(d1) === `DOC-030 access business note ${runTag}` &&
          kindDefault === "General" &&
          kinds.length === 7 &&
          kinds.includes("Executed"),
        JSON.stringify({ posts, kinds, kindDefault, legalView, notes: [noteOf(d1), noteOf(d2)] }),
      );
      check(
        /Draft · theirs/i.test(String(legalView.d1)) &&
          /Draft · theirs/i.test(String(legalView.d2)) &&
          !legalView.d3,
        `Legal types ${JSON.stringify(legalView)} ${JSON.stringify(d1?.versions?.[0]).slice(0, 300)}`,
      );
      return {
        page: `/portal/contracts/${CR}`,
        actual: `Add as offered ${addAs.join(", ")}. Kind started on ${kindDefault} and offered ${kinds.join(", ")}. With one failure injected by the reviewer's browser route, the dialog read "${mid.slice(0, 200)}"; Retry failed uploads sent only the failed file (${posts} POSTs). Both Documents carry the note. Legal's Document list shows the Draft · theirs uploads as ${JSON.stringify(legalView.d1)} / ${JSON.stringify(legalView.d2)}, and the General upload with no type (${JSON.stringify(legalView.d3)}).`,
      };
    },
  );
  await step(
    ...
    BU,
    "G7b.upload-matter",
    "Open Upload documents on the Matter.",
    "Matter uploads have no Kind control.",
    async () => {
      await go(R, `/portal/matters/${MR}`);
      await R.getByRole("button", { name: "Upload documents" }).click();
      const dialog = R.getByRole("dialog", { name: "Upload documents" });
      ...
      await dialog
        ...
        .setInputFiles([
          file(
            `doc030-access-matter-upload-${runTag}.txt`,
            "DOC-030 access fictional Matter upload.\n",
          ),
        ]);
      ...
      return {
        page: `/portal/matters/${MR}`,
        actual: `The Matter Upload documents dialog read "${labels.slice(0, 200)}" with no Kind control; the upload finished.`,
      };
    },
  );
  await step(
    ...
    BU,
    "G8.add-version",
    "Select Add version beside the primary Contract Document; upload one file; expand earlier versions.",
    "Add version opens the dialog with that Document selected and takes one file; Signed copy marks the Version Legal pinned; uploading a Version changes neither designation.",
    async () => {
      await call(nadiaApi, "POST", `/documents/${fx.raviPrimary.id}/executed-version`, {
        versionId: fx.raviPrimary.v1,
      });
      await go(R, `/portal/contracts/${CR}`);
      const region = R.getByRole("region", { name: "Documents" });
      const row = region
        .getByRole("listitem")
        .filter({ hasText: "doc030-access-supporting" })
        .first();
      ...
      await dialog
        ...
        .setInputFiles(
          file(
            `doc030-access-supporting-${runTag}.txt`,
            "DOC-030 access fictional supporting paper, next version.\n",
          ),
        );
      await dialog.getByRole("button", { name: "Upload", exact: true }).click();
      await dialog.waitFor({ state: "hidden", timeout: 20000 });
      ...
      check(
        ...
        JSON.stringify({ selected, multiple, t: t.slice(0, 300), first: first.slice(0, 80) }),
      );
      return {
        page: `/portal/contracts/${CR}`,
        actual: `Legal pinned Version 1 as the signed copy (API). Add version opened Upload documents with "${selected}" and a single-file input. After upload the row read "${t.slice(0, 280)}"; the first row is still the Primary Document.`,
      };
    },
  );
  await step(
    ...
    BU,
    "G9.drop-no-folders",
    "Drop a file onto Documents; look for folder and archive controls.",
    ...
    async () => {
      ...
      const dt = await R.evaluateHandle(() => {
        ...
        d.items.add(
          new File(["DOC-030 access dropped paper"], "doc030-access-dropped.txt", {
            type: "text/plain",
          }),
        );
        return d;
      ...
      return {
        page: `/portal/contracts/${CR}`,
        actual: `Dropping doc030-access-dropped.txt on "Drop files here or click to upload" opened Upload documents with the file listed (cancelled). Documents had no folder or archive controls.`,
      };
    },
  );
  await step(
    ...
    "Use Search Documents with an earlier Version's filename and a name; use Show more.",
    ...
    async () => {
      ...
      check(
        /doc030-access-supporting/.test(byOld) &&
          before === 50 &&
          after === 51 &&
          byName.length === 1,
        JSON.stringify({ before, after, byName, byOld: byOld.slice(0, 200) }),
      );
      return {
        page: `/portal/matters/${DM}`,
        actual: `"supporting-v1" (the first Version's filename) listed the supporting Document whose current Version is newer: "${byOld.slice(0, 160)}". On M-${DM} Documents showed ${before} rows and Show more added the rest (${after}); "page-37" listed ${byName.join(", ")}.`,
      };
    },
  );
  await step(
    ...
    BU,
    "G11.comments",
    "Open Comments, write a reply, attach a file, select Comment; type @; use Comment actions to edit and delete; switch applets with a draft.",
    "The unread badge clears after the comments load; the draft and attachments stay; @ offers people who can hear the comment; Comment actions edit and delete; there is no audience picker.",
    async () => {
      const t = `DOC-030 access ask ${runTag}`;
      await call(nadiaApi, "POST", "/comments", {
        entityType: "matter",
        entityId: fx.mr.id,
        body: `DOC-030 access Legal reply for Ravi ${runTag}`,
        visibility: "full_thread",
      });
      await go(R, `/portal/matters/${MR}`);
      ...
      const attach = file(
        `doc030-access-comment-${runTag}.txt`,
        "DOC-030 access fictional comment attachment.\n",
      );
      ...
      check(
        pickers === 0 &&
          switched.startsWith("DOC-030 access draft kept") &&
          kept.startsWith("DOC-030 access draft kept") &&
          keptFile &&
          posted.includes(`doc030-access-comment-${runTag}.txt`) &&
          mentions.some((o) => /Nadia/.test(o)) &&
          (!final.includes(`${t} edited`) || /deleted/i.test(final)),
        JSON.stringify({ pickers, kept, keptFile, mentions, posted: posted.slice(0, 120) }),
      );
      return {
        page: `/portal/matters/${MR}`,
  ...
  await step(
    ...
    BU,
    "G12.reply-label",
    "Post a reply on the Matter and on the Contract; read the text below the composer.",
    "The reply shows Matter Team or Contract Team; the text below the composer names the audience; Legal Only and older Internal team messages stay outside.",
    async () => {
      await call(nadiaApi, "POST", "/comments", {
        entityType: "matter",
        entityId: fx.mr.id,
        body: `DOC-030 access hidden Legal Only ${runTag}`,
        visibility: "legal_only",
      });
      await call(nadiaApi, "POST", "/comments", {
        entityType: "matter",
        entityId: fx.mr.id,
        body: `DOC-030 access hidden Internal team ${runTag}`,
      ...
      for (const [label, url, module] of [
        ...
        const all = await text(panel);
        if (module === "Matter")
          await R.screenshot({ path: path.join(here, "contributor-reply-label.png") });
        out[module] = {
          row,
          hint,
          hidden:
            all.includes(`hidden Legal Only ${runTag}`) ||
            all.includes(`hidden Internal team ${runTag}`),
          labels: /Legal Only|Internal team/.test(all),
        };
        check(
          new RegExp(label).test(row) &&
            hint === `Visible to Legal and all ${module} team members.`,
      ...
      return {
        page: `/portal/matters/${MR}`,
        actual: `Matter reply row "${out.Matter.row.slice(0, 70)}", composer text "${out.Matter.hint}"; Contract reply row "${out.Contract.row.slice(0, 70)}", composer text "${out.Contract.hint}". Legal Only and Working Team comments on M-${MR} did not appear. See contributor-reply-label.png.`,
      };
    },
  );
  await step(
    ...
    BU,
    "G13.history",
    "Legal changes the Contract and closes the Matter with a closing note one item at a time; open History each time.",
    ...
    async () => {
      ...
      const verdict = historyVerdict(probe);
      const closed = (
        await call(await admin(), "GET", "/matter-statuses")
      ).body.matterStatuses.find((s) => s.category === "closed" && !s.archivedAt);
      const before = await portalActivity(R, "matter", fx.mr.id);
      const note = `DOC-030 access closing note ${runTag}`;
      const mv = await call(nadiaApi, "PATCH", `/matters/${MR}`, {
        statusId: closed.id,
        closingNote: note,
      });
      await sleep(800);
      ...
      check(
        mv.status === 200 &&
          ...
          !panel.includes(note) &&
          /DOC-030 access probe task A/.test(probe.panelText) &&
          /due Dec 15/.test(probe.panelText),
        JSON.stringify({
          mv: mv.status,
          panel: panel.slice(0, 200),
          history: probe.panelText.slice(0, 300),
        }),
      );
      return {
        page: `/portal/contracts/${CR}`,
        actual: `${verdict}. Contract History panel (newest first): "${probe.panelText.slice(0, 700)}". Closing M-${MR} as "${closed.displayName}" with a closing note added ${closeEntries.length} History entry (${closeEntries.map((e) => e.action).join(", ")}); the Matter panel read "${panel.slice(0, 200)}" and did not contain the note. See contributor-portal-history.png. Legal reopened the Matter.`,
      };
    },
  );
  await step(
    ...
    BU,
    "G14.team-add",
    "Open Matter team; Add team member; choose an existing person; Add. Then open a Confidential record's team.",
    "Existing members are excluded; the person joins the same team as in the full app; Business Users added gain Portal access; Confidential records: ask Legal; no remove control.",
    async () => {
      await go(R, `/portal/matters/${MR}`);
      const { options } = await addTeamMember(R, "Matter team", "Amara Nwosu");
      const removes = await (
        await applet(R, "Matter team")
      )
        .getByRole("button", { name: /off the/ })
        .count();
      const A = (await portal("amara")).page;
      ...
      check(
        ...
        JSON.stringify({ options: options.length, legal, removes, cadd, cnote }),
      );
      return {
        page: `/portal/matters/${MR}`,
  ...
  await step(
    ...
    BU,
    "G15.legal-limits",
    "Look for controls for Fields, Description, Value, dates, Status, Type, owners, parties, confidentiality, relationships, Tasks, Key dates, approval requests, signatures and team removal.",
    "These stay with Legal; Stage and Status moves are unavailable.",
    async () => {
      const out = [];
      for (const url of [`/portal/contracts/${CR}`, `/portal/matters/${MR}`]) {
        ...
        out.push({
          ...
          sections: ["Tasks", "Key dates"].filter((w) => main.includes(w)),
        });
      }
      const write = await pageApi(R, "PATCH", `/contracts/${CR}`, {
        title: "DOC-030 access probe",
      });
      ...
      return {
        page: `/portal/contracts/${CR}`,
        actual: `Neither Portal record offered Stage or Status moves, owner, party, confidentiality, relationship, Task, Key date, approval, signing or team-removal controls. Direct record writes answered ${write.status} and ${stage.status}.`,
      };
    },
  );
  await step(
    CG,
    BU,
    "G16.close-end-archive-remove",
    "Legal closes the Matter and ends the Contract; then archives and restores the Matter; then removes Ravi from the Matter team.",
    "Closing or Ending keeps permitted Portal work; archiving removes the record from the Portal; removing Ravi's reach stops access.",
    async () => {
      const closed = (
        await call(await admin(), "GET", "/matter-statuses")
      ).body.matterStatuses.find((s) => s.category === "closed" && !s.archivedAt);
      ...
      const mv = await call(nadiaApi, "PATCH", `/matters/${MR}`, {
        statusId: closed.id,
        closingNote: "DOC-030 access fictional closing note.",
      });
      ...
      const ended = (
        await call(await admin(), "GET", "/contract-statuses")
      ).body.contractStatuses.find((s) => s.stage === "ended" && !s.archivedAt);
      const end = await call(nadiaApi, "PATCH", `/contracts/${CR}`, {
        statusId: ended.id,
      ...
      const commentEnded = await (
        ...
        .isEditable()
        .catch(() => false);
      const back = await call(nadiaApi, "PATCH", `/contracts/${CR}`, {
        statusId: curStatus,
        overrideSoftGate: true,
      });
      ...
      const reopen = await call(nadiaApi, "PATCH", `/matters/${MR}`, {
        statusId: open.id,
        confirmReopen: true,
      });
      await go(N, `/matters/${MR}`);
  ...
  await step(
    ...
    BU,
    "G17.approvals",
    "Select Approvals in the Portal navigation bar; search by Contract title and by requester name; open a request; enter Note (optional); Approve; then Reject another; open Completed.",
    "Your approvals lists Pending requests; the request shows the primary Document; Approve or Reject; a decision is final; completed decisions appear under Completed; only the named approver can decide; the request does not add you to the team.",
    async () => {
      const make = async (k) => {
        const c = (
          await call(nadiaApi, "POST", "/contracts", {
            title: `DOC-030 access Ravi ${k} approval ${runTag}`,
            contractTypeId: fx.types.nda,
            managerId: fx.ids.nadia,
          })
        ).body.contract;
      ...
      const jonasTry = await pageApi(
        (await portal("jonas")).page,
        "POST",
        `/portal/approvals/${ap.id}/decision`,
        { decision: "approved" },
      );
      ...
      const again = await pageApi(R, "POST", `/portal/approvals/${ap.id}/decision`, {
        decision: "rejected",
      });
      ...
      check(
        ...
        JSON.stringify({ byTitle: byTitle.slice(0, 200) }),
      );
      check(
        packet.includes(`doc030-access-approve-paper-${runTag}.txt`) &&
          jonasTry.status >= 400 &&
          buttonsLeft === 0 &&
          again.status === 409 &&
