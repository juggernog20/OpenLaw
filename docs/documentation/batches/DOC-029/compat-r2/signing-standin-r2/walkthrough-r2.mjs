...
const results = {
  ...
  providerMode:
    "isolated local DocuSign protocol stand-in (rewalk/signing/server.mjs, a copy of signing-standin/fixture/server.mjs that also keeps posted Signer tabs); no real provider account",
  stamp: STAMP,
...
async function main(browser) {
  ...
  for (const flow of flows) {
    ...
    flow.title = title;
    flow.v1 = fictionalPdf(
      `${flow.role} primary version 1`,
      " Signature of Signer One: /sig/ Signature of Signer Two: /sig/",
    );
  ...
  for (const flow of flows) {
    ...
    await step(
      role,
      sender.person.name,
      "Before you start (/sig/ paragraph): the paper to send is marked with /sig/ before its Document Version is uploaded; the article paragraph is present in the reviewed bytes",
      async () => {
        const text = readFileSync(
          path.join(root, "docs/user-guides/electronic-signing.md"),
          "utf8",
        );
        expectThat(
        ...
        const dl = await sender.page.request.get(
    ...
    await step(
      ...
      async () => {
        ...
        const sent = await sendEnvelope(page, number, {
          ...
          subject: `DOC-029r2 signing-standin ${role} decline round`,
        });
    ...
    await step(
      ...
      async () => {
        ...
        const sent = await sendEnvelope(page, number, {
          ...
          subject: `DOC-029r2 signing-standin ${role} completion round`,
        });
