// DOC-029r2 inbox round 2 compatibility replay of the round 1 script: fictional Requests submitted by Jonas Weber through the Portal API.
// Each Request is named "DOC-029r2 inbox <role> <purpose> <stamp>". After submission the
// "Old Holdings" Entity is archived, so the conversion Requests carry an archived reference.
// Run: LAB_PASSWORD=... mise exec -- node docs/documentation/batches/DOC-029/compat-r2/inbox-r2/setup-requests-r2.mjs [purposes...]
...
const outPath = path.join(here, "requests-r2.json");
...
for (const [role, label] of Object.entries(roles)) {
  for (const purpose of wanted) {
    ...
    const title = `DOC-029r2 inbox ${label} ${purpose} ${stamp}`;
    const customFields = {
