...
export async function run(log, fx) {
  ...
  const home = must(
    await api(admin.page, "POST", "/contracts", {
      title: `DOC-029r2 access Home contract ${stamp}`,
      contractTypeId: nda,
  ...
  const conf = must(
    await api(admin.page, "POST", "/contracts", {
      title: `DOC-029r2 access confidential contract ${stamp}`,
      contractTypeId: nda,
      managerId: uid(SEED.daniel.email),
      isConfidential: true,
    }),
  ...
  fx.created.push(
    `Contract C-${home.number} DOC-029r2 access Home contract ${stamp}`,
    `Confidential Contract C-${conf.number} DOC-029r2 access confidential contract ${stamp} (Daniel's team only)`,
  );
