...
async function submitPhase() {
  ...
  await step(A, R, "Complete the form and Submit request once; keep the confirmation open", "Confirmation names the R- reference, shows Attaching your files while uploads run, reports no failure, and offers Open request", async () => {
    ...
    await page.getByLabel(/^Description/).fill("DOC-029 portal: Northstar proposes a 60-day evaluation. Counterparty Northstar Evaluation Ltd. Please review the evaluation terms.");
  ...
  await step(A, R, "An attachment fails: the Request still exists and the failure is named", "Confirmation reports the named file did not attach and keeps the R- reference; the Request is saved and the file can go on a reply", async () => {
    ...
    await panel.getByRole("textbox", { name: "New comment" }).fill(`DOC-029 portal: the file that did not attach to R-${failedNumber}.`);
  ...
  await step(A, R, "Legal decides the Request while its files upload", "Confirmation links to a reply on the same Request; the link opens its composer; the submission stays saved", async () => {
    ...
    await page.getByLabel(/^Description/).fill("DOC-029 portal: Legal declines while the file uploads.");
    const [chooser] = await Promise.all([page.waitForEvent("filechooser"), page.getByRole("button", { name: "Choose files" }).click()]);
    await chooser.setFiles([await pdfFile(raceName)]);
    await page.route("**/api/v1/requests/*/attachments", async (route) => {
      const n = Number(new URL(route.request().url()).pathname.split("/")[4]);
      must(await L.api(nadia, "POST", `/requests/${n}/decline`, { reason: "DOC-029 portal: declined during upload." }), "decline");
      await route.continue();
    });
    await page.getByRole("button", { name: "Submit request" }).click();
    const heading = page.getByRole("heading", { name: /^Request R-\d+ is with Legal$/ });
    ...
    await panel.getByRole("textbox", { name: "New comment" }).fill(`DOC-029 portal: the file that did not attach to R-${n}.`);
    const [fc] = await Promise.all([page.waitForEvent("filechooser"), panel.getByRole("button", { name: "Attach files" }).click()]);
  ...
  await step(A, R, "Submission reports an error without confirmation; Your requests shows no new Request", "Error text shows, answers stay, and no Request was saved", async () => {
    ...
    await page.getByLabel(/^Description/).fill("DOC-029 portal: the submission does not reach the server.");
...
async function followPhase() {
  ...
  await step(A, R, "Setup: Jonas has Requests to follow (created through the API; the form itself is V-C09)", "Six Requests with one original PDF each", async () => {
    ...
    for (const [key, slug, customFields] of plan) {
      ...
      const r = must(await L.api(page, "POST", "/requests", { requestTypeId: bySlug[slug].id, departmentId: engineering.id, title, description: `DOC-029 portal description for ${key}.`, urgency: "high", customFields }), `request ${key}`).request;
  ...
  await step(A, "legal_team_member", "Setup: Legal replies at every audience before any decision, and assigns the open Request", "Full thread (with paper), Legal only, and Working team comments post on each Request; R-open is assigned to Nadia", async () => {
    ...
    for (const key of ["open", "contract", "matter", "resolved", "declined"]) {
      ...
      legal[key] = { full: `DOC-029 portal Legal full thread ${tag}`, legalOnly: `DOC-029 portal Legal only note ${tag}`, working: `DOC-029 portal Working team note ${tag}`, file: `doc029-legal-${key}-${runStamp}.pdf` };
  ...
  await step(A, "legal_team_member", "Setup: Legal converts, resolves, and declines through the API", "Contract and Matter conversions, a resolution with a reply, and a decline with a reason succeed", async () => {
    ...
    must(await L.api(nadia, "POST", `/requests/${Q.resolved.number}/resolve`, { reply: `DOC-029 portal resolution reply ${runStamp}` }), "resolve");
    must(await L.api(nadia, "POST", `/requests/${Q.declined.number}/decline`, { reason: `DOC-029 portal decline reason ${runStamp}` }), "decline");
  ...
  await step(A, "legal_team_member", "Setup: Legal posts later replies with paper after the decisions, and 55 extra replies on R-older", "Later Full thread replies and Legal only notes post on the Requests and the converted records", async () => {
    ...
    for (let i = 1; i <= 55; i++) await post("request", Q.older.id, "full_thread", `DOC-029 portal older marker ${String(i).padStart(2, "0")} ${runStamp}`);
  ...
  for (const key of ["open", "resolved", "declined"]) {
    ...
    await step(A, R, `R-${key}: open from Your requests; check title, reference, status, Owner, and banner`, "Title, R- reference, requester status (Open/Resolved/Declined), Owner line, and banner text; decline reason under the status; reply link on decided Requests only", async () => {
      ...
      if (key === "declined") expect(banner.includes(`DOC-029 portal decline reason ${runStamp}`), banner);
    ...
    await step(A, R, `R-${key}: Comments shows Legal's shared replies and paper only`, "Full thread replies and their files show; Legal only and Working team notes do not; no audience choice", async () => {
      ...
      if (key === "resolved") expect(body.includes(`DOC-029 portal resolution reply ${runStamp}`), "resolution reply missing");
    ...
    await step(A, R, `R-${key}: What you submitted shows the saved answers and the original attachment`, "Description, original attachment (downloadable, same bytes), Department, Urgency, and answered questions", async () => {
      const card = page.getByRole("region", { name: "What you submitted" });
      const body = await text(card);
      for (const want of [`DOC-029 portal description for ${key}.`, q.original, "Engineering", "High"]) expect(body.includes(want), `missing ${want}: ${body}`);
  ...
  for (const key of ["contract", "matter"]) {
    ...
    await step(A, R, `R-${key}: the converted Request address opens the resulting ${key}`, `Redirect to /portal/${key}s/<n>; Original request shows the ask; Jonas is Business Owner on the team; Comments shows Full thread replies only; a reply posts on the record`, async () => {
      ...
      expect(orig.includes(`DOC-029 portal description for ${key}.`), `original: ${orig.slice(0, 300)}`);
  ...
  await step(A, R, "Show older loads earlier replies", "The conversation first shows the newest page; Show older adds the earlier replies", async () => {
    ...
    const panel = await openComments(page);
    await panel.getByText(`DOC-029 portal older marker 55 ${runStamp}`).waitFor({ timeout: 20000 });
    const before = await panel.getByText(new RegExp(`^DOC-029 portal older marker \\d\\d ${runStamp}$`)).count();
    ...
    await panel.getByText(`DOC-029 portal older marker 01 ${runStamp}`).waitFor({ timeout: 20000 });
    const after = await panel.getByText(new RegExp(`^DOC-029 portal older marker \\d\\d ${runStamp}$`)).count();
...
async function knowledgePhase() {
  ...
  await step(A, R, "A broken external link opens in a new tab and does not block the Request form", "The external link opens a new tab; the original tab stays on the Portal; a Request can still be submitted", async () => {
    ...
    await page.getByLabel(/^Description/).fill("DOC-029 portal: the external website did not load.");
    await page.getByRole("button", { name: "Submit request" }).click();
