// DOC-029 round 2 independent operator walkthrough, group "operator-install", round 1.
// Scenarios V-C44 (install.md) and V-C45 (deployment-configuration.md), method container-operation,
// against images built from 57e77e38 by the article's own commands. Based on
// rewalk/operator/walkthrough-rw.mjs and operator-install/walkthrough-r1.mjs. Round 2 changes: the
// welcome wizard's Outbound email step collects discrete SMTP fields, and the new "Deploy on a
// private VM" section is walked in the phases private-overlay, private-proxy and private-origin.
//
// Run one phase at a time from the documentation worktree root:
//   mise exec -- node docs/documentation/batches/DOC-029/operator-install/walkthrough-r2-1.mjs <phase>
//
// Private state (the clone, its .env, fixture passwords, browser traces) lives outside the
// repository in WORK. The sanitized log is walkthrough-r2-1.json beside this script. Every attempt
...
const COMMIT = "57e77e386be31b2a319f7143dd54d00123e65efe";
const WORK = "/tmp/claude-1000/doc029r2-opinstall-1";
const PRIVATE = path.join(WORK, "private");
const CLONE = path.join(WORK, "openlaw");
const FIX = path.join(WORK, "fixtures");
const PROJECT = "doc029r2-opinst1";
const FX_PROJECT = "doc029r2-opinst1-fx";
...
const PRIVATE_PORT = 23327;
const PRIVATE_HTTP_PORT = 23328;
const PRIVATE_BIND = "172.17.0.1";
const PRIVATE_HOST = "openlaw-r2.company.example";
const PRIVATE_ORIGIN = `https://${PRIVATE_HOST}:${PRIVATE_PORT}`;
// Stand-in for internal DNS: Node-side requests (Playwright API contexts) resolve the private
// hostname to the private bind address, as the browser does through --host-resolver-rules.
{
  const dns = require("node:dns");
  const original = dns.lookup;
  dns.lookup = function (hostname, options, callback) {
    if (hostname === PRIVATE_HOST) {
      const cb = typeof options === "function" ? options : callback;
      const all = typeof options === "object" && options?.all;
      return process.nextTick(() =>
        all ? cb(null, [{ address: PRIVATE_BIND, family: 4 }]) : cb(null, PRIVATE_BIND, 4),
      );
    }
  ...
  dns.promises.lookup = async function (hostname, options) {
    if (hostname === PRIVATE_HOST) {
      if (options?.family === 6)
        return options?.all
          ? []
          : Promise.reject(Object.assign(new Error("no IPv6"), { code: "ENOTFOUND" }));
      return options?.all
        ? [{ address: PRIVATE_BIND, family: 4 }]
        : { address: PRIVATE_BIND, family: 4 };
    }
    return originalPromise.call(this, hostname, options);
  };
}
const LOCAL = `http://127.0.0.1:${APP_PORT}`;
const ORIGIN = `https://openlaw-r2.localhost:${PROXY_PORT}`;
const LOG = path.join(here, "walkthrough-r2-1.json");
const REVIEWER = "DOC-029r2 independent walkthrough agent (operator-install, round 1)";
...
const log = existsSync(LOG)
  ...
  : {
      ...
      round: "2-1",
      basedOn: "docs/documentation/batches/DOC-029/rewalk/operator/walkthrough-rw.mjs",
      reviewer: REVIEWER,
      reviewerKind: "agent",
      appCommit: COMMIT,
...
async function openBrowser() {
  browser ??= await chromium.launch({
    headless: true,
    args: [
      `--host-resolver-rules=MAP openlaw-r2.localhost 127.0.0.1, MAP ${PRIVATE_HOST} ${PRIVATE_BIND}`,
    ],
  });
  return browser;
}
...
phases.install = async () => {
  ...
  await step(
    ...
    "A detached checkout at 57e77e386be31b2a319f7143dd54d00123e65efe.",
    "container-operation",
    () => {
...
openlaw-r2.localhost:${PROXY_PORT} {
...
`,
  ...
  await step(
    ...
    async () => {
      ...
      await page.screenshot({ path: path.join(here, "r2-1-setup-through-proxy.png") });
      await context.close();
      return `GET ${ORIGIN} answered ${status} through Caddy, and the page shows the "Set up OpenLaw" heading. Screenshot r2-1-setup-through-proxy.png.`;
    },
  );
};
  ...
  try {
    ...
    await step(
      ...
      async () => {
        ...
        await page.getByLabel("Sender name (optional)").fill("DOC-029r2 operator-install");
        await page.getByLabel("Sender email").fill("openlaw@doc029-install.example");
        ...
        return `Filled SMTP server relay, Connection security None, Port 1025, Authentication None, Sender name "DOC-029r2 operator-install" and Sender email openlaw@doc029-install.example. Save relay stored it; Send test email delivered "OpenLaw test email" to ${ADMIN.email} at the owned relay; Continue became available.`;
      },
    );
    await step(
      "V-C44",
      ...
      async () => {
        ...
        let status = {};
        for (let i = 0; i < 60 && status.completed !== true; i++) {
          await sleep(500);
          status = await json(await page.request.get(`${ORIGIN}/api/v1/onboarding`));
        }
...
phases["firstrun-reset"] = async () => {
  await step(
    "V-C44",
    "Reviewer reset after the failed first attempt: empty the installation database before repeating first-run setup",
    "The bundled database is empty again and the origin shows Set up OpenLaw.",
    "container-operation",
    async () => {
      const stop = compose("stop app worker");
      const drop = compose(
        `exec -T postgres psql -U openlaw -d postgres -c "drop database openlaw with (force)" -c "create database openlaw owner openlaw"`,
      );
      const u = up();
      ...
      expect(
        ...
        `stop ${stop.code} drop ${drop.code} up ${u.code} ready ${ready} setup ${JSON.stringify(setup)}`,
      );
      return `The first browser attempt created the Administrator row and a session, but the browser's POST /api/v1/auth/setup never received an answer: the page showed "The server could not be reached. Try again." and the app logged the incoming request with no completion. No proxy error was logged. A reviewer probe with curl through the same proxy on an emptied database answered 201 in 0.09 s. To repeat the article step from an empty instance, the reviewer stopped app and worker, dropped and recreated the openlaw database, and ran up -d --no-build --pull never again (exit 0, /readyz 200, needsSetup ${setup.needsSetup}). This is a reviewer reset, not an article step.`;
    },
  );
};
  ...
  await step(
    ...
    async () => {
      ...
      try {
        ...
        await page.screenshot({ path: path.join(PRIVATE, "invite-link.png") }).catch(() => {});
        await context.close();
      }
...
phases["checks-invite2"] = async () => {
  await step(
    "V-C44",
    "Confirm a test invitation's link is usable (retry with a new test invitation)",
    "A new invitation sent through the configured relay links to the origin; the link sets a password and the recipient signs in. The first link's password also works.",
    "browser-walkthrough",
    async () => {
      const rowan = await apiClient(ORIGIN, COLLEAGUE.email, state.colleaguePassword);
      await rowan.ctx.dispose();
      const invitee = `doc029-r2-install-check-${Date.now()}@doc029-install.example`;
      const since = new Date(Date.now() - 1000).toISOString();
      const r = await inviteThroughSettings(invitee, "DOC-029r2 install check invitee", null);
      ...
      secret("installInviteePassword");
      const { page, context } = await newSession();
      try {
        await page.goto(link.toString());
        await page.getByLabel("New password").fill(state.installInviteePassword);
        await page.getByLabel("Confirm password").fill(state.installInviteePassword);
        await page.getByRole("button", { name: "Set password" }).click();
        await page.getByText("Password set").first().waitFor({ timeout: 30_000 });
      } finally {
        await page.screenshot({ path: path.join(PRIVATE, "invite-link-2.png") }).catch(() => {});
        await context.close();
      }
      const c = await apiClient(ORIGIN, invitee, state.installInviteePassword);
      await c.ctx.dispose();
      expect(c.signInStatus === 200, `invitee sign-in ${c.signInStatus}`);
      return `The first attempt had stored ${COLLEAGUE.name}'s password although the page did not show "Password set": sign-in with that password now answers ${rowan.signInStatus}, and reopening the used link showed "This link has expired or was already used. Ask for a new one." A reviewer probe afterwards opened five further invitation links through the same proxy in fresh browser contexts, and each showed "Password set" within 0.2 s over h2, so the lost answer did not reproduce. Retry: Settings -> Users -> Invite user sent a new test invitation (${r.inviteStatus}); "${msg.Subject}" reached the relay with a link on ${link.origin}. Opening it showed "Password set", and the new recipient signed in (${c.signInStatus}).`;
    },
  );
};
...
async function liveUpdate(ORIGIN) {
...
async function testSend() {
  ...
  const put = await api.put("/api/v1/email-settings", {
    data: {
      ...
      senderName: "DOC-029r2 operator-install",
      senderEmail: "openlaw@doc029-install.example",
    },
  });
  await api.dispose();
  return {
...
// ---------------------------------------------------------------- corrected email paragraph

/** Read the Mailpit message list newest first (ids, recipients, subject, from). */
async function mailList() {
  const r = await fetch(`${MAIL_UI}/api/v1/messages?limit=200`).then((x) => x.json());
  return r.messages ?? [];
}
function appLogLines(since, pattern) {
  return appLogs("app", since)
    .split("\n")
    ...
    .map((l) => {
      ...
      try {
        ...
        return `level ${j.level}: ${j.err?.message ?? j.msg ?? ""}`.slice(0, 160);
      } catch {
        return body.slice(0, 160);
      }
    });
}
/** Invite through Settings -> Users -> Invite user and read the list row before and after reload. */
async function inviteThroughSettings(email, displayName, shotName, base = ORIGIN) {
  const s = await signIn(base, ADMIN.email, state.adminPassword);
  try {
    const page = s.page;
    ...
    const rowBefore = await rowText();
    if (shotName) await page.screenshot({ path: path.join(here, shotName) });
    await page.reload();
    await page.getByRole("button", { name: "Invite user" }).waitFor({ timeout: 20_000 });
    const rowAfter = await rowText();
    const users = await json(await page.request.get(`${base}/api/v1/users`));
    const apiUser = users.users.find((u) => u.email === email);
    return { inviteStatus, dialogOpen, alerts, rowBefore, rowAfter, apiStatus: apiUser?.status };
  } finally {
    await s.context.close();
  }
}

phases["mail-no-from"] = async () => {
  const noFrom = async () => {
    setEnv({ SMTP_URL: "smtp://relay:1025", SMTP_FROM: null });
    const r = up();
    const ready = await waitReady();
    expect(r.code === 0 && ready === 200, `up ${r.code}, ready ${ready}`);
  };
  await noFrom();
  await step(
    "V-C45",
    "SMTP_URL set to the working relay, SMTP_FROM unset: the test email reports that email is unavailable",
    "Source stays env with no From address; saving a relay in the app is refused; the test email is refused with an email-unavailable message; nothing reaches the relay.",
    "container-operation",
    async () => {
      const before = await mailCount();
      const t = await testSend();
      await sleep(3000);
      const after = await mailCount();
      expect(t.source === "env" && t.from === null, `source ${t.source}, from ${t.from}`);
      expect(t.putStatus === 409 && t.status >= 400 && before === after, JSON.stringify(t));
      return `SMTP_URL=smtp://relay:1025 (the working relay) and no SMTP_FROM; containers recreated. email-settings source "${t.source}", fromAddress ${t.from}. Saving an app relay answered ${t.putStatus}. The test email answered ${t.status} "${t.detail}". The relay received ${after - before} messages.`;
    },
  );
  await step(
    "V-C45",
    "SMTP_URL without SMTP_FROM: a sign-in link request reports that email is unavailable",
    "The sign-in page's Email me a sign-in link shows that links cannot be sent; the API refuses; nothing reaches the relay.",
    "browser-walkthrough",
    async () => {
      const before = await mailCount();
      ...
      try {
        ...
        await page.getByLabel("Email").fill(COLLEAGUE.email);
        await page.getByRole("button", { name: "Send link" }).click();
        await page.getByRole("alert").first().waitFor({ timeout: 15_000 });
        shown = (await page.getByRole("alert").allInnerTexts()).join(" | ").trim();
      } finally {
        await context.close();
      }
      await sleep(3000);
  ...
  await step(
    "V-C45",
    "SMTP_URL without SMTP_FROM: a password-setup request reports that email is unavailable",
    "Set up or reset your password -> Send password setup link shows that email is not configured; the API refuses; nothing reaches the relay.",
    "browser-walkthrough",
    async () => {
      const before = await mailCount();
      const probe = await pwRequest.newContext({
  ...
  await step(
    "V-C45",
    "SMTP_URL without SMTP_FROM: an invitation does not report the problem; Settings -> Users -> Invite user lists the person as Invited and no email is sent (#889)",
    "The invitation is accepted with no email-unavailable report; the Users list shows the person as Invited before and after reload; the relay receives nothing.",
    "browser-walkthrough",
    async () => {
      const since = new Date().toISOString();
      const before = await mailCount();
      const invitee = `doc029-r2-no-from-${Date.now()}@doc029-install.example`;
      const r = await inviteThroughSettings(
        invitee,
        "DOC-029r2 no-From invitee",
        "r2-1-invite-without-smtp-from.png",
      );
      await sleep(5000);
      const after = await mailCount();
      ...
      return `Settings -> Users -> Invite user -> Send invite: POST /api/v1/auth/invites answered ${r.inviteStatus}; the dialog closed; alerts on the page: "${r.alerts || "none"}". The row read "${r.rowBefore}" and after reload "${r.rowAfter}"; the API lists status ${r.apiStatus}. The relay received ${after - before} messages (${toInvitee} to the invitee). App log: ${logs || "no mail line"}. Screenshot r2-1-invite-without-smtp-from.png. This matches the article sentence and is tracked as bug #889.`;
    },
  );
  await step(
    "V-C45",
    "SMTP_URL without SMTP_FROM: an Administrator cannot finish the welcome wizard",
    "On a fresh database under the same environment, the wizard's Outbound email step warns that SMTP_FROM is missing, Finish on Review does not complete onboarding, and a direct completion request is refused.",
    "browser-walkthrough",
    async () => {
      const fx = (cmd) => sh(`docker compose exec -T extdb ${cmd}`, { cwd: FIX });
      fx("dropdb -U doc029ext --if-exists doc029wiz");
      const created = fx("createdb -U doc029ext doc029wiz");
      expect(created.code === 0, `createdb exited ${created.code}: ${firstLines(created.stderr)}`);
      setEnv({ DATABASE_URL: `postgres://doc029ext:${state.extdbPassword}@extdb:5432/doc029wiz` });
      const r = up();
      const ready = await waitReady();
      secret("wizardAdminPassword");
      const { page, context } = await newSession();
      let result;
      try {
        expect(r.code === 0 && ready === 200, `up ${r.code}, ready ${ready}`);
        await page.goto(ORIGIN);
        await page.getByRole("heading", { name: "Set up OpenLaw" }).waitFor({ timeout: 30_000 });
        await page.getByLabel("Name", { exact: true }).fill("DOC-029 Wren Wizard");
        await page.getByLabel("Email", { exact: true }).fill("wren.wizard@doc029-install.example");
        ...
        await page
          ...
          .waitFor({ timeout: 30_000 });
        await page.getByRole("button", { name: "Get started" }).click();
        await page.getByLabel("Organization name").fill("DOC-029r2 wizard Organization");
        await page.getByRole("button", { name: "Continue" }).click();
        await page.getByText(/Step 3 of 9/).waitFor();
        await page.getByRole("button", { name: "Continue" }).click();
        await page.getByText(/Step 4 of 9/).waitFor();
        ...
        if (!contDisabled) {
          for (let n = 5; n < 9; n++) {
            const later = page.getByRole("button", { name: "Set up later" });
            if (n >= 6 && (await later.count())) await later.click();
        ...
        await page
          .screenshot({ path: path.join(here, "r2-1-wizard-finish-without-smtp-from.png") })
          .catch(() => {});
        result = {
          emailAlert,
          contDisabled,
      ...
      expect(result.completed === false && result.direct === 409, JSON.stringify(result));
      return `Created an empty database doc029wiz on the external Postgres, pointed DATABASE_URL at it with SMTP_URL set and SMTP_FROM unset, and recreated. Created the fictional Administrator DOC-029 Wren Wizard and walked the wizard to Outbound email (Step 5 of 9), which showed "${result.emailAlert}". Continue was ${result.contDisabled ? "disabled" : "enabled"}${result.walked.length ? `; the reviewer continued to Review (steps ${result.walked.join(", ")}) and selected Finish, which showed "${result.finishAlert}" and stayed at ${result.url}` : ""}. The onboarding state read completed=${result.completed}; a direct completion request answered ${result.direct} "${result.directDetail}". Screenshot r2-1-wizard-finish-without-smtp-from.png. DATABASE_URL was then removed and the containers recreated on the bundled database.`;
    },
  );
};

phases["mail-no-from-requests"] = async () => {
  setEnv({ SMTP_URL: "smtp://relay:1025", SMTP_FROM: null });
  const u = up();
  const ready = await waitReady();
  try {
    await step(
      "V-C45",
      "SMTP_URL without SMTP_FROM: sign-in link and password-setup requests report that email is unavailable (retry for the current sign-in page)",
      "Both API requests are refused with an email-unavailable answer and send nothing; the sign-in pages do not offer a request that cannot be sent.",
      "browser-walkthrough",
      async () => {
        expect(u.code === 0 && ready === 200, `up ${u.code}, ready ${ready}`);
        const before = await mailCount();
        const probe = await pwRequest.newContext({
          baseURL: ORIGIN,
          ignoreHTTPSErrors: true,
        ...
        const ml = await probe.post("/api/v1/auth/magic-link", {
          data: { email: COLLEAGUE.email },
        });
        const mlBody = await json(ml);
        const ps = await probe.post("/api/v1/auth/password-setup", {
          data: { email: COLLEAGUE.email },
        });
        const psBody = await json(ps);
        const methods = await json(await probe.get("/api/v1/auth/methods"));
        await probe.dispose();
        const read = async (p) => {
          const { page, context } = await newSession();
          try {
            ...
            for (const name of ["Set up or reset your password", "Email me a sign-in link"])
              if (await page.getByRole("button", { name }).count()) offered.push(name);
        ...
        const legal = await read("/auth/login");
        const portal = await read("/portal/login");
        await sleep(3000);
        const after = await mailCount();
        expect(
          ml.status() >= 400 && ps.status() >= 400 && before === after,
          `magic ${ml.status()}, setup ${ps.status()}, mail ${before}->${after}`,
        );
        expect(
          /email/i.test(`${mlBody.detail ?? ""} ${mlBody.title ?? ""}`) &&
            /email/i.test(`${psBody.detail ?? ""} ${psBody.title ?? ""}`),
          `magic "${mlBody.detail ?? mlBody.title}", setup "${psBody.detail ?? psBody.title}"`,
        );
        expect(
          legal.offered.length === 0 && portal.offered.length === 0,
          `offered ${JSON.stringify({ legal, portal })}`,
        );
        return `SMTP_URL set to the working relay and SMTP_FROM unset; recreated (/readyz 200). POST /api/v1/auth/magic-link answered ${ml.status()} "${mlBody.detail ?? mlBody.title ?? ""}". POST /api/v1/auth/password-setup answered ${ps.status()} "${psBody.detail ?? psBody.title ?? ""}". /api/v1/auth/methods reports emailConfigured=${methods.emailConfigured}. On /auth/login (after Sign in with a password, where shown) the page offered ${legal.offered.join(", ") || "neither Set up or reset your password nor Email me a sign-in link"}${legal.alerts ? `, alert "${legal.alerts}"` : ""}. On /portal/login it offered ${portal.offered.join(", ") || "neither"}${portal.alerts ? ` and showed "${portal.alerts}"` : ""}. The relay received ${after - before} messages. The first attempt's selectors expected those links on the page; this build hides them while email is not configured, so a person cannot start either request from the sign-in page.`;
      },
    );
  } finally {
...
phases["mail-no-from-password-setup"] = async () => {
  setEnv({ SMTP_URL: "smtp://relay:1025", SMTP_FROM: null });
  const u = up();
  const ready = await waitReady();
  const since = new Date().toISOString();
  try {
    await step(
      "V-C45",
      "SMTP_URL without SMTP_FROM: does a password-setup request report that email is unavailable?",
      "The article says password-setup requests report that email is unavailable.",
      "container-operation",
      async () => {
        const before = await mailCount();
        const probe = await pwRequest.newContext({
        ...
        const ps = await probe.post("/api/v1/auth/password-setup", {
          data: { email: `doc029-r2-setup-${Date.now()}@doc029-install.example` },
        });
        const text = await ps.text();
        await probe.dispose();
        await sleep(2000);
        const after = await mailCount();
        const logged = appLogLines(since, /password-setup|Email is not configured/)
          .filter((l) => /Email is not configured/.test(l))
          .slice(0, 1)
          .join("");
        const logMsg = appLogs("app", since).match(
          /"message":"(Email is not configured[^"]*)"[^}]*"statusCode":(\d+),"expose":(true|false)/,
        );
        const { page, context } = await newSession();
        let offered;
        try {
          ...
          offered = await page
            .getByRole("button", { name: "Set up or reset your password" })
            .count();
        } finally {
          await context.close();
        }
        const summary = `up ${u.code}, /readyz ${ready}. POST /api/v1/auth/password-setup answered ${ps.status()} with body ${text.slice(0, 160)}. The app log holds "${logMsg?.[1] ?? logged}" with statusCode ${logMsg?.[2]} and expose ${logMsg?.[3]}, so the message stays in the log and the answer says only "Internal server error". The sign-in page offered "Set up or reset your password": ${offered ? "yes" : "no"}. The relay received ${after - before} messages.`;
        expect(ps.status() >= 400 && /email/i.test(text), summary);
        return summary;
      },
    );
  } finally {
...
phases["mail-both"] = async () => {
  await step(
    "V-C45",
    "Set both SMTP_URL and SMTP_FROM, then invite a test address: the invitation reaches the relay",
    "Source env with the From address; Settings -> Users -> Invite user sends an invitation that arrives at the relay from SMTP_FROM, with its link on BASE_URL; the link sets a password through the proxy.",
    "browser-walkthrough",
    async () => {
      setEnv({ SMTP_URL: "smtp://relay:1025", SMTP_FROM: RELAY_FROM });
      const up1 = up();
      const ready = await waitReady();
      expect(up1.code === 0 && ready === 200, `up ${up1.code}, ready ${ready}`);
      const api = await adminApi();
      const settings = await json(await api.get("/api/v1/email-settings"));
      await api.dispose();
      const invitee = `doc029-r2-both-${Date.now()}@doc029-install.example`;
      const since = new Date(Date.now() - 1000).toISOString();
      const r = await inviteThroughSettings(invitee, "DOC-029r2 env invitee", null);
      const msg = await waitMail(invitee, null, since, 60_000);
      expect(
      ...
      const link = firstLink(msg);
      expect(link?.origin === ORIGIN, `link origin ${link?.origin}`);
      secret("envInviteePassword");
      const { page, context } = await newSession();
      try {
        await page.goto(link.toString());
        await page.getByLabel("New password").fill(state.envInviteePassword);
        await page.getByLabel("Confirm password").fill(state.envInviteePassword);
        await page.getByRole("button", { name: "Set password" }).click();
        await page.getByText("Password set").first().waitFor({ timeout: 30_000 });
      } finally {
        await context.close();
      }
      const c = await apiClient(ORIGIN, invitee, state.envInviteePassword);
      await c.ctx.dispose();
      expect(c.signInStatus === 200, `invitee sign-in ${c.signInStatus}`);
      return `Set SMTP_URL=smtp://relay:1025 and SMTP_FROM; recreated. email-settings source "${settings.source}", fromAddress "${settings.fromAddress}". Settings -> Users -> Invite user -> Send invite answered ${r.inviteStatus}; row "${r.rowBefore}". The relay received "${msg.Subject}" for the invitee from ${msg.From?.Address}; its link uses origin ${link.origin} (token not recorded). Opening the link through the proxy set a password, and the invitee signed in (${c.signInStatus}).`;
    },
  );
  await step(
    "V-C45",
...
const certDir = () => path.join(PRIVATE, "certs");
const privateCurl = (args) =>
  sh(
    `curl -sS --max-time 10 --cacert ${certDir()}/ca.pem --resolve ${PRIVATE_HOST}:${PRIVATE_PORT}:${PRIVATE_BIND} ${args}`,
    { cwd: WORK },
  );
const listeners = (port) =>
  sh(`ss -ltnH 'sport = :${port}' | awk '{print $4}' | sort -u`, { cwd: WORK })
    .stdout.trim()
    .split("\n")
    .filter(Boolean);

phases["private-overlay"] = async () => {
  await step(
    "V-C45",
    "Private VM: check Compose 2.24.4 or later and show why the overlay needs !override",
    "Compose is new enough. A loopback mapping added without !override keeps the base all-interfaces mapping; with !override only the loopback mapping remains.",
    "container-operation",
    () => {
      const v = sh("docker compose version --short").stdout.trim();
      const [a, b, c] = v.split(".").map(Number);
      const newEnough = a > 2 || (a === 2 && (b > 24 || (b === 24 && c >= 4)));
      const probe = (file, text) => {
        writeFileSync(path.join(CLONE, file), text);
        const r = compose(`-f compose.yml -f compose.operator.yml -f ${file} config --format json`);
        sh(`rm -f ${file}`);
        if (r.code !== 0) return { code: r.code, ports: null };
        const ports = (JSON.parse(r.stdout).services.app.ports ?? []).map(
          (p) => `${p.host_ip ?? "all"}:${p.published}->${p.target}`,
        );
        return { code: 0, ports };
      };
      const plain = probe(
        "doc029-probe-plain.yml",
        `services:\n  app:\n    ports:\n      - "127.0.0.1:${APP_PORT}:3000"\n`,
      );
      const over = probe(
        "doc029-probe-override.yml",
        `services:\n  app:\n    ports: !override\n      - "127.0.0.1:${APP_PORT}:3000"\n`,
      );
      expect(newEnough, `Compose ${v}`);
      expect(
        plain.ports?.some((p) => p.startsWith("all:")),
        `plain ${JSON.stringify(plain)}`,
      );
      expect(
        over.ports?.length === 1 && over.ports[0].startsWith("127.0.0.1:"),
        `override ${JSON.stringify(over)}`,
      );
      return `docker compose version --short is ${v} (2.24.4 or later). Resolving only the app port list (the rest of the expanded configuration was not printed or recorded): with a plain added loopback mapping the app ports were [${plain.ports.join(", ")}], so the all-interfaces mapping stayed; with ports: !override they were [${over.ports.join(", ")}]. The probe files were deleted.`;
    },
  );
  await step(
    "V-C45",
    "Private VM: create compose.private.yml, update COMPOSE_FILE, BASE_URL and PORT, and check that restart does not apply them",
    "The overlay file exists; config --quiet passes; docker compose restart leaves the old BASE_URL in app and worker.",
    "container-operation",
    async () => {
      // Host port 3000 already belongs to another local service on this host, so the article's
      // "change that mapping and the proxy upstream together" path is followed with APP_PORT.
      writeFileSync(
        path.join(CLONE, "compose.private.yml"),
        `services:\n  app:\n    ports: !override\n      - "127.0.0.1:${APP_PORT}:3000"\n`,
      );
      setEnv({
      ...
      await waitReady();
      const appUrl = compose("exec -T app printenv BASE_URL").stdout.trim();
      const wUrl = compose("exec -T worker printenv BASE_URL").stdout.trim();
      expect(q.code === 0 && q.stdout === "", `config --quiet ${q.code}`);
      expect(
        r.code === 0 && appUrl === ORIGIN && wUrl === ORIGIN,
        `after restart app ${appUrl}, worker ${wUrl}`,
      );
      return `Wrote compose.private.yml with ports: !override and the single mapping "127.0.0.1:${APP_PORT}:3000". Host port 3000 is already used by an unrelated local service on this host, so the reviewer used the article's rule to change the mapping and the proxy upstream together (upstream 127.0.0.1:${APP_PORT}). Set COMPOSE_FILE=compose.yml:compose.operator.yml:compose.private.yml, BASE_URL=${PRIVATE_ORIGIN} (the placeholder hostname with the fixture's HTTPS port) and PORT=3000 as the article shows. The project name, image selection, volumes and keys stayed unchanged. config --quiet exited 0 with no output. docker compose restart app worker exited 0, and both containers still had BASE_URL ${appUrl}.`;
    },
  );
};

phases["private-apply"] = async () => {
  await step(
    "V-C45",
    "Private VM: validate and apply with up -d --no-build --pull never; the app port is on loopback only",
    "App and worker get the new BASE_URL; the app publishes only 127.0.0.1:<port>; nothing publishes host port 3000; the direct port is not reachable on other host addresses; records and images are unchanged.",
    "container-operation",
    async () => {
      const q = compose("config --quiet");
      const u = up();
      ...
      const wUrl = compose("exec -T worker printenv BASE_URL").stdout.trim();
      const bindings = sh(
        `docker inspect --format '{{json .HostConfig.PortBindings}}' $(docker compose ps -q app)`,
      ).stdout.trim();
      const img = sh(
        `docker inspect --format '{{.Config.Image}}' $(docker compose ps -q app)`,
      ).stdout.trim();
      const appListen = listeners(APP_PORT);
      const port3000 = sh(
        `docker ps --filter label=com.docker.compose.project=${PROJECT} --format '{{.Ports}}'`,
      ).stdout;
      const other = [PRIVATE_BIND, "192.168.0.59"].map((ip) => {
        const c = sh(
          `curl -sS --max-time 5 -o /dev/null -w '%{http_code}' http://${ip}:${APP_PORT}/readyz`,
          { cwd: WORK },
        );
        return `${ip}:${APP_PORT} curl exit ${c.code}${c.stdout && c.stdout !== "000" ? ` HTTP ${c.stdout}` : ""}`;
      });
      expect(
        q.code === 0 && u.code === 0 && ready === 200,
        `config ${q.code}, up ${u.code}, ready ${ready}`,
      );
      expect(
        appUrl === PRIVATE_ORIGIN && wUrl === PRIVATE_ORIGIN,
        `BASE_URL app ${appUrl} worker ${wUrl}`,
      );
      expect(
        appListen.length === 1 && appListen[0] === `127.0.0.1:${APP_PORT}`,
        `listeners ${appListen}`,
      );
      expect(
        !/:3000->/.test(port3000.replace(/127\.0\.0\.1:23320->3000/g, "")),
        `ports ${port3000}`,
      );
      expect(
        other.every((o) => !/HTTP/.test(o)),
        other.join("; "),
      );
      expect(img === `openlaw-local:${COMMIT}`, `image ${img}`);
      return `config --quiet exited 0; up -d --no-build --pull never exited 0 and recreated the containers; /readyz on 127.0.0.1:${APP_PORT} answered 200. BASE_URL is now ${appUrl} in the app and ${wUrl} in the worker. The app's port bindings are ${bindings}; the only host listener on port ${APP_PORT} is ${appListen.join(", ")}; no project container publishes host port 3000. Direct requests to the app port on other host addresses failed: ${other.join("; ")}. The app still runs ${img} on the same volumes.`;
    },
  );
};

phases["private-proxy"] = async () => {
  await step(
    "V-C45",
    "Private VM: install a certificate from an owned private CA and validate the adapted Caddyfile",
    "The certificate names the hostname and chains to the CA; caddy validate accepts the Caddyfile.",
    "container-operation",
    () => {
      const d = certDir();
      ...
      const gen = sh(
        [
          `openssl req -x509 -newkey rsa:2048 -nodes -days 2 -subj '/CN=DOC-029r2 fictional corporate CA' -keyout ca-key.pem -out ca.pem`,
          `openssl req -newkey rsa:2048 -nodes -subj '/CN=${PRIVATE_HOST}' -keyout openlaw-key.pem -out leaf.csr`,
          `printf 'subjectAltName=DNS:${PRIVATE_HOST}\\nextendedKeyUsage=serverAuth\\n' > ext.cnf`,
          `openssl x509 -req -in leaf.csr -CA ca.pem -CAkey ca-key.pem -CAcreateserial -days 2 -extfile ext.cnf -out leaf.pem`,
          `cat leaf.pem ca.pem > openlaw.pem`,
          `chmod 600 ca-key.pem openlaw-key.pem && chmod 644 openlaw.pem ca.pem`,
        ].join(" && "),
        { cwd: d },
      );
      expect(gen.code === 0, `openssl exited ${gen.code}: ${firstLines(gen.stderr)}`);
      const verify = sh(`openssl verify -CAfile ca.pem leaf.pem`, { cwd: d });
      writeFileSync(
        path.join(FIX, "Caddyfile.private"),
        `{
\tauto_https disable_redirects
\tadmin off
\thttp_port ${PRIVATE_HTTP_PORT}
}
...
`,
      ...
      writeFileSync(
        path.join(FIX, "compose.private-proxy.yml"),
        `services:
  private-proxy:
    image: caddy:2-alpine
    network_mode: host
    volumes:
      ...
      - ${d}/openlaw.pem:/etc/caddy/certs/openlaw.pem:ro
      - ${d}/openlaw-key.pem:/etc/caddy/certs/openlaw-key.pem:ro
`,
      );
      const fxc = (args) =>
        sh(`docker compose -f compose.yml -f compose.private-proxy.yml ${args}`, {
          cwd: FIX,
          timeout: 300_000,
        });
      const v = fxc(
        "run --rm --no-deps private-proxy caddy validate --config /etc/caddy/Caddyfile --adapter caddyfile",
      );
      expect(
        verify.code === 0 && v.code === 0,
        `verify ${verify.code}, validate ${v.code}: ${firstLines(v.stderr.split("\n").slice(-4).join("\n"), 4)}`,
      );
      return `Created a fictional corporate CA and a certificate for ${PRIVATE_HOST} signed by it (openssl verify: "${verify.stdout.trim()}"). Wrote the article's Caddyfile with these adaptations: site address ${PRIVATE_HOST}:${PRIVATE_PORT} instead of port 443, bind ${PRIVATE_BIND} (a private host bridge address standing in for 10.20.30.40), upstream 127.0.0.1:${APP_PORT}, plus admin off and http_port ${PRIVATE_HTTP_PORT} so the fixture does not take host ports 80 and 2019. The tls, header and auto_https disable_redirects lines are as shown. caddy validate exited 0 ("${(`${v.stdout}\n${v.stderr}`.match(/Valid configuration/) ?? ["no confirmation line"])[0]}").`;
    },
  );
};

phases["private-serve"] = async () => {
  await step(
    "V-C45",
    "Private VM: serve HTTPS on the private address with the supplied certificate",
    "A client that trusts the CA gets HTTPS with the configured headers; an untrusting client refuses the certificate; the proxy is not on loopback; the app is reachable only through the proxy from another network namespace.",
    "container-operation",
    async () => {
      const up1 = sh(
        "docker compose -f compose.yml -f compose.private-proxy.yml up -d --pull never private-proxy",
        { cwd: FIX, timeout: 300_000 },
      );
      await sleep(3000);
      const ok = privateCurl(`-o /dev/null -D - https://${PRIVATE_HOST}:${PRIVATE_PORT}/readyz`);
      const status = ok.stdout.match(/^HTTP\/\S+ (\d+)/m)?.[1];
      const headers = [
        "x-frame-options: DENY",
        "referrer-policy: strict-origin-when-cross-origin",
        "x-content-type-options: nosniff",
      ].filter((h) => ok.stdout.toLowerCase().includes(h.toLowerCase()));
      const untrusted = sh(
        `curl -sS --max-time 10 --resolve ${PRIVATE_HOST}:${PRIVATE_PORT}:${PRIVATE_BIND} -o /dev/null https://${PRIVATE_HOST}:${PRIVATE_PORT}/readyz`,
        { cwd: WORK },
      );
      const proxyListen = listeners(PRIVATE_PORT);
      const http = listeners(PRIVATE_HTTP_PORT);
      const lanDirect = sh(
        `curl -sS --max-time 5 -o /dev/null -w '%{http_code}' http://192.168.0.59:${APP_PORT}/readyz`,
        { cwd: WORK },
      );
      const loopProxy = sh(
        `curl -sS --max-time 5 -k -o /dev/null --resolve ${PRIVATE_HOST}:${PRIVATE_PORT}:127.0.0.1 https://${PRIVATE_HOST}:${PRIVATE_PORT}/readyz`,
        { cwd: WORK },
      );
      expect(
        up1.code === 0 && status === "200" && headers.length === 3,
        `up ${up1.code}, status ${status}, headers ${headers}, stderr ${firstLines(ok.stderr)}`,
      );
      expect(untrusted.code !== 0, "a client without the CA accepted the certificate");
      expect(
        proxyListen.length === 1 && proxyListen[0] === `${PRIVATE_BIND}:${PRIVATE_PORT}`,
        `proxy listeners ${proxyListen}`,
      );
      expect(
        lanDirect.code !== 0 && loopProxy.code !== 0,
        `lan direct ${lanDirect.code}/${lanDirect.stdout}, loopback proxy ${loopProxy.code}`,
      );
      return `The private proxy started. curl with only the fictional CA as trust anchor and ${PRIVATE_HOST} resolved to ${PRIVATE_BIND} got HTTP ${status} from /readyz with ${headers.join("; ")}. curl without that CA exited ${untrusted.code} ("${firstLines(untrusted.stderr, 1)}"). The only listener on port ${PRIVATE_PORT} is ${proxyListen.join(", ")}; port ${PRIVATE_HTTP_PORT} listeners: ${http.join(", ") || "none"}. The direct app port on the host's LAN address 192.168.0.59:${APP_PORT} failed (curl exit ${lanDirect.code}), and the proxy did not answer on 127.0.0.1:${PRIVATE_PORT} (curl exit ${loopProxy.code}). A first attempt to test from a separate container on the default bridge network timed out for both the direct port and the working proxy, so that container could not stand in for another device. The cause of the timeout was not investigated.`;
    },
  );
};

phases["private-origin"] = async () => {
  await step(
    "V-C45",
    "Private VM: sign in through the private HTTPS origin; the old origin is no longer accepted; one origin serves the web interface and API",
    "Browser sign-in works at the private origin; sign-in with the previous origin is refused; / returns the web app and /api answers on the same origin.",
    "browser-walkthrough",
    async () => {
      const s = await signIn(PRIVATE_ORIGIN, ADMIN.email, state.adminPassword);
      const me = await json(await s.page.request.get(`${PRIVATE_ORIGIN}/api/v1/me`));
      const title = await s.page.title();
      await s.context.close();
      const old = await apiClient(LOCAL, ADMIN.email, state.adminPassword, ORIGIN);
      await old.ctx.dispose();
      const root = privateCurl(
        `-o /dev/null -w '%{http_code} %{content_type}' https://${PRIVATE_HOST}:${PRIVATE_PORT}/`,
      );
      const sso = privateCurl(
        `-o /dev/null -w '%{http_code} %{content_type}' https://${PRIVATE_HOST}:${PRIVATE_PORT}/api/auth/sso/callback`,
      );
      expect(me.user?.email === ADMIN.email, "no session at the private origin");
      expect(old.signInStatus === 403, `old origin sign-in ${old.signInStatus}`);
      expect(/^200 text\/html/.test(root.stdout), `root ${root.stdout}`);
      return `Signed in as ${ADMIN.name} in the browser at ${PRIVATE_ORIGIN}; /api/v1/me through the private proxy returned the Administrator; page title "${title}". A sign-in request with the previous origin ${ORIGIN} answered ${old.signInStatus}. GET / on the private origin answered "${root.stdout}"; GET /api/auth/sso/callback without an identity provider answered "${sso.stdout}" from the API on the same origin (no OIDC provider was configured, so the SSO return was not tested). No port 5173 service was used.`;
    },
  );
  await step(
    "V-C45",
    "Private VM: new invitation, password-setup and sign-in emails use the private hostname; an earlier email is not rewritten",
    "Invitation, password-setup and sign-in link emails sent after the change link to the private origin; the invitation sent before the change still links to the old origin; the new invitation link sets a password through the private proxy.",
    "browser-walkthrough",
    async () => {
      const oldList = await fetch(
        `${MAIL_UI}/api/v1/search?query=${encodeURIComponent(`to:"${COLLEAGUE.email}"`)}`,
      ).then((x) => x.json());
      const oldest = (oldList.messages ?? []).sort(
        (a, b) => new Date(a.Created) - new Date(b.Created),
      )[0];
      const oldMsg = oldest
        ? await fetch(`${MAIL_UI}/api/v1/message/${oldest.ID}`).then((x) => x.json())
        : null;
      const oldLink = oldMsg ? firstLink(oldMsg) : null;
      const invitee = `doc029-r2-private-${Date.now()}@doc029-install.example`;
      const since = new Date(Date.now() - 1000).toISOString();
      const r = await inviteThroughSettings(
        invitee,
        "DOC-029r2 private VM invitee",
        null,
        PRIVATE_ORIGIN,
      );
      const msg = await waitMail(invitee, null, since, 60_000);
      expect(r.inviteStatus === 201 && msg, `invite ${r.inviteStatus}, mail ${Boolean(msg)}`);
      const link = firstLink(msg);
      secret("privateInviteePassword");
      const { page, context } = await newSession(PRIVATE_ORIGIN);
      try {
        await page.goto(link.toString());
        await page.getByLabel("New password").fill(state.privateInviteePassword);
        await page.getByLabel("Confirm password").fill(state.privateInviteePassword);
        await page.getByRole("button", { name: "Set password" }).click();
        await page.getByText("Password set").first().waitFor({ timeout: 30_000 });
      } finally {
        await context.close();
      }
      const c = await apiClient(PRIVATE_ORIGIN, invitee, state.privateInviteePassword);
      await c.ctx.dispose();
      const probe = await pwRequest.newContext({
        baseURL: PRIVATE_ORIGIN,
        ignoreHTTPSErrors: true,
        extraHTTPHeaders: { origin: PRIVATE_ORIGIN },
      });
      const t1 = new Date(Date.now() - 1000).toISOString();
      const ps = await probe.post("/api/v1/auth/password-setup", { data: { email: ADMIN.email } });
      const psMsg = await waitMail(ADMIN.email, null, t1, 30_000);
      const t2 = new Date().toISOString();
      await sleep(1100);
      const ml = await probe.post("/api/v1/auth/magic-link", { data: { email: COLLEAGUE.email } });
      let mlMsg = null;
      const deadline = Date.now() + 30_000;
      while (Date.now() < deadline && !mlMsg) {
        const m = await waitMail(COLLEAGUE.email, null, t2, 2000);
        if (m && m.ID !== psMsg?.ID) mlMsg = m;
      }
      await probe.dispose();
      const psLink = psMsg ? firstLink(psMsg) : null;
      const mlLink = mlMsg ? firstLink(mlMsg) : null;
      expect(
        link.origin === PRIVATE_ORIGIN && c.signInStatus === 200,
        `invite link ${link.origin}, sign-in ${c.signInStatus}`,
      );
      expect(
        psLink?.origin === PRIVATE_ORIGIN && mlLink?.origin === PRIVATE_ORIGIN,
        `password-setup ${ps.status()} ${psLink?.origin}, magic link ${ml.status()} ${mlLink?.origin}`,
      );
      expect(oldLink?.origin === ORIGIN, `earlier invitation ${oldLink?.origin}`);
      return `Settings -> Users -> Invite user at ${PRIVATE_ORIGIN} answered ${r.inviteStatus}. The invitation "${msg.Subject}" links to ${link.origin} (token not recorded); opening it through the private proxy set a password and the invitee signed in (${c.signInStatus}). A password-setup request for ${ADMIN.name} (${ps.status()}; the earlier attempt for ${COLLEAGUE.name} was refused 429 by the per-address request limit after this run's earlier requests) sent "${psMsg.Subject}" linking to ${psLink.origin}. A sign-in link request (${ml.status()}) sent "${mlMsg.Subject}" linking to ${mlLink.origin}. The invitation sent to ${COLLEAGUE.name} before the change still links to ${oldLink.origin}.`;
    },
  );
  await step(
    "V-C45",
    "Private VM: upload, download, processing and a live update through the private proxy",
    "Bytes match, a PDF reaches ready, and a record frame arrives through the private origin.",
    "container-operation",
    async () => {
      const api = await adminApi(PRIVATE_ORIGIN);
      const ref = await upload(
        api,
        state.contractNumber,
        pdfFile("doc029r2-private.pdf", "uploaded through the private proxy"),
      );
      const d = await download(api, ref);
      const t = await waitText(api, ref);
      await api.dispose();
      expect(
        ref.status === 201 && d.matches && t.state === "ready",
        `upload ${ref.status}, match ${d.matches}, text ${t.state}`,
      );
      const live = await liveUpdate(PRIVATE_ORIGIN);
      return `Uploaded a PDF through ${PRIVATE_ORIGIN} (201); its download matched the SHA-256; text extraction reached "ready". Live update: ${live}`;
    },
  );
};

phases["private-emails"] = async () => {
  await step(
    "V-C45",
    "Private VM: new invitation, password-setup and sign-in emails use the private hostname; an earlier email is not rewritten",
    "Invitation, password-setup and sign-in link emails sent after the change link to the private origin; the invitation sent before the change still links to the old origin; the new invitation link sets a password through the private proxy.",
