...
await step(RA, "legal_team_member", "D5b.tier-labels", "Label check for the article text: Business Users read and post **Full thread** comments and do not see **Legal only** or **Working team**; Legal must choose Full Thread for a message intended for the business. A reader should find those labels on a Contract or Matter.", async () => {
  await go(N, `/contracts/${CO}`);
  const lp = await applet(N, "Comments");
  await lp.getByText("DOC-029 roles Full thread note").waitFor({ timeout: 20000 });
  const audience = (await lp.getByRole("group", { name: "Audience" }).innerText()).replace(/\s+/g, " ");
  ...
  const chipText = items.join("\n").split("\n").filter((line) => !/DOC-029/.test(line)).join(" ");
  ...
  check(found.length === 3, `${observed} The labels Full thread, Legal only and Working team are not shown on Contracts or Matters (found: ${found.join(", ") || "none"}). The visible labels are Contract Team / Matter Team (Full Thread tier), Legal Only, and Internal team (older Working Team comments). See r1-legal-audience-labels.png.`);
  return observed;
});

await step(RA, "business_user", "D6.history-portal-limits", "The History applet shows shared comments and business record changes and excludes Legal Only and Working Team activity. The Portal excludes legal Fields, Tasks, Key dates, Approvals and signing controls.", async () => {
  await go(J, `/portal/contracts/${CO}`);
  ...
  check(!/Legal only note|Working team note/.test(hist) && forbidden.length === 0 && stageButtons === 0, `${hist.slice(0, 200)} ${forbidden}`);
...
await step(RA, "business_user", "D6b.history-business-change", "The History applet shows shared comments and business record changes (a Legal change to a business value such as Region or Description).", async () => {
  await go(N, `/contracts/${CO}`);
  await h1(N);
  ...
  const hist = (await (await applet(J, "History")).innerText()).replace(/\s+/g, " ");
  const api1 = await api(J, "GET", `/portal/activity?entityType=contract&entityId=${fx.openContract.id}`);
...
await step(RA, "business_user", "D9.related-no-reach", "A link or parent relation does not grant a Business User access to the related record.", async () => {
  await go(N, `/matters/${MO}`);
...
await step(CG, "business_user", "G11b.reply-tier-label", "Label check for the article text: your reply appears on the Contract or Matter at **Full thread**.", async () => {
  await go(R, `/portal/matters/${MR}`);
  const panel = await applet(R, "Comments");
  await sleep(1000);
  const text = (await panel.innerText()).replace(/\s+/g, " ");
  await R.screenshot({ path: path.join(here, "r1-portal-reply-tier-label.png") });
  const chips = [...new Set(text.match(/Matter Team|Contract Team|Full thread|Full Thread/g) ?? [])];
  check(/Full thread/.test(text), `Ravi's Portal Comments label each reply "${chips.join(", ")}" and the composer says "${(text.match(/Visible to[^.]*\./) ?? [""])[0]}"; the label Full thread does not appear. See r1-portal-reply-tier-label.png.`);
...
await step(CG, "business_user", "G14.close-archive-remove", "Closing a Matter does not remove Portal work; archiving removes the record from the Portal; removing Ravi's membership stops access.", async () => {
