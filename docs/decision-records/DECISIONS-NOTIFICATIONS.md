...
- **Addendum (2026-08-18, M18/3, [#318](https://github.com/juggernog20/OpenLaw/issues/318))** — **The rest of group 1 fires, and the mention is the one with a rule of its own.** Owner assignment (CTR-004), task assignment (CTR-017), and the comment mention (CMT-007) each became one Notifier call inside the mutation that causes them, joining the approval request that shipped with the engine. Nothing about the machinery moved: the audience, the wall, the actor exclusion, the preferences, and the after-commit wake-up are all still the seam's, and a slice that fires an event now writes a call and a line of email copy.
  ...
  **The mention carries the comment's tier, so DD-016 narrows the audience beside DD-014.** The wall answers "which people does this record reach"; the tier answers "which of them were in the room for what was said". A Legal Only mention therefore reaches nobody the tier excludes, and it is the same predicate asked with one more argument rather than a second rule beside it. The composer's refusal (CMT-007) is the first gate and this is the one no later call site can forget — the wall's own posture, applied to the second boundary.
  ...
  ~~**No mention email carries the comment.** It says who named you, on which record, and where to go. The tier is enforced on the thread and a redact (CMT-006) cannot reach mail that has already left, so the words stay where both of those still hold.~~ **Superseded by the 2026-09-24 addendum under this decision:** mention, comment and reply emails carry the comment's words, read from the comment at send time.
...
- **Addendum (2026-08-18, M18/8, [#323](https://github.com/juggernog20/OpenLaw/issues/323))** — **The tier gates at write time; the wall gates continuously.** M18/3 recorded that DD-016's comment tier narrows a mention's audience beside DD-014's wall. What it did not say is that the two are re-asked on different schedules, and the asymmetry is deliberate rather than a gap.
  ...
  That is coherent because of what each message carries. ~~**No mention email carries the comment's words** (M18/3), and the bell item carries none either — both say who named you and where to go.~~ (Amended by the 2026-09-24 addendum. The email now carries the words and the bell item still does not. The agreement below still holds, because a comment's tier cannot change after posting (CMT-005), so a tier check at send time would give the same answer as the one at write time.) The read side re-applies only the wall, so the row that person still holds and the mail they receive agree with each other; a tier re-check on one and not the other is what would produce a disagreement. A tier that gated continuously would also mean a bell item vanishing because a thread was re-tiered, which is a fact about the conversation and not about the reader's reach.
...
- **Addendum (2026-09-19)** — **A Conversion draft its actor stopped watching joins group 4 as `request.conversion_draft_finished`.** The INT-008 addendum of the same date lets a person close the Convert dialog while preparation runs, and the bell is how they hear it finished. The event is placed in group 4, New requests, rather than group 1: it is Inbox work the reader asked for, the dialog is usually still open when it lands, and an email for every draft would be noise. So the bell is on and the email is opt-in through the group's existing switch, and the audience is the draft's actor alone, because a draft is actor-scoped and nobody else can open it. It carries no actor: the worker finished it, and the person who asked is exactly who the item is for. Its payload snapshots the outcome, the target module and the draft id beside the usual Request number and Title. The bell item and the email address the Inbox detail with a `convert` query that reopens the dialog. `REQUEST_SIDE_BY_GROUP` already names group 4 as the inbox side, so the staff bell scope and the reach check inherited the event without a line of their own, which is the catalog working as designed.

### Addendum, 2026-09-24: comment emails carry the comment's words

This reverses the rule in the M18/3, M18/8 and M21/5 addenda that no email carries a
comment's words. It came out of the email redesign. An email that only says "Priya Shah
mentioned you" makes the reader open the app to read one sentence.

**Which emails.** These emails now carry the words of the comment they are about:

- `comment.mentioned`, on every arm: contract, Matter, and the staff side of a Request.
- `comment.posted`, in group 2.
- `request.replied`, in group 5.

No other event changes. The bell item and Push still carry no words. Push keeps the
NOT-010 record-name privacy, because a lock screen is more public than an inbox.

**Every tier is included.** The fan-out already decides who gets the email. DD-016's tier
narrows the audience when the notification is written (M18/3, M18/4). So every
...
this addendum. The tier needs no check at send time, because CMT-005 makes it
immutable. The wall is still checked at send time, as the M18/8 addendum records.

**A redact cannot reach an email that has already gone out.** This is the one copy of
a comment's words that OpenLaw does not control. We accept it. The same is true for
every tier, so it is not a reason to leave any tier out.

**Long comments are cut.** A body over 280 characters is cut at the last word break
before that limit and ends with "…". A "Read the full comment" link opens the thread. An
@-mention stays as the author typed it (CMT-007). The HTML part may mark the reader's
own name.

**No files.** A comment's attachments (CMT-011) do not go in the email, and the email
does not name them.

**The plain-text part carries the same words**, as a quote under the first sentence. A
reader whose client shows plain text gets the same news.

**An Administrator can turn the words off.** One switch in Settings → Organization →
Notifications turns comment words off for the whole org. It is on by default. With it
off, these emails go out in the form they had before this addendum. The switch applies
to all tiers and to everyone. It has no per-tier or per-person version.
...
- **Status** — Accepted
- **Date** — 2026-08-05
- **Decision** — A single global offset list (Settings → Notifications), seeded `7 days / 1 day / day-of`, applied to every tracked date (key dates, notice deadlines, expiries). Admin-tunable; the original exclusion of per-date lead times is superseded by the 2026-09-09 and 2026-09-10 addenda below. ~~No per-user list.~~ Superseded by the 2026-09-24 addendum below: the list is the organization's default, and a person may set their own. CTR-006's mandated fires are dates within this scheme.
...
### NOT-004 addendum: a person may set their own lead times (2026-09-24)

Blair asked for reminder lead times to be a user setting, not only an organization one.

- **The organization's list is the default.** The Administrator still edits it on Settings → Reminder lead times. It applies to everyone who has not set their own list.
- **A person may replace it for themselves.** Settings → Notifications has a Reminder lead times card. It shows the list that applies to the person. When they turn off "Use the organization's default lead times", the card copies the organization's list as a starting point, and they can add and remove lead times from there. Turning the switch back on clears their list.
- **Their list replaces the default. It does not add to it.** A Key date's own lead times still add to whichever list applies to each recipient (the 2026-09-10 addendum).
...
### Addendum (2026-09-24, M43/1, [#1083](https://github.com/juggernog20/OpenLaw/issues/1083)): the briefing uses A2 · Console+ HTML

The HTML part uses **DES-093**'s shared layout. Count tiles precede the sections, three to a row, with a 20px semibold number over a 12px section label. Each count is the section's total eligible count, including rows omitted by its existing cap.

Each section has a head strip with its name and a count badge. When a section has more eligible rows than the email shows, a right-aligned "View all {n} →" link in that head replaces the HTML's "And N more on Home." line. `{n}` is the total count, not the omitted count. The link opens Home, the same destination as the existing overflow link. The text part keeps "And N more on Home." and its link.

Rows have dark linked titles, unbroken monospace refs, metadata and right-aligned due dates in their tone colour. Date rows have a 100px "when / date" column. DES-093 fixes the dimensions and light-theme tokens for the tiles, sections, why strip and sender line.

The section set and order remain Approvals, Tasks, Dates, Obligations, Knowledge, then Intake. Empty sections stay omitted, and no email is sent if every section is empty. The M29 section caps, subjects, row links, date order and distance, the M31 Unverified marker and the delivery rules remain unchanged. Both parts keep DES-051's register.

The source is `renderConsolePlus` in `render.js` and the sample content in `data.js` at `~/.cache/openlaw-mocks/email/`. That gallery stays outside the repo. [M43 #1080](https://github.com/juggernog20/OpenLaw/issues/1080) carries the spec.
...
| NOT-002 | Event catalog: five groups, defaults by interruptiveness                 | Accepted; group 5's four events added by M20/8 addendum; group 4's first event and its opt-in email by the M21/4 addendum; group 1's `request` arm by the M21/5 addendum; the reply promise following a conversion onto the record by the M21/11 addendum; the side default and the archived-Request arm by the M21/12 addendum; the finished Conversion draft joins group 4 by the 2026-09-19 addendum; mention, comment and reply emails carry the comment's words by the 2026-09-24 addendum |
...
| NOT-004 | Reminder lead times: admin-configurable offsets, seeded 7/1/0            | Accepted; per-date lead times and recipients added by the 2026-09-09 focus-group addendum; per-person lists added 2026-09-24                                                                                                                                                                                                                                                                                                                                                                    |
...
| NOT-006 | The morning digest's anatomy and its delivery rules                      | Accepted; M29 six-section anatomy; M43/1 HTML tiles and section-head overflow link under DES-093                                                                                                                                                                                                                                                                                                                                                                                                |
